...
Dans certaines bibliothèques de calcul
...
\begin{theorem}[Théorème fondamental de la simulation]
...
\begin{theorem}[Simulation de la loi exponentielle]\label{th:simexp}
...
On pourra consulter \cite[IV.4.3, p.79]{bouleau} ou encore \cite[VI.5.15, p. 180]{barbe-ledoux} pour des preuves.
...
\begin{theorem}[Simulation d'une loi discrète à support fini]\label{th:simdiscr}
...
\begin{theorem}[Simulation de la loi binomiale]
...
\begin{theorem}[Simulation de la loi de Poisson]
...
\begin{theorem}[Simulation d'une loi normale]
...
\begin{theorem}[Glivenko-Cantelli]\label{th:gc}
...
\begin{theorem}[Théorème de Moivre-Laplace]\label{th:moivre-laplace}
...
\begin{theorem}[Principe d'invariance de Donsker-Prohorov]\label{th:pid}
...
\begin{theorem}[Théorème de Moivre-Laplace «fort»]\label{th:moivre-laplace-fort}
...
\begin{theorem}[Théorème de Sanov]
...
%% Local Variables: 
%% x-symbol-coding: iso-8859-1
%% x-se Lebesgue $dx$ sur $\dR$~:
  $$
  \frac{1}{2\pi\si^2}\,\sqrt{4\si^%
%
...
%DCD section 4.4, pages 101 et suivantes. 
% Â«Ãchantillon d'une loi et estimation de cette loiÂ»

La statistique paramétrique consiste Ã  considérer qu'un phénomÃ¨ne aléatoire
obéit Ã  une loi de probabilité qui appartient Ã  une famille de loi indexée par
un paramÃ¨tre. Le choix de la famille constitue la Â«modélisationÂ». La véritable
valeur du paramÃ¨tre est inconnue est l'on cherche Ã  la déterminer Ã  partir
d'un Â«échantillonÂ», c'est Ã  dire d'un suite finie de réalisations i.i.d. qui
suivent la loi inconnue cherchée. Les intervalles ou parallélépipÃ¨des de
confiance consistent alors en des Â«morceauxÂ» de l'ensemble des paramÃ¨tres qui
contiennent la Â«véritable valeurÂ» avec au moins une certaine probabilité $p$,
appelée niveau de confiance.  On parle de Â«risqueÂ» pour désigner la
probabilité $1-p$. Pour estimer le paramÃ¨tre, on construit une fonction de
l'échantillon, appelée Â«estimateurÂ», qui convergence, en un certain sens, vers
la valeur du paramÃ¨tre.
...
Par exemple, pour le modÃ¨le $\cE=\dR$ et
...
On modélise l'intention de vote des électeurs Ã  un référendum par $0$ pour
...
%FIXME: vocabulaire : statistique de test = fonction des données, Ã  regarder
...
Dans le cadre de la statistique paramétrique, un test consiste Ã  confronter
...
Considérons un phénomÃ¨ne aléatoire de loi inconnue $P=p_1\delta_1+\cdots+p_k\delta_k$ oÃ¹
...
\begin{exo}[Cas oÃ¹ l'asymptotique du $\chi^2$ ne s'applique pas]
...
\subsubsection{Tes celle de Kolmogorov-Smirnov pour $n$ assez grand, $n \geq 80$
par exemple. On peut alors utiliser les quantiles ÃÂ  $0.95$ et $0.99$
correspondants ÃÂ  $u=1.3581$ et $u=1.629$, pour obtenir un test de risque
$0.05$ et $0.01$ sur l'adÃÂ©quation de la loi de l'ÃÂ©chantillon ÃÂ  $\mu$. Pour $n$
petit, on peut utiliser des tables, cf. \cite{saporta}.  La bibliothÃÂ¨que \SB{}
founit la fonction \texttt{pks} qui donne la valeur de la fonction de
rÃÂ©partition de Kolmogorov-Smirnov et la fonction \texttt{kstwo} qui permet de
tester l'homogÃÂ©nÃÂ©itÃÂ© de deux lois continues. Il est trÃÂ¨s important de se
souvenir que la convergence vers la loi de Kolmogorov-Smirnov n'est garantie
que lorsque la loi $\mu$ n'a pas d'atomes.

\begin{exo}[Test d'adÃÂ©quation pour les gÃÂ©nÃÂ©rateurs alÃÂ©atoires]
  Voici deux petits programmes \ML{} qui font appel au test de
  Kolmogorov-Smirnov pour tester les gÃÂ©nÃÂ©rateurs alÃÂ©atoires gaussiens. Ils
  sont aisÃÂ©ment adaptables aux autres gÃÂ©nÃÂ©rateurs alÃÂ©atoires de distributions
  continues.
\end{exo}
...
La loi hypergéométrique de paramÃ¨tres $(N,N_1,n)\in\eN^*Ã\eN^*Ã\eN^*$
...
$$
%FIXME: dire que l'expo est sans mémoire ou au moins renvoyer Ã  une page
...
Soit $\mun-2)n^{-1}(m-4)^{-1}(m-2)^{-2}$.

%
\subsection{Loi de Student}\label{ss:loi:student}
%

La loi de Student $\cT_n$ ÃÂ  $n$ degrÃÂ©s de libertÃÂ© est la loi de
$$
\frac{\sqrt{n}\,X}{\sqrt{Y}},
$$
oÃÂ¹ $X$ et et $Y$ sont indÃÂ©pendantes et ont pour lois respectives $\cN(0,1)$
et $\chi^2(n)$. Elle a pour densitÃÂ© par rapport ÃÂ  la mesure de Lebesgue $dx$ sur
$\dR$~:
$$
\frac{\Ga((n+1)/2)}{\Ga(n/2)\sqrt{n\pi}}\,\PAR{1+\frac{x^2}{n}}^{-(n+1)/2}.
$$
Elle est centrÃÂ©e et sa variance vaut $n/(n-2)$ pour $n>2$.

%
\subsection{Loi de Kolmogorov-Smirnov}\label{ss:loi:kolmogorov-smirnov}
%

C'est la loi sur $\dR$ de fonction de rÃÂ©partition~:
$$
\bF(u):=
\begin{cases}
  0 & \text{si } u\leq0,\\
...
\end{cases}
$$
Elle intervient dans le test d'adÃÂ©quation de Kolmogorov-Smirnov (cf.
\ref{ss:test-ks} page \pageref{ss:test-ks}). C'est aussi la loi sur supremum
de la valeur absolue d'un pont brownien sur $[0,1]$ (cf. section
\ref{se:vitesse-tgc} page \pageref{se:vitesse-tgc}).

%
\subsection{Statistique d'ordre, quantiles, mÃÂ©diane}\label{ss:loi:statordre}
%

%FIXME: ÃÂ  complÃÂ©ter.

Soit $\mu%
%
\chapter{Informations utiles (et volatiles)}
%
%
...
% \url{http://www.math.u-psud.fr/~agreg/AgregSoft/WEB/logiciels.html} 
% & Logiciels distribués pour l'oral\\ \hline
...
% \url{http://users.powernet.co.uk/kienzle/octave/}
% & Compatibilité \ML{}/\OC{}\\ \hline
% \url{http://www.math.keio.ac.jp/~matumoto/emt.html}
% & The Mersenne Twister\\ \hline
\end{tabular}
\end{sideways}
\end{center}
...
«\emph{La présentation du travail sur une machine devant le jury se fera à
...
la communauté mathématique. Cette crise d'adolescence fait désormais partie du
...
Quant au mot \emph{algorithme}, il vient du nom du mathématicien Abu Ja'far
...
Les noms de fichiers \texttt{HTML} indiqués sous chaque illustration
...
\includegraphics[scale=0.4]{al-khawarizmi}\\
Abu Ja'far Muhammad ibn Musa \textsc{Al-Khwarizmi}\\ 
\url{Al-Khwarizmi.html}

\bigskip

\includegraphics[scale=0.4]{pascal}\\
Blaise \textsc{Pascal}\\
\url{Pascal.html}
...
\includegraphics[scale=0.5]{wiener}\\
Norbert \textsc{Wiener}\\
\url{Wiener_Norbert.html}

\bigskip

\includegraphics[scale=0.5]{turing}\\
Alan Mathison \textsc{Turing}\\
\url{Turing.html}

\bigskip

\includegraphics[scale=0.5]{shannon}\\
Claude Elwood \textsc{Shannon}\\
\url{Shannon.html}

\bigskip

\includegraphics[scale=0.5]{kolmogorov}\\
Andrey Nikolaevich \textsc{Kolmogorov}\\ 
\url{Kolmogorov.html}

\bigskip

\includegraphics[scale=0.5]{levy}\\
Paul \textsc{Lévy}\\
\url{Levy_Paul.html}

\bigskip

\includegraphics[scale=0.5]{doob}\\
Joseph Leo \textsc{Doob}\\
\url{Doob.html}

\bigskip 

\includegraphics[scale=0.35]{von-neumann-eniac}\\
John \textsc{von Neumann} aux côtés de l'ENIAC.\\
\url{Von_Neumann.html}
