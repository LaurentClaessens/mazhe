...
Soit $n\in\dN^*$ une base de numération.  Chaque résultat d'un schéma de
...
méthode décrite pour une matrice de transition $3× 3$. Il fait appel à la
...
Soit $\PAR{X_n,n\in\dN^*}$ une suite de v.a.r. i.i.d. de loi normale et
...
Soient $(U_n,n\in\dN^*)$ une suite de v.a. i.i.d. suivant une loi uniforme sur
...
triplet $(\cE,\cA,(\dP_\te)_{\te\in\Te})$. En d'autres termes, on se donne une
famille de lois de probabilité $\{\dP_\te,\te\in\Te\}$ sur l'espace mesurable
$(\cE,\dA)$ indexée par un ouvert $\Te$ de $\dR^k$. L'exemple le plus
classique est donné par $\cE=\dR$ et
$$
(\dP_\te)_{\te\in\Te}:=\BRA{\cN(m,\si^2),\,(m,\si^2)\in\dR×\dR^+}.
$$
En voici un autre, encore plus simple : $\cE=\{0,1\}$ et
$$
(\dP_\te)_{\te\in\Te}:=\BRA{{\cB}ern(\{0,1\},p),\,p\in[0,1]}.
$$
...
$\dP_{\te_0}$ qui appartient à la famille $\{\dP_\te,\te\in\Te\}$. On parle
...
donc à un jeu de données possible. Le terme «échantillon» de loi $\dP_{\te_0}$
...
$$
f_n:(x_1,\ldots,x_n)\in\dE^n\mapsto f_n(x_1,\ldots,x_n)\in\dE
$$
est appelée \emph{estimateur} du paramètre $\te$. Le \emph{biais} d'un
estimateur est par définition l'écart
$\moy{\dP_{\te_0}}{f_n}-\te:=\moy{}{Y_n}-\te$ entre sa moyenne et la véritable
...
Considérons un phénomène aléatoire de loi inconnue $P=p_1\de_1+\cdots+p_k\de_k$ où
...
phénomène aléatoire est $Q=q_1\de_1+\cdots+q_k\de_k$, où les $q_k$ sont tous non
...
$X:(\Om,\cA,\dP)\to\dR^n$ tel que pour tout $\al\in\dR^n$, la variable
...
vecteur moyenne $m:=(\dE(X_1),\ldots,\dE(X_n))\in\dR^n$ et sa matrice de
covariance $\Ga\in\cS_n^+$ définie par
$$
\Ga:=\PAR{\dE[(X_i-m_i)(X_j-m_j)]}_{1\leq i,j \leq n}
    = \PAR{\dE(X_iX_j)-m_im_j}_{1\leq i,j \leq n}.
$$
...
orthogonales dans $\bL^2(\Om,\cA,\dP)$, c'est-à-dire que pour tous $1\leq i,j
\leq n$, on a $\dE(X_iX_j)=0$. 
...
Réciproquement, donnons nous une matrice infinie $(\bL_{i,j},(i,j)\in\dN^2)$,
...
\item On considère une machine qui peut tomber en panne pendant un
  ...
  \item Montrer que~:
    $$
    \lim_{t\to+\infty}
    \dP\PAR{t \in \bigcup_{n \in\dN}[T_n,S_{n+1}[}=\frac{m_X}{m_X+m_Y}.
    $$
  \end{enumerate}  
...
C'est la loi donnée par~: $\PAR{\de_{a_1}+\cdots+\de_{a_n}}/n$, où les $a_i$ sont
...
C'est la loi $q\de_a+p\de_b$ où $p\in[0,1]$, $q:=1-p$ et $a\neq b$. Le plus
...
\begin{rem}[Schéma de Bernoulli]\label{ss:loi:schema-bernoulli}
  ...
  $\BRA{a_1}×\cdots×\BRA{a_n}×\BRA{0,1}×\BRA{0,1}× \cdots$. La plupart des questions
...
La loi hypergéométrique de paramètres $(N,N_1,n)\in\dN^*×\dN^*×\dN^*$
...
$$
Tout ceci se généralise sans difficultés à un pavé $[a_1,b_1]×\cdots×[a_n,b_n]$ de
$\dR^n$.
...
La loi gamma $\Ga(a,\la)$ de paramètres $(a,\la)\in\dR_+^*×\dR_+^*$ a pour
...
La loi de Weibull $W(\alpha,\lambda)$ de paramètres $(\alpha,\lambda)\in\dR_+^*×\dR_+^*$ est la loi sur
...
La loi bêta de paramètres $(a,b)\in\dR_+^*×\dR_+^*$ est la loi sur $\dR$ de
...
\item Si $\cL((X_n,n\in\dN^*))=\cE(\la)^{\dN^*}$ et
$$
N_t:=\sum_{n=1}^{+\infty}\,\rI_{X_1+\cdots+X_n \leq t}=\inf\BRA{n\in\dN,\,X_1+\cdots+X_n>t},
$$
...
\item $X_n\limP{n}X$
  \begin{enumerate}
  \item ssi $\PAR{\forall\veps>0}\;\;\dP\PAR{\BRA{\ABS{X_n-X}\geq\veps}}\limn{n}0$
  \item ssi $\moy{}{\ABS{X_n-X}\land 1}\limn{n}0$
  \item ssi $\moy{}{\ABS{X_n-X}/(1+\ABS{X_n-X})}\limn{n}0$
  \item ssi de toute sous suite $\PAR{X_{n_k},k\in\dN^*}$, on peut
...
\item $\mu_n\limE{n}\mu$ \quad (avec $\mu_n$ et $\mu$ probas)
  \begin{enumerate}
  \item ssi $\PAR{\forall f\in\cC_b}\;\; \moy{\mu_n}{f}\limn{n}\moy{\mu}{f}$
  \item ssi $\PAR{\forall s\in\dR}\;\; 
    \moy{\mu_n}{e^{is\bullet}}\limn{n}\moy{\mu}{e^{is\bullet}}$
...
\item La famille de v.a. $\PAR{X_i,i\in I}\subset\bL^1$ est uniformément intégrable (UI)
  \begin{enumerate}
  \item ssi $\sup_{i{\in}I}\moy{}{\ABS{X_i}\rI_{\BRA{\ABS{X_i}>c}}}\limn{c}0$
  \item ssi\footnote{Critère «epsilon-delta» ou encore d'équi-intégrabilité.}
    $\sup_{i\in{I}}\moy{}{\ABS{X_i}}<+\infty$ et $\PAR{\forall\veps>0}\PAR{\exists\delta>0}\PAR{\forall
      A}
    \PAR{\dP(A)\leq \delta \Rightarrow \sup_{i\in I}\moy{}{\ABS{X_i}\rI_A}\leq\veps}$
  \item ssi\footnote{Critère de Lavallée-Poussin. cf. \cite[p. 10]{borkar}} $\exists
    \Psi:\dR_+\to\dR_+$ telle que $\sup_{i\in{I}}\moy{}{\Psi\PAR{\ABS{X_i}}}<+\infty$ et
    $\Psi(x)/x\limn{x}+\infty$.
  \end{enumerate}
\item Une famille bornée dans $\bL^1$ est UI.
\item Soit $\PAR{X_n,n\in\dN^*}\subset\bL^1$, il y a équivalence entre~:
  \begin{enumerate}
  \item $X_n\limLeb{n}{1}X$ avec $X\in\bL^1$
  \item $\PAR{X_n,n\in\dN^*}$ est UI et $X_n\limP{n}X$
  \end{enumerate}
\item Si $X_n\limLeb{n}{p}X$ avec $p\geq 1$, alors de toute sous-suite
  $\PAR{X_{n_k},k\in\dN^*}$, on peut extraire une sous-suite qui converge p.s.
  vers $X$
\item Si $X_n\limL{n}\mu$ alors\footnote{Théorème de continuité de Paul Lévy,
    cf. \cite[p. 32]{borkar}.}  $s\mapsto\moy{}{e^{isX_n}}$ converge vers
  $s\mapsto\moy{\mu}{e^{is\bullet}}$ uniformément sur tout compact
\item Soit $\PAR{\mu_n,n\in\dN^*}$ des probas. Si
  $s\mapsto\moy{\mu_n}{e^{is\bullet}}$ converge simplement vers $s\mapsto G(s)$
  ...
  $\mu_n\limE{n}\mu$.
\item Soit $\PAR{X_n,n\in\dN^*}$ v.a.r. indép. et $S_n:=X_1+\cdots+X_n$. Il y a
  équivalence entre\footnote{Théorème de Paul Lévy, cf. \cite[p.
    83]{borkar}}~:
  \begin{enumerate}
  \item $\PAR{S_n,n\in\dN^*}$ converge p.s.
  \item $\PAR{S_n,n\in\dN^*}$ converge en $\dP$
  \item $\PAR{S_n,n\in\dN^*}$ converge en $\cL$
  \end{enumerate}
  ...
  $\PAR{S_n,n\in\dN^*}$ converge p.s. ssi on a à la fois~:
 \begin{enumerate}
 \item $\sum_{n=1}^{+\infty} \dP\PAR{\BRA{X_n\neq Y_n}}<+\infty$
 \item $\sum_{n=1}^{+\infty} \moy{}{Y_n}<+\infty$
 \item $\sum_{n=1}^{+\infty} \var{}{Y_n}<+\infty$ 
 \end{enumerate}
...
\begin{table}[htbp]
  \begin{center}
    \begin{tabular}[c]{|l|l|}\hline
      ...
      Discrète $\sum_{n\geq 0} p_n\de_{a_n}$ ($a_n<a_{n+1}$)
...
\begin{figure}[htbp] 
 ...
 $$
 $$
 \text{CV } \dP \underset{\text{sous suites}}{\Longrightarrow}\text{CV p.s.}
 $$
 $$
 \text{CV } \dP \underset{\text{si UI}}{\Longrightarrow}\text{CV } \bL^1
 $$
