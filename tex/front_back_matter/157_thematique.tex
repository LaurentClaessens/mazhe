...
Le lien entre matrice et application linéaire est donné par la définition \ref{PROPooGXDBooHfKRrv} :
\begin{equation}
...
\end{equation}
ou encore, pour les composantes (équation \ref{EQooAXRJooUwHbjB}),
\begin{equation}       
    T(x)_{\alpha}=\sum_{i}T_{\alpha i}x_i.
\end{equation}
Lorsque nous avons une base orthonormée nous avons aussi la proposition \ref{PROPooZKWXooWmEzoA} :
\begin{equation}       
    T_{\alpha i}=e_{\alpha}\cdot T(e_i)
\end{equation}
où le point est le produit scalaire usuel de \( \eR^n\).

Tant que nous y sommes, nous avons aussi la formule suivante dans \( \eR^n\) muni du produit scalaire usuel (proposition \ref{PROPooZKWXooWmEzoA}) :
\begin{equation}
...
\end{equation}
pour certains éléments \( Q_{i\alpha}\in \eK\). La transformation inverse est donnée par le lemme \ref{LEMooIHZGooOZoYZd} :
\begin{equation}    
...
Voir la proposition \ref{PROPooNYYOooHqHryX}.
