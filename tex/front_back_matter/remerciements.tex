...
\begin{enumerate}
    ...
    \item
        Benôit Tran pour avoir signalé un paquet de typos dans la démonstration de l'ellipsoïde de John-Loewner et ses dépendances.
\end{enumerate}
...
\begin{enumerate}
    \item
        Pour donner un peu de consistance à l'exemple \ref{ExluqIkE}, il faudrait décrire les éléments irréductibles de \( \eZ[i\sqrt{ 2 }]\).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item
        Extension des scalaires. Si \( \{ e_i \}_i\) est une base du \( \eK\)-espace vectoriel \( E\), alors \( \{ 1\otimes e_i \}\) est une base de \( E_{\eL}=\eL\otimes_{\eL}E\) où \( \eL\) est une extension de \( \eK\). C'est vrai ? Que penser de la preuve donné au lemme~\ref{PROPooMHARooUycAts} ?
    \item
...
\begin{enumerate}
    \item
        Comment mettre \( A[X]\) dans le titre d'un \info{enumerate} ?
        
        Mon environnement \info{subproof} est un enumerate, et je voudrais parler de temps en temps de \( A[X]\) dans ses titres, mais \LaTeX\ plante à cause du crochet fermant. Voir la démonstration du lemme~\ref{LEMooDJSUooJWyxCL}. Pour obtenir l'effet, j'ai créé un \info{newcommand} de \info{\textbackslash foo} qui fait \( A[X]\).

        Il y a aussi ce problème pour le titre de l'exemple \ref{ExluqIkE}.
    \item
