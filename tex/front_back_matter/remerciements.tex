...
\begin{enumerate}
    ...
    \item
        Guillaume Barriere pour sa relecture attentive jusqu'aux corps (au moment de mettre sous presse, il continue).
    \item
...
\begin{enumerate}
    ...
    \item
        Pour démontrer qu'une série entière est de classe \(  C^{\infty}\) dans son disque de convergence, on s'appuie sur la proposition~\ref{PropSNMEooVgNqBP} qui donne la différentiabilité et ensuite sur le corollaire~\ref{CorCBYHooQhgara} qui effectue une périeuse récurrence. Toutes ces preuves sont de moi. À relire.

    \item

        Faire le lien entre les fonctions trigonométriques (définies par leur série) et les relations dans un triangle serait une bonne affaire. Indice : la relation entre les fonctions trigonométriques et les rotations de \( \eR^2\) est dans le lemme~\ref{LEMooHRESooQTrpMz} qui a besoin de quelques identités trigonométriques, données dans le lemme~\ref{LEMooJAWBooJGfZIL}.
    ...
    \item
        Que penser de l'exemple~\ref{EXooSWCHooTdHTsl} qui explique pourquoi non seulement
        \begin{equation}
            \int_{\mathopen[ 0 , \infty \mathclose[} e^{i\xi x}\frac{1}{ x }dx
        \end{equation}
        n'existe pas, mais en plus a toutes ses composantes qui divergent ?
\end{enumerate}
