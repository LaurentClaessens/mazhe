...
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooHMSCooRIjcJq}
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \begin{equation}
                \tribA'=\varphi\big( \Lebesgue_{S^1} \big)
            \end{equation}
            où \( \Lebesgue_{S^1}\) est la tribu induite\footnote{Définition \ref{DefDHTTooWNoKDP}.} sur \( \mathopen[ 0 , 2\pi \mathclose[\) depuis la tribu de Lebesgue de \( \eR\), c'est à dire
                \begin{equation}
                    \Lebesgue_{S^1}=\{ M\cap S^1\tq M\in\Lebesgue(\eC) \}.
                \end{equation}
            \item
                La formule
                \begin{equation}
