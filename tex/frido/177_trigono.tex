...
Nous sommes en droit de connaître une valeur approchée de \( \pi\).
...
\begin{proof}
	...
	\end{equation}
	Il existe donc une fonction \( n\colon I\to \eN\) telle que \( \theta(t)=\alpha(t)+2n(t)\pi\). Par continuité de \( \theta\) et \( \alpha\), la fonction \( n\) doit être constante, mais puisque \( \theta(t_0)=\alpha(t_0)\) nous avons \( n=1\).
\end{proof}
...
\begin{definition}
	...
	\end{equation}
	où \( \sin\) et \( \cos\) sont les fonctions de la définition~\ref{PROPooZXPVooBjONka}.
\end{definition}
...
Dans le plan de Gauss, le module d'un complexe $z$ représente la distance entre $0$ et $z$. On appelle \Defn{argument} de $z$ (noté $\arg z$) l'angle (déterminé à $2\pi$ près) entre le demi-axe des réels positifs et la demi-droite qui part de $0$ et passe par $z$. Le module et l'argument d'un complexe permettent de déterminer de façon univoque ce complexe, puisqu'on a la formule
...
\begin{itemize}
	\item
%TODO: S^2  ?? R2 ou C plutot ? ou alors mettre une référence à l'ensemble considéré...
	      ...
	      Le fait qu'il manque un point est sans importance parce que nous n'allons considérer que la mesure de Lebesgue ou des variations simples autour de la mesure de Lebesgue.
	\item
...
\begin{proof}
	...
	\begin{subproof}
		...
		Nous avons obligatoirement \( x_1,x_2\in\mathopen[ 0 , \frac{ \pi }{ 2 } \mathclose[\). Si \( \sin(x_1)=\sin(x_2)\), il existe par le théorème de Rolle~\ref{ThoRolle} un élément \( c\in \mathopen] x_1 , x_2 \mathclose[\) tel que \( \sin'(c)=0\), c'est-à-dire \( \cos(c)=0\). Cela contredirait la proposition~\ref{PROPooMWMDooJYIlis}\ref{ITEMooQKPKooEPeHER} à moins que \( x_1=x_2\).

		\item[Surjectif]
		Soient \( x,y\) tels que \( x^2+y^2=1\). Supposons pour varier les plaisirs que \( x<0\) et \( y>0\). Puisque la fonction \( \cos\) va de \( 0\) à \( -1\) lorsque \( x\) va de \( \pi/2\) à \( \pi\), le théorème des valeurs intermédiaires donne \( t\in\mathopen[ \pi/2 , \pi \mathclose]\) tel que \( \cos(t)=x\). Pour cette valeur de \( x\), nous avons
		\begin{equation}
			\cos^2(x)+\sin^2(x)=1,
		\end{equation}
...
\begin{proof}
	...
	Comme \( \gamma_{t_0}=\gamma\circ f\), nous reconnaissons que \( \gamma_{t_0}\) est une bijection continue.
\end{proof}
...
%TODO: Definition de 'du premier, deuxième type' ?
