...
\begin{corollary}       \label{CORooAKMKooORqcrO}
	Soient \( R>0\) ainsi que le cercle \( S=\{ x\in \eR^2\tq \| x \|=1 \}\) de rayon \( R\). L'application
	\begin{equation}
		\begin{aligned}
...
Nous verrons plus tard (proposition \ref{PROPooDMSTooEOFExj}) que la longueur de l'arc de cercle intercepté par un angle $\theta$ est égal à $\theta$. Les radians sont donc l'unité d'angle les plus adaptés au calcul de longueurs sur le cercle.
...
\begin{proof}
	...
	Vu que \( \gamma_{t_0}=\gamma\circ f\), nous avons que \( \gamma_{t_0}\) est une bijection continue.
\end{proof}


\begin{normaltext}
    La proposition suivante peut se résumer par l'énoncé classique : la longueur d'arc de cercle de rayon \( R\) interceptée par un angle \( \theta\) vaut \( R\theta\).
\end{normaltext}
