...
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooHMSCooRIjcJq}
    Soit la structure usuelle d'espace mesurable \( (\eC,\Lebesgue(\eC))\). Nous posons
    \begin{itemize}
        \item la tribu \( \Lebesgue(\eC)_{S^1}\) induite de la tribu de Lebesgue de \( \eC\) vers \( S^1\),
        \item la bijection \( \varphi\colon \mathopen[ 0 , 2\pi \mathclose[\to S^1\),
            \item la mesure sur \( (S^1,\Lebesgue(\eC)_{S^1})\) donné par
            \begin{equation}
    ...
    \begin{enumerate}
        \item       \label{ITEMooSUNEooRhAdep}
            ...
            \begin{equation}
                \Lebesgue(\eC)_{S^1}=\{A\in\Lebesgue(\eC)\tq A\subset S^1\}=\{A\cap S^1\tq A\in\Lebesgue(\eC)\}
            \end{equation}
        \item
            le triple \( (S^1,\Lebesgue(\eC)_{S^1},\mu')\) est un espace mesuré fini tel que
            \begin{equation}
                \mu'(S^1)=1.
            \end{equation}
        \item
            la tribu \( \Lebesgue(\eC)_{S^1}\) est donnée par 
            \begin{equation}
                \Lebesgue(\eC)_{S^1}=\varphi\big( \Lebesgue(\eR)_{\mathopen[ 0 , 2\pi \mathclose[} \big)
            \end{equation}
            \item
                La formule
                \begin{equation}
                ...
                \end{equation}
                est un produit scalaire sur \( L^2(S^1,\Lebesgue(\eC)_{S^1},\mu')\).
        \item
            Nous avons une bijection isométrique (isomorphisme d'espaces de Hilbert)
            \begin{equation}
            L^2(S^1,\Lebesgue(\eC)_{S^1},\mu')=L^2\big( \mathopen] 0  , 2\pi \mathclose[,\Lebesgue(\eR),\lambda \big)
            \end{equation}
...
\begin{proof}
    La preuve est constituée de beaucoup de petites vérifications, mais pas de gros morceaux.
    \begin{subproof}
        \item[\( \tribA'\) est une tribu sur \( S^1\)]
            D'abord \( S^1\in \Lebesgue(\eC)\) parce que c'est un fermé. Donc \( S^1\in\tribA\). Ensuite, si \( A\in \tribA'\), alors
            \begin{equation}
                S^1\setminus A=(\eC\setminus A)\cap S^1
            \end{equation}
            et donc le complémentaire de \( A\) (par rapport à \( S^1\)) est dans \( \tribA'\) également. Enfin, si \( A_i\in \tribA'\), en particulier \( A_in\in\Lebesgue(\eC)\) et donc \( \bigcup_iA_i\in \Lebesgue(\eC)\). Vu que tous les \( A_i\) sont contenue dans \( S^1\), 
            \begin{equation}
                \bigcup_iA_i=\big( \bigcup_iA_i \big)\cap S^1\in \Lebesgue(\eC).
            \end{equation}
        \item[\( (S^1,\tribA',\mu')\) est mesurable, fini]
            Vu que \( \varphi^{-1}(\emptyset)=\emptyset\) nous avons
            \begin{equation}
                \mu'(\emptyset)=\frac{ \lambda\big( \varphi^{-1}(\emptyset) \big) }{ 2\pi }=\frac{1}{ 2\pi }\lambda(\emptyset)=0.
            \end{equation}

            Si les \( A_i\) sont disjoints, vu que \( \varphi\) est bijective nous avons
            \begin{equation}
                \varphi^{-1}\big( \bigcup_iA_i \big)=\bigcup_i\varphi^{-1}(A_i).
            \end{equation}
            où les \( \varphi^{-1}(A_i)\) sont également disjoints. Donc
            \begin{equation}
                \mu'\big( \bigcup_iA_i \big)=\frac{ \lambda\big( \varphi^{-1}(\bigcup_iA_i) \big) }{ 2\pi }=\frac{ \lambda\big( \bigcap_i\varphi^{-1}(A_i) \big) }{ 2\pi }=\sum_i\frac{ \lambda\big( \varphi^{-1}(A_i) \big) }{ 2\pi }=\sum_i\mu'(A_i).
            \end{equation}
            
            Le fait que \( \mu'\) soit une mesure finie, est simplement
            \begin{equation}
                \mu'(S^1)=\frac{ \lambda\big( \mathopen[ 0 , 2\pi \mathclose[ \big) }{ 2\pi }=1.
            \end{equation}
        \item[Prmière inclusion : \( \tribA'\subset \varphi(\Lebesgue_{S^1})\)]
            Nous rappelons que \( \tribA'=\{ A\in\Lebesgue(\eC)\tq A\subset S^1 \}\).
            Un élément \( A\in \tribA'\) est une partie \( A\in\Lebesgue(\eC)\) telle que \( A\subset S^1\). Nous posons \( M=\varphi^{-1}(A)\subset \mathopen[ 0 , 2\pi \mathclose[\) et nous voudrions prouver que \( M\in\Lebesgue(\eC)\).
                \begin{subproof}
                \item[Si \( A\) est un borélien]
                    Si \( A\) est un borélien de \( \eC\), alors \( \varphi^{-1}(A)\) est un borélien de \( \mathopen[ 0 , 2\pi \mathclose[\) par la proposition \ref{PROPooQFYHooEajmbW}.
                \end{subproof}
                <++>
    \end{subproof}
    <++>
