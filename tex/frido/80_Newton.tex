...
\begin{theorem}[Principe des zéros isolés \cite{Holomorphieus}]     \label{ThoukDPBX}
    Soit un ouvert \( \Omega\subset \eC\). Soient une fonction analytique \(f\colon \Omega\to \eC \) et \( a\in \Omega\), une zéro non isolé de \( f\). Alors \( f\) est nulle sur un voisinage de \( a\).
\end{theorem}
