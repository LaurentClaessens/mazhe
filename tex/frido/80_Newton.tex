...
% Copyright (c) 2011-2016,2018-2020, 2022-2023, 2026
%   Laurent Claessens
...
\begin{lemma}[\cite{MonCerveau}]       \label{LEMooTUQIooEyTLBa}
	Soient un espace topologique séparé \( X\) et un ouvert \( V\subset X\). Nous considérons une fonction\footnote{Pour rappel, dans \( C^0_c\), le \( 0\) signifie «continue» et le \( c\) signifie «à support compact». Donc ici, \( \phi\) est à support compact dans \( V\).} \( \phi\in C^0_c(V,\eC)\). Alors l'extension
	\begin{equation}
		\begin{aligned}
			h\colon X & \to\eC                              \\
...
\begin{proof}
	La proposition \ref{PROPooOVKEooCkJmmO} dit qu'il est suffisant de montrer que \( h\) est continue en tout point. Si \( a\in V\), alors il existe un voisinage de \( a\) contenu dans \( V\) (parce que \( V\) est ouvert). Sur ce voisinage, \( h=\phi\) est continue, et donc \( h\) est continue en \( a\).

	Supposons maintenant que \( a\not\in V\). Pour chaque \( x\in\supp(\phi)\), nous considérons deux ouverts disjoints \( A_x\) et \( B_x\) tels que \( x\in A_x\) et \( a\in B_x\) et tels que \( A_x\subset V\). Étant donné que \( \{ A_x \}_{x\in \supp(\phi)} \) recouvre le compact \( \supp(\phi)\), nous pouvons considérer un sous-ensemble fini \( I\subset\supp(\phi)\) tel que \( A=\bigcup_{i\in I}A_i\) recouvre \( \supp(\phi)\).

	Nous posons alors \( B=\bigcap_{i\in I}B_i\). En tant que intersection finie d'ouverts, \( B\) est un ouvert qui contient \( a\). Nous avons \( A\cap B=\emptyset\). Donc \( h=0\) sur \( A\). Vu que \( h\) est identiquement nulle sur un voisinage de \( a\), l'application \( h\) est continue en \( a\).
\end{proof}
...
\begin{definition}      \label{DEFooMHQMooEvLccV}
	Un \defe{plongement}{plongement} de l'espace topologique \( X\) dans \( Y\) est une application \( f\colon X\to Y\) telle que \( f\colon X\to f(X)\) soit un homéomorphisme (isomorphisme d'espaces topologiques). Note : nous considérons sur \( f(X)\) la topologie induite depuis \( Y\).
\end{definition}
