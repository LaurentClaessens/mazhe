...
Cet exemple sera rendu rigoureux par le propositions \ref{PROPooUMIPooQGXaPd} et \ref{PropJLnPpaw}.
...
À mon avis, la proposition suivante se prouve en approximant tous les compacts comme union d'intervalles et en utilisant la proposition \ref{PropAAjSURG}. Voir le reste du thème \ref{THEMEooDASVooWZLOjw}.
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooZOJHooKwZOFW}
	Soit \( f\) intégrable sur \( \mathopen[ a,b\mathclose]\) vérifiant
	\begin{equation}
		\int_a^cf=0
	\end{equation}
	pour tout \( c\in\mathopen[ a,b\mathclose]\). Alors \( f=0\) presque partout.
	%TODOooTHORooKoHybW. Prouver ça.
\end{proposition}
