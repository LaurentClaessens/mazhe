...
% Si on change l'énoncé, ici, il faut s'assurer que la preuve donnée en NORMALooGYUEooKrYjyz avec des intégrales tienne encore.
...
\begin{proof}
  ...
  il suit que~$(f_i|_K)_i$ est une suite de Cauchy. Donc elle converge unformément vers une
  limite~$f|_K$.
  ...
  \begin{align}
    \left| \frac{1}\delta \left( f(y+\delta) - f(y) \right) - g(y) \right|
...
\begin{normaltext}      \label{NORMALooGYUEooKrYjyz}
    Voici une preuve alternative du théorème \ref{THOooXZQCooSRteSr}. Elle provient de \cite{TrenchRealAnalisys} et utilise des intégrales; elle demande donc plus de dependences.
    % Vu que ce théorème est démontré par ailleurs, les références vers le futur de cette démonstration alternative sont acceptables.
...
\begin{remark}[Ce n'est pas la mesure que nous voulons]     \label{REMooOMYYooNFiKOs}
    ...
    \begin{enumerate}
        ...
        \item       \label{ITEMooJTKCooYQknqo}
            Dans le cas de \( S^1\), nous avons \( \varphi(x)= e^{ix}\), et \( \| \varphi'(x) \|=1\). Donc la mesure donné ici est probablement bien celle que nous voulons. Peut-être à coefficient \( \frac{1}{ 2\pi }\) près pour avoir une normalisation \( \mu'(S^1)=1\). Cela est également une autre histoire qui vous sera contée une autre fois.
    \end{enumerate}
\end{remark}
