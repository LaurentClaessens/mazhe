...
\begin{normaltext}
	Si vous voulez savoir que \( \lim_{h\to 0}\int_a^{a+h}f=0\), c'est le corolaire \ref{CORooSGMXooCNfGNk}. La proposition \ref{PROPooPTZRooCBkZvr} est plus général et dit que dans \( L^p\), il existe \( \delta\) tel que si \( \mu(A)<\delta\), alors \( \int_A| f |^p<\epsilon\).
\end{normaltext}
