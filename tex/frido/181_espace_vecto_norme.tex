...
\begin{proof}
    ...
    \end{equation}
%TODO: pour quoi faire "il faut que le discriminant soit <= 0" ?
    C'est un polynôme du second degré en \( t\) dont le signe est toujours positif (ou nul). Par la proposition \ref{PROPooEZIKooKjJroH} nous en déduisons que le fameux \( b^2-4ac\) doit être négatif ou nul. Nous avons donc
    \begin{equation}
...
%TODO: et si le produit scalaire est negatif ? Il faut rajouter des valeurs absolues partout ?
            \begin{equation}
                2x\cdot y\leq 2\| x \|\| y \|.
            \end{equation}
...
%TODO: lien vers la def "forme quadratique" ?
...
\begin{proof}
    ...
    \begin{equation}
        f(0)\cdot f(y)=0
    \end{equation}
%TODO: Definir "le produit scalaire est ->non dégénéré<-" ??
    pour tout \( y\). Parce que \( f\) est une bijection, nous avons \( f(0)\cdot x=0\) pour tout \( x\). Comme le produit scalaire est non dégénéré cela implique que \( f(0)=0\).
\end{proof}
...
\begin{proof}
    Si \( \langle x, y\rangle =0\), le résultat est évident; nous supposons que \( \langle x, y\rangle \neq0\). Nous posons
    \begin{equation}
    ...
    \begin{subequations}
        \begin{align}
            \| x+y \|^2 & =\langle x+y, x+y\rangle                                        \\
                        ...
                        & \leq \| x \|^2+\| y \|^2+2\| x \|\| y \|\label{SUBEQooQGQBooMRJcUc} =\big( \| x \|+\| y \| \big)^2.
        \end{align}
    \end{subequations}
...
%TODOooZYLXooSbJnaC : il faut mettre au clair quelle est vraiment la topologie faible à partir des ouverts.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Pour \eqref{SUBEQooJPSJooAqXtOJ}, c'est parce que la norme est une norme d'algèbre\footnote{Définition \ref{DefJWRWQue}. Pour rappel, la norme opérateur en est une par le lemme \ref{LEMooFITMooBBBWGI}.}.
    \end{itemize}
...
\begin{proof}
    La preuve suit celle de la proposition \ref{PROPooOXBCooIzLaPe}. Le sens direct est un a fortiori.
...
\begin{proof}
    Le sens direct est un a fortiori.
