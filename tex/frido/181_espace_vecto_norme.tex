...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooEZETooTNStnO}
	Soient des ouverts \( \{ A_i \}_{i=1,\ldots,n}\) d'un espace vectoriel normé. La frontière de l'union est inclue dans l'union des frontières :
	\begin{equation}
		\partial\big( \bigcup_{i=1}^nA_i \big)\subset\bigcup_{i=1}^n\partial A_i.
	\end{equation}
\end{proposition}


\begin{proof}
	Soit \( p\in \partial\bigcup_iA_i\). Soient des réels strictement positifs \( r_k\to 0\). Pour chaque \( k\), il y a un \( i_k\in \{ 1,\ldots,n \}\) tel que \( B(p,r_k)\cap A_{i_k}\neq \emptyset\). Cela définit une application
	\begin{equation}
		\begin{aligned}
			\phi\colon \eN & \to \{ 1,\ldots,n \} \\
			k              & \mapsto i_k.
		\end{aligned}
	\end{equation}
	Par la proposition \ref{PROPooYCDWooXBWVrW}, il existe un \( s\in\{ 1,\ldots,n \}\) tel que \( \phi^{-1}(s)\) est infini. Nous avons donc \( p\in\Adh(A_s)\). Mais \( p\) n'est pas dans l'intérieur de \( A_s\), sinon \( p\) serait à l'intérieur de \( \bigcup_iA_i\). Donc \( p\in\partial A_s\).
\end{proof}
...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooPXMUooTOQCLa}
	...
	\begin{equation}
		\begin{aligned}
			\tau_a\colon \eR & \to \eR                                               \\
			x                & \mapsto \min \Big( [x]\cap \overline{B(a,1/2)} \Big).
		\end{aligned}
	\end{equation}
\end{definition}
...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooJNJQooThnhle}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooGUUCooWujZAv}
	Soit \( q\in \eR^2\). Si \( r<1/2\) alors
	\begin{equation}
		p \colon B(q,r)\to  p\big( B(q,r) \big)
	\end{equation}
	est injective.
\end{lemma}

\begin{proof}
	Soient \( a,b\in B(q,r)\) tels que \( [a]=[b]\). Il existe \( k\in \eZ^2\) tel que \( b=a+k\). Nous avons alors d'une part
	\begin{equation}
		\| a-b \|=\| k \|.
	\end{equation}
	Et d'autre part,
	\begin{equation}
		\| a-b \|\leq \| a-q \|+\| b-q \|<2r<1.
	\end{equation}
	Donc \( \| k \|<1\) et donc \( k=0\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooUENHooYsXspd}
	Soit \( q\in \eR^2\). Si \( r<1/2\), alors l'application
	\begin{equation}
		p \colon B(q,r)\to B\big( [q],r \big)
	\end{equation}
	est une bijection bicontinue\footnote{Continue, inverse continue.}.
\end{proposition}

\begin{proof}
	En plusieurs parties.
	\begin{subproof}
		\spitem[\( p\big( q,r \big)\subset B\big( [q],r \big)\)]
		%-----------------------------------------------------------
		Soit \( s\in B(q,r)\). Nous avons
		\begin{subequations}
			\begin{align}
				d\big( [q],[s] \big) & =\| \tau_q(s)-q \|                                             \\
				                     & =\min_{x\in[s]}\| q-x \| & \text{lem. \ref{LEMooWYCAooYVhcoD}} \\
				                     & \leq \| q-s \|                                                 \\
				                     & <r.
			\end{align}
		\end{subequations}
		...
		\spitem[Injective]
		%-----------------------------------------------------------
		Si \( r<1/2\) alors le lemme \ref{LEMooGUUCooWujZAv} dit que \(p \colon B(q,r)\to  B([q],r) \) est injective.
		...
		\begin{equation}
			d([s],[q])=\| \tau_q(s)-q \|<r.
		\end{equation}
		Mais \( \tau_q(s)=\min\big( [s]\cap \overline{B(q,1/2)} \big)\).  Donc \( \tau_q(s)\in [s]\) et il existe \( k\in \eZ^2\) tel que \( \tau_q(s)=s+k\). En posant \(s_0= \tau_q(z)\), nous avons \( p(s_0)=[s]\), et \( \| s_0-q \|<r\).

		\spitem[Continue]
		%-----------------------------------------------------------
		L'application \( p\) est continue par définition de la topologie quotient sur \( \eR^2/\sim\).

		\spitem[Inverse continue]
		%-----------------------------------------------------------
		Si \( r<1/2\), l'application \(p \colon B(q,r)\to B([q],r)  \) est une bijection continue. Prenons \( r_2<r\). Alors l'application restreinte
		\begin{equation}
			p \colon \overline{B(q,r_2)}\to p\big( \overline{B(q,r_2)} \big)
		\end{equation}
		est une bijection continue définie sur un compact. Le lemme \ref{LEMooPLGTooATIGov} dit que la réciproque
		\begin{equation}
			p^{-1} \colon p\big( \overline{B(q,r_2)} \big)\to \overline{B(q,r_2)}
		\end{equation}
		est continue.

		Donc l'application \(p^{-1} \colon     p\big( B(q,r_2) \big)   \to B(q,r_2)  \) est continue. Et comme \( p\big( B(q,r_2) \big)=B([q],r_2)\), nous concluons que
		\begin{equation}
			p \colon B(q,r_2)\to B([q],r_2)
		\end{equation}
		est une bijection continue de réciproque continue.
	\end{subproof}
\end{proof}
...
\begin{proof}
	Soit \( A\) ouvert et connexe par arcs dans \( \eR^n\). Soit \( p\in A\); nous devons prouver que \( A\setminus\{ p \}\) est connexe par arcs. Soient \( a,b\in A\setminus\{ p \}\) ainsi qu'un chemin \( \gamma\) liant \( a\) à \( b\) dans \( A\).

	Si \( p\) n'est pas dans l'image de \( \gamma\), alors c'est bon.

	Sinon nous supposons que \( \gamma(t_0)=p\). Vu que \( A\) est ouvert, il contient une boule autour de \( p\). En diminuant le rayon de cette boule, nous pouvons supposer que \( \overline{B(p,r)}\subset A\) et \( a,b\not\in \overline{B(p,r)}\).

	Nous considérons l'application continue
	\begin{equation}
		\begin{aligned}
			d\colon \mathopen[ 0,1\mathclose] & \to \eR^+                  \\
			t                                 & \mapsto \| \gamma(t)-p \|.
		\end{aligned}
	\end{equation}
	Elle vérifie \( d(0)>r\), \( d(t_1)=0\) et \( d(1)>r\). D'après le théorème des valeurs intermédiaires \ref{ThoValInter}, il existe \( u_0\in\mathopen] 0,t_0\mathclose[\) tel que \( d(u_0)=r\) et \( u_1\in\mathopen] t_0,1\mathclose[\) tel que \( d(u_1)=r\).

	La partie
	\begin{equation}
		Z=\{ t\in\mathopen[ 0,1\mathclose]\tq d(t)=r \}
	\end{equation}
	est bornée (parce que dans \( \mathopen[ 0,1\mathclose]\)) et fermée (son complémentaire est ouvert par continuité de \( d\)). Donc elle est compacte\footnote{Borel-Lebesgue \ref{ThoXTEooxFmdI}.}. Elle a donc un maximum et un minimum. Soit \( u_m=\min(Z)\) et \( u_M=\max(Z)\). Nous notons \( x=\gamma(u_m)\) et \( y=\gamma(u_M)\).

	Par la connexité du cercle (lemme \ref{LEMooQFQFooDlxkw}, point \ref{SPITEMooGDRIooZJXAIC}), il existe un chemin \( \gamma_c\) sur le cercle \( \| x-m \|=r\) liant \( x\) à \( y\). Il suffit maintenant de mettre bout à bout (lemme \ref{LEMooMMTBooYExLZH}\ref{ITEMooEKYEooMDHSHH}) les chemins \( \gamma\) entre \( a\) et \( x\) puis le chemin \( \gamma_c\) liant \( x\) à \( y\) et encore \( \gamma\) entre \( y\) et \( b\) pour avoir un chemin liant \( a\) et \( b\).

	Ce chemin reste dans \( A\) parce que \( \gamma\) y reste et \( \gamma_c\) reste dans \( \overline{B(p,r)}\subset A\). De plus ce chemin ne passe pas par \( p\) parce que \( \gamma\) reste en dehors de \( B(p,r)\) lorsque \( t\in\mathopen[ 0,u_m\mathclose]\cup \mathopen[ u_M,1\mathclose]\).
\end{proof}
...
\begin{theorem}[Théorème spectral autoadjoint] \label{ThoRSBahHH}
	Un endomorphisme autoadjoint\footnote{Définition \ref{DefLZMcvfj}.} d'un espace euclidien
	\begin{enumerate}
		\item
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooSYTWooMDzrnX}
	La conjugaison complexe est continue.
\end{lemma}

\begin{proof}
	La norme sur \( \eC\) est donnée par la définition \ref{DEFooGUXNooXwCsrq} à partir de laquelle on voir que la conjugaison complexe est une isométrie :
	\begin{equation}
		\| \bar z \|=\| z \|.
	\end{equation}
\end{proof}

\begin{lemma}[\cite{MonCerveau}]	\label{LEMooGEVOooKeVQbb}
	Si \( z_1,z_2\in \eC\) nous avons
	\begin{equation}
		| z_1z_2 |=| z_1 || z_2 |.
	\end{equation}
	%TODOooVXNTooAkvVXG. Prouver ça.
\end{lemma}
...
%-------------------------------------------------------
\subsection{Complétion de bases, procédé de Gram-Schmidt}
%----------------------------------------------------
...
\begin{proof}
	...
	\end{equation}
\end{proof}
Cet algorithme de Gram-Schmidt nous donne non seulement l'existence de bases orthonormée pour tout espace euclidien, mais aussi le moyen d'en construire à partir de n'importe quelle base. Il est également possible de compléter une base avec des vecteurs perpendiculaires aux vecteurs déjà donnés en utilisant le lemme \ref{LEMooFRWKooVloCSM}.
...
%-------------------------------------------------------
\subsection{Applications linéaires dans un espace de Banach}
%----------------------------------------------------



\begin{proposition}[\cite{BIBooLARIooUopQud}]	\label{PROPooUZEHooRiEnOG}
	Soit un espace normé \( E\) et un espace de Banach \( F\). Alors l'espace \( \aL^0(E,F)\) des applications linéaires continues est un espace de Banach.
\end{proposition}

\begin{proof}
	L'espace des application linéaires bornées \( \aL^0(E,F)\) est un espace vectoriel normé pour la norme opérateur de la définition \ref{DefNFYUooBZCPTr}.

	Soit une suite de Cauchy \( (T_n)_{n\in \eN}\) dans \( \aL^0(E,F)\).
	\begin{subproof}
		\spitem[\( (T_n(x))\) est de Cauchy]
		%-----------------------------------------------------------
		Soit \( x\in E\). Nous prouvons que la suite \( (T_n(x))_{n\in \eN}\) est de Cauchy. Soit \( \epsilon>0\). Il existe \( p>0\) tel que si \( n,m\geq p\) alors \( \| T_n-T_m \|<\epsilon\). Pour de tels \( n,m\) nous avons
		\begin{equation}
			\| T_n(x)-T_m(x) \|\leq \| T_n-T_m \|\| x \|\leq \epsilon\| x \|.
		\end{equation}
		Étant donné que \( \| x \|\) est une constante, la suite \( (T_n(x))\) est bien de Cauchy.

		\spitem[L'application \( T\)]
		%-----------------------------------------------------------
		Étant donné que \( F\) est de Cauchy, la suite \( n\mapsto T_n(x)\) converge. Cela nous permet de définir
		\begin{equation}
			\begin{aligned}
				T\colon E & \to F                             \\
				x         & \mapsto \lim_{n\to \infty}T_n(x).
			\end{aligned}
		\end{equation}

		\spitem[Une formule]
		%-----------------------------------------------------------
		Soit \( \epsilon>0\). Nous considérons encore \( p\in \eN\) tel que \( \| T_n-T_m \|<\epsilon\) dès que \( n,m\geq p\). Soit \( x\in \overline{B_E(0,1)}\). Nous avons
		\begin{subequations}
			\begin{align}
			...
			\end{align}
		\end{subequations}
		Nous faisons \( m\to \infty\) en tenant compte du fait que la limite commute avec la norme :
		\begin{subequations}
			\begin{align}
				\lim_{m\to\infty}\| T_n(x)-T_m(x) \| & =\| T_n(x)-\lim_{n\to \infty}T_m(x) \| \\
				                                     & \| T_n(x)-T(x) \|.
			\end{align}
		\end{subequations}
		Nous avons donc montré que pour tout \( n\geq p\) nous avons:
		\begin{equation}
		...
		\begin{subequations}
			\begin{align}
				\sum_{\| x \|\leq 1}\| T(x) \| & \leq \sup_{\| x \|\leq 1}\Big( \| T(x)-T_p(x) \|+\| T_p(x) \| \Big) \\
				                               & \leq \epsilon+\| T_p \|.
			\end{align}
		\end{subequations}
		Donc \( \| T \|\leq \| T_p \|+\epsilon<\infty\).

		\spitem[Limite]
		%-----------------------------------------------------------
		Nous prouvons que \( T\) est bien la limite de la suite \( (T_n)_{n\in \eN}\). C'est simple :
		\begin{equation}
			\| T_n-T \|\leq \sup_{\| x \|\leq 1}\| T_n(x)-T(x) \|\leq\epsilon
		\end{equation}
		dès que \( n\geq N\). Cela prouve que \( \| T_n-T \|\to 0\).
	\end{subproof}
\end{proof}


\begin{proposition}[\cite{BIBooLARIooUopQud}]     \label{PROPooOVGGooNffWJW}
	Le dual\footnote{Définition \ref{DEFooKSDFooGIBtrG}.} d'un espace de Banach\footnote{Définition \ref{DefVKuyYpQ}.} est de Banach.
\end{proposition}

\begin{proof}
	Si \( E\) est un espace de Banach sur le corps \( \eK\), son dual est \( \aL^0(E,\eK)\). La proposition \ref{PROPooUZEHooRiEnOG} en fait un Banach.
\end{proof}
...
\begin{theorem}[Bornée si et seulement si continue\cite{MaurayAnalSpec,GKPYTMb}]      \label{ThoCaMpKO}     \label{PROPooQZYVooYJVlBd}
	Soient \( E\) et \( F\) des espaces vectoriels normés. Pour une application linéaire \( f\colon E\to F\), les assertions suivantes sont équivalentes :
	\begin{enumerate}
		\item		\label{ITEMooIQDEooUVuQxu}
		      \( f\) est continue en un point de \( E\),
		\item		\label{ITEMooQMQPooXCyQXyBV}
		      \( f\) est continue sur \( E\),
		\item		\label{ITEMooBOYYooGjePma}
		      \( f\) est bornée.
	\end{enumerate}
\end{theorem}

\begin{proof}
	La proposition \ref{PROPooHKGZooDaNaHU} donne déjà l'équivalence \ref{ITEMooIQDEooUVuQxu} \( \Leftrightarrow\) \ref{ITEMooQMQPooXCyQXyBV}.
...
La proposition suivante doit surement beaucoup à la proposition \ref{PROPooDQBOooByBvmj}.
...
Voir aussi la définition \ref{LEMooORANooRLJYWw} pour la norme sur les applications \( k\)-multilinéaires.

\begin{proof}
	Nous posons
	\begin{equation}
		A=\{ \| \alpha(x_1,\ldots,x_n)\| \tq \| x_i \| \leq 1 \},
	\end{equation}
	et
	\begin{equation}
		B=\{ \lambda\in \eR\tq \| \alpha(x_1,\ldots,x_n) \|\leq \lambda\| x_1 \|\ldots \| x_n \| \},
	\end{equation}
	et ensuite \( M=\sup(A)\), \( m=\inf(B)\).

	\spitem[\( m\leq M\)]
	%-----------------------------------------------------------
	Pour cela nous prouvons que \( M\in B\). Pour \( x_1,\ldots,x_n\in E_1\times\ldots\times E_n\) nous posons \( y_i=x_i/\| x_i \|\) de telle sorte que
	\begin{subequations}
		\begin{align}
			\| \alpha(x_1,\ldots,x_n) \| & =\| x_1 \|\ldots\| x_n \|\| \alpha(y_1,\ldots,y_n) \| \\
			                             & \leq M\| x_1 \|\ldots \| x_n \|.
		\end{align}
	\end{subequations}
	Cela prouve que \( M\in B\) et donc que \( M\geq m\) parce que \( m=\inf(B)\).

	\spitem[\( M\leq m\)]
	%-----------------------------------------------------------
	Nous montrons que \( M\leq \lambda\) pour tout \( \lambda\in B\). Soient donc \( \lambda\in B\) et \( \| x_i \|\leq 1\). Vu que \( \lambda\in B\) nous avons
	\begin{equation}
		\lambda\geq \frac{ \| \alpha(x_1)\ldots \alpha(x_n) \| }{ \| x_1 \|\ldots \| x_n \| }.
	\end{equation}
	Nous avons
	\begin{subequations}
		\begin{align}
			\| \alpha(x_1,\ldots,x_n) \| & \leq \frac{ \| \alpha(x_1,\ldots,x_n) \| }{ \| x_1 \|\ldots \| x_n \| } \\
			                             & \leq \lambda.
		\end{align}
	\end{subequations}
	En prenant le supremum sur les \( \| x_i \|\leq 1\) nous trouvons \( M\leq \lambda\). Étant donné que cela est vrai pour tout \( \lambda\in B\) nous avons \( M\leq \inf(B)=m\).

	\spitem[C'est une norme]
	%-----------------------------------------------------------
	Pour \( \alpha\in \aL_k(E_1,\ldots,E_n,F)\) nous notons
	\begin{subequations}
		\begin{align}
			N(\alpha) & = \inf\{ \lambda\in \eR\tq\,\forall (x_1,\ldots,x_n)\in E_1\times \ldots E_n,\, \| \alpha(x_1,\ldots,x_n) \|\leq \lambda \| x_1 \|\ldots\| x_n \| \} \\
			          & =\sup\{ \| \alpha(x_1,\ldots,x_n) \|\tq\| x_i \|\leq 1 \}
		\end{align}
	\end{subequations}
	Nous prouvons que \( N\) est une norme sur \( \aL_k\).

	\spitem[\( N(\alpha)=0\) implique \( \alpha=0\)]
	%-----------------------------------------------------------
	Nous supposons que \( \alpha\neq 0\), c'est à dire qu'il existe \( a_1,\ldots,a_n\) tels que \( \alpha(a_1,\ldots,a_n)\neq 0\). Nous posons \( y_i=a_i/\| a_i \|\). Nous avons
	\begin{equation}
		\alpha(y_1,\ldots,y_n)=\frac{ \alpha(a_1,\ldots,a_n) }{ \| a_1 \|\ldots \| a_n \| },
	\end{equation}
	et donc
	\begin{subequations}
		\begin{align}
			\| \alpha \| & =\sup\{ \| \alpha(x_1,\ldots,x_n) \|\tq \| x_i \|\leq 1 \} \\
			             & \geq \| \alpha(y_1,\ldots,y_n) \|                          \\
			             & >0.
		\end{align}
	\end{subequations}

	\spitem[Inégalité triangulaire]
	%-----------------------------------------------------------
	Soient \( x_1,\ldots,x_n\) tels que \( \| x_i \|\leq 1\). Nous avons
	\begin{subequations}
		\begin{align}
			N(\alpha+\beta) & \leq \| (\alpha+\beta)(x_1,\ldots,x_n) \|                     \\
			                & =\| \alpha(x_1,\ldots,x_n)+\beta(x_1,\ldots,x_n) \|           \\
			                & \leq \| \alpha(x_1,\ldots,x_n) \|+\| \beta(x_1,\ldots,x_n) \| \\
			                & \leq N(\alpha)+N(\beta).
		\end{align}
	\end{subequations}
\end{proof}
