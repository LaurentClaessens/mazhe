...
\begin{example}
	...
	\end{equation}
	L'ensemble $A_3$ n'est pas ouvert, ni fermé, ni borné dans la topologie de $\eR^2$. En fait, $\eQ$ a un intérieur vide et sa fermeture est $\eR$. L'ensemble $\eN$, par contre est fermé et non borné. On peut remarquer que tous les points de $\eN$ sont points isolés. La fermeture de $A_3$ est alors $\eN\times \eR$ et son intérieur est vide. On peut dessiner la fermeture de cet ensemble comme une famille de droites verticales $x=n$, pour tout $n$ dans $\eN$.
\end{example}
...
Si \( f\) est une fonction \( f\colon \eR\to \eR\), que signifie \( \lim_{x\to \pm\infty} f(x)\) ? Il s'agit de considérer la fonction élargie
\begin{equation}
	\begin{aligned}
...
%TODO: decider \bar\eR  ou \overline ?
...
Notons que ce résultat se généralise : si \( f\) est continue et pas égale à \( r\) en \( a\), alors elle continue à n'être pas égale à \( r\) dans un voisinage de \( a\).
...
\begin{proof}
	...
	\]
	Hélas, ce $\delta_1$ n'est pas celui qu'il faut parce que nous travaillons avec $\lambda f$ au lieu de $f$, ce qui fait qu'au lieu d'avoir $| f(x)-f(a) |$, nous avons $| \lambda f(x)-\lambda f(a) |=| \lambda |\cdot | f(x)-f(a) |$.  Ce que $\delta_1$ opère avec $(\lambda f)$, c'est
	\[
		(| x-a |\leq\delta_1)\Rightarrow  | (\lambda f)(x)- (\lambda f)(a)|\leq | \lambda |\epsilon_1.
	\]
	Ce que nous apprend la continuité de $f$, c'est que pour chaque choix de $\epsilon_1$, on a un $\delta_1$ qui réalise cette implication. Comme cela est vrai pour chaque choix de $\epsilon_1$, essayons avec $\epsilon_1=\epsilon/| \lambda |$ pour voir ce que ça donne. Nous avons donc un $\delta_1$ tel que
	\[
...
\begin{proposition}     \label{PROPooUMVGooIrhZZg}
	Pour tous nombres complexes $z = a+bi$ et $z^\prime$, nous avons
	\begin{enumerate}
...
\begin{proof}
	...
	\end{equation}
	et donc que \( \real(a\bar b)=| a | | b |\). Comme \( | a\bar b |=| a | | b |\) (proposition \ref{PROPooUMVGooIrhZZg}\ref{ITEMooFXKYooUOXbwH}), nous avons
	\begin{equation}
		\real(a\bar b)=| a\bar b |.
	\end{equation}
	Nous sommes dans le cas d'égalité du lemme \ref{LEMooHTAWooQNrXSL}. Donc \( a\bar b\in \eR\). Et il est manifestement positif parce qu'égal à \( | a | | b |\).
...
\begin{proof}
	...
	\begin{subproof}
		...
		\end{equation}
		Maintenant, \( x\bar y+y\bar x=2\real(xy)\), \( \real(z)\leq | z |\) et la racine carrée étant croissante\footnote{Lemme \ref{LEMooSBOAooOOIotR}.}, nous avons
		\begin{equation}
...
\begin{proposition}
	L'ensemble \( (\eC, | . |)\) est un espace vectoriel normé\footnote{Définition d'une norme : \ref{DefNorme}.}.
\end{proposition}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item
		      Si \( | z |=0\), alors, en notant \( z=a+bi\) nous avons \( a^2+b^2=0\). La somme de deux positifs n'est nulle que quand les deux positifs sont nuls. Donc \( a=b=0\).
		\item
...
Il est possible de dire que la racine carrée est continue en $0$, bien qu'elle ne soit pas définie sur un ouvert autour de $0$\ldots en tout cas pas un ouvert au sens que le lecteur a en tête. Nous allons rentabiliser un bon coup notre travail sur les espaces métriques.
