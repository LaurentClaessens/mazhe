...
\begin{proposition}[\cite{ooRCDWooONrayj}]     \label{PropDerrLin}
    ...
    \begin{enumerate}
        ...
        \item   \label{ITEMooGHJHooQXsajQ}
            $(f+g)'=f'+g'$.
        \item
            Si \( f\) et \( g\) sont des fonctions dérivables tel que \( g(x_0)\neq 0\) alors \( f/g\) est également dérivable en \( x_0\) et 
            \begin{equation}
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            \frac{ h(x+\epsilon)-h(x) }{ \epsilon }&=\frac{1}{ \epsilon }\frac{ g(x)f(x+\epsilon)-g(x+\epsilon)f(x) }{ g(x+\epsilon)g(x) }\\
            &=\frac{1}{ \epsilon }\frac{ g(x)f(x+\epsilon)-g(x)f(x)+g(x)f(x)-g(x+\epsilon)f(x) }{ g(x+\epsilon)g(x) }\\
            &=\frac{1}{ \epsilon }\left( \frac{ f(x+\epsilon)-f(x) }{ g(x+\epsilon) }- \frac{ g(x+\epsilon)f(x)-g(x)f(x) }{ g(x+\epsilon)g(x) } \right)
        \end{align}
    \end{subequations}
    Le second terme dans la parenthèse est égal à
    \begin{equation}
        \frac{ f(x) }{ g(x) }\frac{ g(x+\epsilon)-g(x) }{ g(x+\epsilon) }.
    \end{equation}
