...
\begin{proof}
    Souvenons-nous que, en général, nous avons
    \begin{equation}
        \lim_{x\to a} f(x)=b
    \end{equation}
    si pour tout voisinage \( V\) de \( b\), il existe un voisinage \( W\) de \( a\) tel que \( z\in W\) implique \( f(z)\in V\).

    Ceci étant dit, nous passons à la preuve.
    \begin{subproof}
        \item[Sens direct]
            Nous supposons que \( \lim_{z\to \infty} f(z)=\infty\). Soit \( M>0\); nous considérons le voisinage \( V=\overline{ B(0,M) }^c\cup\{ \infty \}\). Par définition de la limite, il existe un voisinage \( W\) de \( \infty\) tel que \( z\in W\Rightarrow f(z)\in V\). Ce voisinage est de la forme \( K^c\cup\{ \infty \}\). Vu que \( K\) est compact, il est borné et il existe \( R>0\) tel que \( K\subset B(0,R)\).

            Avec tout cela nous avons la chaîne suivante d'implications :
            \begin{equation}
                | z |>R\Rightarrow z\in K^c\Rightarrow z\in W\Rightarrow f(z)\in V\Rightarrow | f(z) |>M.
            \end{equation}
...
\begin{proposition}[\cite{ooRIPVooMlBiAH,MonCerveau}]
    Si \( n\geq 1\), alors l'équation \( z^n=a+bi\) admet une\footnote{Pas spécialement une seule, comme vous en conviendrez en pensant à \( z^2=1\) qui a déjà les solutions \( 1\) et \( -1\).} solution pour \( z\in \eC\).
\end{proposition}

\begin{proof}
    En plusieurs étapes selon la parité de \( n\).
    \begin{subproof}
        \item[Si \( n\) est impair]
            ...
            \begin{equation}
                ...
                \end{aligned}
            \end{equation}
            La proposition \ref{PROPooJKYJooFqbQMr} ne permet pas de conclure immédiatement parce que le polynôme \( f\), bien que de degré impair, n'est pas à coefficients réels.
            \begin{subproof}
        \item[\( | f |\) a une borne inférieure]
            La proposition \ref{PROPooPWVWooGuftxZ} nous dit que \( f\) est équivalente à l'infini à la fonction \( z\mapsto z^n\). Soit \( m\) une valeur atteinte par \( | f |\); par exemple \( m=| f(1) |\). Il existe \( R\) tel que \( | z |>R\) implique \( | f(z) |>m\). Nous considérons \( R'=\max\{ R,2 \}\). La restriction de \( | f |\) au compact \( \overline{ B(0,R') }\) est une fonction continue sur un compact. Elle atteint donc ses bornes par le théorème \ref{ThoMKKooAbHaro}. Soit \( z_0\) un point sur lequel \( | f |\) a son minimum. 
            
            Ce \( z_0\) est un minimum global de \( | f |\) sur \( \eC\). En effet, \( 1\in B(0,R')\), donc \( | f(z_0) |\leq | f(1) |<f(z)\) pour tout \( z\) hors de \( \overline{ B(0,R') }\). Le but est de montrer que \( f(z_0)=0\).

        \item[Un polynôme intermédiaire]
            ...
            \begin{equation}
                ...
                \end{aligned}
            \end{equation}
            autrement dit, \( g(z)=(z_0+z)^n-(a+bi)\). Cela est encore un polynôme en \( z\) que nous notons \( g(z)=\sum_{i=0}^nA_iz^i\). Nous avons \( A_0=z_0^n-(a+bi)\) et \( A_1=nz_0^{n-1}\).
            \item[Si \( z_0\neq 0\)]
                Dans le cas \( z_0\neq 0\) nous avons \( A_1\neq 0\). La proposition \ref{LEMooTTOYooXaukuH} nous dit qu'il existe une fonction \( \psi\) telle que
                \begin{equation}
                    g(z)=\big( 1+\alpha(z) \big)(A_0+A_1z)
                \end{equation}
                avec \( \lim_{z\to 0} \alpha(z)=0\). Nous calculons cela sur la droite \( z(t)=\frac{ A_0 }{ A_1 }t\) pour \( t\in \eR\) :
                \begin{equation}
                    g\left( \frac{ A_0 }{ A_1 }t \right)=\big( 1+\alpha(A_0t/A_1)2tA_0 \big)=2tA_0\big( 1+\alpha(A_0t/A_1) \big)
                \end{equation}
                Nous avons donc
                \begin{equation}
                    \lim_{t\to 0} | g\left( \frac{ A_0 }{ A_1 }t \right) |=0.
                \end{equation}
                Par continuité de \( g\) en \( 0\), nous avons donc \( g(0)=0\). Or \( g(0)=f(z_0)=z_0^n-(a+bi)\). Donc nous avons prouvé que
                \begin{equation}
                    z_0^n-(a+bi)=0,
                \end{equation}
                c'est à dire que \( z_0\) est une racine \( n\)\ieme\ de \( a+bi\).

            \item[Si \( z_0=0\)]

                Si \( z_0=0\), alors \( f(z)=z^n-(a+bi)\) est une fonction telle que \( | f(z) |\) a un minimum en \( z=0\), c'est à dire un minimum qui vérifie \( | f(0) |=| a+bi |\), et ça c'est pas bien. Heureusement nous allons voir que ce n'est pas possible.

                \begin{subproof}
                    \item[Si \( a\neq 0\)]
                    
                        Pour \( \delta>0\in \eR\) nous avons
                        \begin{subequations}
                            \begin{align}
                                \real\big(f(\pm\delta)\big)&=(\pm1)^{n}\delta^n-a\\
                                \imag\big(f(\pm\delta)\big)&=-b
                            \end{align}
                        \end{subequations}
                        Le nombre \( n\) étant impair, nous avons \( (-1)^n=-1\). Du coup nous avons
                        \begin{equation}
                            | f(\delta) |^2=| \delta^n-a |^2+b^2
                        \end{equation}
                        et
                        \begin{equation}
                            | f(-\delta) |^2=| -\delta^n-a |^2+b^2
                        \end{equation}
                        Forcément soit \( | f(\delta) |\) soit \( | f(-\delta) |\) sera plus petit que \( | a |^2+| b |^2\). Cela prouve que \( a+bi\) n'est pas un minimum global. Contradiction, tout ça. Donc pas \( z_0=0\) dans le cas \( a\neq 0\).

                    \item[Si \( b\neq 0\)]
                        Nous faisons la même chose, mais avec \( z=\pm i\delta\) au lieu de \( z=\pm\delta\).
                \end{subproof}
                Nous avons fini de prouver que \( z^n=a+bi\) a une solution dans \( \eC\) pour tout \( n\) impair.
                
        \end{subproof}

    \item[Si \( n=2\)]
        L'équation \( z^2=a+bi\) a pour solution \( \pm\xi\) où
        \begin{equation}
    ...
    \item[Si \( n=2^k\)]
        Par récurrence sur \( k\). Nous savons que
        \begin{equation}
            z^{2^{k+1}}=(z^{2^k})^2.
        \end{equation}
        ...
        \begin{equation}
            z^{2^{k+1}}=a+bi.
        \end{equation}
    \item[Si \( n\) est quelconque]

        Le théorème \ref{ThoAJFJooAveRvY} nous dit que \( n=2^lm\) où \( m\) est impair. L'équation \( z^{2^lm}=a+bi\) se résout en deux étapes en utilisant les cas déjà faits et l'égalité
        \begin{equation}
            z^{2^lm}=(z^m)^{2^l}.
        \end{equation}
    \end{subproof}
...
\begin{proof}
    Nous effectuons une preuve par l'absurde en supposant que le polynôme
    \begin{equation}
        \begin{aligned}
            f\colon \eC&\to \eC \\
        ...
        \end{aligned}
    \end{equation}
    n'a pas de racines dans \( \eC\). Nous notons
    \begin{equation}
        n=2^km
    \end{equation}
    où \( m\) est impair.

    Vu que \( \lim_{z\to \infty} f(z)=\infty\), il existe \( R\) tel que \( | f(z) |>| f(1) |\) pour tout \( z\) hors de \( B(0,R)\). Donc \( | f |\) restreinte à \( \overline{ B(0,R) }\) est une fonction à continue sur un compact qui admet donc un minimum. Et ce minimum est global. Soit \( z_0\) un\footnote{«un» et non «le» parce qu'il n'y a pas unicité a priori.} point de \( \eC\) où ce minimum est atteint.

    Nous posons \( g(z)=f(z_0+z)\) et nous définissons ses coefficients \( A_i\) par
    \begin{equation}
