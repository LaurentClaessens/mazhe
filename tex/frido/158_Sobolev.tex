...
\begin{lemmaDef}
    ...
    \end{equation}
    pour tout \( \varphi\in \swD(I)   \).

    Lorsqu'une telle fonction existe, elle est la \defe{dérivée faible}{dérivée faible} de \( f\).
\end{lemmaDef}
...
\begin{example}[La fonction de Heaveside n'a pas de dérivée faible]
    ...
    pour tout \( \varphi\in\swD(x>0)\). La proposition \ref{PropAAjSURG} nous dit que \( g=0\) presque partout sur \( \mathopen] 0 , \infty \mathclose[\).
        ...
        Par conséquent, \( \int_{\eR}g\varphi=0\) pour toute \( \varphi\in \swD(\eR)\).
\end{example}
<++>

\begin{remark}
    La dérivée faible ne doit pas être confondue avec la dérivée au sens des distributions définie en \ref{}. Nous avons donc trois notions distinctes de dérivation pour une fonction :
    \begin{itemize}
        ...
        \item
            ...
            \begin{subproof}
                ...
                \item [\( u\in H^1(I)\) avec \( u'=v\)]
                    Attendu le corollaire~\ref{CORooIIEAooNmbkTo} qui permet de permuter intégrale et limite dans \( L^2(I)\) et les limites que nous venons de prouver,
                    \begin{equation}
