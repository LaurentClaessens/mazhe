...
\begin{proof}
	Si \(f \colon E\to F  \) est linéaire, alors \( f(x)=\sim_{n=0}^{\infty}\alpha_n(x)\) avec \( \alpha_k(x)=0\) pour \( k\neq 1\) et \( \alpha_1(x)=f(x)\).
\end{proof}
...
\begin{theorem}[\cite{BIBooTIAZooFwCEtZ}]	\label{THOooSZHVooKSEmuI}
	Soient des espaces de Banach \( X\) et \( X\) ainsi qu'une application \(f \colon X\to T  \) satisfaisant
	\begin{enumerate}
		\item
		      \( f\) est de classe \( C^1\),
		\item
