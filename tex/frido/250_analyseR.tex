...
\begin{definition}[\cite{BIBooTIAZooFwCEtZ}]	\label{DEFooAIHMooKbWsBt}
	Soient deux espaces de Banach \( E\) et \( F\). Une application \(f \colon E\to F  \) définie sur un voisinage de \( x_0\in E\) est \defe{analytique}{analytique entre espaces de Banach} en \( x_0\) si elle peut être écrite sous la forme\footnote{Juste pour rappel, ce que nous notons ici \( \alpha_k(x-x_0)\) est ce qui serait noté \( \alpha_k(x-x_0)^k\).}
	\begin{equation}
...
\begin{theorem}[\cite{BIBooTIAZooFwCEtZ}]	\label{THOooHSXLooMCzTTD}
...
%-------------------------------------------------------
\subsection{Série de puissance dans \( \End(E)\)}
%----------------------------------------------------
...
\begin{proposition}	\label{PROPooCHTEooWWzSqK}
	Soit un espace de Banach \( E\). Soit \( T\) inversible et continu dans \( \End(E)\). Nous posons
	\begin{equation}
		\rho=\frac{1}{ \| T^{-1} \|}.
	\end{equation}
	Pour tout \( S\in S\in B\big( T,1/\| T^{-1} \| \big)\), nous avons :
	\begin{enumerate}
		\item
		      \( (T+S)\) est inversible.
		\item
...
\begin{proof}
	Nous commençons par considérer \( T=\id\). Pour cela nous posons
	\begin{equation}
		\begin{aligned}
			f\colon B(\mtu,1) & \to \End(E)                    \\
			A                 & \mapsto \sum_{k=0}^{\infty}A^k
		\end{aligned}
	\end{equation}
	où \( B(\mtu,1)\) est le boule de rayon \( 1\) pour la norme opérateur.
