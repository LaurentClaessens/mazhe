...
\begin{proof}
	...
	\begin{subequations}
		...
		\end{align}
	\end{subequations}
	Étant donné que toutes les normes sur \( \eR^n\) sont équivalentes (théorème \ref{ThoNormesEquiv}), il existe un \( s>0\) tel que
	\begin{equation}
		| x_1 |+\ldots+| x_n |<s\| x \|,
	\end{equation}
	et donc tel que (utilisation de \eqref{EQooZTKGooFeKFXO})
	\begin{equation}
		\| \sum_{k=N+1}^M\sum_{\alpha\in \Lambda_k\setminus K}a_{\alpha}x^{\alpha} \|
		\leq \sum_{k=N+1}^M\| a_k \|\big( | x_1 |+\ldots+| x_n | \big)^k
		\leq s\sum_{k=N+1}^M\| a_k \|\| x \|^k\leq s\epsilon.
	\end{equation}
	Avec ça nous continuons \eqref{SUBEQSooUJGQooXuWdDr} :
	\begin{equation}
		\| \sum_{\alpha\in K}a_{\alpha}x^{\alpha}-S \|\leq \epsilon(s+1).
	\end{equation}
