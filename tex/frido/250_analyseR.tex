...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooGCLEooBVPHcu}
	Soit un espace de Banach \( E\). Si \( A\in \End(E)\) et si \( \alpha\in\eC \), il existe
	\begin{equation}
		\alpha\in\aL_n\big( \End(E),\End(E) \big)
	\end{equation}
...
\begin{proof}
	...
	Pour prouver que \( \alpha\) est continue, nous utilisons la proposition \ref{PROPooDQBOooByBvmj}.
\end{proof}
