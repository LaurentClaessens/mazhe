...
% Copyright (c) 2011-2017, 2019, 2021-2023, 2026
%   Laurent Claessens
...
\begin{theorem}     \label{THOooQJSRooMrqQct}
	...
	\begin{enumerate}
		...
		\item	\label{ITEMooLJAXooZeSzor}
		      L'application \(\psi \colon T(n)\times_{\rho} \gO(n)\to \Isom(\eR^n)  \) est un isomorphisme et en particulier
		      \begin{equation}
			      \psi\big( (v,A)\cdot (w,B) \big)=\psi(v,A)\circ\psi(w,B)
		      \end{equation}
		      pour tout \( v,w\in T(n)\) et \( A,B\in \gO(n)\).
		\item     \label{ITEMooSKUPooBDvNWX}
		      Une isométrie de \( \eR^n\) est une application affine\footnote{Définition \ref{DEFooUAWZooXcMKve}.}.
		\item     \label{ITEMooBZJHooWCPkfM}
		      Une isométrie de \( \eR^n\) qui fixe \( 0\) est dans \( \gO(n)\).
...
\begin{proof}
	Point par point.
	\begin{subproof}
		\spitem[Pour \ref{ITEMooLLUIooIGsknv}]
		%-----------------------------------------------------------
		...
		\begin{subproof}
			...
			\spitem[\( \psi\) est un morphisme]		\label{SPITEMooWJPAooGOmSsd}
		...
		\spitem[Pour \ref{ITEMooLJAXooZeSzor}]
		%-----------------------------------------------------------
		Le fait que ce soit une bijection et un morphisme vient d'être démonté dans \ref{ITEMooLLUIooIGsknv} et \ref{SPITEMooWJPAooGOmSsd}.

		\spitem[Pour \ref{ITEMooSKUPooBDvNWX}]
		%-----------------------------------------------------------
		...
		\spitem[Pour \ref{ITEMooBZJHooWCPkfM}]
		%-----------------------------------------------------------
		Soit \( g\in\Isom(\eR^n)\) tel que \( g(0)=0\). Nous avons \( v\in \eR^n\) et \( \Lambda\in\gO(n)\) tels que \( g=\psi(v,\Lambda)\). En l'appliquant à \( 0\) nous trouvons :
		\begin{equation}
			\psi(v,\Lambda)(0)=(\tau_cv\circ\Lambda)(0)=\tau_v(0)=v.
		\end{equation}
		Donc \( g(0)=0\) implique \( g=\psi(0,\Lambda)=\Lambda\in\gO(n)\).


		\spitem[Pour \ref{ITEMooQLNPooSyHaps}]
		%-----------------------------------------------------------
...
\begin{proposition}[\cite{MonCerveau,BIBooMistral}]	\label{PROPooOGHYooYgPkuE}
	Si \(f \colon \eR^2\to \eR^2  \) est une isométrie avec \( a\in \eR^2\) comme unique point fixe, alors
	\begin{equation}
		f=\tau_{a}\circ A\circ \tau_{-a}
	\end{equation}
	où \( A\) est une isométrie de \( (\eR^2,\| . \|)\) ayant \( 0\) comme unique point fixe.

	De plus avec l'application \(\psi \colon T(n)\times \gO(n)\to \Isom(\eR^n)  \) du théorème \ref{THOooQJSRooMrqQct}, nous avons \( A=\psi^{-1}(f)_2\)
\end{proposition}

\begin{proof}
	Nous posons \( A=\tau_{-a}\circ f\circ \tau_a\). Nous utilisons la décomposition du théorème \ref{THOooQJSRooMrqQct} :
	\begin{equation}
		f(x)=\Lambda(x)+v
	\end{equation}
	avec \( \Lambda\in \gO(2)\) et \( v\in \eR^2\). En posant \( x=a\) nous avons
	\begin{equation}
		a=f(a)=\Lambda(a)+v,
	\end{equation}
	donc \( v=a-\Lambda(a)\) et
	\begin{equation}
		f(x)=\Lambda(x)+a-\Lambda(a).
	\end{equation}
	Nous avons
	\begin{subequations}
		\begin{align}
			A(x) & =f(x+a)-a                       \\
			     & =\Lambda(x+a)+a-\Lambda(a)-a    \\
			a=\Lambda(x)+\Lambda(a)+a-\Lambda(a)-a \\
			     & =\Lambda(x).
		\end{align}
	\end{subequations}
	Nous avons prouvé que \( A=\Lambda\).

	Nous devons encore prouver que l'unique point fixe de \( \Lambda\) est \( 0\). Supposons que \( u\) soit un point fixe de \( \Lambda\). Nous avons
	\begin{subequations}
		\begin{align}
			f(u+a) & =\Lambda(u+a)+a-\Lambda(a)          \\
			       & =\Lambda(u)+\Lambda(a)+a-\Lambda(a) \\
			       & =u+a.
		\end{align}
	\end{subequations}
	Donc \( u+a\) est point fixe de \( f\). Donc \( u+a=a\) et donc \( u=0\). Cela prouve que si \( \Lambda\) a un point fixe, alors c'est \( 0\). Évidemment \( 0\) est un point fixe de \( \Lambda\) parce que \( \Lambda\) est linéaire.
\end{proof}


%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Réflexion par rapport à un hyperplan}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++


\begin{lemmaDef}[Réflexion par rapport à un hyperplan]		\label{LEMooPXIOooSlMIIY}
	Soit un espace euclidien\footnote{Vectoriel avec produit scalaire, définition \ref{DefLZMcvfj}.} \( E\). Soit un hyperplan affin\footnote{Définition \ref{DefLZMcvfj}.} \( H\) de \(E\). Nous notons \( H_0\) l'hyperplan vectoriel parallèle à \( H\). Il existe une unique application \(\sigma_H \colon E\to E  \) telle que
	\begin{enumerate}
		\item
		      \( \big( \sigma_H(x)-x \big)\cdot u=0\) pour tout \( u\in H_0\).
		\item
		      Pour tout \( x\in E\),
		      \begin{equation}
			      \frac{ \sigma_H(x)+x }{2}\in H
		      \end{equation}
	\end{enumerate}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooVQOTooUEGYiY}
	Soit un espace euclidien \( E\). Soit un hyperplan \( H\) et \( a\in E\). Si \( \sigma_H\) est la réflexion\footnote{Définition \ref{LEMooPXIOooSlMIIY}.} par rapport à \( H\), alors la réflexion par rapport à \( H+a\) est donnée par
	\begin{equation}
		\sigma_{H+a}=\tau_a\circ\sigma_H\circ\tau_{-a}.
	\end{equation}
\end{proposition}

\begin{proof}
	Nous posons \( \alpha=\tau_a\circ\sigma_H\circ\tau_{-a}\) et nous prouvons que \( \alpha\) vérifie les deux propriétés de la définition \ref{LEMooPXIOooSlMIIY}.
	\begin{subproof}
		\spitem[Si \( H\) est vectoriel]
		%-----------------------------------------------------------
		Nous commençons par supposer que \( H\) est un hyperplan vectoriel. Soient \( u\in H\) et \( x\in E\). Nous avons
		\begin{subequations}
			\begin{align}
				\big( \alpha(x)-x \big)\cdot u & =\big( \sigma_H(x-a)+a-x \big)\cdot u   \\
				                               & =\big( \sigma_H(x-a)-(x-a) \big)\cdot u \\
				                               & =0
			\end{align}
		\end{subequations}
		parce que \( \sigma_H\) est une réflexion.

		Nous considérons ensuite (grâce à la proposition \ref{PROPooHLRRooHWtHSR}) un \( n\in E\) tel que
		\begin{equation}
			H=\{ x\in E\tq x\cdot n=0 \}.
		\end{equation}
		Nous posons
		\begin{equation}
			m=\frac{ \alpha(x)+x }{ 2 },
		\end{equation}
		et nous devons prouver que \( m\in H+a\), c'est-à-dire que \( m-a\in H\) ou encore \( (m-a)\cdot n=0\). Nous calculons donc
		\begin{subequations}
			\begin{align}
				\frac{ \alpha(x)-x-2a }{ 2 }\cdot n & =\frac{ a+\sigma_H(x-a)+x-2a }{2}\cdot n \\
				                                    & =\frac{ \sigma_H(x-a)+(x-a) }{2}\cdot n  \\
				                                    & =0
			\end{align}
		\end{subequations}
		parce que c'est la propriété de \( \sigma_H\) appliquée au point \( x-a\).

		\spitem[Si \( H \) n'est pas vectoriel]
		%-----------------------------------------------------------
		Soit \( h\in H\). L'hyperplan \( H_0=H-h\) est vectoriel, et \( H+a=H_0+(h+a)\). Donc
		\begin{equation}
			\sigma_{H+a}=\sigma_{H_0+(h+a)}=\tau_{h+a}\circ\sigma_{H_0}\circ \tau_{-h-a}=\tau_a\circ\underbrace{\big( \tau_h\circ\sigma_{H_0}\circ\tau_{-h} \big)}_{=\sigma_H}\circ\tau_{-a}=\tau_a\circ\sigma_H\tau_{-a}.
		\end{equation}
	\end{subproof}
\end{proof}
...
La proposition suivante dit que si la composée de deux réflexions est une isométrie, alors les axes sont des droites sécantes.
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooXMPPooJbuJiw}
	Soient deux droites parallèles \( l_1,l_2\) dans \( \eR^2\). Nous notons \( \sigma_i\) la réflexion\footnote{Définition \ref{LEMooPXIOooSlMIIY}.} d'axe \( l_i\). Si \( \sigma_1\circ\sigma_2\) est une isométrie, alors \( l_1=l_2\).
\end{proposition}

\begin{proof}
	Soit \( l_0\) la droite parallèle à \( l_i\) passant par \( 0\) (existe par la proposition \ref{PROPooPWNWooYuyrOc}\ref{ITEMooFWIWooBBORjU} et c'est bien une parallèle commune par \ref{PROPooADJNooMyXUxG}\ref{ITEMooXUBUooJoEKRi}). Soit \( u\in l_0\). Nous considérons \( b\in \eR^2\) tel que \( \sigma_1(0)=2b\). Nous avons donc \( b\in l_1\) et \( b\cdot u=0\).

	Étant donné que la droite \( D=\{ \lambda b \}_{\lambda\in \eR}\) intersecte \( l_1\) (pour \( \lambda=1\)), elle intersecte aussi \( l_2\) (proposition \ref{PROPooADJNooMyXUxG}\ref{ITEMooOSSTooCzrUeT}). Donc il existe \( \mu\in\eR\) tel que \( \lambda b\in l_2\).

	Le point \( y=(\sigma_2\circ\sigma_1)(0)\in \eR^2\) est l'unique point tel que
	\begin{subequations}
		\begin{numcases}{}
			(y-2b)\cdot u=0\\
			\frac{ y+2b }{ 2 }\in l_2.
		\end{numcases}
	\end{subequations}
	Nous le cherchons sous la forme \( y=\lambda b\). D'abord \( (\lambda b-2b)\cdot u=0\) parce que \( b\cdot u=0\). Ensuite en posant \( \lambda=2(\mu-1)\), la seconde condition est vérifiée. Bref,
	\begin{equation}
		(\sigma_2\circ\sigma_1)(0)=2(\mu-1)b.
	\end{equation}

	Pour que la composée soit une isométrie il faut donc que \( (\mu-1)b=0\). Il y a deux possibilités: \( \mu=1\) et \( b=0\).
	\begin{subproof}
		\spitem[Si \( \mu=1\)]
		%-----------------------------------------------------------
		Dans ce cas \( b\in l_2\) et donc \( b\in l_1\cap l_2\). Les droites \( l_1\) et \( l_2\) sont donc confondues (proposition \ref{PROPooADJNooMyXUxG}\ref{ITEMooSXVYooBUvvxj}).

		\spitem[Si \( b=0\)]
		%-----------------------------------------------------------
		Dans ce cas \( \sigma_1(0)=0\), et donc si \( l_2\neq l_1\) nous avons \( 0\not\in l_2\) et donc \( (\sigma_2\circ\sigma_1)(0)\neq 0\). Dans ce cas \( \sigma_1\circ\sigma_2\) n'est pas une isométrie.
	\end{subproof}
\end{proof}
...
\begin{proof}
	Nous commençons par prendre une base quelconque \( \{ b_1,\ldots,b_{n-1} \}\) de \( H\). Le lemme \ref{LEMooFRWKooVloCSM} nous permet de considérer un vecteur \( e_n\) perpendiculaire à tous les \( b_i\). Donc \( e_n\perp H\).

	Ensuite nous utilisons Gram-Schmidt \ref{PropUMtEqkb} sur la base \( \{ b_1,\ldots,b_{n-1} \}\) de \( H\). Il en sort une base orthonormée \( \{ e_1,\ldots,e_{n-1} \}\) de \( H\).
\end{proof}
...
\begin{proof}
	Nous commençons par l'identité. Pour \(\id \colon \eR^n\to \eR^n  \), nous avons \( \psi^{-1}(\id)=(0,\id)\) et donc \( \epsilon(\id)=\det\big( \psi^{-1}(\id)_2 \big)=\det(\id)=1\).

	En ce qui concerne le produit, nous considérons \( f,g\in\Isom^+(\eR^n)\) ainsi que \( v,w\in T(n)\) et \( A,B\in \gO(n)\) tels que \( f=\psi(v,A)\) et \( g=\psi(w,B)\). Le théorème \ref{THOooQJSRooMrqQct}\ref{ITEMooLJAXooZeSzor} nous donne
	\begin{equation}
		f\circ g=\psi(v,A)\circ\psi(w,B)=\psi\big( (v,A)\cdot (w,B) \big).
	\end{equation}
	Donc \( \psi^{-1}(f\circ g)=(v,A)\cdot (w,B)=(\ldots, A\circ B)\). La première composante ne nous intéresse pas. Au final
	\begin{equation}
		\epsilon(f\circ g)=\det\big( \psi^{-1}(f\circ g)_2 \big)=\det(A\circ B)=\det(A)\det(B)=1.
	\end{equation}
\end{proof}
