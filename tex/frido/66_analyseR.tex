...
Autrement dit, la fonction \( f \) est dérivable dans la direction 
...
\begin{theorem}[\cite{MonCerveau}]     \label{THOooKBTYooWFtoSF}
    Soient :
    \begin{enumerate}
        \item
            trois espaces vectoriels normés \( U\), \( V\) et \( W\) de dimension fine
        \item
            \( a\in V\), un voisinage \( \mO\) de \( a\)
        \item
            des applications \( g\colon \mO\to U\) et \( f\colon f(\mO)\to W\)
        \item
            des bases de \( U\), \( V\) et \( W\) de telle sorte que parler des dérivées partielles ait un sens.
    \end{enumerate}
    Nous supposons que
    \begin{enumerate}
        \item
            \( f\) est de classe \( C^1\) en \( g(a)\),
        \item
            toutes les dérivées partielles \( (\partial_kg)(a)\) existent.
    \end{enumerate}
