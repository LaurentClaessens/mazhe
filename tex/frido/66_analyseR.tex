...
\begin{proof}
	Il y a deux possibilités : soit \( f\) est bornée, soit elle n'est pas bornée. Si \( f\) est bornée, c'est la proposition \ref{PropMTmBYeU}. Si \( f\) n'est pas bornée, nous allons montrer que \( \ell=\infty\) est la limite.

	Soit \( M>0\). Étant donné que \( f\) n'est pas bornée, il existe un \( x_M\in \eR \) tel que \( f(x_M)>M\). Mais comme \( f\) est croissante, pour tout \( x>x_M\) nous avons \( f(x)>M\). Cela prouve que \( \lim_{x\to \infty}f(x)=\infty\).
\end{proof}
