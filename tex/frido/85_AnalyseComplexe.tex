...
\begin{theorem}[Série de Laurent\cite{BIBooUBUAooHyhrlg}]       \label{THOooMKJOooVghZyG}
    ...
    \end{equation}
    et une fonction \( f\) holomorphe dans cette couronne. Alors il existe une unique suite \( (a_n) \) dans \( \eC\) telle que
    \begin{equation}
...
\begin{proof}
    Pour \( r\in\mathopen] r_1 , r_2 \mathclose[\) nous posons
        ...
        \begin{equation}
            f_s(t)=\sum_{n\in \eZ}c_n(s) e^{int}
        \end{equation}
        avec
        \begin{equation}
            c_n(s)=\frac{1}{ 2\pi }\int_0^{2\pi}f_s(t) e^{-int}dt.
        \end{equation}
        <++>
\end{proof}
<++>
