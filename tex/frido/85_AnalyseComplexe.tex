...
%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
\subsubsection{Une définition possible}
%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    où \( U=\eR^2\setminus\{ (x,0)\tq x\leq 0 \}\). Nous devons vérifier que \( F\) est différentiable et que sa différentielle en un point de \( U\) est une similitude.
    ...
    \begin{subproof}
        ...
        \item[\( \partial_xv(0,y)\), \( y\neq 0\)]

            Note : \( y\neq 0\). Nous écrivons la définition :
            \begin{equation}
                \frac{ \partial v }{ \partial x }(0,y)=\lim_{\epsilon\to 0}\frac{ v(\epsilon,y)-v(0,\epsilon) }{ \epsilon }.
            \end{equation}
            Pour étudier cette limite (et en particulier pour savoir si elle existe) nous séparons la limite à gauche et à droite. Nous commençons par \( \epsilon\to 0^+\); dans ce cas nous avons \( v(\epsilon,y)=\arctan(y/\epsilon)\) et \( v(0,\epsilon)=\pi/2\).

            Nous en sommes donc à étudier la limite
            \begin{equation}
                \lim_{\epsilon\to 0^+}\frac{ \arctan(y/\epsilon)-\pi/2 }{ \epsilon }.
            \end{equation}
            Et pour montrer que cette limite existe et vaut \( -1/y\) nous allons montrer que la limite simple existe et vaut cette valeur. Pour ce faire, rien de tel que la règle de l'Hospital parce que la dérivée d'\( \arctan\) est déjà connue du théorème \ref{THOooUSVGooOAnCvC}\ref{ITEMooMNHLooOVhIIb}. Nous avons
            \begin{equation}
                \lim_{\epsilon\to 0}\frac{ \arctan(y/\epsilon)-\pi/2 }{ \epsilon }=\lim_{\epsilon\to 0}\frac{ 1 }{ 1+\frac{ y^2 }{ \epsilon^2 } }\times \frac{ -y }{ \epsilon^2 }=-\frac{ 1 }{ y }.
            \end{equation}
            Notons qu'il faut comprendre la première égalité comme «si ça existe à droite, alors ça existe à gauche et c'est égal».

            En ce qui concerne la dérivée à gauche (\( \epsilon\to 0^-\)), \( v(0,\epsilon)=-\pi/2\) et \( \arg(\epsilon,y)=\pm\pi+\arctan(y/\epsilon)\) où le signe dépend du signe de \( y\) (voir \eqref{EQooPJVFooSEKTny}). La règle de l'Hospital demandant de dériver par rapport à \( \epsilon\), le calcul se révèle être exactement le même que celui effectué plus haut. Donc nous avons prouvé que
            \begin{equation}        \label{EQooMSPYooPFmXOc}
                \frac{ \partial v }{ \partial x }(0,y)=-\frac{1}{ y }.
            \end{equation}

        \item[\( \partial_yv(x,y)\), \( x>0\)]

            Dans la zone \( x>0\) nous avons \( v(x,y)=\arctan(y/x)\) et donc
            \begin{equation}        
                \frac{ \partial v }{ \partial y }(x,y)=\frac{ x }{ x^2+y^2 }
            \end{equation}
            où nous avons utilisé une dérivation en chaîne et la dérivée d'arc tangente.

        \item[\( \partial_xv(x,y)\), \( x<0\) et \( y>0\)]
            Dans cette zone, \( v(x,y)=\arg(x+iy)=\pi+\arctan(y/x)\). La dérivée se fait sans problèmes et
            \begin{equation}\label{EQooESNZooGCszri}
                \frac{ \partial v }{ \partial x }(x,y)=\frac{ -y }{ x^2+y^2 }.
            \end{equation}
        \item[\( \partial_xv(x,y)\), \( x<0\) et \( y<0\)]
            C'est la même formule avec \( -\pi\) au lieu de \( \pi\). Et donc même calcul.
    ...
    \begin{equation}
        \frac{ \partial v }{ \partial x }(x,y)=\frac{ -y }{ x^2+y^2 }
    \end{equation}
    pour \( x<0\) et \( y\neq 0\). Les dérivées partielles en \( x<0\) avec \( y=0\) ne nous intéressent pas parce que nous savons déjà que la fonction n'est pas continue sur cette zone\footnote{Faites le calcul de \( \partial_yv(x,0)\) en séparant les limites à gauche et à droite et vous verrez qu'elles ne sont pas égales.}.

    Les quelque dérivées partielles suivantes sont faciles à calculer pour \( x>0\) :

    \begin{equation}
        \begin{aligned}[]
            \frac{ \partial u }{ \partial x }=\frac{ x }{ x^2+y^2 },&&\frac{ \partial v }{ \partial y }=\frac{ x }{ x^2+y^2 }\\
            \frac{ \partial u }{ \partial y }=\frac{ y }{ x^2+y^2 },&&\frac{ \partial v }{ \partial x }=\frac{ -y }{ x^2+y^2 }.
        \end{aligned}
    \end{equation}

    La limite de \eqref{EQooESNZooGCszri} lorsque \( x\to 0\) donne bien \eqref{EQooMSPYooPFmXOc}, ce qui donne la continuité de cette dérivée partielle.
...
\begin{remark}
    ...
    D'autres personnes pourraient vous dire que notre logarithme «n'est pas bien définit parce que si on fait le tour dans un sens ou dans l'autre nous n'obtenons pas la même valeur pour \( \ln(z)\) lorsque \( z\) est sur \( U\)». Et cela avec des arguments aussi forts que «\( 2\pi\) et \( 0\), c'est le même point».

    Nous préférons être bien clairs sur ce point : notre fonction \( \ln\) est parfaitement définie sur \( \eC^*\) et \( 2\pi\) n'est pas la même chose que zéro. En particulier \( \arg( e^{2i\pi})=0\) et \(  e^{-i\pi}=\pi\) et non \( \pi\).
\end{remark}

De toutes les déterminations possible du logarithme, celle que nous avons choisie (un peu arbitrairement) pour s'appeler «logarithme» et accaparer de la notation «\( \ln\)» est \( \ln_{\pi^+}\). Elle est d'une certaine manière celle qui arrive le plus naturellement.

En effet si nous pensons au logarithme népérien \( \ln\colon \eR^+\to \eR\) que nous voulons prolonger sur \( \eR\), nous devons poser
\begin{equation}
    \ln(-x)=\ln(-1)+\ln(x)
\end{equation}
pour \( x>0\). Que peut valoir \( \ln(-1)\) ? Il doit vérifier \(  e^{\ln(-1)}=-1\). La première valeur qui nous tombe sous la main est \( \ln(-1)=\pi\). Bien entendu, d'autres possibilités étaient possibles, comme \( \ln(-1)=2017\pi\).

%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
\subsubsection{Pas d'unicité : développement en série}
%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

Pour \( z_0\in \eC^*\) nous pouvons écrire un développement en série de la réciproque de l'exponentielle autour de \( z_0\).
