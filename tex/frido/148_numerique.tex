...
% Copyright (C) 2010-2013,2016,2018-2019, 2022-2023
%   Laurent Claessens
...
\begin{enumerate}
	\item
	      Dans le cas de la bissection, nous obtenons toujours \( p_i=1\).
	\item
...
Si nous n'avons pas de formule analytique pour \( f\), mais seulement la possibilité de calculer \( f(x)\) pour tout \( x\). Newton ne fonctionne pas, mais la bissection fonctionne.
...
Psychologiquement, on est tenté de prendre \( x_0\) et \( x_1\) de part et d'autre de \( \alpha\) (pensant à la bissection), mais en réalité ce n'est pas obligatoire du tout et n'a aucune influence. Il faut seulement les prendre très proches de \( \alpha\).
...
\begin{enumerate}
	...
	\item
	      Un problème sous la forme \( f(x)=0\) peut utiliser des méthodes de bissection, Newton ou autres.
\end{enumerate}
...
La majorité des méthodes numériques de résolution d'équations du type \( f(x)=0\) ou \( x=g(x)\) seront sous la forme de suites. Avec questions à la clef :
\begin{enumerate}
	\item
...
Pour démarrer une bissection, il est toujours bon de prendre l'intervalle \( \mathopen[ a , b \mathclose]\) de façon à ne contenir qu'une seule solution.
...
Le test d'arrêt de la méthode de bissection se base uniquement sur la taille de l'intervalle qui reste. En effet si nous avons
\begin{equation}
	| b_n-a_n |\leq \epsilon
\end{equation}
...
\begin{enumerate}
	...
	\item
	      ...
	      En Python 3, le symbole \info{/} désigne bien la division usuelle, mais Sage utilise Python 2\footnote{Cela était vrai au moment d'écrire l'exemple. Aujourd'hui, Sage utilise la version 3. Il n'y a aucune raison d'importer \info{future}.}.
	\item
	...
	\item
	      Nous faisons \info{amp=toll+1} parce que nous voulons absolument lancer le cycle au moins une fois. Sinon, le \info{x} à retourner ne serait pas défini au moment de sortir du cycle (si le cycle n'est pas exécuté).
	\item
	...
	\item
	      ...
	      Il peut cependant avoir une fonction telle que \( f(1)=10^{-50}\) et \( f(2)=0\). L'algorithme de bissection risque de s'arrêter si \( x_n=1\). Parce que la machine risque de calculer \( f(x_n)=0\).
...
\begin{normaltext}
	Notons l'importance de la continuité de \( f\). Par exemple que ferait la bissection sur la fonction \( f(x)=1/x\) pour l'intervalle \( \mathopen[ -3 , 1 \mathclose]\) ?
...
\begin{normaltext}
	Dans le monde réel, il arrive souvent d'utiliser une méthode de bissection pour se donner un point de départ pour une autre méthode.
\end{normaltext}
...
\begin{example}[Bissection]
	Pour la méthode de bissection, nous avons \( s=1\) parce que chercher \( x_{n+1}\), il faut seulement calculer \( f(x_n)\).
\end{example}
...
\begin{example}
	...
	Nous avons donc \( x\oplus y=x\). Le nom de l'erreur qui consiste à avoir \( x\oplus y=x\) est \defe{relation anormale}{relation anormale}.
...
\begin{example}
	...
	\begin{enumerate}
		...
		\item
		      Effectuer une bissection pour la savoir.
		\item
		      Sachant que
		      \begin{equation}
			      \alpha\simeq 1.358500220734946,
		      \end{equation}
		      quelle est l'erreur relative\footnote{Définition \ref{DEFooRUHVooGgoyiP}.} ?
	\end{enumerate}
	...
	Note: ce serait une très intéressante question de savoir prouver qu'il y a exactement \( 3\) racines et qu'elles sont dans ces intervalles\quext{Si vous avez une idée de comment faire, écrivez-moi.}.

	La plus grande solution est \( \alpha_2\). Pour le reste, Python fait très bien le boulot.

	\lstinputlisting{tex/frido/codeSnip_6.py}

	Au bout de 10 itérations, on trouve ceci comme estimation : \( \hat \alpha = 1.358642578125\). En ce qui concerne l'erreur relative, elle vaut
	\begin{equation}
		| \frac{ \alpha-\hat \alpha }{ \alpha } |\simeq 0.00010479.
	\end{equation}

	Pour être propre, vous devriez modifier le programme pour qu'il vous calcule l'erreur relative à chaque étape.
\end{example}
...
\begin{definition}
	Soient \( n+1\) nœuds \( x_i\) pour la fonction \( f\). Les \defe{différences divisées}{différence!divisée} sont :
	\begin{description}
		\item[Ordre \( 0\)]
		      \begin{equation}
			      f[x_i]=f(x_i)
