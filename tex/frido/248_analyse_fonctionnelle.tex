...
\begin{definition}[Hyperplan qui sépare]		\label{DEFooQUDEooMrgcCV}
	Soit \( E\) un espace vectoriel topologique ainsi que \( A\), \( B\) des sous-ensembles de \( E\).
	\begin{enumerate}
		\item
...
\begin{theorem}[Hahn-Banach, seconde forme géométrique, cas réel] \label{ThoACuKgtW}
	Soient un espace vectoriel topologique réel localement convexe\footnote{Définition~\ref{DefPJokvAa}.} \( E\) ainsi que des parties \( A\), \( B\) qui sont
	\begin{enumerate}
		\item
		      des convexes
		\item
...
\begin{theorem}[Hahn-Banach, seconde forme géométrique, cas complexe\cite{BIBooQEQMooIzsqzA}] \label{THOooEOOYooPdTGLi}
	Soit un espace vectoriel topologique \( X\) sur \( \eC\). Soient \( A,B\subset X\) tels que
	\begin{enumerate}
		\item
		      disjoints,
		\item
		...
		\item
		      convexes,
		\item
		      \( A\) est compact dans \( X\),
		\item
		      \( B\) est fermé dans \( X\).
	\end{enumerate}
	Alors il existe \( \phi\in X'\) et \( \alpha>0\) tels que
...
\begin{proof}
	L'espace \( X\) est complexe. Nous considérons \( E\) l'espace réel correspondant (proposition \ref{PROPooAIMVooYtZVJV}). Nous lui appliquons la version réelle de Hahn-Banach (théorème \ref{ThoACuKgtW}) : il existe \( \alpha\in \eR\) et une application linéaire \(f \colon X\to \eR  \) telle que
	\begin{equation}
		f(a)<\alpha<f(b)
	\end{equation}
