...
\begin{lemma}[\cite{BIBooMGFTooLkfVxy}]
	...
	\begin{enumerate}
		...
		\item		\label{ITEMooFIZBooDnYKSO}
		      Nous avons égalité des normes\quext{Pas encore démontrée\ldots}
		      \begin{equation}
			      \| f \|_{\aL_\eC(X,\eC)}=\| u \|_{\aL_\eR(X,\eR)}.
		      \end{equation}
		      %TODOooQANSooOQgkAu. Prouver ça. C'est à dire ITEMooFIZBooDnYKSO
	\end{enumerate}
...
\begin{proposition}[\cite{TQSWRiz}]	\label{PROPooSKBXooQDOmcL}
	Soit un espace vectoriel topologique \( E\). Soit un ouvert connexe non vide \( C\subset E\). Soit \( x_0\not\in C\). Alors il existe \( f\in E'\)\footnote{\( E'\) est l'ensemble des applications linéaires continues sur \( E\) à valeurs dans \( \eR\).} tel que \( f(x)<f(x_0)\) pour tout \( x\in C\).
\end{proposition}
