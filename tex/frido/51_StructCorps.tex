...
\begin{proof}
    ...
    \begin{equation}
        X^4+1\in \eF_p[X]
    \end{equation}
    et \( \alpha\), une racine dans une extension\quext{Dans la source que je suivais (je ne sais plus où), on parlait ici de «fermeture» de \( \eF_pj\) et non d'exetension. Il me semble que parler simplement d'extension suffit. Vous confirmez ?} de \( \eF_p\)\footnote{Voir par exemple la proposition \ref{PROPooDPOYooFHcqkU} pour l'existence d'une extension comme il faut.}. Nous posons \( \theta=\alpha+\alpha^{-1}\) et nous calculons
    \begin{equation}
...
% Ce théorème est ici, et ne devrait bouger que sous grand contrôle. En effet, il existe de nombreuses démonstrations, faisant appel à de nombreuses choses. Entre autres :
% - par le théorème des valeurs intermédiaures
% - par l'analyse complexe
% - de façon «directe», en utilisant tout de même la forme trigonométrique des nombres complexes.
% Bref, les autres démonstrations feraient arriver ce théorème plus bas dans le Frido. Et risqueraient de poser de lourds problèmes d'organisation
% vu que les références ne peuvent être faites que vers le haut.
% Imaginez par exemple les conséquences de n'avoir pas le droit d'utiliser ce théorème avant la trigonométrie. Sachant que cette dernière est définie avec les séries entières \ldots
% Beaucoup de choses devraient être déplacées.

\begin{theorem}[Théorème de d'Alembert\cite{rqrNyg,ooPSLMooAVODjn}]     \label{THOooIRJYooBiHRyW}
