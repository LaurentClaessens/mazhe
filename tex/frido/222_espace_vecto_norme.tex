...
\begin{definition}[Application multilinéaire]       \label{DefFRHooKnPCT}
	Soient des espaces vectoriels \( V_1,\ldots,V_k\) ainsi qu'un espace vectoriel \( W\). Une application \(T \colon V^k\to W  \) est dite \defe{\( k\)-linéaire}{application!multilinéaire} si pour tout \( (v_1, \ldots,v_k)\in V_1\times \ldots\times V_k\) les applications
	\begin{equation}
		\begin{aligned}
			T_i\colon V_i & \to W                                                 \\
	...
	Si tous les espaces \( V_i\) sont sur le même corps \( \eK\), une \defe{forme multilinéaire}{forme multilinéaire} est une application multilinéaire \( V_1\times \ldots\times V_k\to \eK\).

	L'ensemble des applications \( k\)-multilinéaires \( V_1\times \ldots\times V_k\to W\) peut être noté \( \aL(V_1,\ldots,V_k,W)\), et l'ensemble des applications \( k\)-linéaires sur \( V\) à valeurs dans \( W\) est noté \( \aL_k(V,W)\).
\end{definition}
...
\begin{definition}\index{déterminant!forme linéaire alternée}       \label{DEFooYWOBooUGJojy}
	Soit \( E\), un \( \eK\)-espace vectoriel. Une forme multilinéaire \defe{alternée}{forme multilinéaire alternée} sur \( E\) est une application \( k\)-linéaire \( f\colon E^k\to \eK\) telle que \( f(v_1,\ldots, v_k)=0\) dès que \( v_i=v_j\) pour certains \( i\neq j\).
\end{definition}

\begin{definition}	\label{DEFooZFCKooGIoYrE}
	Soit \( E\), un \( \eK\)-espace vectoriel. Une forme multilinéaire \(f \colon E^k\to \eK  \) est \defe{antisymétrique}{formle multilinéaire antisymétrique} telle que
	\begin{equation}
