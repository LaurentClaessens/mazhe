...
\begin{enumerate}
	\item
	      La convergence absolue, définition \ref{DefVFUIXwU},			% FIPXKBKEWWSgGKEvxnHm -- ceci est pour rendre cette ligne unique.
	\item
	      la convergence normale, définition \ref{DefVBrJUxo}. C'est la même que la convergence absolue, mais dans le cas particulier d'un espace de fonctions muni de la norme uniforme.
	\item
	      la convergence uniforme, définition \ref{DEFooPABSooPMXMOV}.		% EYXLooOyDzXBoJFIMooQZPKAR -- pour rendre cette ligne unique.
\end{enumerate}
...
\begin{propositionDef}[Famille sommable]  \label{DefIkoheE}
	Si \( \{ v_i \}_{i\in I}\) est une famille de vecteurs dans un espace vectoriel normé indexée par un ensemble quelconque \( I\). Nous disons que cette famille est \defe{sommable}{famille!sommable} de somme \( v\) si pour tout \( \epsilon>0\), il existe un \( I_0\) fini dans \( I\) tel que pour tout ensemble fini \( K\) tel que \( I_0\subset K\) nous avons\footnote{La somme sur la partie finie \( K\) est la définition \ref{DEFooLNEXooYMQjRo}.}
	\begin{equation}
...
\begin{proposition}[Associativité\cite{MonCerveau}]	\label{PROPooHGXZooXHIHeh}
	Soit une famille sommable \( (u_i)_{i\in I}\) de réels positifs. Soit une partition (disjointe) \(  \{ I_j \}_{j\in J}  \) de \( I\). Nous posons\footnote{Existe par le lemme \ref{LEMooSBYEooNXzqJU}.}
	\begin{equation}
		S_j=\sum_{i\in I_J}u_i.
	\end{equation}
	Alors \( (S_j)_{j\in J}\) est sommable et
	\begin{equation}
		\sum_{i\in I}u_I=\sum_{j\in J}S_j=\sum_{j\in J}\left( \sum_{i\in I_j}u_i \right).
	\end{equation}
\end{proposition}

\begin{proof}
	Notons \( v=\sum_{i\in I}u_i\). Soit \( \epsilon>0\). Il nous faut un \( J_0\) fini dans \( J\) tel que si \( K\subset J\) est fini et \( J_0\subset K\) alors \( \| \sum_{j\in K}S_j-v \|\leq \epsilon\).

	Étant donné que \( (u_i)_{i\in I}\) est sommable, il existe \( I_0\) fini dans \( I\) tel que pour tout \( L\) fini dans \( I\) tel que \( I_0\subset K\), nous ayons \( \| \sum_{i\in L}u_i-v \|\leq \epsilon\).

	Nous posons
	\begin{equation}
		J_0=\{ j\in J\tq I_{j}\cap I_0\neq 0 \}.
	\end{equation}
	Cela est une partie finie de \( J\) parce que \( I_0\) est fini et les \( I_j\) sont disjoints. Si \( K\subset J\) est fini et vérifie \( J_0\subset K\), alors en posant \( L=\bigcup_{j\in K}I_j\) nous avons (avec la proposition \ref{PROPooJLQAooAEbIvZ}) :
	\begin{equation}
		\sum_{j\in K}S_j  =\sum_{j\in K}\sum_{i\in I_j}u_I  =\sum_{i\in L}u_i
	\end{equation}
	et donc
	\begin{equation}
		\| \sum_{j\in K}s_j-v \| = 	 	\| \sum_{i\in L}u_i-v \|<\epsilon.
	\end{equation}
\end{proof}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooNNMFooUHMcSE}
	Soit une suite \( (u_n)\) dans un espace de Banach telle que \( \sum_{k=0}^{\infty}u_k<\infty\). Alors
	\begin{equation}
		\lim_{p\to \infty}\sum_{k=p}^qu_k=0,
	\end{equation}
	plus précisément, pour tout \( \epsilon>0\), il existe \( N>0\) tel que si \( q\geq p\geq N\), alors
	\begin{equation}
		|\sum_{k=p}^q u_k |<\epsilon.
	\end{equation}
\end{proposition}

\begin{proof}
	Soit \( \epsilon>0\). La suite des sommes partielles
	\begin{equation}
		s_n=\sum_{k=0}^nu_k
	\end{equation}
	est convergente dans \( \eC\) et est donc une suite de Cauchy. Il existe donc \( N\in \eN\) tel que si \( p,q\geq N\), \( | s_p-s_p |<\epsilon\) et donc tel que
	\begin{equation}
		| \sum_{k=p+1}^qu_k |<\epsilon.
	\end{equation}
\end{proof}
...
\begin{proposition}[Critère de Cauchy uniforme pour les séries\cite{BIBooPCEDooMHkbnu}]	\label{PROPooEBYWooOXUwry}
	Soit un ensemble \( X\) et un espace vectoriel normé \( V\). Une suite \( (f_n)_{n\in \eN}\) d'applications \(f_n \colon X\to V  \) est uniformément sommable sur \( X\) si et seulement si pour tout \( \epsilon>0\), il existe un \( N\in \eN\) tel que si \( p>q\geq N\), alors pour tout \( x\in X\) nous avons
	\begin{equation}
