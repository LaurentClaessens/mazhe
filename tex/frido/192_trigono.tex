...
\begin{proof}
    ...
    \begin{subproof}
        ...
        Nous passons au cas \( (2,3,5)\), et comme ça va être long et douloureux\footnote{Mais pas autant que le théorème~\ref{THOooSTHXooXqLBoT}, cependant.}, nous sautons un niveau d'indentation.
                ...
                Tous les éléments de \( G\) sont dans un fixateur de type \( \Fix(x)\), et comme l'ordre d'un élément divise l'ordre du groupe (corolaire~\ref{CorpZItFX}), tous les éléments de \( G\) ont un ordre \( 2\), \( 3\) ou \( 5\). Nous sommes dans un cas très particuliers parce que
                \begin{itemize}
...
Pour information, la proposition~\ref{PROPooNBRBooEQBypy} nous fera la même limite sans développements.
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Quelque intégrales avec de la trigonométrie}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
La fonction \( x\mapsto \sin(x)/x\) est appelée \defe{sinus cardinal}{sinus cardinal}. Elle sert à plein de choses; nous allons montrer que la fonction \( x\mapsto | \sin(x)/x |\) a une intégrale sur \( \eR\) qui vaut \( \infty\). Cela nous permettra de donner un exemple d'une fonction dans \( L^1(\eR)\) dont la transformée de Fourier n'est pas dans \( L^1(\eR)\) (lemme \ref{LEMooROPHooOSguhN}).
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Elle est continue en zéro\footnote{Lemme \ref{LEMooZYNEooYkwsWD}.}.
...
\begin{proof}
    La définition de l'intégrale de Lebesgue pour une fonction de signe non constant est la définition \ref{DefTCXooAstMYl} qui nous demande de commencer par calculer l'intégrale des parties positives et négatives.

    La partie positive est :
    \begin{equation}
        f^+(t)=\begin{cases}
            \frac{ \sin(t) }{ t }    &   \text{si } t\in \mathopen[ 2k\pi , (2k+1)\pi \mathclose]\\
            0    &    \text{sinon }.
        \end{cases}
    \end{equation}
    Il est vite vu que l'intégrale de cette fonction sur \( \mathopen[ 0 , \infty \mathclose[\) vaut \( \infty\). En effet si \( I\subset \mathopen[ 0 , 2\pi \mathclose]\) est la partie sur laquelle \( \sin(t)>0.9\), les fonctions étagées
    \begin{equation}
        \psi_N(t)=\sum_{k=0}^n0.9\mtu_{I+2k\pi}
    \end{equation}
    minorent \( f^+\). L'intégrale de \( \psi_N\) vaut \( N\) fois la mesure de \( I\). Donc le supremum sur les \( N\) vaut \( \infty\).
\end{proof}
...
\begin{lemma}[\cite{BIBooCFXJooWrArNT}]
    La limite
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    où \( N(b)\) est un entier bien choisit. En tout cas \( \lim_{b\to \infty}N(b)=\infty\).

    Sur l'intervalle \( \mathopen[ k\pi , (k+1)\pi \mathclose]\), nous avons \( \sin(t)=(-1)^k| \sin(t) |\). Nous pouvons toujours majorer \( | \sin(t) |\) par \( 1\).
\end{proof}
<++>
