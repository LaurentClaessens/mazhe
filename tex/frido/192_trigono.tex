...
\begin{proposition}
    Si \( G\) est un groupe de pavage de \( \eR^2\) et si \( f\colon \eR^2\to \eR^2\) est une bijection affine, alors le groupe
    \begin{equation}
        f\circ G\circ f^{-1}=\{f\circ g\circ f^{-1}\tq g\in G\}
    \end{equation}
    est un groupe de pavage.
\end{proposition}

\begin{proof}
    Soit un compact \( K\) tel que \( (G,K)\) soit un pavage. Nous notons \( K'=f(K)\). Vu que \( f\) est affine, \( K'\) est encore compact\footnote{Il suffit de prouver que \( f(K)\) est fermé et borné par le théorème de Borel-Lebesgue \ref{ThoXTEooxFmdI}.} Nous avons :
    \begin{subequations}
        \begin{align}
...
\begin{center}
   \input{auto/pictures_tex/Fig_AUMZoodbKuHtXe.pstricks}
\end{center}

et

\begin{center}
   \input{auto/pictures_tex/Fig_CRHZooMcaRhrUh.pstricks}
\end{center}
...
\begin{lemma}[\cite{MonCerveau}]
    Si \( G\) est un groupe de pavage\footnote{Définition \ref{DEFooJPHKooRgCBJs}.} de \( \eR^2\), et si \( \alpha\colon \eR^2\to \eR^2\) est une bijection affine, alors le groupe \( G'=\alpha G\alpha^{-1}\) est également un groupe de pavage.
\end{lemma}

\begin{proof}
    Si \( G\) est un groupe de pavage de \( \eR^2\), il existe un compact \( K\subset \eR^2\) tel que 
    \begin{itemize}
        \item \( G\cdot K=\eR^2\);
        \item si \( g_1,g_2\in G\) satisfont \( g_1\cdot\Int(K)\cap g_2\cdot\Int(K)\neq \emptyset\), alors \( g_1\cdot K=g_2\cdot K\).
    \end{itemize}
    Cela est simplement la définition du fait que \( (G,K)\) est un pavage du plan.

    Montrons que \( \big( G',\alpha(K) \big)\) est également un pavage.
\end{proof}
<++>
