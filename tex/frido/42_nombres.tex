...
%-------------------------------------------------------
\subsection{Les théorèmes encapsulant}
%----------------------------------------------------
...
\begin{definition}\label{Def_applic_fct}
    Soit \(\ G\) une relation binaire entre deux ensembles \(E\) et \( F\). Si, pour tout \(x \in E\), et tous \(y,\ y' \in F\), les relations \(x\mR_G y\) et \(x\mR_G y'\) entraînent \( y = y'\), alors on peut associer à chaque \( x \in E\) apparaissant comme première composante dans \( G \), l'élément unique \(y \in F\) qui est en relation avec \(x\) dans \(G\).

    Cette association définit une \defe{fonction}{fonction} de \(E\) dans \(F\).

    Lorsque de plus, étant donné  \(x \in E\), il existe toujours un \( y \in F\) tel que \(x\mR_G y\), alors on parle d'\defe{application}{application}.
\end{definition}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooRHNFooHFUOEx}
	Soit une application surjective \(f \colon A\to B  \). Il existe une application \(\alpha \colon B\to A  \) telle que
	\begin{equation}
		\alpha(b)\in f^{-1}(b)
	\end{equation}
	pour tout \( b\in B\).
...
\begin{proof}
	...
	Et nous allons utiliser le lemme de Zorn\footnote{Lemme \ref{LemUEGjJBc}.}.
	\begin{subproof}
		\spitem[\( \mA\) est non vide]
		%-----------------------------------------------------------
		Nous considérons un élément \( a\in A\) et nous posons \( b=f(a)\). Alors en posant \( X=\{ b \}\) et \(\alpha \colon \{ b \}\to A  \) donné par \( \alpha(b)=a\), le couple \( (X,\alpha)\) est bien un élément de \( \mA\).

		\spitem[Involutif]
		%-----------------------------------------------------------
		Soit une partie \( \mF=\{ (X_i,\alpha_i) \}_{i\in I}\) totalement ordonnée de \( \mA\). Nous posons \( X=\bigcup_iX_i \) et \(\alpha \colon X\to A  \) par \( \alpha(b)=\alpha_i(b)\) dès que \( b\in X_i\).

		L'élément \( (X,\alpha)\) est dans \( \mA\) et majore \( \mF\).
	\end{subproof}
	Nous pouvons donc utiliser le lemme de Zorn. L'ensemble \( \mA\) contient un maximum \( (Y,\beta)\), et nous allons montrer que \( Y=B\).
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\( \subset\)]
		%-----------------------------------------------------------
		Soit \( a\in A\). Alors \( a\in A\subset B\), de telle sorte que \( f(a)\in f(B)\). Mais \( a\) n'est pas dans \( B\setminus A\), et comme \( f\) est injective, \( f(A)\cap f(B\setminus A)=\emptyset\). Donc, vu que \( f(a)\in f(A)\), nous avons \( f(a)\notin f(B\setminus A)\).

		\spitem[Dans l'autre sens]
		%-----------------------------------------------------------
		Suppose \( y\in f(B)\setminus f(B\setminus A)\). Vu que \( y\in f(B)\), il existe \( b\in B\) tel que \( y=f(b)\). Mais comme \( y\notin f(B\setminus A)\), nous avons \( b\notin B\setminus A\). Autrement dit, \( b\in A\) et donc \( f(b)\in f(A)\).
	\end{subproof}
\end{proof}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooFTFJooXUXOTY}
	Soit un anneau commutatif \( A\). Si \( a,b\in A\) alors :
	\begin{enumerate}
		\item
		      \( (-a)b=-ab\)
...
\begin{definition}[Anneau totalement ordonné\cite{MonCerveau}]	\label{DEFooWACWooDWvXKJ}
	Un anneau \( A\) est \defe{totalement ordonné}{ordre!dans un anneau}\index{corps!ordonné} si il existe une relation d'ordre total\footnote{Définition~\ref{DEFooVGYQooUhUZGr}.} tel que
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		Nous avons \( a\geq 0\). Nous ajoutons \( b\) des deux côtés pour avoir \( x+y\geq y\geq 0\).
	\end{subproof}
\end{proof}
...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		En posant \( y'=-y\) nous avons \( y'\geq 0\) (par la proposition \ref{PROPooELXCooCYzEVD}\ref{ITEMooRRUGooRUpYwt}). Nous avons alors \( xy'\geq 0\) par la définition \ref{DEFooWACWooDWvXKJ}\ref{CONDooBYYDooElXgPO}. Nous avons alors \( xy=-xy'\geq 0\).

		\spitem[Pour \ref{ITEMooDZUJooVWuLbD}]
		%-----------------------------------------------------------
		Nous posons \( a'=-a\) et \( b'=-b\). Par \ref{PROPooELXCooCYzEVD}\ref{ITEMooRRUGooRUpYwt} nous avons \( a',b'\geq 0\). Donc \( a'b'\geq 0\) par le point \ref{ITEMooNLAQooHtERZJ}. Mais \( a'b'=(-a)(-b)=ab\) par la proposition \ref{PROPooFTFJooXUXOTY}\ref{ITEMooHUJFooKkylVt}.

		\spitem[Pour \ref{ITEMooRGYAooCUIfss}]
		%-----------------------------------------------------------
		Prenons \( a\geq 0\). Supposons que \( 1\leq 0\). Alors nous aurions \( 1a\leq 0\) par le point \ref{ITEMooNLAQooHtERZJ}. Mais \( 1a=a\geq 0\). Donc \( 1> 0\).
...
\begin{proposition}[Valeur absolue\cite{MonCerveau}]	\label{DEFooJXKVooErANPh}
...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		C'est la définition et le fait que \( -x\neq 0\) lorsque \( x\neq 0\) par la proposition \ref{PROPooELXCooCYzEVD}\ref{ITEMooWNPVooFeMQAp}.
...
\begin{lemma}[Propriétés de la valeur absolue]  \label{LemooANTJooYxQZDw}
	Soit \( \eK\) un corps totalement ordonné muni de la valeur absolue usuelle\footnote{De la proposition \ref{DEFooJXKVooErANPh}.}. Si \( x,y\in \eK\) alors :
	\begin{enumerate}
...
\begin{theorem}[division euclidienne \cite{RWWJooJdjxEK}]       \label{THOooKDJVooRIJRHP}
	...
	Quelques définitions associées :
	\begin{enumerate}
		\item
		      Si \( r=0\), nous disons que \( a\) est \defe{divisible}{divisible} par \( b\).
		\item
		      Si		\( n\) est divisible par \( 2\), alors nous disons qu'il est \defe{pair}{nombre pair}.
		      \item\label{ITEMooDLHHooTmWSnN}
		      Si		\( n\) n'est pas divisible par \( 2\), alors nous disons qu'il est \defe{impair}{nombre impair}.
	\end{enumerate}
\end{theorem}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooYFUBooJUZgwH}
	Quelques propriétés de l'ordre dans \( \eZ\).
	\begin{enumerate}
	...
	%TODOooEDWPooRDujTs. Prouver ça. J'en mets deux parce que je parie que ce sera fait.
\end{proposition}
Cette proposition est en partie similaire à \ref{PROPooELXCooCYzEVD}.
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooWNLYooBwDjEK}
	Si \( a,b\in \eZ\) nous avons\footnote{Pour la valeur absolue, définition \ref{DEFooJXKVooErANPh}.}
	\begin{equation}
		| ab |=| a || b |
	\end{equation}
\end{proposition}
...
\begin{proof}
	C'est le fait que \( \eZ\) est un anneau totalement ordonné\footnote{Proposition \ref{PROPooADGIooTioqdu}.} et \eqref{EQooORNAooBYFeFk}.
\end{proof}
...
\begin{proposition} \label{PROPooYJBMooZrzkNX}
	Soit \( a,b\in \eZ\) tels que \( a\) divise\footnote{Diviseur, définition \ref{DiviseursAnneau} et pour la valeur absolue c'est \ref{DEFooJXKVooErANPh}.} \( b\). Alors \( | a |\leq | b |\).
\end{proposition}

\begin{proof}
	Il existe \( x\in \eZ\) tel que \( ax=b\). Par la proposition \ref{PROPooWNLYooBwDjEK}, nous avons \( | ax |=| a || x |=| b |\). Comme \( | x |\geq 1\), le lemme \ref{LEMooSVDDooWsyxNP} dit que
	\begin{equation}
		| b |=| a || x |\geq | a |.
	\end{equation}
\end{proof}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooCMOEooSoeclk}
	...
	\begin{enumerate}
		\item
		      Si \( n\) est impair\footnote{Définition \ref{THOooKDJVooRIJRHP}\ref{ITEMooDLHHooTmWSnN}.}, alors \( n-1\) et \( n+1\) sont pairs.
		\item
...
\begin{proof}
	Soit \( n\) impair. Alors il existe \( n=2k+r\) avec \( r<2\) et \( r\neq 0\), c'est à dire \( r=1\) : \( n=2k+1\) pour un certain \( k\). Donc \( n-1=2k+1-1=2k\) est pair et \( n+1=2k+1+2=2(k+1)\) est pair.

	Le cas impair est du même tonneau.
\end{proof}
...
%-------------------------------------------------------
\subsection{Structure de groupe additif sur \( \eZ\)}
%----------------------------------------------------

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooIKAZooFdhVWQ}
	Le couple \( (\eZ,+)\) est un groupe commutatif dont le neutre est \( 0\).
	%TODOooIMNIooCUJXrq. Prouver ça.
\end{proposition}
