	...
	\begin{subproof}
		...
		\begin{subproof}
			...
			\begin{itemize}
				...
				\item
				      Si \( u\neq x\) et \( v=x\), alors c'est le même cas que le précédent avec quelques adaptations.
				\item
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooGJLEooCQdegS}
	Toute union finie d'ensembles finis et finie.
	%TODOooKMEXooVttdhP. Prouver ça.
\end{proposition}


\begin{proposition}[\cite{MonCerveau}]	\label{PROPooYCDWooXBWVrW}
	Soient un ensemble infini \( A\), un ensemble fini \( B\) et une application \(\phi \colon A\to B  \). Alors il existe un \( s\in B\) tel que \( \phi^{-1}(s)\) est infini.
\end{proposition}

\begin{proof}
	Nous avons \( A=\bigcup_{b\in B}\phi^{-1}(b)\). Donc si \( \phi^{-1}(b)\) était fini pour tout \( b\in B\), l'ensemble \( A\) serait une union finie d'ensembles finis et donc fini par la proposition \ref{PROPooGJLEooCQdegS}.
\end{proof}
...
\begin{normaltext}		\label{NORMooLNLHooLYKwDu}
	...
	Voici quelques résultats que je doute être possible de démontrer sans passer par les naturels.
	\begin{enumerate}
		\item
