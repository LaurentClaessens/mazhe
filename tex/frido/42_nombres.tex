...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		En posant \( y'=-y\) nous avons \( y'\geq 0\) (par le point \ref{ITEMooRRUGooRUpYwt}). Nous avons alors \( xy'\geq 0\) par la définition \ref{DEFooWACWooDWvXKJ}\ref{CONDooBYYDooElXgPO}. Nous avons alors \( xy=-xy'\geq 0\).

		\spitem[Pour \ref{ITEMooDZUJooVWuLbD}]
		%-----------------------------------------------------------
		Nous posons \( a'=-a\) et \( b'=-b\). Par le point \ref{ITEMooRRUGooRUpYwt} nous avons \( a',b'\geq 0\). Donc \( a'b'\geq 0\) par le point \ref{ITEMooNLAQooHtERZJ}. Mais \( a'b'=(-a)(-b)=ab\) par la proposition \ref{PROPooFTFJooXUXOTY}\ref{ITEMooHUJFooKkylVt}.
...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{subproof}
			...
			%-----------------------------------------------------------
			La définition de la valeur absolue donne \( | x |=x\) et \( | y |=-y\). Par ailleurs la proposition \ref{PROPooELXCooCYzEVD}\ref{ITEMooNLAQooHtERZJ} dit que \( xy\leq 0\) de telle sorte que \( | xy |=-xy\).
			...
			\spitem[Si \( x,y\leq 0\)]
			%-----------------------------------------------------------
			Même chose, en utilisant \ref{PROPooELXCooCYzEVD}\ref{ITEMooDZUJooVWuLbD}.
		\end{subproof}
	\end{subproof}
\end{proof}
