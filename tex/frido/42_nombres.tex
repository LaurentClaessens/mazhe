...
\begin{proof}
    Soit un ensemble infini \( A\) dans \( \eN\). Le lemme \ref{LEMooFHEOooSHPGgU} nous indique que toute partie non vide de \( \eN\) possède un minimum. Nous en profitons pour définir \( \sigma\colon \eN\to A\) par récurrence\footnote{Il est du meilleur ton de citer le théorème \ref{THOooEJPYooZFVnez}, mais la lâcheté et la prudence peuvent vous inciter à faire comme si c'était évident et ne pas insister.} :
    \begin{subequations}
        \begin{numcases}{}
            \sigma(0)=\min(A)\\
