...
Nous supposons ici connaître, et avoir démontré les rudiments du calcul dans \( \eN\) et \( \eZ\). En particulier les produits remarquables, et les implications comme \( a>b\) implique qu'il n'existe pas de \( x\) dans \( \eN\) tel que \( a+x=b\).
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooLAJCooFwxXrV} implique \ref{ITEMooQNTFooSRrVPK}]
            ...
            Si \( ab=ac\), alors \( a(b-c)=0\) et l'hypothèse dit que doit \( a=0\), soit \( b-c=0\). Donc si \( a\neq 0\), alors \( b-c=0\).
...
\begin{proof}
    ...
    \begin{subproof}
        \item[Sens direct]
            Nous supposons que \( A\) est un corps. D'une part tous les éléments non nuls sont inversibles, c'est-à-dire \( A^*\subset U(A)\). i
...
Le théorème~\ref{THOooYXJIooWcbnbm} nous dira que tous les \( \sqrt{n}\) sont irrationnels dès que \( n\) n'est pas un carré parfait. Voici déjà le résultat pour \( n=2\).
...
La proposition suivante donne une suite de rationnels qui convergerait dans \( \eR\) vers \( \sqrt{ A }\) (non encore défini à ce stade). Il est expliqué dans \cite{BIBooMPXEooQLKhku} que la suite est motivée par la méthode de Newton \cite{THOooDOVSooWsAFkx}.
...
\begin{proof}
    ...
    \begin{subproof}
        \item[La suite \( s_k\)]
            ...
            \begin{equation}
                s_1=\frac{ s_0^2 }{ 4(A+s_1) }=\frac{ (x_0^2-A)^2 }{ 4(A+x_0^2-A) }=\frac{ (x_0^2-A) }{ 4x_0^2 }>0.
            \end{equation}
...
\begin{normaltext}[Quelque notations entre \( \eQ\) et \( \eR\)]      \label{NORMooWBYNooBQaPPk}
    Si \( k\mapsto x_k\) est une suite, nous notons \( (x_k)\) avec des parenthèses la suite elle-même. Le \( k\) dans \( (x_k)\) est un indice muet, et dans les cas où il peut y avoir une ambiguïté, nous pouvons noter \( (x_k)_{k\in \eN}\). Cette dernière notations est plus lourde, mais plus exacte.
...
\begin{proposition}     \label{PROPooZSQYooWRKNGY}
    Soit une suite convergente \( x_k\stackrel{\eQ}{\longrightarrow}x\). Alors
    \begin{equation}
        \varphi(x_k)\stackrel{\eR}{\longrightarrow}\varphi(x)
    \end{equation}
...
\begin{proof}
    Le fait d'avoir une convergence \( x_k\to x\) dans \( \eQ\) implique que la suite \( (x_k)\) est de Cauchy, par la proposition \ref{PropFFDJooAapQlP}\ref{ItemRKCIooJguHdji}.
    
    Le lemme \ref{LemooRTGFooYVstwS} nous indique que \( \varphi(x_k)\) est une suite dans \( \eR\) qui converge vers \( \bar x\), la classe de la suite \( (x_k)\).

    À prouver : \( \varphi(x)=\bar x\). Autrement dit, nous devons prouver que la classe de la suite constante \( a_k=x\) et la classe de la suite \( b_k=x_k\) sont les mêmes.

    La suite \( (x_k-x)\) est de Cauchy dans \( \eQ\) et converge vers zéro par hypothèse. Donc les suites \( (x_k)\) et \( (x)\) sont dans la même classe.
\end{proof}
...
Dans cette section, nous définissons \( \sqrt{ x }\) pour \( x\in\eR^+\). Vous notez que c'est fait de façon assez algébrique\footnote{Discutable parce que des limites sont utilisées.}, ou en tout cas, en restant proche des définitions. Des définition plus technologiques utilisant la continuité de \( x\mapsto x^n\) et qui prouvent que c'est bijectif sur un domaine choisit avec prudence existent, et c'est fait dans la définition \ref{DEFooJWQLooWkOBxQ}. Il est même expliqué dans \cite{BIBooMPXEooQLKhku} que la méthode décrite ici permet de définir \( \sqrt[n]{ x }\) pour tout \( n\) entier, et pas seulement pour \( n=2\).
...
\begin{proof}
    ...
    \end{equation}
    c'est la classe de la suite de Cauchy donnée par les \( x_k^2\).
    
    La proposition \ref{PROPooSTQXooHlIGVf} nous dit également que \( (x_k^2)\) est une suite de Cauchy et que
    \begin{equation}
        x_k^2\stackrel{\eQ}{\longrightarrow}q.
    \end{equation}
    La proposition \ref{PROPooZSQYooWRKNGY} donne alors
