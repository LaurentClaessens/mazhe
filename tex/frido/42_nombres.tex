...
\begin{definition}[\cite{BIBooAKHUooProFGE,BIBooWNKRooETlebF}]
    Soient deux ensembles \( A\) et \( B\).
    \begin{enumerate}
        \item
            Les ensembles \( A\) et \( B\) sont \defe{équipotents}{équipotent} si il existe une bijection entre \( A\) et \( B\). Nous notons \( A\simeq B\).
        \item
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Il est inductif]
            ...
            \end{equation}
            et \( \varphi(x)=\varphi_i(x)\) pour n'importe quel\quext{Étant donné que je suis devenu paranoïaque, je me demande si on utilise l'axiome du choix ici. Écrivez-moi si vous savez la réponse.} \( i\in I\) tel que \( x\in X_i\).

        \item[Lemme de Zorn]

            Vu que \( \mA\) est un ensemble inductif, nous pouvons utiliser le lemme de Zorn \ref{LemUEGjJBc} et considérer un élément maximum, c'est à dire une paire \( (Y,\varphi)\) où \( Y\subset A\) et \( \varphi\colon \{ 0,1 \}\times Y\to Y\) est injective.

            Il y a deux cas : soit \( Y\) est équipotent à \( A\), soit il ne l'est pas.

        \item[Si \( Y\) est équipotent à \( A\)]

            Nous considérons une bijection \( \psi\colon A\to Y\) et nous montrons que l'application
            \begin{equation}
                \begin{aligned}
                    f\colon \{ 0,1 \}\times A&\to A \\
        ...
        \item[Si \( Y\) n'est pas équipotent à \( A\)]

            Nous montrons que ce cas n'est pas possible. Notons que \( Y\) est certainement subpotent à \( A\) parce que \( Y\subset A\).
            \begin{subproof}
                \item[\( A\setminus Y\) est inifni]
                    Supposons le contraire. Si \( A\setminus Y\) est fini, alors \( Y\cup(A\setminus Y)\) est équipotent à \( Y\) par le lemme \ref{LEMooUFCAooSyZtZj}. Oui mais \( Y\cup(A\setminus Y)=A\), et nous avons dit que \( Y\) n'est pas équipotent à \( A\).

                    Bref, oui : \( A\setminus Y\) est infini.

                \item[La contradiction]
                    La proposition \ref{PROPooUIPAooCUEFme} nous donne une partie dénombrable \( N\subset A\setminus Y\). Nous allons construire une injection \( \psi\colon \{ 0,1 \}\times (Y\cup N)\to Y\cup N\) qui prolonge \( \varphi\), contredisant ainsi la maximalité de \( (Y,\varphi)\).

                    Nous considérons une bijection \( g\colon \{ 0,1 \}\times N\to N\) (lemme \ref{LEMooUFCAooSyZtZj} et le fait que \( N\) soit infini par \ref{PROPooBYKCooGDkfWy}). Vu que \( N\) est dans \( A\setminus Y\), les parties \( Y\) et \( N\) sont disjointes et nous pouvons écrire
                    \begin{equation}
...
\begin{corollary}
    Si \( S\) est un ensemble infini, alors \( S\) possède deux sous-ensembles disjoints \( A\) et \( B\) qui sont tout deux en bijection avec \( S\).
\end{corollary}

\begin{proof}
    La proposition \ref{PropVCSooMzmIX} donne une bijection \( \varphi\colon \{ 1,2 \}\times S\to S\). Il suffit de poser \( A=\varphi(1,S)\) et \( B=\varphi(2,S)\).
\end{proof}

La proposition suivante est une généralisation de la proposition \ref{PropVCSooMzmIX}. Elle implique, entre autres choses, qu'il existe une bijection entre \( \eR\) et \( \eR\times \eR\).
\begin{proposition}
    Si \( A\) est infini et surpotent à \( B\), alors \( A\times B\) est idempotent à \( A\).
\end{proposition}
