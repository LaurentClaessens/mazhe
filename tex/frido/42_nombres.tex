...
Le théorème de compatibilité cardinale énonce que si \( A\) et \( B\) sont des ensemble, alors nous avons toujours \( A\succeq B\) ou \( A\preceq B\) (ou les deux; dans ce cas \( A\approx B\) par Cantor-Schröder-Bernstein).
\begin{theorem}[Théorème de compatibilité cardinale\cite{MonCerveau,BIBooTSOKooCWxMwj}]
...
\begin{proof}
    ...
    \begin{subproof}
        \item[Ce que nous allons faire]
            ...
            \begin{itemize}
                \item \( A_i\) et \( B_i\) sont disjoints
        ...
        \item[Les propriétés]
            ...
            \begin{equation}
                A_j\subset B_{j-1}\subset B_{j-1}\subset \ldots \subset B_i.
            \end{equation}
        ...
        \item[Une injection]
            ...
            \item[La bijection]
                Nous venons de prouver que \( A\times \eN\preceq A\). L'inéquipotence dans l'autre sens, \( A\times \eN\succeq A\) étant évidente, le théorème de Cantor-Schröder-Bernstein \ref{THOooRYZJooQcjlcl} conclu que \( A\times \eN\approx A\).
    \end{subproof}
\end{proof}
...
\begin{lemma}[\cite{MonCerveau}]       \label{LEMooIVCBooHWQiZB}
...
\begin{proof}
    ...
    \end{equation}
    Le corolaire \ref{CORooJCSIooOeOICJ} nous permet de considérer deux parties disjointes \( A_1\) et \( A_2\) de \( A\) avec des bijections
    \begin{subequations}
        \begin{align}
        ...
        \end{align}
    \end{subequations}
    Nous posons \( B_1=\varphi_1(B)\) et \( B_2=\varphi_2(B)\). Ce sont deux parties disjointes de \( A\) parce que \( B_1\cap B_2=\varphi_1(B)\cap \varphi_2(B)\subset A_1\cap A_2=\emptyset\). 
    
    Nous avons le calcul :
    \begin{subequations}
        \begin{align}
            A\setminus B&\approx A_1\setminus B_1     \label{SUBEQooPSTFooTglZie}\\
            &\succeq B_2    \label{SUBEQooTUWMooHIBLmJ}\\
            &\approx B.     \label{SUBEQooXPDDooDQVbdX}
        \end{align}
    \end{subequations}

    \begin{probleme}
        Problème de justification pour \ref{SUBEQooTUWMooHIBLmJ}. On peut corriger en disant que \( \varphi_1\colon A\setminus B_1\to A\setminus B\) est une injection et que donc \( A\setminus B\succeq A\setminus B_1\).

        Ce qui est justifié plus bas est que \( A\setminus B_1\succeq B_2\).

        Au final on aurait la suite \( A\setminus B\succeq A\setminus B_1\succeq B_2\approx B\).
    \end{probleme}

    Justifications :
    \begin{itemize}
        \item Pour \ref{SUBEQooPSTFooTglZie}, c'est la restriction \( \varphi_1\colon A\setminus B\to A_1\setminus B_1\) qui est encore une bijection.
        \item Pour \ref{SUBEQooTUWMooHIBLmJ}, c'est le fait que \( A_1\) et \( A_2\) sont disjoints et donc que \( B_2\subset A_2\subset A\setminus A_1\subset A\setminus B_1\). Donc il y a une surjection \( A\setminus B_1\to B_2\).
        \item Pour \ref{SUBEQooXPDDooDQVbdX}, les restrictions \( \varphi_i\colon B\to B_i\) sont des bijections, de telle sorte que \( B_1\approx B\approx B_2\).
    \end{itemize}
    
    Au final, la partie \( A\setminus B\) est surpotente à \( B\) et le lemme \ref{LEMooXMVDooIWLWis} nous dit que
    \begin{equation}
        A=(A\setminus B)\cup B\approx A\setminus B.
    \end{equation}
\end{proof}
