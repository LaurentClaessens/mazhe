...
\begin{proof}
    Vu que \( A\) est infini, il existe \( A'\) strictement inclus à \( A\) et une bijection \( \sigma\colon A'\to A\).
\end{proof}
<++>
...
\begin{normaltext}
    La proposition \ref{PROPooOBKMooWEGCvM} peut être prouvée plus facilement en acceptant le théorème de Cantor-Schröder-Bernstein \ref{THOooRYZJooQcjlcl}. Il existe une injection \( A\to \eN\) parce que \( A\) est une partie de \( \eN\). Mais vu que \( A\) est infini, il possède une partie dénombrable. Cela donne une surjection \( A\to \eN\) et donc une injection \( \eN\to A\). Le théorème de Cantor-Schröder-Bernstein conclu.
\end{normaltext}
...
\begin{proof}
    Soient des ensembles finis ou dénombrables \( \{ A_i \}_{i\in \eN}\). Nous posons \( A=\bigcup_{i\in \eN}A_i\). Si \( A\) est fini, la proposition est faite. Sinon, \( A\) est infini et il existe certainement une surjection \( A\to \eN\).

    Nous considérons aussi une partition de \( \eN\) en une infinité dénombrable de parties \( \eN_i\), chacune infinie dénombrable (par exemple parce que \( \eN\times \eN\) est équipotent à \( \eN\) par le théorème \ref{THOooDGOVooRdURVi} ou la proposition \ref{PROPooLPKUooAlsYJg}).

    Les ensembles \( A_i\) étant finis ou dénombrables, il existe des surjections \(\sigma_i\colon \eN_i\to A_i\). Pour chaque \( n\in \eN\), il existe un unique \( i\in \eN\) tel que \( n\in \eN_i\); donc nous avons une bijection \( f\colon \eN\to \eN\) telle que \( n\in \eN_{f(n)}\). Bref, l'application
    \begin{equation}
        \begin{aligned}
            \sigma\colon \eN&\to A \\
            n&\mapsto \sigma_{f(n)}(n)
        \end{aligned}
    \end{equation}
    est une surjection.
\end{proof}
