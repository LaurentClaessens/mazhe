...
\begin{definition}
    Soit un ensemble ordonné \( (E,\leq)\) et une partie \( A\) de \( E\). Nous disons que \( m\in A\) est un \defe{minimum}{minimum!ensemble ordonné} si pour tout \( y\in E\), l'élément \( m\) est comparable à \( m\) et \( m\leq y\).
\end{definition}
...
\begin{remark}
    Nous étudierons plus tard la notion de caractéristique d'un anneau\footnote{définition \ref{}} ou d'un corps. La proposition \ref{} montrera que tout corps totalement ordonné est nécessairement de caractéristique nulle.
\end{remark}
...
\begin{proof}
    ...
    Enfin supposons que \( x_n\to a\in \eQ\). Dans ce cas nous aurions \( x_n^2\to a^2=2\) (proposition \ref{}). Mais nous savons par la proposition \ref{PropooRJMSooPrdeJb} que \( a^2=2\) est impossible dans \( \eQ\).
\end{proof}
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            ...
            &\leq \frac{1}{ (q+1)! }\sum_{k=0}^{\infty}\frac{1}{ (q+1)^k }  \label{SUBEQooNDPTooDSEYEJ}\\
    ...
    Justifications :
    \begin{itemize}
        \item Pour \eqref{SUBEQooAXILooEAcpVB}
        \item Pour \eqref{SUBEQooNDPTooDSEYEJ} et \eqref{SUBEQooEMHJooSnCUiK}, c'est la proposition \ref{} qui donne la convergence de la série ainsi que sa valeur.
    \end{itemize}
    <++>
...
\begin{proof}
    ...
    \begin{equation}
        x_n>\frac{ \alpha }{2}>0
    \end{equation}
    Par conséquent \( x\in\modE^+\) parce que \( x\in\modE\) et les \( x_n \) sont positifs à partir d'un certain rang.
\end{proof}
...
\begin{proof}
    ...
    L'élément \( a\) de \( \eR\) admet un représentant \( (a_n)\) qui est une suite de Cauchy dans \( \eQ\). C'est donc une suite bornée, c'est-à-dire qu'il existe \( m,q\in \eN\) tels que \( | a_n |\leq m/q\) pour tout \( n\) (proposition \ref{PropFFDJooAapQlP}\ref{ItemRKCIooJguHdjii}). Soit \( M\) un naturel strictement plus grand que \( m/q\). 
...
\begin{proof}
    Le lemme \ref{LemooRordonne} dit que \( \eR\) est totalement ordonné. Soient \( x,y\in \eR\) avec \( x>0\); posons \( a=\frac{ y }{ x }\). Le lemme \ref{LemooMWOUooVWgaEi} nous donne un \( p \in \eN\) tel que \(p > a\). On conclut alors avec le lemme \ref{LemooYNOVooOwoRwD}: on obtient \( px > y \) 
\end{proof}
...
\begin{proof}
    ...
    \end{equation}
    Cet ensemble n'est pas vide à cause du lemme \ref{LemooMWOUooVWgbFi}; de plus, comme \( |x|q \leq n_0\) pour un certain \( n_0 \) (à cause du lemme \ref{LemooMWOUooVWgaEi}), l'ensemble \( E\) est majoré par \( n_0\). Donc \( E\) possède un plus grand élément \( p\) qui vérifie
    \begin{equation}
    ...
    Il suffit alors de choisir \( s=\frac{ p+1 }{ q }\).
\end{proof}
