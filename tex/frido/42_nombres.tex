...
\begin{corollary}       \label{CORooVNHKooRkKtXf}
    Soient deux ensembles \( X,Y\), une application \( g\colon X\to Y\) et une application \( h\colon Y\to Y\). Alors il existe une unique application \( H\colon X\times \eN\to Y\) telle que
\begin{enumerate}
    \item
  $H(x , 0) = g(x)$   pour tout élément $x\in X$;
  \item
 $H(x , n+1 ) = h( H( x , n) )$ pour tout élément \( x\in X\) et pour tout \( n\in \eN\).
...
\begin{proof}
    Soit \( A\), une partie infinie de \( \eN\). Le lemme \ref{LEMooFHEOooSHPGgU} nous indique que toute partie non vide de \( \eN\) possède un minimum. Nous en profitons pour définir \( \sigma\colon \eN\to A\) par récurrence\footnote{Il est du meilleur ton de citer le théorème \ref{THOooEJPYooZFVnez} et le lemme \ref{LEMooFHEOooSHPGgU}, mais la lâcheté et la prudence peuvent vous inciter à faire comme si c'était évident et ne pas insister.} :
    \begin{subequations}
        \begin{numcases}{}
            \sigma(0)=\min(A)\\
