...
\begin{theorem}     \label{THOooOXMHooXYgMqb}
    Il existe un\footnote{Nous verrons plus tard que toute partie infine d'un triplet naturel fournit un nouveau triplet naturel; il en existe donc plusieurs.} triplet naturel.
\end{theorem}
...
\begin{proof}
    ...
    \item[\( f\circ g=\id\)]
        ...
        \begin{subequations}
            ...
            \end{align}
        \end{subequations}
        Donc \( s_2(x)\in A\). Nous en déduisons que \( A=\mN_2\) par le point \ref{ITEMooXPYEooFajywh} de la définition \ref{DEFooBJBOooWlblAx} de triplet naturel.
    \item[\( g\circ f=\id\)]
...
Si \( z\in \eZ\), ou disons que \( z\in \eN\) lorsque \( z\in \iota(\eN)\). C'est un abus de notation qu'il est difficile de ne pas faire.
...
\begin{proof}
    Supposons pour fixer les idées que \( p\leq q\). Dans ce cas \( \{ 0,\ldots, p \}\) est une partie stricte de \( \{ 0,\ldots, q \}\). Vu que \( \{ 0,\ldots, q \}\) est fini (lemme \ref{LEMooYIWDooIQFSad}), il n'y a pas de bijections avec ses parties strictes.
\end{proof}
...
\item[\ref{ITEMooZJFUooSNUSIk} existence]
    L'existence est ce que nous venons de faire.
...
\begin{proof}
    ...
    Nous ordonnons \( \mA\) par l'inclusion : \( (X,\varphi)\leq (Y,\phi)\) lorsque \( X\subset Y\) et \( \phi|_X=\varphi\). La tambouille usuelle montre que \( \mA\) est un ensemble inductif et le lemme de Zorn \ref{LemUEGjJBc} donne l'existence d'un élément maximul que nous notons \( (B,\varphi)\).
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Pour \ref{ITEMooYBEGooTlHgNz}]
            ...
            Vu que \( (A,+)\) est un groupe commutatif de neutre \( 0\), nous avons
            \begin{equation}
                [a]+[b]=[a+b]=[b+a]=[b+a],
            \end{equation}
            ainsi que
...
\begin{proposition}     \label{PROPooJBQVooNqWErk}
    Soient un groupe commutatif \( (G,+)\), un ensemble fini \( I\) est un ensemble fini, une application \( f\colon I\to G\) et une bijection \( \sigma\colon I\to I\). Alors
    \begin{equation}
...
\begin{definition}[module sur un anneau\cite{ooJGVOooSjQBVh}]       \label{DEFooHXITooBFvzrR}
    ...
    Si \( M\) et \( N\) sont des \( A\)-modules, un \defe{morphisme}{morphisme de modules} de \( A\) vers \( B\) est une application \( f\colon A\to B\) qui 
    \begin{enumerate}
        \item
    ...
    \end{enumerate}
    L'ensemble des morphismes entre \( M\) et \( N\) est noté \( \Hom_A(M,N)\). Si \( B\) st une sous-anneau nous parlons de \( \Hom_B(M,N)\) pour parler des morphismes de groupes qui ne vérifient \( f(a\cdot x)=a\cdot f(x)\) que pour \( a\in B\).
\end{definition}
...
\begin{proof}
    ...
    \item[Injectif]
        Supposons que \( f(\phi_1)=f(\phi_2)\). Alors en appliquant \( g\) est deux côtés, il vient \( \phi_1=\phi_2\).
    \end{subproof}
\end{proof}
