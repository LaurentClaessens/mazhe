...
\begin{proof}
    Supposons pour commencer que \( B\) soit fini. Nous pouvons écrire
    \begin{equation}
        A=(A\setminus B)\cup B.
    \end{equation}
    La partie \( A\setminus B\) est infinie, sinon \( A\) serait l'union de deux ensembles finis et le lemme \ref{LEMooYHGCooAwsVQN} dirait que \( A\) est infini. Le lemme \ref{LEMooUFCAooSyZtZj} nous dit alors que \( (A\setminus B)\cup B\) est équipotent à \( A\setminus B\). Voila qui règle le lemme dans le cas où \( B\) est fini.

    Nous supposons à présent que \( B\) est infini.
\end{proof}
<++>
