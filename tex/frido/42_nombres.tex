...
La définition suivante donne la somme sur un ensemble fini. Pour aller plus loin, et sommer sur des ensembles infinis, il faut regarder la définition \ref{DefHYgkkA}. Dans cet énoncé, l'existence d'une bijection entre \( I\) et \( \{ 0,\ldots, N \}\) ainsi que l'unicité du \( N\) pour lequel c'est possible sont donnés par la proposition \ref{PROPooJLGKooDCcnWi}.
\begin{propositionDef}[\cite{MonCerveau}]       \label{DEFooLNEXooYMQjRo}
    Soient un ensemble fini \( S\), un anneau commutatif \( A\), une fonction \( f\colon S\to A\) ainsi que deux bijections \( \varphi_1\colon \{ 0,\cdots N \}\to S\), \( \varphi_2\colon \{ 0,\ldots, N \}\to S\).
    \begin{equation}
