...
\begin{proof}
    ...
    Nous allons utiliser le lemme de Zorn \ref{LemUEGjJBc} à l'ensemble
    \begin{equation}
       \mA=\Big\{  (X,Y,\varphi)  \tq
        \begin{cases}
            X\subset A\\
    ...
    Attention : dans le Frido, la notation \( f\colon A\to B\), signifie que \( f\) est définie sur tout l'ensemble \( A\), mais pas qu'elle est surjective sur \( B\).

    Nous passons rapidement sur le fait que cet ensemble est inductif, et nous considérons tout de suite un élément maximal \( (X,Y,\varphi)\). Nous avons forcément \( Y=B\) parce que \( (X,B,\varphi)>(X,Y,\varphi)\) : agrandir de façon «artificielle» l'espace d'arrivée ne change pas le fait que \( \varphi\) soit injective. Notez donc que, bien que \( Y=B\), nous n'avons pas spécialement \( \varphi(X)=B\) parce que \( \varphi\) n'est peut-être pas surjective.

    Il y a deux possibilités : soit \( \varphi(X)=B\), soit \( \varphi(X)\neq B\).
    \begin{subproof}
        \item[Si \( \varphi(X)=B\)]
