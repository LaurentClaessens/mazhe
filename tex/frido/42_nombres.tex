...
\begin{definition}[Ordre sur \( \eR\)]      \label{DEFooBXHJooOEYPRI}
    Si \( a,b\in \eR\) nous notons \( a\leq b\) si et seulement si \( b-a\in\modE^+\). Nous notons \( a\geq b\) si et seulement si \( b-a\in\modE^+\). Nous notons aussi \( a>b\) si et seulement si \( b-a\in \eR^+\setminus\{ 0 \}\), etc.
\end{definition}
...
\begin{proof}
    Il s'agit de vérifier, dans l'ordre, les définitions \ref{DefooFLYOooRaGYRk}, \ref{DEFooVGYQooUhUZGr} et \ref{DefKCGBooLRNdJf}\ref{ITEMooOOOVooJWwIQr}.
    \begin{subproof}
    \item[Réflexivité]
        Pour savoir si \( x\geq x\), nous devons nous demander si \( x-x\in\modE^+\). C'est le cas parce que \( x-x=\bar 0\in \modE^+\) par le lemme \ref{LEMooJOYQooCDlhHW}\ref{ITEMooKRBYooZGhhch}.
    \item[antisymétrie]
        Nous supposons que \( x\geq y\) et \( y\geq x\). Cela signifie que \( x-y\in \modE^+\) et \( y-x\in\modE^+\). Le lemme \ref{LEMooRKSXooFsIohe}\ref{ITEMooRQVKooCnwWOY} nous indique alors que \( x-y=-(y-x)\in \modE^-\). Donc
        \begin{equation}
            x-y\in\modE^+\cap\modE^-=\{ \bar 0 \}.
        \end{equation}
