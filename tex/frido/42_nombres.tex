...
\begin{proof}
	Soit un ensemble \( E\) totalement ordonné. Nous considérons l'ensemble
	\begin{equation}
		\mA=\{ X\subset E\tq \text{toute partie finie de }  X \text{ a un maximum.} \}.
	\end{equation}
	L'ensemble \( \mA\) est ordonné par inclusion. Vu que \( E\) majore toute partie de \( \mA\), l'ensemble \( \mA\) est inductif. Il a donc un maximum par le lemme de Zorn \ref{LemUEGjJBc}. Soit \( Y\) le maximum de \( \mA\). Nous montrons que \( Y=E\).

	Supposons que \( Y\neq E\) et considérons \( a\in E\setminus Y\). La partie \( Y\cup\{ a \}\) majore \( Y\) (pour l'inclusion), et nous montrons maintenant que \( Y\cup\{ a \}\in \mA\), de telle sorte à avoir une contradiction.

	Soit une partie finie \( A\) de \( Y\cup\{ a \}\). La partie \( A\cap Y\) est une partie finie\footnote{Lemme \ref{LEMooTUIRooEXjfDY}.} de \( Y\) et possède donc un maximum que nous notons \( a_{max}\).
	\begin{subproof}
		\spitem[Si \( a\in A\)]
		%-----------------------------------------------------------
		Si \( a<a_{max}\) alors \( a_{max}\) est un maximum de \( A\). Si \( a>a_{max}\) alors \( a\) est un maximum de \( A\).
		\spitem[Si \( a\not\in A\)]
		%-----------------------------------------------------------
		Alors \( A\subset Y\) et \( a_{max}\) est un maximum de \( A\).
	\end{subproof}
	Donc dans tous les cas \( A\) a un maximum. Cela contredit la maximalité de \( Y\) et donc prouve que \( Y=E\).

	Conclusion : \( E\in \mA\), ce qui est précisément ce que nous voulions prouver.
\end{proof}
...
\begin{definition}[Valeur absolue, corps valué\cite{BIBooVKGHooSPijZp,BIBooNRMUooJmwzpn}]       \label{DEFooBWXXooAkBBRS}
	Un \defe{anneau valué}{anneau valué}\index{corps valué} \( A\) est un annau totalement ordonné muni d'une application \( | . |\colon A\to \eR^+\) telle que
	\begin{enumerate}
		\item
