...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{equation}
			\mA=\{ J\subset I\tq s(J)\neq t(J) \}.
		\end{equation}
		Vu que \( \mA\) est une partie de \( \partP(I)\) qui est fini\footnote{Par le lemme \ref{LEMooNTVKooTfvHfA}.}, l'ensemble \( \mA\) est fini.

		Muni de l'inclusion, \( \mA\) est un ensemble partiellement ordonné par le lemme \ref{LEMooZUAZooFlziuO}.
	\end{subproof}
\end{proof}
...
\begin{propositionDef}      \label{DEFooNEVNooJlmJOC}
	...
	\begin{subequations}		\label{SUBEQSooKJOFooOOxDKH}
		\begin{numcases}{}
			f(0)=\alpha(0) 	\label{SUBEQooCCBKooQTbwrQ}\\
			f(k)=f(k-1)+\alpha(k).
		\end{numcases}
	\end{subequations}
