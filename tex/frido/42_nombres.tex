...
\begin{probleme}
    Vous noterez l'utilisation du lemme de Zorn, et donc de l'axiome du choix, pour une affaire ne concernant que des ensembles finis. À mon avis c'est inévitable, mais je n'en suis pas certain.

    Il me semble que si nous démontrions d'abord que tout ensemble fini est en bijection avec \( \{ 0,\ldots, N \}\) nous pourrions démontrer ce lemme \ref{LEMooPGPVooZzlFvf} sans appel supplémentaire au lemme de Zorn.
\end{probleme}
...
\begin{proof}[Deuxième preuve de \ref{LEMooTNMHooBpdzab}\cite{BIBooZFPUooIiywbk}]
    ...
    \begin{subproof}
        ...
        \item[\( f(X)\subset M\) pour tout \( M\in \mM\)]
            ...
            \end{itemize}
        \item[\( X\in\mM\)]
            Vu que \( Y\in M\) pour tout \( M\) dans \( \mM\), nous avons \( Y\subset \bigcap_{M\in \mM}M=X\). Nous devons prouver \( f(X)\subset X\). Nous venons de prouver que \( f(X)\subset M\) pour tout \( M\in \mM\), donc
            \begin{equation}
