...
\begin{definition}      \label{DefooFLYOooRaGYRk}
    ...
    \begin{enumerate}
        \item
            \emph{réflexivité} : \( x\leq x\)
        \item
            \emph{antisymétrie} : \( x\leq y\) et \( y\leq x\) implique \( x=y\)
        \item
            \emph{transitivité} : \( x\leq y\) et \( y\leq z\) implique \( x\leq z\).
    \end{enumerate}
\end{definition}
...
\begin{remark}
%TODO: refs
Nous étudierons plus tard la notion de caractéristique d'un anneau ou d'un corps. On peut montrer qu'un corps totalement ordonné est nécessairement de caractéristique nulle.
\end{remark}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item 
            ...
            Donc, en prenant \( p,q\geq n\), nous avons \( |  x_p|>\frac{ \alpha }{2}\) et \( | x_q |>\frac{ \alpha }{2}\). On en déduit que
            \begin{equation}
                \left| \frac{1}{ x_p }-\frac{1}{ x_q } \right| \leq \frac{ | x_q-x_p | }{ | x_px_q | }\leq \frac{ 4 }{ \alpha^2 }| x_q-x_p |.
            \end{equation}
            Mais comme \( (x_n)\) est de Cauchy, il suffit de choisir \( p\) et \( q\) à la fois plus grand que \( n\) et plus grand que le \( n_0\) qui fait \( | x_p-x_q |\leq \epsilon\) pour \( p,q\geq n_0\) pour obtenir
            \begin{equation}
...
\begin{proof}
    ...
    Enfin supposons que \( x_n\to a\in \eQ\). Dans ce cas nous aurions \( x_n^2\to a^2=2\). Mais nous savons par la proposition \ref{PropooRJMSooPrdeJb} que \( a^2=2\) est impossible dans \( \eQ\).
\end{proof}
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            ...
            &\leq \sum_{k=q+1}^p\frac{1}{ (q+1)! }\frac{1}{ (q+1)^{k-q-1} }\\
