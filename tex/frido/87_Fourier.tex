...
\begin{proposition}[\cite{MesIntProbb}] \label{PropKPsjyzT}
    ...
    \end{equation}
    est continue\footnote{Nous verrons plus tard, avec la formule d'inversion dans \ref{} que c'est même une bijection.}.

    Nous avons l'inverse donnée par la formule
    \begin{equation}
        \TF\big( \TF(f) \big)(x)=f(-x)
    \end{equation}
    pour tout \( f\in \swS(\eR^d)\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        \item[Première inclusion : \(  \TF\big( \swS(\eR^d) \big)\subset\swS(\eR^d)\)]
...
\begin{proposition}[\cite{MesIntProbb,KXjFWKA}]  \label{PROPooLWTJooReGlaN}
    L'application
    \begin{equation}
        \TF\colon \swS(\eR^d)\to \swS(\eR^d)
    \end{equation}
...
\begin{proof}
    Le fait que la transformée de Fourier sur \( \swS(\eR^d)\) prenne ses valeurs dans \( \swS(\eR^d)\) est déjà prouvé dans \ref{PropKPsjyzT}.
