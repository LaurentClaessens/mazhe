...
% Copyright (c) 2011-2013,2015-2019, 2023-2026
%   Laurent Claessens
...
\begin{definition}      \label{DEFooRIFYooUUUoha}
	...
	\end{enumerate}
\end{definition}
Une telle suite est régularisante parce que souvent \( \rho_n\in\swD(\eR^d)\), ce qui donne \( f*\rho_n\in C^{\infty}\) par le corolaire~\ref{CORooBSPNooFwYQrc}. Un exemple de suite régularisante est dans le lemme \ref{LEMooTDWSooSBJXdv}.
...
\begin{proposition}	\label{PROPooFTGEooAouzOO}
	Il existe un \( k>0\) tel que la suite
	\begin{equation}
		\begin{aligned}
			\rho_n\colon \eR^d & \to \eR                                                                                                         \\
			x                  & \mapsto \begin{cases}
				                             0                                                                         & \text{si } \| x \|\geq 1/n  \\
				                             \frac{ k }{ \epsilon^d }\exp\big( -\| x \|^2/(\epsilon^2-\| x \|^2) \big) & \text{si } \| x \|<\epsilon
			                             \end{cases}
		\end{aligned}
	\end{equation}
	est une suite régularisante dans \( L^1(\eR^d)\cap C^{\infty}(\eR^d)\).
	%TODOooVYXZooGbYUbd. Prouver ça.
\end{proposition}
...
\begin{proof}
	Soit \( n\in \eN\). Nous considérons
	\begin{equation}
		\begin{aligned}
			g\colon \mathopen[ -n,n\mathclose] & \to \eR       \\
			x                                  & \mapsto f(x).
		\end{aligned}
	\end{equation}
	et
	\begin{equation}
		G(x)=\int_{-n}^xg(t)dt.
	\end{equation}
	Nous savons par la proposition \ref{PROPooUMIPooQGXaPd} que \( G\) est dérivable presque partout sur \( \mathopen[ -n,n\mathclose]\) et que là où c'est dérivable, \( G'=g=f\). Autrement dit, il existe une partie \( A_n\) de mesure nulle telle que si \( x\in \mathopen[ -n,n\mathclose]\setminus A_n\), alors \( f(x)=G'(x)\).

	Par ailleurs notez que
	\begin{equation}
		F(x)=G(x)+\int_{-\infty}^{-n}f(t)dt.
	\end{equation}
	Autrement dit \( F\) et \( G\) diffèrent d'une constante. Donc pour \( x\in\mathopen[ -n,n\mathclose]\setminus A_n\) nous avons que \( F\) est dérivable et \( F'(x)=f(x)\).

	Tout cela étant valable pour tout \( n\), nous avons \( F'(x)=f(x)\) pour tout \( x\in \eR\setminus\bigcup_{n=0}^{\infty}A_n\). En tant qu'union dénombrable de parties de mesure nulle, \( \bigcup_{n=0}^{\infty}A_n\) est de mesure nulle par le lemme \ref{LemIDITgAy}.
\end{proof}
...
\begin{proposition}[\cite{BIBChatGPT}]	\label{PROPooSPDIooKntTyz}
