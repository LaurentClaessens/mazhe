...
\begin{lemma}[Formule de Leibnitz\cite{BIBooYDMJooGDtdbo}]
...
\begin{normaltext}
    Ici, \( \Omega\) est un ouvert de \( \eR^d\), et \( K\) est un compact de \( \eR^d\). Toutes les fonctions sont sur \( \eR^d\) à valeurs dans \( \eC\). Nous rappelons que les semi-normes \( p_{K,m}\) sont définies en \ref{PROPooVZFHooKfSpfO} et que la topologie engendrée par une semi-norme est définie en \ref{DEFooZTKAooWYUyDa}.
...
\begin{enumerate}
    \item
        Sur \(  C^{\infty}(\Omega)\), la topologie des semi-normes \( p_{K,m}\) où \( K\) parcours les compacts de \( \Omega\) et \( m\) parcours \( \eN\).
...
\end{definition}
Oui, c'est bien une famille de semi-normes indicée par l'ensemble \( \swD(\Omega)\). Il n'y en a donc à priori pas du tout une quantité dénombrable.
...
\begin{proof}
    ...
    \end{equation}
    Si \( i<\alpha\) alors dans chaque terme de Leibnitz\footnote{Lemme \ref{}.}, il y aura un facteur \( (x-x_0)\), et la prise de \( \delta_{x_0}\) annulera. Si par contre \( i\geq \alpha\) alors il y aura le terme
    \begin{equation}
...
\item[Notations]
    ...
    \begin{equation}        \label{EQooCNPIooSiqAzt}
        ...
        \end{aligned}
    \end{equation}
    Les familles de semi-normes \( (p_{K,m})\) et \( (q_{K,m})\) ne sont pas indexées par les mêmes ensembles et n'ont pas le même domaine. Bien que les formules \eqref{EQooYZWHooVRrnlU} et \eqref{EQooCNPIooSiqAzt} se ressemblent, elles n'ont rien à voir l'une avec l'autre.
...
\begin{lemma}[\cite{BIBooPITOooZANjFn}]
    Soit un ouvert \( \Omega\subset \eR^d\). Soient des ouverts \( \{ \mO_i \}_{i\in I}\) tels que \( \bigcup_{i\in I}\mO_i=\Omega\). Soient des distributions \( T_i\in \swD'(\mO_i)\) telles que \( r_{\mO_i\cap\mO_j}(T_i)=r_{\mO_i\cap\mO_j}(T_j)\) pour tout \( i,j\) tels que \( \mO_i\cap\mO_j\neq\emptyset\).

    Nous considérons un compact \( K\) dans \( \Omega\) ainsi que des parties finies \( J,L\) de \( I\) telles que \( K\subset \bigcup_{j\in J}\mO_j\) et \( K\subset \bigcup_{l\in L}\mO_l\).

    Soient des partitions de l'unité \( \{ \psi_j \}_{j\in J}\) et \( \{ \varphi_k \}_{l\in L}\) subordonnées à ces deux recouvrements de \( K\). Alors
    \begin{equation}
        \sum_{j\in J}T_k(\psi_j\phi)=\sum_{l\in L}(\varphi_l\phi)
    \end{equation}
    pour tout \( \phi\in  C^{\infty}(\Omega)\) tel que \( \supp(\phi)\subset K\).
\end{lemma}


\begin{proposition}[Principe de recollement\cite{BIBooPITOooZANjFn}]
    Soient un ouvert \( \Omega\) de \( \eR^d\) ainsi que des ouverts \( \{ \mO_i \}_{i\in I}\) de \( \eR^d\) tels que \( \bigcup_{i\in I}\mO_i=\Omega\).

    Pour chaque \( i\in I\) nous supposons avoir un élément \( T_i\in\swD'(\Omega_i)\) tels que
    \begin{equation}
        r_{\mO_i\cap \mO_j}(T_i)=r_{\mO_i\cap\mO_j}(T_j)
    \end{equation}
    pour tout \( i,j\in I\) tels que \( \mO_i\cap\mO_j\neq \emptyset\).

    Alors il existe une unique distribution \( T\in \swD'(\Omega)\) telle que \( r_{\mO_i}(T)=T_i\) pour tout \( i\) dans \( I\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
    \item[Unicité]
        Soient \( T\) et \( U\) des distributions qui satisfont à la demande. Nous posons \( S=T-U\) et nous allons prouver que \( S=0\). Grâce à la linéarité de \( r_{\mO_i}\) (lemme \ref{LEMooCXIZooAbeMpF}),
        \begin{equation}
            r_{\mO_i}(S)=0
        \end{equation}
        pour tout \( i\) dans \( I\).

        Soit \( \phi\in \swD(\Omega)\); nous notons \( K=\supp\phi)\subset\Omega\). Vu que les \( \mO_i\) recouvrent \( \Omega\), ils recouvrent \( K\). Il existe dont une partie finie \( J\) de \( I\) telle que \( K\subset\bigcup_{j\in J}\mO_j\). Et si nous prenions une partition de l'unité\footnote{Théorème \ref{THOooQFCQooSlgLpz}.} subordonnée à ces \( \mO_j\) ?

        Soit \( \{ \psi_j \}_{j\in J}\) une telle partition de l'unité. Vu que le support de \( \phi\) est borné, le support de \( \psi_k\phi\) est également borné. Et comme un support est toujours fermé, l'application \( \psi_k\phi\) est dans \( \swD(\mO_k)\). En utilisant consciencieusement le lemme \ref{LEMooCXIZooAbeMpF}\ref{ITEMooGXSKooYomqpg} nous avons
        \begin{equation}
            S(\phi)=S\big( \sum_{k=1}^n\psi_k\phi \big)=\sum_kS(\psi_k\phi)=\sum_{k=1}^n(r_{\mO_k}S)(\psi_k\phi)=0
        \end{equation}
        parce que \( r_{\mO_k(S)}=0\).
    \end{subproof}
    <++>
\end{proof}
<++>
