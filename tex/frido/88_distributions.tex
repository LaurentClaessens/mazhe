...
\begin{lemma}[Formule de Leibnitz\cite{BIBooYDMJooGDtdbo,BIBooWWKNooJcsaRy}]      \label{LEMooOLQTooEHJuBc}
	Soient des fonctions \( f,g\) chacune dérivable \( p\) fois sur \( \eR^d\). Alors pour tout multiindices\footnote{Voir \ref{NORMooRRZCooMOKAzY}.} \( \alpha\) de taille plus petite ou égale à \( p\), nous avons
	\begin{equation}
