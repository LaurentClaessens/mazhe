...
\begin{proof}
    ...
    \item[La fonction \( \xi\)]
        Le corollaire \ref{CORooHHZXooXmwGmC} nous permet de considérer \( \xi\in \swD\big( B(0,R) \big)\) telle que
        \begin{equation}
            \int_{\eR}\xi(x)dx=1.
        \end{equation}
    ...
    \item[La récurrence]
        ...
        Soit donc \( \phi\in\swD(\Omega\times \eR^{n+1})\). La fonction \( y\mapsto T\big( \phi(.,y) \big)\) est continue et à support compact. Donc Fubini \ref{ThoWTMSthY} s'applique et nous allons permuter les intégrales une à une avec \( T\). Avant cela nous définissons quelque fonctions intermédiaires.
