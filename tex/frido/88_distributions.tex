...
\begin{definition}[Toplogie sur \( C^{\infty}_c(K)\)]		\label{DEFooJJJYooIrekSp}
	...
	\begin{equation}
		p_{K,m}(f)=\sup_{| \alpha |\leq m}\| \partial^{\alpha}f \|_K
	\end{equation}
\end{definition}
\index{topologie!sur \(  \swD(K)\) }

\begin{definition}[Topologie sur \( C^{\infty}_c(\Omega)\)]		\label{DEFooVSCRooLyYBzT}
	Soit un ouvert \( \Omega\) dans \( \eR^n\). Une seminorme \( p\) sur \( C^{\infty}_c(\Omega)\) est \defe{bonne}{bonne seminorme} si pour tout compact \( K\subset \Omega\), la restriction \( p|_{C^{\infty}_c(K)}\) est continue.
...
\begin{proposition}[\cite{BIBooFBCEooQXpHce, MonCerveau}]
	Nous avons
	\begin{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[\( k_i\to \infty\)]
		%-----------------------------------------------------------
		Prouvons que \( \lim_{i\to \infty}k_i=\infty\). Par l'absurde, nous supposons que \( k_i<n\) pour tout \( i\). Nous posons alors
		\begin{equation}
		...
		\end{equation}
		En posant \( i=s\), nous trouvons que \( \supp(\phi_{k_s})\subset K_s\), ce qui est faut. Nous en déduisons que \( (k_i)_{i\in \eN}\) n'est pas bornée.
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		Il s'agit bien d'un max parce qu'il n'y a qu'un nombre fini de termes non nuls.
...
\begin{proof}
	...
	\begin{subproof}
		...
		Avant de continuer, je vous conseille de bien lire l'enchainement des définitions \ref{DefFGGCooTYgmYf}, \ref{DEFooJJJYooIrekSp}, \ref{DEFooVSCRooLyYBzT}, ainsi la proposition \ref{PropQPzGKVk} pour comprendre pourquoi \( f_n\stackrel{\swD(A)}{\longrightarrow}0\) si et seulement si \( p_{K_0,m}(f_n)\to 0\) pour tout \( m\) pour un certain compact \( K_0\) contenant tous les supports de \( f_n\).
