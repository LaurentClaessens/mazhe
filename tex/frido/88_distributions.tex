...
\begin{example}[La fonction de Heaveside n'a pas de dérivée faible]     \label{EXooRVGHooTWOCtF}
    ...
    pour tout \( \varphi\in\swD(x>0)\). La proposition \ref{PropAAjSURG} nous dit que \( g=0\) presque partout sur \( \mathopen] 0 , \infty \mathclose[\).
        ...
        La fonction \( H\) n'a donc pas de dérivée faible. Notons cependant que cela ne présume en rien la possibilité d'accepter une dérivée au sens des distributions\footnote{Nous verrons dans l'exemple \ref{EXooFTHGooPgOclr} que la dérivée de Heaveside au sens des distributions est le delta de Dirac.}.
\end{example}
...
Nous avons déjà vu dans l'exemple \ref{EXooRVGHooTWOCtF} que la fonction de Heaveside n'a pas de dérivée faible. Voici tout de même la dérivée de la fonction de Heaveside au sens des distributions.
\begin{example}     \label{EXooFTHGooPgOclr}
    Intuitivement, la fonction de Heaveside a une dérivée nulle partout sauf en \( x=0\) où sa dérivée serait infinie; nous nous attendons à un delta de Dirac. Par définition, la dérivée de \( H\) au sens des distributions est la distribution \( H'\) qui fait
    \begin{equation}
        H'(\varphi)=-\langle H, \varphi'\rangle 
    \end{equation}
    pour tout élément \( \varphi\in\swD(\eR)\).
\end{example}
<++>
