...
\begin{proposition}[\cite{LoFdlw}]      \label{PROPooSAOTooIlpJoY}
	...
	\end{enumerate}
	% Une preuve est est dans l'écrit d'algèbre de 2013.
\end{proposition}
...
\begin{probleme}
	Je crois qu que les éléments de la décomposition polaire sont des polynômes en \( M\). Écrivez moi si vous pouvez confirmer ou infirmer.
\end{probleme}
...
\begin{definition}
	Un point \( a\) d'un ensemble convexe \( C\) est un \defe{point extémal}{point extrémal dans un convexe} si \( C\setminus \{ a \}\) est convexe.
\end{definition}
