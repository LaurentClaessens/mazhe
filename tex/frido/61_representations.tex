...
\begin{proposition}[\cite{BIBooXGHGooNQUvHR}]	\label{PROPooVWHJooXaMvTj}
	...
	\end{equation}
	Cette application \( \tilde \phi_x\) est une bijection.
\end{proposition}


\begin{theorem}[\cite{BIBooXGHGooNQUvHR}]     \label{ThojrLKZk}
	Soit \( G\) un groupe topologique localement compact et \( \sigma\)-compact\footnote{Définition \ref{DEFooSOLWooWlSiUn}.} agissant continûment et transitivement sur un espace topologique localement compact\footnote{Définition~\ref{DefEIBYooAWoESf}.} \( E\). Soit \( x\in E\). Nous notons \( \Stab(x)\) le stabilisateur\footnote{Définition \ref{DEFooMDYGooLrOERP}.} de \( x\).
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	est un homéomorphisme\footnote{Isomorphisme d'espaces topologiques, définition \ref{DEFooYPGQooMAObTO}.}.
	%TODOooIJGAooObcQjK. prouver ça.
\end{theorem}
