...
% Copyright (c) 2011-2013,2016-2020,2022
%   Laurent Claessens
...
%TODO: Formellement, G_x n'est pas introduit ici...
    Soit \( G\) un groupe topologique localement compact et dénombrable à l'infini\footnote{Cela signifie qu'il est une réunion dénombrable de compacts} agissant continûment et transitivement sur un espace topologique localement compact\footnote{Définition~\ref{DefEIBYooAWoESf}.} \( E\). Alors l'application
    \begin{equation}
        \begin{aligned}
...
\begin{proof}
    ...
    \end{equation}
    Cet ensemble est le même que \( S^{2n-1}\) parce que \( |z_k|^2=x_k^2+y_k^2\). Nous avons une bijection continue entre \( S^{n-1}\) et \( S^{n-1}_{\eC}\) et donc un homéomorphisme (lemme~\ref{LemIbrsFT}). Soit \( a\in S^{n-1}_{\eC}\), nous avons
    \begin{subequations}
        \begin{align}
            G\cdot a  & =S^{n-1}_{\eC}  \\
                G_a   & \simeq G(n-1).
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    En tant qu'image d'un connexe par une application continue, \( S_m\) est connexe par la proposition~\ref{PropGWMVzqb}. Puisque les \( S_m\) ont l'identité en commun, le groupe dérivé
    \begin{equation}
...
\begin{proof}
    ...
    \item[\ref{ITEMooZHYRooFGKaifiv} implique~\ref{ITEMooZHYRooFGKaifi}] Évident \ldots encore faut-il se souvenir d'invoquer l'invariance du déterminant par changement de base.
    \end{subproof}
...
\begin{proof}
    ...
    \end{equation}
    alors que nous savons que \( \| Tx \|,\| Ux \|\leq 1\), donc \( \| Tx \|=\| Ux \|=1\). La seule possibilité est d'avoir \( \lambda=1\) et donc que \( U=T\) parce que nous avons \( Tx=Ux\) de norme \( 1\), pour tout \( x\). Au final \( A\) n'est pas le milieu d'un segment dans \( \mB\).
