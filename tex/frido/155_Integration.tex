...
\begin{definition}[chemin de Jordan]        \label{DEFooPGIBooCrLNxs}
    Pour rappel, une chemin \( \gamma\colon \mathopen[ 0 , 1 \mathclose]\to \eR^n\) est \defe{régulier}{régulier!chemin} si il est \( C^1\) et si \( \gamma(t)\neq 0\) pour tout \( r\). Le chemin est de \defe{Jordan}{chemin de Jordan} si \( \gamma(1)=\gamma(0)\) et si \( \gamma\colon \mathopen[ a , b [\to \eR^n\) est injective.
\end{definition}
