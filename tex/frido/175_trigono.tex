...
\begin{normaltext}[Formule d'Euler]		\label{NORMooXOVSooEwjecU}
	En posant \( x=\pi\) dans \eqref{EQooRVPJooTMwNTU} nous trouvons la soit-disant géniale formule d'Euler :
	\begin{equation}
		e^{i\pi}=-1.
	\end{equation}
	...
	Lorsque \( x\) est réel, nous avons la formule \( \exp(x)=e^x\) qui est un théorème tout à fait non triviale liant la fonction exponentielle et la fonction puissance. Il n'y a pas de tel résultat pour \( z\) complexe. Dans le cas de \( z\) complexe, la formule \( \exp(z)=e^z\) est juste une définition de la fonction puissance dans les complexes.
