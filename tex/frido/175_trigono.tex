...
\begin{proof}
    Le lemme \ref{LEMooHWQYooXcNLts} dit que si \( T\) est une période de \( f\), alors \( T\) est également une période de \( f'\). Le lemme \ref{LEMooBBCAooHLWmno} donne les liens de dérivation entre \( \sin\) et \( \cos\).

    La proposition \ref{PROPooFRVCooKSgYUM} dit que \( \cos\) est périodique. Vu que \( \sin=-\cos'\) par le lemme \ref{LEMooBBCAooHLWmno}, la fonction \( \sin\) est périodique. Si \( T\) est une période de \( \cos\), alors \( T\) est une période de \( \sin\).
