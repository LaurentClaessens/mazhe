...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Paramétrisation du cercle}
%---------------------------------------------------------------------------------------------------------------------------
...
\begin{proposition}[Taylor pour cosinus]     \label{PROPooNPYXooTuwAHP}
    ...
    \begin{enumerate}
        \item
	        Il existe une fonction $h_1\in o(x^2)$ telle que 
            \begin{equation}
                \cos(x)=1-\frac{ x^2 }{ 2 }+h_1(x).
            \end{equation}
        \item
            Il existe aussi une autre fonction $h_2\in o(x^4)$ telle que 
            \begin{equation}
                \cos(x)=1-\frac{ x^2 }{ 2 }+\frac{ x^4 }{ 4! }+h_2(x).
            \end{equation}
    \end{enumerate}
\end{proposition}

\begin{example}
    Cherchons le développement limité à l'ordre \( 5\) de \( \tan(x)=\frac{ \sin(x) }{ \cos(x) }\). Nous utilisons les formules \eqref{subeqCBQooTZGxQpcos} et \eqref{subeqCBQooTZGxQpsin} ainsi que les dérivées données dans la proposition \ref{LEMooBBCAooHLWmno} et les valeurs spéciales \eqref{SUBEQooTTNNooXzApSM} pour savoir les développements de sinus et cosinus\footnote{Encore une fois, vous êtes très fortement encouragés à calculer vous-même ces développements à partir de la formule de Taylor-Young \ref{EQooBKZDooTqYyIB}.} :
    \begin{subequations}
        \begin{align}
