...
% Copyright (c) 2011-2013,2016-2023, 2025-2026
%   Laurent Claessens
...
%-------------------------------------------------------
\subsection{Exponentielle avec «\( e\)»}
%----------------------------------------------------

\begin{normaltext}		\label{NORMooUZZDooXUEPvT}
	La vraie fonction exponentielle utilisée en mathématique est la fonction \( z\mapsto\exp(z)\) définie par la série. Avec quelques contorsions de notations, on peut faire le lien avec \( z\mapsto e^z\). La vraie proposition intéressante est la proposition \ref{PropCELWooLBSYmS} qui montre que si \( x\) est réel, alors \( \exp(x)=e^x\).

	En ce qui concerne les nombres complexes, \( e^z\) est pratiquement définie comme étant \( \exp(z)\) juste pour avoir le plaisir de dire «\( e^{\i\pi}=-1\)» et dire que c'est une formule incroyable. Alors qu'en réalité le nombre \( e\) n'apparaît pas dans cette formule.
\end{normaltext}

\begin{normaltext}		\label{NORMooROGZooDtzzpQ}
...
\begin{definition}      \label{DEFooRBTDooNLcWGj}			% Ce label ne devrait pas être appelé ailleurs que dans cette section.
...
\begin{proposition}     \label{PROPooWSDKooJREQGk}	% Ce label ne devrait pas être appelé ailleurs que dans cette section.
...
\begin{proof}
	Nous prenons \( z=x+iy\) avec \( x,y\in \eR\). Par définition \( e^z=e^xe^{iy}\) où \( e^x=\exp(x)\) par la proposition \ref{PropCELWooLBSYmS} et \( e^{iy}=\exp(iy)\) par définition. Bref :
	\begin{equation}
		e^{x+iy}=\exp(x)\exp(y)=\left( \sum_{k=0}^{\infty}\frac{ x^k }{ k! } \right)\left( \sum_{l=0}^{\infty}\frac{ (iy)^l }{ l! } \right).
	\end{equation}
	Nous sortons maintenant de notre manche le produit de Cauchy \ref{PROPooPKGEooZKyxwo} :
	\begin{subequations}
		\begin{align}
			\exp(x)\exp(iy) & =\sum_{n=0}^{\infty}\sum_{k=0}^n\frac{ x^k }{ k! }\frac{ (iy)^{n-k} }{ (n-k)! }                                         \\
			                & =\sum_{n=0}^{\infty}\frac{1}{ n!}\sum_{k=0}^n\frac{ n! }{ k!(n-k)! }x^k(iy)^{n-k}                                       \\
			                & =\sum_{n=0}^{\infty}\frac{1}{ n!}\sum_{k=0}^n\binom{ n }{ k }x^k(iy)^{n-k}        & \text{def. \ref{DEFooHXNFooOBKgqD}} \\
			                & =\sum_{n=0}^{\infty}\frac{1}{ n!}(x+iy)^n                                         & \text{prop. \ref{PropBinomFExOiL}}  \\
			                & =\exp(x+iy)
		\end{align}
	\end{subequations}
	Et voila.
...
\begin{lemma}[\cite{MonCerveau}]        \label{LEMooTDGKooWdpUTD}	% Ce label ne devrait pas être appelé ailleurs que dans cette section.
...
Je vous invite à remarquer qu'aucune des propositions de ce paragraphe n'est utilisé ailleurs dans le Frido. Donc la fonction \( z\mapsto e^z\) est complètement inutile en math. Ce qui est réellement utilisé est \( z\mapsto\exp(z)\).
...
\begin{normaltext}[Formule d'Euler]		\label{NORMooXOVSooEwjecU}
	...
	Le contenu mathématique de la formule d'Euler est ceci :
	\begin{itemize}
		\item
		      On défini \( \exp\), \( \cos\) et \( \sin\) par leurs séries.
		\item
		      On prouve que \( \exp(ix)=\cos(x)+i\sin(x)\) (facile).
		\item
		      On prouve que \( \sin\) et \( \cos\) sont périodiques (pas facile), on note \( \pi\) la demi-période.
		\item
		      On trouve les valeurs particulières de \( \cos(\pi)\) et \( \sin(\pi)\).
		\item
		      On découvre que \( \exp(i\pi)=-1\).
	\end{itemize}
	Le nombre \( e\) n'intervient à aucun moment.

	Après, par analogie avec la proposition \ref{PropCELWooLBSYmS} (pas triviale du tout) qui dit \( \exp(x)=e^{x}\) pour les réels \( x\), on définit \( e^z=\exp(z))\). Mais là c'est juste une notation par analogie avec un fait non trivial uniquement valide sur le réels.

	Si vous aimez les formules qui lient un peu toutes les constantes, vous avez la formule de Stirling.
\end{normaltext}
...
\begin{proposition}	\label{PROPooPMWWooGgGfwr}
	Une autre formule pour tout \( x,y\in \eR\) :
	\begin{equation}
...
\begin{proof}
	Nous partons des formules d'addition d'angles du lemme \ref{LEMooJAWBooJGfZIL} pour calculer
	\begin{equation}
		\cos(a+b)-\cos(a-b)=-2\sin(a)\sin(b).
	\end{equation}
	Et il suffit maintenant de poser \( x=a+b\), \( y=a-b\).
\end{proof}
...
\begin{proposition}[\cite{ooUMDHooHrJpfV,MonCerveau}]      \label{PROPooMWMDooJYIlis}
	...
	\begin{enumerate}
		...
		\item       \label{ITEMooQKPKooEPeHER}
		      ...
		      \end{equation}
		\item               \label{ITEMooMEXUooGBfSInJ}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item
		      %TODOooIPNMooAeiyKf. Prouver ça. C'est-à-dire ITEMooFHCVooQxKPKu.
		\item
...
\begin{lemma}[Positivité\cite{MonCerveau}]      \label{LEMooFESYooBoiuol}
	...
	\begin{enumerate}
		...
		\item		\label{ITEMooBRVEooFhxohI}
		      Si \( x\in\mathopen] 0,\pi\mathclose[\) alors \( \sin(x)>0\).
		      %TODOooOXEJooYkwZLY. Prouver ça.
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	...
	\begin{subproof}
		...
		C'est la proposition \ref{PROPooMWMDooJYIlis}\ref{ITEMooMEXUooGBfSInJ}.
		\spitem[Pour \ref{ITEMooANEPooLGmYtc}]
		C'est la proposition \ref{PROPooMWMDooJYIlis}\ref{ITEMooWFNUooYAybDB}.
		\spitem[Pour \ref{ITEMooXXISooBycstN}]
		%-----------------------------------------------------------
		C'est la proposition \ref{PROPooMWMDooJYIlis}\ref{ITEMooWFNUooYAybDB}.
		...
		\spitem[Pour \ref{ITEMooBRVEooFhxohI}]
		%-----------------------------------------------------------
		%TODOooOXEJooYkwZLY. Prouver ça.
	\end{subproof}
\end{proof}

\begin{proposition}[Tableau de signes]	\label{PROPooJGEHooOohyAj}
	Voici le tableau de signe de \( \sin\) et \( \cos\) :
	\begin{equation}
		...
		\end{array}
	\end{equation}
	La \( 2\pi\)-période permet de savoir le signe partout dans \( \eR\).
\end{proposition}

\begin{proof}
	Les valeurs en \( 0\) sont immédiates en substituant \( x=0\) dans la série. Entre \( 0\) et \( \pi/2\), c'est la proposition \ref{PROPooMWMDooJYIlis}\ref{ITEMooWFNUooYAybDB}. Les valeurs en \( \pi/2\) sont \ref{PROPooMWMDooJYIlis}\ref{ITEMooWFNUooYAybDB}. Pour le reste, nous pouvons utiliser \ref{PROPooMWMDooJYIlis}\ref{ITEMooIRALooBMGOXP}.
\end{proof}


\begin{lemma}[\cite{BIBChatGPT}]       \label{LEMooPARBooTXbbiB}
	Soient \( x,y\in \eR\).
	\begin{enumerate}
		\item		\label{ITEMooDPLMooEflCUZ}
		      Nous avons \( \cos(x)=\cos(y)\) si et seulement si
		      \begin{equation}
			      y\in \{ x+2k\pi \}_{k\in \eZ}\cup\{ -x+2k\pi \}_{k\in \eZ}.
		      \end{equation}
		\item		\label{ITEMooXPNVooVtFqEY}
...
\begin{proof}
	Pour \ref{ITEMooDPLMooEflCUZ}, nous partons de la proposition \ref{PROPooPMWWooGgGfwr} qui nous permet de dire que \( \cos(x)=\cos(y)\) si et seulement si
	\begin{equation}
		\sin\left( \frac{ x+y }{2} \right)\sin\left( \frac{ x-y }{2} \right)=0.
	\end{equation}
	Le tableau de signe de la proposition \ref{PROPooJGEHooOohyAj} nous dit quand les sinus s'annulent : \( \sin(t)=0\) si et seulement si \( t\in\{ k\pi \}_{k\in \eZ}\). Donc si \( \cos(x)=\cos(y)\), nous avons soit \( (x+y)/2\in\{ k\pi \}_{k\in \eZ}\) soit \( (x-y)/2\in\{ k\pi \}_{k\in \eZ}\). Dans le premier cas nous avons \( y\in\{ -x+2k\pi \}_{k\in \eZ}\) et dans le second cas \( y\in\{ x+2k\pi \}_{k\in \eZ}\).

	Pour le sens inverse, nous supposons que $ y\in \{ x+2k\pi \}_{k\in \eZ}\cup\{ -x+2k\pi \}_{k\in \eZ}$, et on refait le calcul à l'envers pour vérifier qu'alors \( \cos(x)=\cos(y)\).

	La preuve de \ref{ITEMooXPNVooVtFqEY} est du même tonneau\quext{Mais je n'ai pas vérifié. Si ça bloque quelque part, écrivez-moi.}.
\end{proof}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\end{equation}
		Nous avons déjà vu que \( \sin(\pi/3)=\sqrt{ 3 }/2\). En posant \( x=\sin(\pi/6)\) nous avons également \( \cos(\pi/6)=\sqrt{ 1-x^2 }\) parce que nous savons que la fonction cosinus est positive sur \( \mathopen[ 0 , \pi/2 \mathclose]\) (proposition \ref{PROPooMWMDooJYIlis}\ref{ITEMooMEXUooGBfSInJ}). Nous avons donc l'équation
		\begin{equation}
