...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    est continue par le lemme \ref{}. Donc elle commute avec la limite. Nous notons \( x\) et \( y\) les limites des suites \( (x_n)\) et \( (y_n)\). Le calcul est le suivant\footnote{Assurez vous d'être capable de justifier les étapes.} :
    \begin{equation}
...
\begin{lemma}   \label{LEMooRWJYooOIJkZc}
    Sur \( \eR^2\), l'application \( N(x,y)=| x |+| y |\) est une norme qui ne dérive pas d'un produit scalaire\footnote{La norme d'un produit scalaire est la proposition  \ref{PropEQRooQXazLz}.}.
\end{lemma}

\begin{proof}
    Le fait que ce soit une norme est la proposition \ref{PROPooCLZRooIRxCnZ}.

    Nous allons voir qu'elle ne dérive pas d'un produit scalaire en montrant qu'elle ne vérifie pas l'identité du parallélogramme.
...
\end{itemize}
Une question qui vient est la compatibilité entre ces deux constructions. Est-ce que la topologie associée à la norme maximum est la topologie produit ? Oui.
...
\begin{proof}
    Nous allons présenter la preuve dans le ces d'un produit de deux espaces vectoriels normés \( V\times W\).
