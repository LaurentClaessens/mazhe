...
\begin{proof}
    Soient un compact \( K\) de \( \eR\) et une suite \( (x_k)\) contenue dans \( \eK\). Soit \( a\in K\). Si une sous-suite de \( (x_k)\) converge vers \( a\), alors pour tout voisinage \( V_a\) de \( a\) nous avons \( x_k\in V_a\) pour une infinité de valeurs de \( k\)\footnote{Cela n'est pas tout à fait équivalent à dire que \( V_a\) contient une infinité de valeurs de la suite, car la suite pourrait contenir une sous-suite constante.}.
    ...
    Les voisinages \( \mO_a\) recouvrent \( K\); nous pouvons en extraire un sous-recouvrement fini (c'est la définition \ref{DefJJVsEqs}). Nous avons donc des points \( a_1,\ldots, a_n\) tels que 
    \begin{equation}
