...
\begin{proof}
    ...
    \begin{subequations}
        \begin{numcases}{}
            f(W\cap W'\cap A)\subset f(W\cap A)\subset V, \text{ et}\\
...
\begin{proposition}\label{PropXKUMiCj}
    ...
    \begin{enumerate}
        ...
        \item\label{ItemXYmGHFaii}
            Si \( \{ F_i \}\) est une famille de fermés telle que \( \bigcap_{i\in I}F_i \bigcap K =\emptyset\), alors il existe un sous-ensemble fini \( A\) de \( I\) tel que \( \bigcap_{i\in A}F_i \bigcap =\emptyset\).
...
\begin{proof}
    ...
    \begin{equation}
        \bigcap_{i\in A}F_i \bigcap K =\emptyset.
    \end{equation}
...
\begin{proof}
    ...
    \end{equation}
    L'ensemble \( W=\bigcap_{x\in S}W_x\) est une intersection finie d'ouverts autour de \( y\) et est donc un ouvert autour de \( y\). De plus \( W\) n'intersecte pas \( K\) parce que si \( x\in K\) alors \( x\in V_s\) pour un certain \( s\in S\) par conséquent \( x\) n'est pas dans \( W_s\) et donc a fortiori pas dans \( W \).
...
\begin{proof}
    ...
    \begin{enumerate}
        \item
            \( C_j\subset A\cup B\) parce que \(A \cup B\) recouvrent tous les \( C_i \),
        \item
...
Une erreur courante est de croire que la borne est la limite\dots ce que ce lemme n'affirme pas.
...
\begin{proof}
	Soit $(x_n)$ une suite contenue dans la partie bornée $A\subset\eR$. Nous disons qu'un élément $x_k$ de la suite est \emph{maximal} s'il est plus grand ou égal à tous les suivants : $x_k\geq x_{k'}$ dès que $k'\geq k$.

	Si la suite a un nombre infini d'éléments maximaux, alors la suite des éléments maximaux est décroissante. Si nous n'avons qu'un nombre fini de points maximaux, alors la suite de départ est croissante à partir du dernier point maximal\quext{Cette affirmation est-elle vraie?}.
...
\item $10/10$ majore les notes qu'on peut obtenir\footnote{À moins qu'il y ait des histoires de bonus\dots} à un devoir.
\item l'intervalle $[4,\infty[$ n'a pas de majorants.
\end{itemize}
\end{example}

\begin{definition}
Le \defe{supremum}{supremum} d'un ensemble est le plus petit majorant. En d'autres termes, $s$ est un supremum de $A$ si tout nombre plus petit que $s$ ne majore pas $A$, ou encore,
\[
...
\begin{probleme}
Entre autres: justifier qu'il est unique, avant de parler \emph{du} sup, et replacer la déf au bon endroit (peut-être la section?)
\end{probleme}
