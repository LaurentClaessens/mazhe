...
\begin{definition}[Limite d'une fonction]\label{DefYNVoWBx}
    Soient \( X\) et \( Y\) des espaces topologiques, \( A\subset X\) et \( a\in\bar A\). L'élément \( y\in Y\) est une \defe{limite}{limite!d'une fonction} de \( f\colon A\to Y\) en \( a\) si pour tout voisinage \( V\) de \( y\), il existe un voisinage \( W\) de \( a\) dans \( X\) tel que pour tout \( x\neq a\) dans \( W\), \( f(x)\in V\).
...
\begin{remark}
    Nous ne saurions trop insister sur la condition \( x\neq a\) dans la définition. Cela est intimement liée à ce que nous racontons dans \ref{SUBSECooVHKCooYRFgrb}.
\end{remark}
