...
\begin{proof}
	Tout repose sur le fait que pour tout \( x,y\in \eR\), \( | x |\leq | x+iy |\). Prouvons la continuité de \( u\). Soit \( a\in K\). Soit \( \epsilon>0\) et \( \delta>0\) tel que si \( | f(a+\delta)-f(a) |<\epsilon\). Alors nous avons
	\begin{subequations}
		\begin{align}
			| u(a+\delta)-u(a) |\leq | u(a+\delta)-u(a)+iv(a+\delta)-iv(a) | \\
			 & =| f(a+\delta)-f(a) |                                         \\
			 & <\epsilon.
		\end{align}
	\end{subequations}
	En ce qui concerne les normes pour tout \( x\in K\) nous avons
	\begin{equation}
		| u(x) |\leq | u(x)+iv(x) |=| f(x) |.
	\end{equation}
\end{proof}
