...
\begin{normaltext}
    ...
    \begin{itemize}
        ...
        \item «azertyA2019A» est un mot de passe fort.
    \end{itemize}
    Il n'en est rien. Le premier est considérablement meilleur que le second, même si le second, très superficiellement, mélange les lettres majuscules, minuscules et les chiffres.
...
\begin{normaltext}
    La démonstration donnée dans \cite{BIBooXUZHooOHWxiF} s'assure d'abord de l'existence d'une intégrale (lemme \ref{LEMooWKSWooPptdEm}) et fait ensuite pose \( A=\int_0^1g_a(t)dt\) et fait le calcul suivant :
    \begin{equation}
...
\begin{normaltext}
    Nous savons au moins par culture générale que l'exponentielle est l'unique solution de l'équation différentielle
    \begin{subequations}
        \begin{numcases}{}
            y'=y\\
            y(0)=1.
    ...
    \begin{equation}
        g_a'(x)=g_a(x)g_a'(0),
    \end{equation}
    nous n'en sommes pas loin. Il faut encore savoir si il existe un \( a>0\) tel que \( g_a'(0)=1\). Notre culture générale nous dit qu'un tel réel existe et est la fameuse constante \( e\).

    Nous nous attelons maintenant à la tâche de montrer l'existence de la chose.
\end{normaltext}

\begin{lemma}[\cite{BIBooXUZHooOHWxiF}]
    Si \( a>0\) alors 
    \begin{enumerate}
        \item
            \( g_a'(0)\neq 0\),
        \item
            en posant \( \alpha=1/g'_a(0)\) nous avons
            \begin{equation}
                g_{a^{\alpha}}'(0)=1.
            \end{equation}
    \end{enumerate}
...
\begin{proof}
    ...
    \begin{equation}
        g_a'(x)=g_a(x)g_a'(0).
    \end{equation}
    Donc \( g_a'(0)=0\) implique que \( g_a=0\), ce qui n'est pas le cas.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
vérifie l'équation fonctionnelle \eqref{EQooULHBooByFVec}.

Nous pouvons nous demander à quel point cette équation caractérise la fonction puissance.
...
\begin{proposition}[\cite{BIBooXUZHooOHWxiF}]       \label{PROPooJDPEooYTDVtU}
    La fonction \( g_a\) vérifie l'équation fonctionnelle et il existe un unique \( e>0\) tel que \( g_e\) vérifie l'équation exponentielle.
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Pour \( q=1/n\)]
            ...
            \end{equation}
            Cela implique que \( f(\frac{1}{ n })^n=a\). La proposition \ref{PROPooXQYFooPxoEHE} indique qu'il existe un unique \( x>0\) tel que \( x^n=a\). Vu que nous savons déjà que \( f\) est partout positive, cette contrainte fixe \( f(1/n)\) et la définition \ref{DEFooJWQLooWkOBxQ} nous permet d'écrire
            \begin{equation}
                f(\frac{1}{ n })=a^{1/n}.
            \end{equation}
        \item[Pour \( q\in \eQ\)]
    ...
    \begin{subproof}
        \item[Démonstration de \ref{ITEMooQHOMooNVzSxn}]
            ...
            Nous considérons les parties\footnote{Il est de bon gout de citer le lemme \ref{LemooHLHTooTyCZYL} pour dire qu'ils sont non vides.}
            \begin{subequations}
                \begin{align}
                    A=\{ q\in \eQ\tq q<x \}\\
                    B=\{ q\in \eQ\tq q>x \}.
    ...
    Nous faisons maintenant la preuve de \ref{ITEMooCNXOooZcrxeB}.
\end{proof}
<++>
