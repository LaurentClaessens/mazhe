...
\begin{definition}[\cite{TrenchRealAnalisys}]
    Soit un espace (dont la nature n'est pas très importante) \( \Omega\) et une partie \( A\) de \( \Omega\). Nous notons
    \begin{equation}
    \| g \|_A=\sup_{x\in A}\| g(x) \|
    \end{equation}
    lorsque \( g\) est une fonction \( g\colon \Omega\to \eC\). C'est la norme supremum limitée à la partie \( A\).
