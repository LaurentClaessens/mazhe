...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Si \( 0<r\leq 1\)]
		%-----------------------------------------------------------
		Nous utilisons la proposition \ref{PROPooHEWFooJpfzEL} pour chaque \( \omega\in \Omega\) :
		\begin{equation}
			\big|  f(\omega)+g(\omega)  \big|^r\leq \Big( | f(\omega) |+| g(\omega) | \Big)^r\leq | f(\omega) |^r+| g(\omega) |^r.
		\end{equation}
		En intégrant sur \( \Omega\) nous avons le résultat.
		\spitem[Si \( r\geq 1\)]
		%-----------------------------------------------------------
		Soit \( \omega\in \Omega\). Nous utilisons la proposition \ref{PROPooPMIGooRAjROX} :
		\begin{equation}
			| f(\omega)+g(\omega) |^r\leq 2^{r-1}\big( | f(\omega) |^r+| g(\omega) |^r \big).
		\end{equation}
		En intégrant les deux côtés sur \( \Omega\), nous avons le résultat.
	\end{subproof}
\end{proof}
