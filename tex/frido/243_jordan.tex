...
\begin{theorem}[\cite{LEMooEJRMooNJhMov}]		\label{THOooDGXSooMMHTds}
	Soit un compact \( K\) dans \( \eC\). Alors
	\begin{equation}
		G(K)=\mG(K)/\mE(K)\simeq \eZ^N
	\end{equation}
	où \( N\) est le nombre de composantes connexes bornées de \( \eC\setminus K\)\footnote{Qui est au maximum dénombrable par le lemme \ref{LEMooBHOGooXxBYGA}.}.
\end{theorem}<++>
