...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		est continue sur le compact \( S\). Donc elle a un minimum strictement positif (parce que \( S\) et \( G\) ne s'intersectent pas). Nous considérons \( r>0\) strictement plus petit que ce minimum. L'ensemble \( \{ B(x,r) \}_{x\in S}\) recouvre le compact \( S\) avec des ouverts. Donc il y a un sous-recouvrement fini que nous notons \( \{ B(x_i,r) \}_{i=1,\ldots,n}\). Nous notons aussi \( D_j=\overline{B(x_i,r_i)}\).

		Nous repassons ensuite ces boules une par une de façon à s'assurer qu'aucune n'est tangente à une autre. Pour cela, nous allons nous assurer qu'aucune n'est tangente à une précédente. Supposons que \( D_i\) est tangente à \( D_j\) avec \( i>j\). En remplaçant \( r_i\) par un \( r_i'\in\mathopen] r_i,r\mathclose[\), nous gardons la propriété de recouvrir \( S\) tout en évitant la tangence avec \( D_j\) et en n'intersectant toujours pas \( G\). Parmi l'infinité de choix, seul un nombre fini provoque une tangence avec un autre \( D_k\). Nous les évitons.

		De cette manière nous avons \( \{ B(x_i,r'_i) \}_{i=1,\ldots,n}\) qui recouvre \( S\), n'intersecte pas \( G\) et tels qu'aucune boule n'est tangente à aucune autre. Et nous notons \( K=\bigcup_{i=1}^nD_i\).
