...
\begin{proposition}[\cite{BIBooPBIWooJtbKgk}]	\label{PROPooTUPHooLnSCkd}
...
\begin{proof}
	...
	\begin{subproof}
		...
		Par construction \( \ell\cap G=\emptyset\). Si \( \ell\) n'intersecte pas \( S\), alors on est bon. Si il y a des intersections, alors il faut un peu plus travailler.
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		est continue sur le compact \( S\). Donc elle a un minimum strictement positif (parce que \( S\) et \( G\) ne s'intersectent pas). Nous considérons \( r>0\) strictement plus petit que ce minimum. Étant donné que \( S\) est compact, la proposition \ref{PROPooLRJAooSTapDM} s'applique et nous considérons des boules \( \{ B(a_i,r_i) \}_{i=1,\ldots,n}\) recouvrant \( S\) sans être tangentes et telles que leur union est connexe.

		\spitem[Le chemin]
		%-----------------------------------------------------------
		Nous nommons \( a\) point d'intersection entre \( \ell\) et \( K\) qui est le plus haut (proche du bord haut du rectangle) et \( b\) celui qui est le plus bas. Le chemin que nous cherchons suit \( l\) entre le bord haut et \( a\), puis suit le bord de \( K\) jusqu'à \( b\). Cela est possible parce que le bord est un chemin continu fermé (proposition \ref{PROPooXYNJooQGYgfw}) n'intersectant le bord droit du rectangle qu'un un seul point. Il y a forcément un des deux sens dans lequel on peut parcourir \( \partial K\) de \( a\) à \( b\) sans intersecter le rectangle.

		Enfin le chemin se termine en suivant \( \ell\) jusqu'au bord inférieur du rectangle.
	\end{subproof}
\end{proof}
