...
\begin{propositionDef}[\cite{BIBooMistral}]	\label{PROPooWLQEooYpufqW}
	...
	\begin{enumerate}
		\item
		      \( u\cdot v>0\)
		\item
		      \( p+tu\in D\) pour tout \( t\) assez petit.
	\end{enumerate}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooCYYSooHVnjDl}
	...
	Alors il existe \( u\in \eR^2\) qui ne pointe vers l'intérieur\footnote{Voir la définition \ref{PROPooWLQEooYpufqW}.} d'aucun des cercles.
\end{proposition}
...
\begin{proof}
	...
	Nous avons montré que \( p\) était intérieur à \( K\), ce qui est contraire aux hypothèses.
\end{proof}
...
\begin{proof}
	Nous supposons que \( p=0\). Sinon, il faut juste tout translater. Il y a un nombre fini de disques qui ne touchent pas \( p\); leur adhérence est donc compacte et la distance à \( p\) y est donc bornée par le bas par un nombre strictement positif. Bref, nous commençons par prendre un rayon \( r_a>0\) tel que \( B(p,r_a)\) n'intersecte que des disques qui passent par \( p\). À partir de maintenant à chaque fois que nous parlerons d'une boule autour de \( p\), nous sous-entendrons que le rayon est au moins aussi petit que \( r_a\).
\end{proof}<++>
