...
Si vous cherchiez l'inégalité de Hölder dans \( L^p\), c'est la proposition \ref{ProptYqspT}.
...
\begin{corollary}
    ...
    \end{equation}
    pour tout \( x,t\geq 0\), avec une égalité si et seulement si \( x^p=y^q\).
\end{corollary}
...
\begin{theorem}[Inégalité de Hölder\cite{BIBooVEXYooXCkRQV}]        \label{THOooYHMJooBlXfpl}
    ...
    \begin{equation}
        | x\cdot y |=\sum_{i=1}^n| x_iy_i |\leq \| x \|_p\| x_ \|_q.
    \end{equation}
    Il y a égalité si et seulement si \( | x_i |^p=| y_i |^q\) pour tout \( i\).
\end{theorem}
<++>
