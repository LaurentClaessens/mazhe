...
\begin{proof}
    ...
    % position 382218354
    Soit \( h\) tel que \( a+h\in B(a,r)\). Nous allons montrer que \( f(a)\leq f(a+h)\); cela montrera que \( x=a\) est un minimum local. Pour cela nous utilisons un développement de Taylor : il existe \( c\in \mathopen] a , a+h \mathclose[\) tel que
        \begin{equation}
...
\begin{proof}
    Nous devons prouver que pour toute paire de points \( A,B\) sur le graphe de \( f\), le segment \( \mathopen[ A , B \mathclose]\) est au-dessus du graphe de \( f\). Ledit graphe étant d'ailleurs constitué de droites joignant \( (0,0,0)\) et les points du grahe de \( g\) (situé en \( x=1\)).
    ...
    \begin{subproof}
        ...
        \item[Autre corde]
            ...
            Si \( A\) et \( B\) sont dans le plan \( x=0\) alors c'est d'accord parce que le grahe de \( f\) dans le plan est le même que celui de la fonction strictement convexe \( g\).
            ...
            Nous en concluons que si \( A,B\in\mC\) alors \( \mathopen] A , B \mathclose[\) est soit complètement au-dessus de \( \mC\) soit complètement en-dessous de \( \mC\).
        
    \end{subproof}
    
    Nous prouvons à présent que toutes les cordre sont au-dessus du graphe de \( f\). Soient quatre points \( A,B,C,D\) sur \( \mC\) tels que la corde \( \mathopen] A , B \mathclose[\) soit au-dessus et la corde \( \mathopen] C , D \mathclose[\) soit en-dessous. Nous considérons le chemin \( \gamma\) parcourant le segment \( \mathopen[ A , B \mathclose]\) tel que \( \gamma(0)=A\) et \( \gamma(1)=B\), ainsi que le chemin \( \sigma\) parcourant \( \mathopen[ C , D \mathclose]\) de la même manière : \( \sigma(0)=C\) et \( \sigma(1)=D\). L'application
    \begin{equation}
        \begin{aligned}
            c\colon \mathopen[ 0 , 1 \mathclose]^2&\to \eR^3 \\
            (t,u)&\mapsto u\gamma(t)+(1-u)\sigma(t) 
        \end{aligned}
    \end{equation}
    est continue. Si \( a\in \mathopen] 0 , 1 \mathclose[\) alors \( c(0,a)\) est au-dessus de \( \mC\) et \( c(1,a)\) est en-dessous de \( \mC\). Par continuié, il existe \( t_0\in \mathopen] 0 , 1 \mathclose[\) tel que \( c(t_0,a)\in \mC\).

    Mais \( c(t_0,a)\) est sur le segment joignant \( \gamma(t_0)\) à \( \sigma(t_0)\) donc il ne devrait pas être sur \( \mC\).
\begin{probleme}
    Si \( \gamma(t_0)\) et \( \sigma(t_0)\) sont aligné à \( O \) ?
\end{probleme}
\end{proof}
<++>
