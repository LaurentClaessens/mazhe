...
Nous devons prouver que l'inéquation \eqref{EQooFXXZooBZhJYY} tient également lorsque \( \theta x+(1-\theta)y=0\) pour une certaine valeur \( \theta=\theta_0\in \mathopen] 0 , 1 \mathclose[\).
    ...
    Soient \( a,b\in \eR^n\) tels qu'en posant
    \begin{equation}
        \gamma(\theta)=\theta a+(1-\theta)b
    \end{equation}
    nous ayons \( \gamma(\theta_0)=0\). Nous posons
    \begin{equation}
        l_1(\theta)=\theta\| a \|^p
    \end{equation}
    et
    \begin{equation}
        l_2(\theta)=\theta\| a \|^p+(1-\theta)\| b \|^p.
    \end{equation}

    D'abord, vu que \( \theta\in \mathopen] 0 , 1 \mathclose[\) et que \( p>0\) nous avons \( \theta^p<\theta\) et donc
        \begin{equation}
            f_p\big( \gamma(\theta) \big)=\| \gamma(\theta) \|^p=\theta
        \end{equation}
        <++>

    Vu que \( \| a \|^p>0\) et \( \| b \|^p>0\) nous avons
    \begin{equation}
        l_2(\theta_0)=\theta_0\| a \|^p+(1-\theta_0)\| b \|^p>0.
    \end{equation}
    Nous avons aussi
    \begin{equation}
        l_2(\theta)>l_1(\theta)
