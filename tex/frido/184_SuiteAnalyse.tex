...
% Copyright (c) 2011-2015,2018-2026
%   Laurent Claessens
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooUXWKooKEjyrb}
	Soient deux ouverts \( A_i\) (\( i=1,2\)), deux intervalles ouverts \( I_i\) contenant \( 0\) ainsi que deux applications continues \(\gamma_i \colon I_i\to A_i  \) telles que \( \gamma_1(0)=\gamma_2(0)\). Nous supposons que pour tout \( t\in I_1\cap I_2\) tel que au moins un des deux \( \gamma_i(t)\) est dans \( A_1\cap A_2\), nous ayons \( \gamma_1(t)=\gamma_2(t)\).

	Alors \( \gamma_1=\gamma_2\) sur \( I_1\cap I_2\).
\end{lemma}
...
\begin{proof}
	...
	Étant donné que \( \gamma_1\) et \( \gamma_2\) sont continues, \( S\) est ouvert et nous avons \( t_0\not\in S\). Autrement dit, \( \gamma_1(t_0)=\gamma_2(t_0)\).

	Par ailleurs, le point \( a=\gamma_1(t_0)=\gamma_2(t_0)\) est dans \( A_1\cap A_2\) qui est un ouvert. Donc il existe \( \epsilon>0\) tel que \( \gamma_i(t)\in A_1\cap A_2\) pour tout \( t\in B(t_0,\epsilon)\). En particulier, \( t_0+\epsilon/2\) n'est pas dans \( S\), ce qui contredit la définition de l'infimum. Nous en déduisons que \( S\) est vide et que \( \gamma_1(t)=\gamma_2(t)\) pour tout \( t\in I_1\cap I_2\).
\end{proof}

\begin{proposition}[\cite{BIBooNRCAooLccdiL}]	\label{PROPooLTSZooBXBqtE}
	Soit une application \(F \colon V\to \eR^n  \) de classe \( \mA\) (\( C^k\), \( C^{\infty}\) ou analytique) où \( V\) est un ouvert de \( \eR^n\). Soit \( x_0\in V\). Il existe un ouvert \( W\) autour de \( x_0\) dans \( V\), un \( \epsilon>0\) et une application
	\begin{equation}
		\phi\colon \mathopen] -\epsilon,\epsilon\mathclose[\times W \to V
	\end{equation}
	de classe \( \mA\) telle que pour tout \( x\in W\) nous avons
	\begin{subequations}
		\begin{numcases}{}
			\frac{d}{dt} \left[ \phi(t,x)  \right]_{t=s}=F\big( \phi(t,x) \big)\\
			\phi(0,x)=x.
		\end{numcases}
	\end{subequations}

	De plus sur \( (U_2,\epsilon_2,\phi_2)\) ont la même propriété, alors \( \phi=\phi_2\) sur \( \mathopen] -\tau,\tau\mathclose[\times (U\times U_2)\) où \( \tau=\min(\epsilon,\epsilon_2)\).
	%TODOooKFWDooRMkGYh. Prouver ça.
\end{proposition}
...
\begin{proposition}[\cite{BIBooCQKFooJiwBtL}]     \label{PROPooCXHPooSlRsEJ}
	...
	Dans ce cas,
	\begin{enumerate}
		\item		\label{ITEMooFJNAooCKGQVZ}
		      Si \( k\) est pair, alors \( a\) est un point d'extrémum local de \( f\). Si \( f^{(k)}(a)>0\) alors c'est un minimum local et si \( f^{(k)}(a)<0\), c'est un maximum local.
		\item		\label{ITEMooKNIOooOUXDJM}
...
\begin{proof}
	En deux parties.
	\begin{subproof}
		\spitem[Pour \ref{ITEMooFJNAooCKGQVZ}]
		%-----------------------------------------------------------
		Supposons que \( f^{(k)}(a)<0\). Alors, parce que \( f^{(k)}\) est continue, il existe un \( \delta>0\) tel que \( f^{(k)}(x)<0\) pour tout \( x\in B(a,\delta)\). La Taylor avec reste de Lagrange (proposition \ref{PropResteTaylorc}), il existe \( x\in \mathopen] a,x,\mathclose[\) tel que
		\begin{equation}
			f(x)=\sum_{j=0}^{k-1}\frac{ f^{(j)}(a) }{ j! }(x-a)^j+\frac{ f^{(k)}(x) }{ !k }(x-a)^k.
		\end{equation}
		Après avoir effacé tous les termes nuls (\( f^{(j)}(a)=0\) pour \( j=1,\ldots,k-1\)) il nous reste
		\begin{equation}
			f(x)=f(a)+\frac{ f^{(k)}(c) }{ k! }(x-a)^k.
		\end{equation}
		Étant donné que \( k\) est pair, nous avons \( (x-a)^k>0\) et vu que \( f^{(k)}(x)<0\) nous avons \( f(x)<f(a)\) et donc \( a\) est un minimum local (sur \( B(a,\delta)\)).
		\spitem[Pour \ref{ITEMooKNIOooOUXDJM}]
		%-----------------------------------------------------------
		Tout la même chose, mais cette fois le signe de \( (x-a)^k\) change suivant que \( x>a\) ou \( x<a\). Il y aura donc des \( x\in B(a,\delta)\) avec \( f(x)>f(a) \) et d'autres avec \( f(x)<f(a)\).
	\end{subproof}
\end{proof}
