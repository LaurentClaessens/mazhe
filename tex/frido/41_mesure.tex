...
\begin{proposition}
    Soient un espace mesuré \( (\Omega,\tribA,\mu)\), un ensemble \( \Omega'\) et une bijection continue d'inverse continue \( \varphi\colon \Omega\to \Omega'\). Nous supposons que \( \Omega\) et \( \Omega'\) sont des espaces topologiques\footnote{C'était déjà dans l'hypothèse comme quoi $\varphi$ est continue.}.
    
    En posant 
    \begin{itemize}
        \item \( \tribA'=\varphi(\tribA)\),
        \item \( \mu'(A)=\mu\big( \varphi^{-1}(A) \big)\),
    \end{itemize}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\( \mu'\) est une mesure positive]
            ...
            \end{equation}
    \end{subproof}
    Passons à cette histoire de boréliens. Si \( A\in \Borelien(\Omega')\), alors \( A=\varphi\big( \varphi(A) \big)\in\varphi\big( \Borelien(\Omega) \big)\) parce que \( \varphi\) est continue et donc borélienne (proposition \ref{ThoJDOKooKaaiJh}). Le même raisonnement fonctionne dans l'autre sens parce que nous avons supposé que \( \varphi\) est continue et d'inverse continu.
\end{proof}
...
\begin{proposition}\label{PropFYPEOIJ}
    Si \( f_n\colon X\to \eR\) est une suite de fonctions mesurables et positives, alors la fonction \( \sum_nf_n\) est mesurable.
\end{proposition}
