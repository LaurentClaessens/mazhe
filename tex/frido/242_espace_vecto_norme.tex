...
\begin{proof}
	Soit \( \epsilon>0\). Il existe \( r>0\) tel que si \( x\in B_{X_1}(0,r)\) et \( y\in B_{X_2}(0,r)\) alors \( \| f(x) \|/\| x \|<\epsilon\) et \( \| g(y) \|/\| y \| <\epsilon\).
\end{proof}<++>
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooPESTooQmWGRJ}
	Soient des espaces vectoriels normés \( X_1, X_2,Y_1,Y_2\) ainsi que des applications \(f \colon X_1\to Y_1  \) et \(g \colon X_2\to Y_2  \) de classe \( \mA\) (\( C^k\) ou analytique). Alors l'application
	\begin{equation}
		f\times g \colon X_1\times X_2\to Y_1\times Y_2
	\end{equation}
	est dans la class \( \mA\).
\end{proposition}
