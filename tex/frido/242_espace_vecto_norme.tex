...
\begin{proposition}[Composition]		\label{PROPooLRRMooRzrTNE}
	Soient trois espaces vectoriels normés \( E,V,W\). Si les applications \(f \colon E\to V  \) et \( g \colon V\to W  \) sont de classe \( C^p\), alors la composée \(g\circ f \colon E\to W  \) est de classe \( C^p\).
\end{proposition}
...
\begin{probleme}
	Je n'ai pas vérifié si la proposition suivante est correcte. Écrivez-moi pour me dire si c'est bon. De préférence avec une démonstration ou un lien vers une démonstration.
\end{probleme}
