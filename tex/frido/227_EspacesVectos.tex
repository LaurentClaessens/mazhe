...
\begin{lemmaDef}      \label{DEFooTMSEooZFtsqa}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	et sa prolongation par linéarité. Ces élements du dual \( E^*\) forment une base appelée \defe{base duale}{base!duale}.
\end{lemmaDef}
...
\begin{proof}
	Vu que \( \{ \alpha_i \}\) est une base, nous pouvons décomposer \( \beta_i\) de la façon suivante : \( \beta_i=\sum_jB_{ij}\alpha_j\). Il nous reste à déterminer que \( B=A^{-1}\).
\end{proof}<++>
