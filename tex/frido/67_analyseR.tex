...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Différentielle de fonctions à plusieurs variables}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++


\begin{proposition}[\cite{MonCerveau}]	\label{PROPooPRDIooNYOesw}
	Soient des espaces vectoriels normés de dimensions finies \( V\) et \( W\). Soit une fonction différentiable \(f \colon V\to W  \). Nous avons
	\begin{equation}
		df_a(u)=\sum_{i=1}^nu_k(\partial_kf)(a).
	\end{equation}
\end{proposition}

\begin{proof}
	Nous devons calculer
	\begin{equation}		\label{EQooMBURooBWNgOQ}
		\lim_{h\to 0} \frac{ f(a+h)-f(a)-\sum_{i=1}^nh_k(\partial_kf)(a) }{ \| h \| }.
	\end{equation}
	Pour cela nous considérons \( \epsilon>0\) et \( h\in V\) tel que \( \| h \|=\epsilon\). Nous posons \( h=\epsilon u\) avec \( \| u \|=1\) et nous utilisons la formule des accroissements finis (proposition \ref{PROPooYYSMooUDxtlB}). Le numérateur de \eqref{EQooMBURooBWNgOQ} est
	\begin{equation}
		\sum_{k=1}^n\epsilon u_k(\partial_kf)\big( a+\sum_{i=k+1}^n\epsilon u_ie_i \big)+\epsilon\alpha(\epsilon)-\sum_{i=1}^n\epsilon u_k(\partial_kf)(a).
	\end{equation}
	Le dénominateur est juste \( \| h \|=\epsilon \). Nous pouvons simplifier par \( \epsilon\) :
	\begin{equation}
		\sum_{k=1}^nu_k(\partial_kf)\big( a+\sum_{i=k+1}^n\epsilon u_i e_i \big)+\alpha(\epsilon)-\sum_{i=1}^nu_k(\partial_kf)(a).
	\end{equation}
	Et quand nous faisons la limite \( \epsilon\to 0\), tout part et il reste zéro.
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooCAZBooJZKXod}
	Soient des espaces vectoriels \( S,E_1,\ldots,E_n\) de dimensions finies. Soient des applications différentiables \(a_i \colon S\to  E_i \). Alors l'application
	\begin{equation}
		\begin{aligned}
			\varphi\colon S & \to E_1\times \ldots\times E_n           \\
			x               & \mapsto \big( a_1(x),\ldots,a_n(x) \big)
		\end{aligned}
	\end{equation}
	est différentiable et
	\begin{equation}
		(d\varphi_a)(h)=\Big( (da_1)_a(h),\ldots,(da_n)_a(h) \Big).
	\end{equation}
\end{proposition}
...
\begin{proof}
	Nous devons calculer
	\begin{equation}
		\lim_{h\to 0}\frac{ \varphi(a+h)-\varphi(a)-\big( (d\varphi_1)_a(h),\ldots,(d\varphi_n)_a(h) \big) }{ \| h \| }.
	\end{equation}
	Nous considérons \( \epsilon>0\) et posons \( h=\epsilon u\) avec \( \| u \|=1\). Nous utilisons les accroissements finis du lemme \ref{LEMooNMTAooLgMkgH}. Après simplification par \( \epsilon\) comme dans la preuve de \ref{PROPooPRDIooNYOesw}, il reste
	\begin{equation}
		\sum_{k=1}^nu_k(\partial_k\varphi)\big( a+\sum_{i=k+1}^b\epsilon u_ie_i \big)+\alpha(\epsilon)-\big( (da_1)_a(u),\ldots,(da_n)_a(u) \big).
	\end{equation}
	Lorsqu'on fait \( \epsilon\to 0\), il reste
	\begin{equation}		\label{EQooLBBYooKqIdBh}
		\sum_{k=1}^nu_k(\partial_k\varphi)(a)-\big( (da_1)_a(u),\ldots,(da_n)_a(u) \big).
	\end{equation}
	La dérivée d'une fonction à valeurs dans un espace vectoriel est la dérivée composante par composante :
	\begin{equation}
		(\partial_k\varphi)(a)=\big( (\partial_k\varphi_1)(a),\ldots,(\partial_k\varphi_n)(a) \big).
	\end{equation}
	Mais \ref{PROPooPRDIooNYOesw} nous dit que pour chaque \( i\) nous avons
	\begin{equation}
		\sum_{k=1}^nu_k(\partial_k\varphi_i)(a)=(d\varphi_i)_a(u).
	\end{equation}
	Il reste à substituer ça dans \eqref{EQooLBBYooKqIdBh} en nous souvenant que \( \varphi_i=a_i\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooCTXUooTGJbIK}
	Soient des espaces vectoriels normés de dimensions finies \( E_1,\ldots,E_n, S, F\) ainsi que des fonctions différentiables \(a_i \colon S\to E_i  \) et \(\phi \colon E_1\times \ldots\times E_n\to F  \). Alors l'application
	\begin{equation}
		\begin{aligned}
			f\colon S & \to F                                        \\
			x         & \mapsto \phi\big( a_1(x),\ldots,a_n(x) \big)
		\end{aligned}
	\end{equation}
	est différentiable et
	\begin{equation}
		df_a(u)=\sum_{i=1}^n(d_i\phi)_{\big( a_1(a),\ldots,a_n(a) \big)}\big( (da_i)_a(u) \big).
	\end{equation}
\end{proposition}

\begin{proof}
	Il s'agit de remarquer que \( f=\phi\circ \varphi\) avec \( \varphi\) comme dans la proposition \ref{PROPooCAZBooJZKXod}, et d'utiliser la différentielle de la composée, proposition \ref{PropDiffCompose}.
\end{proof}


\begin{proposition}[\cite{MonCerveau}]	\label{PROPooBEYNooJlrFTC}
	Soient des espaces de Banach \( V,W,E\). Soit une fonction différentiable \(f \colon V\times W\to E  \). Pour chaque \( v\in V\), l'application partielle
	\begin{equation}
		\begin{aligned}
			f_v\colon W & \to E             \\
			w           & \mapsto    f(v,w)
...
\begin{proof}
	...
	\begin{subequations}		\label{EQooDUQNooHrWalt}
		\begin{align}
			\lim_{\substack{ h\to 0        \\ h\in W }  }\frac{ f_v(a+h)-f_v(a)-df_{(v,a)}(u,0) }{ \| h \| }&= \lim_{\substack{ h\to 0 \\ h\in W }  }\frac{ f(v,a+h)-f(v,a)-df_{(v,a)}(u,0) }{ \| h \| }\\
			 & \leq\lim_{\substack{ h\to 0 \\ h\in W }  }\frac{ f(v,a+h)-f(v,a)-df_{(v,a)}(u,0) }{ \| (l,h) \| }.
		\end{align}
	\end{subequations}
	La dernière inégalité est vraie pour tout \( l\in V\).

	Soit \( \epsilon>0\). Étant donné que \( f\) est différentiable, il existe \( \delta>0\) tel que si \( (l,h)\in B\big( (0,0),\delta \big)\) alors
	\begin{equation}
		\frac{	f(v,a+h)-f(v,a)-df_{(v,a)}(u,0) }{ \| (l,h) \| }<\epsilon.
	\end{equation}
	Si \( \| h \|<\epsilon\) nous avons
	\begin{equation}
		\frac{	f(v,a+h)-f(v,a)-df_{(v,a)}(u,0) }{ \| h \|} < \frac{	f(v,a+h)-f(v,a)-df_{(v,a)}(u,0) }{ \| (l,h) \|} <\epsilon.
	\end{equation}
	Donc la limite \eqref{EQooDUQNooHrWalt} est bien zéro.
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooUXWIooTVVURR}
	Soit une fonction \(f \colon \eR\to \eR  \) telle que \( \lim_{x\to 0}f(x)=0\). Alors
	\begin{equation}
...
\begin{proof}
	Soit \( \epsilon>0\). Il existe \( \delta>0\) tel que si \(| x |\leq \delta \) alors \( | f(x) |<\epsilon\). Étant donné que \( | x |\leq \| (x,y) \|\), si \( (x,y)\in B\big( (0,0),\delta \big)\) nous avons \( | x |\leq \delta\) et donc \( | f(x) |<\epsilon\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooIOMBooXoNBTM}
	Soient des applications \(f \colon E\to \eR  \) et \(g \colon F\to \eR  \). Nous supposons que \( f\) est différentiable en \( a\in E\) et que \( g\) est différentiable en \( b\in F\). Alors l'application
	\begin{equation}
		\begin{aligned}
			P\colon E\times F & \to \eR          \\
			(x,y)             & \mapsto f(x)g(y)
		\end{aligned}
	\end{equation}
	est différentiable en \( (a,b)\) et
	\begin{equation}
		dP_{(a,b)}(u,v)=df_a(u)g(b)+f(a)dg_b(v).
	\end{equation}
\end{proposition}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			\tilde f\colon E\times F & \to \eR      \\
			(x,y)                    & \mapsto f(x)
		\end{aligned}
	\end{equation}
	et similairement pour \( \tilde g\). Nous avons alors \( P(x,y)=\tilde f(x,y)\tilde g(x,y)\). Il nous suffit alors de prouver que \( \tilde f\) et \( \tilde g\) sont différentiables. Nous prouvons que \( d\tilde f_{(a,b)}(u,v)=df_a(u)\). Cela est un calcul :
	\begin{subequations}
		\begin{align}
			\lim_{h\to 0}\frac{ \tilde f\big( (a,b)+(h_1,h_2) \big) -\tilde f(a,b)-df_a(h_1) }{ \| h \| } & =\lim_{h\to 0}\frac{ f(a+h_1)-f(a)-df_a(h_1) }{ \| h \| }                   \\
			                                                                                              & \leq \lim_{(h_1,h_2)\to (0,0)}\frac{ f(a+h_1)-f(a)-df_a(h_1) }{ \| h_1 \| }
		\end{align}
	\end{subequations}
	parce que \( \| h_1 \|\leq \| (h_1,h_2) \|\). Nous déduisons que le tout est nul par la proposition \ref{PROPooONYBooQuqAPR} et le fait que
	\begin{equation}
		\lim_{h_1\to 0} \frac{ f(a+h_1)-f(a)-df_a(h_1) }{ \| h_1 \| }=0
	\end{equation}
	par définition de la différentiabilité de \( f\).
\end{proof}
...
\begin{lemma}       \label{LEMooASGNooCWqAmN}
	Les projections canoniques sont des applications différentiables.
\end{lemma}

\begin{proof}
	Parce que ce sont des application linéaires, voir lemme \ref{LEMooZSNMooCfjzOB}.
\end{proof}

\begin{lemma}       \label{LEMooGYJUooOudNLH}
	Toute fonction polynômiale\footnote{Polynôme à \( n\) variables, définition \ref{DEFooFDBAooHvfMue}.} à \( n\) variables est différentiable comme application de \( \eR^n\) vers \( \eR\).
\end{lemma}

\begin{proof}
	Chacune des fonctions \( x\mapsto x^k\) est différentiable. La proposition \ref{PROPooIOMBooXoNBTM} dit alors que les applications \( (x_1,\ldots,x_n)\mapsto x_1^{k_1}\ldots x_n^{k_n}\) sont différentiables.
\end{proof}
...
\begin{proof}
	Le numérateur et le dénominateur sont différentiables par le lemme \ref{LEMooGYJUooOudNLH}. Ensuite \( x\mapsto\frac{1}{ Q(x)}\) est différentiable en utilisant la composition (proposition \ref{ThoAGXGuEt}) et l'inverse (proposition \ref{PROPooQWIUooFaNzTx}). Et enfin le produit (proposition \ref{PROPooZKYBooLLNKHu}) fait le travail.
\end{proof}
...
\begin{example}     \label{EXooELTHooDdJyJE}
	...
	\begin{enumerate}
		\item
		      ...
		      Il est vite vu que la formule \eqref{EqDerrPaert0041x} n'a pas de limite pour $(x,y)\to(0,0)$. Ceci \emph{ne prouve pas} que la différentielle de $f$ n'existe pas en $(0,0)$. L'existence de la différentielle implique la continuité de la fonction, et non de la différentielle elle-même. En effet, une différentielle peut exister en un point sans qu'elle soit la limite de la différentielle aux autres points. Nous verrons par exemple, dans l'exercice \ref{exo0035}\ref{Item0035d}, un exemple de fonction dérivable\footnote{Pour rappel, en dimension un, la dérivée est \emph{exactement} la notion de différentielle.} en $0$, mais dont la dérivée n'est pas continue en zéro.
