...
\begin{proof}
    ...
    \item[\ref{ITEMooAHOHooDZgtSB}]
        ...
        \item[\ref{ITEMooTXXBooBmDtzd}]
            En ce qui concerne le fait d'être antisymétrique, permuter \( v_k\) et \( v_l\) revient à calculer le nombre \( \det_B( v_{\sigma_{kl}(1)},\ldots, v_{\sigma_{kl}(n)} )\) au lieu de \( \det_B(v_1,\ldots, v_n)\). Cela revient à changer la somme \( \sum_{\sigma}\) en \( \sum_{\sigma\circ\sigma_{kl}}\). Cela ajoute \( 1\) à \( \epsilon(\sigma)\) vu que l'on ajoute une permutation.

            Nous prouvons à présent que \( \det\) est alternée.

            Notons que le lemme \ref{LemHiHNey} dit qu'une forme alternée est antisymétrique. La démonstration de l'antisymétrie est donc superflue ici. Attention cependant qu'en caractéristique \( 2\), l'alternée n'implique pas l'antisymétrie.
