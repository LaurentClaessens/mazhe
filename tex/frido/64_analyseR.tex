...
\begin{definition}[Racince]
    Pour \( n\in \eN\) nous définissons \( \sqrt[n]{  }=f_n^{-1}\). Lorsque \( n\) est pair, la fonction \( \sqrt[n]{  }\) n'est définie que sur \( \eR^+\), et lorsque \( n\) est impair, elle est définie sur tout \( \eR\).
\end{definition}
...
\begin{lemma}
    Soient \( \alpha>0\), une suite de rationnels strictement croissante \( \alpha_i\to \alpha\) ainsi que les fonctions
    \begin{equation}
        \begin{aligned}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[ \( x,y\in \eQ\)]
            Soient \( y=m/n\) avec \( m\in \eZ\), \( n\in \eN\) et \( q\in \eQ\). Nous avons, en utilisant la partie déjà démontrée et le lemme \ref{LEMooIDLJooZALNaD},
            \begin{equation}
                (a^q)^{m/n}=\big( (a^q)^m \big)^{1/m}=(a^{mq})^{1/n}
            \end{equation}
            Il ne nous reste donc qu'à traitet les cas de la forme \( (a^q)^{1/n}\). Par définition le nombre \( (a^q)^{1/n}\) est le réel positif \( X\) tel que \( X^n=a^q\). Il est facile de vérifier que \( X=a^{q/n}\) vérifie cette propriété :
            \begin{equation}
                (a^{q/n})^n=a^{nq/n}=a^q
            \end{equation}
            par la partie que nous avons déjà prouvée.
