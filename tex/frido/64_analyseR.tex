...
\begin{proof}
    Nous notons, comme à l'accoutumée, \( f_a(x)=a^x\). Soit une suite \( (x_n)\) telle que
    \begin{itemize}
        \item \( x_k\to 0\)
        \item \( x_k\neq 0\)
        \item \( (x_k)\) est décroissante.
    \end{itemize}
    \begin{subproof}
        \item[Convergence]
            Par la proposition \ref{PROPooGCBZooTcyGtO}, la suite \( k\mapsto f_a(x_k)\) est décroissante. Et vu que \( f_a(0)=1\), la suite \( f_a(x_k)\) est décroissante et bornée vers le bas par \( 1\). Elle est donc convergente. 

        \item[Toutes vers la même limite]

            Il existe \( \delta\geq 0\) tel que la limite de \( x_k\) soit \( 1+\delta\). Nous prouvons que la valeur de \( \delta\) ne dépend pas du choix de la suite \( x_k\). Soit donc \( (y_k)\) vérifiant les mêmes conditions, et supposons que \( f_a(y_k)\) converge vers \( 1+\delta'\) avec \( \delta'<\delta\).

            Soit \( k\) tel que \( f_a(y_k)<\delta\). Vu que \( x_k\to 0\), il existe \( l\) tel que \( x_k<y_k\). Nous avons alors
            \begin{equation}
                1<f_a(x_k)<f_a(y_k)<1+\delta.
            \end{equation}
            D'où l'obligation d'avoir \( \delta=\delta'\).

        \item[La limite]

            Il nous est maintenant loisible de chercher la limite pour une suite de notre choix. Nous prenons \( x_n=1/n\). Le nomre \( a^{1/n}\) est la solution (positive) de \( x^n=a\). Mais si \( s>1\), alors il existe \( n\) tel que \( s^n>a\) (proposition \ref{PROPooVXKBooQPPjMn}), donc pour tout \( s>1\), il existe \( n\) tel que la solution de \( x^n=a\) vérifie \( x<1\). Cela signifie que \( \lim_{n\to \infty} a^{1/n}<s\) pour tout \( s>1\). Autrement dit, cette limite est \( 1\).
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{numcases}
    \end{subequations}
    Nous en déduisons \( x^{mn}=a^n\) et \( y^{mn}=a^m\). En multipliant l'une par l'autre nous trouvons \( (xy)^{mn}=a^{m+n}\), ce qui signifie que \( xy\) est solution de \( t^{mn}=a^{n+m}\), c'est à dire
    \begin{equation}
        xy=a^{(m+n)/mn}.
    \end{equation}
\end{proof}
