...
\begin{probleme}
    Attention : le lemme \ref{LEMooUAFBooAwiXxj} est faux. Il faut la notion de fonction Cauchy-continue. Voir l'exemple \ref{EXooWZNCooQkKdtJ}.
\end{probleme}
...
\begin{proof}
    ...
    \item[Unicité]
        ...
        Soit \( \epsilon>0\). Par la continuité\quext{L'argument qui suit est probablement faux parce qu'il fait intervenir une uniforme continuité. On en discute ici : \url{https://github.com/LaurentClaessens/mazhe/issues/124}.} de \( f\), il existe \( \delta\) tel que si \( | p-q |<\delta\) (pour \( p,q\in \eQ\)), alors \( | f(p)-f(q) |<\epsilon\). Par ailleurs, \( (q_i)\) est de Cauchy dans \( \eR\) (théorème~\ref{THOooNULFooYUqQYo} qu'il est toujours bon de citer de temps en temps). Il existe donc \( N\) tel que \( i,j>N\) implique \( | q_i-q_j |<\delta\).
