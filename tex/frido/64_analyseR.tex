...
\begin{proof}
    ...
    \item[\( \lim_{x\to a} \tilde f(x)=\lim_{q\to a} f(q)\)]
        ...
        Nous avons déjà traité la limite de droite, et nous savons qu'elle vaut \( \lim f(q_i)\) pour n'importe quelle suite \( q_i \to x\) (\( q_i\in \eQ\)).

        Nous montrons que la limite de gauche existe en utilisant la caractérisation séquentielle de la limite (proposition~\ref{PROPooJYOOooZWocoq}). Pour ce faire, nous montrons que pour toute suite \( x_k\to a\) nous avons \( \lim \tilde f(x_k)=\lim_{q\to a} f(q)\).
...
\begin{definition}[Exposant rationnels, racines]
    ...
    Pour \( p,q\in \eN\) nous définissons enfin
    \begin{equation}
        a^{p/q}=(a^p)^{1/q}.
    \end{equation}
...
\begin{lemma}[\cite{MonCerveau}]
    Soient \( m,n\in \eN\) et \( a>0\) dans \( \eR\). Alors
    \begin{equation}
        a^{1/m}a^{1/n}=a^{\frac{1}{ n }+\frac{1}{ m }}.
    \end{equation}
\end{lemma}

\begin{proof}
    Nous posons \( x=a^{1/m}\) et \( y=a^{1/n}\), c'est à dire
    \begin{subequations}
        \begin{numcases}{}
            x^{m}=a\\
            y^{n}=a.
        \end{numcases}
    \end{subequations}
    Nous en déduisons \( x^{mn}=a^n\) et \( y^{mn}=a^m\)<++>
\end{proof}
<++>
