...
\begin{definition}[Fonction, domaine, image, graphe]
  Soient \( X\) et \( Y\) deux ensembles. Une \defe{fonction}{fonction} \( f\) définie sur \( X\) et à valeurs dans \( Y\) est une correspondance qui associe à chaque élément \( x\) dans \( X\) {\bf au plus} un élément \( y\) dans \( Y\). On écrit \( y= f(x)\).
  \begin{itemize}
...
Afin de pouvoir étudier la topologie des espaces métriques, il faut connaître quelques propriétés des réels, parce que nous allons étudier la fonction \og distance\fg qui est une fonction continue à valeurs dans les réels.
...
Pour la définition générale d'un compact, consulter \ref{DefJJVsEqs}.
...
\begin{proof}
    ...
    \end{equation}
    Puisque la suite est croissante pour tout \( m>\alpha(K)\) nous avons \( x_m+\epsilon>l\), ce qui signifie \( | x_m-\ell |<\epsilon\).
\end{proof}
