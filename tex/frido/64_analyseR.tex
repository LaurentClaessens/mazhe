...
\begin{subequations}
    ...
    \end{numcases}
\end{subequations}
pour autant que ces fonctions sont définies\quext{Attention : dans les pages à venir, ces notations sont bien violées. En particulier, souvent il est écrit \( f\) au lieu de \( g\).}.
...
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooVADRooLCLOzP}
    ...
    \begin{enumerate}
        \item       \label{ITEMooQHYRooJIewyp}
            Pour \( a>0\), la fonction \( x\mapsto a^x\) est continue sur \( \eR\).
        \item       \label{ITEMooIZBLooSGtWIp}
            Pour \( a>1\), la fonction \( x\mapsto a^x\) est croissante.
