...
La proposition suivante, qui traite de la composition de limites pointées, montre que la limite épointée ne passe pas gratuitement à la composition.
...
Nous avons donc \( 9\) cas résumés dans le tableau suivant.
\begin{equation}       
...
Le choix entre la limite pointée ou épointée a été discuté en de nombreuses occasions \cite{BIBooKNWHooBRoxme,BIBooNUKAooVMqppa,BIBooDILKooUcmUVD,BIBooJDPPooVONaQV,BIBooUIAFooHqKjQh}.
...
\begin{example}
    ...
    Cette fonction est l'exemple-type de différence entre limite pointée et limite pointée.
\end{example}
...
Le voici avec des limites pointées. Pour faire la différence, j'adopte la notation \( {LP}\) pour la limite pointée et \( {LE}\) pour la limite épointée. En ce qui concerne les conventions de notations \( {LP}_{x\to a}f(x)=\ell\) signifie que la limite pointée existe et vaut \( \ell\). Même chose pour la limite épointée.


Le lien entre limite pointée, épointée et continuité est donné par ceci\quext{Écrivez-moi si vous connaissez un énoncé du même type donnant une condition équivalente à la limite épointée en n'utilisant que la limite pointée et la continuité -- ou autres notions attenantes.}.

Cela est peut-être une raison de présenter la limite pointée chez les petits. Mais ce n'est pas une raison pour les grands, que du contraire. L'énoncé de la proposition \ref{PROPooCQZZooMiZfQE} est à peine plus compliquée que celui de \ref{PROPooCQZZooMiZfQE}, mais elle dit un peu plus alors que la démonstration est la même.

Donc non, la proposition \ref{THOooNPBQooEMOYpd} n'ajoute pas d'hypothèse par rapport à \ref{PROPooCQZZooMiZfQE}. Au contraire, elle en enlève : la proposition \ref{PROPooNWCMooCaDMex} demande pour \( g\) une limite épointée au lieu d'une limite pointée. En ce qui concerne \( f\), les hypothèses sont les mêmes. La proposition \ref{PROPooNWCMooCaDMex} concerne une classe de fonctions un peu plus grande.

Au final, la proposition «épointée» \ref{PROPooNWCMooCaDMex} est un poil plus compliquée, mais elle a une hypothèse un peu plus faible et une conclusion un peu plus faible (existence d'une limite pointée). Bref, il s'agit d'un résultat différent.

Inutile de faire tout un plat pour dire que votre version préférée est «plus simple». Forcément, le théorème avec la limité pointée est plus simple : il a plus d'hypothèses.

Nous ne sommes plus des enfants, et nous sommes parfois prêt à payer un énoncé un presque rien plus subtil pour avoir un résultat un tout petit peu plus complet. Nous recevons la difficulté avec résignation et la subissons avec courage pour le plaisir d'écrire un théorème qui sera un micro-cheveu plus général.
...
\begin{proposition}
    Soient \( f\colon \Omega_f\to \eR\) ainsi que \( g\colon \Omega_g\to \eR\) telles que
    \begin{enumerate}
        \item
