...
\begin{lemma}[\cite{MonCerveau}]        \label{LEMooUAFBooAwiXxj}
    ...
    \begin{enumerate}
        ...
        \item
            Il existe un unique prolongement continue \( \tilde f\colon \eR\to \eR\).
        \item
...
\begin{proof}
    ...
    \item[\( \lim_{x\to a} \tilde f(x)=\lim_{q\to a} f(q)\)]

        Soit \( a\in \eR\). Nous allons montrer que
        \begin{equation}
            \lim_{x\to a} \tilde f(x)=\lim_{q\to a} f(q).
        \end{equation}
        À gauche il s'agit d'une limite « usuelle » d'une fonction \( \eR\to \eR\); et à droite c'est la limite d'une fonction \( \eQ\to \eR\). 

        La limite de gauche, si elle existe, peut être calculée à partir de n'importe quelle suite de réels qui converge vers \( a\). Nous avons déjà traité la limite de droite, et nous savons qu'elle vaut \( \lim f(q_i)\) pour n'importe quelle suite \( q_i \to x\).

        Nous montrons que le nombre \( \lim_{q\to a} f(q)\) vérifie les conditions pour être la limite \( \lim_{x\to a} \tilde f(x)\). Soit \( \epsilon>0\) et une suite \( q_k\to a\) avec \( q_k\in \eQ\). Nous avons
        \begin{equation}
            | \tilde f(x)-\lim_{q\to a} f(q) |\leq | \tilde f(x)-\tilde f(q_k) |+| \tilde f(q_k)-\lim_{q\to a} f(q) |
        \end{equation}
        La limite \eqref{EQooSGCMooKtpVMy} nous permet de choisir \( k\) assez grand pour que \( | \tilde f(q_k)-\lim_{q\to a} f(q) |<\epsilon\).

        Si \( x\in \eQ\), nous avons \( \tilde f(q_k)=f(q_k)\) et la continuité de \( f\) permet de trouver \( k\) assez grand pour que le premier terme soit également petit. Si \( x\) n'est pas rationnel nous devons traiter avec
        \begin{equation}
            | \lim_{q\to x} f(q)-f(q_k) |.
        \end{equation}
        Le critère séquentiel de la limite \ref{}<++>
...
Nous pouvons également facilement poser
\begin{equation}
    x^{-n}=\frac{1}{ x^n }
\end{equation}
pour \( n\geq 0\). Cela donne donne donc \( x^n\) pour \( x\in \eR\) et \( n\in \eZ\) a l'exception de \( x=0\) lorsque \( n<0\).

Nous étudions quelque propriétés de cette fonction pour \( n>0\) fixé.

\begin{lemma}\label{LEMooLDCHooCneVKT}
    Soit \( n\in \eN\). La fonction $f_n\colon x\mapsto x^n$ est continue.
\end{lemma}

\begin{proof}
    Soit \( x\in \eR\). En vertu de \ref{ThoLimCont} nous allons prouver que \( \lim_{\epsilon\to 0}f_n(x+\epsilon)=f_n(x)\). Pour cela nous utilisons la formule du binôme \ref{PropBinomFExOiL} dont nous séparons le premier terme :
    \begin{equation}
        f_n(x+\epsilon)=(x+\epsilon)^n=x^n+\sum_{k=1}^n{n\choose k}x^{n-k}\epsilon^k.
    \end{equation}
    À la limite \( \epsilon\to 0\), tous les termes disparaîssent sauf le premier (qui ne dépend pas de \( \epsilon\)). Donc
    \begin{equation}
        \lim_{\epsilon\to 0}f_n(x+\epsilon)=x^n=f_n(x),
    \end{equation}
    et \( f_n\) est continue.
\end{proof}
...
Nous sommes encore loin d'avoir défini \( x^y\) pour tout \( x,y\in \eR\). La suite dans la définition \ref{DEFooYHVNooLXbibY}.
