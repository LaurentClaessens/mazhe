...
\begin{proof}
    ...
    \item[Intermède]
        ...
        \begin{equation}        \label{EQooNSYCooTmECjs}
            \lim f(q_i)
        \end{equation}
        lorsque \( (q_{i})\) est une suite quelconque de rationnels qui converge vers \( x\).
    ...
    \item[\( \lim_{x\to a} \tilde f(x)=\lim_{q\to a} f(q)\)]
        ...
        \end{equation}
        À gauche il s'agit d'une limite « usuelle » d'une fonction \( \eR\to \eR\); et à droite c'est la limite d'une fonction \( \eQ\to \eR\). Notons que l'existence d'aucune des deux limites n'est encore acquise.

        La limite de gauche, si elle existe, peut être calculée à partir de n'importe quelle suite de réels qui converge vers \( a\).
