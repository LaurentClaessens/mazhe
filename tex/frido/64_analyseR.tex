...
\begin{proof}
    ...
    \end{equation}
    Soit \( s\), un minorant de \( | g(x) |\) sur un voisinage de \( a\); vu que la limite en \( a\) est \( \ell'\neq 0\), nous pouvons prendre par exemple \( s=\ell'/2\) : \( | g(x) |>\ell'/2\) sur \( B(a,\delta)\) dès que \( \delta\) est assez petit. Nous considérons \( x\in B(a,\delta)\).
...
Quoi qu'il en soit, nous ne pourrons pas donner la formule générale de dérivation de \( x\mapsto a^x\) avant d'avoir vu les logarithmes (proposition \ref{PROPooKUULooKSEULJ}).
