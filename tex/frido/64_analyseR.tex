...
\begin{proposition}     \label{PROPooJJRZooACUmWN}
	La liste des intervalles\footnote{Définition \ref{DefEYAooMYYTz}.} de \( \eR\) est donnée dans la proposition \ref{PROPooHPMWooQJXCAS}.
	\begin{enumerate}
		\item		\label{ITEMooOEUKooRNNzaJ}
		      \( \mathopen]-\infty , a \mathclose[=\{ x\in \eR\tq x< a \}\)  est ouverte.
		\item \label{ITEMooGYSQooRhqHnX}
		      \( \mathopen] a , \infty \mathclose[=\{ x\in \eR\tq x>a \}\) est ouvert
		\item		\label{ITEMooCCPRooYPqFpD}
		      \( \mathopen]-\infty , a \mathclose]=\{ x\in \eR\tq x\leq a \}\) n'est ni ouvert ni fermé.
		\item		\label{ITEMooDMPGooQPbpmj}
		      \( \mathopen[ a , b \mathclose]=\{ x\in \eR\tq a\leq x\leq b \}\) est fermé.
		\item		\label{ITEMooCIZXooGNZaun}
		      \( \mathopen] a , b \mathclose]=\{ x\in \eR\tq a<x\leq b \}\) n'est ni ouvert ni fermé.
		\item	\label{ITEMooIKLSooFzrgXd}
		      \( \mathopen] a , b \mathclose[=\{ x\in \eR\tq a<x<b \}\) est ouvert.
		\item		\label{ITEMooCHNKooHxCbaQ}
		      \( \mathopen[ a , b \mathclose[=\{ x\in \eR\tq a\leq x<b \}\) n'est ni ouvert ni fermé.
		\item	\label{ITEMooZIFNooBwwoGC}
		      \( \mathopen[ a , \infty \mathclose[=\{ x\in \eR\tq x\geq a \}\) n'est ni ouvert ni fermé.
		\item		\label{ITEMooLSIPooZimrIK}
		      \( \mathopen] -\infty , \infty \mathclose[=\eR\) est ouvert et fermé.
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Pour \ref{ITEMooOEUKooRNNzaJ}]
		%-----------------------------------------------------------
		Si \( x<a\) alors en prenant \( \epsilon<| x-a |\) nous avons \( B(x,\epsilon)\subset \mathopen] -\infty,a\mathclose[\).

			Le complémentaire de \( \mathopen[ a,b\mathclose]\) est donc une union d'ouverts. Elle est ouverte.
			\spitem[Pour \ref{ITEMooGYSQooRhqHnX}]
			%-----------------------------------------------------------
			Même raisonnement que pour \ref{ITEMooOEUKooRNNzaJ}.
			\spitem[Pour \ref{ITEMooCCPRooYPqFpD}]
			%-----------------------------------------------------------
			La partie \( \mathopen] -\infty,a\mathclose]\) est ouverte parce que son complémentaire est \( \mathopen] a,\infty\mathclose[\) ouvert par \ref{ITEMooGYSQooRhqHnX}.
			\spitem[Pour \ref{ITEMooDMPGooQPbpmj}]
			%-----------------------------------------------------------
			Le complémentaire est \( \{ x<a \}\cup \{ x>b \}\). La partie \( \{ x<a \} \) est ouverte par \ref{ITEMooCCPRooYPqFpD}. Et la partie \( \{ x>b \}\) est ouverte par \ref{ITEMooGYSQooRhqHnX}. Le complémentaire de \( \mathopen[ a,b\mathclose]\) est donc une union d'ouverts. Elle est ouverte.
			\spitem[Pour \ref{ITEMooCIZXooGNZaun}]
			%-----------------------------------------------------------
			La partie \( \mathopen] a,b\mathclose]\) n'est pas ouverte parce que \( b\) est dedans alors que \( B(b,\epsilon)\) contient \( b+\epsilon/2\) qui n'est pas dans \( \mathopen] a,b\mathclose]\). Elle n'est fermée non plus parce le complémentaire contient \( a\) qui a alors le même problème : le complémentaire n'est pas ouvert.

		\spitem[Pour \ref{ITEMooIKLSooFzrgXd}]
		%-----------------------------------------------------------
		Soit \( x\in \mathopen] a,b\mathclose[\). Nous avons \( x>a\) et donc l'existence de \( \delta_1>0\) tel que \( x-\delta_1>a\). De même il existe \( \delta_2>0\) tel que \( x+\delta_2<b\). En posant \( \epsilon=\min\{ \delta_1,\delta_2 \}\) nous avons \( B(x,\epsilon)\subset \mathopen] a,b\mathclose[\). En effet si \( y\in B(x,\epsilon)\) nous avons
		\begin{equation}
			y>x-\epsilon>x-\delta_1>a
		\end{equation}
		et
		\begin{equation}
			y<x+\epsilon<x+\delta_2<b.
		\end{equation}
		\spitem[Pour \ref{ITEMooCHNKooHxCbaQ}]
		%-----------------------------------------------------------
		Même raisonnement que pour \ref{ITEMooCIZXooGNZaun}.

		\spitem[Pour \ref{ITEMooZIFNooBwwoGC}]
		%-----------------------------------------------------------
		Même raisonnement que pour \ref{ITEMooCCPRooYPqFpD}.
		\spitem[Pour \ref{ITEMooLSIPooZimrIK}]
		%-----------------------------------------------------------
		Il est ouvert parce que pour tout \( x\in \eR \) et tout \( \delta>0\), \( B(x,\delta)\subset \eR\). Il est fermé parce que son complémentaire est \( \emptyset\) qui est ouvert.
	\end{subproof}
\end{proof}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooVLHWooKiAQTr}
	La fonction \( x\mapsto x^2\) est :
	\begin{enumerate}
		\item
		      Décroissante\footnote{Définition \ref{DEFooFTZAooLrtNEo}.} sur l'intervalle \( \mathopen] -\infty , 0 \mathclose]\)
		\item
		      Croissante sur l'intervalle \( \mathopen[ 0 , \infty \mathclose[\)
		\item
		      Ni croissante ni décroissante sur l'intervalle \( \mathopen[ -4 , 3 \mathclose]\).
	\end{enumerate}
	%TODOooVBWHooHhbbFp. Prouver ça.
\end{proposition}
...
\begin{proof}
	Supposons que \(f \colon I\to \eR  \) est strictement croissante. Soient \( a\neq b\) dans \( I\). Supposons que \( a<b\). Alors \( f(a)<f(b)\) et donc \( f(a)\neq f(b)\). Idem si \( a>b\).
\end{proof}
...
\begin{proposition}[\cite{MonCerveau, BIBooMistral}]	\label{PROPooQNXVooNtBaxH}
	Le maximum de fonctions continues est continue.

	Plus précisément. Soit un espace topologique \( X\). Soit un ensemble fini d'applications continues \(f_i \colon X\to \eR  \). Alors l'application
	\begin{equation}
		\begin{aligned}
			f\colon X & \to \eR              \\
			x         & \mapsto \max_if_i(x)
...
\begin{proof}
	Nous faisons avec deux fonctions. Pour plus, il suffit de faire une récurrence. Nous avons :
	\begin{equation}
		\max(f_1,f_2)=\frac{ f_1+f_2+| f_1-f_2 | }{ 2 }.
	\end{equation}
	Étant donné que la valeur absolue \( | . |\) est continue, le résultat découle de la continuité de fonctions composée, de sommes et de produit par un scalaire (\( 1/2\)).
\end{proof}
