...
\begin{proof}
    ...
    %TODOooBTPVooTOrwQd: finir la preuve
...
%TODO: Par "connexité" et pas "convexité" ?
        Soit \( z\in C\) et \( t\in \mathopen] 0 , 1 \mathclose[\); nous notons \( P=\pr_Cx\). Vu que \( y\) et \( P\) sont dans \( C\) et que \( C\) est convexe\footnote{Définition \ref{}.}, le point \( z=ty+(1-t)P\) est également dans \( C\), et par conséquent,
		\begin{equation}
...
\begin{equation}
	S_f(x)=\int_0^xf(t)dt.
\end{equation}
Nous pouvons nous demander si, pour une fonction \( f\) donnée, il existe une ou plusieurs primitives, c'est-à-dire si il existe une ou plusieurs fonctions \( F\) telles que \( F'=f\). La réponse viendra par \ref{}.
