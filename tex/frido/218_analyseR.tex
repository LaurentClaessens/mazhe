...
\begin{definition}
	...
	\begin{equation}	\label{EqConditionUnifCont}
		\forall\varepsilon>0,\,\exists\delta>0\tq\,\forall x,y\in D,\,\| x-y \|\leq\delta \Rightarrow| f(x)-f(y) |<\varepsilon.
	\end{equation}
\end{definition}
...
\begin{proof}
	...
	\begin{subequations}     \label{EQooGXWZooDJZNzE}
		\begin{numcases}{}
			\left| \sum_{i=0}^n a_iz^i \right|=\big( 1+\alpha(z) \big)| a_nz^n |.         \\
...
Le lemme suivant parle de polynôme sur \( \eC\). Vous pouvez l'adapter à \( \hat \eR\) et \( \overline \eR\).
...
\begin{proof}
	Soient les racines \( \lambda_1\), \( \lambda_2\) et \( \lambda_3\) du polynôme en question. Toutes trois sont dans \( \eC\). Supposons que \( \lambda_1\) ne soit pas réelle. Alors \( \lambda_2\) ou \( \lambda_3\) doit être égale à \( \overline{\lambda_1}\). Disons \( \lambda_2\). Nous avons donc les racines \( \lambda_1\), \( \overline{\lambda_1}\) et \( \lambda_3\). Le polynôme se factorise alors en
	\begin{equation}        \label{EQooELMMooNbpBgg}
		a(X-\lambda_1)(X-\overline{\lambda_1})(X-\lambda_3).
	\end{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		Le groupe \( \psi(G)\) est connexe et résoluble. En effet \( \psi(G)\) est connexe en tant qu'image d'un connexe par une application continue (proposition~\ref{PropGWMVzqb}). Et il est résoluble en tant qu'image d'un groupe résoluble par un morphisme par la proposition~\ref{PropBNEZooJMDFIB}. Puisque \( \psi(G)\) est un sous-groupe résoluble et connexe de \( \GL(V)\) et que la dimension de \( V\) est strictement plus petite que celle de \( \eC^n\), une récurrence sur la dimension indique que \( \psi(G)\) est conjugué à un groupe de matrices triangulaires. C'est-à-dire qu'il existe une base de \( V\) dans laquelle toutes les matrices \( g_1\) (avec \( g\in G\)) sont triangulaires supérieures.
...
\begin{proof}
	...
	\end{equation}
	C'est le moment de se demander comment se présente la matrice \( T_s\). En tenant compte du fait que \( (D_s)_{ik}=\delta_{i,k}s^{1-i}\) nous avons
	\begin{equation}
...
\begin{proposition}     \label{PROPooXEQLooHvzVVm}
	Soient des espaces vectoriels normés \( E\) de dimension \( n\) et \( F\) de dimension \( m\) sur \( \eK\) (corps normé). Nous considérons une base \( \{ e_s \}_{s=1,\ldots, n}\) de \( E\) et \( \{ f_{\alpha} \}_{\alpha=1,\ldots, m}\) de \( F\).
...
%TODO: Pourquoi cette def. est ici, et pas plus haut ? Et meme est-on sure que cette def. n'existe pas déjà avant ?
...
\begin{proposition}     \label{PROPooDRHMooYzXbkl}
	Soit un espace vectoriel normé \( E\) de dimension finie. Soit une suite d'opérateurs \( T_n\in \End(E)\). Si \( \{ e_i \}\) est une base de \( E\) et si \( T_n(e_i)\stackrel{E}{\longrightarrow}e_i\) pour tout \( i\), alors \( T_n\stackrel{\End(E)}{\longrightarrow}\id\).
\end{proposition}

\begin{proof}
	Nous utilisons l'application \( \psi\colon \eM(n,\eK)\to \End(E)\) définie en \ref{DEFooJVOAooUgGKme}. Elle nous permet d'écrire
	\begin{equation}
...
Si \( E\) est un espace vectoriel normé, nous avons déjà défini son dual topologique \( E'\) comme étant l'ensemble des applications linéaires continues \( E\to \eC\) ou \( E\to \eR\) selon le corps de base de \( E\). C'est la définition \ref{DefJPGSHpn}.

\begin{proposition}
	Soient un espace vectoriel normé \( E\) et un élément \( v\in E\) vérifiant \( \| v \|=1\). Il existe une forme \( \varphi\in E'\) telle que \( \| \varphi \|=1\) et \( \varphi(v)=1\).
\end{proposition}

\begin{proof}
	Nous allons utiliser le théorème de la base incomplète \ref{THOooOQLQooHqEeDK}. Pour cela nous considérons \( I=E\) et la partie clairement génératrice \( G=\{ e_i=i \}_{i\in I}\) (si vous avez bien suivi, \( G=E\) en fait; rien de bien profond). Nous considérons ensuite \( I_0=\{ v \}\). Le théorème de la base incomplète nous donne l'existence de \( I_1\) tel que \( I_0\subset I_I\subset I\) et tel que \( B=\{ e_i \}_{i\in I_1}\) est une base.
	...
	Nous passons maintenant à la définition de \( \varphi\colon E\to \eK\). Pour \( x\in E\) nous commençons par écrire
	\begin{equation}
	...
	Nous devons encore montrer que \( \| \varphi \|\leq 1\). Un élément \( x\in E\) s'écrit toujours sous la forme
	\begin{equation}
	...
	Notons que \( \varphi\) est continue (et donc bien dans \( E'\)) parce qu'elle est bornée (proposition \ref{PROPooQZYVooYJVlBd}).
\end{proof}
...
\begin{proof}
	...
	\begin{equation}
%TODO: cette notation  _m  est-elle introduite precedemment ? Sinon, ne vaut-il pas mieux d'écrire exhaustivement  _{\eR^m} ?
...
\begin{proof}
	...
	\begin{equation}
		A^n=Q^{-1}D^nQ.
	\end{equation}
	La matrice \( D^n\) est diagonale et \( D^n_{ii}=\lambda_i^n\). Par la proposition \ref{PROPooUMVGooIrhZZg}\ref{ITEMooFXKYooUOXbwH}, nous avons \( | \lambda_i |^n=1\).
%TODO: Le ref ne doit pas etre la bonne ; ou alors, je ne vois pas pourquoi   | \lambda_i |^n=1
	...
	\begin{subproof}
		...
		\end{equation}
		En ce qui concerne la norme \( \| D \|_2\), nous avons le lemme \ref{LEMooCSBVooZzqxqg} qui nous annonce que \( \| D \|_2=\max_i \{| \lambda_i |\} \). Dans notre cas, nous avons donc \( \| D \|_2=1\) et
		\begin{equation}
...
%TODO : Remplacer dans \spitem  le M par D  du début de la preuve ?
		\spitem[La matrice \( A\)]
		...
		\end{equation}
%TODO "appliquer ce la partie déjà faite" ? Pas français
...
\begin{proof}
	Puisque \( D\) est une droite, il existe \( v\in V\) tel que \( D-v\) soit une droite vectorielle que nous notons \( L\). Nous allons montrer que \( D-a=D-v\). Comme \( a\) est arbitraire, cela suffit.
...
\begin{proof}
	...
	\begin{subproof}
		...
		Soit \( z_0\in C\) et \( r=\| x-z_0 \|\). La boule fermée \( \overline{ B(x,r) }\) est compacte\footnote{C'est ceci qui ne marche plus en dimension infinie.} et intersecte \( C\). Puisque \( C\) est fermé, l'ensemble \( C'=C\cap\overline{ B(x,r) }\) est compact. Tous les points qui minimisent la distance entre \( x\) et \( C\) sont dans \( C'\); la fonction
		\begin{equation}
			\begin{aligned}
				C' & \to \eR        \\
				z  & \mapsto d(x,z)
...
%TODO: Par "connexité" et pas "convexité" ?
