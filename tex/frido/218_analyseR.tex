...
\begin{proposition}     \label{PROPooVOUTooOtiGLG}
	Soient des espaces métrique \( (E,d)\) et \( (E',d')\).
	\begin{enumerate}
		\item		\label{ITEMooOCZCooKzxEja}
		      Toute application uniformément continue \(f \colon E\to E'  \) est continue;
		\item		\label{ITEMooZLAJooBTwiAE}
		      la composée de deux fonctions uniformément continues est uniformément continue;
		\item		\label{ITEMooBDITooXNENCy}
		      Pour une application uniformément continue \( f\) nous considérons une application \(\delta_f \colon \eR^+\to \eR^+  \) telle que \( d\big( f(x),f(y) \big)<\epsilon\) pour tout \( d(x,y)<\delta_f(\epsilon)\). Alors l'application
		      \begin{equation}
			      \delta_{g\circ f}=\delta_f\circ\delta_g
		      \end{equation}
		      fonctionne pour \( g\circ f\).
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	En deux parties.
	\begin{subproof}
		\spitem[Pour \ref{ITEMooOCZCooKzxEja}]
		%-----------------------------------------------------------
		Nous voyons que \( f\) est continue en \( x_0\in E\). Soit \( \epsilon>0\). Nous prenons le \( \delta\) correspondant dans la définition de l'uniforme continuité. Nous avons alors \( d(x,x_0)<\delta\) implique \( d\big( f(x),f(x_0) \big)<\epsilon\).
		\spitem[Pour \ref{ITEMooZLAJooBTwiAE}]
		%-----------------------------------------------------------
		Soient des applications uniformément continues \(f \colon E\to F  \) et \(g \colon F\to G  \). Soit \( \epsilon>0\). Nous considérons \( \delta_1\) tel que \( d\big( g(x),g(y) \big)<\epsilon\) pour tout \( d(x,y)<\delta_1\). Et ensuite nous considérons \( \delta_2\) tel que \( d\big( f(x),f(y) \big)<\epsilon\) pour tout \( x,y\) tels que \( d(x,y)<\delta_1\). Maintenant si \( d_E(x,y)<\delta_2\) nous avons
		\begin{equation}
			f_F\big( f(x),f(y) \big)<\delta_1,
		\end{equation}
		et donc
		\begin{equation}
			d_G\Big( g\big( f(x) \big),g\big( f(y) \big) \Big)<\epsilon.
		\end{equation}

		\spitem[Pour \ref{ITEMooBDITooXNENCy}]
		%-----------------------------------------------------------
		Il suffit de déballer un peu ce qu'on a fait pour \ref{ITEMooZLAJooBTwiAE}.
	\end{subproof}
\end{proof}
