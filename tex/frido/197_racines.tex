...
\begin{lemma}[\cite{BIBooOHJHooJHJyyp, MonCerveau}]
    Soient des nombres complexes distincts \( a_1,\ldots, a_N\). Alors
    \begin{equation}
        \frac{1}{ \prod_{i=1}^N(X-a_i) }=\sum_{i=1}^N\frac{ \lambda_i }{ X-a_i }
    \end{equation}
    où 
    \begin{equation}
...
\begin{proof}
    Nous posons \( P(X)=\prod_{i=1}^N(X-a_i)\), et nous calculons :
    \begin{subequations}
        \begin{align}
            \sum_{i=1}^N\frac{ \lambda_i }{ X-a_i }&=\sum_{i=1}^N\frac{ \lambda_i\prod_{k\neq i}(X-a_k) }{ \prod_{k=1}^N(X-a_i) }\\
            &=\frac{1}{ P(X) }\sum_{i=1}^N\lambda_i\prod_{k\neq i}(X-a_k)
        \end{align}
    \end{subequations}
    Il s'agit maintenant de prouver que la somme vaut toujours \( 1\). Nous posons
    \begin{equation}
        S(X)=\sum_{i=1}^N\frac{ \prod_{k\neq i}(X-a_k) }{ \prod_{k\neq i}(a_i-a_k) }.
    \end{equation}
    Calculons \( S(a_l)\) :
    \begin{equation}
