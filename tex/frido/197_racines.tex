...
% Copyright (c) 2011-2013,2016-2026
%   Laurent Claessens
...
\begin{propositionDef}  \label{DefWXBkOxg}

	Il existe une unique application \(\mu \colon \eN^*\to \eN^*  \) telle que pour tout \( n\in \eN^*\),
	\begin{equation}		\label{EQooLREYooSRKsJh}
		\sum_{d\divides n}\mu(d)=\begin{cases}
			1 & \text{si } n=1 \\
			0 & \text{sinon. }
		\end{cases}
	\end{equation}
	Celle fonction, nommée \defe{fonction de Möbius}{fonction!de Möbius} est donnée par
	\begin{equation}		\label{EQooTFBLooEIpcAI}
		\mu(n)=\begin{cases}
			0  & \text{si } n\text{ est divisible par un carré différent de } 1\text{,}                                                                                                                                                \\
			1  & \text{si } n\text{ est le produit d'un nombre pair\footnote{Y compris zéro. Donc \( \mu(1)=1\) parce que le produit sur un ensemble vide vaut \( 1\), voir \ref{NORMooDBOFooQCwbOY}.} de nombres premiers distincts,} \\
			-1 & \text{si } n\text{ est le produit d'un nombre impair de nombres premiers distincts,}
		\end{cases}
	\end{equation}
	Autre manière de le dire : si la décomposition en facteurs premiers\footnote{Théorème \ref{ThoAJFJooAveRvY}.} de \( n\) est \( n=\prod_ip_i^{\alpha_i}\), alors
	\begin{enumerate}
		\item
		      \( \mu(n)=0\) si un des \( \alpha_i\) est \( \alpha_i\geq 2\). Dans ce cas \( n\) est divisible par \( p_i^2\).
		\item
		      \( \mu(n)=1\) si il y a un nombre pair de \( i\) avec \( \alpha_i\neq 0\) et que pour eux, \( \alpha_i=1\)
		\item
		      \( \mu(n)=-1\) si il y a un nombre impair de \( i\) avec \( \alpha_i\neq 0\) et que pour eux, \( \alpha_i=1\)
	\end{enumerate}
\end{propositionDef}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Existence]
		%-----------------------------------------------------------
		Nous vérifions que la fonction \( \mu\) donnée par \eqref{EQooTFBLooEIpcAI} vérifie la condition \eqref{EQooLREYooSRKsJh}.
		\begin{subproof}
			\spitem[Pour \( n=1\)]
			%-----------------------------------------------------------
			Nous vérifions la condition \eqref{EQooLREYooSRKsJh} avec \( n=1\). D'abord la somme sur \( d\divides n\) contient seulement \( d=1\) et donc
			\begin{equation}
				\sum_{d\divides 1}\mu(d)=\mu(1).
			\end{equation}
			Étant donné que \( 1\) n'a pas de facteurs premiers, c'est un produit de zéro facteurs premiers (donc un nombre pair), et donc \( \mu(1)=1\).

			\spitem[Pour \( n>1\)]
			%-----------------------------------------------------------
			Nous nommons \( P\) l'ensemble des facteurs premiers de \( n\) et \( s=\Card(P)\). Dans la somme \( \sum_{d\divides n}\mu(d)\), les seuls termes non nuls sont ceux pour lesquels \( d\) est sans facteurs carrés. Un tel \( d\) est un produit d'éléments de \( P\) et donc s'écrit \( \prod_{p\in D}p\) pour un certain sous-ensemble \( D\) de \( P\). Bref :
			\begin{equation}
				\sum_{d\divides n}\mu(d)=\sum_{D\subset P}\mu\big( \prod_{p\in D}p \big)
			\end{equation}
			où la somme est sur les parties de \( P\). Vu que \( \mu(d)\) mesure le fait que \( d\) ait un nombre pair ou impair de facteurs premiers,
			\begin{equation}
				\mu\big( \prod_{p\in D}p \big)=(-1)^{\Card(D)}.
			\end{equation}
			Nous avons donc déjà
			\begin{equation}
				\sum_{d\divides n}\mu(d)=\sum_{D\subset P}(-1)^{\Card(D)}=\sum_{t=0}^{\Card(P)}(-1)^t\Card\big( D\subset P\tq\Card(D)=t \big)
			\end{equation}
			où la dernière égalité revient à grouper les termes de la somme sur \( D\) en termes où \( D\) a le même cardinal. Combien de sous-ensemble de cardinal \( t\) a-t-on dans \( P\) ? C'est le lemme \ref{LEMooUTDTooXAmvdF} qui dit qu'il y en a \( \binom{ n }{ k }\). Nous avons donc
			\begin{subequations}
				\begin{align}
					\sum_{d\divides n}\mu(d) & =\sum_{D\subset P}(-1)^{\Card(D)}                                                                         \\
					                         & =\sum_{t=0}^{\Card(P)}(-1)^t\Card\big( D\subset P\tq\Card(D)=t \big)                                      \\
					                         & =\sum_{t=0}^s\binom{ s }{ t }(-1)^t                                                                       \\
					                         & =(-1+1)^s                                                            & \text{prop. \ref{PropBinomFExOiL}} \\
					                         & =0.
				\end{align}
			\end{subequations}
		\end{subproof}
		...
		\spitem[Unicité]
		%-----------------------------------------------------------
		Supposons que l'application \(f \colon \eN^*\to \eN^*  \) vérifie la condition \ref{EQooLREYooSRKsJh}. Nous allons montrer que \( f\) est alors définie par une relation de récurrence qui la définit complètement. D'abord en écrivant la relation avec \( n=1\) nous trouvons
		\begin{equation}
			\sum_{d\divides 1}f(d)=1.
		\end{equation}
		Et comme il y a seulement \( d=1\) dans la somme, nous trouvons immédiatement \( f(1)=1\). Ensuite nous savons que si \( n>0\),
		\begin{equation}
			\sum_{d\divides n}f(d)=0.
		\end{equation}
		Nous sortons le terme \( d=n\) de cette somme :
		\begin{equation}
			0=\sum_{d\divides n}f(d)=f(n)+\sum_{\substack{ d\divides n \\ d<n }  }f(d).
		\end{equation}
		Cela montre que \( f(n)\) est donnée par
		\begin{equation}
			f(n)=\sum_{\substack{d\divides n \\ d<n }  }f(d).
		\end{equation}
		Donc \( f(n)\) est entièrement définie par les valeurs de \( f(d)\) avec \( d<n\).
	\end{subproof}
\end{proof}


\begin{normaltext}			\label{NORMooAULBooKxNwLJ}
	La fonction de Möbius arrive dans cette vidéo de Numberphile après 55 minutes de souffrances :
	\begin{center}
		\url{https://www.youtube.com/watch?v=XhA5U9pFXuU}
	\end{center}
...
\begin{proposition}[\cite{POkXeBE,MonCerveau}]     \label{PROPooOVYJooFvmxyj}
	Soient \( p,q\in \eN^*\) premiers entre eux. Alors
	\begin{equation}		\label{EQooXEIAooMGIhtZ}
		\mu(pq)=\mu(p)\mu(q).
	\end{equation}
\end{proposition}

\begin{proof}
	Si \( p\) ou \( q\) a un facteur carré, alors \( pq\) aussi et les deux membres de \eqref{EQooXEIAooMGIhtZ} sont nuls. Supposons que ce ne soit pas le cas. Soit \( D_p\) l'ensemble des diviseurs premiers de \( p\), et \( D_q\) celui de \( q\). Nous avons \( D_p\cap D_q=\emptyset\), sinon \( p\) et \( q\) ne seraient pas premiers entre eux.

	La valeur de \( \mu(p)\) est la parité de \( \Card(D_p)\). Comme \( D_{pq}=D_p\cup D_q\) et que l'union est disjointe, la parité de \( \Card(D_{pq})\) est le produit des deux.
\end{proof}
