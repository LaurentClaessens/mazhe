...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            c\colon \eN^N&\to \{ 0,1 \} \\
            x&\mapsto \prod_{i=1}^Nb_i(x),
        \end{aligned}
    \end{equation}
    et nous décomposons la somme en \( V_n(N)_a\) et son complémentaire dans \( V_n(N)\) :
    \begin{equation}
        \sum_{x\in V_n(N)}\prod_{i=1}^Nb_i(x)=\sum_{x\in V_n(N)_a}c(x)+\sum_{x\in V_n(N)\setminus V_n(N)_a}c(x).
    \end{equation}
    <++>
\end{proof}
<++>

\begin{theorem}[\cite{fJhCTE,NHXUsTa,BIBooLIDJooYWosFk,KXjFWKA}] \label{THOooQDYWooCOiUMb}
    Soient \( N\in \eN\) et \( a\in \eN^N\) tel que \( \pgcd(a_i,\ldots, a_{N})=1\). Alors
    \begin{equation}
...
\begin{theorem}[\cite{fJhCTE,NHXUsTa,BIBooLIDJooYWosFk,KXjFWKA}] \label{ThoDIDNooUrFFei}
...
\begin{proposition}[\cite{KXjFWKA,BIBooLIDJooYWosFk}]     \label{PropWUFpuBR}
