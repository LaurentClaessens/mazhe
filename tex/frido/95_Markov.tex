...
\begin{definition}      \label{DEFooGDPFooWsvfRv}
    Soit \( E\) un ensemble au plus dénombrable et \( (\Omega,\tribF,P)\) un espace de probabilité. Une \defe{chaine de Markov}{chaine!de Markov} à valeurs dans \( E\) est une famille \( (X_n)_{n\in\eN}\) de variables aléatoires telles que pour tout \( x_0,\ldots,x_{n+1}\in E\),
    \begin{equation}
...
\begin{definition}[Matrice de transition]
    Nous notons \( Q\) la matrice de transition\index{matrice de transition} qui est éventuellement infinie:
    \begin{equation}
        Q_{xy}=p(x,y).
    \end{equation}
\end{definition}
...
\begin{proof}
    Lorsque \( E\) est un ensemble fini, il n'y a pas de sport; quand \( E\) est du type de \( \eN\) non plus. Nous supposons donc que \( E\) est quelconque et nous utilisons la définition \ref{DefHYgkkA}. Vu que les \( p_n(x,y)\) sont des probabilités, ce sont des nombres réels positifs et la définition fonctionne.
\end{proof}
