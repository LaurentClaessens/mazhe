...
% Copyright (c) 2012-2013,2015,2018-2024
%   Laurent Claessens
...
\begin{definition}      \label{DefGJEBooZvuIAV}
	Une matrice dont tous les éléments sont positifs ou nuls et dont la somme de chaque ligne vaut \( 1\) est une \defe{matrice stochastique}{matrice stochastique}.
\end{definition}
...
\begin{probleme}		\label{PROBooNEMXooLbPpIN}
	Je ne suis pas totalement certain du lemme suivant (\ref{LEMooYKFZooUEOWbi}). Soyez \randomGender{prudent}{prudente}, et écrivez-moi si vous avez une démonstration ou un contre-exemple.

	Le lemme \ref{LEMooQBOIooMNbJlV} donne une partie de la preuve, je crois.
\end{probleme}

\begin{lemma}		\label{LEMooYKFZooUEOWbi}
	...
	% TODOooOBMGooXDBIac. Prouver ça.
	% Quand c'est fait, fermer https://github.com/LaurentClaessens/mazhe/issues/270
\end{lemma}

\begin{definition}
	Soit une matrice stochastique \( p\). Une \defe{loi stationnaire}{loi stationnaire} pour \( p\) est un vecteur \( \mu\) tel que
	\begin{enumerate}
		\item
		      \( \mu=\mu P\),
		\item
...
\begin{proof}
	...
	\begin{subproof}
		...
		Sinon nous supposons que \( P\) n'a que des éléments positifs, quitte à considérer \( P^n\) au lieu de \( P\). Nous notons \( d\) le plus petit élément de \( P\); il vérifie \( d\leq \frac{ 1 }{2}\) parce que la somme des éléments d'une ligne de la matrice \( P\) doit être égale à \( 1\).
...
Le \defe{graphe de transition}{graphe!de transition (chaine de Markov)} d'une chaine de Markov est le graphe dont les sommets sont les éléments de l'espace des états de la chaine et dont les sommets sont reliés par des arêtes pondérées par la probabilité de transition correspondante.
...
\begin{definition}[Temps de première atteinte]		\label{DEFooDWLAooINqgep}
	Soit \( (X_k)\) une chaine de Markov dont l'espace des états est noté \( E\). Pour chaque \(  x\in E\) nous notons
	\begin{equation}
		T(x)=\inf\{ k\geq 1\tq X_k=x \}.
	\end{equation}
	Ce nombre est le	\defe{temps de première atteinte}{temps de première atteinte} de l'état \( x\).

	Si \( X_0=x\), alors \( T(x)\) est le \defe{temps de retour}{temps de retour} en \( x\). Si \( p\in \eN\) nous notons
	\begin{equation}
...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Récurrent et transient}
%---------------------------------------------------------------------------------------------------------------------------
...
Nous rappelons\footnote{Définition \ref{DEFooDWLAooINqgep}.} que \( T(x)\) est le temps de première atteinte de l'état \( x\). Nous notons\nomenclature[M]{\( \pi(x)\)}{lié au temps de retour}
...
%-------------------------------------------------------
\subsection{Périodique et irréductible}
%----------------------------------------------------
...
\begin{propositionDef}		\label{DEFooBNBVooBuWQqi}
	...
	Dans ce cas nous disons que la chaine est \defe{apériodique}{chaine de Markov apériodique}.
\end{propositionDef}
...
\begin{proof}
	...
	\end{equation}
	Commençons par préciser les espaces sur lesquels nos variables aléatoires sont définies. Nous avons
	\begin{equation}
...
%-----------------------------------
\subsubsection{Cadeau numberphile}
...
\begin{theorem}[loi des grands nombres pour les chaines de Markov]\index{loi!des grands nombres!pour les chaines de Markov}
...
\begin{proof}
	...
	\end{equation}
	% TODOooTSZOooRpotik : il me semble que je dois encore terminer cette preuve.
\end{proof}
...
\begin{theorem}[Convergence en loi des chaines de Markov]
	Si \( (X_n)\) est
	\begin{enumerate}
		\item
		      irréductible,
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[La probabilité d'extinction est un point fixe de \( G\)]
