...
\begin{proposition}[\cite{BIBooOHTZooXCTwxj}]       \label{PROPooBYELooKLsthC}
    ...
    \begin{enumerate}
        ...
        \item
            Une application \( f\colon E\times F \to E\).
    \end{enumerate}
...
\begin{proof}
    ...
    \end{equation}
    Le lemme \ref{LEMooWAOSooBsGucQ} dit que les variables aléatoires \( f(X_n, Z_{n+1})\) et \( g(X_0,Z_1,\ldots, Z_n)\) sont indépendantes.
\end{proof}
<++>
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}[]
        X_{n+1}(\omega)=j&&\text{si} \sum_{k=0}^{j-1}P_{X_n(\omega),k}\leq Z_{n+1}(\omega)<\sum_{k=0}^jP_{X_n(\omega),k}.
        \end{aligned}
    \end{equation}
    %TODOooRWJLooHkwdAc finir à partir d'ici
\end{proof}
<++>
...
\begin{proposition}[\cite{GMbugcT}]     \label{PROPooCAORooMEaxtq}
