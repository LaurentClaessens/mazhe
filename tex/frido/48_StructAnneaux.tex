...
On rappelle la définition \ref{DefHXJUooKoovob} d'un anneau ainsi que les notations $0$ pour le neutre additif et $1$ pour le neutre multiplicatif.
...
\begin{definition}[Diviseurs dans un anneau]
	Soient \( a, b \in A \). On dit que $a$ divise $b$, ou que $a$ est un \defe{diviseur}{diviseur!dans un anneau} de $b$ s'il existe \( c \in A \) tel que \( ac = b \).
\end{definition}
Un cas particulier est le cas des diviseurs de zéro.
...
\begin{theorem}     \label{ThoProjModpAlsUR}
    Soient des sous modules \( M_1,\ldots,M_n\) du module \( M \) tels que \( M=M_1\oplus\ldots\oplusM_n\). Les applications \( p_i\) définies par
    \begin{equation}
...
\begin{example}
    Nous verrons au théorème \ref{ThoBUEDrJ} que si \( A\) est intègre, alors l'anneau des polynômes sur \( A\) est intègre.
\end{example}
...
\begin{example}     \label{ExybCZyl}
    Si \( A\) est un anneau intègre, l'anneau \( A[X]\) des polynômes sur \( A\) est également intègre. En effet si \( P\) et \( Q\) sont deux polynômes non nuls, alors le coefficient du terme de plus haut degré de \( PQ\) est donné par le produit des coefficients de plus haut degré de \( P\) et \( Q\) qui est non nul parce que \( A\) est intègre.
\end{example}
