...
Cet exemple ne fonctionne pas avec \( \eZ\) au lieu de \( \eN\) parce que le stathme doit avoir des valeurs dans \( \eN\). Cela ne veut cependant pas dire qu'il n'existe pas de stathme sur \( \eZ\); cela veut seulement dire que \( \alpha(x)=x\) ne fonctionne pas. L'exemple \ref{EXooXQKLooXdAWuX} montrera que \( \eZ\) ne possède pas de stathme euclidien en montrant qu'il n'est pas principal.
...
\begin{example}[\( \eZ\) n'est ni principal ni euclidien]       \label{EXooXQKLooXdAWuX}
    L'idéal \( (2)=2\eZ\) n'est pas un idéal principal de \( \eZ\), parce que tout multiple entien d'un nombre pair reste pair. Donc \( \eZ\) possède des idéaux non principaux et n'est pas conséquent pas un anneau principal.

    Il ne peut alors pas non plus être euclidien par la proposition \ref{Propkllxnv}. 
\end{example}
...
\begin{corollary}       \label{CORooINGJooHdOKgn}
    Si \( A\) est un anneau intègre\footnote{Définition \ref{DEFooTAOPooWDPYmd}.}, l'anneau \( A[X]\) des polynômes à coefficients dans \( A\) est un anneau euclidien et principal.
\end{corollary}
