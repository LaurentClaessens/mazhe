...
\begin{definition}[\cite{BIBooSTWWooItiMUp}]
...
\begin{proof}
    Soient \( J\) fini dans \( I\) ainsi que des éléments \( a_j\in A\) (\( j\in J\)). Nous supposons que \( \sum_{j\in J}a_je_j=0\). Calculons un peu :
    \begin{equation}
...
\begin{theorem}[\cite{BIBooSTWWooItiMUp}]
...
L'ensemble des polynômes sur \( A\) sera simplement \( A^{(\eN)}\). Vu que \( \eN\) est un ensemble bien particulier possédant plein de structure, nous allons pouvoir mettre sur \( A^{(\eN)}\) une structures non seulement de \( A\)-module (ça c'est déjà fait), mais en plus d'anneau ainsi qu'une évaluation.
\begin{definition}      \label{DEFooFYZRooMikwEL}
    L'ensemble des \defe{polynômes}{polynômes} sur \( A\) est \( A^{(\eN)}\).
\end{definition}
...
\begin{definition}      \label{DEFooNXKUooLrGeuh}
    ...
    \begin{description}
        \item[L'évaluation] Si \( x\in A\) et si \( P\in A^{(\eN)}\), nous définissons \( P(x)\) par
            \begin{equation}
                P(x)=(\sum_{i=0}^{n}a_ie_i)(x)=\sum_{i=0}^na_ix^i,
            \end{equation}
            étant entendu que \( x^0=1\) dans \( A\).
    \item[Le produit] C'est ici que la structure particulière de \( \eN\) est utilisée. Nous définissons le produit \( A^{\eN}\times A^{(\eN)}\to A^{(\eN)}\) par
