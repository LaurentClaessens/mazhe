...
\begin{example}[\( \eZ\) n'est ni principal ni euclidien]       \label{EXooXQKLooXdAWuX}
    L'idéal \( (2)=2\eZ\) n'est pas un idéal principal de \( \eZ\), parce que tout multiple entier d'un nombre pair reste pair. Donc \( \eZ\) possède des idéaux non principaux et n'est pas conséquent pas un anneau principal.
...
\begin{normaltext}
    Le théorème \ref{ThodivEuclPsFexf} nous incite à utiliser le degré comme stathme euclidien sur \( A[X]\) dès que \( A\) est un anneau intègre. Or cela ne fonctionne en génral pas parce que très peu de polynômes ont a priori un coefficient dominant inversible.

    En exemple typique, l'anneau \( \eZ[X]\) n'est pas euclidien. Il suffit de reprendre le même argument que celui donné dans l'exemple \ref{EXooXQKLooXdAWuX} : l'idéal \( 2\eZ[X]\) dans \( \eZ[X]\) n'est pas principal. Donc \( \eZ[X]\) n'est pas un idéal principal et par conséquent pas euclidien non plus.
\end{normaltext}

\begin{lemma}       \label{LEMooIDSKooQfkeKp}
    Si \( \eK\) est un corps\footnote{Définition \ref{DefTMNooKXHUd}.}, alors l'anneau \( \eK[X]\) est euclidien et intègre.
\end{lemma}
