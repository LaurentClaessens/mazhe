...
\begin{proof}
    Vu la définition du produit dans le quotient nous avons
    \begin{equation}
        \bar X\bar X=\bar X^2=\overline{ X^2 }.
    \end{equation}
    Nous avons de façon similaire \( (\bar X)^k=\overline{ X^k }\). Par cela et la linéarité de l'application «prise de classe» nous avons bien
    \begin{equation}
        Q(\bar X)=\overline{ Q(X) }.
    \end{equation}
    Nous savons aussi par ailleurs que \( Q(X)=Q\), comme expliqué dans le lemme \ref{LEMooGKWQooVOyeDX}.
\end{proof}
