...
\begin{proof}
    ...
    \end{equation}
    D'autre part, vu que \( ab=q^k\), nous avons
    \begin{equation}
...
\begin{proof}
    ...
    Vu que les pgcd et ppcm sont uniques (lemme \ref{LEMooBJVJooFyuFeN}), nous avons prouvé que \( \delta\) et \( \mu\) sont les nombres recherchés.
\end{proof}
...
\begin{lemma}[\cite{BIBooGIDYooKaacXf}]
...
\begin{theorem}[\cite{XPXxPl}]
    Soient un anneau principal et \( S\ubset A\) non vide.
    \begin{enumerate}
        \item
    ...
    En particulier, toute partie d'un anneau principal admet un PGCD et un PPCM.
\end{theorem}
