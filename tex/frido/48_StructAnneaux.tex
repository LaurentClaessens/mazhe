...
\begin{lemma}[\cite{MonCerveau}]		\label{LEMooUTDTooXAmvdF}
	...
	\begin{equation}        \label{EQooSPVNooHoPnOe}
		\Card(C)={n\choose k}
	\end{equation}
	où les ${n\choose k}$ sont les \defe{coefficients binomiaux}{coefficients binomiaux} donnés par
	\begin{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		D'abord \( pA\) est strictement inclus à \( A\) parce que \( p\) n'étant pas inversible, l'élément \( 1\) n'est pas dans \( pA\).
...
\begin{proof}
	...
	\end{equation}
	Par récurrence nous construisons les \( q_^{(m)}_i\).
\end{proof}
