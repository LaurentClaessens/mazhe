...
\begin{normaltext}
    Nous allons voir que le concept d'élément irréductible n'est vraiment utile que dans le cas des anneaux intègres.
\end{normaltext}
...
\begin{definition}[\cite{ooWBLYooLYwALS}]       \label{DEFooZCRQooWXRalw}
    Soit une anneau commutatif \( A\). Un élément \( p\in A\) est \defe{premier}{élément premier} si il est
    \begin{enumerate}
        \item
            non nul,
        \item
...
\begin{proposition}     \label{PROPooWMNPooZdvOBt}
    Tout élément premier d'un anneau intègre\footnote{Si pas intègre, voir l'exemple \ref{EXooEIUEooCZCPMC}.} est irréductible\footnote{Toutes les définitions dans le thème \ref{THEMEooVIQIooOcFAQS}.}.
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooHPAIooYoQzqD} implique \ref{ITEMooBTEAooWlFUTX}]
            En plusieurs points.
            \begin{itemize}
                \item D'abord \( (p)\) est strictement inclus à \( A\). Oui, ça fait partie de l'hypothèse \ref{ITEMooHPAIooYoQzqD}.
                \item Soient \( a,b\in A\) tels que \( ab\in (p)\).
            %ooCZWCooObvyqC aussi
            \end{itemize}
            <++>
    \end{subproof}
    <++>
