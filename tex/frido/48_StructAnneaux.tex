...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{subproof}
			...
			\spitem[Cas général]
			%-----------------------------------------------------------
			Si \( | H |\) n'est pas la puissance d'un nombre premier, alors \( | H |\) le corolaire \ref{CORooWBSQooQOEmaC}
		\end{subproof}<++>
...
\begin{proof}
	...
	\begin{equation}
		n=\prod_{i=1}^rp_i^{k_i}
	\end{equation}
	avec au moins \( r=2\), et les \( k_i\) non nuls. En prenant \( a=p_1^{k_1}\) et \( b=\prod_{i=2}^rp_i^{k_i}\) nous avons bien \( n=ab\) et
\end{proof}<++>
