...
%TODO: pour (1) et (2), la notation des indices "en dehors" de l'ensemble me semble fausse. Plutot : Union sur i ou k des ensembles de classes ?
    \begin{enumerate}
        \item
...
\begin{lemma}[Lemme d'Euclide\cite{BTDWooZCyXfb}]       \label{LemAXINooOeuMJZ}
    Si un nombre premier $p$ divise le produit de deux nombres entiers $a$ et $b$, alors $p$ divise $a$ ou $b$.
\end{lemma}
\index{Euclide!lemme}
...
\begin{proof}
    ...
    \begin{equation}
        ab=\prod_ip_i^{a_i+b_i}=\big( \prod_ip_i^{q_i} \big)^k=\prod_ip_i^{kq_i}.
    \end{equation}
...
%TODO: tres mauvaise demonstration : que sont les \delta_p ? pas les m_p plutot ? Pourquoi c'est le PLUS Grand ? (PGcd)
    Nous commençons par montrer que \( \delta\) est le pgcd de \( a\) et \( b\). Puisque \( \delta_p=\min\{ a_p,b_p \}\), nous avons \( \delta_p\leq a_p\) et \( \delta_p\leq b_p\). Le lemme \ref{LEMooDTQQooYoJABt} nous dit alors que \( \delta\divides a\) et \( \delta\divides b\). De même, si \(s\divides a\) et \( s\divides b\), nous avons \( s_p\leq a_p\) et \( s_p\leq b_p\), ce qui montre que \( s_p\leq m_p\) et donc que \( s\divides \delta\).
%TODO: Je preferai voir la demonstration... !
...
\begin{subequations}            \label{EqYCQVooZzHuRq}
    ...
    \end{numcases}
\end{subequations}
avec \( k\in\eZ\). Si nous voulons réellement que les solutions dans \( \eN\) et non dans \( \eZ\), il faut seulement un peu restreindre les valeurs de \( k\). Il en reste un nombre fini parce que l'équation pour \( x\) borne \( k\) vers le bas tandis que celle pour \( y\) borne \( k\) vers le haut.
...
\begin{definition}\label{DefrXUixs}
    On dit de deux éléments \( a,b\in A\) qu'ils sont \defe{associés}{associés!éléments d'un anneau} si ils vérifient les hypothèses du lemme~\ref{LemRmVTRq}; en d'autres termes, $a$ et $b$ sont associés si il existe un inversible \( u\in A\) tel que \( a=ub\).
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    sont injectives. En tant que applications injectives entre ensembles finis, elles sont surjectives. Il existe donc \( b\) et \( c\) tels que \( 1=ba=ac\). On trouve que \( b\) et \( c\) sont égaux parce que
...
%TODO: pourquoi utiliser \sum plutot que le sigle de l'union ? (on a \bigcap pour l'intersection)
