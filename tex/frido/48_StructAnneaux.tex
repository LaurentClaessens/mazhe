...
\begin{proposition}
    Soient \( A\) un anneau commutatif non nul et \( I\) un idéal dans \( A\). L'idéal \( I\) est maximal de \( A\) si et seulement si \( A/I\) est un corps.
\end{proposition}
...
\begin{probleme}
  Comme on le voit, il y a des redites. Tout contributeur qui a une proposition de réorganisation est le bienvenu.
\end{probleme}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooMQWVooNocVEU} implique~\ref{ITEMooJBXGooEISNuW}]
            ...
            Maintenant \( I\) est un idéal premier non réduit à \( \{ 0 \}\). Vu que \( A\) est un anneau principal, il existe \( x\in A\) tel que \( I=(x)\). Nous devons prouver que \( x\) peut être choisi irréductible; et nous allons faire plus : nous allons prouver que \( x\) ne peut être que irréductible\quext{ça me semble un peu trop facile. Lisez attentivement, et écrivez-moi pour dire sit vous êtes d'accord ou pas.}.
