...
%TODOooEXYZooSqMOom. Énoncer et prouver cette proposition pour un corps un peu plus général.
% en particulier pour utilisation dans LemZrUUOz.
...
\begin{proof}
	Pour cette preuve, nous allons écrire \( \pgcd(a,b)\) l'ensemble des PGCD de \( a\) et \( b\), c'est-à-dire la classe d'association d'un PGCD.
	...
	\begin{subproof}
		...
		\begin{equation}
			1\in\pgcd(a,b)=\sum_{x\in\{ a,b \}}(x)=(a)+(b).
		\end{equation}
		%TODOooPDZCooHXdlzh. Il faut ici citer correctement le lemme LEMooARNUooXqrJGa, ITEMooEZAPooJuvce parce que tel quel, ça n'a pas beaucoup de sens.
		\spitem[\( \Leftarrow\)]
		%-----------------------------------------------------------
		À l'inverse, si nous avons \( ua+vb=1\), alors \( 1\in (a)+(b)\), mais puisque \( (a)+(b)\) est un idéal principal, \( (1)=(a)+(b)\) et donc \( 1\in \pgcd(a,b)\) par le lemme \ref{LEMooARNUooXqrJGa}\ref{ITEMooEZAPooJuvceO}.
	\end{subproof}
\end{proof}
...
\begin{proof}
	Le théorème de Bézout \ref{CorimHyXy} donne des éléments \( u,v,x,y\in A\) tels que \( ua+vb=1\) et \( xa+yc=1\). En multipliant ces équations l'une avec l'autre,
	\begin{equation}
		(ua+vb)(xa+yc)=1.
	\end{equation}
