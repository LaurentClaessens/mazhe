...
\begin{propositionDef}[demi-plan]       \label{PROPooENHRooZAMPiX}
    Soit une droite \( D\) ainsi que de deux applications affines associées \( f_1\) et \( f_2\). Nous posons
    \begin{subequations}
        \begin{align}
            H_1^+=\{ x\in \eR^2\tq f_1(x)>0 \}\\
            H_1^-=\{ x\in \eR^2\tq f_1(x)<0 \}
        \end{align}
    \end{subequations}
    ainsi que
    \begin{subequations}
        \begin{align}
            H_2^+=\{ x\in \eR^2\tq f_2(x)>0 \}\\
            H_2^-=\{ x\in \eR^2\tq f_2(x)<0 \}
        \end{align}
    \end{subequations}
    Alors \( \{ H_1^+,H_1^- \}=\{ H_2^+,H_2^- \}\), c'est à dire soit \( H_1^+=H_2^+\) et \( H_1^-=H_2^-\), soit le contraire.

    Les parties \( H_1^+\) et \( H_1^-\) sont les \defe{demi-plans}{demi-plan} associés à \( D\).
\end{propositionDef}
...
\begin{lemma}
    Soit une droite \( D\). Soit une application continue \( g\colon \eR^2\to \eR^2\) telle que \( g(x)=0\) si et seulement si \( x\in D\). Supposons l'existence de \( a,b\in \eR^2\) tels que \( g(a)>0\) et \( g(b)<0\).

    Alors les demi-plans de \( D\) sont les parties \( \{ x\tq g(x)>0 \}\) et \( \{ x\tes g(x)<0 \}\).
\end{lemma}
<++>
