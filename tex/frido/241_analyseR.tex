...
\begin{proposition}     \label{PROPooADJNooMyXUxG}
	...
	\begin{enumerate}
		...
		\item		\label{ITEMooOSSTooCzrUeT}
		      Soient trois droites \( d_1\), \( d_2\), \( d_3\). Si \( d_1\cap d_2\neq\emptyset\) et si \( d_3\parallel d_1\) alors \( d_3\cap d_2\neq\emptyset\).
	\end{enumerate}
	%TODOooPOKGooUTBVQN. Prouver les autres points.
\end{proposition}

\begin{proof}
	Pour \ref{ITEMooSXVYooBUvvxj}, il s'agit du lemme \ref{LEMooLIASooAvjTvt}.
	%TODOooPOKGooUTBVQN. Prouver les deux points.
\end{proof}
...
\begin{proposition}     \label{PROPooPWNWooYuyrOc}
	...
	\begin{enumerate}
		\item		\label{ITEMooFWIWooBBORjU}
		      Il existe une unique droite parallèle à \( d\) contenant \( p\).
		\item		\label{ITEMooWJDDooRiobAu}
...
\begin{proposition}	\label{PROPooOKGGooEXvWVm}
	Soient deux droites \( d_1,d_2\) dans l'espace vectoriel \( E\). Nous supposons que \( d_1\cap d_2=\{ a \}\). Soit \( x\in E\) qui n'est ni sur \( d_1\) ni sur \( d_2\). Alors il existe un unique \( m\in d_2\) tel que la droite \( d(m,x)\) soit parallèle à \( d_1\).
\end{proposition}

\begin{proof}
	La proposition \ref{PROPooPWNWooYuyrOc} dit qu'il existe une unique droite \( d_3\) passant par \( x\) et parallèle à \( d_1\). En vertu de la proposition \ref{PROPooADJNooMyXUxG}\ref{ITEMooOSSTooCzrUeT}, \( d_3\cap d_2\neq \emptyset\). Étant donné que \( x\in d_3\) mais \( x\not\in d_2\), les droites \( d_2\) et \( d_3\) ne sont pas confondues. Il existe donc un unique point d'intersection \( d_2\cap d_3=\{ m \}\). Ça c'est pour l'existence.

	En ce qui concerne l'unicité, si \( s\neq m\) est sur \( d_2\) alors la droite \( d(s,x)\) n'est pas \( d_3\) et passe par \( x\). En vertu de l'unicité dans \ref{PROPooPWNWooYuyrOc}\ref{ITEMooFWIWooBBORjU}, la droite \( d(s,x)\) n'est pas parallèle à \( d_1\).
\end{proof}


\begin{proposition}[\cite{MonCerveau}]	\label{PROPooQFWYooOxSMQI}
	Soient deux droites \( d_1,d_2\) dans \( \eR^2\) telles que \( d_1\cap d_2=\{ a \}\). Alors
	\begin{equation}
		\bigcup_{\substack{ s_1\in d_1 \\ s_2\in d_2 }  }d(s_1,s_2)=\eR^2.
	\end{equation}
\end{proposition}

\begin{proof}
	Soit \( x\in \eR^2\). Par la proposition \ref{PROPooOKGGooEXvWVm}, il existe un unique \( m\in d_1\) tel que \( d(x,m)\parallel d_2\). Nous prenons n'importe quel autre point \( s_1\in d_1\). Nous avons alors que la droite \( d(x,s_1)\) n'est pas parallèle à \( d_2\). Soit \( s_2=d(x,s_1)\cap d_2\).

	Nous avons \( x\in d(s_1,s_2)\) parce qu'il y a unicité de la droite passant par \( s_1\) et \( s_2\) (proposition \ref{PROPooRSKLooVrxrFz}\ref{ITEMooQSJMooKUBHwf}) et que cette droite est \( d(x,s_1)\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooUTTPooDiRHNH}
	Soient deux droites parallèles \( d_1\) et \( d_2\). Soient \( a\in d_1\) et \( b\in d_2\). Alors \( d_2=d_1+b-a\).
	%TODOooOFRAooDJEVuU Prouver ça.
\end{proposition}
...
\begin{proof}
	...
	Le fait que \( (ED)\) soit parallèle à \( (CB)\) implique qu'il existe \( \sigma\in \eR\) tel que \( E-D=\sigma w\) (proposition \ref{PROPooKWJLooIYVKhs}).
...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooLUYWooRPehYU}
	Deux courbes sont \defe{tangentes}{courbes tangentes} si elles ont une tangente commune en un point d'intersection.
\end{definition}


\begin{propositionDef}[cercles tangents\cite{MonCerveau}]	\label{PROPooOCXJooSDeEoB}
	Soient deux cercles non confondus \( C_i\) de centres \( c_i\) et de rayons \( r_i\) (\( i=1,2\)). Nous avons équivalence entre
	\begin{enumerate}
		\item		\label{ITEMooSBZFooIIDgvf}
		      L'intersection \( C_1\cap C_2\) est un unique point.
		\item		\label{ITEMooLZHQooWsbQZl}
		      Il existe un droite tangents commune passant par un point de \( C_1\cap C_2\).
		\item	\label{ITEMooNIZQooSeAWLm}
		      La distance \( \| c_1-c_2 \|\) est égal soit à \( r_1+r_2\) soit à \( r_1-r_2\).
		\item		\label{ITEMooZUKWooPUVpwH}
		      Il existe un vecteur \( u\) et un point d'intersection \( p\) tels que
		      \begin{equation}
			      u\cdot (c_1-p)=u\cdot(c_2-p)=0.
		      \end{equation}
		\item		\label{ITEMooXRTVooUxEHrk}
		      Les deux centres sont alignés avec un point d'intersection.
	\end{enumerate}
	Dans ce cas, nous disons que les cercles \( C_1\) et \( C_2\) sont \defe{tangents}{cercles tangents}.
\end{propositionDef}

\begin{proof}
	En plusieurs implications.
	\begin{subproof}
		\spitem[\ref{ITEMooLZHQooWsbQZl}\( \Rightarrow\)\ref{ITEMooNIZQooSeAWLm}]
		%-----------------------------------------------------------
		Nous notons \( p\) le point d'intersection où passe la tangente commune. Notons aussi \( v_i=c_i-p\). Si la droite est \( \{ p+tu \}_{t\in \eR}\), alors la tangence nous dit que \( u\cdot v_1=0\) et \( u\cdot v_2=0\).

		Nous considérons une base orthonormée de \( \eR^2\) de la forme \( \{ u,e_2 \}\). Nous décomposons
		\begin{subequations}
			\begin{align}
				v_1 & =a_1u+b_1e_2  \\
				v_2 & =a_2u+b_2e_2.
			\end{align}
		\end{subequations}
		Les conditions \( u\cdot v_i=0\) donnent \( a_i=0\) et donc \( v_i=b_ie_2\) pour tout \( i\). Donc \( | b_i |=r_i\). Petit calcul :
		\begin{subequations}
			\begin{align}
				\| c_1-c_2 \| & =\| v_1+p-v_2-p \|  \\
				              & =\| v_1-v_2 \|      \\
			...
			\end{align}
		\end{subequations}
		Nous avons supposé que \( r_1\geq r_2\). Donc la dernière valeur absolue peut valoir soit \( r_1-r_2\) soit \( r_1+r_2\) en fonction des signes de l'un ou l'autre.

		\spitem[\ref{ITEMooZUKWooPUVpwH}\( \Rightarrow\)\ref{ITEMooXRTVooUxEHrk}]
		%-----------------------------------------------------------
		Nous considérons une base orthonormée \( \{ u,v \}\) de \( \eR^2\). Nous décomposons \( c_i-p\) dans cette base :
		\begin{subequations}
			\begin{align}
				c_1-p & =a_1u+b_1v \\
				c_2-p & =a_2u+b_2v
			\end{align}
		\end{subequations}
		L'hypothèse \( u\cdot (c_i-p)=0\) donne \( c_1-p=b_1v\) et \( c_2-p=b_2v\). Donc \( c_1\), \( c_2\) et \( p\) sont sur la droite
		\begin{equation}
			\{ p+\lambda v \}.
		\end{equation}
		Avec \( \lambda=0\) on a \( p\), avec \( \lambda=b_1\) on a \( c_1\) et avec \( \lambda=b_2\) on a \( c_2\).
	\end{subproof}
	%TODOooCFDXooXbfmcF. Terminer la preuve. C'est déjà dans ma liste.
\end{proof}
...
\begin{proposition}		\label{PROPooMEXAooZJJPYx}
