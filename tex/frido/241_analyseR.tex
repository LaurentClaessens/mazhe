...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[\( \Leftarrow\)]
		%-----------------------------------------------------------
		Soit une application affine \(f \colon \eR^2\to \eR  \). Nous montrons que \( D=\{ f=0 \}\) est une droite. Il existe une application linéaire \(\alpha \colon \eR^2\to \eR  \) et \( a\in \eR\) tels que \( f(x)=\alpha(x)+a\). Posons \( D_0=\{ \alpha=0 \}\).
	\end{subproof}
\end{proof}
