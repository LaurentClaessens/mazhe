...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooTIIUooNtbQLz}
	...
	\begin{enumerate}
		\item		\label{ITEMooSTSZooKMQImS}
		      Si \( w\neq 0\in D_0\), alors \( D_0=\{ \lambda w\tq \lambda\in \eR \}\).
		\item		\label{ITEMooWNRSooLcCikX}
		      Si \( w\neq\in D_0\) et \( b\in D\), alors \( D=\{ b+\lambda w\tq \lambda\in \eR \}\).
	\end{enumerate}
\end{proposition}
...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooDLBFooYmJPhQ}
	Soient deux points \( a\) et \( b\) dans \( \eR^2\). Nous posons \( c=(a+b)/2\), et nous disons que la \defe{médiatrice}{médiatrice} du segment \( \mathopen[a,b\mathclose]\) est la partie
	\begin{equation}
		M=\{ x\in \eR^2\tq (x-c)\cdot (a-b)=0 \}.
	\end{equation}
\end{definition}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooPOKCooMoEfOB}
	La médiatrice du segment \( \mathopen[ A,B\mathclose]\) est une droite.
	%TODOooTCSSooAcjyOB
\end{proposition}
...
Si \( A'\) est l'image de \( A\) par \( \sigma_{\ell}\) alors \( \ell\) est la médiatrice du segment \( [A,A']\). Voir lemma \ref{LEMooTCIEooXdyuHu}.
