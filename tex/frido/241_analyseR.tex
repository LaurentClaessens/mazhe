...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooKBLLooRstGSU}
	Deux droites \( D\) et \( E\) sont \defe{parallèles}{droites parallèles} si il existe \( a\in V\) tel que \( E=D+a\).
\end{definition}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooZMVZooSRwCLT}
	Soient deux droites parallèles\footnote{Définition \ref{DEFooKBLLooRstGSU}.} \( D\) et \( E\). Soient \( a,b\in D\) et \( x,y\in E\). Alors il existe \( \sigma\in \eK\) tel que \( b-a=\sigma(y-x)\).
...
\begin{proof}
	...
	\end{equation}
	Vu que \( \{ u,v \}\) est une base\footnote{Parce que les points \( A\), \( B\) et \( C\) ne sont pas alignés.} de \( \eR^2\), l'unicité de la décomposition d'un vecteur dans une base ainsi que \ref{EQooMTYXooLBlpBm} donne que \( \mu=\sigma\) et \( \lambda=\sigma\). Nous avons donc déjà prouvé que
	\begin{equation}
		\mu=\sigma=\lambda.
	\end{equation}
