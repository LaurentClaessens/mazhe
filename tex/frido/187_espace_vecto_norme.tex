...
\begin{proof}
	...
	\end{equation}
	Pour traiter la valeur absolue, nous pouvons dire que la limite \( t\to 0\) existant, les limites à gauche et à droite existent et sont toutes deux égales à zéro\footnote{Cela n'est même pas un résultat en plus. C'est la proposition \ref{PROPooREUUooUNDFRi}, avec le chemin $ \gamma $ pris sur $ \mathopen[ -1,0\mathclose[$ et sur $ \mathopen] 0,1\mathclose]$ séparément.}. À partir de
	\begin{equation}
		\lim_{t\to 0^+}\frac{ f(a+tu)-f(a)-df_a(tu) }{ | t | }=0,
	\end{equation}
