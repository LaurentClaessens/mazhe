...
\begin{proof}
	...
	\begin{subequations}
		\begin{align}
			A & =\{ \pi\in S_{(k,l)}\tq \pi(1)=1 \}    \\
			B & =\{ \pi\in S_{(k,l)}\tq \pi(k+1)=1 \}.
		\end{align}
	\end{subequations}
	...
	\begin{equation}
		\sum_{\pi\in A}T(v_{\pi(1)},\ldots,v_{\pi(k)})S(v_{\pi(k+1)},\ldots,v_{\pi(k+l)})=\sum_{\pi\in A}(i_{v_1}T)(v_{\pi(2),\ldots,v_{\pi(k)}})S\big( v_{\pi(k+1)},\ldots,v_{\pi(k+l)} \big).
	\end{equation}
	C'est le moment de profiter des lemmes \ref{DEFooDFBEooFElghU} et \ref{LEMooGAMAooOAFhrc} pour changer la somme.
\end{proof}<++>
