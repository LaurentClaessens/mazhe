...
% Copyright (c) 2011-2015,2017-2020,2023-2026
%   Laurent Claessens
...
Rappel : une intégrale impropre est la définition \ref{DEFooINPOooWWObEz}.

\begin{proposition}[Critère de Cauchy de convergence uniforme pour les intégrales impropres\cite{BIBooPQGLooQqJGUH}]	\label{PROPooQDLZooYRABia}
	Soit une fonction \(f \colon \mathopen[ a,b\mathclose[\times S\to \eR  \). Nous supposons que pour tout \( \epsilon>0\) il existe \( r_0\in \mathopen[ a,b\mathclose[\) tel que si \( r_0<r_1,<r<b\),
	\begin{equation}
		\sup_{y\in S}\big| \int_{r_1}^rf(x,y)dx \big| <\epsilon.
	\end{equation}

	Alors l'intégrale impropre
	\begin{equation}
		\int_a^bf(x,y)dx=\lim_{r\to a^+}\int_r^bf(x,y)dx
	\end{equation}
	converge uniformément sur \( S\).
	%TODOooGRAIooMrpLvC. Prouver ça.
\end{proposition}
...
\begin{lemma}[Transformation d'Abel\cite{BIBooVMEDooNFFqpc,MonCerveau}]	\label{LEMooZBARooCFlMyM}
	Soient deux suites \( (u_n)\) et \( (v_n)\) dans \( \eC\). Soit \( p\leq q\) dans \( \eN\setminus\{ 0 \}\). En posant
	\begin{equation}
		V_n=\sum_{k=1}^nv_k,
	\end{equation}
	nous avons
	\begin{equation}		\label{EQooVCMBooOiwCVi}
		\sum_{n=p}^qu_nv_n=-u_pV_{p-1}+\sum_{n=p}^{q-1}(u_n-u_{n+1})V_n+u_qV_q.
	\end{equation}
\end{lemma}

\begin{proof}
	Nous fixons \( p\) et nous faisons une récurrence sur \( q\) en commençant par \( q=p\). En posant \( q=p\) dans \eqref{EQooVCMBooOiwCVi}, le membre de gauche se réduit à \( u_pv_p\) et celui de droite devient
	\begin{equation}
		-u_pV_{p-1}+0+u_pV_p=u_p(V_p-V_{p-1})=u_pv_p.
	\end{equation}

	Pour la récurrence, nous notons
	\begin{equation}
		f(q)=-u_pV_{p-1}+\sum_{n=p}^{q-1}(u_n-u_{n+1})V_n+u_qV_q
	\end{equation}
	et
	\begin{equation}
		g(q)=\sum_{n=p}^qu_nv_n.
	\end{equation}
	Nous avons déjà montré que \( f(p)=g(p)\), et maintenant nous prouvons que \( f(q+1)=g(q+1)\) sous l'hypothèse que \( f(q)=g(q)\). Pour cela nous calculons \( f(q+1)-g(q+1)\).
	\begin{equation}
		\diamondsuit = f(q+1)+g(q+1)  =-u_pV_{p-1}+\sum_{n=p}^q(u_n-n_{n+1})V_n+u_{q+1}V_{q+1}-\sum_{n=p}^{q+1}u_nv_n
	\end{equation}
	En ce qui concerne la dernière somme, en utilisant l'hypothèse de récurrence, nous avons
	\begin{equation}
		\sum_{n=p}^{q+1}u_nv_q=g(q)+u_{q+1}v_{q+1}=f(q)+u_{q+1}v_{q+1}.
	\end{equation}
	Donc
	\begin{equation}
		\diamondsuit = \underline{-u_pV_{p-1}}+\underline{\sum_{n=p}^{q-1}(u_n-u_{n+1})V_n}+(\underline{u_q}-u_{q+1})V_q+u_{q+1}V_{q+1}-f(q)-u_{q+1}v_{q+1}
	\end{equation}
	Notez que les termes soulignés forment \( f(q)\). Nous avons donc
	\begin{equation}
		\diamondsuit=f(q)-u_{q+1}V_q+u_{q+1}V_{q+1}-f(q)-u_{q+1}v_{q+1}=u_{q+1}(-V_q+V_{q+1}-v_{q+1})=0.
	\end{equation}
\end{proof}

\begin{proposition}[\cite{BIBooVMEDooNFFqpc}]	\label{PROPooZELPooQrHBuv}
	Soit \( \eK\) étant \( \eR\) ou \( \eC\). Soit une suite \( (u_n)\) dans \( \eK\) et une suite \( (v_n)\) dans un espace de Banach sur \( \eK\). Si
	\begin{enumerate}
		\item
		      \( u_n\to 0\),
		\item		\label{ITEMooZAFCooSOaOXw}
		      \( \sum_{k=0}^{\infty}| u_{k+1}-u_k |\) converge,
		\item
		      \( s_n=\sum_{k=0}^{n}v_k\) est bornée,
	\end{enumerate}
	alors la série \( \sum_{k=0}^{\infty}u_nv_n\) converge.
\end{proposition}

\begin{proof}
	Soient \( p,q\in \eN\). Par le lemme \ref{LEMooZBARooCFlMyM}, nous avons
	\begin{equation}
		\sum_{k=p}^qu_kv_k=-u_pV_{p-1}+\sum_{k=p}^{q-1}(u_k-u_{k-1})V_k+u_qV_q,
	\end{equation}
	et en prenant la norme\footnote{Et en utilisant \ref{LEMooGEVOooKeVQbb}.}, et en notant \( \alpha=\sup_{k\geq 1}| V_k |\) (qui est fini par hypothèse), nous avons
	\begin{subequations}
		\begin{align}
			\| \sum_{k=p}^qu_kv_k \| & \leq | u_p |\| V_{p-1} \|+\sum_{k=p}^{q-1}| u_k-u_{k+1} |\| V_k \|+| u_q |\| V_q \| \\
			                         & \leq\alpha\Big( | u_p |+\sum_{k=p}^{q-1}| u_n-u_{n+1} |+| u_q | \Big).
		\end{align}
	\end{subequations}
	L'hypothèse \ref{ITEMooZAFCooSOaOXw} nous permet d'utiliser le critère de Cauchy \ref{PROPooNNMFooUHMcSE} pour les séries et dire que si \( p\) est grand,
	\begin{equation}
		\| \sum_{k=p}^qu_kv_k \|\leq \alpha\Big(  | u_p |+\sum_{k=p}^{q-1}| u_n-u_{n+1} |+| u_q | \Big)<\epsilon.
	\end{equation}
	Cela signifie que la suite des sommes partielles \( S_n=\sum_{k=0}^nu_kv_k\) est de Cauchy dans l'espace complet \( E\). Donc elle converge.
\end{proof}


%-----------------------------------
\subsubsection{Critère de Dirichlet}


\begin{theorem}[Critère de Dirichlet\cite{BIBooPQGLooQqJGUH}]	\label{THOooTLUZooGeOvke}
	Soient des applications continues \(g,h \colon \mathopen[ a,b\mathclose[\times S\to \eR  \). Nous supposons que
	\begin{enumerate}
		\item
		      \( \partial_1g\) est continue sur \(\mathopen[ a,b\mathclose[\times S \).
		\item		\label{ITEMooZKXTooVocyVc}
		      Nous avons
		      \begin{equation}
			      \lim_{x\to b^-}\Big( \sup_{y\in S}| g(x,y) | \Big)=0.
		      \end{equation}
		\item		\label{ITEMooEPPPooPrbHYe}
		      Il existe \( M\in \eR\) tel que
		      \begin{equation}
			      \sup_{y\in S}\big| \int_a^xh(u,y)du \big|<M
		      \end{equation}
		      pour tout \( a\leq x<b\).
		\item		\label{ITEMooOQNNooPDdEQi}
		      L'intégrale impropre
		      \begin{equation}
			      \int_a^b|(\partial_1g)(x,y)|dx
		      \end{equation}
		      converge uniformément sur \( S\).
	\end{enumerate}
	Alors l'intégrale
	\begin{equation}
		\int_a^bg(x,y)h(x,y)dx
	\end{equation}
	converge uniformément sur \( S\).
\end{theorem}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[La fonction \( H\)]
		%-----------------------------------------------------------
		Nous posons
		\begin{equation}
			H(x,y)=\int_{a}^xh(u,y)du
		\end{equation}
		de telle sorte que \( \partial_1H=h\) par la proposition \ref{PropEZFRsMj}.

		\spitem[Intégration par parties]
		%-----------------------------------------------------------
		Ensuite nous pouvons prendre \( r_1,r\) tels que \( a\leq r<r_1<b\) et faire une intégration par partie\footnote{Proposition \ref{PROPooRLFIooQHnyJY}.} :
		\begin{subequations}		\label{SUBEQSooETSHooKfuRQv}
			\begin{align}
				\int_r^{r_1}g(x,y)h(x,y)dx & =\int_r^{r_1}g(x,y)(\partial_1H)(x,y)dx                                \\
				                           & =g(r_1,y)H(r_1,y)-g(r,y)H(r,y)-\int_r^{r_1}(\partial_xg)(x,y)H(x,y)dx.
			\end{align}
		\end{subequations}
		Soit \( \epsilon>0\).
		...
		%-----------------------------------------------------------
		Étant donnée l'hypothèse \ref{ITEMooOQNNooPDdEQi}, il existe \( s_0\) tel que si \( a\leq s_0<r<r_1<b\) on ait
		\begin{equation}
			\int_r^{r_1}| (\partial_1g)(x,y) |dx<\epsilon
		\end{equation}
		pour tout \( y\in S\).

		D'autre part, l'hypothèse \ref{ITEMooZKXTooVocyVc} nous dit que si \( s_0\) est assez grand et si \( r>s_0\),
		\begin{equation}
			\sup_{x\geq r}| g(x,y) |\leq \sup_{x\geq r}\big( \sup_{y\in S}| g(x,y) | \big)<\epsilon.
		\end{equation}

		Et l'hypothèse \ref{ITEMooEPPPooPrbHYe} donne en plus \( | H(x,y) |<M\).

		\spitem[Retour à l'intégrale par partie]
		%-----------------------------------------------------------
		Nous pouvons maintenant faire plein de majorations dans \eqref{SUBEQSooETSHooKfuRQv}.
		\begin{subequations}
			\begin{align}
				|\int_r^{r_1}g(x,y)h(x,y)dx| & \leq |g(r_1,y)H(r_1,y)|+|g(r,y)H(r,y)|+ \int_r^{r_1}|(\partial_xg)(x,y)H(x,y)|dx \\
				                             & \leq 2M\sup_{x\geq r}| g(x,y) |+M\int_r^{r_1}| (\partial_xg)(x,y) |              \\
				                             & \leq 3M\epsilon.
			\end{align}
		\end{subequations}
		\spitem[Conclusion]
		%-----------------------------------------------------------
		Le critère de Cauchy \ref{PROPooQDLZooYRABia} conclu que l'intégrale converge uniformément.
	\end{subproof}
\end{proof}

\begin{remark}
	Étant donné que la fonction sinus est bornée, il est tentant de l'utiliser comme \( g\) dans le critère de Dirichlet (théorème~\ref{THOooTLUZooGeOvke}). Hélas,
	\begin{equation}
		\int_0^T\sin(xt)dt=-\frac{ 1 }{ x }\big( \cos(xT)-\cos(x) \big),
	\end{equation}
	qui n'est pas bornée en \( x\). Poser \( g(x,t)=\sin(xt)\) \emph{ne fonctionne pas} pour assurer la convergence uniforme sur un intervalle qui contient des \( x\) arbitrairement proches de \( 0\). Le critère d'Abel avec \( \varphi(x,t)=\sin(xt)\) ne permet que de conclure à l'uniforme convergence \emph{sur tout compact} ne contenant pas \( 0\). C'est toutefois souvent suffisant pour étudier la continuité ou la dérivabilité en se servant coup du compact. Voir \ref{NORMooZWECooHvRgBw}.
	%1403337912512
\end{remark}
...
\begin{proposition}[\cite{BIBooQUMGooOxmczN}]     \label{PROPooIEUTooAfHqaC}
	Soient des intervalles compacts\footnote{Les intervalles compacts sont de la forme \( \mathopen[ a,b\mathclose]\). Ce sont les seuls fermés et bornés de la liste de la proposition \ref{PROPooJJRZooACUmWN}. Voir aussi Borel-Lebesgue \ref{ThoXTEooxFmdI}.} \( I\) et \( J\) de \( \eR\). Soient des fonctions continues \(f \colon I\times J\to \eR  \) et \(a,b \colon I\to J  \).
	\begin{enumerate}
		\item
		      L'application
		      \begin{equation}
			      \begin{aligned}
				      \phi\colon I\times J\times J & \to \eR                  \\
				      (x,u,v)                      & \mapsto \int_u^vf(x,t)dt
			      \end{aligned}
		      \end{equation}
...
\begin{proof}
	Pour chaque \( x\in I\) nous notons
	\begin{equation}
		\begin{aligned}
			f_x\colon J & \to \eR         \\
			t           & \mapsto f(x,t).
		\end{aligned}
	\end{equation}
	La fonction \( f_x\) est continue\footnote{Par le lemme \ref{LEMooRTWNooMZTWgS}.} En notons \( u=a(x)\) et \( v=b(x)\), l'intégrale \( F(x)=\int_u^vf_x(t)dt\) est l'intégrale d'une application continue sur un compact. Pas de problèmes pour l'existence.

	Nous passons à la continuité de \( \phi\). Soient \( (x_0,u_0,v_0)\in I\times J\times J\) et \( \epsilon>0\). Nous devons évaluer
	\begin{equation}		\label{EQooUCSDooZMlIct}
		\phi(x,u,v)-\phi(x_0,u_0,v_0).
	\end{equation}
	En utilisant l'additivité des domaines de l'intégrale (proposition \ref{PropOPSCooVpzaBt}), nous avons \( \phi(x,a,b)=\phi(x,a,h)+\phi(x,h,b)\). Le truc est maintenant d'utiliser cette relation pour décomposer \eqref{EQooUCSDooZMlIct} de telle sorte à n'avoir que des bouts facilement majorables :
	\begin{subequations}		\label{EQooXKAMooJQqhYZ}
		\begin{align}
			\phi(x,u,v) & =\phi(x,u,u_0)+\phi(x,u_0,v)                  \\
			            & =\phi(x,u,u_0)+\phi(x,u_0,v_0)+\phi(x,v_0,v).
		\end{align}
	\end{subequations}
	Vu que \( f\) est continue sur le compact\footnote{En tant que produit de compacts de \( \eR\) par le lemme \ref{LemCKBooXkwkte}.} \( I\times J\), elle y est uniformément continue (théorème de Heine \ref{PROPooBWUFooYhMlDp}) et bornée (théorème \ref{ThoMKKooAbHaro}) :
	\begin{enumerate}
		\item
		      Il existe \( \eta>0\) tel que si \( | x-x_0 |<\eta\), alors \( | f(x,t)-f(x_0,t) |<\epsilon\).
		\item
		      il existe \( M>0\) tel que \( | f(x,t) |<M\) pour tout \( (x,t)\in I\times J\).
	\end{enumerate}
	Avec ça, si \( | x-x_0 |<\eta\) on a :
	\begin{equation}
		\begin{aligned}[]
			| \phi(x,u,u_0) | & \leq M| u-u_0 |, \\
			| \phi(x,v_0,v) | & \leq M| v-v_0 |,
		\end{aligned}
	\end{equation}
	ainsi que
	\begin{subequations}
		\begin{align}
			| \phi(x,u_0,v_0)-\phi(x_0,u_0,v_0) | & \leq \int_{u_0}^{v_0}| f(x,y)-f(x_0,t) |dt \\
			                                      & \leq | v_0-u_0 |\epsilon.
		\end{align}
	\end{subequations}
	En prenant de plus \( | u-u_0 |<\epsilon\) et \( | v-v_0 |<\epsilon\) nous majorons \eqref{EQooXKAMooJQqhYZ} :
	\begin{equation}
		| \phi(x,u,v)-\phi(x_0,u_0,v_0) |\leq \big( 2M+| u_0-v_0 | \big)\epsilon.
	\end{equation}
	Donc \( \phi\) est continue.

	L'application \( F(x)=^p\big( x,a(x),b(x) \big)\) est donc continue en tant que composée d'applications continues.
\end{proof}

\begin{theorem}[\cite{BIBooQUMGooOxmczN}]	  \label{THOooZPUYooQAlUTa}
	Soient des intervalles compacts \( I,J\) dans \( \eR\). Soient des applications \(f \colon I\times J\to \eR  \) et \(a,b \colon I\to J  \) telles que
	\begin{enumerate}
		\item
		      Les applications \( a\) et \( b\) sont de classe \( C^1\).
		\item
		      L'application \( f\) est continue.
		\item
		      L'application \( \partial_1f \) est continue sur \( I\times J\).
	\end{enumerate}
	Nous posons :
	\begin{equation}
		\begin{aligned}
	...
	\begin{enumerate}
		...
		\item
		      Nous avons la formule
		      \begin{equation}
			      F'(x)=b'(x)f\big( x,b(x) \big)-a'(x)f\big( x,a(x) \big)+\int_{a(x)}^{b(x)}(\partial_1f)(x,t)dt.
		      \end{equation}
	\end{enumerate}
\end{theorem}

\begin{proof}
	Le fait que \( F\) existe et soit continue est la proposition \ref{PROPooIEUTooAfHqaC}. Nous reprenons la fonction
	\begin{equation}
		\begin{aligned}
			\phi\colon I\times J\times J & \to \eR                   \\
			(x,u,v)                      & \mapsto \int_u^vf(x,t)dt.
	...
	Fixons \( x_0\in I\) et \( u_0\in J\). Nous considérons l'application
	\begin{equation}
		\begin{aligned}
			\varphi\colon J & \to \eR                                                                  \\
			v               & \mapsto \phi(x_0,u_0,v)=\int_{u_0}^vf(x_0,t)dt=\int_{u_0}^vf_{x_0}(t)dt.
		\end{aligned}
	\end{equation}
	Nous avons \( \varphi'(v)=f_{x_0}(v)=f(x_0,v)\) (primitive et intégrale, proposition \ref{PropEZFRsMj}). Nous écrivons cela sous la forme
	\begin{equation}
		(\partial_v\phi)(x,u,v)=f(x,v).
	\end{equation}
	De même nous avons
	\begin{equation}
		(\partial_u\phi)(x,u,v)=-f(x,u).
	\end{equation}

	Nous fixons \( u\) et \( v\) et nous posons
	\begin{equation}
		S(x)=\int_u^vf(x,t)dt,
	\end{equation}
	...
	\begin{enumerate}
		\item
		      Vu que \( f\) est continue, pour chaque \( f\) l'application \( t\mapsto f(x,t)\) est \( L^1\big( \mathopen[ u,v\mathclose] \big)\).
		\item
		      La dérivabilité de \( x\mapsto f(x,t)\) est dans l'hypothèse.
		\item
		      Par hypothèse \( \partial_1f\) est continue, de ses deux variables; elle est donc bornée par \( M\) sur le compact \( I\times J\). Donc poser \( G(t)=M\) fait l'affaire.
	\end{enumerate}
	Nous avons donc
	\begin{equation}
		S'(x)=\int_u^v(\partial_xf)(x,t)dt.
	\end{equation}
	Et nous notons ça sous la forme
	\begin{equation}
		(\partial_x\phi)(x,u,v)=\int_u^v(\partial_xf)(x,t)dt.
	\end{equation}

	Et maintenant nous pouvons passer à la dérivée de \( F\). Nous utilisons la proposition \ref{PROPooCTXUooTGJbIK} pour écrire
	\begin{equation}
		\begin{aligned}[]
			F'(x) & =(\partial_x\phi)\big( x,a(x),b(x) \big)       \\
			      & +a'(x)(\partial_u\phi)\big( x,a(x),b(x) \big)  \\
			      & +b'(x)(\partial_v\phi)\big( x,a(x),b(x) \big),
		\end{aligned}
	\end{equation}
	qui est continue en tant que composée de fonctions continues.
\end{proof}
...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Théorème de Cauchy-Lipschitz réel}
%---------------------------------------------------------------------------------------------------------------------------
...
Si vous voulez un théorème de Cauchy-Lipschitz avec paramètre vraiment intéressant, allez voir le théorème~\ref{PROPooPYHWooIZhQST}.
...
%-------------------------------------------------------
\subsection{Théorème de Caucy-Lipschitz analytique (tentative)}
%----------------------------------------------------

\begin{probleme}
	La démonstration ici n'est pas terminée; il manque un détail technique vers la fin. Si vous savez comment terminer, dites-le moi.

	\url{https://math.stackexchange.com/questions/5101596}\\
	\url{https://math.stackexchange.com/questions/5111835}\\
	\url{https://math.stackexchange.com/questions/5113042}
\end{probleme}

\begin{theorem}[Cauchy-Lipschitz analytique TENTATIVE \cite{BIBChatGPT}]		\label{THOooZEBOooQJOSQj}
	% NE PAS CITER CE THÉORÈME PARCE QUE SA DÉMONSTRATION N'EST PAS TERMINÉE.
	% Le label est juste ici parce que je le cite dans mes questions sur math.stackexchange

	Nous considérons\quext{Je suis presque certain qu'on peut prendre n'importe que espace de Banach pour \( \Lambda\), mais ça demanderait d'adapter la proposition \ref{PROPooBHKJooUZUKnc}\ref{ITEMooMWMSooRoXLgL}.} \( \Lambda=\eR^m\) ainsi qu'un ouvert \( U\) de \( \eR^n\), un ouvert \( \Gamma\) de \( \Lambda\) et une application \(f \colon \eR\times U\times \Gamma\to \eR  \) tels que
	\begin{enumerate}
		\item
		      L'application \( f\) est Lipschitzienne en \( y\) : pour tout compact \( K\) de \( \eR\times U\times \Gamma\), il existe une constante \( L_K\) telle que
		      \begin{equation}
			      \| f(y,y_1,\lambda)-f(t,y_2,\lambda) \|\leq L_K\| y_1-y_2 \|
		      \end{equation}
		      pour tout \( (t,y_1,\lambda),(t,y_2,\lambda)\) dans \( K\).
		\item
		      L'application \( f\) est analytique\footnote{Définition \ref{DEFooAIHMooKbWsBt}.} sur \( \overline{\eR\times U\times \Gamma}\).
	\end{enumerate}
	Alors pour tout \( (t_0,y_0,\lambda_0)\in \eR\times U\times \Gamma\), il existe un \( R>0\) et une unique application
	\begin{equation}
		y \colon   B(t_0,R)\times B(\lambda_0,R) \to \eR^n
	\end{equation}
	telle que
	\begin{subequations}		\label{EQooPTYDooMOxFti}
		\begin{numcases}{}
			(\partial_ty)(t,\lambda)=f\big( t,y(t,\lambda),\lambda \big)\\
			y(t_0,\lambda)=y_0.
		\end{numcases}
	\end{subequations}
	pour tout \( (t,\lambda)\in B(t_0,\epsilon)\times B(\lambda_0,R)\).

	Cette application \( y\) est analytique sur \( B(t_0,\epsilon)\times B(\lambda_0,R)\).
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Cylindre de sécurité]
		%-----------------------------------------------------------
		Soit \( (t_0,y_,\lambda_0)\). Il existe \( \rho>0\) tel que \( f\) est analytique sur le compact \( \overline{B(t_0,\rho)}\times \overline{B(y_0,\rho)}\times \overline{B(\lambda_0,\rho)}\). En particulier \( f\) y est continue et donc bornée. Soit \( M>0\) tel que
		\begin{equation}
			\| f(t,x,\lambda) \|<M
		\end{equation}
		pour tout \( (t,x,\lambda)\in \overline{B(t_0,\rho)}\times \overline{B(y_0,\rho)}\times \overline{B(\lambda_0,\rho)}\).

		\spitem[L'espace \( \mF\)]
		%-----------------------------------------------------------
		Nous fixons un \( r<\rho/2M\) et nous posons
		\begin{equation}
			X=\overline{B(t_0,r)}\times \overline{B(\lambda_0,\rho)}
		\end{equation}
		et \( \mF\) est
		\begin{equation}
			\mF=\Big(   \{ y\in C^0(X,\eR^n)\tq \| y \|_{\infty}<\rho \},\| . \|_{\infty} \Big)
		\end{equation}
		où \( \| . \|_{\infty}\) est la norme supremum sur \( X\).

		\spitem[\( \mF\) est de Banach]
		%-----------------------------------------------------------
		Le lemme \ref{LemdLKKnd} dit ça.

		\spitem[Une borne]
		%-----------------------------------------------------------
		Si \( y\in\mF\), alors et si \( s\in B(t_0,r)\) et \( \lambda\in B(\lambda_0,\rho)\) alors d'abord \( \| y(s,\lambda) \|<\rho\) parce que \( y\in\mF\), et donc \( \big( s,y(s,\lambda),\lambda \big)\in B\big( (t_0,y_0,\lambda_0),\rho \big)\). Avec ça nous avons
		\begin{equation}		\label{EQooYPDTooLRKJrX}
			\| f\big( s,y(s,\lambda),\lambda \big) \|<M.
		\end{equation}
		Donc en prenant \( t\in B(t_0,r)\) nous avons
		\begin{subequations}
			\begin{align}
				\| \int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big) \|ds & \leq\int_{t_0}^t\| f\big( s,y(s,\lambda),\lambda \big) \| \\
				                                                        & \leq | t-t_0 |M                                           \\
				                                                        & \leq rM                                                   \\
				                                                        & \leq \rho/2                                               \\
				                                                        & <\rho.
		...
		\spitem[Une continuité]
		%-----------------------------------------------------------
		Soit \( y\in \mF\). Nous posons
		\begin{equation}
			\begin{aligned}
				g\colon X   & \to \eR^n                                                  \\
				(t,\lambda) & \mapsto \int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big)ds.
			\end{aligned}
		\end{equation}
		Nous avons déjà prouvé dans \eqref{EQooYPDTooLRKJrX} que l'intégrale ne posait pas de problèmes. Nous prouvons maintenant que \( g\) est continue. Soient donc \( (t_1,\lambda_1)\in X\) et \( d>0\) ainsi que\footnote{Oui, nous utilisons la topologie induite parce que \( X\) est fermé.}
		\begin{equation}
			(t,\lambda)\in B\big( (t_1,\lambda_1),d \big)\cap X.
		\end{equation}
		En particulier \( | t-t_1 |<d\) et \( | \lambda-\lambda_1 |<d\). Et nous calculons
		\begin{subequations}		\label{SUBEQooOBONooAnUwAO}
			\begin{align}
				g(t_1,\lambda_1)-g(t,\lambda) & =\int_{t_0}^{t_1}f\big( s,y(s,\lambda_1),\lambda_1 \big)ds-\int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big)ds                                                  \\
				                              & =\int_{t_0}^tf\big( s,y(s,\lambda_1),\lambda_1 \big)+\int_t^{t_1}f\big( s,y(s,\lambda_1),\lambda_1 \big)-\int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big)ds    \\
				                              & =\int_{t_0}^t\big[    f\big( s,y(s,\lambda_1),\lambda_1 \big) -f\big( s,y(s,\lambda),\lambda \big) \big]ds-\int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big)ds.
			\end{align}
		\end{subequations}
		En ce qui concerne les normes de tout ça, vu que \( y\) est continue sur le compact \( \mathopen[ t_0,t\mathclose]\) si \( d\) est petit, \( y(s,\lambda_1)\) et \( y(s,\lambda)\) peuvent être rendus aussi proche qu'on veut (uniformément en \( s\) par \ref{PROPooBWUFooYhMlDp}). Puis comme \( f\) est continue, on a
		\begin{equation}
			\| \int_{t_0}^t\big[    f\big( s,y(s,\lambda_1),\lambda_1 \big) -f\big( s,y(s,\lambda),\lambda \big) \big]ds-\int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big)ds \|\leq | t-t_0 | \epsilon.
		\end{equation}
		Pour l'autre terme en prenant \( d\) assez petit pour \( d<\epsilon/M\),
		\begin{subequations}
			\begin{align}
				\| \int_{t_0}^tf\big( s,y(s,\lambda),\lambda \big) \|\leq | t-t_0 |M\leq dM\leq \epsilon.
			\end{align}
		\end{subequations}
		Tout cela pour dire que si \( d\) est petit, alors
		\begin{equation}
			\| g(t_1,\lambda_1)-g(t,\lambda) \|\leq \epsilon (| t-t_0 |+1).
		\end{equation}
		Bref, \( g\) est continue.

		\spitem[L'application \( \Phi\)]
		%-----------------------------------------------------------
		Tout cela nous permet de considérer l'application
		\begin{equation}
			\begin{aligned}
				\Phi\colon \mF     & \to \mF                                                 \\
				\Phi(y)(t,\lambda) & = y_0+ \int_{t_0}^tf\big(s, y(s,\lambda),\lambda \big).
			\end{aligned}
		\end{equation}

		\spitem[\( \Phi\) est une contraction]
		%-----------------------------------------------------------
		Contraction : relisez la définition \ref{DEFooRSLCooAsWisu}. Vu que \( \overline{B(t_0,\rho)}\times \overline{B(y_0,\rho)}\times \overline{B(\lambda_0,\rho)}\) est compact, il existe un \( L(\rho)>0\) tel que \begin{equation}
			\| f\big( s,y_1(s,\lambda),\lambda \big)-f\big( s,y_2(s,\lambda),\lambda \big) \|<L\| y_1-y_2 \|.
		\end{equation}
		Et notez au passage que que \( L(\rho)\) peut être choisit décroissant en \( \rho\). Calculons un petit peu :
		\begin{subequations}
			\begin{align}
				\| \Phi(y_1)-\Phi(y_2) \|_{\infty} & =\sup_{(t,\lambda)\in X}\| \Phi(y_1)(t,\lambda)-\Phi(y_2)(t,\lambda) \|                                                             \\
				                                   & \leq \sup_{(t,\lambda)\in X}\int_{t_0}^t\| f\big(s,y_1(s,\lambda),\lambda\big)- f\big( s,y_2(s,\lambda),\lambda \big) \|_{\infty}ds \\
				                                   & \leq\sup_{(t,\lambda)\in X}\int_{t_0}^tL\| y_1(s,\lambda)-y_2(s,\lambda) \|ds                                                       \\
				                                   & \leq\sup_{(t,\lambda)\in X}| t-t_0 |L\| y_1-y_2 \|_{\infty}                                                                         \\
				                                   & \leq\rho L(\rho)\| y_1-y_2 \|_{\infty}.
			\end{align}
		\end{subequations}
		Il suffit de choisir \( \rho\) assez petit pour avoir \( \rho L(\rho)<1\) pour que \( \Phi\) soit une contraction.

		\spitem[Picard]
		%-----------------------------------------------------------
		Le théorème de Picard \ref{ThoEPVkCL} nous dit que sur \( \Phi\) a un unique point fixe et que si \( \alpha_0\in \mF\), alors en posant \(\alpha_{n+1j=\Phi(\alpha_n)}\), alors la suite \( (\alpha_n)_{n\in \eN}\) converge vers ce point fixe que nous allons noter \( \alpha\).

		\spitem[Tout point fixe est solution]
		%-----------------------------------------------------------
		Nous prouvons maintenant que le point fixe de \( \mF\) est une solution du problème. Si \( \alpha\) est le point fixe, alors \( \Phi(\alpha)-\alpha=0\), c'est-à-dire
		\begin{equation}
			y_0+\int_{t_0}^tf\big( s,\alpha(s,\lambda),\lambda \big)ds-\alpha(t,\lambda)=0.
		\end{equation}
		En évaluant en \( t=t_0\) nous avons déjà \( \alpha(t_0,\lambda)=y_0\) comme il fallait. De plus, le théorème «primitive et intégrale» \ref{PropEZFRsMj} nous indique qu'en posant
		\begin{equation}
			g_{\lambda}(t)=f\big( t,\alpha(t,\lambda),\lambda \big)
		\end{equation}
		et
		\begin{equation}
			F_{\lambda}(t)=\int_{t_0}^tf\big( f(s,\alpha(s,\lambda),\lambda) \big)ds,
		\end{equation}
		la fonction \( F_{\lambda}\) est primitive de \( g_{\lambda}\). Nous avons donc
		\begin{equation}
			(\partial_t\alpha)(t,\lambda)=\partial_t\big( \Phi(\alpha) \big)(t,\lambda)=F'_{\lambda}(t)=g_{\lambda}(t)=f\big( t,\alpha(t,\lambda),\lambda \big).
		\end{equation}
		Donc \( \alpha\) vérifie l'équation \ref{EQooPTYDooMOxFti}.

		\spitem[Toute solution est un point fixe]
		%-----------------------------------------------------------
		Supposons que \( \alpha\) soit une solution. Nous démontrons que c'est un point fixe de \( \Phi\) :
		\begin{subequations}
			\begin{align}
				\Phi(\alpha)(t,\lambda) & =y_0+\int_{t_0}^tf\big(  s,\alpha(s,\lambda),\lambda\big)ds \\
				                        & =y_0+\int_{t_0}^t(\partial_t\alpha)(s,\lambda)ds            \\
				                        & =y_0+\alpha(t,\lambda)-\alpha(t_0,\lambda)                  \\
				                        & =\alpha(t,\lambda).
			\end{align}
		\end{subequations}
		Donc nous avons bien \( \Phi(\alpha)=\alpha\).

		\spitem[Pour la suite]
		%-----------------------------------------------------------
		Nous savons que l'équation \eqref{EQooPTYDooMOxFti} possède une unique solution et que cette solution est le point fixe de \( \mF\). De plus nous savons que si \( \alpha_0\) est n'importe quel élément de \( \mF\), alors en posant \( \alpha_{n+1}=\Phi(\alpha_n)\), la suite \( (\alpha_n)_{n\in \eN}\) est convergente dans \( \mF\) vers un élément \( \alpha\) qui est solution de \eqref{EQooPTYDooMOxFti}.

		\spitem[La suite \( (\alpha_n)_{n\in \eN}\)]
		%-----------------------------------------------------------
		Choisissons
		\begin{equation}
			\begin{aligned}
				\alpha_0\colon B(t_0,\rho)\times B(\lambda_0,\rho) & \to \eR    \\
				(t,\lambda)                                        & \mapsto 0.
			\end{aligned}
		\end{equation}
		C'est une application analytique. Et posons \( \alpha_{n+1}=\Phi(\alpha_n)\). Vu que \( \mF\) a la norme uniforme, la convergence \( \alpha_n\to \alpha\) est uniforme. De plus la proposition \ref{PROPooBHKJooUZUKnc} dit que \( \alpha_n\) est analytique.

		\begin{probleme}
			Je voudrais utiliser le théorème \ref{THOooZHXMooICpUZx}, mais je ne sais pas comment vérifier la condition \ref{ITEMooISLSooIfUEXH}.
		\end{probleme}
		%TODOooYKUNooPLoayv. Résoudre ce problème.
	\end{subproof}
\end{proof}
