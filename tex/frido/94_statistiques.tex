...
% Copyright (c) 2011-2015,2018-2019, 2022, 2023
%   Laurent Claessens
...
Un \defe{modèle statistique}{modèle statistique} est un triplet
\begin{equation}
...
\begin{definition}
	...
	\end{equation}
	un modèle statistique. Un \defe{modèle d'échantillonnage}{modèle!échantillonnage} de taille \( n\) associé au modèle statistique \( \statS\) est la donnée d'une famille de \( n\)-échantillons \( (X_{\theta,1},\ldots,X_{\theta,n})\) telle que pour tout \( \theta\in\Theta\), l'échantillon \( (X_{\theta,i})\) soit de variable parente \( X_{\theta}\).
\end{definition}
...
\begin{definition}
	Un estimateur est \defe{convergeant}{estimateur convergeant} ou \defe{consistant}{consistance!estimateur}\index{estimateur!consistant} si pour tout \( \theta\in\Theta\), la suite de variables aléatoires \( \hat\theta_n(X_1,\ldots,X_n)\) converge en probabilité vers \( \theta\).
\end{definition}
...
\end{equation}
Typiquement, \( \alpha=5\%\). Le nombre \( \alpha\) est le \defe{niveau de confiance}{niveau de confiance} que nous nous fixons à priori.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
On appelle \defe{région de confiance exacte}{région!de confiance exacte} au niveau de confiance \( 1-\alpha\) une région aléatoire \( \Lambda(x_1,\ldots,x_n)\) telle que
\begin{equation}
...
\begin{example}
	...
	\input{auto/pictures_tex/Fig_ChiSquaresQuantile.pstricks}
...
\begin{theorem}[Théorème de Pearson]\index{théorème!Pearson}\index{Pearson!théorème}
	Nous avons
	\begin{equation}
