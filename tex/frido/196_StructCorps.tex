...
\begin{lemma}[\cite{BIBooYYZPooMgFzkp}]
    ...
    Alors il existe une application \( \sigma\colon \eL\to \eF\) telle que \( \sigma\circ \alpha=\iota\).
\end{lemma}

\begin{proof}
    Vu que \( \eL\) est monogène, si \( \mu_a\in \eK[X]\) est le polynôme minimal de \( a\in \eK\), alors \( \eL\simeq \eL[X]/(\mu_a)\).
\end{proof}
<++>
