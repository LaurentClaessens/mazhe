...
\begin{proposition}[\cite{MonCerveau}]     \label{PROPooYSFNooFGbbCi}
	...
	\begin{enumerate}
		...
		\item       \label{ITEMooATPTooVXKdlK}
		      \( \eK(\alpha)_{\eL}=\{ P(\alpha)_{\eL}/Q(\alpha)_{\eL} \tq P,Q\in \eK[X], Q(\alpha)\neq 0\) \}.
	\end{enumerate}
	Le point \ref{ITEMooATPTooVXKdlK} pourrait être écrit \( \eK(\alpha)_{\eL}=\{ R(\alpha)_{\eL}\tq R\in \eK(X) \}\), avec sous-entendu que seuls les \( R\in \eK(X)\)  tels que \( R(\alpha)\) est bien défini sont pris en compte\footnote{Merci à jeanas pour m'avoir fait remarquer le danger\cite{BIBooPEDGooCQRqcV}.}.
\end{proposition}
...
\begin{definition}[Corps de rupture]      \label{DEFooVALTooDJJmJv}
...
\begin{normaltext}		\label{NORMooIQIRooEoDjwm}
	Un polynôme sur \( \eQ\) peut avoir de nombreuses racines sur \( \eC\). Par exemple \( X^4-5X^2+6\) a pour racines \( \pm\sqrt{ 2 }\) et \( \pm\sqrt{3}\). Il existe donc à priori quatre corps de rupture différents : \( \eQ[\sqrt{ 2 }]\), \( \eQ[-\sqrt{ 2 }]\), \( \eQ[\sqrt{ 3 }]\) et \( \eQ[-\sqrt{ 3 }]\). Ensuite vous pouvez réfléchir pour savoir si l'un ou l'autre de ces quatre sont identiques ou isomorphes; ce n'est pas la question ici.

	La proposition suivante met un tout petit peu d'ordre dans les corps de ruptures en disant que le corps \( \eK[X]/(P)\) est quand même plus pertinent que les autres.
\end{normaltext}
...
\begin{lemma}[\cite{MonCerveau}]        \label{LEMooQEJHooAmSNxU}
	Soient un corps \( \eK\), une extension \( \eL\) et des éléments \( \alpha_1,\ldots, \alpha_n\) dans \( \eL\). Alors\footnote{Dans l'expression \eqref{EQooDYLXooYtZtwC}, seuls les \( r\) tels que \( r(\alpha_1,\ldots,\alpha_n)\) est bien défini sont pris en compte. Il faut exclure les \( r\) sont le dénominateur s'annule en \( (\alpha_1,\ldots,\alpha_n)\). Voir \cite{BIBooPEDGooCQRqcV}.}
	\begin{equation}		\label{EQooDYLXooYtZtwC}
