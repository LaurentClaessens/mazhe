...
\begin{normaltext}      \label{NORMALooTPOIooVZAfUo}
    Nous insistons sur le fait que nous ne définissons le concept de corps de rupture pour un polynôme irréductible à coefficients dans un corps. D'abord si \( P\) n'est pas irréductible, la question de chercher un corps de rupture n'a pas beaucoup de sens.

    Ensuite, si \( P\in A[X]\) où \( A\) est un anneau, nous devons trouver un corps \( \eK\) qui contient \( A\) comme sous-anneau. Ou tout au moins nous voudrions que \( A[X]/(P)\) soit un corps. Si $A$ n'est pas intègre, c'est directement raté.

    Mais soyons honnête : nous ne savons pas si il existe un critère clair disant pour quel classe d'anneaux on peut construire un corps de décomposition.
\end{normaltext}
