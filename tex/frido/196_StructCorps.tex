...
\begin{normaltext}
    Nous insistons sur le fait que nous ne définissons le concept de corps de rupture que pour un polynôme irréductible. Il y a plusieurs raisons très liées entre elles :
    \begin{enumerate}
        \item
            Si \( P\) n'est pas irréductible, il n'y a pas garantie d'existence. Par exemple \(P= (X+2)(X+3)\) dans \( \eQ[X]\) n'est pas irréductible, et il n'existe pas d'extensions de \( \eQ\) dans lequel ait une autre racine que \( 2\) et \( 2\); sinon il y aurait une factorisation de \( P\) en au moins trois facteurs, ce qui violerait le degré.
        \item
            Si \( P\) n'est pas irréductible, le quotient \( \eK[X]/(P)\) n'est pas un corps (voir plus bas).
    \end{enumerate}
\end{normaltext}

\begin{normaltext}
    Nous de définissons pas ce qu'est un corps de rupture pour un polynôme sur un anneau qui n'est pas un corps. En effet pour qu'un corps \( \eK\) puisse dignement être nommé «corps de rupture de \( P\in A[X]\)», il faut au moins que \( \eK\) soit un corps contenant \( A\) comme sous-anneau. 

    Notons quelque éléments à ce sujet :
    \begin{enumerate}
        \item
            Si \( A\) est un anneau qui n'est pas intègre, alors il n'existe aucun corps contenant \( A\) comme sous-module.
        \item
            Si \( A\) est intègre, et si \( P\in A[X]\) est irréductible, le quotient \( A[X]/(P)\) n'est pas un corps en général.
        \item
            Nous pourrions donc à la limite définir un corps de rupture pour un anneau commutatif et intègre en passant immédiatement au corps des fractions. Mais quitte à ne parler de de polynômes à coefficients dans un anneau contenu dans un corps autant directement ne parler que de polynômes à coefficients dans ledit corps.
    \end{enumerate}
    <++>
