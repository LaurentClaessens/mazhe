...
\end{definition}
    ...
    Nous identifions le plus souvent \( \eK\) avec \( i(\eK)\subset \eL\), mais il faut savoir que le corps \( \eL\) étendant \( \eK\) n'est pas toujours un sur-corps de \( \eK\). En particulier, l'ensemble \( \eL\) peut ne pas être un extension de l'ensemble \( \eK\).
...
\begin{definition}[Extension algébrique, clôture algébrique]      \label{DEFooREUHooLVwRuw}
    ...
    \begin{enumerate}
        ...
        \item       \label{ITEMooEIWVooVjJRoR}
            L'extension \( \eL\) est \defe{algébriquement close}{extension algébriquement clos} si tout \( \eL\) est algébriquement clos (définition \ref{DEFooYZOYooAesmnP}).
        \item
...
\begin{proof}
    ...
    Soit un corps algébriquement clos \( \eK\). Nous allons par récurrence sur le degré des polynômes. Si \( P\) est un polynôme de degré \( 1\), alors il est scindé.
...
\begin{proof}
    ...
    \end{equation}
    Un corps est toujours un anneau intègre (lemme \ref{LemAnnCorpsnonInterdivzer}), c'est-à-dire que la règle du produit nul est utilisable. Dans notre cas, le produit nul \eqref{EQooXHSCooVdMiva} nous indique que \( \alpha-a_i=0\) pour (au moins) un des \( i\). Donc effectivement \( \alpha\) est l'un des \( a_i\).
\end{proof}
...
\begin{proof}
    Nous allons montrer que tous les éléments de \( \eL\) sont dans l'image de \( \alpha\). Soit donc \( l\in \eL\). Vu que l'extension \( \alpha\colon \eK\to \eL\) est une extension algébrique, il existe un polynôme \( P\in \alpha(\eK)[X]\) tel que \( P(l)=0\).
...
\begin{proof}
    ...
    Pour prouver que \( \eK(\alpha)_\eL\) est bien une extension, nous devons trouver un homomorphisme de corps \( j\colon \eK\to \eK(\alpha)_{\eL}\). Il se fait que prendre \( j=j_{\eL}\) fonctionne parce que par définition, \( \eK(\alpha)_{\eL}\) est une partie de \( \eL\) contenant l'image de \( j_{\eL}\).
\end{proof}
...
\begin{proof}
    ...
    \begin{equation}
        \sqrt{ n }=\frac{ x-a }{ b-y }.
    \end{equation}
    Vu que \( \sqrt{ n }\) n'est pas un rationnel, une telle écriture est impossible. Donc \( b=y\). Nous avons alors \( x+y\sqrt{ n }=a+y\sqrt{ n }\) et du coup aussi \( x=a\).
\end{proof}
...
\item
    ...
    \end{enumerate}
    Le plus souvent, l'indice \( \eL\) dans \( \eK(A)_{\eL}\) et \( \eK[A]_{\eL}\) est omis parce que le contexte est clair\quext{Et je me demande si il est possible de trouver un cas tordu où \( \eK(A)_{\eL}\neq \eK(A)_\eM\). Par exemple lorsque \( A\) est dans \( \eL\) et \( \eM\), mais que \( \eL\) n'est pas inclus dans \( \eM\), ni \( \eM\) dans \( \eL\).}, et nous avons même très souvent \( \eK\subset \eL\) en tant que ensembles. Dans ce cas, l'application \( j\) est l'identité et elle sera omise.
...
\item[Conclusion]
    ...
    Nous sommes donc dans la situation où \( \sigma\colon \eF_1\to \eF_2\) est une extension de corps où \( \eF_1\) est algébriquement clos et \( \eF_2\) est algébrique. Le lemme \ref{LEMooYVHKooWhewKp} conclu que \( \sigma(\eF_1)=\eF_2\), c'est-à-dire que \( \sigma\) est surjectif. En tant que morphisme de corps, \( \sigma\) était déjà injective; elle est donc bijective.
