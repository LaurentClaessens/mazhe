...
\begin{definition}  \label{DefZCYIbve}
    Soit \( \eL\) une extension\footnote{Définition \ref{DEFooFLJJooGJYDOe}.} de \( \eK\) et \( A\subset \eL\).
    \begin{enumerate}
        \item
...
\begin{lemma}[\cite{BIBooYYZPooMgFzkp}]
    Soient un corps \( \eK\) et une extension \( \alpha\colon \eK\to \eL\) engendrée par un élément algébrique \( a\in \eK\). Soit une extension algébriquement close \( \iota\colon \eK\to \eF\). 

    Alors il existe un morphisme de corps \( \sigma\colon \eL\to \eF\) tel que \( \sigma\circ \alpha=\iota\).
\end{lemma}

\begin{proof}
    Vu que \( \eL\) est monogène, si \( \mu_a\in \eK[X]\) est le polynôme minimal de \( a\in \eK\), alors les points \ref{ItemJCMooDgEHaji} et \ref{ItemJCMooDgEHajii} de la proposition \ref{PropURZooVtwNXE} disent que \( \eL\simeq \eK[a]\simeq \eK[X]/(\mu_a)\).

    Les coefficients de \( \mu_a\) sont dans \( \eK\), donc nous pouvons voir \( \mu_a\in \eL[X]\). Comme \( \eL\) est algébriquement clos, le polynôme \( \mu_a\) possède une racine (au moins) \( b\in \eF\) : \( \mu_a(b)=0\).

    Nous posons
    \begin{equation}
        \begin{aligned}
            \sigma'\colon \eK[X]/(\mu_a)&\to \eF \\
            \overline{ \sum_k s_kX^k }&\mapsto \sum_k\iota(s_k)b^k. 
        \end{aligned}
    \end{equation}
    Montrons que cela est bien défini. Si \( P=\sum_ks_kX^k\) et \( \mu_a=\sum_ka_kX^k\), alors
    \begin{equation}
        \sigma'(\overline{ P+\mu_a })=\sigma'\big( \overline{ \sum_k (a_k+s_k)X^k } \big)=\sum_ka_kb^k+\sum_ks_kb^k=\mu_a(b)+\sigma'(\overline{ P })=\sigma'(\overline{ P }).
    \end{equation}
    L'application \( \sigma'\) est un morphisme de corps.

    Si \( \varphi\colon \eL\to \eK[X]/(\mu_a)\) est l'isomorphisme dont nous parlions plus haut, il suffit maintenant de poser \( \sigma=\sigma'\circ \varphi\) pour avoir le morphisme demandé.
\end{proof}
...
\begin{theorem}[Steinitz\cite{BIBooRTTUooSPwAKJ,BIBooFQQWooIuoZyf}]       \label{THOooEDQKooLEGlDv}
    ...
    \begin{enumerate}
        ...
        \item
            Si \( \eF_1\) et \( \eF_2\) sont deux clôtures algébriques d'un même corps, il existe un isomorphisme de corps \( \varphi\colon \eF_1\to \eF_2\) tel que \( \varphi(k)=k\) pour tout \( k\in \eK\).
    \end{enumerate}
\end{theorem}
...
\item[Conclusion]
    ...
    \end{subproof}
    Nous passons à présent à la partie «unicité». Soient \( \eF_1\) et \( \eF_2\) des clôtures algébriques de \( \eK\); nous allons utiliser le lemme de Zorn sur l'ensemble
    \begin{equation}
        \mA=\Big\{  (\eL,\rho)  \tq
        \begin{cases}
            \eK\setminus \eL\setminus \eF_1
            \eF_1 \text{ est une extension algébrique de }\eL\\
            \rho\colon \eL\to \eF_2\text{ est un } \eK \text{homomorphisme de corps}
        \end{cases}
    \Big\}
    \end{equation}
    que nous ordonnons par inclusion. Cela est non vide parce que \( \eK\) est dedans. Nous passons sur les détails de du fait que \( \mA\) est inductif, et nous utilisons le lemme de Zorn pour considérer un élément maximum \( (\eL,\rho)\).
\end{proof}
