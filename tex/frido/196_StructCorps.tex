...
Voici un lemme qui dit qu'une clôture algébrique est en quelque sorte une extension algébrique maximale.
\begin{lemma}[\cite{MonCerveau}]
    Soient un corps \( \eK\) et une extension algébrique \( \eL\) de \( \eK\). Nous supposons que pour toute extension algébrique de \( \eL\) nous avons \( \eL=\eF\)

    Alors \( \eL\) est algébriquement clos\footnote{Définition \ref{DEFooREUHooLVwRuw}\ref{ITEMooEIWVooVjJRoR}.}.
\end{lemma}

\begin{proof}
    Soit un polynôme \( P\in \eK[X]\). Nous voudrions prouver que \( P\) a des racines dans \( \eL\). Pour cela, nous voyons \( P\) comme un polynôme sur \( \eL[X]\) et, grace à la proposition \ref{PROPooUBIIooGZQyeE} nous considérons un corps de rupture \( \eF\) pour \( P\). Vu que \( \eF\) est une extension de \( \eL\), nous avons \( \eL=\eF\). Donc \( \eL\) contient des racines de \( P\).
\end{proof}
...
\begin{theorem}[Steinitz\cite{BIBooRTTUooSPwAKJ}]       \label{THOooEDQKooLEGlDv}
...
\begin{proof}
    Nous considérons un ensemble \( \Omega\) qui contient \( \eK\), qui est strictement surpotent\footnote{Définition \ref{DEFooXGXZooIgcBCg}.} à \( \eK\) et qui est infini non dénombrable si \( \eK\) est fini.
\end{proof}
<++>
