...
% Copyright (C) 2010-2013,2016-2017, 2019-2021, 2023, 2025
%   Laurent Claessens
...
\begin{enumerate}
	\item
	      Les erreurs de représentation de nombres : troncature et propagation de décalages (\emph{drift}),
	\item
	...
	\item
	      Le fait que certaines méthodes sont non-déterministes (Monte-Carlo) mène à des problèmes pour les tests unitaires des implémentations.
\end{enumerate}
...
\begin{definition}[Représentation en virgule flottante\cite{BIBooFMSZooPmMDKU}]     \label{DEFooLYONooBNskty}
	La \defe{représentation en virgule flottante normalisée}{Représentation!virgule flottante normalisée} en base \( b\) d'un nombre est la donnée de
	\begin{enumerate}
		\item
...
\begin{definition}[Représentation en simple précision (binaire)]        \label{DEFooEIOZooYLDVjs}
	...
	\begin{description}
		\item[Exposant \( q\) générique\cite{ooMPTNooYbSwJS}]
		      Si \( q\neq 0\) et \( q\neq 255\) alors le nombre est \defe{normalisé}{nombre!normalisé}. La règle de lecture est alors
		...
		\item[Exposant \( q\) égal à \( 255\)]
		      La règle de lecture est alors au cas par cas ou à peu près.
		      \begin{enumerate}
			      \item
			            \( \SimplePrec(s,q=255,a=0)=+\infty\).
			      \item
...
La précision du nombre donné en \eqref{EQooFRPYooRnxiFP} aurait été embarrassante si le type avait été un nombre en simple précision. Précision technique : en Python, le type \info{int} n'a pas de limite supérieure à part la mémoire.
...
\begin{definition}		\label{DEFooRUHVooGgoyiP}
...
\begin{normaltext}
	...
	Pour la méthode de bissection, le nombre de décimales augmente de \( 1\) à chaque itération, mais seulement en base \( 2\). En base \( 10\), de façon générique\footnote{C'est-à-dire sauf coup de malchance ou coup de chance.} il faut entre \( 3\) et \( 4\) itérations pour avoir une décimale de plus.
\end{normaltext}
