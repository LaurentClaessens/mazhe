...
\begin{lemma}       \label{LEMooWRCIooWkMpoF}
	L'application\footnote{\( Q(E)\) sont les formes quadratiques réelles sur \( E\), définition \ref{DefBSIoouvuKR}.}
	...
	La topologie que nous considérerons sur \( Q(E)\) est celle de la norme \( N\).
\end{lemma}

\begin{proof}
	Il faut vérifier les conditions de la définition \ref{DefNorme} d'une norme. En deux parties.
	Si \( N(q)=0\), alors \( | q(u) |=0\) pour tout \( u\in B(0,1)\). Mais si \( x\in E\), il existe \( \lambda\in \eR\) tel que \( x=\lambda u\) avec \( u\in B(0,1)\). Avec ce \( u\) nous avons
	\begin{equation}
		q(x)=q(\lambda u)=\lambda^2q(u)=0.
	\end{equation}

	En ce qui concerne l'inégalité triangulaire,
	\begin{subequations}
		\begin{align}
			N(q+f) & =\sup_{\| u \|=1}| q(u)+f(u) |                                                                                         \\
			       & \leq \sup_{\| u \|=1}| q(u) |+\sup_{\| u \|=1}| f(u) | & \text{prop. \ref{PROPooQULXooVzUtAi}\ref{ITEMooBOMOooTDUpgb}} \\
			       & =N(q)+N(f).
		\end{align}
	\end{subequations}

	Si \( \lambda\in \eR\) alors
	\begin{subequations}
		\begin{align}
			N(\lambda q) & =\sup_{\| u \|=1}| (\lambda q)(u) |                                                                              \\
			             & =\sup_{\| u \|=1}\Big( | \lambda || q(u) | \Big)                                                                 \\
			             & =| \lambda |\sup_{\| u \|=1}| q(u) |             & \text{prop. \ref{PROPooQULXooVzUtAi}\ref{ITEMooMWCRooSqwXgN}} \\
			             & =| \lambda |N(q).
		\end{align}
	\end{subequations}
\end{proof}
...
\begin{definition}[Espace euclidien]			\label{DefLZMcvfj}
...
%-------------------------------------------------------
\subsection{Autoadjoint}
%----------------------------------------------------

\begin{proposition}[Adjoint\cite{MonCerveau}]	\label{PROPooYCSWooBnVwFv}
	Soit un espace euclidien\footnote{Avec produit scalaire, définition \ref{DefLZMcvfj}.} réel \( E\). Soit un endomorphisme \(A \colon E\to E  \). Alors il existe un unique opérateur \( B\in \End(E)\) tel que
	\begin{equation}
		x\cdot Ay=Bx\cdot y
	\end{equation}
	pour tout \( x,y\in E\).

	Cet opérateur est l'\defe{adjoint}{adjoint} de \( A\) et sera noté \( A^{\dag}\) ou \( A^t\).
	%TODOooYUCJooNXhwst. Prouver ça.
\end{proposition}

\begin{definition}[Autoadjoint\cite{MonCerveau}]	\label{DEFooYAWTooGUpAvw}
	Si \( E\) est un espace euclidien réel, nous disons qu'un opérateur \( A\in \End(E)\) est \defe{autoadjoint}{opérateur autoadjoint} si \( A^t=A\).

	L'ensemble des opérateur autoadjoints de \( E\) sera noté \( \gS(E)\) ou \( S(n,\eR)\) lorsque ce sera sur \( \eR^n\). La notation \( S(n,\eR)\) devrait être réservée aux matrices, mais je ne promets rien.
\end{definition}

\begin{lemma}[\cite{MonCerveau}]	\label{LEMooECJFooXjrBdX}
	Si \( A\) est un opérateur autoadjoint, alors sa matrice est symétrique dans toute base.
	%TODOooTSZEooXzJfxq. Prouver ça.
\end{lemma}
...
\begin{normaltext}	\label{NORMooYCCGooYyOEdo}
