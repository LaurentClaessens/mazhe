...
\begin{normaltext}
	...
	\begin{enumerate}
		\item
		      Cette fonction est nommée «totient» en anglais.
		\item
...
\begin{lemma}[\cite{BIBooKYTNooKQzrGS}]       \label{LEMooBEJOooDqTirj}
	Si \( p\) est un nombre premier, alors \( \varphi(p^n)=p^n-p^{n-1}\).
\end{lemma}

\begin{proof}
	Les éléments de \( \{ 1,\ldots,p^n \}\) qui ont un \( \pgcd\) différent de \( 1\) avec \( p^n\) sont des nombres qui s'écrivent sous la forme \( qp\) avec \( q\leq p^{n-1}\) (corolaire~\ref{CORooQIMHooUzLUJY}). Il y a évidemment \( p^{n-1}\) tels nombres.

	Donc les éléments de \( \{ 1,\ldots,p^n \} \) qui sont relativement premiers à \( p^n\) sont au nombre de \( p^n-p^{n-1}\) (tous sauf les \( p^{n-1}\) dont on vient de parler).
\end{proof}
