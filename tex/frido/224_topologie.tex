...
\begin{proposition}[\cite{BIBooFOQNooHXrkip}]       \label{PROPooDUAVooEfrEGI}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \end{equation}
        \spitem[Pour \ref{ITEMooWCCSooMeqQHL} en supposant que \( M\) est femré]
        % -------------------------------------------------------------------------------------------- 
...
\begin{propositionDef}[\cite{BIBooSGHOooYSASxY, MonCerveau}]     \label{PropKZDqTR}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \begin{subproof}
            ...
            \spitem[\( s^{-1}\) est continue]
            % -------------------------------------------------------------------------------------------- 
            Soit \( \mO\) ouvert dans \( V_2\); nous devons prouver que \( s(\mO)\) est ouvert dans \( V/V_1\). La proposition \ref{PROPooDRPLooONCwYs} nous dit que ce sera le cas si \( p^{-1}\big( s(\mO) \big)\) est ouverte dans \( V\).

            Nous allons noter \( \sigma\) la partie \( p^{-1}\big( s(\mO) \big)\) :
            \begin{equation}
                \sigma=p^{-1}\big( s(\mO) \big)=\mO+V_1=\{ x+y_1\tq x\in \mO,y_1\in V_1 \}.
            \end{equation}
            Par définition de la topologie induite, il existe un ouvert \( A\) de \( V\) tel que \( \mO=A\cap V_2\). 

            Pour prouver que \( \sigma\) est ouvert, nous considérons un point de \( \sigma\) et nous montrons que ce point accepte un voisinage (dans \( V\)) contenu dans \( \sigma\). Nous commençons par un point \( x\in A\cap V_2\).

            Il existe une boule \( B(x,r)\) contenue dans \( A\) parce que \( A\) est un ouvert de \( V\) qui est métrique. Prouvons que \( B(x,r)\subset \sigma\). Soit \( y\in B(x,r)\). Nous notons \( y=x+h\) et nous décomposons \( h\) en \( h=h_1+h_2\) avec \( h_1\in V_1\) et \( h_2\in V_2\). Vu que \( y\in B(x,r)\) nous avons \( \| h \|=\| y-x \|\leq r\).
            % ah non ça ne suffit pas pour dire | h_2 |<r.  TODOooWQILooFgiklx Il faut terminer ce bout.
            % Quand ce théorème est prouvé, répondre à ma question
            % https://math.stackexchange.com/questions/4515564/topological-direct-sum-of-vector-spaces
        \end{subproof}
        <++>
