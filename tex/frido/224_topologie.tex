...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooTJQPooMHtOAv}
	Soit une suite \( (x_k)\) dans l'espace vectoriel topologique \( X\). L'élément \( x\) est une limite de \( (x_k)\) si et seulement si pour tout ouvert \( \mO\) contenant \( 0\), il existe \( N\in \eN\) tel que \( x_n-x\in\mO\) pour tout \( n\geq N\).
\end{proposition}
...
\begin{proof}
	Soit un ouvert \( \mO\) autour de \( x+y\). La partie \( \mO-x\) est un ouvert\footnote{Par la proposition \ref{PROPooJYLVooPpLWFX}.} autour de \( y\) et la partie \( \mO-y\) est un ouvert autour de \( x\). Vu que \( x\) est une limite de \( (x_k)\), il existe \( N_1\in \eN\) tel que si \( n\geq N_1\), alors \( x_n\in \mO-y\). Idem pour la suite \( (y_k)\) : il existe \( N_2\) tel que si \( n\geq N_2\), alors \( y_n\in \mO-x\).

	En posant \( N=\max\{ N_1,N_2 \}\), pour tout \( n\geq N\), il existe \( a_n,b_n\in\mO\) tels que
	\begin{subequations}
		\begin{numcases}{}
			x_n=a_n-y\\y_n=b_n-x.
		\end{numcases}
