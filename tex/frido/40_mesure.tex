...
% Copyright (c) 2011-2016,2018-2019,2022
...
\begin{proposition}[\cite{SFYoobgQUp}]
    ...
    \begin{equation}
%TODO: problème ici: les boreliens sont definis plus bas ... ?
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\( \Baire(X)\subset\tribF\)]
            ...
            \begin{subproof}
            \item[Vide et tout l'ensemble] C'est facile : \( \emptyset\) et \( X\) sont dans \( \tribF\).
...
%    Ceci est une phrase servant à aligner correctement le \info{description} qui suit.
    ...
    \item[La définition est bonne]
        : l'ensemble sur lequel l'infimum est pris n'est pas vide : il suffit de poser \( A_1=S\) et \( A_{n\geq 2}=\emptyset\).
    \item[Le vide]
        D'abord \( \mu^*(\emptyset)=0\) parce que \( \emptyset\in\tribD\). Prendre ensuite \( A_n=\emptyset\).
    \item[Inégalité d'inclusion]
        Soient \( X\subset Y\) dans \( \partP(S)\) et \( A=\bigcup_nA_n\). Si \( Y\subset A\) alors \( X\subset A\), donc
        \begin{equation}
            \left{ \sum_n\mu(A_n)\tq A_n\in\tribD,X\subset\bigcup_nA_n \left} \subset\left{ \sum_n\mu(A_n)\tq A_n\in\tribD,Y\subset\bigcup_nA_n \right} ,
        \end{equation}
    ...
    \item[Inégalité par union dénombrable]
        ...
        nous avons
        \begin{equation}
%TODO: pourquoi faut-il un 1/2^{n} alors que la somme court sur n ? un 1/n ne suffit pas ?
    ...
    \item[Restriction]
        ...
        En ce qui concerne l'inégalité inverse nous considérons une suite \( A_n\) dans \( \tribD\) telle que \( A\subset\bigcup_nA_n\). Puisque \( A\in\tribD\) et que \( \tribD\) est une algèbre de parties nous avons \( A\cap A_n\in \tribD\) et \( \bigcup_n(A\cap A_n)=A\in\tribD\). Par conséquent
        \begin{equation}
...
La définition suivante est une simple copie de la définition générale \ref{DefWUPHooEklLmR} d'une mesure sur un ensemble de parties. La seule différence est que dans le cas d'une tribu, l'union d'ensembles est automatiquement dans la tribu et la condition \ref{ItemQFjtOjXiii} ne demande pas de le préciser.
...
\begin{proof}
    ...
    Nous passons à l'inégalité dans l'autre sens. D'abord si \( \mu(A\cap S_n)=\infty\) pour un certain \( n\), alors cela vaut encore \( \infty\) pour tous les \( n\) suivants, et la limite est \( \infty\) sans problème. Donc nous supposons que \( \mu(A\cap S_n)<\infty\) pour tout \( n\in \eN\). De plus, quitte à renommer les indices, nous pouvons supposer que \( S_0=\emptyset\).

    Un élément \( x\) est dans \( S\) si et seulement si il existe \( n\geq 0\) tel que \( x\in S_{n+1}\). En prenant le plus petit de ces \( n\) nous avons \( x\notin S_n\) (éventuellement \( n=0\)) et donc
    \begin{equation}
...
\begin{proof}
    Soit \( (E_n)\) la suite prise par hypothèse; nous considérons \( \mu_n\) et \( \nu_n\), les restrictions de \( \mu\) et \( \nu\) à \( E_n\), c'est-à-dire
    \begin{subequations}
        \begin{align}
        \mu_n(A)=\mu(A\cap E_n)\\
        \nu_n(A)=\nu(A\cap E_n).
    ...
    \begin{equation}
        \mu(A\cap E_n)=\nu(A\cap E_n)<\infty,
    \end{equation}
%TODO: "cad" ? je ne comprends pas, c'est ce qu'il faut montrer...
    c'est-à-dire que \( \mu_n=\nu_n\) sur \( \tribE\). Par ailleurs, \( E\cap E_n=E_n\in\tribE\), donc
    \begin{equation}
        \mu_n(E)=\nu_n(E)<\infty.
    \end{equation}
%TODO: c'est quoi le singleton E ?
    Par conséquent \( \mu_n=\nu_n\) sur la classe \( \tribE'=\tribE\cup\{ E \}\) : \( \tribE'\subset\tribD\).

%TOTO: pourquoi les mesures sont finies ?
...
\begin{proof}
    ...
    \item[Inégalité par union dénombrable]
        ...
        \end{equation}
%TODO: meme remarque que precedemment pour 1/2^{n} ??
