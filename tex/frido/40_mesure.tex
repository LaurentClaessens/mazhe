...
\begin{propositionDef}[Tribu induite, tribu-trace\cite{BIBooIDTRooMHRyUm}]      \label{DefDHTTooWNoKDP}
    ...
    Elle est la \defe{tribu induite}{tribu!induite}\index{induite!tribu} de \( R\) depuis \( S\). Elle est aussi nommée «tribu trace».
\end{propositionDef}
...
\begin{definition}[Mesure sur un ensemle de parties\cite{ooZFLHooQmmfyS}]       \label{DefWUPHooEklLmR}
    Soit \( S\) un ensemble et \( \tribC\) un ensemble de parties de \( S\) contenant \( \emptyset\). Une \defe{mesure positive}{mesure!sur un ensemble de parties} sur \( (S,\tribC)\) est une application \( \mu\colon \tribC\to \mathopen[ 0 , \infty \mathclose]\) telle que
    \begin{enumerate}
        \item
            \( \mu(\emptyset)=0\),
        \item
    ...
    La mesure est \defe{finie}{mesure finie} si \( \mu(S)<\infty\) et \defe{\( \sigma\)-finie}{mesure \( \sigma\)-finie} s'il existe une suite \( (S_n)\) dans \( \tribC\) telle que \( S=\bigcup_nS_n\) et \( \mu(S_n)<\infty\).
\end{definition}
...
Sans hypothèses sur l'ensemble de parties, nous ne pouvons pas dire grand chose de plus.
...
\begin{lemma} \label{LemPMprYuC}
    À propos de mesure d'unions et de différences dans un espace mesurable.
    \begin{enumerate}
        \item
...
La proposition suivante sert à parler d'extension de tribu induite. La principale application est de pouvoir parler de la tribu de Lebesgue sur une partie de \( \eR^n\). Exemple dans la proposition \ref{PROPooHMSCooRIjcJq} qui donne une structure d'espace mesuré dans \( S^1\) à partir de la mesure de Lebesgue sur \( \eC\).

\begin{proposition}
    Soit un espace mesuré \( (\Omega,\tribF,\mu)\). Nous considérons un mesurable \( M\in \tribF\) ainsi que sa tribu induite \( \tribF_M=\{ A\cap M\tq A\in \tribF \}\). Nous notons \( \hat\tribF\) la tribu complétée de \( \tribF\) dans \( \Omega\) et \( \hat\tribF_M\) la complétée de \( \tribF_M\) dans \( M\) (où nous avons considéré la mesure restreinte de \( \mu\)).

    Alors
    \begin{equation}
        \hat\tribF_M=\{ M\cap A\tq A\in\hat\tribF \},
    \end{equation}
    c'est à dire que nous pouvons compléter avant ou après de prendre les intersections.

    Nous avons aussi
    \begin{equation}
        \hat\tribF_M=\{ A\subset M\tq A\in\hat\tribF \},
    \end{equation}
\end{proposition}
...
\begin{proof}
    En deux inclusions.
    \begin{subproof}
        \item[Première : \( \hat\tribF_M\subset\{ M\cap A\tq A\in\hat\tribF \}\)]
            Un élément de \( \hat\tribF_M\) est de la forme \( B\cup N\) où \( B\in \tribF_M\) et où \( N\) est négligeable\footnote{Pour rappel, une partie est négligeable quand elle est inclue à une partie de mesure nulle.} dans \( M\). Vu que \( B\in\tribF_M\), il existe \( A\in\tribF\) tel que \( B=A\cap N\).

            Nous avons alors
            \begin{equation}
                B\cup N=(A\cup N)\cap M
            \end{equation}
            avec \( N\) négligeable dans \( M\) et donc également négligeable dans \( \Omega\). Donc \( A\cup N\in \hat\tribF\).
        \item[Deuxième : \( \{ M\cap A\tq A\in\hat\tribF \}\subset \hat\tribF_M\)]
            ...
            \end{equation}
            Il s'agit maintenant de nous assurer que cette décomposition implique que \( M\cap(B\cup N)\in \hat\tribF_M\).
            ...
            \end{equation}
            avec \( \mu(M\cap N_1)=0\). Cela pour dire que \( M\cap N\) est négligeable dans \( M\). La décomposition \eqref{EQooWWQEooDRlnLN} est donc bien une union d'un élément de \( \tribF_M\) avec un négligeable de \( M\), et donc bien un élément de \( \hat\tribF_M\).
    \end{subproof}
    Pour l'égalité \( \hat\tribF_M=\{ A\subset M\tq A\in\hat\tribF \}\), il suffit de remarquer que \( \hat\tribF\) est une tribu sur \( \Omega\) et que \( M\) est dans \( \hat\tribF\). Donc la proposition \ref{PROPooUNNSooMUQKfp} donne directement
    \begin{equation}
        \{ A\subset M\tq A\in\hat\tribF \}=\{ M\cap A\tq A\in \hat\tribF \}.
    \end{equation}
\end{proof}
