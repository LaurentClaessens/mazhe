...
% Copyright (c) 2011-2020, 2022-2026
%   Laurent Claessens
...
\begin{probleme}		\label{PROBooSWDQooHlSZPD}
	La fonction de répartition \ref{DefooYAZVooNdxDCx} est \( F_X(x)=P(X\leq x)\). La densité \( f_X\) doit vérifier
	\begin{equation}
		F_X(x)=\int_{-\infty}^xf_X(t)dt,
	\end{equation}
	et la relation entre les deux est \( f_X=F_X'\).

	Il me faudrait un énoncé précis et une démonstration.
	%TODOooXOKRooTAJQgO. Trouver un tel énoncé et le prouver.
\end{probleme}
...
La définition usuelle d'événements indépendants est \( P(A\cap B)=P(A)P(B)\). Nous allons évidemment fait quelque chose de plus compliqué et plus général.

\begin{definition}[Indépendance\cite{CourgGudRennes}]      \label{DEFooVYCUooKWvReO}
	Soit un espace probabilisé \( (\Omega,\tribA,P)\).
	\begin{enumerate}
		\item		\label{ITEMooMAJVooDOkohJ}
		      Un ensemble fini \( (\mA_i)_{i\in I}\) de parties mesurables de \( \Omega\) est \defe{indépendante}{indépendance!familles d'ensembles mesurables} si pour tout choix de \( A_i\in\mA_i\),
		      \begin{equation}		\label{EQooERKOooPBRACZ}
			      P(\bigcap_{i\in I}A_i)=\prod_{i\in I}P(A_i).
		      \end{equation}
		\item		\label{ITEMooRVBBooJcLHbY}
		      Si \( \{ \mA_i \}_{i\in I}\) est un ensemble quelconque de familles de parties mesurables de \( \Omega\), nous disons qu'elles sont \defe{indépendantes}{indépendance} si pour tout \( J\) fini dans \( I\), l'ensemble \( \{ \mA_j \}_{j\in J}\) est indépendant.
		\item		\label{ITEMooPCQXooGPKyeo}
		      Une famille \( \{ A_i \}_{i\in I}\) d'événements est \defe{indépendantes}{indépendance!évènements} si les tribus \( \{ \sigma(A_i) \}_{i\in I}\) engendrées\footnote{La tribu engendrée par une seule partie \( A\) n'est pas très méchante: \( \sigma(A)=\{ A,A^c,\emptyset,\Omega \}\).} par les \( A_i\) sont indépendantes au sens de la définition \ref{ITEMooRVBBooJcLHbY}.
	\end{enumerate}
\end{definition}
...
\begin{lemma}       \label{LemIndepEvenCompl}
	Soit un espace probabilisé \( (\Omega,\tribA,P)\). Soient des évènements \( (A_i)_{i=0,\ldots,n}\). Si
	\begin{equation}
		P(A_0\cap\ldots A_n)=P(A_0)\cap\ldots\cap P(A_n),
	\end{equation}
	alors
	\begin{equation}
		P(A_0^c\cap A_1\cap\dots\cap A_n)=P(A_0^c)P(A_1)\ldots P(A_n).
	\end{equation}
	Et plus généralement l'égalité reste vraie si on remplace plusieurs des \( A_i\) par \( A_i^c\).
\end{lemma}

\begin{proof}
	Soit \( I\) un sous-ensemble de \( \{ 1,\ldots,n \}\). Nous avons
	\begin{subequations}
		\begin{align}
...
\begin{proposition}	\label{PROPooNLRBooJzDyiF}
	Les évènements \( \{ A_1,\ldots,A_n \}\) sont indépendants si et seulement si pour tout choix de \( \{ i_1,\ldots,i_k \}\subset\{ 1,\ldots,n \}\) nous avons
	\begin{equation}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\( \Rightarrow\)]
		%-----------------------------------------------------------
		Si les parties \( A_1,\ldots,A_n\) sont indépendantes, leurs tribus sont indépendantes. En suivant les définitions \ref{DEFooVYCUooKWvReO}\ref{ITEMooPCQXooGPKyeo}\ref{ITEMooRVBBooJcLHbY}\ref{ITEMooMAJVooDOkohJ}, et en choisissant en particulier \( A_i\) dans chacun des \( \sigma(A_i)\), la condition \eqref{EQooERKOooPBRACZ} donne ce qu'il fallait :
		\begin{equation}
		...
		\spitem[\( \Leftarrow\)]
		%-----------------------------------------------------------
		Chacun des choix de \( B_i\) dans \( \sigma(A_i)\) peut être \( A_i\), \( A_i^c\), \( \emptyset\) ou \( \Omega\). Si un des \( B_i\) est vide, les intersections sont automatiquement vides et les probabilités sont nulles. ok. Les \( B_i\) qui sont \( \Omega\) ne changent rien parce que \( P(\Omega)=1\) et \( \Omega\cap X=X\).

		Si un ou plusieurs des \( B_i\) est \( A_i^c\), alors c'est le lemme\ref{LemIndepEvenCompl} qui fait le travail.
	\end{subproof}
\end{proof}
...
\begin{lemma}       \label{LEMooWBAZooLVVRjO}
	...
	%TODOooGWRCooXyPwOa. Prouver ça.
\end{lemma}


\begin{lemma}[\cite{MonCerveau,VincentBa}]	\label{LEMooJBIVooXyPiDs}
	Si des \( \pi\)-systèmes sont indépendants, alors les tribus engendrées sont indépendantes.
\end{lemma}

\begin{proof}
	Soient des \( \pi\)-systèmes indépendants \( (\mS_i)_{i\in I}\). Nous considérons les tribus engendrées \( \tribF_i=\sigma(\mS_i)\), et nous prouvons que ces tribus sont indépendantes.

	Fixons \( A_i\in\mS_i\) pour \( i=2,\ldots,n\), et considérons les applications
	\begin{equation}
		\begin{aligned}
			\mu_1\colon \tribF_1 & \to \mathopen[ 0,1\mathclose]             \\
			B_1                  & \mapsto P(B_1\cap A_2\cap\ldots\cap A_n).
		\end{aligned}
	\end{equation}
	et
	\begin{equation}
		\begin{aligned}
			\sigma_1\colon \tribF_1 & \to \mathopen[ 0,1\mathclose]      \\
			B_1                     & \mapsto P(B_1)P(A_2)\ldots P(A_n).
		\end{aligned}
	\end{equation}
	\begin{subproof}
		\spitem[\( \mu_1\) est une mesure]
		%-----------------------------------------------------------
		Vérifions la définition \ref{DefBTsgznn}. Si \( \{ B_i \}_{i\in \eN}\) sont des éléments disjoints de \( \tribF_1\) alors
		\begin{equation}
			\big( \bigcup_{i=1}^{\infty} \big)\cap A_2\cap\ldots\cap A_n=\bigcup_{i=1}^{\infty}(B_i\cap A_2\cap\ldots\cap A_n),
		\end{equation}
		et l'union à droite est une union disjointe. Nous avons alors
		\begin{subequations}
			\begin{align}
				\mu_1\big( \bigcup_{i=0}^{\infty}B_i \big) & =P\Big( (\bigcup_{i=1}^{\infty}B_i)\cap A_2\cap\ldots\cap A_n \Big)                                \\
				                                           & =P\Big( \bigcup_{i=1}^{\infty}(B_i\cap A_2\cap\ldots\cap A_n) \Big)                                \\
				                                           & =\sum_{i=1}^{\infty}P(B_i\cap A_2\cap\ldots\cap A_n)                & \text{\( P\) est une proba.} \\
				                                           & =\sum_{i=1}^{\infty}\mu_1(B_i).
			\end{align}
		\end{subequations}
		Donc \( \mu_1\) est une mesure.

		\spitem[\( \sigma_1\) est une mesure]
		%-----------------------------------------------------------
		Même type de calculs :
		\begin{subequations}
			\begin{align}
				\sigma_1\big( \bigcup_{i=1}^{\infty}B_i \big) & =P\big( \bigcup_{i=1}^{\infty}B_i \big)P(A_2)\ldots P(A_n) \\
				                                              & =\big( \sum_{i=1}^{\infty}P(B_i) \big)P(A_2)\ldots P(A_n)  \\
				                                              & =\sum_{i=1}^{\infty}\big( P(B_i)P(A_2)\ldots P(A_n) \big)  \\
				                                              & =\sum_{i=1}^{\infty}\sigma_1(B-i).
			\end{align}
		\end{subequations}
	\end{subproof}
	Donc \( \mu_1\) et \( \sigma_1\) sont deux mesures qui coïncident sur le \( \pi\)-système \( \mS_1\). Nous en déduisons que \( \mu_1=\sigma_1\) sur \( \tribF_1\) par la proposition \ref{PROPooWFCWooCCzZLT}.

	Maintenant nous fixons \( B_1\in\tribF_1\) ainsi que \( A_i\in\mS_i\) pour \( i=3,\ldots,n\). Nous définissons alors
	\begin{equation}
		\begin{aligned}
			\mu_2\colon \tribF_2 & \to \mathopen[ 0,1\mathclose]                \\
			B_2                  & \mapsto P(B_1\cap B_2\cap A_3\ldots\cap A_n)
		\end{aligned}
	\end{equation}
	et
	\begin{equation}
		\begin{aligned}
			\sigma_2\colon \tribF_2 & \to \mathopen[ 0,1\mathclose]            \\
			B_2                     & \mapsto P(B_1)P(B_2)P(A_3)\ldots P(A_n).
		\end{aligned}
	\end{equation}
	Nous démontrons comme avant\quext{Je n'ai pas vérifié. Vous devriez le faire, et si vous avez un problème, écrivez-moi.} que \( \mu_2\) et \( \sigma_2\) sont des mesures qui coïncident sur \( \tribF_2\). Et nous continuons ainsi. Au final, pour la dernière étape nous fixons \( B_1\in \tribF_1\) jusqu'à \( B_{n-1}\in \tribF_{n-1}\) et nous prouvons à l'aide de \( \mu_n\) et \( \sigma_n\) que pour tout \( B_n\in \tribF_n\) :
	\begin{equation}
		P(B_1\cap \dots B_n)=P(B_1)\ldots P(B_n),
	\end{equation}
	montrant ainsi que les \( \tribF_i\) sont indépendantes.
\end{proof}

\begin{lemma}[Lemme de regroupement\cite{VincentBa}]\index{lemme de regroupement}  \label{LemHOjqqw}
	Soit \( (\Omega,\tribF,P)\) un espace de probabilité. Soient des \( \pi\)-systèmes\footnote{Définition \ref{DEFooGYADooTudZmJ}.} indépendants\footnote{Indépendance, définition \ref{DEFooVYCUooKWvReO}.} \( (\mS_i)_{i\in I}\) contenus dans \( \tribF\). Pour chaque \( i\in I\) nous notons \( \tribF_i=\sigma(\mS_i)\).

	Soit une partition \( (I_j)_{j\in J}\) de \( I\). Pour chaque \( j\in J\) notons
	\begin{equation}		\label{EQooLMFGooDfRbim}
		\mG_i=\sigma\big( \bigcup_{i\in I_j}\mS_i \big).
	\end{equation}
	Alors les tribus \( \mG_i\) sont indépendantes.
\end{lemma}

\begin{proof}
	Pour chaque \( j\in J \) nous posons
	\begin{equation}
		\mL_j=\{ \bigcap_{i\in K}A_i\tq \begin{cases}
			\text{\( K\) est fini dans \( I_j\) } \\
			A_i\in \mS_i.
		\end{cases}\}
	\end{equation}

	\begin{subproof}
		\spitem[Chaque \( \mL_j\) est un \( \pi\)-système]
		%-----------------------------------------------------------
		Deux éléments de \( \mL_j\) sont \( \bigcap_{i\in K}A_i\) et \( \bigcap_{j\in L}A_j\). Leur intersections est \( \bigcap_{i\in K\cup L}A_i\). Vu que \( K\cup L\) est encore fini dans \( I_j\), l'intersection est bien dans \( \mL_j\).

		\spitem[Si \( i\in I_j\), alors \( \mS_i\subset\mL_j\)]
		%-----------------------------------------------------------
		Il suffit de prendre \( K=\{ i \}\) et de faire varier \( A_i\) dans \( \mS_i\).


		\spitem[Première conclusion]
		%-----------------------------------------------------------
		Nous déduisons de cela que
		\begin{equation}		\label{EQooIMGTooEwmKHI}
			\bigcup_{i\in I_j}\mS_i\subset \mL_j.
		\end{equation}

		\spitem[\( \mL_j\) est indépendant de \( \mL_l\)]
		%-----------------------------------------------------------
		Un élément de \( \mL_j\) est \( \bigcap_{i\in L}A_i\) avec \( A_i\in \mS_i\) et \( L\) fini dans \( I_j\). Un élément de \( \mL_l\) est \( \bigcap_{k\in M}A_k\) où \( A_k\in\mS_k\) et \( M\) est fini dans \( I_l\). Étant donné que \( I_j\cap I_l=\emptyset\), les parties \( M\) et \( L\) sont disjointes. Donc
		\begin{equation}
			P\big( \bigcap_{i\in L}A_i\cap\bigcap_{k\in M}A_k \big)=P\big( \bigcap_{i\in L\cup M}A_i \big).
		\end{equation}
		Mais \( \bigcap_{i\in L\cup M}A_i\) est une intersection qui prend un élément dans chacun des \( \mS_i\) (avec \( i\in L\cup M\)). Par indépendance des \( \mS_i\) nous avons
		\begin{subequations}
			\begin{align}
				P\big( \bigcap_{i\in L\cup M}A_i \big) & =\prod_{i\in L\cup M}P(A_i)                                           & \text{indép. des \( \mS_i\)} \\
				                                       & =\big( \prod_{i\in L}P(A_i) \big)\big( \prod_{i\in M}P(A_i) \big)     & M\cap L=\emptyset            \\
				                                       & =P\big( \bigcap_{i\in L}A_i \big)P\big( \bigcap_{i\in M}P(A_i) \big).
			\end{align}
		\end{subequations}
		Nous avons prouvé que les \( \mL_j\) sont indépendants.

		\spitem[Monter en indépendance]
		%-----------------------------------------------------------
		Nous avons vu dans la proposition \ref{LEMooJBIVooXyPiDs} que si les \( \pi\)-systèmes \( \mL_j\) sont indépendants, alors les tribus associées sont indépendantes : les tribus \( \sigma(\mL_i)\) sont indépendantes.

		En utilisant \eqref{EQooIMGTooEwmKHI} et la définition \eqref{EQooLMFGooDfRbim}, nous avons
		\begin{equation}
			\mG_j=\sigma\big( \bigcup_{i\in I_j}\mS_i \big)\subset \sigma(\mL_j).
		\end{equation}
		Étant donné que les \( \mL_j\) sont indépendants, les \( \mG_j\) sont indépendants.
	\end{subproof}
\end{proof}

\begin{lemma}[Lemme des regroupement pour les variables aléatoire\cite{VincentBa}]	\label{LEMooDPGVooDNrUYU}
