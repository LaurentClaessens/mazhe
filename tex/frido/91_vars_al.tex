...
\end{definition}
Cela ne devrait pas être sans rappeler les définitions \ref{DefAbsoluCont} et \ref{DEFooJNQYooKUcezo}.
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooDXCVooBxaToe}
	...
	\end{equation}
\end{proposition}
% Quand la démonstration est faite, supprimer PROBooSWDQooHlSZPD.
...
\begin{lemma}[\cite{BIBooORZFooWKgLCH}]     \label{LEMooLNIZooDWUvRN}
	...
	Alors la densité conjointe de variable aléatoire \( X\) est
	\begin{equation}
		\begin{aligned}
			g\colon \eR^n & \to \eR                                                      \\
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Intégrale \( 1\)]
		%-----------------------------------------------------------
		Nous prouvons que \( \int_{\eR^n}g=1\). Soit \( \sigma\in S_n\) (une permutation de \( \{ 1,\ldots,n \}\)). Nous considérons la bijection
		\begin{equation}
			\begin{aligned}
				\varphi_{\sigma}\colon \eR^n & \to \eR^n              \\
		...
		\end{equation}
		où \( Z\) est de mesure (le Lebesgue) nulle. Nous prouvons, pour tout \( \sigma\in S_n\), la formule \( \int_{\eR^n_{\sigma}}\prod_{i=1}^nf(x_i)=\int_{\eR^n_{\id}}\prod_{i=1}^nf(x_i)\).
	\end{subproof}
