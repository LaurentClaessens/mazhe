...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooCXSWooByucuN}
	...
	\begin{enumerate}
		\item
		      Nous avons égalité des mesures\footnote{Voir les définitions \ref{DEFooBKJVooRJdMeA} et \ref{PropooVXPMooGSkyBo}.}
		      \begin{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[\( P(X\in A)\)]
		%-----------------------------------------------------------
		Soit un borélien \( A\subset \eR^n\). Nous montrons à présent que \( P(X\in A)=\int_Ag(x)dx\). Pour \( \sigma\in S_n\), nous posons \( A_{\sigma}=A\cap\eR^n_{\sigma}\). Nous avons \( A=\bigcup_{\sigma\in S_n}A_{\sigma}\cup Z\) avec \( P(Z)=0\) (la partie \( Z\) est pour les parties \( x_{\sigma(i)}=\sigma_{\sigma(i+1)}\)).

		Nous faisons un changement de variables (formule \eqref{EQooLYAWooTArAZR}) avec l'application
		\begin{equation}
			\begin{aligned}
				\phi_{\sigma}\colon \eR^n & \to \eR^n                                     \\
				x                         & \mapsto (x_{\sigma(1)},\ldots, x_{\sigma(n)})
			\end{aligned}
		\end{equation}
		dont le jacobien vaut \( J_{\phi_{\sigma}}=(-1)^{\sigma}\). Cela donne
		\begin{subequations}
			\begin{align}
				P(X\in A) & =\sum_{\sigma}P(X\in A_{\sigma})                                                                                                         \\
				          & =\sum_{\sigma}\int_A\prod_if(x_i)\mtu_{\eR^n_{\sigma}}(x)dx                                                                              \\
				          & =\sum_{\sigma}\int_{\phi_{\sigma}(A)}\prod_if\big( \phi_{\sigma}^{-1}(x)_i \big)\mtu_{\eR^n_{\sigma}}\big( \phi_{\sigma}^{-1}(x) \big)dx
			\end{align}
		\end{subequations}
		Étant donné que \( \phi_{\sigma}\) mélange les coordonnées, et que le produit les prends toutes, nous avons \( \prod_if\big( \phi_{\sigma}^{-1}(x)_i \big)=\prod_if(x_i)\). Par définition, si \( x\in \eR^n_{\id}\), nous avons \( \phi_{\sigma}(x)\in \eR^n_{\sigma}\). De même, si \( \phi_{\sigma}^{-1}(x)\in \eR^n_{\id}\), alors \( x\in \eR^n_{\sigma}\). Cela pour dire que \( \mtu_{\eR^n_{\sigma}}\big( \phi_{\sigma}^{-1}(x) \big)=\mtu_{\eR^n_{\id}}(x)\). Nous avons alors
		\begin{equation}
			P(X\in A)=\sum_{\sigma}\int_{\phi_{\sigma}^{-1}(A)}\prod_if(x_i)\mtu_{\eR^n_{\id}}(x)dx=\frac{1}{ n!}\sum_{\sigma}\int_{\phi^{-1}_{\sigma}(A)}g(x)dx.
		\end{equation}
		Les parties \( \phi_{\sigma}^{-1}(A)\) sont disjointes et leur union est \( A\) (à partie de mesure nulle près), nous pouvons utiliser la \( \sigma\)-additivité de l'intégrale (proposition \ref{PropOPSCooVpzaBt}) Nous avons donc
		\begin{equation}
			P(X\in A)=\frac{1}{ n!}\sum_{\sigma}\int_{\phi^{-1}_{\sigma}(A)}g(x)dx=\frac{1}{ n!}\int_{\bigcup_{\sigma\in S_n}\phi_{\sigma}^{-1}(A)}g(x)dx=\frac{1}{ n!}\int_Ag.
		\end{equation}
