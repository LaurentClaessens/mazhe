...
\begin{definition}
    Soit \( 1\leq p<\infty\). Le \defe{moment}{moment} d'ordre \( p\) d'une variable aléatoire \( X\in L^p(\Omega,\tribA,P)\) est l'espérance
    \begin{equation}
        m_n(X)=E(X^n).
    \end{equation}
\end{definition}
...
\begin{proposition}[\cite{Marazzi}] \label{PropZBnsCgh}
...
\begin{lemma}[\cite{MonCerveau}]
    Soit un espace de probabilité \( (\Omega,\tribF,P)\). Nous considérons une variable aléatoire \( X\) à valeurs réelles, prenant une quantité finie de valeurs différentes :
    \begin{equation}
        X\colon \Omega\to \{ y_0,\ldots, y_N \}
    \end{equation}
    avec \( y_0=0\) (éventuellement \( P(X=y_0)=0\)). Si \( A\) est mesurable dans \( \Omega\), alors
    \begin{equation}
        E(X|A)=\sum_{k=1}^{N}y_kP(X=y_k|A).
    \end{equation}
\end{lemma}

\begin{proof}
    Nous partons de la définition \ref{DEFooOMLCooJgrbpx} de l'espérance conditionnelle : \( E(X|A)=E(X\mtu_A)/P(A)\).
\end{proof}
<++>
