...
\begin{definition}      \label{DEFooTLQUooJvknvi}
    Si \( E\) et \( F\) sont deux espaces vectoriels normés nous notons \( \GL(E,F)\)\nomenclature[Y]{\( \GL(E,F)\)}{les bijections linéaires et continues} le sous-espace de \( \aL(E,F)\) des bijections continues linéaires de \( E\) vers \( F\). Un élément de \( \GL(E,F)\) est donc linéaire, continue et bijective.
\end{definition}
...
Inégalités sur la norme opérateur. Soient \( E\) et \( F\), des espaces vectoriels normés. Nous notons \( \| . \|\) les normes sur \( E\), \( F\), et nous notons également \( \| . \|\) la norme opérateur qui en découle sur \( \aL(E,F)\). 
    \begin{enumerate}
        \item
            La norme opérateur est une norme d'algèbre\footnote{Définition \ref{DefJWRWQue}.} sur \( \GL(E,F)\) :
    \begin{equation}
        \| AB \|\leq \| A \|\| B \|
    \end{equation}
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    Le premier facteur est égal à \( \| A \|\) parce que \( B\) est surjective. Le second est \( \| B \|\) par définition. Donc nous avons bien \( \| AB \|\leq \| A \|\| B \|\).
