...
\begin{definition}      \label{DEFooYKCSooURQDoS}
    Un opérateur est \defe{orthogonal}{orthogonal!opérateur} lorsque \( AA^*=\id\) où \( A^*\) est l'adjoint de \( A\) définit en \ref{DEFooROVNooFlTbSK}.
\end{definition}
...
\begin{definition}      \label{DEFooBSXUooQOFZbd}
    Une \defe{rotation plane}{rotation!plane} est une application linéaire \( f\colon \eR^2\to \eR^2\) qui est
    \begin{itemize}
        \item orthogonale (définition \ref{DEFooYKCSooURQDoS}) : $f^*\circ f=\id$,
        \item de déterminant \( 1\).
    \end{itemize}
\end{definition}

Autrement dit, les rotations sont les éléments de \( \SO(2)\).
