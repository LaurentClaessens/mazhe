...
\begin{proof}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	et nous regardons si ce contexte vérifie les hypothèses du théorème~\ref{ThoMWpRKYp}.
	\begin{subproof}
		\spitem[\( f_t\in L(\Omega)\)]
		%-----------------------------------------------------------
		Pour chaque \( t\) fixé, nous posons \( f_t(\omega)=f(t,\omega)\). Cette application est dans \( L^1(\Omega)\) parce que \( | f_t(\omega) |=1\) pour tout \( \omega\) et \( P\) est une mesure finie.
		\spitem[\( f_{\omega}\) est dérivable]
		%-----------------------------------------------------------
		Pour chaque \( \omega\) fixé, nous posons \( f_{\omega}(t)=f(t,\omega)\). Cette fonction est dérivable, et nous avons \( f'_{\omega}(t)=iX(\omega)e^{itX(\omega)}\). Cela peut d'ailleurs aussi bien être écrit
		\begin{equation}
			\frac{ \partial f }{ \partial t }(t,\omega)=iX(\omega)e^{itX(\omega)}.
		\end{equation}
		%TODOooKCHXooXAMeXi finir de vérifier les hypothèses et faire bien la récurrence.
	\end{subproof}
