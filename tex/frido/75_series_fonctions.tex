...
\begin{proof}
    ...
    \begin{subproof}
        \item[Permuter somme et intégrale]
            Nous voudrions permuter la somme et l'intégrale dans \( \int_0^1\sum_if_i(t)dt\). Pour cela nous commençons par regarder quelque majorations de normes.
            ...
            \begin{equation}
                    ...
                    \leq M\| du_i \|_{\infty}
            \end{equation}
            Pour la première inégalité, nous avons utilisé le lemme \ref{LEMooIBLEooLJczmu}. Pour la seconde, le fait que \( du_i\) est continue sur le compact \( \gamma\big( \mathopen[ 0 , 1 \mathclose] \big)\). Donc la norme est majorée par le nombre \( \| du_i \|_{\infty}\) qui est fini. Notons que la norme \( \| . \|_{\infty}\) ne réfère pas à un supremum sur \( E\), mais seulement sur l'image de \( \gamma\).
            ...
            \begin{equation}
                \| g_n(t) \|\leq \sum_{i=0}^n\| f_i(t) \|\leq M\sum_{i=0}^n\| du_i \|_{\infty}\leq M\sum_{i=0}^{\infty}\| du_i \|_{\infty}
            \end{equation}
            où la dernière série converge par hypothèse.
        ...
        \item[Différentielle]
            ...
            \begin{subequations}
                \begin{align}
                    ...
                    &=\Dsdd{ \int_0^1\sum_n(du_n)_{\gamma_s(t)}\big( \gamma_s'(t) \big)dt }{s}{0}
                \end{align}
            \end{subequations}
            <++>
    ...
    Nous considérons ce chemin en coordonnées normales\footnote{C'est à dire telle que \( \| \gamma'(t)=1 \|\) pour tout \( t\), voir la définition \ref{DEFooJJQFooEITCvG} et le lemme \ref{LEMooLADUooBlHjuT}.} et nous notons \( l\) sa longueur. Par définition \eqref{EqEFIZyEe},
    \begin{equation}
    ...
    \end{equation}
    Nous allons utiliser cela pour calculer \( du_x(v)\) selon la bonne vieille formule du lemme \ref{LemdfaSurLesPartielles}
    \begin{equation}
        du_x(v)=\Dsdd{ u(x+sv) }{s}{0}.
    \end{equation}
