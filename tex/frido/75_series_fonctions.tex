...
\begin{proof}
    Nous avons, par la définition \ref{}<++>
    ...
    La relation \eqref{EQooEJQSooWCczXy} de démontre d'abord pour \( x\in \eN\), puis pour \( x\in \eQ\) et enfin pour \( x\in\eR\). Si \( n\in \eN\) alors la relation \eqref{EQooEJQSooWCczXy} donne immédiatement
    \begin{equation}
        \ln(a^n)=n\ln(a).
    \end{equation}
    ...
    Nous passons à \( q=-m/n\in \eQ^-\), c'est à dire toujours \( m,n\in \eN\). Nous avons 
    \begin{equation}
        \ln(a^{-q})=\ln(\frac{1}{ a^q }).
    \end{equation}
    
\end{proof}
<++>

\begin{lemma}[\cite{MonCerveau}]        \label{LEMooGYOCooBWznrC}
    Pour \( a>0\) et tout \( x\in \eR\) nous avons :
    \begin{equation}
        a^x=\exp\big( x\ln(a) \big)
    \end{equation}
\end{lemma}

\begin{lemma}[\cite{MonCerveau}]       \label{LEMooFXARooZeuVhm} 
    Nous avons
    \begin{equation}
        \exp(x)=e^x
    \end{equation}
