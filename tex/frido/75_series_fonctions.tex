...
Le polynôme de Taylor d'une fonction définie par une série entière s'obtient en tronquant la série. C'est une assez bonne nouvelle que nous allons démontrer maintenant.
...
\begin{proposition}[\cite{MonCerveau}]
    ...
    Il existe une fonction \( \alpha\) telle que
    ...
    \end{equation}
    L'égalité \eqref{EQooSXUJooFjsVek} est valable sur le disque de convergence.
\end{proposition}
...
\begin{proof}
    Le corollaire \ref{CorCBYHooQhgara} nous indique que \( f\) est de classe \(  C^{\infty}\) sur \( \mathopen] -R , R \mathclose[\) et que nous pouvons dériver terme à terme.
        ...
        En utilisant la formule \eqref{EqSOFdwhw} nous trouvons :
        \begin{equation}
            f^{(p)}(x)=\sum_ka_k(x^k)^{(p)}=\sum_{k=p}^{\infty}a_k\frac{ k! }{ (k-p)! }x^{k-p}
        \end{equation}
        En évaluant cela en \( x=0\), tous les termes s'annulent sauf \( k=p\) :
        \begin{equation}
            f^{(p)}(0)=p!a_p.
        \end{equation}
