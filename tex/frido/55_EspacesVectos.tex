...
Le théorème suivant montre que les matrices et les applications linéaires, dans le cas des espaces vectoriels \( \eK^n\) sont deux présentations de la même chose.
\begin{theorem}[\cite{MonCerveau}]
    Soit un corps commutatif. Lorsque \( A\in \eM(n,\eK)\), nous considérons l'application linéaire \( f_A\in \End(\eK^n)\) donné par
    \begin{equation}
        \begin{aligned}
            f_A\colon \eK^n&\to \eK^n \\
            x&\mapsto \sum_{ij}A_{ij}x_je_j 
        \end{aligned}
    \end{equation}
    où \( \{ e_i \}\) est la base canonique de \( \eK^n\) et \( \{ x_i \}\) sont les coordonnées de \( x\) dans cette base.
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    est un isomorphisme d'anneaux\footnote{C'est également un isomorphisme de corps au sens où \( f_{A^{-1}}=f_A^{-1}\), mais nous n'avons pas encore étudié les matrices inversibles. Voir \ref{}.} et un isomorphisme d'algèbres\quext{J'espère avoir vérifié toutes les conditions. Soyez prudent et systématique.}.
\end{theorem}

\begin{proof}
    Plusieurs choses à prouver.
    \begin{subproof}
        \item[La petite formule]
            En utilisant le fait que \( (e_j)_k=\delta_{jk}\) nous avons
            \begin{equation}
                f_A(e_j)=\sum_{kl}A_{kl}(e_j)_le_k=\sum_kA_{kj}e_k,
            \end{equation}
            ce qui donne \( A_{kj}=f_A(e_j)_k\).
        \item[Injective]
            Soient deux matrices telles que \( f_A=f_B\). En utilisant la formule à peine prouvée,
            \begin{equation}
                A_{ij}=f_A(e_j)_i=f_B(e_j)_i=B_{ij},
            \end{equation}
...
\begin{proposition}[Réduction de Gauss\cite{MonCerveau}]
...
\begin{proof}
    ...
    \end{equation}
    Donc \( C\) est également un inverse à droite de \( A\). Nous avons donc en particulier \( A(B-C)=0\).
\end{proof}
<++>
