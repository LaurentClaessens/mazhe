...
\begin{enumerate}
    ...
    \item Par ailleurs, sur les matrices carrées d'ordre \( n \) fixé, le produit de deux matrices est bien défini. Ainsi, \( \eM(n,\eA)\) se voit conférer une structure d'anneau, dont le neutre pour la multiplication est la matrice carrée\footnote{En toute rigueur, nous devrions préciser la taille de cette matrice dans la notation, mais elle sera toujours claire dans le contexte.} \( \mtu\) donnée par
\begin{equation}
    \mtu_{ij}=\begin{cases}
        1    &   \text{si } i=j\\
        0    &    \text{sinon.}
