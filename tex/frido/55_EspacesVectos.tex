...
\begin{rem}
On note les isomorphismes naturels \( \eM(1\times m,\eA) ~ \eA^m\) et \( \eM(n\times 1,\eA) ~ \eA^n\).
\end{rem}
...
On dit qu'une matrice \( A \) est \defe{orthogonale}{matrice!orthogonale} si son inverse est sa transposée, c'est-à-dire si \( A^{-1} = A^t \). On note \( O(n,\eA) \) l'ensemble des matrices carrées d'ordre \( n \) à coefficients dans \( \eA \), qui sont orthogonales.
\end{definition}
...
\begin{example}
	...
	\begin{description}
		...
		En choisissant des coordonnées adéquates, les droites peuvent être aussi vues commes graphes de fonctions affines. Dans le cas de $\eR^2$, on retrouve la fonction de l'exemple~\ref{ex_affine}, pour \( n = m = 1 \).
...
Nous multiplions l'égalité \eqref{EQooFRQRooSMsQQB} par \( Q^{-1}_{\alpha j}\) \footnote{Attention à la bonne interptétation de ce nombre: on fait bien référence à l'élément situé en \( (\alpha, j) \) de la matrice \( Q^{-1} \), et pas autre chose.} et nous sommons sur \( \alpha\) :
\begin{equation}
