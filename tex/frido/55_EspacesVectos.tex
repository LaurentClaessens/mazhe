...
\begin{propositionDef}
    ...
    \begin{equation}
        AB=BA=\mtu.
    \end{equation}
    Si une telle matrice existe, la matrice \( A\) est dite \defe{inversible}{inversible!matrice}, et la matrice \( B\) ainsi définie est notée \( A^{-1}\).
\end{propositionDef}
...
\begin{definition}[\cite{ooKBOMooSkKHvu}]
...
\begin{proposition}[Permuter des lignes ou des colonnes \( L_k\leftrightarrow L_l\)\cite{ooKBOMooSkKHvu,MonCerveau}]
...
\begin{proposition}[\cite{ooKBOMooSkKHvu}]      \label{PROPooZIVNooUPsBtr}
...
\begin{proposition}[\cite{ooKBOMooSkKHvu}]      \label{PROPooYMFQooOYCpEX}
    Soit une matrice \( A\in \eM(n,\eK)\). Nous considérons la matrice \( B\) obtenue à partir de \( A\) par la substitution de lignes \( L_k\to L_k+\lambda L_l\). Alors
    \begin{equation}
        \det(A)=\det(B).
    \end{equation}
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation}
        \det(B)=\det(A)+\det(C).
    \end{equation}
    Mais dans la matrice \( C\), nous avons \( L_k=\lambda L_l\), ce qui implique \( \det(C)=0\) par la proposition \ref{PROPooZIVNooUPsBtr}\ref{ITEMooWBUTooGuGkmN}. Donc \( \det(A)=\det(B)\) comme il se devait.
\end{proof}
...
\begin{proposition}
    Si \( A\) est une matrice et si \( B\) est la matrice obtenue à partie de \( A\) par la substitution \( L_k\to \lambda L_k\), alors
    \begin{equation}
        B=TA
    \end{equation}
    avec \( T=\delta+(\lambda-1)E_{kk}\).

    C'est à dire que a matrice \( T\) est une matrice de multiplication de ligne par un scalaire.
\end{proposition}

\begin{proof}
    C'est un cas particulier de la proposition \ref{PROPooILVTooTnbfxU} en prenant \( k=l\) et en adaptant le \( \lambda\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]
    Soient les matrices déjà vues
    \begin{itemize}
        \item permutation de lignes : $S=\delta+E_{kl}+E_{lk}-E_{kk}-E_{ll}$,
        \item ajout d'un multiple d'une ligne à une autre : $U=\delta+\lambda E_{kl}$,
        \item multiplication d'une ligne par un scalaire : $T=\delta+(\lambda-1)E_{kk}$.
    \end{itemize}
    Nous avons les déterminants
    \begin{subequations}
        \begin{align}
            \det(S)&=-1\\
            \det(T)&=\lambda\\
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\( \det(T)\)]
            Nous calculons le déteminant de la matrice \( T=\delta+(\lambda-1)E_{kk}\). La formule du déterminant donne
            \begin{equation}
...
\begin{definition}      \label{DEFooSNDJooXODUgR}
    ...
    La \defe{matrice}{matrice d'une application linéaire} de \( t\) pour ces bases est la matrice \( A\in \eM(\eK)\) dont les éléments sont donnés par
    \begin{equation}
...
\begin{normaltext}
    ...
    \begin{equation}        \label{EQooPEPQooOqcIZn}
...
\begin{normaltext}      \label{NORMALooHNPMooFvFYCC}
...
Si \( \eK\) est un corps commutatif, l'espace vectoriel \( \eK^n\) est particulier, parce qu'il a une base canonique. Si des matrices \( A,B\in \eM(n,\eR)\) sont données, nous pouvons leur associer des applications linéaires \( f_A\) et \( f_B\).
...
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooIYVQooOiuRhX}
    Soit un corps commutatif \( \eK\). Nous notons \( f_A\colon \eK^n\to \eK^n\) l'application linéaire associée à la matrice \( A\in \eM(n,\eK)\) par la formule\footnote{Qui n'est rien d'autre que \eqref{EQooPEPQooOqcIZn}.}
    \begin{equation}
        f_A(x)=\sum_{ij}A_{ji}x_ie_j.
    \end{equation}
    Nous considérons l'application
    \begin{equation}
        \begin{aligned}
            \psi\colon \eM(n,\eK)&\to \End(\eK^n) \\
            A&\mapsto f_A 
        \end{aligned}
    \end{equation}
    ainsi définie.
    \begin{enumerate}
        \item
            L'application \( \psi\) est un morphisme d'anneaux\footnote{Il n'est cependant pas question de dire que c'est un isomorphisme parce qu'il y a des éléments non inversibles autant dans \( \eM(n,\eK)\) que dans \( \End(\eK^n)\). Quant à dire que c'est un morphisme de groupes, n'y pensez même pas parce que ni \( \eM(n,\eK)\) ni \( \End(\eK^n)\) ne sont des groupes.}, c'est à dire que
        \begin{equation}
            f_{AB}=f_A\circ f_B.
        \end{equation}
    \item
...
\begin{proof}
    En utilisant la convention \eqref{EQooPEPQooOqcIZn} à propos du lien entre matrice et applications linéaires, nous avons d'une part
    \begin{equation}
        f_{AB}(e_k)=\sum_{ij}(AB)_{ij}(e_k)_je_i=\sum_{ij}(AB)_{ij}\delta_{kj}e_i=\sum_i(AB)_{ik}e_i.
    \end{equation}
    Et d'autre part,
    \begin{subequations}
        \begin{align}
            (f_A\circ f_B)(e_k)&=f_A\big( \sum_iB_{ik}e_i \big)\\
            &=\sum_iB_{ik}f_A(e_i)\\
            &=\sum_{ij}B_{ik}A_{ji}e_j\\
            &=\sum_{ij}(AB)_{jk}e_j.
        \end{align}
    \end{subequations}
    Nous avons donc égalité \( f_{AB}(e_k)=(f_A\circ f_B)(e_k)\). Égalité sur une base implique égalité sur tout l'espace (application linéaire).
\end{proof}
