...
\begin{proposition}
    En ce qui concerne les déterminants\footnote{Définition \ref{LEMooQTRVooAKzucd} pour les applications linéaires et \ref{DEFooYCKRooTrajdP} pour les matrices.}, nous avons
    \begin{enumerate}
    \item
...
\begin{normaltext}
    ...
    \begin{equation}        \label{EQooPEPQooOqcIZn}
        f_A(x)=\sum_ix_if_A(e_i)=\sum_{i\alpha}f_A(e_i)_{\alpha}f_{\alpha}=\sum_{i\alpha}A_{\alpha i}x_if_{\alpha}.
    \end{equation}
    Cela peut encore s'écrire
    \begin{equation}
        f_A(x)_{\alpha}=\sum_iA_{\alpha i}x_i=(Ax)_{\alpha}
    \end{equation}
    ...
    \begin{equation}
        f_A(x)=Ax.
    \end{equation}
    Cette écriture n'est valable que si les bases de \( E\) et de \( F\) sont ultra-fixées. En effet, elle signifie que le vecteur \( f_A(x)\in F\) est le vecteur dont les composantes dans la base \( \{ f_{\alpha} \}\) sont données par le produit de la matrice \( A\) par l'élément de \( \eK^n\) dont les composantes sont celles de \( x\) dans la base \( \{ e_i \}\).
\end{normaltext}
...
\begin{normaltext}
    Si \( \eK\) est un corps commutatif, l'espace vectoriel \( \eK^n\) est particulier, parce qu'il a une base canonique. Si des matrices \( A,B\in \eM(n,\eR)\) sont données, nous pouvons leur associer des applications linéaires \( f_A\) et \( f_B\) sans danger. En effet, qui irait utiliser sur \( \eK^n\) une base autre que la base canonique ? Hum \ldots hum \ldots Soyez tout de même prudent.
\end{normaltext}
