    ...
    \begin{subproof}
        \item[\ref{ITEMooSOHWooEHJFZD}]
            ...
            \begin{equation}
                B_{ij}=A_{i \alpha(j)},
            \end{equation}
            ou encore : \( A_{ij}=B_{i\alpha(j)}\). Pour \( \sigma\in S_n\) donné nous avons
            \begin{equation}
                \prod_{i=1}^nA_{i\sigma(i)}=\prod_{i=1}^nB_{i(\alpha \sigma)(i)},
            \end{equation}
            donc
            \begin{equation}
                \det(A)=\sum_{\sigma\in S_{n}}\epsilon(\sigma)\prod_{i=1}^nA_{i\sigma(i)}=\sum_{\sigma\in S_{n}}\epsilon(\sigma)\prod_{i=1}^nB_{i(\alpha\sigma)(i)}.
            \end{equation}
            C'est le moment d'utiliser la proposition \ref{PROPooWJQQooFINSEc} à propos de somme sur des groupes avec \( G=S_n\) et 
            \begin{equation}
                f(\sigma)=\epsilon(\sigma)\prod_{i=1}^nB_{i\sigma(i)}.
            \end{equation}
            Nous avons \( \sum_{\sigma\in S_n}f(\sigma)=\sum_{\sigma\in S_n}f(\alpha\sigma)\).
