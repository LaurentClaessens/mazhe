...
\begin{theorem}[Théorème de la base incomplète] \label{ThonmnWKs}
    ...
    \begin{enumerate}
        ...
        \item     \label{ItemBaseMemeCardinal}
...
\begin{definition}
    La \defe{dimension}{dimension} d'un espace vectoriel de type fini est le cardinal d'une\footnote{Le théorème de la base incomplète~\ref{ThonmnWKs}\ref{ItemBasesMemeCardinal} montre que cette définition ne souffre d'aucune ambiguïté.} de ses bases.
\end{definition}
...
\begin{definition}
    Soit un anneau \( \eA\) ainsi que des entiers \( m\), \( n\) strictement positifs. L'ensemble \( \eM(n\times n,A)\) est l'ensemble des applications
    \begin{equation}
        \{ 1,\ldots, n \}\times \{ 1,\ldots, m \}\to \eA,
    \end{equation}
    et est appelé ensemble des \defe{matrices}{matrice} \(n\times m\) sur \( \eA \).
\end{definition}
Si \( A\) est une matrice, nous notons \( A_{ij}\) au lieu de \( A(i,j)\) l'image de \( (i,j)\) par l'application \( A\). Si \( n=m\) alors nous disons que la matrice est \defe{carrée}{carrée!matrice}, et nous notons \( \eM(n,\eA)\) au lieu de \( \eM(n\times n,\eA)\).

Si \( A,B\in \eM(n\times m,\eA ) \) et \( \lambda\in \eA \),  nous définissons
\begin{description}
...
\end{description}
La somme et le produit par un scalaire confèrent à \( \eM(n\times m,\eA)\) une structure de \( \eA \)-module.

La notion de produit de deux matrices est à peine plus subtile; c'est une application
\begin{equation}
    \begin{aligned}
...
Quelques remarques méritent d'être soulignées.
\begin{remark}
\begin{enumerate}
    ...
    \item Par ailleurs, sur les matrices carrées d'ordre \( n \) fixé, le produit de deux matrices est bien défini. Ainsi, \( \eM(n,\eA)\) se voit conférer une structure d'anneau, dont le neutre pour la multiplication est la matrice carrée \( \mtu_n\) (notée aussi \( \mtu\) lorsqu'il n'y a pas d'ambiguïté sur l'ordre), donnée par
\begin{equation}
    \mtu_{ij}=\begin{cases}
        1    &   \text{si } i=j\\
        0    &    \text{sinon.}
...
\begin{lemma}[\cite{MonCerveau}]\label{LemInvarianceCycliqueTrace}
    Nous avons :
    \begin{enumerate}
        \item
            \( (AB)^t=B^tA^t\),
...
\begin{proof}
    Supposons que \( B_1\) et \( B_2\) soient des inverses de \( A\). Considérons le produit de matrices \( B_2AB_1 \). En utilisant le fait que \( B_1\) soit inverse à droite, on obtient
   \begin{equation}
        B_2(AB_1) = B_2 \mtu = B_2.
    \end{equation}
Mais comme \( B_2\) est inverse à gauche, on a aussi
    \begin{equation}
        (B_2A)B_1 = \mtu B_1 = B_1.
    \end{equation}
    Il s'ensuit que \( B_2=B_1\), d'où l'unicité.
\end{proof}
% TODO: généraliser à un anneau quelconque!

\begin{definition}[Groupe linéaire de matrices]
On note \( \GL(n,\eA) \) l'ensemble des matrices carrées d'ordre \( n \) à coefficients dans \( \eA \), qui sont inversibles. En d'autres termes, \( \GL(n,\eA) = U (\M (n,\eA) \).
\end{definition}
...
\begin{proof}
    D'abord il y a des implicites dans l'énoncé. Vu que nous voulons soit par hypothèse soit par conclusion que les matrices \( A\) et \( B\) soient équivalentes, nous supposons qu'elles ont même dimension. Soient donc \( A\) et \( B\) deux matrices carrées représentant des applications linéaires de l'espace vectoriel \( E=\eK^n\) vers lui-même.
...
\begin{probleme}
Comme il a été mentionné en début de section, on n'a pas défini ce qu'est une application linéaire. Il faut donc revoir la preuve ci-dessus... ou définir les applications linéaires avant.
\end{probleme}
...
\begin{proposition}[Algorithme des facteurs invariants\cite{KXjFWKA}]   \label{PropPDfCqee}
    Soit \( (A,\delta)\) un anneau euclidien muni de son stathme  et \( U\in \eM(n \times m,A)\). Alors il existe \( d_1,\ldots, d_s\in A^*\) et des matrices \( P\in\GL(m,A)\), \( Q\in \GL(n,A)\) tels que nous ayons
    \begin{equation}
        U=P \begin{pmatrix}
            \begin{matrix}
                d_1    &       &       \\
...
\begin{definition}  \label{DefDQRooVGbzSm}
    Si \( E\) et \( F\) sont des espaces vectoriels nous munissons \( \aL(E, F)\) d'une structure d'espace vectoriel en définissant la somme et le produit par un scalaire de la façon suivante. Si $T$ et $U$ sont des élément de $\aL(E,F)$ et si $\lambda$ est un réel, nous définissons les éléments $T+U$ et $\lambda T$ par
    \begin{enumerate}
        \item
            $(T+U)(x)=T(x)+U(x)$;
        \item
...
\begin{normaltext}
    Pour bien faire, il faudrait vérifier que la définition~\ref{DefDQRooVGbzSm} produit effectivement une structure d'espace vectoriel sur \( \aL(E,F)\). Par ailleurs, remarquons au besoin que les définitions font appel à la structure de \( F \).
\end{normaltext}
...
% TODO: comme pour les matrices, c'est un résultat "d'anneau", avec peut-être une subtilité sur les domaines, à préciser.
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    et nous voulons prouver que c'est une bijection. Étant donné que nous sommes en dimension finie, nous avons égalité des dimensions de \( \eM_n(\eK)\) et \( \left(\eM(n,\eK)\right)^*\), et il suffit de prouver que \( f\) est injective. Soit donc \( A\) telle que \( f_A=0\). Nous l'appliquons à la matrice \( (E_{ij})_{kl}=\delta_{ik}\delta_{jl}\) :
    \begin{equation}
            0=f_A(E_{ij})
            =\sum_{k}(AE_{ij})_{kk}
...
\begin{proof}
    ...
    \begin{equation}
        \Tr(AMN)=\Tr(ANM)
    \end{equation}
    pour toutes matrices \( M, N\in \eM(n,\eK)\). L'invariance cyclique de la trace\footnote{Lemme~\ref{LemInvatianceCycliqueTrace}.} appliqué au membre de droite nous donne \( \Tr(AMN)=\Tr(MAN)\), ce qui signifie que
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
la dernière égalité découlant de l'invariance cyclique de la trace\footnote{Lemme~\ref{LemInvarainceCycliqueTrace}.}. Ce que nous cherchons est \( M\in \GL(n,\eK)\) telle que \( \phi(M)=0\). Nous commençons par trouver \( N\in\GL(n,\eK)\) telle que \( \Tr(J_rN)=0\). Celle-là est facile : c'est
    \begin{equation}
        N=\begin{pmatrix}
            0    &   1    \\
            \mtu_{n-1}    &   0
