...
\begin{normaltext}		\label{NORMooANZNooTUhDdk}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooOMUHooYFXJSY}
	Si \( E\) est un espace vectoriel de dimension \( d\) sur le corps \( \eK\), et si \( \{ e_1,\ldots,e_d \}\) est une base, alors l'application
	\begin{equation}
		\begin{aligned}
		...
		\end{aligned}
	\end{equation}
	est une bijection linéaire.
	%TODOooBVTOooMqYNRG. Prouver ça.
\end{proposition}
...
\begin{proposition}	\label{PROPooEQYBooMYIFKR}
	Si \( V\) est un espace vectoriel sur le corps \( \eK\) et si \( \lambda\in \eK\), l'application
	\begin{equation}
		\begin{aligned}
			T_{\lambda}\colon V & \to V             \\
			x                   & \mapsto \lambda x
		\end{aligned}
	\end{equation}
	est linéaire.
...
\begin{proof}
	Soient \( \lambda,\mu\in \eK\) ainsi que \( a,b\in V\). Nous avons
	\begin{subequations}
		\begin{align}
			T_{\lambda}(a+b) & =\lambda(a+b)                                                                                \\
			                 & =\lambda a+\lambda b           & \text{cf. \ref{NORMooANZNooTUhDdk}\ref{ITEMooYQMWooNNvHSb}} \\
			                 & =T_{\lambda}(a)+T_{\lambda}(b)
		\end{align}
	\end{subequations}
	et
	\begin{subequations}
		\begin{align}
			T_{\lambda}(\mu a) & =\lambda(\mu a)                                          \\
			                   & =\mu (\lambda a)     & \text{\( \eK\) est comm. et ass.} \\
			                   & =\mu T_{\lambda}(a).
		\end{align}
	\end{subequations}
\end{proof}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooARZGooMOlgsl}
	Soient des espaces vectoriels \( E,F\) avec \( \dim(E)\geq \dim(F)\) et une application linéaire de rang maximum \(f \colon E\to F  \). Soit une base \( \{ e_1,\ldots,e_m \}\) de \( E\). Alors il existe une une application linéaire \(\phi \colon E\to E  \) qui
	\begin{enumerate}
		\item
		      est une permutation des vecteurs de base,
		\item
		      \( (f\circ\phi)\big( \Span\{ e_1,\ldots,e_n \} \big)=\eR^n\).
	\end{enumerate}
	%TODOooCVSKooLXfzrV. Prouver ça.
\end{lemma}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooOMZUooDvwbhf}
	Soient des espaces vectoriels \( E\) et \( F\) de dimension finie. Si \(f \colon E\to F  \) est une application linéaire injective, alors
	\begin{enumerate}
		\item		\label{ITEMooSTNLooGfZYMp}
		      \( \rank(f)=\dim(E)\).
		\item		\label{ITEMooTAONooSUTnHq}
		      \( \dim(E)\leq \dim(F)\).
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	En deux parties.
	\begin{subproof}
		\spitem[Pour \ref{ITEMooSTNLooGfZYMp}]
		%-----------------------------------------------------------
		Soit une base \( \{ e_1,\ldots,e_n \}\)	de \( E\). Étant donné que \( f\) est injective, la partie \( \{ f(e_1),\ldots,f(e_n) \}\) est libre dans \( F\). En effet si \( \sum_i a_if(e_i)=0\) alors \( f(\sum_ia_ie_i)=0\) et donc \( \sum_ia_ie_i=0\) parce que \( f\) est injective. La dernière égalité implique que \( a_i=0\) parce que \( \{ e_i \}_{i=1,\ldots,n}\) est libre.

		Donc
		\begin{equation}
			\rank(f)=\dim\big( f(E) \big)\geq n=\dim(E).
		\end{equation}

		D'autre part le théorème du rang, sous forme de l'équation \ref{EQooUEOQooLySRiE} dit que \( \rank(f)\leq \dim(E)\).
		\spitem[Pour \ref{ITEMooTAONooSUTnHq}]
		%-----------------------------------------------------------
		Nous avons déjà prouvé que \( \{ f(e_i) \}_{i=1,\ldots,n}\) est une partie libre de \( F\). Le théorème de la base incomplète \ref{THOooOQLQooHqEeDK} dit alors qu'il existe une base de \( F\) contenant cette partie libre. Vu que \( \Card\big( \{ f(e_i) \}_{i=1,\ldots,n} \big)=n\), nous avons \( \dim(F)\geq n\).
	\end{subproof}
\end{proof}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooVGDDooRmUDUi}
	Soient une application linéaire \(f \colon E\to F  \) ainsi que \( v_1,\ldots,v_2\in E\). Alors
	\begin{equation}
		\Span\big( \{ f(v_i) \}_{i=1,\ldots,k} \big)=f\big( \Span(\{ v_i \}_{i=1,\ldots,k}) \big).
	\end{equation}
\end{lemma}

\begin{proof}
	Dans un sens, un élément générique de \( \Span\big( \{ f(v_i) \}_{i=1,\ldots,k} \big)\) est
	\begin{equation}
		\sum_{i=1}^ka_jf(v_i)=f\big( \sum_ia_iv_i \big)\in f\big( \Span(\{ v_i \}_{i=1,\ldots,k}) \big).
	\end{equation}
	Dans l'autre sens, un élément générique de \( f\big( \Span(\{ v_i \}_{i=1,\ldots,k}) \big)\) est
	\begin{equation}
		f\big( \sum_ia_iv_i \big)=\sum_ia_if(v_i)\in\Span\big( \{ f(v_i) \}_{i=1,\ldots,k} \big).
	\end{equation}
\end{proof}
