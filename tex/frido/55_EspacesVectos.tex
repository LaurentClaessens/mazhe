...
\begin{proposition}[\cite{ooKBOMooSkKHvu}]
...
\begin{proposition}[\cite{ooKBOMooSkKHvu}]
    Soit une matrice \( A\in \eM(n,\eK)\). Nous considérons la matrice \( B\) obtenue à partir de \( A\) par la permutation de lignes \( L_k\leftrightarrow L_l\). Alors
    \begin{equation}
        B=SA
    \end{equation}
...
\begin{proposition}[\cite{MonCerveau}]
    ...
    \begin{subequations}
        \begin{align}
            S&=\delta+E_{kl}+E_{lk}-E_{kk}-E_{ll}\\
            U&=\delta+\lambda E_{kl}\\
            T&=\delta+(\lambda-1)E_{kk}
        \end{align}
    \end{subequations}
