...
%TODO: cette prop. ne fait pas état d'une écriture avec \theta_1 ou \theta_2... ce qui est utile pour la preuve plus bas, j'en conviens ! Il faut trouver autre chose.
	\begin{equation}
...
Soit \( \Omega=\mathopen] 0 , 1 \mathclose[\) et l'équation différentielle
	...
	En posant \( c_i=c(x_i)\) et \( u_i=u_h(x_i)\), les inconnues du problème sont les nombres \( u_i\) (\( i=1,\ldots, N\)). Elles sont immédiatement résolues pour \( u_0\) et \( u_{N+1}\). Pour les autres, il faut écrire et résoudre un système d'équations linéaire.
...
\begin{definition}
	...
	Le schéma discret \( (L_hu_h)(x_i)=f_i\) est \defe{convergent}{convergent!schéma discret} si \( \lim_{h\to 0} \| e_h \|=0\). Si de plus, il existe une constante \( C\), et \( p>0\), tels que
	\begin{equation}
		\| e_h \|\leq Ch^p,
	\end{equation}
