...
% Copyright (c) 2010-2016,2018-2020, 2022-2026
...
\begin{proposition}[\cite{MonCerveau}]     \label{PROPooJYGVooShNewy}
	...
	Nous supposons que \( \gamma^{-1}\colon \Gamma\to \mathopen[ 0 , 1 \mathclose]\) est continue\quext{Je ne suis pas certain que cette hypothèse soit indispensable.}
...
La proposition suivante dit essentiellement que la longueur d'un chemin est minoré par la longueur de son graphe.
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooXENVooMvkTZW}
...
\begin{proposition}[\cite{MonCerveau}]     \label{PROPooVXDNooPZYKPr}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Connexité par arcs : le chemin]
		...
		\spitem[Minoration de la longueur de \( \alpha\)]
...
\begin{definition}
	Le repère orthonormal \( \{ \tau(s), \nu(s),\beta(s) \}\) est le \defe{repère de Frenet}{repère!de Frenet} au point \( \gamma_N(s)\).
\end{definition}
...
\end{equation}
Ensuite, nous dérivons la définition \( \beta(s)=\tau(s)\times\nu(s)\) en utilisant la formule de Leibniz \eqref{EqFormLeibProdscalVect} :
\begin{equation}
	\beta'(s)=\tau'(s)\times\nu(s)+\tau(s)\times\nu'(s).
\end{equation}
...
\begin{equation}        \label{eqProdvectogpgpp}
	...
	\end{aligned}
\end{equation}
parce que le deuxième terme dans la parenthèse est un multiple de \( \gamma'(t)\), de telle sorte que son produit vectoriel avec \( \gamma'(t)/\| \gamma'(t) \|\) soit nul. En prenant la norme,
...
Voici un théorème qui résume ce que nous venons de faire.
...
\begin{definition}[\cite{ooIEJXooIYpBbd}]\label{DEFooFAENooXWgBmh}
	...
	Nous nommons aussi \defe{nombre de tours}{nombre!tours d'une courbe plane} de \( \gamma\)  le nombre \( \Turn(\gamma)=K/2\pi\).
\end{definition}
...
\begin{lemma}[\cite{ooIEJXooIYpBbd}]
	Soit une courbe régulière de classe \( C^2\) \( \gamma\colon \mathopen[ a , b \mathclose]\to \eR^2\). Sa courbure totale\footnote{Définition \ref{DEFooFAENooXWgBmh}.} est donnée par
	\begin{equation}
		K=\theta(b)-\theta(a).
	\end{equation}
\end{lemma}
...
\begin{proof}
	Il suffit de remplacer dans la définition \eqref{EQooTIFWooQflOfd} de la courbure totale l'intégrande par son expression du lemme~\ref{LEMooWLAUooKetUiW} :
	\begin{equation}
...
\begin{lemmaDef}        \label{DEFooTKBUooNVcheO}
	Soit une application continue \( \phi\colon S^1\to S^1\). Un \defe{relèvement}{relèvement} de \( \phi\) est une application continue \( \varphi\colon \eR\to \eR\) une application continue telle que
	\begin{equation}
...
Les définitions à propos d'homotopie sont \ref{DEFooHJQTooYUFcee} et \ref{DEFooLXDTooDPgxqL}.



\begin{lemma}[\cite{MonCerveau}]        \label{LEMooTDTAooCRSYzI}
...
\begin{lemma}[\cite{BIBooQKARooMHqitK}]     \label{LEMooQNFCooWorCxN}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Si \( \mathopen\lbrack a , t \mathclose\lbrack\subset S\)]
		% -------------------------------------------------------------------------------------------- 
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Le degré pour les applications \( S^1\to S^1\)]
		...
		\spitem[Indice de rotation pour des courbes dans \( \eR^2\)]
...
\begin{theorem}[\cite{ooIEJXooIYpBbd}]      \label{THOooEQWOooBCRMMZ}
	Pour une courbe simple fermée de classe \( C^2\) dans \( \eR^2\),
	\begin{enumerate}
		\item
		      le nombre de tours\footnote{Nombre de tours et courbure totale, définition \ref{DEFooFAENooXWgBmh}.} vaut \( \pm 1\),
		\item
		      la courbure totale vaut \( \pm 2\pi\).
	\end{enumerate}
\end{theorem}

\begin{proof}
	Soit une telle courbe \( \gamma\) et son image \( \Gamma\). Notons tout de suite que les deux points sont équivalents parce que \( K=2\pi\Turn(\gamma)\).
	\begin{subproof}
		\spitem[Choix d'un point et d'une tangente]
		...
		\end{equation}
		où \( \alpha\) est une fonction vérifiant \( \lim_{t\to 0}  \alpha(t)/t =0\). Nous avons à calculer la limite de
		\begin{equation}
		...
		\spitem[La géométrie de \( \sigma_2\)]
		...
		\spitem[Un relèvement pour \( \sigma_2\)]
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		avec \( \theta(2\pi)\) défini par continuité. Nous allons cependant voir, en étant plus prudent, que cette définition n'assure pas la continuité (surtout en \( t=\pi\)).
...
\begin{proof}
	...
	\begin{subproof}
		...
		\end{equation}
		est fermé. Pour cela nous montrons que son complémentaire est ouvert. Soit \( r\not\in B\). Soit \( a\in \eR^2\); nous allons prouver que \( q_{a,r'}\) est hors de \( A\) pour tout \( r'\in B(r,\epsilon)\). Nous savons que \( q_{a,r'}\not\in A\), c'est à dire qu'il existe \( t_0\in \mathopen[ 0,1,\mathclose]\) tel que \( q_{a,r}\big( \gamma(t_0) \big)>0\). Étant donné que cette expression est continue en \( r\), il existe un voisinage \( B(r,\epsilon)\) tel que \( q_{ar,'}\big( \gamma(t_0) \big)>0\) pour tout \( r'\in B(r,\epsilon)\).
		\spitem[Unicité]
		...
		\spitem[Au moins deux intersections]
		...
		\begin{subproof}
			\spitem[Impossible d'avoir zéro intersection]
			%-----------------------------------------------------------
			...
			\spitem[Impossible d'avoir une seule intersection]
			%-----------------------------------------------------------
		...
		% il faut une ligne vide ici, sinon le sous-titre disparaît
		\spitem[Intersection avec les demi-arcs]
		%-----------------------------------------------------------
...
\begin{proof}
	...
	\end{equation}
	qui est bien définie parce que \( \gamma_x\) est une bijection entre \( B(0,\delta)\) et son image. La fonction \( g\) est continue par le théorème de la bijection \ref{ThoKBRooQKXThd}, et même dérivable par la proposition~\ref{PROPooSGTBooFxUuXK}. De plus, vu la formule \eqref{EQooELIHooDxUFxH}, la fonction \( g^{-1}\) est de classe \( C^1\) parce que \( (g^{-1})'\) est une composée d'applications continues.
	...
	\begin{subproof}
		...
		\spitem[Si \( g\) est seulement de classe \( C^1\)]
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Il existe \( q\) tel que \( p\in l_q\)] Vu que \( D\) est fermé, le point \( p\) est dans \( D\), et donc dans tous les \( H_q\). Supposons qu'il soit dans l'intérieur de tous les \( H_q\). Alors nous considérons la fonction
		\begin{equation}
			r(q)=\frac{ d(p,l_q) }{ 2 }
		\end{equation}
		définie sur \( \Gamma\). C'est une fonction continue\footnote{L'équation de la droite \( l_q\) a des coefficients continus parce que \( \gamma\) est de classe \( C^1\).} strictement positive définie sur le compact \( \Gamma\) qui possède donc un minimum strictement positif. Si \( r_0\) est ce minimum, alors \( B(p,r_0)\) est incluse à tous les \( H_q\), ce qui ferait que \( p\) est à l'intérieur de \( D\). Nous concluons à l'existence de \( q\in \Gamma\) tel que \( p\in l_q\).
		\spitem[Le point où ça décolle]
		...
		\spitem[Graphe d'une fonction strictement convexe] En suivant le lemme~\ref{LEMooGEVEooHxPTMO}, l'ensemble \( \Gamma\) est localement (autour de \( r\)) le graphe d'une fonction convexe au-dessus de \( l_r\). Soit \( g\colon B(0,\delta)\to \eR^2\) cette fonction convexe.
...
À propos de paramétrage normal.

\begin{lemma}       \label{LEMooHMFSooFlhanD}
	Soit \(   \gamma\colon \mathopen[ 0 , L \mathclose]\to \eR^2    \) une courbe simple, fermée de classe \( C^2\) en paramétrage normal. Nous considérons sa fonction d'angle \(\theta \colon \mathopen[ 0,L\mathclose]\to \eR  \).
...
\begin{proof}
	Le théorème~\ref{THOooEQWOooBCRMMZ} nous dit que si \( \theta(0)=a\) alors \( \theta(2\pi)\) ne peut valoir que \(a+2\pi\) ou \( a-2\pi\). Le théorème des valeurs intermédiaires nous dit alors que \( \theta\) prend toutes les valeurs entre \( a\) et \( a+2\pi\) ou \( a-2\pi\).
\end{proof}
...
\begin{proof}
	Nous considérons la courbe \( \gamma\) munie d'un paramétrage de vitesse \( 1\), c'est-à-dire avec \( \| \gamma'(t) \|=1\) pour tout \( t\). Si \( \theta\) est sa fonction d'angle\footnote{Définition \ref{LEMDEFooLPWJooAnWZjb}.}, alors nous avons \( \theta'=\kappa\) par le lemme~\ref{LEMooWLAUooKetUiW}. Donc la fonction \( \theta\) est monotone si et seulement si la courbure ne change pas de signe. Nous allons montrer que \( \theta\) est monotone si et seulement si \( \gamma\) est convexe.
...
\begin{proof}
	...
	\begin{subproof}
		...
		Donc il existe \( a\in A_x\) tel que \( a\in\mathopen] \inf(A_x) , s \mathclose[\). Vu que \( a\in A_x\), nous avons \( x\in U_a\), et vu que \( a<s\), nous avons \( \bar U_a\subset U_s\). Donc
			...
			\spitem[\( \{ x\tq f(x)<t \}\) est ouvert pour tout \( t\in \eR\)]
			% -------------------------------------------------------------------------------------------- 
			...
			\spitem[\( \{ x\tq f(x)\leq t \}\) est fermé pour tout \( t\in \eR\)]
			% -------------------------------------------------------------------------------------------- 
...
\begin{theorem}[Théorème d'Urysohn\cite{BIBooQKARooMHqitK}]     \label{THOooKYYEooLFcNpg}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\( D\) est dense dans \( \mathopen\lbrack 0 , 1 \mathclose\rbrack\)]
		% -------------------------------------------------------------------------------------------- 
		...
		\begin{subproof}
			...
			\begin{subequations}
				...
				\end{align}
			\end{subequations}
			\spitem[Si \( k\) est pair et \( l\) est impair]
			% -------------------------------------------------------------------------------------------- 
			Laissé aux bons soins \randomGender{du lecteur}{de la lectrice}. J'ai pas vérifié, mais \cite{BIBooQKARooMHqitK} dit que c'est similaire. Écrivez-moi si vous voyez un problème.
			\spitem[Si \( k\) et \( l\) sont impairs]
			% -------------------------------------------------------------------------------------------- 
			Laissé aux bons soins \randomGender{du lecteur}{de la lectrice}. J'ai pas vérifié, mais \cite{BIBooQKARooMHqitK} dit que c'est similaire. Écrivez-moi si vous voyez un problème.
		\end{subproof}
		\spitem[Résumé]
		...
		\spitem[Ce \( f\) est ce qu'il nous fait]
		% -------------------------------------------------------------------------------------------- 
...
\begin{normaltext}
	Les fractions de la forme \( a/2^b\) sont nommées \defe{fractions dyadiques}{fraction dyadique} et nous avons déjà vu qu'elles sont denses dans \( \eR\) dans le corolaire \ref{CorDensiteDyadiques}.
\end{normaltext}
