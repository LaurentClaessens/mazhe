...
\begin{lemma}[\cite{MonCerveau}]        \label{LEMooTDTAooCRSYzI}
    ...
    Alors \( \gamma\) est homotope au segment joignant \( \gamma(a)\) et \( \gamma(b)\).
\end{lemma}
...
\begin{probleme}
    À mon avis, la réponse acceptée et la plus votée dans \url{https://math.stackexchange.com/q/2813209} est fausse parce que choisir les \( \tau\) revient à prouver que l'ensemble sur lequel c'est possible est inductif.

    Si vous savez comment combler le trou dans cette preuve, n'hésitez pas à me le faire savoir parce que je trouve qu'utiliser le sous-recouvrement fini sur l'image de \( \gamma\) un moyen plus direct que ce qui est donné ici.
\end{probleme}
