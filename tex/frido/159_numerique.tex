...
\begin{equation}
    L_{k+1}=P_k^{-1}L_{k-1}P_kM_k^{-1}.
\end{equation}
où \( M_k^{-1}\) est connue par ses paramètres. La matrice \( M_k^{-1}\) est de la forme
\begin{equation}
    (M_k^{-1})_{ij}=\delta_{ij}+\delta_{jk}a_i
\end{equation}
avec \( a_i=0\) si \( i<k\). Ces matrices sont à multiplier par des matrices triangulaires inférieures \( T\). Voyons-en la règle. Si \( T\) est triangulaire inférieure, \( T_{ij}=0\) si \( i<j\). Du coup,
\begin{equation}
