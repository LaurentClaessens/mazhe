...
\begin{lemma}       \label{LemjcztYH}
    Soient un espace vectoriel \( E\), un endomorphisme \( u\in \End(E)\), ainsi que ses sous-espaces propres \( \{ E_{\lambda} \}_{\lambda\in \Spec(u)}  \)\nomenclature[A]{\( E_{\lambda}(u)\)}{Espace propre de \( u\)}. Toute somme finie de la forme
    \begin{equation}
...
\begin{proof}
    Nous utilisons le lemme \ref{LEMooDQMQooInVVDY}. Soient \( v_i\in V_{\lambda_i}\) un choix de vecteurs tels que 
    ...
    \end{equation}
    dès que \( i\neq j\). Nous appliquons l'opérateur \( \prod_{k\neq j}(A-\lambda_k\mtu)\) à l'égalité \eqref{EQooROAXooFpgxxF} :
    \begin{subequations}
        \begin{align}
    ...
    Justifications.
    \begin{itemize}
        \item Pour \eqref{SUBEQooHHGJooTvCcDb}. Pour chaque \( k\) et \( i\) nous avons \( (A-\lambda_k)v_i=Av_i-\lambda_kv_i=\lambda_iv_i-\lambda_kv_i\) parce que \( v_i\) est un vecteur propre de \( A\) pour la valeur propre \( \lambda_i\).
...
\begin{definition}[Matrice définie positive, opérateur définit positif]    \label{DefAWAooCMPuVM}
    ...
    Même définitions pour une matrice.
\end{definition}
