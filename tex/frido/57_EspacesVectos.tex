...
\begin{proof}
    ...
    \begin{equation}        \label{EQooROAXooFpgxxF}
        \sum_{i=1}^pv_i=0. 
    \end{equation}
    Soit un entier \( j\) entre \( 1\) et \( p\). Nous allons montrer que \( v_j=0\). Pour cela nous remarquons d'abord que
    \begin{equation}        \label{EQooGVPYooXRPEVU}
        \prod_{k\neq j}(\lambda_i-\lambda_k)=0, \forall i\neq j
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        E=\sum_{i=1}^nE_i.
    \end{equation}
    Cette dernière somme n'est éventuellement pas une somme directe. Si \( i\neq j\), alors \( Q_iQ_j\) est multiple de \( P\); plus précisément, en utilisant la commutativité de \( \eK[X]\),
    \begin{equation}
        Q_iQ_j=\prod_{k\neq i}P_k\prod_{l\neq j}P_l=\prod_{\substack{k\neq i\\k\neq j}}P_k\prod_kP_l=S_{ij}P
    \end{equation}
    où \( S_{ij}\) est un polynôme. Toujours avec \( i\neq j\), en utilisant le lemme \ref{LemQWvhYb} ainsi que la commutativité de \( \eK[X]\)\footnote{Lemme \ref{LEMooWVUXooQlaepO}.},
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    Étant donné que sur chaque \( S_i\) nous avons \( f|_{S_i}=f_i\), l'espace \( S=S_1\oplus\ldots\oplus S_r\) est stable par \( f\). Alors, nous avons
    \begin{subequations}
        \begin{align}
            E &= E_1\oplus\ldots\oplus E_r\\
...
\begin{proof}
    ...
    \end{equation}
    alors que par hypothèse de récurrence le membre de droite est dans \( \Span(B)\). L'ensemble \( B\) est alors générateur de \( E_{f,x}\) et donc une base de celui-ci. Nous avons donc bien \( \dim(E_{f,x})=p_{f,x}\).
...
\begin{proof}
    ...
    \begin{equation}
        \begin{pmatrix}
         ...
         \end{pmatrix}
    \end{equation}
    où les étoiles représentent des blocs à priori non nuls. En tout cas il est visible que sous cette forme, \( (X-z)^k\) divise \( \chi_u\).
\end{proof}
    ...
    \begin{subproof}
       \item[\( \Rightarrow\)]
           ...
           \begin{equation}
               ...
               \end{pmatrix}.
           \end{equation}
           Étant donné que \( u\) est nilpotent, la matrice \( A\) l'est aussi. L'hypothèse de récurrence dit alors que \( A\) est strictement triangulaire supérieure (ou en tout cas peut le devenir par un changement de base adéquat).
...
\begin{proof}
    Si \( A\) est diagonalisable avec les valeurs propres \( \lambda_i\) de norme \( 1\) dans \( \eC\), alors \( A^k\) est la matrice diagonale avec les \( \lambda_i^k\) sur la diagonale. L'ensemble des telles matrices est borné dans \( \GL(n,\eC)\)\footnote{Vous voyez le lien entre norme matricielle, norme opérateur, valeurs propres et tout ça ?}.
    %TODOooUWZKooVtfvlT faire le lien avec la norme opérateur. Est-ce que GL est un groupe de matrices ou d'opérateurs. Il y a surement une confusion ici.
...
\begin{proof}
    ...
    \end{equation}
    où les \( \lambda_i\) sont les valeurs propres de \( M\). \( \langle x,Mx \rangle\) est strictement positif pour tout \( x\) si et seulement si tous les \( \lambda_i\) sont strictement positifs.
