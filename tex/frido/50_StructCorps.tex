...
\begin{definition}[\cite{BIBooCWTCooFdXIjl,BIBooBXBQooNZljtJ}]      \label{DEFooLXSBooCHIUFU}
    Soint un corps \( \eK\) et un polynôme irréductible \( P\in \eK\). Nous disons que \( P\) est \defe{séparable}{polynôme séparable} si il existe une fermeture algébrique de \( \eK\) dans laquelle \( P\) se factorise en polynômes de degré \( 1\) deux à deux distincts.
...
\begin{proposition}[\cite{BIBooCWTCooFdXIjl}]       \label{PROPooPRGQooOkEmag}
    Soient un corps \( \eK\), un polynôme non constant \( P\in \eK[X]\) ainsi qu'une extension algébriquement close \( \Omega\) et un corps de décomposition \( \eM\) pour \( P\). Alors les propriétés suivantes sont équivalentes :
    \begin{enumerate}
        \item
            \( \pgcd(P,P')=1\).
        \item
...
\begin{proposition}         \label{PROPooMOZFooDGemBJ}
