...
\begin{propositionDef}       \label{DEFooCYDPooEdRbyl}
    Soit \( \affE\) un espace affine de dimension trois muni d'un repère barycentrique. 

    Nous notons \( D(a,b,c)\) l'ensemble des éléments de \( \affE\) dont les coordonnées barycentriques (normalisées) \( (x,y,z)\) vérifient \( ax+by+cz=0\). 

    L'ensemble \( D(a,b,c)\) est un sous-espace de dimension \( 1\) de \( \affE\).

    Nous nommons \defe{droite affine}{droite affine} une telle partie de \( \affE\).

    C'est un espace de dimension un parce qu'il y a aussi la condition \( x+y+z=1\).
\end{propositionDef}
