...
\begin{definition}      \label{DEFooULVAooXJuRmr}
    Soient des espaces vectoriels \( V \) et \( W\) sur le corps \( \eK\). Une application \( T\colon V\to W\) est dite \defe{linéaire}{linéaire!application} si
    \begin{itemize}
        \item $T(x+y)=T(x)+T(y)$ pour tout $x$ et $y$ dans \( V\),
        \item $T(\lambda x)=\lambda T(x)$ pour tout $\lambda$ dans $\eK$ et \( x\) dans \( V\).
    \end{itemize}
\end{definition}
Si vous avez bien suivi, les égalités dans la définition \ref{DEFooULVAooXJuRmr} sont des égalités dans \( W\).

L'ensemble des applications linéaires de \( V\) vers \( W\) est noté $\aL(V,W)$\nomenclature{$\aL(V,W)$}{Ensemble des applications linéaires de $V$ dans $W$}.
...
\begin{definition}[Quelque ensembles d'applications linéaires]      \label{DEFooOAOGooKuJSup}
    ...
    \begin{itemize}
        \item L'ensemble des applications linéaires \( E\to F\) est noté \( \aL(E,F)\).
