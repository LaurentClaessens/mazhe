...
\begin{enumerate}
    ...
    \item       \label{ITEMooUBZIooDDcfWg}
        ...
        Cette question provient aussi de \cite{ooEEWJooZvNYpA}. On donne aussi quelque exemple dans \ref{SUBSECooEDMJooTXBfOu}.
...
\begin{enumerate}
    ...
    \item
        Soit un espace vectoriel normé \( V\) (de dimension pas spécialement finie). Soit aussi \( x\) fixé dans \( V\). Est-ce qu'il existe une forme linéaire continue \( \varphi\colon V\to \eR\) telle que \( \| \varphi \|=1\) et \( \varphi(x)=1\) ?

        En admettant un théorème de la base incomplète dans \( V\), ok. Je prend une base qui commence par \( x\), et défini \( \varphi(x)=1\) et \( \varphi=0\) sur tous les autres éléments de la base. Mais cela demande le lemme de Zorn, voir par exemple \cite{ooXEFKooHikcdE}.

        L'existence seule d'une base (qui demande aussi le lemme de Zorn) est en réalité suffisante. Si \( \{ e_i \}_{i\in I}\) est une base, et si \( x=\sum_{i\in I_0}x_ie_i\) (\( I_0\) est une partie finie de \( I\)) alors on peut substituer \( x\) à n'importe que \( e_i\) (\( i\in I_0\)) et avoir encore une base.

        J'utilise cela dans la proposition \ref{PROPooMAQSooCGFBBM}\ref{ITEMooTCYPooHYfCEl}.

        Que penser du lemme \ref{LEMooJGORooFEZgau} ? Est-il correct et bien prouvé ?
\end{enumerate}
