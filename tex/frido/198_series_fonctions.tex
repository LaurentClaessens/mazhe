...
\begin{proof}
    Nous posons \( f(x)=\ln(ax)\) qui est une fonction dérivable\footnote{Dérivée du logarithme, proposition \ref{}.}. Alors \( f'(x)=\frac{ a }{ ax }=\frac{1}{ x }\). Cette fonction \( f\) est donc une primitive de \( \frac{1}{ x }\) et il existe une constante \( K\) telle que
    \begin{equation}
        f(x)=\ln(x)+K.
    \end{equation}
