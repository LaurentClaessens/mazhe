...
\begin{itemize}
    \item 
        un rayon de convergence plus grand que zéro,
    \item
...
\begin{normaltext}      \label{NORMooADIZooUmevqk}
    ...
    \end{equation}
    nous n'avons aucune garantie de \( T=f\), même pas sur le rayon de convergence\footnote{Définition \ref{DefZWKOZOl}.} de la série entière définissant \( P\). Et nous n'avons pas de garanties d'avoir un rayon de convergence plus grand que \( 0\).
...
\begin{theorem}[Inégalité de Taylor\cite{ooNVJGooGKwDWG}]       \label{THOooEUVEooXZJTRL}
    Soit une fonction \( f\colon \eR\to \eR\) dérivabe \( n+1\) fois et telle que \( | f^{(n+1)}(x) |\leq M_N\) sur \( B(a,d)\). Alors
    \begin{equation}
...
\begin{proof}
    ...
    Nous pouvons écrire \eqref{EQooOWJMooHATpMV} pour \( n+1\) au lieu de \( n\); cela donne
    \begin{equation}
        f(x)=P_n(x)+\int\cdots,
    \end{equation}
    et donc
