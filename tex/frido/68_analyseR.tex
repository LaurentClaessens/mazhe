...
\begin{normaltext}
	À la question «Est-ce qu'il existe une fonction différentiable dont les dérivées partielles ne sont pas continues», en juillet 2026, Mistral propose
	\begin{equation}
		f(x,y)=\begin{cases}
		...
		\end{cases}
	\end{equation}
	M'est avis que ce n'est pas vrai. Les dérivées partielles sont continues en \( (0,0)\).
\end{normaltext}


\begin{proposition}[\cite{BIBChatGPT}]     \label{PROPooJPRUooNOcXPJ}
	Il existe une fonction différentiable dont les dérivées partielles ne sont pas continues.
\end{proposition}
...
\begin{proof}
	...
	\end{equation}
	\begin{subproof}
		\spitem[La fonction \( f\) est différentiable et \( df_0=0\)]
		%-----------------------------------------------------------
		Il suffit de suivre la définition. Nous posons \( T(h)=0\) et nous calculons \( \lim_{h\to0}| f(h) |/\| h \|\). Pour cela nous prenons une boule de rayon \( r\) autour de \( 0,0\); la proposition \ref{PROPooKSGXooOqGyZj} dit que tout point à l'intérieur (à part \( (0,0)\)) peut être écrit tout la forme \( (\rho \cos(\theta), \rho\sin(\theta))\) pour un \( 0<\rho<r\). En calculant à peine,
		\begin{equation}
			\frac{ | f\big( \rho\cos(\theta),\rho\sin(\theta) \big) | }{ \| (\rho\cos(\theta),\rho\sin(\theta)) \| }=\frac{ | r^2\cos^2(\theta)\sin(1/\rho\cos(\theta)) | }{ r }\leq r.
		\end{equation}
		Nous avons donc bien que \( f(x,y)/\| (x,y) \|\ to0\). Et donc que
		\begin{equation}
			df_0=0.
		\end{equation}

		\spitem[\((\partial_1f)(0,0))=0\)]
		%-----------------------------------------------------------
		Simple calcul :
		\begin{subequations}
			\begin{align}
				(\partial_1f)(0,0) & =\lim_{\epsilon\to 0}\frac{ f(\epsilon,0)-f(0,0) }{ \epsilon } \\
				                   & =\lim_{\epsilon\to 0}\epsilon\sin(1/\epsilon)                  \\
				                   & =0.
			\end{align}
		\end{subequations}

		\spitem[\( (\partial_1f)(x,y)\)]
		%-----------------------------------------------------------
		Les règles de calcul de produit, de composées et de fonctions trigonométriques (lemme \ref{LEMooBBCAooHLWmno}) nous donnent :
		\begin{equation}
			(\partial_1f)(x,y)=2x\sin\left( \frac{1}{ x} \right)-\cos\left( \frac{1}{ x} \right)
		\end{equation}
		lorsque \( x\neq 0\).

		Calculons \( \lim_{x\to 0}(\partial_1f)(x,0)\). D'une part
		\begin{equation}
			\lim_{x\to 0}2x\sin\left( \frac{1}{ x} \right)=0,
		\end{equation}
		mais d'autre part l'application \( \cos(1/x)\) n'a pas de limites pour \( x\to 0\). Donc l'application \( x\mapsto (\partial_1f)(x,0)\) n'a pas de limites pour \( x\to 0\).
	\end{subproof}
\end{proof}
...
\begin{lemma}[Ouverture de la condition de rang maximum\cite{BIBChatGPT}]	\label{LEMooRSDHooDpNEjo}
	Soit une fonction \(f \colon \eR^m\to \eR^n  \) de classe \( C^1\). Nous supposons que \( df_a\) est de rang \( m\). Alors il existe \( r>0\) tel que \( \rank(df_x)=m\) pour tout \( x\in B(a,r)\).
\end{lemma}

\begin{proof}
	Le lemme \ref{LEMVecsaRgFixe} dit qu'il existe une base \( \{ e_1,\ldots,e_m \}\) de \( \eR^m\) telle que \( df_a(e_i)\neq 0\) pour tout \( i\). Vu que \( f\) est de classe \( C^1\), les fonctions \( x\mapsto df_x(e_i)\) sont continues. Donc on a un \( r>0\) tel que \( df_x(e_i)\neq 0\) pour tout \( x\in B(a,r)\). Pour chacun de ces \( x\), l'application \( df_x\) est de rang \( m\).
\end{proof}
...
Une avalanche de conditions équivalentes à l'holomorphie sera donnée dans le théorème \ref{THOooOGOCooUalFaG}.
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooLFNVooUOCGMF}
	Si \(f \colon \eC\to \eC  \) est continue en \( 0\), alors
	\begin{equation}
		\lim_{z\to 0}f(z)=\lim_{z\to 0}f(\bar z).
	\end{equation}
\end{lemma}

\begin{proof}
	L'application de conjugaison complexe \(c \colon \eC\to \eC  \) est continue\footnote{Lemme \ref{LEMooSYTWooMDzrnX}.}. La continuité de \( f\) en \( 0\) donne l'existence de la limite que nous notons \( \lim_{z\to 0}f(z)=L\). Nous utilisons la caractérisation séquentielle de la continuité de \( f\) est \( 0\) (proposition \ref{PropXIAQSXr}). Pour toute suite \( (z_n)_{n\in \eN}\) telle que \( z_n\to 0\) nous avons \( \lim_{n\to \infty}f(z_n)=L\). En particulier nous avons \( c(z_n)\to 0\) et donc
	\begin{equation}
		\lim_{n\to \infty}f\big( c(z_n) \big)=0,
	\end{equation}
	c'est à dire \( \lim_{n\to \infty}(f\circ c)(z_n)=L\). Et vu que cette égalité est vraie pour toute suite \( z_n\to 0\), nous en déduisons
	\begin{equation}
		\lim_{z\to 0}(f\circ c)(z)=L.
	\end{equation}
\end{proof}
...
\begin{definition}[Fonction holomorphe]  \label{DefMMpjJZ}
	En deux parties.
	\begin{enumerate}
		\item
		      Une application \(f \colon \eC^n\to \eC  \) est \defe{holomorphe}{holomorphe} sur \( \Omega\) si \( df_z\) existe et est \( \eC\)-linéaire, c'est à dire si
		      \begin{equation}
			      df_z(ih)=idf_z(h).
		      \end{equation}
		\item
		      Une application \(f \colon \eC^n\to \eC^n  \) est \defe{holomorphe}{holomorphe} si chacune de ses composantes est holomorphe.
	\end{enumerate}
\end{definition}


\begin{proposition}	\label{PROPooBXIWooUvIJMW}
	Soit \( \Omega\) un ouvert dans \( \eC\). Une fonction \( f\colon \Omega\to \eC\) est holomorphe\footnote{Définition \ref{DefMMpjJZ}.} si et seulement si elle est \( \eC\)-dérivable sur \( \Omega\).
	%TODOooHAVRooGymwqI. Prouver ça.
\end{proposition}
...
\begin{proposition}     \label{PropKJUDooJfqgYS}
	...
	\begin{equation}        \label{EqPAEFooYNhYpz}
		(df_{z_0})(z)=f'(z_0)z
	\end{equation}
	où \( f'\) est la dérivée complexe de \( f\).
\end{proposition}
...
Nous venons de voir dans \eqref{EqPAEFooYNhYpz} que si \( f'\) est la dérivée complexe de la fonction holomorphe \( f\), alors \( df_{z_0}\) est l'opérateur de multiplication par \( f'(z_0)\). Nous allons un peu approfondir ce lien pour lier la dérivée complexe et la dérivée usuelle.

\begin{lemma}[\cite{MonCerveau}]	\label{LEMooKJOEooLRUzSA}
	Si \(f \colon \eR\to \eC  \) est différentiable, alors \( df_{t_0}\) est l'opérateur de multiplication par \( f'(t_0)\) où \( f'\) est la dérivée usuelle.
\end{lemma}

\begin{proof}
	Nous écrivons \( f(t)=u(t)+iv(t)\) pour les fonctions réelles \( u\) et \( v\). Par linéarité,
	\begin{equation}
		df_{t_0}(t)=du_{t_0}(t)+idv_{t_0}(t).
	\end{equation}
	Le lemme \ref{LEMooXZGQooYxDRRU} dit que \( du_{t_0}\) est l'opérateur de multiplication par \( u'(t_0)\). Donc
	\begin{equation}
		df_{t_0}(t)=\big( u'(t_0)+iv'(t_0) \big)t=f'(t_0)t.
	\end{equation}
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooBLGMooYeZZYJ}
	Soient une application holomorphe \(f \colon \eC\to \eC  \) ainsi qu'un chemin \( C^1\) \(\gamma \colon \mathopen[ a,b\mathclose]\to \eC  \). Alors
	\begin{equation}
		(f\circ\gamma)'(t)=(f'\circ\gamma)(t)\gamma'(t)
	\end{equation}
	où \( f'\) est la dérivée complexe et les autres dérivées sont les dérivées usuelles.
\end{proposition}

\begin{proof}
	Nous passons par la différentielle parce que la différentielle est l'opérateur de multiplication par la dérivée autant dans le cas «usuel» (lemme \ref{LEMooKJOEooLRUzSA}) que dans le cas complexe (équation \ref{EqPAEFooYNhYpz}). Nous avons
	\begin{subequations}
		\begin{align}
			d(f\circ\gamma)_{t_0}(t) & =df_{\gamma(t_0)}(d\gamma)_{t_0}(t)                                  \\
			                         & =f'\big( \gamma(t_0) \big)\gamma'(t_0)t.	\label{SUBEQooPWRLooOgzBMt}
		\end{align}
	\end{subequations}
	Notez que pour \eqref{SUBEQooPWRLooOgzBMt} nous avons utilisé \ref{LEMooKJOEooLRUzSA} et \ref{EqPAEFooYNhYpz}.

	Et comme \( d(f\circ\gamma)_{t_0}\) est l'opérateur de multiplication par \( (f\circ\gamma)'(t_0)\), nous en déduisons que
	\begin{equation}
		(f\circ\gamma)'(t_0)=f'\big( \gamma(t_0) \big)\gamma'(t_0)=(f'\circ\gamma)(t_0)\gamma'(t_0).
	\end{equation}
\end{proof}
...
\begin{example}[Une application \( C^{\infty}\) mais pas \( \eC\)-dérivable]
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	Vu que c'est une application linéaire, elle est différentiable une infinité de fois et sa différentielle est elle-même\footnote{Lemme \ref{LEMooZSNMooCfjzOB}.}. C'est donc une application \( C^{\infty}\).
...
\begin{theorem}[Accroissements finis, inégalité de la moyenne]\label{val_medio_2}
	...
	\end{equation}
	%TODOooPPUSooVCscYV. Énoncer et prouver dans le cadre d'un espace vectoriel normé de dimension finie général.
\end{theorem}
...
Les notions de classe \( C^k\) et \( C^{\infty}\) sont dans la définition \ref{DefPNjMGqy}.

\begin{lemma}[\cite{MonCerveau}]	\label{LEMooXKBPooMsDLuq}
	Toute application linéaire est de classe \( C^{\infty}\).
	%TODOooZXSWooLmwyvh. Prouver ça.
\end{lemma}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooJAUVooBhhDkD}
	Si \( f\) et \( g\) sont des fonctions de classe \( C^{k}\), alors \( f+g\) et \( fg\) sont de classe \( C^k\).
	%TODOooIGDNooBGMoLY. Prouver ça.
\end{proposition}


\begin{proposition}		\label{PROPooMGFBooHWGXyC}
	Tout polynôme \(P \colon \eR^n\to \eR^m  \) est de classe
	\( C^{\infty}\).
\end{proposition}

\begin{proof}
	Nous posons \( e_n(x)=x^n\), et nous prouvons par récurrence que \( e_n\) est de classe \( C^{\infty}\). D'abord \( e_0\) est une fonction constante et donc est \( C^{\infty}\) lemme \ref{LEMooXKBPooMsDLuq}.

	Ensuite si \( e_n\) est de classe \( C^{\infty}\), alors le lemme \ref{LEMooZQIAooVbgFKo} donne \( e_{n+1}(x)=xe_n(x)\) est \( C^{\infty}\) parce qu'il s'agit du produit de \( e_n\) par \( x\mapsto 0\) et que le produit conserve le caractère \( C^{\infty}\) par la proposition \ref{PROPooJAUVooBhhDkD}.
\end{proof}


\begin{proposition}[Exemple en dimension infinie\cite{MonCerveau}]        \label{PROPooZCUSooLUUrxi}
	L'endomorphisme de dérivation sur l'espace des fonctions\footnote{Les classes \( C^k\), définition \ref{DefPNjMGqy}.} \( C^{\infty}(\eR,\eR)\) n'a pas de polynôme minimal.
\end{proposition}

\begin{proof}
	Nous considérons l'opérateur de dérivation \(\mD \colon C^{\infty}(\eR,\eR)\to C^{\infty(\eR,\eR)}  \). Comme demandé dans la définition \ref{DefCVMooFGSAgL} nous considérons la partie
	\begin{equation}
		I_{\mD}=\{ P\in \eR[X]\tq P(\mD)=0 \}.
	\end{equation}
	Nous montrons que \( I_{\mD}=\{ 0 \}\). Soit \( P=\sum_{k=0}^{n}a_kX^k\in \eR[X]\). Nous considérons \( f(x)=x^{n+1}\) qui est de classe \( C^{\infty}\) parce que c'est ce que dit la proposition \ref{PROPooMGFBooHWGXyC}. Nous considérons aussi le plus petit \( k\) pour lequel \( a_k\neq 0\). Le lemme \ref{LEMooZQIAooVbgFKo}\ref{ITEMooQKYJooKqZhRS} donne
	\begin{equation}
		(a_k\mD^k)(f)(x)=a_k\frac{ n! }{ (n-k)! }x^{n-k}.
	\end{equation}
	Tous les autres termes de \( P(\mD)f\) sont de degrés plus petits. Donc \( P(\mD)f\) est un polynôme non nul.

	Tout ça pour dire que \( I_{\mD}=\{ 0 \}\) et que donc \( \mD\) n'a pas de polynôme minimal.
\end{proof}
...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Différentielle et dérivées partielles multiples}
%---------------------------------------------------------------------------------------------------------------------------
