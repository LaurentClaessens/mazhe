...
\begin{lemma}
    Soient une application différentiable \( f\colon V\to W\) ainsi que \( \psi\in\aL\big( V,\aL(V,W) \big)\). Soit
    \begin{equation}
        \begin{aligned}
            g\colon V&\to \aL\big( \aL(V,W) \big) \\
            x&\mapsto f(x)\cdot \psi. 
        \end{aligned}
    \end{equation}
    Alors \( g\) est différentiable et
    \begin{equation}
        dg_x=[h\mapsto df_x(h)\cdot \psi].
    \end{equation}
\end{lemma}

Notons que nous n'avons pas \( dg_x=df_x\cdot \psi\). En effet, \( df_x\in \aL( V,W )\), de telle sorte que \( df_x\cdot\psi\in \aL\big( V,\aL\big( V,\aL(V,W) \big) \big)\). Les espaces ne s'emboîtent pas dans le bon ordre.
