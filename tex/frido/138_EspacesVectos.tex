...
Soit \( \eK\) un corps (commutatif) et une extension \( \eL\) de \( \eK\). Soient \( E\) et \( F\), des \( \eK\)-espaces vectoriels de dimension finie.
...
\begin{proposition}
%TODO je ne comprends pas: f o h  va de G vers F, mais g va de F vers E .  Comment sont-elles egales ?  Il n'y a pas de groupe G ? que E et F ?
...
\begin{proof}
    ...
    \item[\( \psi\) est injective]
        ...
        \end{equation}
        Par hypothèse nous avons \( \psi(g)_{\lambda}=\psi(h)_{\lambda}\), et aussi
        \begin{equation}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooSOYYooZVibjrvi} implique~\ref{ITEMooSOYYooZVibjrv}]
            Soit \( v\) un vecteur cyclique de \( f\). Un endomorphisme \( g\) donne lieu à un polynôme car par l'hypothèse \( f\) est cyclique, il existe des uniques \( a_k\) (\( k=0,\ldots, n-1\)) tels que
            \begin{equation}
...
%TODO: mal choisie ! Nous avons deja \varphi_x\colon \eK[X]&\to E  et x=a
            \begin{equation}
