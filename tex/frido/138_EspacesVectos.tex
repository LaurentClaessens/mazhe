...
\begin{equation}        \label{EQooYGVHooHYbMHX}
	...
	\end{aligned}
	%TODOooXCIIooLiUGFS  Il faut prouver que cette définition est correcte pour les classes.
	% C'est dans ma liste.
\end{equation}
...
\begin{proof}
	...
	\end{subproof}
	%TODOooTENIooVqmkUO il faut faire les autres parties de cette preuve.
\end{proof}
...
Note au passage : il y a un abus systématique de notation entre \( e_i\in E\) et \( \tau(e_i)=1\otimes e_i\in E_{\eL}\).
...
\begin{lemma}       \label{LEMooWZGSooONEnjZ}
	...
	De même\quext{Je n'ai pas vérifié cette deuxième partie. Soyez \randomGender{prudent}{prudente}.} si \( f\colon E\to F\) et \( \tilde h\colon F_{\eL}\to G_{\eL}\), alors
	\begin{equation}
...
\begin{theorem}[Réduction de Jordan]        \label{ThoGGMYooPzMVpe}
	Soit \( E\) un espace vectoriel sur \( \eK\), et \( f\in\End(E)\) un endomorphisme dont le polynôme caractéristique \( \chi_f\) est scindé\footnote{Définition \ref{DefCPLSooQaHJKQ}. C'est à cause de cette hypothèse que \( \eK=\eR\) n'est pas le bon exemple de contexte de travail.}. Il existe une base de \( E\) dans laquelle la matrice de \( f\) s'écrit sous la forme
	\begin{equation}
		M=\begin{pmatrix}
			J_{n_1}(\lambda_1) & 0      &                    \\
			0                  & \ddots &                    \\
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\ref{ITEMooSOYYooZVibjriv} implique~\ref{ITEMooSOYYooZVibjriii}]
		...
		\spitem[\ref{ITEMooSOYYooZVibjriii} implique~\ref{ITEMooSOYYooZVibjri}]
		...
		\spitem[\ref{ITEMooSOYYooZVibjri} implique~\ref{ITEMooSOYYooZVibjrii}]
		...
		\spitem[\ref{ITEMooSOYYooZVibjrii} implique~\ref{ITEMooSOYYooZVibjrvi}]
		...
		\spitem[\ref{ITEMooSOYYooZVibjrvi} implique~\ref{ITEMooSOYYooZVibjrv}]
		...
		\end{equation}
		pour avoir \( \psi(g)=P \).
		\spitem[\ref{ITEMooSOYYooZVibjrv} implique~\ref{ITEMooSOYYooZVibjriv}]
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Si \( A\) est trigonalisable]
		...
		\spitem[Si \( A\) n'est pas trigonalisable]
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\ref{ITEMooLRXIooLWaYqJi} implique~\ref{ITEMooLRXIooLWaYqJii}]
		...
		\spitem[\ref{ITEMooLRXIooLWaYqJii} implique~\ref{ITEMooLRXIooLWaYqJiii}]
		...
		\spitem[\ref{ITEMooLRXIooLWaYqJiii} implique~\ref{ITEMooLRXIooLWaYqJi}]
