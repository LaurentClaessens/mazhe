...
\begin{proposition}     \label{PROPooHJJCooTXlzdH}
    ...
    Il existe une application linéaire \( h\colon F\to G\) telle que \( f\circ h=g\) si et seulement si il existe une application linéaire \( \tilde h\colon F_{\eL}\to G_{\eL}\) telle que \( f_{\eL}\circ \tilde h=g_{\eL}\).
\end{proposition}
...
\begin{remark}  \label{RemmQjZOA}
    Les matrices compagnons ne sont pas les seules dont le polynôme caractéristique est égal au polynôme minimal. En fait les matrices dont le polynôme caractéristique est égal au polynôme minimal sont denses dans les matrices. En effet une matrice dont le polynôme minimal n'est pas égal au polynôme caractéristique a un polynôme caractéristique avec une racine double. Il est possible, en modifiant arbitrairement peu la matrice, de séparer la racine double en deux racines distinctes.
\end{remark}
...
\begin{proof}
    ...
    Étant donné que \( \dim\eK[f]=d\) et que \( T\) est injective, \( \dim\Image(T)=d\). Étudions l'orthogonal de l'image :
    \begin{subequations}
        \begin{align}
    ...
    \end{equation}
    Pour \( 1\leq i\leq j-1\), nous avons supposé \( P_i=Q_i\). Étant donné que \( f|_{F_i}\) est semblable à \( C_{P_i}\) et \( f|_{G_i}\) est semblable à \( C_{Q_i}\), la matrice de \( f|_{E_i}\) est semblable à la matrice de \( f|_{G_i}\). En particulier,
    \begin{equation}
...
\begin{theorem}[Réduction de Jordan]        \label{ThoGGMYooPzMVpe}
    ...
    \begin{equation}
        ...
        \end{pmatrix}
    \end{equation}
    où les \( \lambda_i\) sont les valeurs propres de \( f\) (avec éventuelles répétitions) et \( J_{n_i}(\lambda)\) représente le bloc \( m_i\times m_i\)
    \begin{equation}
...
\begin{definition}[Commutant \( \comC(f)\) de \( f\)]       \label{DEFooIDMMooHIpUmT}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooSOYYooZVibjrvi} implique~\ref{ITEMooSOYYooZVibjrv}]
            ...
            \end{equation}
            Cela est une bonne définition parce que \( \{ f^k(v) \} \) est une base.
...
Nous considérons encore un espace vectoriel \( E\) de dimension finie \( n\) et un endomorphisme \( f\colon E\to E\). Nous notons \( \mu\) son polynôme minimal et \( \mu_x\) le polynôme minimal ponctuel en \( x\).
    ...
    \begin{subproof}
        \item[\ref{ITEMooLRXIooLWaYqJi} implique~\ref{ITEMooLRXIooLWaYqJii}]
            ...
            \begin{subproof}
                ...
                \item[Injectif]
                    Si \( \psi(\bar P)=0\), alors \( \varphi_a( P)=0\), c'est à dire que \( P\in \ker(\varphi_a)\).  Mais, par définition~\ref{DEFooUICRooBGYhqQ} du polynôme minimal ponctuel, \( \mu_a\) est générateur de \( \ker(\varphi_a)\); donc il existe \( Q\in \eK[X]\) tel que \( P=Q\mu_a\). Par conséquent \( \bar P=0\), ce que nous voulions.
                \item[Surjectif]
                    Si \( x\in E_a\) alors il existe des coefficients \( x_k\in \eK\) tels que \( x=\sum_{k=0}^{\deg(\mu_a)-1}x_kf^k(a)\), c'est-à-dire \( x=P(f)a=\varphi_a(P)=\psi(\bar P)\). 
            \end{subproof}
