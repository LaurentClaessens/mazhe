...
\begin{definition}[Polynôme scindé]  \label{DefCPLSooQaHJKQ}\label{DefPolynomeScinde}
	Nous disons que \( P\in\eK[X]\) est \defe{scindé}{polynôme scindé} sur \(\eK\) si il est produit dans \(\eK[X]\) de polynômes de degré~\( 1\).

	Soient \( P\in\eK[X]\) un polynôme irréductible, et \( \eL\) un corps, extension du corps \( \eK\). On dit que \( P \) est \defe{scindé}{polynôme!scindé} dans \( \eL \) si \( P \) se décompose en un produit de polynômes de degré 1 dans \( \eL[X]\).
\end{definition}
...
% TODOooAZCWooBMwRxk : voir qui référentie ce lemme et mettre la référence vers le point correct.
