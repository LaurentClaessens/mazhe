...
\begin{proof}
    ...
    Pour le point \ref{ITEMooBPOZooYeFGjl}.

    Nous montrons que \( \{ P,PQ+R \}\) a les mêmes diviseurs que \( \{ P,R \}\).
    \begin{subproof}
        \item[\( A\divides\{ P,PQ+R \}\) implique \( A|\divides\{ P,R \}\)]
        
            Il existe des polynômes \( B_1\) et \( B_2\) tels que \( P=AB_1\) et \( PQ+R=AB_2\). Donc
            \begin{equation}
                R=AB_2-PQ=AB_2-AB_1Q=A(B_2-B_1Q)
            \end{equation}
            donc oui, \( A\) divise \( R\).
        \item[\( A\divides\{ P,R \}\) implique \( A|\divides\{ P,PQ+R \}\)]
            
            Il existe des polynômes \( B_1\) et \( B_2\) tels que \( P=AB_1\) et \( R=AB_2\). Donc
            \begin{equation}
        ...
        \item[Conclusion]
            ...
            Les paires \( \{ P,PQ+R \}\) et \( \{ P,R \}\) ont même ensemble de diviseurs, et donc même \( \pgcd\).

    \end{subproof}

    Pour le point \ref{ITEMooYXAHooXibkgV}.

    \begin{subproof}
    \item[\( A\divides \{ P,QR \}\) si et seulement si \( A\divides\{ P,R \}\)]
...
Supposons d'abord que \( A\in \eK[X]\) divise \( P\) et \( QR\). Le théorème de Bézout \ref{ThoBezoutOuGmLB} assure l'existence de polynômes $U$ et $V$ tels que $PU+QV=1$. Ensuite l'hypothèse de division nous donne des polynômes \( B_1\) et \( B_2\) tels que $P=AB_1$ et $QR=AB_2$.  Nous avons :
            \begin{equation}
                    1=PU+QV=AB_1U+QV.
            \end{equation}
            Ce la prouve que \( A\) est premier avec $Q$ grâce encore à Bérout, mais dans l'autre sens. Donc \( A\) est premier avec \( Q\) et \( A\divides QR\). Donc \( A|R\) par le lemme de Gauss \ref{ThoLLgIsig}.
                   
    Dans l'autre sens, si $A|R$ alors on a évidemment : $A|QR$.

    \item[Conclusion]

    Nous avons Les diviseurs de ${P,QR}$ sont exactement les diviseurs de ${P,R}$. En conséquence de quoi nous concluons que les paires ${P,QR}$ et ${P,R}$ ont le même $\pgcd$.
    \end{subproof}
    
    Pour le point \ref{ITEMooYXAHooXibkgV}.

    \begin{subproof}
    \item[\( A\divides\{ P,QR \}\) implique \( A\divides\{ P,R \}\)]
        
        Il existe \( U_1,U_2\) tels que \( PU_1+QU_2=1\) ainsi que \( B_1\) tel que \( P=AB_1\). Donc
        \begin{equation}
            1=AB_1U_1+QU_2,
        \end{equation}
        ce qui prouve que \( A\) est premier avec \( Q\). Le lemme de Gauss \ref{ThoLLgIsig} montre alors que \( A\divides R\).

    \item[\( A\divides\{ P,R \}\) implique \( A\divides\{ P,QR \}\)]

        Maintenant vous devriee être capables de faire ça.

    \item[Conclusion]

        Les paires \( \{ P,QR \}\) et \( \{ P,R \}\) ont les mêmes diviseurs et donc même \( \pgcd\).
...
\begin{remark}
    Dans le cas des polynômes à plusieurs indéterminées, le résultat n'est plus vrai\footnote{Exemple \ref{ExGRHooBNpjSP}.}, mais nous avons la proposition \ref{PropTETooGuBYQf}.
\end{remark}
