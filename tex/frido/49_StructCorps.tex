...
%TODO: Pourquoi Z 1_{\eA} ?  Qu'est ce que A ici ... K ?
    \begin{equation}
...
\begin{theorem}[\cite{BIBooIXPJooEyrkRc,BIBooDYIOooNtTWFI, BIBooQTFDooPOdQhr}]     \label{THOooSZXWooVeHdrh}
    Soit un nombre premier de Sophie Germain (définition \ref{DEFooCVFJooKCdVVD}). Il n'existe pas de solution dans \( \eZ^3\) au système
    \begin{equation}
        \begin{cases}
            x^p+y^p+z^p=0   \\
            [xyz]_p\neq 0.
...
\begin{proof}
    ...
    Nous avons prouvé le théorème pour les nombres \( (x,y,z)\) qui n'ont pas de facteurs communs. Si \( k\) était un diviseur commun de \( x\), \( y\) et \( z\), alors \( (x/k,y/k,z/k)\) serait encore une solution, ce qui n'est pas le cas.
\end{proof}
...
\begin{proof}
    Il suffit de prouver que si \( m,n\in \Sigma\), alors le produit \( mn\) est également dans \( \Sigma\). Si \( N\) est le stathme euclidien sur \( \eZ[i]\), alors  \( n\in \Sigma\) si et seulement si il existe \( z,z'\in \eZ[i]\) tel que \( N(z)=n\) et \( N(z')=m\). Si \( z,z'\in \eZ[i]\), alors \( zz'\in \eZ[i]\) et de plus
    \begin{equation}
        N(zz')=N(z)N(z')=nm.
    \end{equation}
...
\begin{proof}
    ...
    Nous commençons par écrire le quotient \( \eZ[i]/(p)\) sous d'autres formes. D'abord en remarquant que si \( I\) et \( J\) sont deux idéaux, on a \( (\eA/I)/J\simeq (\eA/J)/I\), alors, en tenant compte du fait que \( \eZ[i]=\eZ[X]/(X^2+1)\), nous avons
    \begin{equation}
...
%TODO: la première partie de la preuve est imprécise, voire fausse : je comprends : \( a\mod n_2=a_2\mod n_1\), pas  \( a\mod n_2=a_2\mod n_2\)!   Comment fait-on apparaitre le modulo n1xn2 ?
    \begin{subequations}
        \begin{align}
            a\mod n_1 &= a_1u_2n_2\mod n_1        \\
...
\begin{proof}
    ...
    \begin{subproof}
        \item[Définition de \( J_k\)]
            ...
            dès que \( k\neq l\).
        \item[\( I_i+J_i=A\)]
%TODO: Pourquoi \( J_i\subset  I_k\) ??? au dessus, c'est l'inverse !
