...
\begin{normaltext}      \label{NORMooJCUYooZALUBM}
...
Ceci pour dire que oui, \( \eK(\alpha)_{\eL}\) dépend de \( \eL\), mais que si le contexte est clair, nous omettrons de préciser le corps «ambiant» dans lequel nous faisons l'extension de \( \eK\) par \( \alpha\).
...
\begin{proposition}     \label{PROPooYSFNooFGbbCi}
    ...
    \begin{enumerate}
        \item
            \( \eK(\alpha)=\Frac\big( \eK[\alpha] \big)\),
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\ref{ITEMooWMQTooLnepQl} implique \ref{ITEMooYTEBooUuEfBz}]
            ...
            Nous montrons que non-\ref{ITEMooYTEBooUuEfBz} implique non-\ref{ITEMooWMQTooLnepQl}. Nous disons donc que \( \alpha\) est transcendant sur \( \eK\); cela implique par la proposition \ref{ooUZDQooOasiRQ}\ref{ITEMooUZDQooOasiRQ} que \( \eK[\alpha]=\eK[X]\) en tant qu'anneaux. Donc \( \eK[\alpha]\) n'est pas un corps parce que \( \eK[X]\) ne l'est pas.
...
\begin{proposition}[Existence d'un corps de rupture]
    Soit un corps \( \eK\) et un polynôme non constant \( P\). Le corps \( \eK[X]/(P)\) est un corps de rupture pour \( P\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[L'isomorphisme]
            ...
            Nous considérons l'application
            \begin{equation}        \label{EQooZUEMooBjdiSB}
                \begin{aligned}
                    \varphi\colon \eK[X]/(P)&\to \eK(\bar X) \\
                    \bar Q&\mapsto Q(\bar X) 
                \end{aligned}
            \end{equation}
            et nous allons montrer que c'est un isomorphisme de corps.

        \item[Bien définie]

            L'équation \eqref{EQooZUEMooBjdiSB} définit bien \( \varphi\). En effet si \( \bar Q_1=\bar Q_2\), alors \( Q_2=Q_1+P\) et vu que \( P(\alpha)=0\) nous avons bien \( Q_1(\alpha)=Q_2(\alpha)\). 

        \item[Injective]

            Si \( \varphi(\bar Q_1)=\varphi(\bar Q_2)\). Nous avons, pour $Q_1$, les égalités
            \begin{equation}
                Q_1(\bar X)=\overline{ Q_1(X) }=\bar Q_1,
            \end{equation}
            et égalités similaires pour \( Q_2\). Donc \( \bar Q_1=\bar Q_2\).
