...
\begin{example}
    Soit \( \eK=\eF_{29}\). Le nombre \( 29\) étant premier, \( \eK\) est un corps premier\footnote{Corps premier, définition \ref{}. Lien avec le fait que \( 29\) soit premier, voir \ref{}.}. C'est le corps des entiers modulo \( 29\). Nous avons donc
    \begin{equation}
...
\begin{proof}
    Le petit théorème de Fermat \cite{ThoOPQOiO} nous dit que \( m^{q-1}\in [1]_q\). Mais \( m^{q-1}=m^{2p}=(m^p)^2\). Donc
    \begin{equation}
        (m^p)^2\in[1]_q.
    \end{equation}
    Vu que \( q\) est premier, l'anneau \( \eZ/q\eZ\) est intègre par le corolaire \ref{CorZnInternprem}. Vu que \( 1\neq -1\), la proposition \ref{LEMooHDJIooPTpUCJ} fonctionne et nous avons \( m^p\in[\pm 1]_q\).
\end{proof}

\begin{theorem}[\cite{BIBooIXPJooEyrkRc,BIBooDYIOooNtTWFI, BIBooQTFDooPOdQhr}]     \label{THOooSZXWooVeHdrh}
    Soit un nombre premier de Sophie Germain (définition \ref{DEFooCVFJooKCdVVD}). Il n'existe pas de solutions dans \( \eZ^3\) au système
    \begin{equation}
        \begin{cases}
            x^p+y^p+z^p=0   \\
            [xyz]_p\neq 0.
...
\begin{proof}
    Nous supposons que \( (x,y,z)\in \eZ^3\) est une solution tell que \( \pgcd(x,y,z)=1\). 
    \begin{subproof}
        \item[\( x\), \( y\) et \( z\) sont premiers deux à deux]
            ...
            Ce qui est vrai pour le coupe \( (x,y)\) est encore vrai pour \( (x,z)\) et pour \( (y,z)\).
        ...
        \item[\( \pgcd(u,y+z)=1\)]
            ...
            \begin{subequations}
                ...
                \end{align}
            \end{subequations}
            Nous avons donc \( p[y]_{\alpha}^{p-1}=0\). Mais \(  \eZ/\alpha\eZ\) est un anneau intègre\footnote{Corollaire \ref{CorZnInternprem}.}. Donc la règle du produit nul s'applique\footnote{Proposition \ref{DEFooTAOPooWDPYmd}.} et nous avons deux possibilités : \( [p]_{\alpha}=0\) ou \( [y]_{\alpha}^{p-1}=0\). 

            D'abord \( \alpha\) divise déjà \( x\). Or \( \pgcd(x,y)=1\). Donc \( \alpha\) ne peut pas diviser \( y\) (à part si \( \alpha=1\) évidemment), et à fortiori pas \( y^{p-1}\) non plus. Nous en déduisons que \( [p]_{\alpha}=0\). Vu que \( p\) est premier, nous avons \( p=\alpha\).

            Mais nous avons déjà vu que \( \alpha\) divisait \( x\). Donc \( 0=[x]_{\alpha}=[x]_p\). Mais cela n'est pas possible parce que \( [xyz]_p\neq 0\).
        ...
        \item[Utilisation d'un lemme]
            ...
            \begin{equation}        \label{EQooKMPZooCShIIT}
                ...
                \end{cases}
            \end{equation}
            Ce que nous venons de faire pour \(y+x\) peut être fait pour les autres. Donc il existe \( a,b,c\in \eZ\) tels que
            \begin{subequations}
                \begin{align}
                    y+z&=a^p\\
                    x+z&=b^p\\
        ...
        \item[\( {[y^p]_q=[\pm 1]_q}\)] 
            ...
            \end{equation}
            parce que nous avons choisit que \( q\) divise \( x\). Nous avons maintenant les implications suivantes :
    ...
    Nous avons prouvé le théorème pour les triples \( (x,y,z)\) qui n'ont pas de facteurs communs. Si \( k\) est un diviseur commun de \( x\), \( y\) et \( z\), alors \( (x/k,y/k,z/k)\) est encore une solution, et donc n'existe pas.
\end{proof}
