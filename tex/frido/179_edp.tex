...
\begin{proposition}[Équation de transport à coefficients variables\cite{ooAUICooVUjyqo,ooPMPXooEpbDkm}]     \label{PROPooVQLBooQyFfEH}
    Soit une fonction \( c\colon \eR^2\to \eR\) de classe \( C^1\) en ses deux variables et uniformément Lipschitziennes en sa première variable\quext{Dans \cite{ooPMPXooEpbDkm}, on ne demande que continue puis uniformément Lipschitz. Moi je crois que ce n'est pas assez pour assurer la dérivabilité de \( Y\) par rapport à \( x\), et encore moins pour permuter les dérivées dans \( \partial^2_{tx}Y\).} et \( g\in C^1(\eR)\). Alors l'équation aux dérivées partielles de premier ordre
...
\begin{proof}
    Nous commençons par étudier l'existence et l'unicité de la fonction \( Y\) définie par le problème \ref{SUBEQSooTOCEooHRhXiv}. La fonction \( c\) ici est dans les hypothèses de la fonction $f$ du théorème de Cauchy-Lipschitz global \ref{THOooZIVRooPSWMxg}. D'où l'existence et l'unicité de la fonction \( s\mapsto Y(s;x,y)\) sur \( \eR\) pour chaque \( (x,y)\) donné\footnote{Avec l'énoncé tel que donné dans \cite{ooAUICooVUjyqo}, il faut utiliser la technique de \ref{NORMooZROGooZfsdnZ} pour l'existence globale, parce que la fonction \( b\) là-dedans n'est pas dans les mêmes hypothèses.}.

    \begin{subproof}

        \item[Existence]

     Nous pouvons nous lancer dans la preuve proprement dite en vérifiant que la formule \eqref{EQooLFYNooJewRhE} définit bien une solution du problème. D'abord nous avons
    \begin{subequations}
        \begin{align}
            \frac{ \partial u }{ \partial x  }(x,y)&=g'\big( Y(0;x,y) \big)\frac{ \partial Y }{ \partial x }(0;x,y)\\
            \frac{ \partial u }{ \partial y  }(x,y)&=g'\big( Y(0;x,y) \big)\frac{ \partial Y }{ \partial y }(0;x,y)
        \end{align}
    \end{subequations}
    En remplaçant dans l'équation,
    \begin{equation}
        \frac{ \partial u }{ \partial x }+c\frac{ \partial u }{ \partial y }=g'\big( Y(0;x,y) \big)h(0;x,y)
    \end{equation}
    où \( h\) est la fonction définie par
    \begin{equation}
        h(s;x,y)=\frac{ \partial Y }{ \partial x }(s;x,y)+c(x,y)\frac{ \partial Y }{ \partial y }(s;x,y).
    \end{equation}
    
        \item[Une EDO pour \( h\)]
        
            Nous prouvons que, pour \( (x,y)\) donné, la fonction \( \varphi(s)=h(s;x,y)\) vérifie l'équation différentielle ordinaire
            \begin{subequations}
                \begin{numcases}{}
                    \varphi'(s)=\alpha(s)\varphi(s)\\
                    \alpha(s)=\frac{ \partial c }{ \partial y }\big( Y(s;x,y),s \big).
                \end{numcases}
            \end{subequations}
            Bien entendu, \( \alpha\) dépend aussi de \( x\) et \( y\).
