...
\begin{propositionDef}[\cite{ooGSDHooLgtHCb}]       \label{DEFooGJYXooOiJQvP}
	...
	\begin{enumerate}
		...
		\item		\label{ITEMooECDBooJdJEza}
		      Nous notons \( a/b\) la classe de \( (a,b)\). Si \( a\neq 0\), alors l'inverse de \( a/b\) est \( b/a\).
	\end{enumerate}
...
\begin{lemma}       \label{LEMooBJRCooIZnaid}
	Soient \( a,b,x,y\in \eZ\). Si \( b,y\neq 0\), nous avons \( ay=bx\) dans \( \eZ\) si et seulement si nous avons \( ab^{-1}=xy^{-1}\) dans \( \eQ\)\footnote{Rappel de \ref{NORMooVYEMooUiUfnE} : si \( a\in \eZ\), nous nommons \( a\in \eQ\) l'élément \( a/1\).}.
\end{lemma}
