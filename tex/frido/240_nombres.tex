...
\begin{normaltext}		\label{NORMooVYEMooUiUfnE}
	En vertu de la proposition \ref{PROPooUULNooKbwuEw}, à partir de maintenant, nous allons identifier la partie \( f(\eZ)\) à \( \eZ\). Nous nous autorisons donc à dire que \( 4\in \eQ\) ou que \( -7\in \eQ\), et même que \( 0\in \eQ\).
\end{normaltext}
...
\begin{proof}
	...
	\begin{equation}
		(xy)b^2=yayb=aby^2.
	\end{equation}
	Vu que \( ab\geq 0\) et \( y^2\geq 0\), le membre de droite est positif (proposition \ref{PROPooYFUBooJUZgwH}). Nous avons donc aussi \( xyb^2\geq 0\). La proposition \ref{PROPooYFUBooJUZgwH}\ref{ITEMooYUNEooAKZJEh} dit encore que dans ce cas \( xy\geq 0\).
\end{proof}
...
\begin{propositionDef}[Ordre sur \( \eQ\)\cite{RWWJooJdjxEK}]      \label{DEFooZEXXooUtOhqB}
	...
	\end{equation}
	si et seulement si \( ad\leq bc\) dans \( \eZ\).

	Avec cette définition, \( (\eQ,\geq)\) est un ensemble totalement ordonné\footnote{Définition \ref{DefooFLYOooRaGYRk} pour un ordre et \ref{DEFooVGYQooUhUZGr} pour total.}.
\end{propositionDef}
...
\begin{proof}
	...
	\begin{description}
		\item[Réflexif]
			Vu que \( ab\leq ab\) dans \( \eZ\), nous avons \( a/b\leq a/b\) dans \( \eQ\).
		\item[Antisymétrique]
			Si \( a/b\leq x/y\) et \( x/y\leq a/b\), nous avons les inégalités \( ay\leq bx\) et \( bx\leq ay\) dans \( \eZ\). Vu que \( \eZ\) est totalement ordonné\footnote{Proposition \ref{PROPooMYYDooOABOdB}.}, cela implique que \( bx=ay\). Le lemme \ref{LEMooBJRCooIZnaid} donne alors \( ab^{-1}=xy^{-1}\).
	\end{description}<++>
\end{proof}<++>
