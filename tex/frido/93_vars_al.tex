...
% Copyright (c) 2011-2013,2016-2020, 2023-2026
%   Laurent Claessens
% See the file fdl-1.3.txt for copying conditions.

%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Les lois usuelles : du blabla}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\begin{normaltext}
	Nous, nous faisons des math. En math les mots «boules», «hasard», «en remettant dans l'urne» ne sont pas définis. Considérez donc l'ensemble du blabla ci-dessous comme nul et non avenu.
\end{normaltext}
...
\begin{definition}	\label{DEFooHKZIooBXmNBb}
	Une variable aléatoire \(X \colon \Omega\to \{ 0,1 \}  \) est de \defe{de Bernoulli}{variable aléatoire de Bernoulli} de paramètre \( p\in \mathopen[ 0,1\mathclose]\) si \( P(X=0)=1-p\) et \( P(X=1)=p\).
\end{definition}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooJGAQooSSmybt}
	Si \(X \colon \Omega\to \{ 0,1 \}  \) est une variable aléatoire de Bernoulli, alors
	\begin{enumerate}
		\item		\label{ITEMooOKZQooOmIOJj}
		      \( E(X)=p\),
		\item		\label{ITEMooAWMOooOGhKzB}
		      \( \Var(X)=p(1-p)\).
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	En deux points.
	\begin{subproof}
		\spitem[Pour l'espérance]
		%-----------------------------------------------------------
		Nous devons calculer \( E(X)=\int_{\Omega}X(\omega)d\omega\) en utilisant la définition \eqref{EqDefintYfdmu}. La fonction \( X\) est elle-même une fonction simple et donc étagée. En posant \( A_0=X^{-1}(\{ 0 \})\), \( A_1=X^{-1}(\{ 1 \})\) et \( Z=\Omega\setminus (A_0\cup A_0)\), la partie \( Z\) est de mesure nulle et nous avons
		\begin{equation}
			X(\omega)=0\times \mtu_{A_0}(\omega)+1\times \mtu_{A_1}(\omega)+f(\omega)\mtu_{Z}(\omega)
		\end{equation}
		où \( f\) est une fonction qui n'a aucune importance. Nous avons donc
		\begin{equation}
			\int_{\Omega}X=1\times P(A_1)=p.
		\end{equation}

		\spitem[Pour la variance]
		%-----------------------------------------------------------
		La définition \ref{DEFooBHQOooVzpsob} nous pousse à faire le calcul suivant :
		\begin{subequations}
			\begin{align}
				\Var(X) & =\| X-E(X) \|^2_{L^2}=\int_{\Omega}(X-p)^2=(-p)^2P(A_0)+(1-p)^2P(A_1)=p^2(1-p)+(1-p)^2p=p(1-p).
			\end{align}
		\end{subequations}
		Notez qu'ici nous n'avons pas précisé qu'il y avait aussi potentiellement une partie de mesure nulle.
	\end{subproof}
\end{proof}
...
\begin{definition}[\cite{MonCerveau}]	\label{DEFooTSYAooZSLIdY}
	Une variable aléatoire \(X \colon \Omega\to \{ 0,\ldots,n \}  \) est \defe{binomiale}{loi binomiale} de paramètres \( n\) et \( p\) si il existe \( n\) variables aléatoires \(Y_i \colon \Omega\to \{ 0,1 \}  \) (\( i=1,\ldots,n\)) indépendantes\quext{Je crois que ça implique que \( \Card(\Omega)\geq 2^n\), mais ce n'est pas important ici.} telles que
	\begin{equation}
		X=Y_1+\ldots+Y_n.
	\end{equation}
\end{definition}

\begin{proposition}[\cite{MonCerveau,BIBooLDIPooRKBZxS}]    \label{PROPooIBZKooYikwjJ}
	Si \(X \colon \Omega\to \{ 1,\ldots,n \}  \) est binomial de paramètres \( n,p\) alors
	\begin{enumerate}
		\item
		      Pour chaque \( k=1,\ldots,n\) nous avons
		      \begin{equation}
		...
		\item
		      ...
		      \end{equation}
	\end{enumerate}
	%TODOooRYNHooIVgFNl. Prouver ça.
\end{proposition}

\begin{proof}
	Soient des variables aléatoires de Bernoulli indépendantes \( (Y_i \colon \Omega\to \{ 0,1 \})_{i=1,\ldots,n}\) telles que \( X=Y_1+\ldots+Y_n\).
	\begin{subproof}
		\spitem[Pour \( P(X=k)\)]
		%-----------------------------------------------------------
		Nous considérons l'ensemble \( C_k=\{ s\in \{ 0,1 \}^n\tq \sum_is_i=k \}\) du lemme \ref{LEMooUTDTooXAmvdF}. Pour chaque \( s\in C_k\) nous posons
		\begin{equation}
			\Omega_s=\{ \omega\in \Omega\tq\forall i, Y_i(\omega)=s_i  \}.
		\end{equation}
		Nous avons \( P(X=k)=\sum_{s\in C_k}P(\Omega_s)\). Étant donné que les variables aléatoires \( Y_i\) sont indépendantes et que
		\begin{equation}
			P(Y_i=s_i)=\begin{cases}
				p   & \text{si } s_i=1  \\
				1-p & \text{si } s_i=0,
			\end{cases}
		\end{equation}
		nous avons
		\begin{equation}
			P(\Omega_s)=\prod_{i=1}^nP(Y_i=s_i)=p^k(1-p)^{n-k}.
		\end{equation}
		Pour le voir, vous pouvez couper le produit en deux parties : les facteurs avec \( s_i=1\) et ceux avec \( s_i=0\). Enfin nous pouvons faire le calcul
		\begin{subequations}
			\begin{align}
				P(X=k) & =\sum_{s\in C_k}P(\Omega_s)                                            \\
				       & =\sum_{s\in C_k}p^k(1-p)^{n-k}                                         \\
				       & =\Card(C_k)p^{k}(1-p)^{n-k}                                            \\
				       & =\binom{ n }{ k }p^k(1-p)^{n-k} & \text{lem. \ref{LEMooUTDTooXAmvdF}.}
			\end{align}
		\end{subequations}
		...
		\begin{equation}
			E(X)=\int_{\Omega}X=\int_{\Omega}\sum_iY_i=\sum_i\int_{\Omega}Y_i=\sum_iE(Y_i).
		\end{equation}
		Vu que \( Y_i\) est de Bernoulli nous avons \( E(Y_i)=p\) par \ref{PROPooJGAQooSSmybt}\ref{ITEMooOKZQooOmIOJj}. Nous avons donc \( E(X)=np\).

		\spitem[Pour la variance]
		%-----------------------------------------------------------
		Les variables aléatoires \( Y_i\) étant indépendantes, le lemme \ref{LemVarXpYsmindep} permet de juste les additionner. La valeur de \(\Var(Y_i)=p(1-p) \) est donnée dans le lemme \ref{PROPooJGAQooSSmybt}\ref{ITEMooAWMOooOGhKzB}. Donc \( \Var(X)=np(1-p)\).
	\end{subproof}
\end{proof}
...
\begin{theorem}[\wikipedia{fr}{Loi_de_Poisson}{Wikipédia}]      \label{ThojDZjuj}
...
\begin{definition}[Loi exponentielle]       \label{DEFooTSFNooULWNHY}
	La \defe{loi exponentielle}{loi exponentielle} de paramètre \( \lambda>0\), notée \( \dE(\lambda)\) est la mesure de probabilité sur \( \big( \eR,\Borelien(\eR) \big)   \) ayant pour densité\footnote{Densité, définition \ref{DEFooRNKZooRczFwB}.} l'application
	\begin{equation}
		\begin{aligned}
			f\colon \eR & \to \eR^+ \\
			x           & \mapsto
		...
		\end{aligned}
	\end{equation}
	%TODOooLTHWooHTTkCb: prouver que ça donne bien une mesure de probabilité
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooKLJIooUFfGwa}
	Soit une variable aléatoire \(X \colon \Omega\to \eR  \). Si pour tout \( t>0\) nous avons
	\begin{equation}
		P(X>t)=e^{-\lambda t},
	\end{equation}
	alors \( X\) suit une loi exponentielle.
	%TODOooYITNooEghRKs. Prouver ça.
\end{proposition}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooXRMWooYjPMWN}
...
\begin{proposition}[Somme de variables aléatoires exponentielles indépendantes\cite{BIBooQMIXooVvDwBw}]	\label{PROPooBZREooUTvonZ}
...
\begin{proof}
	...
	\end{equation}
	Cette dernière intégrale vérifie les hypothèses du théorème~\ref{THOooZPUYooQAlUTa}, de telle sorte que \( \varphi\) soit une fonction dérivable et \( \varphi'(x)=f(x)\).
...
%TODOooTVYNooWYbUPy. En faire une proposition, et dire que c'est une réciproque partielle de THOooYRIMooSREVEO
\begin{equation}
...
\begin{theorem}[\cite{ProbaDanielLi}]		\label{THOooLRZNooFXWwqy}
...
\begin{proof}
	%TODOooJNPRooYtpRxd Faire la preuve avec m différente de 0.
	Nous effectuons la preuve avec \( m=0\)\quext{À mon avis le cas \( m\neq 0\) se déduit alors facilement. Écrivez-moi pour me donner la preuve quand vous l'avez.}. Nous choisissons l'espace probabilisé \( (\Omega,\tribA)=(\eR^r,\Borelien(\eR^r))\) où \( r\) est le rang de \( K\) muni de la probabilité de densité
...
Lorsqu'une matrice symétrique et positive \( K\) est donnée, nous avons créé un vecteur gaussien de covariance \( K\) en créant \( X=AY\) où \( Y\) est le vecteur gaussien «le plus simple» et où \( A\) est donnée par \( AA^t=K\). La proposition suivante montre l'inverse : un vecteur gaussien \( X\) peut se réduire au vecteur gaussien «le plus simple» en utilisant la transformation \( Y=A^{-1}X\) où \( A\) est encore donnée par \( AA^t=K\). Ce résultat nous permettra de voir les vecteurs gaussiens généraux comme des «changements de coordonnées» par rapport au vecteur gaussien simple \( Y=(Y_1,\ldots, Y_d)\) avec \( Y_i\sim\dN(0,1)\).
...
\begin{definition}		\label{DEFooKWKNooWbGAmf}
	La loi \( \chi^2\)\index{loi!\( \chi^2\)} à \( d\) degrés de liberté est la loi de la variable aléatoire \( Y_1^2+\cdots+Y_d^2\) si les \( (Y_i)\) sont des variables aléatoires normales indépendantes centrées et réduites.
...
%TODO : à mon avis tout cela est très général et mériterait d'être mis en corolaire de la loi des grands nombres.
...
\lstinputlisting{tex/frido/code_sage2.py}
...
\begin{example}
	...
	\end{equation}
	c'est-à-dire environ \( n=10^{11}\), ce qui soit dit en passant est très largement au delà des capacités de la commande de scilab\cite{BIBooMXHQooDAXmCz}\quext{Je n'ai pas vérifié si c'est encore le cas avec les nouvelles versions de scilab. Écrivez-moi si vous le savez.}.
\end{example}
