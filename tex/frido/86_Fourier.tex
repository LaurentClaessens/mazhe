...
Le \defe{noyau de Fejér}{noyau!Fejér}\index{Fejér!noyau} est la moyenne de Cesàro des noyaux de Dirichlet :
\begin{equation}
...
%TODO: "\tilde S_k(f)" au lieu de "S_k(f) ?
...
\begin{proof}
    ...
    \end{equation}
%TODO: mauvais francais: (un a fortiori sur)   sur quoi ? sur la table ?
...
%TODO Par densité de la densité ???
...
\begin{proposition}[Critère de Weyl\cite{ytMOpe,KXjFWKA}]  \label{PropDMvPDc}
    ...
    \begin{enumerate}
        ...
        \item\label{ItemKWcZTHqiii}
            Pour tout \( p\in\eN^\star\) nous avons
            \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    Cela est démontré pour la fonction caractéristique \( \varphi=\mtu_{\mathopen[ x , 1 [}\). Si nous prenons une fonction caractéristique \( \mtu_{\mathopen[ a , b \mathclose]}\), nous avons le même genre de preuve parce que \( \mtu_{\mathopen[ a , b [}\) est une combinaisons linéaire de fonctions du type \( \mtu_{\mathopen[ x , 1 [}\).
    ...
    \item[\ref{ItemKWcZTHqii}\( \Rightarrow\)\ref{ItemKWcZTHqiii}]
        Comme \( e^{2i\pi px_k}=\cos(2\pi px_k)+i\sin(2\pi px_k)\) est une fonction périodique, c'est immédiat.
...
\begin{proof}
    Nous commençons par supposer que \( f\in\mD\) n'ait qu'un seul point de discontinuité, \( x_0\). Alors nous considérons la fonction \( f_n\) qui est égale à \( f\) sur \( S^1\setminus B(x_0,\frac{1}{ n })\) et qui sur \( B(x_n,\frac{1}{ n })\) est le segment de droite joignant \( f(x_0-\frac{1}{ n })\) et \( f(x_0+\frac{1}{ n })\). C'est une fonction continue, et de plus nous avons
    \begin{equation}
        | f_n(x) |\leq \| f \|_{\infty}
    \end{equation}
%TODO: c'est quoi le "raccord affin" ? Ref ?
...
\begin{proof}
    ...
    \end{equation}
%TODO: Il faut exclure le cas n=0 je pense : -> 1/0 non défini  => n\in \eN^\star
...
\begin{proof}
    ...
    \begin{equation}
%TODO: je ne comprend pas le (n^2-n) !?  Plutot n  tout court ?
...
\begin{equation}    \label{EqdMtbOB}
    ...
    \end{cases}
\end{equation}
%TODO: moi pas comprendre "le terme numéro \( k\) de la suite \( n\)". Je comprends : c'est 1 (voir code line 882)
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Pour \eqref{SUBEQooXSCEooIJXFxT}. Comme \( f(T)=f(0)\), et que \( t\mapsto e^{-2i\pi nt/T}\) est périodique de période \( T\), \( f(T) e^{-2i\pi n}-f(0) e^{i0}=0\).
    \end{itemize}
\end{proof}
...
\begin{theorem}[Série de Laurent\cite{BIBooUBUAooHyhrlg}]       \label{THOooMKJOooVghZyG}
    ...
    \item       \label{ITEMooOYCPooZZAyKs}
            Cette série converge uniformément sur tout compact de \( C\) (ensemble des couronnes).
    \item
...
\begin{proposition}     \label{PROPooBMZGooLoaGLK}
    ...
    avec
    \begin{equation}
        a_n=\frac{1}{ 2\pi i }\int_{B(a,r)}\frac{ f(z) }{ (z-a)^{n+1} }dz.
    \end{equation}
\end{proposition}
