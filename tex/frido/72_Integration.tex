...
La condition de rang maximum sert à éviter le genre de cas de la figure~\ref{LabelFigExempleNonRang} qui représente l'image de l'ouvert \( \mathopen] -1 , 1 \mathclose[\) par l'application \( F(t)=(t^2,t^3)\).
	...
	Nous nous proposons maintenant de calculer la surface de la sphère \( S^2=x^2+y^2+z^2=R^2\). L'application \( F\colon B( (0,0),R)\to \eR^3\) donnée par
	\begin{equation}
		F(x,y)=\begin{pmatrix}
			x \\
			y \\
...
\begin{proposition}     \label{PROPooAVVNooOOlSzr}
	L'aire\footnote{Définition \ref{DEFooPZRDooWbbBXy}.} du parallélogramme\footnote{Définition \ref{DEFooDTFCooCTdaDO}.}  basé en \( a\) et formé sur \( u\) et \( u\) (que nous supposons n'être pas colinéaires) est donné par
	\begin{equation}
...
\begin{theorem}[Brouwer dans \( \eR^n\) version continue] 		\label{ThoRGjGdO}
	Toute application continue\footnote{Une fonction continue sur un fermé de \( \eR^n\) est à comprendre pour la topologie induite.} \( \overline{B(0,)}  \to \overline{B(0,1)}\) admet un point fixe.
\end{theorem}
\index{théorème!Brouwer}

\begin{proof}
	Soit une application continue \(f \colon \overline{B(0,)}\to \overline{B(0,1)}  \). Nous commençons par définir une suite de fonctions
	\begin{equation}
...
\begin{remark}
	...
	Cette définition ne dépend pas du paramétrage choisie, mais le signe change selon le sens du chemin.
\end{remark}
