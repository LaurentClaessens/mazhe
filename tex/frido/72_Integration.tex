...
\begin{definition}[Variété orientable]      \label{DEFooSWREooNdQpdA}
    Une variété $M$ est \defe{orientable}{variété orientable} s'il existe un atlas de $M$ tel que deux cartes quelconques ont toujours même orientation\footnote{Définition \ref{}.}. Une variété est \defe{orientée}{variété !orientée} lorsque qu'un tel choix d'atlas est fait.
\end{definition}
...
\begin{lemmaDef}   \label{LemGPmRGZ}
    Soit une variété \( M\) munie d'un recouvrement par un nombre fini d'ouverts\footnote{Si \( M\) n'est pas compacte, alors il faut utiliser une version un peu plus élaborée du lemme\cite{SpindelGeomDoff}.}\( \{ U_i \}\). Alors il existe une famille de fonctions \( f_i\in  C^{\infty}(M)\) telle que
    \begin{enumerate}
        \item
            \( \supp f_i\subset U_i\),
        \item
...
\begin{probleme}
J'ai du mal à croire ce lemme \ref{LemGPmRGZ} parce que si \( M=\mathopen] -1 , 1 \mathclose[\cup \mathopen] 3 , 3 \mathclose[\) et que \( U_1=\mathopen] -1 , 1 \mathclose[\) et \( U_2=\mathopen] 3 , 4 \mathclose[\), je ne vois pas comment il est possible d'avoir une fonction définie sur un compact à l'intérieur de \( U_1\) et qui vaut \( 1\) sur tout \( U_1\).
\end{probleme}
...
\begin{propositionDef}[Intégrale d'une forme sur une variété]       \label{DEFooOMQLooGiJWZS}
    ...
    Si \( \{ f_i \}\) est une partition de l'unité subordonnée\footnote{Définition \ref{LemGPmRGZ}.} à un atlas de \( M\) nous définissons
    \begin{equation}
...
\begin{proof}
   ...
   Le point \( g(x) \) est la solution du système
    ...
    %TODOooYNVUooWFTttD faire une référence à la définition de forme de volume.
    ...
    \begin{equation}
        ...
        =0
    \end{equation}
    %TODOooMRSDooXmtdjS Les points 1 et 4 demandent de citer un théorème.
...
\begin{probleme}    \label{PROBooSSOBooCsovCy}
    La démonstration souffre de quelque problèmes. Voir en particulier la question et la réponse ici : \url{  https://math.stackexchange.com/questions/1411178/proof-of-brouwer-fixed-point-theorem-using-stokess-theorem}
\end{probleme}
...
\begin{definition}[\cite{BIBooACKCooYGRoEO}]
	Une force $F\colon D\subset\eR^n\to \eR^n$ est \defe{conservative}{champ de vecteur conservatif} si elle dérive d'un potentiel, c'est-à-dire s'il existe une fonction $V\in C^1(D,\eR)$ telle que
	\begin{equation}
		F(x)=(\nabla V)(x).
	\end{equation}
\end{definition}
