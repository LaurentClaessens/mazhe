...
\begin{proposition}[Analytique et Lipschitz\cite{MonCerveau,BIBooMistral}]	\label{PROPooSLVEooFeDlcO}
	Soit un ouvert \( U\) de \( \eR^n\) et une fonction \(f \colon \eR^n\to \eR^n  \) analytique\footnote{Application analytique, définition \ref{DEFooAIHMooKbWsBt}.} sur \( \bar U\). Alors pour tout compact \( K\) de \( U\), il existe un réel \( L_K>0\) tel que
	\begin{equation}
		\| f(a)-f(b) \|\leq L_K\| a-b \|
	\end{equation}
	pour tout \( a,b\in K\).
...
\begin{proof}
	Étant donné que \( f\) est analytique, elle est \( C^{\infty}\) (proposition \ref{PROPooGMWRooPIzwus}). En particulier l'application \(df \colon K\to \aL(\eR^n,\eR^n)  \) est continue et donc bornée. Nous utilisons le théorème des accroissements finis \ref{val_medio_2} :
	\begin{equation}
		\| f(a)-f(b) \|\leq \sup_{x\in \mathopen[ a,b\mathclose]}\| df_x \|\| b-a \|\leq \sup_{x\in K}\| df_x \|\| b-a \|.
	\end{equation}
	Nous avons donc le résultat avec \( L_k=\sup_{x\in K}\| df_x \|\).
\end{proof}


\begin{theorem}[Cauchy-Lipschitz réel analytique\cite{BIBChatGPTDifficile}]	\label{THOooHDHCooLMkfaB}
	Nous posons \( \Lambda=\eR^m\) et nous considérons un ouvert \( U\) de \( \eR^n\), un ouvert \( \Gamma\) de \( \Lambda\) ainsi qu'une fonction \(f \colon \eR\times \eR^n \times \Gamma\to \eR^n \)  analytique\footnote{Application analytique, définition \ref{DEFooAIHMooKbWsBt}.} sur \( \overline{\eR\times U\times \Gamma}\).

	Alors
	\begin{enumerate}
		\item
		      Pour tout \( (t_0,y_0,\lambda)\in \eR\times U\Gamma\), il existe \( R>0\) et une unique application
		      \begin{equation}
