...
% Copyright (c) 2008-2009,2011-2017, 2019-2020
%   Laurent Claessens
...
\begin{definition}  \label{DefKMCGooOeFKlA}
	Dans le cas de l'équation différentielle \( y'(t)=f\big( y(t),t \big)\) pour \( y\colon \eR\to \eR^n\), un point \( a\in \eR^n\) est un \defe{point d'équilibre}{point d'équilibre} lorsque la fonction constante \( y(t)=a\) est une solution.
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Majoration de \(  q\big( y(t) \big)'  \)]
		Nous posons
		\begin{equation}
			\begin{aligned}
				r\colon \eR^n & \to \eR^n        \\
		...
		\spitem[Si \( q(y_0)<\delta  \) alors \( q\big( y(t) \big) < \delta \)]
		...
		\spitem[À propos de \(  e^{\beta t}q(y)\)]
		...
		\spitem[Décroissance exponentielle]
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Comment théorème de Cauchy-Lipschitz s'applique]
		...
		\spitem[Les solutions restent positives]
...
\begin{definition}[\cite{BIBooLKCRooZoJskD}]
