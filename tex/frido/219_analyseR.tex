...
\begin{proposition}[\cite{ooRCDWooONrayj,ooVNAOooAuQSse,ooOGGJooCGQgDO}]        \label{PROPooOUZOooEcYKxn}
	...
	\begin{enumerate}
		...
		\item       \label{ITEMooMQERooBCqnvS}
		      ...
		      \end{equation}
		      Cette formule est appelée \defe{règle de Leibniz}{Leibnitz}\footnote{Pour des formes plus générales, voir la proposition \ref{PROPooTEACooKXUXKl} et le lemme \ref{LEMooOLQTooEHJuBc}.}.
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Pour \ref{ITEMooMQERooBCqnvS}, règle de Leibniz]
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooZQIAooVbgFKo}
	Soit le monôme \(f \colon \eR\to \eR  \) donné par \( f(x)=x^n\). Alors
	\begin{enumerate}
		\item		\label{ITEMooXPYKooZeZsMg}
		      \( f\) est dérivable et \( f'(x)=n x^{n-1}\)
		\item		\label{ITEMooQKYJooKqZhRS}
		      Si \( k\leq n\) nous avons
		      \begin{equation}
			      f^{(k)}(x)=\frac{ (n-1)! }{ (n-k-1)! }x^{n-k-1}.
		      \end{equation}
	\end{enumerate}
\end{lemma}

\begin{proof}
	Pour le point \ref{ITEMooXPYKooZeZsMg}, c'est la règle de Leibniz de la proposition \ref{PROPooOUZOooEcYKxn}\ref{ITEMooMQERooBCqnvS} couplée à une récurrence sur \( n\) et la dérivée \( x'=1\) de la proposition \ref{PROPooMEXAooZJJPYx}.

	Pour \ref{ITEMooQKYJooKqZhRS}, nous faisons une récurrence sur \( k\), le cas \( k=1\) étant le point \ref{ITEMooXPYKooZeZsMg}. Nous avons
	\begin{subequations}
		\begin{align}
			f^{(k+1)}(x) & =(f^{(k)})'(x)                                \\
			             & =\frac{ (n-1)! }{ (n-k-1)! }(n-k-1)x^{n-k-2}  \\
			             & =\frac{ (n-1)! }{ (n-k-2) }x^{n-k-2}          \\
			             & =\frac{ (n-1)! }{ (n-(k+1)-1) }x^{n-(k+1)-1}.
		\end{align}
	\end{subequations}
\end{proof}
...
\begin{proof}
	...
	Pour le produit vectoriel nous utilisons le développement du lemme \ref{LEMooYKNPooGVMuse} et la règle de Leibnitz de la proposition \ref{PROPooOUZOooEcYKxn}\ref{ITEMooMQERooBCqnvS} :
	\begin{subequations}
		\begin{align}
			\frac{d}{dt} \left[  u(t)\times v(t) \right] & = \sum_{ijk}\epsilon_{ijk}\frac{d}{dt} \left[ u(t)_iv(t)_j  \right] \\
			                                             & = \sum_{ijk}\epsilon_{ijk}\big[   u'(t)_iv(t)_j+u(t)_iv'(t)_j \big] \\
...
\begin{proposition}[difféomorphisme entre \( \eR\) et un ouvert borné]	\label{PROPooKPACooElwCbh}
	L'application
...
\begin{proposition}[\cite{BIBooIWVIooXgvRYc}]	\label{PROPooFEJXooMjsvFo}
	L'application
	\begin{equation}
		\begin{aligned}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Calcul de la réciproque]
		%-----------------------------------------------------------
...
\begin{theorem}[Inégalité des accroissements finis\cite{BIBooOOUMooCVcoXv}]	\label{THOooRALIooVGDcFi}
...
\begin{proposition}[Règle de L'Hôpital pour \( \frac{ 0 }{ 0 }\)\cite{ooHQARooDfptJC}]     \label{PROPooBZHTooHmyGsy}
...
\begin{proposition}[L'Hôpital pur \( \frac{ \infty }{ \infty }\)]      \label{PROPooTJVCooMeUhIy}
...
\begin{proof}
	Nous avons \( F'=G'\) et donc le corolaire \ref{CorNErEgcQ} dit qu'il existe \( c\) telle que \( F=G+c\).
\end{proof}
...
\begin{definition}
	Soit \( f\) une fonction de \( \eR^2\) dans \( \eR\) et soit \( c\) dans \( \eR\).  La \defe{\( z\)-section}{section!de graphe} de \( \Graph f\) à la hauteur \( c\) est donnée par
	\[
