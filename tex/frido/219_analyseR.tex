...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		Puisque \( g\) est dérivable en \( b\), la seconde ligne existe et \( u\) est continue en \( y=b=f(a)\). C'est la définition de la dérivée.
...
%TODO: il y a un probleme avec R^n (surtout le n !). Soit g est a valeurs dans \eR^n => corriger la ligne 200. Soit on enleve le n ?
...
Nous faisons la preuve pour le produit scalaire; sans doute que pour le produit vectoriel, une preuve semblable serait menée.
\begin{proof}
...
%TODO: il manque 3 termes non ? (3 x 3) -1 = 8
...
\begin{example}[difféomorphisme entre \( \eR\) et un ouvert borné]      \label{EXooGKPNooZtmJen}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	\( f\) est continue en \( x=1\) : il suffit de calculer les deux valeurs. En ce qui concerne la dérivabilité en \( x=1\), nous devons étudier
	\begin{equation}
...
\begin{theorem}[Généralisation du théorème de Rolle\cite{ooNRTLooCpjVdc}]       \label{THOooXDTBooFeSZoK}
	Soient \( -\infty\leq a<b\leq +\infty\). Soit une fonction dérivable \( f\colon \mathopen] a , b \mathclose[\to \eR\) telle que
		...
		\end{equation}
		avec \( \ell\in \overline{\eR} \). Alors il existe \( x\in \mathopen] a , b \mathclose[\) tel que \( f'(x)=0\).
\end{theorem}
...
%TODO : Prendre des variables secondaires notées "prime" comme pour la dérivée est "confusant" a',g' = pareil, non ? Je suggère alpha, beta, (ou a_1, b_1 ?) , et pour l'intervalle d'arrivée de phi, on n'a qu'à prendre: a_{\varphi},b_{\varphi}
...
\begin{proposition}[Règle de l'Hospital pour \( \frac{ 0 }{ 0 }\)\cite{ooHQARooDfptJC}]     \label{PROPooBZHTooHmyGsy}
	...
	\begin{equation}        \label{EQooJHWYooLGdbPH}
		\lim_{x\to a^+} \frac{ f(x) }{ g(x) }=\ell.
	\end{equation}
	Ici \( \ell\in \overline{\eR} \), et les hypothèses garantissent l'existence de la limite \eqref{EQooJHWYooLGdbPH}.
\end{proposition}
	...
	\begin{enumerate}
		...
		\item
		      \begin{equation}        \label{EQooVFYCooMjOGtI}
			      \lim_{x\to a^+} \frac{ f'(x) }{ g'(x) }=\ell\in \overline{\eR} .
		      \end{equation}
	\end{enumerate}
	Alors
	\begin{equation}
...
\begin{proof}
	Soit un intervalle \( \mathopen] x , y \mathclose[\) strictement inclus dans \( \mathopen] a , b \mathclose[\) avec \( x,y\in\eR\). Par le théorème des accroissements finis généralisés~\ref{THOooRIIBooOjkzMa} il existe \( c\in \mathopen] x , y \mathclose[\) tel que
		...
		\begin{equation}
			\frac{ f'(t) }{ g'(t) }\in V
		\end{equation}
		pour tout \( t\in \mathopen] a , y \mathclose[\). Nous utilisons ici avec subtilité le fait que ces intervalles sont une base de la topologie autour de \( \infty\). Maintenant \( f(y)\) et \( g(y)\) sont fixés et sont des nombres réels. Comme \( \lim_{x\to a} g(x)=\infty\) nous pouvons choisir \( r<y\) tel que nous ayons simultanément
		\begin{enumerate}
			\item
...
sage: plot3d(f,(x,-3,3),(y,-3,3))
	...
	\end{center}
	À part que l'ordinateur l'a montré, est-ce qu'on peut comprendre pourquoi le graphe de la fonction \( x^2+y^2\) ressemble à un bol ? En coordonnées cylindriques, le graphe s'écrit
	\begin{equation}
		z=r^2.
	\end{equation}
...
\begin{lemma}[\cite{MonCerveau}]        \label{LEMooYLIHooFBQyzC}
	...
	\end{equation}
	vérifie
%TODO: manque condition sur b   \forall b \in ??
	\begin{equation}
...
\begin{remark}
	De nombreuses sources parlent de dérivée \defe{dans la direction}{dérivée!direction} du vecteur \( v\) en définissant (avec une certaine raison) une \defe{direction}{direction} dans \( \eR^m\) comme étant un vecteur de norme \( 1\).
...
\begin{lemma}       \label{LEMooVOTHooPJcrWH}
	Nous notons \( \eK\) le corps \( \eR\) ou \( \eC\). Soient deux espaces vectoriels \( E\) et \( F\) sur \( K\). Soient une application \( f\colon E\to F\) ainsi que \( a,u\in E\) tels que \( \frac{ \partial f }{ \partial u }(a)\) existe.
...
\begin{example}
	...
	\[
		\partial_v f(a)=\lim_{\begin{subarray}{l}
				t\to 0\\ t\neq 0
			\end{subarray}}
		\frac{t^3 v_1^2 v_2}{t^5 v_1^4+ t^3 v_2^2}=\frac{v_1^2}{v_2},
	\]
