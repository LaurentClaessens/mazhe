...
\begin{proposition}[\cite{MonCerveau}] \label{PropRERZooYcEchc}
    Soit un espace mesuré \( (\Omega,\tribA,\mu)\) et une fonction \( f\in L^p(\Omega)\) telle que
    \begin{equation}
        \int_{\Omega}fg=0
    \end{equation}
    pour tout \( g\in L^q(\Omega)\). Alors \( f=0\).
\end{proposition}

\begin{proof}
    Nous rappelons que les éléments de \( L^p\) sont mesurables par définition; tous les représentants sont également mesurables parce que deux représentants diffèrent uniquement sur un ensemble de mesure nulle, qui est mesurable. Nous rappelons de plus que dire que \( \varphi\) est intégrable signifie que \( \real(f)^+\), \( \real(f)^-\), \( \imag(f)^+\) et \( \imag(f)^-\) sont intégrables.
    ...
    Prenons \( g=\mtu_{A_i}\). Par hypothèse nous avons \( \int_{\Omega}f\mtu_{A_i}d\mu=0\). En particulier,
    \begin{equation}
