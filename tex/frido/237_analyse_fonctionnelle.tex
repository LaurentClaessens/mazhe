...
\begin{lemma}       \label{LEMooMCYAooGMzbbs}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	Nous considérons sur \( \Fun(E,\eF)\) la topologie initiale\footnote{Définition \ref{PROPooGOEVooZBAOQh}.} des \( \{ f_x \}_{x\in E}\).

	La suite \( (\omega_n)\) dans \( \Fun(E,\eF)\) converge vers  \(\omega\in \Fun(E,\eF)\) si et seulement si \( \omega_n(x)\to\omega(x)\) pour tout \( x\in E\).
\end{lemma}

\begin{proof}
	C'est le lemme \ref{LEMooZLZKooHElBOs} appliqué à \( \Fun(E,\eF)\).
\end{proof}
...
\begin{proof}
	Nous considérons à nouveau \( \Fun(E, \eF)\) muni de la topologie du lemme \ref{LEMooMCYAooGMzbbs}. Nous considérons l'inclusion
	\begin{equation}
		\begin{aligned}
...
\begin{proposition}[\cite{BIBooFDGQooYferue,BIBChatGPT}]	\label{PROPooGQLOooNSgZnZ}
