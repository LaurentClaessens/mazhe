...
\begin{proposition}[\cite{BIBooOGSYooGDuyMJ}]	\label{PROPooJNIQooOXGCKY}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		C'est une topologie séparée par la proposition \ref{PROPooSEQRooZoQPbZ}.
		...
		Nous avons vu dans le lemme \ref{LEMooWBIBooOFElVy} que \( \overline{B_{E'}(0,1)}=K\cap E'\). Nous pouvons donc considérer (au moins) deux topologies dessus : celle induite de \( K\) et celle induite de \( E'\). Nous nommons \( \tau_P\) la topologie de \( \overline{B_{E'}(0,1)}\) induite de \( K\) et \( \tau_{\sigma}\) celle induite de \( E'\). Toujours d'après le lemme \ref{LEMooWBIBooOFElVy}, nous avons \( \tau_P=\tau_{\sigma}\).
	\end{subproof}
\end{proof}<++>
