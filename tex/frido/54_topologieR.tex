...
% Copyright (c) 2011-2018, 2020-2021
...
\begin{corollary}       \label{CORooSIKCooTncoQm}
    Un espace métrique compact est connexe si et seulement s'il est bien enchainé.
\end{corollary}

\begin{proof}
    Dans le sens direct, c'est la proposition \ref{PROPooBUNOooIvfugn}. Dans l'autre sens, si \( X\) est compact, alors \( X\) est fermé par la proposition \ref{PropUCUknHx}. Et vu qu'il est fermé et bien enchainé, la proposition \ref{PROPooXHTWooZibddZ} implique qu'il est connexe.
\end{proof}
...
\begin{definition}  \label{DefPNXlwmi}
    Si \( E\) est un espace vectoriel, une \defe{seminorme}{seminorme} sur \( E\) est une application \( p\colon E\to \eR\) telle que
    \begin{enumerate}
        \item
            \( p(x)\geq 0\),
...
La seule différence avec une norme est simplement qu'une seminorme peut s'annuler en des éléments non-nuls de l'espace.
...
\begin{definition}[Topologie et seminormes\cite{SOdaAdx,MUbDonp}]      \label{DEFooZTKAooWYUyDa}
...
\begin{proposition}[\cite{DRcmzcB}] \label{PropLOwUvCO}
    ...
    \end{equation}
    est la même que celle «usuelle» donnée par les seminormes. En disant «la même» nous entendons le fait que les ouverts sont les mêmes : \( A\) est ouvert pour une des deux topologies si et seulement s'il est ouvert pour l'autre.
\end{proposition}
...
\begin{theorem}[théorème de l'application ouverte\cite{BIBooNJJUooDaGnPZ, BIBooYJXXooTvzpDW,BIBooMKAVooDLCzUX}]
    Soient des espaces vectoriels normés complete \( E\) et \( F\). Si l'application \( f\colon E\to F\) est linéaire, surjective et continue, alors elle est ouverte.
\end{theorem}
