...
\begin{definition}[\cite{MonCerveau, BIBooCPBMooLVUNRi, ooYDVWooPWLUGW}]        \label{DEFooDHQDooFfIvsX}
    Soient un espace topologique \( X\) et un espace vectoriel topologique \( Y\). Une famille \( H\) d'applications \( X\to Y\) est \defe{équicontinue}{équicontinu} en \( a\in X\) si pour tout voisinage de \( 0\) dans \( Y\), il existe un voisinage de \( a\) dans \( X\) tel que 
    \begin{equation}
        h(U)\subset h(a)+V
    \end{equation}
    pour tout \( h\in H\).
