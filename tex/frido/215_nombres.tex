...
\begin{definition}[cycle]
    Un \defe{cycle}{cycle dans le groupe symétrique} dans \( S_E\) est une permutation de la forme \( (a_1,\ldots, a_k)\) où les \( a_i\) sont des éléments distincts de \( E\) et dont l'action sur \( E\) est définie par
    \begin{equation}
        (a_1,\ldots, a_k)x=\begin{cases}
            a_{i+1}    &   \text{si $x=a_i$ avec $i\leq k-1$}\\
            a_1    &    \text{si }x=a_k\\
            x    &    \text{sinon.}
        \end{cases}
    \end{equation}
