...
\begin{lemma}[\cite{YHRSDGc}]       \label{LEMooCGKXooYWjRwk}
    Soit un espace topologique métrique \( (\Omega,d)\). Nous considérons sa tribu des boréliens\footnote{Définition \ref{}.} \( \Borelien(\Omega)\) ainsi qu'une mesure finie \( \mu\) sur \( \big(\Omega,\Borelien(\Omega)\big)\). 
...
\begin{theorem}[Théorème d'approximation, thème \ref{THEMEooKLVRooEqecQk}\cite{YHRSDGc}]
...
\begin{theorem}[Théorème d'approximation, thème \ref{THEMEooKLVRooEqecQk}\cite{YHRSDGc}]     \label{ThoAFXXcVa}
    ...
    Il existe \( f\in C^0(\Omega,\eR)\) nulle hors de \( W\) vérifiant \( 0\leq f\leq 1\) et
    \begin{equation}
        \int_{\Omega}| \mtu_A-f |^pd\mu(x)<\epsilon.
    \end{equation}
\end{theorem}
