...
\begin{proposition}     \label{PropOPSCooVpzaBt}
    Si \( A,B\subset \Omega\) sont des ensembles disjoints et si \( f\colon \eR\to \eR^+\) est intégrable sur \( A\cup B\) alors les intégrales \( \int_Af\) et \( \int_Bf\) existent et
    \begin{equation}
...
\begin{lemma}   \label{LemPfHgal}
    Soit \( f\) une fonction à valeurs dans \( \eR\) telle que \( | f(x)|\leq g(x) \) pour tout \( x\in\Omega\). Si \( g\) est intégrable, alors \( f\) est intégrable.
\end{lemma}
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    Nous posons \( f_+(x)=f(x)\mtu_{A_+}\) et \( f_-(x)=f(x)\mtu_{A_-}\). Par la proposition \ref{PropOPSCooVpzaBtAVOIRPOSITIF}, nous avons \( f=f_+-f_-\) et
    \begin{equation}
...
\begin{lemma}[\cite{MonCerveau}]
    Soit un espace vectoriel \( V\) de dimension finie, muni de la norme \( N\). Soient une base \( \{ e_i \}\) de \( V\), et une fonction \( f\colon (\Omega,\tribA,\mu)\to V\) telle que la norme \( N(f)\colon \Omega\to \eR^+\) soit intégrable. Nous notons \( f_i\) les composantes de \( f\) : \( f(x)=\sum_if_i(x)e_i\).

    Alors pour chaque \( i\), la fonction \( | f_i |\colon \Omega\to \eR^+\) est intégrable.
\end{lemma}
...
\begin{proof}
    ...
    Si \( A_n\) sont des éléments disjoints de \( \tribF\) tels que \( \bigcup_nA_n\in\tribF\). Alors en utilisant la proposition \ref{PropOPSCooVpzaBtAVOIRPOSITIF}, nous avons le calcul suivant :
    \begin{equation}
