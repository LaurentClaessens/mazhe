...
\begin{normaltext}		\label{NORMooJIRRooDuqmru}
...
\begin{lemma}       \label{LEMooKAXFooIPyzJC}
	Soit \( a>0\) dans \( \eR\) et \( b>1\) dans \( \eR\). Alors
	\begin{enumerate}
		\item
		      Si \( \lambda>1\) dans \( \eR\) nous avons
		      \begin{equation}
			      \lambda a>a.
		      \end{equation}
		\item 		\label{ITEMooDIVAooDjQXRL}
		      Si \( 0<\lambda<1\) dans \( \eR\) nous avons
		      \begin{equation}
			      \lambda a<a.
		      \end{equation}
	\end{enumerate}
...
%-------------------------------------------------------
\subsection{\( \eR\), \( \eQ\) et \( \eR\setminus \eQ\)}
%----------------------------------------------------

\begin{normaltext}		\label{NORMooXRGFooPCujZI}
	Pour rappel, \( \eQ\)  est le corps des fractions de \( \eZ\), et \( \eR\) est l'ensemble des classes de suites de Cauchy dans \( \eQ\). Comme expliqué dans \ref{NORMooJIRRooDuqmru}, à partir du moment où \( \eR\) est défini, nous notons «\( \eQ\)» ce qui en réalité \( \varphi(\eQ)\) où \( \varphi\) est l'application qui à \( q\in \eQ\) fait correspondre la classe d'équivalence des suites de Cauchy dans \( \eQ\) convergeant dans \( \eQ\) vers \( q\). Voir la proposition \ref{PropooEPFCooMtDOfP}.
\end{normaltext}

\begin{normaltext}
	Pour se convaincre que \( \eR\setminus \eQ\) n'est pas vide, on peut remarquer que la proposition \ref{PropooRJMSooPrdeJb} dit qu'il n'existe pas d'éléments \( q\in \eQ\) tel que \( q^2=2\). Or la proposition \ref{PROPooUHKFooVKmpte} dit qu'un tel élément existe dans \( \eR\).

	Voila. Vous êtes convaincu ? Maintenant nous allons en faire une preuve un peu plus formelle,qui ne mélange pas \( \eQ\) et \( \varphi(\eQ)\).
\end{normaltext}

\begin{lemma}[\cite{MonCerveau}]	\label{LEMooAYBFooHEUPZG}
	Plusieurs choses plus ou moins reliées.
	\begin{enumerate}
		\item		\label{ITEMooTHFAooVPPHCc}
		      La partie \( \eR\setminus \eQ\) n'est pas vide.
		\item		\label{ITEMooVXUOooOVByJv}
		      Si \( x\in \eR\setminus \eQ\), et si \( q\in \eQ\setminus\{ 0 \}\) alors \( qx\in \eR\setminus \eQ\).
		\item		\label{ITEMooHXTAooGBWeLL}
		      Si \( x\in \eR\setminus \eQ\) et si \( q\in \eQ\), alors \( x+q\in \eR\setminus \eQ\).
		\item		\label{ITEMooFOQRooRoftNW}
		      Soit \( \epsilon>0\) dans \( \eR\). Il existe \( x\in \eR\setminus\eQ\) tel que \( 0<x<\epsilon\).
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	En plusieurs points.
	\begin{subproof}
		\spitem[Pour \ref{ITEMooTHFAooVPPHCc}]
		%-----------------------------------------------------------
		Les propositions \ref{PROPooLBRCooBtJqwt} et \ref{PROPooSTQXooHlIGVf} (au choix) donnent des suites de Cauchy \( (x_n)_{n\in \eN}\) dans \( \eQ\) qui ne convergent pas dans \( \eQ\). Il n'existe donc pas de \( q\in \eQ\) tel que \( (x_n)_{n\in \eN}\in \bar q=\varphi(q)\). Par contre la classe de \( (x_n)_{n\in \eN}\) est bien dans \( \eR\).

		\spitem[Pour \ref{ITEMooHXTAooGBWeLL}]
		%-----------------------------------------------------------
		Si \( x+q=p\in \eQ\), alors \( x=p-q\in \eQ\).

		\spitem[Pour \ref{ITEMooVXUOooOVByJv}]
		%-----------------------------------------------------------
		Supposons que \( qx = p\in \eQ\). Vu que \( q\neq 0\), nous avons \( x=pq^{-1}\in \eQ\) parce que \( \eQ\) est un corps (c'est même sa définition \ref{DEFooBPJNooRqrblS}).

		\spitem[Pour \ref{ITEMooFOQRooRoftNW}]
		%-----------------------------------------------------------
		Soit \( \epsilon>0\). Soit \( r\in \eR\setminus \eQ\) (existence par \ref{ITEMooTHFAooVPPHCc}). Nous considérons :
		\begin{enumerate}
			\item
			      \( \epsilon'\in \eQ\) avec \( \epsilon\in \eQ\) (par exemple \( 1/n\) donné par le lemme \ref{LemooHLHTooTyCZYL}\ref{ITEMooCVDSooAjimCL}).
			\item
			      \( q\in \eQ\) avec \( q>r\). Par exemple \( r<q<r+1\) sonné par le lemme \ref{LemooHLHTooTyCZYL}\ref{ITEMooGVTMooQsoTCi}.
			\item
			      \( p=\epsilon' q^{-1}\in \eQ\).
		\end{enumerate}
		Avec tout ça, la proposition \ref{PROPooWYPNooCcBERG}\ref{ITEMooDCOXooAKcSKk} nous donne \( pr<pq\). Au final :
		\begin{equation}
			pr<pq=\epsilon'<\epsilon.
		\end{equation}
		Vu que \( p\in \eQ\) et \( r\in \eR\setminus \eQ\), la proposition \ref{PROPooWYPNooCcBERG}\ref{ITEMooVXUOooOVByJv} assure que \( pr\in \eR\setminus\eQ\).
	\end{subproof}
\end{proof}
...
Nous avons déjà défini la notion d'intervalle pour un espace totalement ordonné en \ref{DefEYAooMYYTz}.
...
\begin{proposition}[\cite{MonCerveau}]     \label{PROPooHPMWooQJXCAS}
	Tous les intervalles\footnote{Définition \ref{DefEYAooMYYTz}.} de \( \eR\) sont d'une des formes suivantes :
	\begin{enumerate}
		\item
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooQULXooVzUtAi}
	Soient un ensemble \( E\).
	\begin{enumerate}
		\item		\label{ITEMooBOMOooTDUpgb}
		      Soit une partie \( A\subset E\). Soient deux applications \(f,g \colon E\to \eR^+  \). Nous avons
		      \begin{equation}
			      \sup_{x\in A}\big( f(x)+g(x) \big)\leq\sup_{x\in A}f(x)+\sup_{x\in A}g(x).
		      \end{equation}
		\item		\label{ITEMooMWCRooSqwXgN}
		      Si \( A\subset \eR^+\) et si \( \lambda\geq 0\) alors
		      \begin{equation}
			      \sup(\lambda A)=\lambda\sup(A).
		      \end{equation}
	\end{enumerate}
	%TODOooTHDLooGxhHDz. Prouver ça.
\end{proposition}
...
\begin{proof}
	Par le définition \ref{LEMooLEXTooGAQxGB}, nous avons
	\begin{equation}
		\integer(\lambda)\leq \lambda\leq \integer(\lambda)+1<\integer(\lambda)+2.
	\end{equation}
\end{proof}
...
%-------------------------------------------------------
\subsection{Conjugaison complexe}
%----------------------------------------------------
...
\begin{definition}      \label{DEFooQDDVooRYDsAJ}
	...
	Le conjugué complexe de \( z\in \eC\) sera le plus souvent noté \( \bar z\).
\end{definition}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooKBOIooRJPXAH}
	Si \( x,y\in \eC\), alors nous avons \( \bar x\bar y=\overline{xy} \).
\end{proposition}

\begin{proof}
	Il suffit de poser \( x=a+bi\) et \( y=c+di\). Un peu de calcul donne
	\begin{equation}
		\bar x\bar y=(a-bi)(c-di)=(ac-bd)-(ad+bc)i.
	\end{equation}
	De même \( xy=(ac-bd)+(ad+bc)i\), et donc bien
	\begin{equation}
		\overline{xy}=(ac-bd)-(ad+bc)i.
	\end{equation}
\end{proof}
...
%-------------------------------------------------------
\subsection{Norme, topologie, etc.}
%----------------------------------------------------


Rien ici. La norme sur \( \eC\) demande la racine carrée qui sera vue plus tard.

Quelques liens :
\begin{enumerate}
	\item
	      La norme sur \( \eC^n\) est dans la proposition \ref{DEFooGUXNooXwCsrq}.
	\item
	      La proposition \ref{PROPooVSVMooZrqxdc} pour \( | x+y |\leq |x|+| y |\).
	\item
