...
Vous voulez une fonction de classe \( C^{\infty}\) qui n'est pas nulle partout ? En voici une.
...
\end{equation}
Cette fonction est encore de classe $ C^{\infty}$. En effet, \( \int_0^m\) est un simple nombre strictement positif sans histoires tandis que la proposition \ref{PropEZFRsMj} dit que \( x\mapsto \int_0^x\varphi(t)dt\) est une primitive de \( \varphi\). Vu que \( \varphi\) est déjà de classe \(  C^{\infty}\), sa primitive l'est également.
...
À partir de là nous considérons les fonctions
\begin{subequations}
    \begin{align}
...
\begin{lemma}[Lemme d'Urysohn]      \label{LEMooECTNooKagaRU}
    Soit un ouvert \( \mU\) de \( \eR\) et un compact \( K\) inclus dans \( \mU\). Il existe une fonction \( f\colon \eR\to \mathopen[ 0 , 1 \mathclose]\) de classe  \(  C^{\infty}\) telle que
    \begin{enumerate}
        \item
            \( f(x)=1\) pour \( x\in K\),
        \item
...
Notons que la fonction du lemme \ref{LEMooECTNooKagaRU} n'épouse pas spécialement très bien la forme de \( K\). Si par exemple \( K=\mathopen[ 0 , 1 \mathclose]\cup \mathopen[ 10 , 11 \mathclose]\), la fonction \( f\) sera égale à \( 1\) au moins sur \( \mathopen[ 0 , 11 \mathclose]\).
