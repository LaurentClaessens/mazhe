...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[\( f=g\)]
		%-----------------------------------------------------------
		Il nous reste à prouver que l'application \( g\) est en réalité la limite \( f\) dont on parle dans l'énoncé. Nous devons calculer \( \lim_{N\to\infty}\| f(x)-\sum_{k=0}^Na_k(x) \|\). Nous avons le calcul suivante :
		\begin{subequations}
			\begin{align}
		...
		\end{equation}
		Et voila. Il suffit maintenant de faire \( N\to \infty\) des deux côtés pour avoir
		\begin{equation}
