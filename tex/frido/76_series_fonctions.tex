...
\begin{lemma}[Lemme de Urysohn\cite{BIBooOUGZooCoBnry}]	\label{LEMooKMZPooUOwlpt}
	Si \( X\) est un espace topologique normal et si \( A\) et \( B\) sont des fermés disjoints dans \( X\), alors il existe une fonction \(f \colon X\to \mathopen[ 0,1\mathclose]  \) telle que
	\begin{enumerate}
		\item
		      \( f\) est continue,
		\item
...
\begin{definition}[Fonction analytique entre espaces de Banach\cite{BIBooTIAZooFwCEtZ}]	\label{DEFooAIHMooKbWsBt}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooWAUZooRHfGCl}
	Soit \( \lambda>0\). Si
	\begin{equation}
		\sum_{k=0}^{\infty}a_k\| x \|^k<\infty
	\end{equation}
	pour tout \( x\in B(0,R)\) alors
	\begin{equation}
		\sum_{k=0}^{\infty}a_k\lambda^k\| x \|^k<\infty
	\end{equation}
	pour tout \( x\in B(0,R/\lambda)\).
\end{lemma}

\begin{proof}
	Simple calcul. Si \( x\in B(0,R/\lambda)\) alors \( \lambda x\in B(0,R)\) et donc
	\begin{equation}
		\sum_{k=0}^{\infty}a_k\lambda^k\| x \|^k=\sum_{k=0}^{\infty}a_k\| \lambda x \|^k<\infty.
	\end{equation}
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooKQXDooSGexpm}
	Soient deux espaces vectoriels de dimension finie \( E\) et \( F\). Sur \( E\) nous posons les normes \( \| . \|_1\) et \( \| . \|_2\) et sur \( F\) nous considérons les normes \( N_1\) et \( N_2\). Si
	\begin{equation}
		f \colon (E,\| . \|_1)\to (F,N_1)
	\end{equation}
	est analytique sur \( B(x_0,R)\) alors il existe un \( r>0\) tel que
	\begin{equation}
		f \colon (E,\| . \|_2)\to (F,N_2)
	\end{equation}
	soit analytique sur \( B(x_0,r)\).
\end{proposition}

\begin{proof}
	Il suffit de suivre la définition \ref{DEFooAIHMooKbWsBt}. Nous avons
	\begin{equation}
		f(x)=\sum_{k=0}^{\infty}\alpha_k(x-x_0)^k
	\end{equation}
	avec
	\begin{equation}
		\sum_{k=0}^{\infty}\| \alpha_k \|_1\| x-x_0 \|_1^k<\infty
	\end{equation}
	pour tout \( x\in B(x_0,R)\).

	Nous nous souvenons du théorème \ref{ThoNormesEquiv} qui dit que toutes les normes sont équivalente : il existe des \( \lambda_i\) et \( \mu_i\) tels que
	\begin{subequations}
		\begin{numcases}{}
			\lambda_1\| x \|_1\leq \| x \|_2\leq \lambda_2\| x_1 \|\\
			\mu_1N_1(y)\leq N_2(y)\leq \mu_2N_1(y)
		\end{numcases}
	\end{subequations}
	pour tout \( x\in E\) et pour tout \( y\in F\). Nous avons alors
	\begin{subequations}
		\begin{align}
			\| \alpha_k \|_2 & =\sup_{\| x_i \|_2\leq 1}N_2\big( \alpha_k(x_1,\ldots,x_k) \big)                                                \\
			                 & \leq \sup_{\| x_i \|\leq 1/\lambda_1}\mu_2N_1\big( \alpha_k(x_1,\ldots,x_k) \big)                               \\
			                 & =\sup_{\| x_i \|\leq 1}\mu_2N_1\Big( \alpha_l(\frac{ x_1 }{ \lambda_1 },\ldots,\frac{ x_k }{ \lambda_1 }) \Big) \\
			                 & =\frac{ \mu_2 }{ \lambda_1^k }\sup_{\| x_i \|_1=1}N_1\big( \alpha_k(x_1,\ldots,x_k) \big)                       \\
			                 & =\frac{ \mu_2 }{ \lambda_1^k }\| \alpha_k \|_1.
		\end{align}
	\end{subequations}
	Nous avons alors
	\begin{subequations}
		\begin{align}
			\sum_{k=0}^{\infty}\| \alpha_k \|_2\| x-x_0 \|_2^k & \leq\sum_{k=0}^{^\infty}\frac{ \mu_2 }{ \lambda_1^k }\| \alpha_k \|_1\lambda_2^k\| x-x_0 \|_1^k           \\
			                                                   & =\mu_2\sum_{k=0}^{\infty}\left( \frac{ \lambda_2 }{ \lambda_1 } \right)^k\| \alpha_k \|_1\| x-x_0 \|_1^k.
		\end{align}
	\end{subequations}
	Le lemme \ref{LEMooWAUZooRHfGCl} dit que la dernière somme est \( <\infty\) dès que \( x\in B(x_0,r)\) avec \( r=\lambda_1 /\lambda_2\).
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooADXXooXAPCBL}
	Soient des espaces de Banach \( E_i\) et \( F\). Si l'application \(f \colon E_1\times \ldots \times E_n\to F  \) est analytique et si \( a_i\in E_i\), alors l'application partielle
	\begin{equation}
		\begin{aligned}
			g\colon E_n & \to F                           \\
...
\begin{proof}
	Soient une application \(f \colon A\times B\to V  \) ainsi que \( (a,b)\in A\times B\). Nous supposons que \( f\) est analytique en \( (a,b)\) et nous posons
	Nous posons
	\begin{equation}
		\begin{aligned}
			g\colon B & \to V          \\
		...
		\end{aligned}
	\end{equation}
	et nous montrons que \( g\) est analytique en \( b\).

	Il existe un \( r>0\) tel que pour tout \( (x,y)\in B\big( (a,b),r \big)\) nous ayons
	\begin{equation}		\label{EQooFUIEooBSpNus}
		f(x,y)=\sum_{k=0}^{\infty}\alpha_k\big( (x,y)-(a,b) \big)
	\end{equation}
	où
	\begin{equation}
		\alpha_k \colon (A\times B)^k\to V
	\end{equation}
	est \( k\)-multilinéaire. Et d'ailleurs nous devrions écrire \( \alpha_k\big( (x,y) \big)\) au lieu de \( \alpha_k(x,y)\). Par exemple quand nous écrivons \( \alpha_2(x,y)\), ce que nous voulons réellement écrire est
	\begin{equation}
		\alpha_2\big( (x,y),(x,y) \big).
	\end{equation}
	Bref, nous écrivons \eqref{EQooFUIEooBSpNus} sous la forme
	\begin{equation}
		f(x,y)=\sum_{k=0}^{\infty}\alpha_k(x-a,y-b).
	\end{equation}
	En ce qui concerne la fonction \( g\), si \( y\) est dans un voisinage de \( b\) nous avons
	\begin{equation}
		g(y)=f(a,y)=\sum_{k=0}^{\infty}\alpha_k(0,y-b).
	\end{equation}
	Nous posons
	\begin{equation}
		\begin{aligned}
			\beta_k\colon B & \to V                              \\
			y               & \mapsto \alpha_k\big( (0,y) \big),
		\end{aligned}
	\end{equation}
	de telle sorte à écrire
	\begin{equation}
		g(y)=\sum_{k=0}^{\infty}\beta_k(y-b).
	\end{equation}
	Nous devons montrer que \( \beta_k\) est \( k\)-multilinéaire et qu'elle vérifie les conditions de la définition \ref{DEFooAIHMooKbWsBt}. En ce qui concerne la linéarité,
	\begin{equation}
		\beta_k(\lambda y)=\alpha_k\big( (0,\lambda y) \big)=\lambda_k\big( \lambda(0,y) \big)=\lambda^k\alpha_k\big( (0,y) \big)=\lambda^k\beta_k(y).
	\end{equation}

	En ce qui concerne la série \eqref{EQooQYBGooKyoNHA}, commençons par étudier \( \| \beta_k \|\) :
	\begin{subequations}
		\begin{align}
			\| \beta_k \| & =\sum_{\| y \|=1}\| \beta_k(y) \|                 \\
			              & =\sup_{\| y \|=1}\| \alpha_k\big( (0,y) \big) \|.
		\end{align}
	\end{subequations}
	Notez que
	\begin{equation}
		\{ (0,y)\tq \| y \|=1 \}\subset\{ (x,y)\tq  \| (x,y) \|=1 \},
	\end{equation}
	donc
	\begin{equation}
		\sup_{\| y \|=1}\| \alpha_k\big( (0,y) \big)  \|\leq \sup_{\| (x,y) \|=1}\| \alpha_k(x,y) \|=\| \alpha_k \|.
	\end{equation}
	Tout cela pour dire que \( \| \beta_k \|\leq \| \alpha_k \|\). Nous pouvons donc faire la majoration
	\begin{subequations}
		\begin{align}
			\sum_{k=0}^{\infty}\| \beta_k \|\| y-b \|^k & \leq \sum_{k=0}^{\infty}\| \alpha_k \|\| y-b \|^k \\
			                                            & \leq\sum_{k=0}^{\infty}\| (x-a,y-b) \|^k          \\
			                                            & <\infty.
		\end{align}
	\end{subequations}
\end{proof}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooZRNAooUrDJOI}
	Toute application linéaire entre espaces vectoriels de dimensions finies est analytique.
\end{lemma}
...
\begin{proposition}[Composition d'applications analytiques\cite{MonCerveau}]	\label{PROPooQGGHooXPBhLT}
	Si \( E,F,G\) sont des espaces de Banach et si \(f \colon F\to G  \) et \(g \colon E\to F  \) sont analytiques, alors \(f\circ g \colon E \to G  \) est analytique.
	%TODOooOKJZooERcskS. Prouver ça.
	% Je suis d'ailleurs un peu étonné qu'il n'y avait pas encore la composition d'applications analytiques n'est pas encore prouvée.
\end{proposition}
...
\begin{proof}
	Nous avons \( f(x)=x+a=a_0+a_1(x)\) en posant \( a_0=a\) et \( a_1(x)=x\). L'application \( a_1\) est \( 1\)-multilinéaire.
\end{proof}
...
\begin{proof}
	C'est la composition de l'application analytique \( f\) avec l'application analytique \( x\mapsto x+h\). Il suffit alors d'appliquer les propositions \ref{PROPooFEMWooDwAYyO} et \ref{PROPooQGGHooXPBhLT}.
\end{proof}




\begin{lemma}[\cite{MonCerveau}]	\label{LEMooLYIIooNDLRrH}
	Soit \( m\geq n\). Nous considérons la projection \(\proj_n \colon \eR^m\to \eR^n  \). Soit une application \(h \colon \eR^n\to \eR^k  \) telle que \( h\circ\proj_n\) est analytique. Alors \( h\) est analytique.
\end{lemma}

\begin{proof}
	Nous introduisons l'inclusion
	\begin{equation}
		\begin{aligned}
			j\colon \eR^n & \to \eR^m      \\
			x             & \mapsto (x,0).
		\end{aligned}
	\end{equation}
	C'est une application analytique parce que linéaire (lemme \ref{LEMooZRNAooUrDJOI}). Alors nous avons \( h=(h\circ\proj_n)\circ j\). En tant que composée d'applications analytiques, \( h\) est analytique\footnote{Proposition \ref{PROPooQGGHooXPBhLT}.}.
\end{proof}
