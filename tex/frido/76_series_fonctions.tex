...
\begin{proposition}[Fonctions plateau\cite{BIBooPITOooZANjFn}]       \label{PROPooBOZIooAhKbPs}
    Soit un ouvert \( \Omega\subset \eR^d\) ainsi qu'un compact \( K\) dans \( \Omega\). Il existe une fonction \( \psi\in C^{\infty}(\eR^n)\) telle que
    \begin{enumerate}
        \item
            \( \supp(\psi)\) est compact dans \( \Omega\),
        \item
            \( \psi\geq 0\)
        \item
            \( \psi=1\) sur \( V\), un voisinage ouvert de \( K\).
    \end{enumerate}
\end{proposition}
...
\begin{theorem}[Partition de l'unité\cite{BIBooPITOooZANjFn}]
...
\begin{proof}
    ...
    \begin{equation}
        \sum_{j=1}^n\phi_j(x)=\frac{ \sum_{j}\psi_j(x) }{ 1-\tetha(x)+\sum_k\psi_k(x) }=1.
    \end{equation}
