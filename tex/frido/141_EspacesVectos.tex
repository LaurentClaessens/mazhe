...
% Copyright (c) 2011-2022, 2025-2026
...
\begin{propositionDef}[Hyperplan\cite{MonCerveau,BIBooXYPGooIYVgoZ,BIBooXKSMooJHzoRo}]      \label{DEFooEWDTooQbUQws}
	...
	Avec ça nous pouvons poser deux définitions.
	\begin{enumerate}
		\item
		      Quand les conditions \ref{ITEMooUCWQooToKrop} et \ref{ITEMooXVIPooNxijzA} sont vérifiées, nous disons que \( H\) est un \defe{hyperplan vectoriel}{hyperplan vectoriel} de \( E\).
		\item
		      Un \defe{hyperplan}{hyperplan} (affin) de \( E\) est une partie de la forme \( H+a\) où \( H\) est un hyperplan vectoriel et \( a\in E\).
	\end{enumerate}
\end{propositionDef}
...
%-------------------------------------------------------
\subsection{Hyperplan dans un espace euclidien}
%----------------------------------------------------

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooHLRRooHWtHSR}
	Soit un espace euclidien \( (E,\cdot)\). Soit un hyperplan vectoriel\footnote{Définition \ref{DEFooEWDTooQbUQws}.} \( H\).
	\begin{enumerate}
		\item
		      Il existe \( n\in E\) tel que
		      \begin{equation}
			      H=\{ x\in E\tq x\cdot n=0 \}.
		      \end{equation}
		\item
		      Si \( n_1,n_2\in E\) sont tels que
		      \begin{equation}
			      H=\{ x\in E\tq x\cdot n_1=0 \}= \{ x\in E\tq n_2\cdot x=0 \},
		      \end{equation}
		      alors il existe \( \lambda\in \eK\) tel que \( n_2=\lambda n_1\).
	\end{enumerate}
	%TODOooWCWXooSyhhGK. Prouver ça.
\end{proposition}



%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Trouver la matrice d'une symétrie donnée}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
\label{SecMtrSym}
