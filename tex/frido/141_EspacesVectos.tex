...
Si vous êtes pressés, vous pouvez aller lire la définition \ref{DEFooKTVDooSPzAhH} de produit tensoriel d'espaces vectoriels. Mais si vous étiez vraiment pressés, vous ne seriez pas en train de lire des choses sur le produit tensoriel (il vous suffit de savoir que \( x\otimes y\) n'est finalement que la concatenation de \( x\) et \( y\)).
...
\begin{definition}[\cite{ooWHNKooYVCiYc}]
...
\begin{definition}[\cite{ooWHNKooYVCiYc}]       \label{DEFooKTVDooSPzAhH}
    ...
    \begin{subequations}
        \begin{align}
            ...
            A_3&=\{ \lambda\delta_{v,w}-\delta_{(v,\lambda w)}\tq v\in V,w\in W,\lambda\in \eK \}.
        \end{align}
    \end{subequations}
...
\begin{proposition}     \label{PROPooIWZDooRRZNCf}
    L'espace vectoriel \( V\otimes W\) est un produit tensoriel entre \( V\) et \( W\).
\end{proposition}
<++>
