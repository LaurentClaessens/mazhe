...
\begin{proposition}[\cite{ooNHIGooYlXxMf}]
...
\begin{proof}
    Le fait que \( \varphi\) soit bien définie est un effet du lemme \ref{LEMooOJEBooQruWEp} qui donne une fonction sur \( F(V^*\times W)\) dont le \( \varphi\) qui nous concerne est un quotient. D'autre part, le lemme \ref{LEMooUQZHooWjIGsy} donne une structure de produit tensoriel de \( V^*\) par \( W\) sur \( \aL(V,W)\).
\end{proof}
<++>
