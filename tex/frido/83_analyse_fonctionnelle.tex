...
Nous allons prouver quelque lemmes qui permettent de prolonger des fonctionnelles d'un sous-espace vers un sous-espace contenant un nombre fini de dimensions en plus.
...
\begin{lemma}[\cite{MonCerveau}]       \label{LEMooWEMFooEHIaxY}
    Soient des espaces vectoriels normés \( V\) et \( W\). Si \( \alpha\colon V\to W\) est une bijection linéaire et isométrique, alors l'application
    \begin{equation}
        \begin{aligned}
            \beta\colon V'&\to W' \\
...
\begin{lemma}[\cite{PROPooBBNBooGcXDRH}]        \label{LEMooMSYAooGEMgoc}
