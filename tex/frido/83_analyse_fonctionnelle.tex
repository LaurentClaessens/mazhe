...
\begin{lemma}[\cite{BIBooMXTXooOsgBRx}]
    Soient un espace vectoriel réel normé \( X\) ainsi qu'un sous-espace \( M\). Soient \( \ell\in M'\)\footnote{\( M'\) sont les applications linéaires bornées de \( M\) vers \( \eR\).} et \( x_1\in X\setminus M\). On pose \( M_1=\Span\{ M, x_1 \}\). Alors nous avons \( \ell_1\in M_1'\) tel que
    \begin{enumerate}
        \item
...
Ce lemme permet de prolonger une forme linéaire une dimension à la fois. Rien ne nous permet de prolonger d'une infinité de dimensions.
