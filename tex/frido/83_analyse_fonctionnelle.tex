...
\begin{probleme}
    Je n'ai pas vérifié si le lemme \ref{LEMooWEMFooEHIaxY} est correct, mais à mon avis ça doit être bon parce que la structure du dual est déterminée par la structure d'espace vectoriel normé.

    Soyez toutefois \randomGender{prudent}{prudente}, et écrivez-moi si vous connaissez une preuve.
\end{probleme}

\begin{lemma}[\cite{MonCerveau}]       \label{LEMooWEMFooEHIaxY}
    Soient des espaces de Banach \( V\) et \( W\). Si \( \alpha\colon V\to W\) est une bijection linéaire et isométrique, alors l'application
    \begin{equation}
        \begin{aligned}
            \beta\colon V'&\to W' \\
        ...
        \end{aligned}
    \end{equation}
    est une bijection linéaire isométrique.
\end{lemma}
...
\begin{normaltext}
    ...
    \begin{equation}        \label{EQooJVDEooNGYEtg}
        (L^p)'=L^q
    \end{equation}
    au sens d'une bijection isométrique. Vu que \( 1<p<\infty\), nous avons aussi \( 1<q<\infty\) et donc \( (L^q)'=L^p\). En prenant le dual des deux côtés de \eqref{EQooJVDEooNGYEtg}, 
    \begin{equation}
        (L^p)''=(L^q)'=L^p,
    \end{equation}
    et nous avons prouvé que \( L^p\) est réflexif.
