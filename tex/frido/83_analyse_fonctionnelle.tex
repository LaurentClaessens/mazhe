...
\begin{theorem}[\cite{ooQCDSooCpqDvB}]      \label{THOooPCHDooITWKpC}
	Soient un ouvert \( \Omega\) dans \( \eR^n\) et un recouvrement ouvert \( \{ U_i \}_{i\in I}\) de \( \Omega\). Il existe :
	\begin{enumerate}
		\item
		      Une \( C^\infty\)-partition de l'unité \( \{ \psi_j \}_{j\in J}\) telle que \( \supp(\psi_j)\) est compact dans un des \( U_i\).
		\item       \label{ITEMooFGMJooQPLqGY}
		      Une \( C^{\infty}\)-partition de l'unité \( \{ \alpha_i \}_{i\in I}\) subordonnée aux \( U_i\) telle que pour tout compact \( K\), seul un nombre fini des \( \alpha_i\) est non nul.
	\end{enumerate}
	% TODOooNEHMooDLvNjr. Prouver ça.
	% C'est probablement mieux de démontrer THOooUGQCooFVySMP à la place de celui-ci.
	Ce théorème ne donne pas une partition de l'unité à support compact subordonnée au recouvrement. Soit la partition est à support compact, soit elle est subordonnée. En général pas les deux en même temps.
\end{theorem}
