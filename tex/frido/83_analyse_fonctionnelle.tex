...
\begin{definition}[Espace uniformément convexe\cite{BIBooPYEZooTxohAd}]
...
\begin{definition}[Projection normale\cite{BIBooPYEZooTxohAd}]
...
\begin{theorem}[\cite{BIBooVHQSooTrLCzQ,BIBooPYEZooTxohAd}]
...
\begin{lemma}
    ...
    \begin{equation}
        | a+b |^q+| a-b |^q\leq 2\big( | a |^p+| b |^ \big)^{q-1}.
    \end{equation}
\end{lemma}
...
\begin{proposition}[Inégalité de Clarkson\cite{BIBooVHQSooTrLCzQ}]      \label{PROPooJDOQooWsGlkr}
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \begin{equation}
                \| f+g \|_p^q+\| f-g \|_p^q\leq 2\Big( \| f_p \|^p +\| g \|_p^p \Big)^{q-1}.
            \end{equation}
    \end{enumerate}
\end{proposition}
...
\begin{proposition}     \label{PROPooFNLJooDlyIKV}
    Si \( 1<p<\infty\), l'espace \( L^p(\Omega,\tribA, \mu)\) est uniformément convexe.
\end{proposition}
...
\begin{proposition}     \label{PROPooTZMRooCvQtGg}
    Si \( 1<p<\infty\), et si \( V\) est un sous-espace vectoriel fermé de \( L^p(\Omega,\tribA, \mu)\), alors la projection normale de \( a\in L^p\) sur \( V\) existe et est unique.
\end{proposition}
