...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    et nous commençons par prouver que \( \eta\) est convexe. Cela est une fonction de class \(  C^{\infty}\) à part en \( a=1\) (à cause de \( | x-1 |\) qui n'est pas dérivable en \( x=1\)). Nous calculons les dérivées en séparant les cas \( a>1\) et \( 0\leq a <1\).

    \begin{subproof}
        \item[Calculs de dérivées secondes]
            Commençons par \( \eta_1(a)=(a^{1/p}+1)^p\). Les dérivées demandent un peu de calculs pour lesquels nous ne donnons pas tous les détails. Promis, il n'y a pas de choses vraiment subtiles\footnote{Écrivez moi si il y a un problème.}. On trouve
            \begin{equation}
                \eta_1'(a)=a^{1/p-1}(a^{1/p}+1)^{p-1}
            \end{equation}
            et
            \begin{equation}
                \eta_1''(a)=-\frac{ p-1 }{ p }a^{1/p-2}(a^{1/p}+1)^{p-2}.
            \end{equation}
            En ce qui concerne \( \eta_2(a)=| a^{1/p}-1 |^{p}\), nous avons 
            \begin{equation}
                \eta_2(a)=| a^{1/p}-1 |^p=(a^{1/p}-1)^p
            \end{equation}
            parce que nous sommes dans le cas \( a>1\). La dérivée est donnée par
            \begin{equation}
                \eta'_2(a)=a^{1/p-1}(a^{1/p}-1)^{p-1},
            \end{equation}
            et
            \begin{equation}
                \eta''_2(a)=\frac{ p-1 }{ p }a^{1/p-2}(a^{1/p}-1)^{p-2}.
            \end{equation}
            Si au contraire \( 0<a<1\) alors \( \eta_2(a)=(1-a^{1/p})^p\) et nous avons
            \begin{equation}
                \eta_2'(a)=-a^{1/p-1}(1-a^{1/p})^{p-1}
            \end{equation}
            ainsi que
            \begin{equation}
                \eta''_2(a)=a^{1/p-2}\frac{ p-1 }{ p }(1-a^{1/p})^{p-2}.
            \end{equation}
            En comparant tous les bouts nous pouvons écrire, pour \( a\neq 0\) :
            \begin{subequations}
                \begin{align}
                    \eta_1''(a)&=-\frac{ p-1 }{ p }a^{1/p-2}(a^{1/p}+1)^{p-2}\\
                    \eta_2''(a)&=\frac{ p-1 }{ p }a^{1/p-2}| 1-a^{1/p} |^{p-2}.
                \end{align}
            \end{subequations}
            Nous avons donc cette belle dérivée seconde
            \begin{equation}
                \eta''(a)=\frac{ p-1 }{ p }a^{1/p-2}\big( | 1-a^{1/p} |^{p-2}-(a^{1/p}+1)^{p-2} \big)
            \end{equation}
        valable pour \( a\in \mathopen] 0 , \infty \mathclose]\setminus\{ 1 \}\).
    \item[Convexe]
    Vu que \( \eta''>0\) sur \( \mathopen] 0 , 1 \mathclose[\cup\mathopen] 1 , \infty \mathclose[\), la fonction \( \eta\) est convexe par le théorème \ref{ThoGXjKeYb}.
    \end{subproof}
    Le lemme \ref{LEMooNUDOooVfVPkw} conclu que \( \xi\) est convexe.
\end{proof}
<++>
