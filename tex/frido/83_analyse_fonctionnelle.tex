...
\begin{propositionDef}[Intégrale d'une forme sur une variété]       \label{DEFooOMQLooGiJWZS}
	Soient une variété \( M\), deux atlas \( \{ (\varphi_i,U_i) \}_{i\in I}\) et \( \{ (\psi_j,V_j) \}_{j\in J}\) ainsi que des partitions de l'unité \( \{ f_i \}_{i\in I}\) subordonnée\footnote{Définition \ref{THOooQFCQooSlgLpz}.} à \( \{ U_i \}\) et \( \{ g_j \}\) subordonnées à \( \{ V_j \}_{j\in J}\).

	Alors
	\begin{equation}
		\sum_{i\in I}\int_{\varphi_i(U_i)}f_i\omega=\sum_{j\in J}\int_{\psi_j(V_j)}g_j\omega.
	\end{equation}
	Ce nombre est ce que nous nommons l'\defe{intégrale}{intégrale sur une variété} de \( \omega\) sur \( M\) :
	\begin{equation}
		\int_M\omega
	\end{equation}
\end{propositionDef}
...
%-------------------------------------------------------
\subsection{Brouwer avec Stokes}
%----------------------------------------------------
...
% Attention : il vaut mieux ne pas citer ce théorème parce qu'il se base sur la version C^infini qui est démontrée par Stokes.
% Il faut citer THOooZMUAooSPlqUx.
