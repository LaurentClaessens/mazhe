...
\begin{definition}[Partition de l'unité]	\label{DEFooEDFIooHKnGdE}
	Soit un espace topologique \( X\) muni d'un recouvrement ouvert \( \{ U_i \}_{i\in I}\). Une \defe{partition de l'unité}{partition de l'unité} subordonnée à \( \{ U_i \}_{i\in I}\) est un ensemble de fonctions \( \{ \phi_i \}_{i\in I}\) telles que
	\begin{enumerate}
		\item
		      \( \phi_i(X)\subset \mathopen[ 0,1\mathclose]\) pour tout \( i\in I\).
		\item
		      \( \supp(\phi_i)\subset U_i\).
		\item
		      \( \sum_{i\in I}\phi_i(x)=1\) pour tout \( x\in X\).
	\end{enumerate}
	Si \( \mA\subset\Fun(X)\), nous disons que \( \{ \phi_i \}_{i\in I}\) est une \( \mA\)-partition de l'unité si \( \phi_i\in\mA\) pour chaque \( i\).
\end{definition}


\begin{theorem}[Partition de l'unité sur un compact\cite{BIBooPITOooZANjFn}]       \label{THOooQFCQooSlgLpz}
