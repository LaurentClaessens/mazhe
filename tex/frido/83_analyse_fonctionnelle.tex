...
\end{equation}
Le produit de convolution est un peut subtil parce que \( f(t-x)\) n'est pas défini a priori pour tout \( t,x\in \mathopen[ 0 , 2\pi \mathclose[\), vu que \( f\) n'est définie que sur \( \mathopen[ 0 , 2\pi \mathclose[\). Une solution serait de considérer implicitement la fonction prolongée par périodicité.
\begin{equation}
    (f*g)<++>
\end{equation}
<++>
