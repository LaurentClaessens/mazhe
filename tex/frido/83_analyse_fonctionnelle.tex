...
Nous considérons \( \Omega=\eR^d\) ou \( (S^1)^d\). Une \defe{approximation de l'unité}{approximation!de l'unité} sur \( \Omega\) autour de \( a\in \Omega\) est une suite \( (\varphi_n)\) dans \( L^1(\Omega)\) telle que
    \begin{enumerate}
        \item
...
\begin{proof}
    ...
    D'autre part, si \( y\in\mathopen] -r , r \mathclose[\), alors
...
Le lemme suivant montre que que les boules autour de \( 1\) sont délimitées par la droite en pointillé de la figure \ref{CaptionFigJOQVoolPTsYPZK}.
\begin{lemma}
    Soit \( r>0\). Nous avons
    \begin{equation}
...
\begin{proof}
    ...
    \item[\( \tau_d\subset \tau_i\)]
        ...
        Soient \( A\in\tau_d\) et \( a\in A\). Nous devons trouver \( S\in \tau_i\) tel que \( a\in S\) et \( S\subset A\). Cela prouverait que tout élément de \( A\) possède un voisinage (pour la topologie induite) contenu dans \( A\), et donc que \( A\) est ouvert pour la topologie induite, c'est à dire \( A\in \tau_i\).

        Soit \( r>0\) tel que \( B_d(a,r)\subset A\). Gardons en tête que ce \( r\) peut être redéfini à tout moment pour le prendre plus petit. Notre objectif est de montrer l'existence de \( \delta>0\) tel que \( B_{\eC}(a,\delta)\cap S^1\subset B_d(a,r)\).

        Pour fixer les idées, nous considérons \( x,y\in \eR\) tels que 
\end{subproof}
