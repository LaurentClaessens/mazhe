...
\begin{definition}      \label{DEFooMGXSooWioKie}
	...
	Étant donné que \( \aL_k(\eR^n,\eR)\) est un espace vectoriel normé (définition \ref{DEFooTAUWooNDJJEO}), nous savons ce qu'est une forme différentielle continue ou de classe \( C^k\).
\end{definition}
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooXZGQooYxDRRU}
	Soit un espace vectoriel normé \( V\). Si \(f \colon \eR\to V  \) est différentiable en \( a\), alors \( df_a\) est l'opérateur de multiplication par \( f'(a)\).
\end{lemma}

\begin{proof}
	La définition \ref{DEFooOYFZooFWmcAB} dit que \( f'(a)=(\partial_1f)(a)\). Le lemme \ref{LemdfaSurLesPartielles} dit alors que
	\begin{equation}
		f'(a)=(\partial_1f)(a)=df_a(1).
	\end{equation}
	En utilisant la linéarité de \( df_a\),
	\begin{equation}
		df_a(t)=tdf_a(1)=f'(a)t.
	\end{equation}
\end{proof}
