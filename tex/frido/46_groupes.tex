...
\begin{proof}
    Soient \( a\) et \( b\), premiers entre eux, et \( x\in \eN\). On applique le théorème de division euclidienne à $x$ et \( a + b \): il existe des entiers \( p, r_0 \), uniques, tels que 
    \begin{equation*}
       x = (p-1)(a+b) + r_0\hspace{3em}\text{avec }0 \leq r_0 < a+b.
    \end{equation*}
    En d'autres termes, \( p(a+b)\) est le premier multiple de \( a+b\) supérieur ou égal à $x$. De plus, $p$ est positif car $x$ l'est. Nous avons alors
    \begin{equation}
        p(a+b)-ua-vb=x
    \end{equation}
    où \( u\) et \( v\) sont donnés par le théorème de Bézout. Il s'agit maintenant de savoir si nous pouvons être assuré d'avoir \( p>u\) et \( q>v\) dès que \( x\) est assez grand. Pour cela nous considérons les nombres \( u_i\) et \( v_i\) définis par
    \begin{equation}
        u_ia+v_ib=i
    \end{equation}
    pour \( i=1,\ldots, a+b\). Nous posons \( u^*=\max\{ u_i \}\), \( v^*=\max\{ v_i   \}\), et \( p^*=\max\{ u^*,v^* \}\). Maintenant si \( x>p^*(a+b)\), alors
    \begin{equation}
        x=p(a+b)-u_ka-v_kb
    \end{equation}
    où \( k=x-p(a+b)\).
