...
\begin{proof}
    \begin{enumerate}
        \item
            Il est clair que \( 1_G \in NH \). Soit \( nh, n'h' \) deux élements de \( NH \); alors \( nhn'h' = nhn'h^{-1}hh' \), et comme \( N \) est normal dans \( G \), le produit \( hn'h^{-1} \) est dans \( N \), de même que  \( nhn'h^{-1} \), tandis que \( hh' \in H \). Par ailleurs, \( nh \in HN \), car \( nh = hh^{-1}nh \), les trois derniers facteurs formant un  élément de \( N \) par normalité; de même \( hn \in NH \), montrant que \( NH = HN \). Enfin, comme \( (nh)^{-1} = h^{-1} n^{-1}) \), les inverses de \( NH \) sont dans \( HN = NH \).
        \item 
        ...
        \item
            ...
            Le noyau de \( f\) est \( \Kernel f=H\cap N\). En effet si \( a\in H\cap N\), nous avons \( f(a)=\sigma(a) = aH = H\), et donc \( H\cap N\subset \Kernel f\); d'autre part, si \( h\in H\) vérifie \( h\in\Kernel f\), alors \( f(h)=hN=N\), ce qui est uniquement possible si \( h\in N\).
