...
% Copyright (c) 2011-2023, 2025-2026
%   Laurent Claessens
...
\begin{definition}[Suite de composition]  \label{DefJWZSooNcntfK}
	...
	\begin{enumerate}
		\item
		      ...
		      \end{equation}
		      et telle que \( G_{i+1}\) est normal\footnote{Nous rappelons au cas où, que «normal» signifie «distingué», définition \ref{DEFooNIIMooFkZgvX}.} dans \( G_i\).
		\item
...
\begin{proposition}[\cite{BIBooTIRCooTIWtGD}]	\label{PROPooEQKFooHPeLCR}
	Soit une groupe \( G\) et une suite de composition \( \Sigma\) de \( G\). Nous avons équivalence entre :
	\begin{enumerate}
		\item		\label{ITEMooFEQHooAIUgCq}
		      \( \Sigma\) est strictement décroissante et n'admet pas de raffinement strictement décroissante autre que elle-même.
		\item		\label{ITEMooMRMFooASVQGy}
		      Chaque groupe de la suite est normal maximum dans le suivant (\( G_{i+1}\) est normal dans \( G_i\)).
		\item		\label{ITEMooPPKHooJBpDGj}
		      Les quotients de la suite sont des groupes simples.
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[\ref{ITEMooFEQHooAIUgCq} \( \Rightarrow\) \ref{ITEMooMRMFooASVQGy}]
		%-----------------------------------------------------------
		Nous considérons la suite de composition strictement décroissante
		\begin{equation}
			\{ e \}=G_n\subsetneq G_{n-1}\subsetneq\ldots\subsetneq G_1\subsetneq G_0=G.
		\end{equation}
		Nous supposons que \( G_{i+1}\) n'est pas normal maximum dans \( G_i\). C'est à dire qu'il existe un sous-groupe \( N\) tel que
		\begin{equation}
			G_{i+1}\subsetneq N\subsetneq G_i
		\end{equation}
		tel que \( N\) est normal dans \( G_i\). Notez que \( G_{i+1}\) est normal dans \( G_i\) et est donc à fortiori normal dans \( N\). Bref, en ajoutant \( N\) nous avons un raffinement strictement décroissant de \( \Sigma\).
		\spitem[\ref{ITEMooMRMFooASVQGy} \( \Rightarrow\) \ref{ITEMooFEQHooAIUgCq}]
		%-----------------------------------------------------------
		Nous faisons non-\ref{ITEMooFEQHooAIUgCq} implique non-\ref{ITEMooMRMFooASVQGy}. Dit que \( \Sigma\) a un raffinement strictement décroissant signifie qu'il existe un sous-groupe \( N\) tel que
		\begin{equation}
			G_{i+1}\subsetneq N\subsetneq G_i
		\end{equation}
		où \( G_{i+1}\) est normal dans \( N\) et \( N\) est normal dans \( G_i\). Donc \( G_{i+1}\) n'est pas normal maximum dans \( G_i\).
		\spitem[\ref{ITEMooMRMFooASVQGy} \( \Leftrightarrow\) \ref{ITEMooPPKHooJBpDGj}]
		%-----------------------------------------------------------
		C'est la proposition \ref{PROPooUWLHooVUxTHB}.
	\end{subproof}
\end{proof}

\begin{theorem}[Jordan-Hölder\cite{BIBooTIRCooTIWtGD}]\label{ThoLgxWIC}
...
\begin{proof}
	...
	\begin{subproof}
		...
		Soit \( D\), l'ensemble des suites de composition strictement décroissantes de \( G\). Nous montrons d'abord que \( D\) est non vide. Si \( | G |=1\), c'est à dire \( G=\{ e \}\), nous avons la suite vide. Sinon nous avons la suite \( G,\{ ej \}\). Donc \( D\) est non vide.

		Nous savons par la proposition \ref{PROPooUIXNooJdKuAs} que si \( (G_i)_{i=0,\ldots,n}\) est une suite de composition, alors
		\begin{equation}
			\prod_{i=1}^n| G_i/G_{i+1} |=| G |.
		\end{equation}

		Si de plus \( (G_i)\) est strictement décroissante, nous avons le lemme \ref{LEMooZJAQooARqTCG} qui dit que \( | G_i/G_{i+1} |\geq 2\).

		Donc si \( s\in D\) est de longueur \( n\), nous avons \( \Card(G)\geq 2^n>n\). Donc les longueurs des éléments de \( D\) sont bornées par \( \Card(G)\). Soit \( s_M\in D\) de longueur maximale. Elle n'a pas de raffinement strictement décroissant. La proposition \ref{PROPooEQKFooHPeLCR} dit qu'alors tous les quotients sont simples.
