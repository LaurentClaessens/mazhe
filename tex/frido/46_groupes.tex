...
\begin{proof}
%TODO: Qu'est ce qu'un produit "vide" ? Def. de l'identité ??
...
\begin{definition}[Ordre d'un groupe et d'un élément]       \label{DEFooKWBCooMlmpCP}
	...
	\begin{enumerate}
		\item
%TODO: Peut-on faire reference à PROPooJLGKooDCcnWi ? Et distinguer (ou unifier ?) la notation | G | , de \Card(G) ??
		      Si \( G\) est un groupe, l'\defe{ordre}{ordre!d'un groupe} est la cardinalité de \( G\) et est noté \( | G |\).
%TODO: Notion de groupe fini, infini ??
		\item
...
\begin{proof}
	...
	\end{equation}
	nous allons utiliser le deuxième théorème d'isomorphisme (\ref{THOooURXUooQJvkjx}\ref{ItembgDQEN}) que nous appliquons à \( H=A\cap B\) et \( N=A'(A\cap B')\). La vérification que \( H\) normalise \( N\) est usuelle. Nous commençons par écrire
	...
	\end{equation}
%TODO: probleme de mélange entre B' et B ?  \( x\in A\cap B\) et non \( x\in A\cap B'\) . Plus loin aussi ?
	...
	\end{equation}
%TODO: \( A\cap B'\) est un sous-ensemble de \( A\cap B'\) ? Pas plutot : un sous-ensemble de \( A\cap B\) ?
	...
	\end{equation}
	Nous devons encore justifier \( B'(A\cap B)=(A\cap B)B'\) et \( B'(A'\cap B)=(A'\cap B)B'\). Vérifions la première égalité, et laissons la seconde \href{https://abstrusegoose.com/395}{au lecteur}.
...
\begin{proof}
	...
	\begin{equation}
		\left| { G_i }/{ G_{i+1} \right| = \frac{ | G_i | }{ | G_{i+1} | }.
	\end{equation}
...
\begin{proof}
	...
	Les propriétés que \( \psi(G)_i\) soit normal dans \( \psi(G)_{i+1}\), et que \( \psi(G)_i/\psi(G)_{i+1}\) soit abélien, proviennent directement de la proposition~\ref{PropSRMJooIDPBoW}.
\end{proof}
...
\begin{proof}
	...
	Les dernières trois égalités écrites disent que \( \phi_{g^{-1}}\) est l'inverse\footnote{Si vous décidez de dire ça à un jury dans un concours, soyez prêts à préciser les domaines.} de \( \phi_g\).
\end{proof}
...
\begin{definition}[Stabilisateur]       \label{DEFooMDYGooLrOERP}
	Si \( G\) est un groupe agissant que l'ensemble \( E\) et si \( x\in E\), nous notons \( G_x\) ou \( \Stab(x)\) le \defe{stabilisateur}{stabilisateur} de \( x\) :
	\begin{equation}
...
\begin{proof}
	...
	\end{enumerate}
	Par conséquent
%TODO: Pourquoi ne pas utiliser \Card()  ici ?? (et plus bas)
	\begin{equation}
