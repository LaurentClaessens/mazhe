...
\begin{definition}[Groupe simple]   \label{DefGroupeSimple}
    Un groupe est dit \defe{simple}{simple!groupe} si les seuls sous-groupes normaux qu'il admet sont lui-même et le sous-groupe réduit à l'identité.
\end{definition}
...
\begin{lemma}   \label{LemZhxMit}
    Soient \( G\) et \( H\) deux groupes monogènes\footnote{Définition \ref{DEFooWMFVooLDqVxR}.} de même ordre. Soient \( g\) un générateur de \( G\) et \( h\), un générateur de \( H\). Il existe un isomorphisme de \( G\) sur \( H\) qui envoie \( g\) sur \( h\).
\end{lemma}

\begin{proof}
    On construit cet isomorphisme à la main. Soit \( \phi : G \to H \), qui envoie $g$ sur $h$. Il est clair, de par la définition de morphisme, que \( \phi(e_G) = e_H \). Soit $n$ l'ordre (identique!) de $g$ et de $h$, s'il est fini.
    
    Alors, tout élément $x$ de $G$ s'écrit \( x = g^m \) pour un certain entier \( m \in \eN_{n-1} \) (ou \( m \in \eZ \) si l'ordre est infini), et cet entier est unique. Mais alors, \( \phi (x) = \phi(g^m) = (\phi(g))^m = h^m \).
    
    Il s'ensuit que $\phi$ est injective, car si \( y = g^k \), on a \( \phi(y) = h^k \), et \( \phi (x) = \phi (y) \) si et seulement si \(h^{m-k} = e_H \), ce qui implique $m = k$ et donc $x = y$.
    
    De plus, $\phi$ est surjective, car un élément \( z \in H \) s'écrit sous la forme \( z = h^m \) et il est donc atteint par \( g^m \).
\end{proof}
