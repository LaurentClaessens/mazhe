...
\begin{lemma}[\cite{MonCerveau}]   \label{LemZhxMit}
    Soient \( G\) et \( H\) deux groupes monogènes\footnote{Définition \ref{DEFooWMFVooLDqVxR}.} de même ordre. Soient \( g\) un générateur de \( G\) et \( h\), un générateur de \( H\). 

    Il existe un isomorphisme de \( G\) sur \( H\) qui envoie \( g\) sur \( h\).
\end{lemma}
...
\begin{proof}
    ...
    \begin{subproof}
        \item[L'ordre de \( G\) est fini et vaut \( n\)]
            Si \( k\in \eN\), nous notons \( k\mod n\) le plus petit élément positif de la classe de \( k\) modulo \( n\). La classe elle-même est notée \( [k]_n\).

            Nous construisons l'isomorphisme \( \phi\colon G\to G\) de la façon suivante: 
            \begin{equation}
                \phi(g^m)=h^{m\mod n}.
            \end{equation}
            Cela est une bonne définition parce que une égalité du type \( g^m=g^{m'}\) implique que \( m\) et \( m'\) soient dans la même classe modulo \( n\). Notons que pour tout \( m\in \eN\), l'application \( \phi\) vérifie immédiatement
            \begin{equation}
                \phi(g^m)=h^m.
            \end{equation}
            Nous vérifions que cela est une isomorphisme entre \( G\) et \( H\).
            \begin{subproof}
            \item[Morphisme]
                Pour l'identité, si \( x=e_G\) alors \( m=0\) et \( \phi(e_G)=h^0=e_H\). Et si \( x=g^k\), \( y=g^l\) alors \( \phi(xy)=\phi(g^{k+l})=h^{k+l}=\phi(g^k)\phi(g^l)=\phi(x)\phi(y)\).
            \item[Injectif]
            ...
            \item[Surjectif]
                Vu que \( h\) est monogène et engendré par \( h\), tout élément de \( H\) est de la forme \( h^m=\phi(g^m)\).
            \end{subproof}
        \item[L'ordre de \( G\) est infini]

                Si l'ordre de \( G\) est infini alors un élément \( x\in G\) s'écrit de façon unique sous la forme \( x=g^m\) avec \( m\in \eZ\). Dans ce cas nous définissons à nouveau \( \phi(g^m)=h^m\).
...
\begin{lemmaDef}[PPCM et PGCD]
    Soient \( p,q\in\eZ^*\). 
    \begin{enumerate}
        \item
    Les parties \( p\eZ\cap q\eZ\) et \( p\eZ+q\eZ\) sont des sous-groupes de \( \eZ\). 
        \item
    Il existe des entiers \( \ppcm(p,q)\) et \( \pgcd(p,q)\) tels que
\begin{subequations}
    \begin{align}
...
% TODO : j'ai l'impression que cette subsubsection sur la décompisition en facteurs premiers peut être mise après le calcul effectif de Bezout.
