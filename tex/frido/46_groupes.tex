...
%TODO: preuve.
%TODO: réfléchir à un meilleur placement de ce lemme.
...
\begin{probleme}
    Choses à vérifier :
    \begin{itemize}
        \item La proposition \ref{ooQIMHooUzLUJY} demande sans doute de parler de la décomposition en facteurs premiers des entiers. Probablement à déplacer.
...
\begin{proof}
    Si $H$ est un tel sous-groupe d'un groupe $G$, alors $G$ se partitionne en deux classes: $H$ et \( xH \) avec \( x \notin H \). Puisque la classe à droite \( Hx \) n'est pas $H$, on a \( xH = Hx \), et $H$ est normal dans $G$ par la proposition \ref{propGroupeNormal}.
\end{proof}
...
\begin{definition}  \label{DefActionGroupe}
    ...
    \begin{gather*}
        \phi_{1_G}(x) = x, \hspace{2em} \forall x \in E;\\
...
\begin{remark}
    Les deux axiomes d'action de groupe permettent de montrer que, pour tout \( g \in G \), \(\phi_{g^{-1}} \) est la réciproque de \( \phi_g \).
\end{remark}
...
\begin{corollary}
    ...
    \begin{equation}
        \Card(C_g)=| G:Z_G |
    \end{equation}
    où $Z_G$ est le centre\footnote{Définition \ref{defGroupeCentre}.} de \( G\).
\end{corollary}
...
%TODO: définir ce qu'est un p-groupe, au bon endroit (à côté du théorème de Lagrange?).
