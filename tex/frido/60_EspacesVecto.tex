...
\begin{example}
    ...
    Les éléments non nuls hors diagonale sont \( A_{13}\), \( A_{31}\) et \( A_{23}\). Elle n'est donc pas irréductible; nous avons par exemple la parition \( I=\{ 1,3 \}\), \( J=\{ 2 \}\) pour le critère de la proposition \ref{PROPooZTYDooZAxQxF}\ref{ITEMooNLVXooYSQKwO}. 


    Les valeurs propres de la matrice sont sur des bords de disques de Gershgorin, sans être sur tous les bords, comme ça aurait été le cas par le théorème \ref{THOooTXAPooQqsBCj} si la matrice avait été irréductible.
\end{example}
<++>
