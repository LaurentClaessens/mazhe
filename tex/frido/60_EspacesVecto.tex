...
\begin{proposition}[\cite{ooZGNYooGgPFhl}]
...
\begin{definition}
    ...
    \end{equation}
    sont les \defe{disques de Gershgorin}{Gershgorin!disque}.
\end{definition}

\begin{theorem}[Gershgorin]     \label{THOooUJNFooHpvCCF}
    Soit \( A\in \eM(n,\eR)\). Si \( \lambda\in \eC\) est valeur propre de \( A\) alors
    \begin{equation}
        \lambda\in\overline{ B(A_{ii},\sum_{j\neq i}| A_{ij} |) }
    \end{equation}
    pour un certain \( i\).
\end{theorem}
...
\begin{lemma}
    Soit \( A\in \eM(n,\eR)\) et une valeur propre \( \lambda\) de \( A\) telle que
    \begin{equation}
        \lambda\notin\bigcup_{i=1,\ldots, n}B\big( A_{ii},\sum_{j\neq i}| A_{ij} | \big).
    \end{equation}
    Soit \( u\in \eR^n\) un vecteur propre pour \( \lambda\) et \( i\) tel que \( | u_i |=\| u \|_{\infty}\). Alors
    \begin{equation}
        | A_{ii}-\lambda |=\sum_{j\neq i}| A_{ij} |.
    \end{equation}
    
    Si de plus \( j\) est directement connecté à \( i\), alors
    \begin{enumerate}
        \item
            \( | u_j |=\| u \|_{\infty}\)
        \item
...
\begin{proof}
    Nous écrivons la \( i\)\ieme ligne de \( Au=\lambda u\) : \( \sum_{k=1}^nA_{ik}u_k=\lambda u_i\), qui donne
    \begin{equation}        \label{EQooREKEooGnANmG}
        | A_{ii}-\lambda | |u_i |\leq \sum_{k\neq i}A_{ik}u_k.
    \end{equation}
    Mais comme \( | u_i |=\| u \|_{\infty}\) cela donne
    \begin{equation}
        | A_{ii}-\lambda |\leq \sum_{k\neq i}| A_{ik} |.
    \end{equation}
    Vu que par hypothèse, \( \lambda\) n'est pas dans la boule ouverte, cette inégalité est en réalité une égalité :
    \begin{equation}
        | A_{ii}-\lambda |=\sum_{k\neq i}| A_{ik} |.
    \end{equation}
    
    Si de plus \( j\) est directement connecté à \( i\) via la matrice \( A\) nous repartons de \eqref{EQooREKEooGnANmG} en remplaçant \( | A_{ii}-\lambda |\) par \( \sum_{k\neq i}| A_{ik} |\) :
    \begin{equation}
        | A_{ii}-\lambda | |u_i |\leq \sum_{k\neq i}| A_{ik} | |u_k |,
    \end{equation}
    ou encore
    \begin{equation}
        \sum_{k\neq i}| A_{ik} |\big( | u_i |-| u_k | \big)\leq 0.
    \end{equation}
    Mais chacun des termes est positif. Donc tous les termes sont nuls. C'est à dire que dès que \( A_{ik}\neq 0\) nous avons \( | u_i |=| u_k |=\| u \|_{\infty}\).
\end{proof}
<++>


\begin{theorem}[Gershgorin numéro 2]        \label{THOooTXAPooQqsBCj}
    Soit une matrice \( A\) irréductible\footnote{Définition \ref{DEFooXIREooQtlzkO}.} dans \( \eM(n,\eR)\) ainsi qu'une valeur propre \( \lambda\) de \( A\). Alors soit
    \begin{equation}
        \lambda\in\bigcup_{i=1,\ldots, n}B\big( A_{ii},\sum_{j\neq i}| A_{ij} | \big),
    \end{equation}
    soit
    \begin{equation}
        \lambda\in\bigcap_{i=1,\ldots, n}\partial B\big( A_{ii},\sum_{j\neq i}| A_{ij} | \big).
    \end{equation}
    C'est à dire que soit \( \lambda\) est à l'intérieur d'une des boules, soit elle est sur l'intersection de tous les bords.
\end{theorem}

\begin{proof}
    Nous écrivons la \( i\)\ieme ligne de \( Au=\lambda u\).
\end{proof}
<++>
