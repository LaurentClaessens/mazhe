...
Attention : le passage à \eqref{SUBEQooDRZFooAuuaad} n'est pas une trivialité.


\begin{example} \label{ExZHZYcNH}
    Voyons commet la différentielle seconde fonctionne. Soient deux espaces vectoriels de dimension finie \( V\) et \( W\). Pour que les choses soient claires, nous avons :
    \begin{subequations}
        \begin{align}
            f&\colon V\to W\\
            df&\colon V\to \aL(V,W)\\
    ...
    Soit \( f\in C^2(V,W)\); nous notons \( \varphi=df\colon V\to \aL(V,W)\) et donc nous voulons étudier la fonction
    \begin{equation}
