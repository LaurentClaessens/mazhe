...
\begin{proposition}[Caractérisation séquentielle d'un fermé]    \label{PropLFBXIjt}
	Soient \( X\) un espace métrique\footnote{Définition \ref{}.} et \( F\subset X\). L'ensemble \( F\) est fermé si et seulement si toute suite contenue dans \( F\) et convergeant dans \( X\) converge vers un élément de \( F\).
\end{proposition}
...
\begin{proposition}[Inégalité triangulaire point-partie\cite{BIBooTHPCooHuDKOy}]	\label{PROPooTXYMooDGnfKB}
	Soit un espace métrique\footnote{Espace métrique, définition \ref{}.} \( (X,d)\) et soit une partie non vide \( A\subset X\). Pour tout \( x,y\in A\) nous avons :
	\begin{equation}
