...
\begin{proposition}[Inégalité triangulaire point-partie\cite{BIBooTHPCooHuDKOy}]	\label{PROPooTXYMooDGnfKB}
	Soit un espace métrique \( (X,d)\) et soit une partie non vide \( A\subset X\). Pour tout \( x,y\in A\) nous avons :
	\begin{equation}
...
\begin{proof}
	...
	\end{equation}
	Vu que \(d \colon X\times X\to X  \) est continue, en prenant la limite \( n\to \infty\), à droite nous avons zéro\footnote{Parce que \( d\) est continue, proposition \ref{}.} et donc
	\begin{equation}
	...
	\end{equation}
	ce qui signifie que \( f\) est continue en \( x\).
\end{proof}
