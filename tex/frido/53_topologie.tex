...
\begin{proposition}[Topologie initiale\cite{BIBooFDGQooYferue}]     \label{PROPooGOEVooZBAOQh}
...
\begin{lemma}[\cite{BIBooFDGQooYferue,BIBooWTRYooMLJtCL}]     \label{LEMooVRGFooNgbwKu}
	Soient des espaces topologiques \( \{ (Y_i,\tau_i) \}_{i\in I}\), et \( X\), un ensemble. Nous considérons des applications \( \varphi_i\colon X\to Y_i\).

	L'ensemble
	\begin{equation}
		\mB=\left\{ \bigcap_{j=1}^n \varphi^{-1}_{\alpha_j}(U_j)\tq n\in \eN,\alpha_1,\ldots,\alpha_n\in I,U_k\in \tau_k \right\}
	\end{equation}
