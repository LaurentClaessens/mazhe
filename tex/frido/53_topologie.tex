...
\begin{definition}[Produit d'espaces topologiques, thème~\ref{THEMEooYRIWooDXZnhX}]      \label{DefIINHooAAjTdY}
    ...
    \begin{equation}
        X=\prod_{i=1}^nX_i
    \end{equation}
    muni de la topologie obtenue en postulant que \( U\subset X\) est un ouvert si et seulement si pour tout \( x\in U\), il existe \( U_i\in \tau_i\) tels que
    \begin{equation}
...
\begin{proof}
%TODO: ou est la definition de l'adhérence, terme employé ici, et non defini jusqu'à présent (juste ci-dessous !) ?
...
%TODO: tu es sur de la ref ITEMooJDUUooVMvAOn  pour un compact ?
    ...
    Donc pour la topologie de \( \mathopen[ 0 , 1 \mathclose]\), toutes les boules ouvertes \( B(x,\delta)\) avec \( x\in\mathopen[ 0 , 1 \mathclose]\) sont incluses dans \( \mathopen[ 0 , 1 \mathclose]\).
\end{example}
...
\begin{definition}      \label{DEFooSVWMooLpAVZRInt}
    ...
    \begin{enumerate}
        \item
            Un point \( x\in X\) est \defe{intérieur}{point intérieur} à \( A\) si il est contenu dans un ouvert inclus dans \( A\).
        \item
...
\begin{proof}
    ...
    \end{itemize}
%TODO: Bon, alors là, je suis perdu : \bar A ici n'est pas l'adhérence !!, mais plutot le complementaire, non ?? => Utiliser A^c ???
...
%TODO: Probleme: qu'est ce que \( \bar A \) ici ?
    ...
    \begin{subproof}
        ...
        \item[Symétrique]
            ...
            \begin{equation}
                \lim_{x\to a} \beta(x)=0
            \end{equation}
            parce que \( \lim_{x\to a} 1 + \alpha(x)=1\) et \( \lim_{x\to a} -\alpha(x)=0\).
        \item[Transitive]
...
\begin{proof}
    ...
    \item[L'espace lui-même]
        ...
        \item[Intersection finie]
            ...
            \begin{equation}
%TODO: je ne comprends pas cette equation.  Plutot : \bigcap_{i=1}^nB_i\cup\{ \omega \}=\left( \bigcap_{i=1}^nB_i \right)\cup\{ \omega \}
            ...
            \begin{equation}
                \bigcap_{i=1}^nA_i=B\cap\big( \bigcap_{i=1}^nB_i\cup\{ \omega \} \big)=B\cap\big( \bigcap_{i=1}^nB_i \big)
            \end{equation}
...
%TODO: c'est quoi 'le cas précédent' ?  Pourquoi ne pas dire : En posant E =...    ?
            \begin{equation}
                B\cap\left( \bigcap_{i=1}^mB_i\cup\{ \omega \} \right)=B\cap\big( E\cup\{ \omega \} \big)=B\cap E
            \end{equation}
            avec \( X\setminus E\) compact dans \( X\).

            Puisqu'elle est compacte, la partie \( X\setminus E\) est fermée, et donc \( E\) est ouvert. Le tout est au final un ouvert de \( X\), et donc dans \( \hat\tau\).
    \end{subproof}
...
\begin{proposition}[\cite{ooMKWJooLSkGfh}]
  ...
  \end{align}
%TODO: probleme: la definition de homéomorphismes vient plus tard :(
...
%TODO: ou est defini "topologie est métrique" ?
    Puisque \( \eK\) est un corps dont la topologie est métrique\footnote{Pour rappel, nous supposons toujours ça.}, il existe une boule \( S'=B(0,\delta)\subset S\).
...
\begin{proof}
    ...
    \begin{subproof}
        \item[\( \Rightarrow\)]
            Nous supposons que \( f\) est continue en \( a\in X\). Soit un voisinage \( W\) de \( f(a)\) dans \( Y\). Par continuité de \( f\) en \( a\), il existe un voisinage \( V\) de \( W\) tel que \( f(V)\subset W\). A fortiori, \( f\big( V\setminus{{a}} \big)\subset W\) comme le demande la définition de la limite.
        \item[\( \Leftarrow\)]
...
%TODO: je pense que ce paragraphe doit apparaitre plus tot dans le document ???
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\begin{propositionDef}      \label{PROPooAWAKooKRmbGT}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
%TODO:  une distance ici...    Sauf si "métrique" est defini quelque part ?
...
\begin{proof}
    Soit par l'absurde un \( \epsilon>0\) contredisant le lemme. Il n'existe pas d'ensemble fini autour des points desquels les boules de taille \( \epsilon\) recouvrent \( X\).
...
\begin{proof}
   ...
   \begin{equation}
%TODO: Notation ^c  à appliquer ici ??? Adherence ?
...
\begin{proof}
	...
	\begin{equation}
		A=f(K)=\{ f(x)\tq x\in K \}.
	\end{equation}
	Nous considérons le supremum $M=\sup A=\sup_{x\in K}f(x)$ avec la convention : si $A$ n'est pas borné supérieurement, nous posons $M=\infty$ (voir définition~\ref{DefSupeA}).
...
%TODO: je trouve cela mal écrit parce qu'on ne voit pas distinctement comment on passe d'une ligne à l'autre. Je préfererai des systemes d'inéquations qui se combinent à la fin
        \begin{align}
...
\begin{proof}
%TODO: probleme ici \int  ? C'est l'interieur \Int ?
...
\begin{lemma}[\cite{BIBooXHRKooZrKDcs}]     \label{LEMooAVIJooFhdXXr}
    Soit une application \( N\colon E\to \mathopen[ 0 , \infty \mathclose[\) telle que
        ...
        \begin{equation}
%TODO: ce ne serait pas   \leq 1   au lieu de  = 1  ???
            B = \{ x\in E\tq N(x)=1 \}
        \end{equation}
...
\begin{proof}
    ...
    \item[\( \Rightarrow\)]
        ...
        \begin{subequations}
            \begin{align}
                N\big( \lambda x+(1-\lambda)y \big) & \leq N(\lambda x)+N\big( (1-\lambda)y \big) = \lambda N(x)+(1-\lambda)N(y)\\
