...
\begin{proposition}[\cite{BIBooDMSUooCZKkdj}]
...
\begin{proof}
    ...
    \item[\( \Leftarrow\)]
        Soit un ouvert \( \mO\) de \( Y\). Nous devons prouver que \( f^{-1}(\mO)\) est un ouvert de \( X\). Pour cela nous prouvons que \( f^{-1}(\mO)\) contient un ouvert autour de chacun de ses éléments\footnote{Théorème \ref{ThoPartieOUvpartouv}.}
