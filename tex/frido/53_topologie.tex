...
\begin{proof}
    ...
    \item[\ref{ITEMooOEZYooFBNaOZ} implique \ref{ITEMooNIPZooIDPmEf}]
        Nous avons évidemment l'union disjointe \( X=A\cup(X\setminus A)\). Nous supposons que \( A\) est ouvert et fermé, donc \( X\setminus A\) est également ouvert et fermé (c'est la définition \ref{DEFFermeooNSAAooHxZbAo} d'un fermé). Bref, \eqref{EQooZBDJooPqjRYk} est union de fermés disjoints et par hypothèse nous avons soit \( A=\emptyset\) soit \( X\setminus A=\emptyset\).
...
\begin{definition} \label{DefJJVsEqs}
    Une partie $A$ d'un espace topologique est \defe{compacte}{compact} si il vérifie la propriété de Borel-Lebesgue : pour tout recouvrement de $A$ par des ouverts (c'est-à-dire une collection d'ouverts dont la réunion contient $A$) on peut extraire un recouvrement fini.
\end{definition}
...
\begin{theoremDef}[Topologié métrique]     \label{ThoORdLYUu}
