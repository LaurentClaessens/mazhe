...
\begin{definition}[Base de topologie\cite{BIBooUMGRooSwJMRo,ooKBUGooWCSiXh}]\label{DefQELfbBEyiB}
    Soit un espace topologique \( (X,\mT)\). Une partie \( \mB\) de \( \mT\) est une \defe{base de topologie}{base de topologie} pour \( X\) si tout élément de \( \mT\) est une réunion d'éléments de \( \mB\).
\end{definition}

\begin{definition}[Base de topologie\cite{ooKBUGooWCSiXh}]   
...
\begin{definition}[Topologie induite\cite{BIBooTQXWooMOxuoy}] \label{DefVLrgWDB}
...
\begin{proposition} \label{PropNBSooraAFr}
    Un espace métrique séparable\footnote{Qui possède une partie dense dénombrable, définition~\ref{DefUADooqilFK}.} accepte une base de topologie\footnote{Base de topologie, définition \ref{}.} dénombrable.
