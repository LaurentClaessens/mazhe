...
\begin{theorem}[\cite{ooGQTBooJKpoVP}]
...
\begin{proof}
    ...
    \begin{subproof}
        \item[La différentielle partielle \( Df\)]
            Pout \( t\) fixé nous notons \( Df_{(t,x)}\) la différentielle de \( \varphi_t\) en \( x\); si on aime les notations lourdes : \( Df_{(t,x)}=(d\varphi_t)_{x}\). Nous avions introduit le point \( a\in \Omega\), et cela nous permet de considérer
            \begin{equation}
                \begin{aligned}
                    Df\colon \eR\times \GL(n,\eR)&\to \GL(n,\eR) \\
                    (t,\psi)&\mapsto Df_{(t,y_a(t))}\circ \psi. 
                \end{aligned}
            \end{equation}
            Nous savons que \( f\) est de classe \( C^1\), donc toutes ses dérivées partielles sont continues (proposition \ref{PropDerContCun}). À \( t\) fixé, toutes les dérivées partielles de \( \varphi_t\) sont continues (ce sont les mêmes) en \( x\) et en \( t\). Donc \( Df\) est continue en \( t\). La continuité en \( \psi\) est le fait que la composition d'application linéaire est une application continue sur \( \GL(n,\eR)\)\footnote{Cela n'est pas une trivialité. Repensez à la topologie sur \( \GL(n,\eR)\).}. L'application \( Df\) est même localement Lipschitz en \( \psi\) (sa seconde variable). 

        \item[L'équation aux variations]

            Nous pouvons donc poser l'équation différentielle ordinaire
            \begin{subequations}        \label{EQooQONGooBrxuSA}
                \begin{numcases}{}
                    \frac{ d\psi }{ dt }=Df_{(t,y_a(t))}\circ\psi(t)\\
                    \psi(t_0)=\id.
                \end{numcases}
            \end{subequations}
            qui est une équation différentielle ordinaire pour \( \psi\colon \eR\to \GL(n,\eR)\) rentrant dans le cadre de Cauchy-Lipschitz en identifiant \( \GL(n,\eR)\) avec \( \eR^{n^2}\).  Quel est le domaine de définition de \( \psi\) ? C'est un ouvert autour de \( t_0\). Nous réduisons \( W\) de telle sorte que la solution \( \psi\) soit définie sur \( W\).  

            L'équation \eqref{EQooQONGooBrxuSA} s'appelle l'\defe{équation aux variations}{équation!aux variations}.

        \item[\( \psi\) est continue en \( (t,x)<++>\)<++>]<++>
    \end{subproof}
\end{proof}
<++>
