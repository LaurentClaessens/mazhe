...
\begin{proposition}[Régularité \( C^1\) du flot\cite{ooEGXQooRwPKcC}]        \label{PROPooINLNooDVWaMn}
    Soit un intervalle ouvert \( I \) de \( \eR\) et un ouvert connexe \( \Omega\) de \( \eR^n\). Soit une fonction \( f\in C^1\big( I\times \Omega,\eR \big)\),  ainsi que \( a\in \Omega\) et \( t_0\in I\).
...
\begin{proof}
    ...
    \item[Régularité \( C^1\)]
        ...
        L'équation \eqref{EQooTOJMooLXLfVv} est de type Cauchy-Lipschitz à paramètre (le paramètre est \( x\)) dont les propriétés sont dans le théorème \ref{THOooDTCWooSPKeYu}. Nous savons que la solution est unique et continue parce que \( F\) est une composée de fonctions continues. Ici nous utilisons de façon cruciale le fait que nous avons déjà démontré la continuité \( C^1\) du flot \( (t,x)\mapsto \varphi(t,x)\).
        ...
        Toutes les dérivées partielles étant continues, la fonction \( (t,x)\mapsto \varphi(t,x)\) est de classe \( C^1\) (proposition \ref{PropDerContCun}).
...
\begin{proposition}[Cauchy-Lipschitz avec paramètre, régularité \( C^p\)\cite{ooCMJNooHgCXBS,ooSDCGooACTQQH,ooZWJZooBocVPb}]       \label{PROPooPYHWooIZhQST}
    Soit un intervalle ouvert \( I\) de \( \eR\), un connexe ouvert \( \Omega\) de \( \eR^n\) et un intervalle ouvert \( \Lambda\) de \( \eR^d\). Soit une fonction \( f\in C^1( I\times \Omega\times \Lambda, \eR^n)\) localement Lipschitz en \( \Omega\). Soient \( t_0\in I\), \( y_0\in \Omega\) et \( \lambda_0\in \Lambda\). Il existe un voisinage compact de \( (t_0,y_0,\lambda_0)\) sur lequel le problème
    \begin{subequations}
        \begin{numcases}{}
        ...
        \end{numcases}
    \end{subequations}
    possède une unique solution. De plus \( (t,\lambda)\mapsto y_{\lambda}(t)\) est de classe \( C^1\) par rapport à ses deux variables.
\end{proposition}
...
\begin{proof}
    ...
    \begin{subequations}        \label{SUBEQSooIBTNooHzYImh}
        ...
        \end{numcases}
    \end{subequations}
    Le théorème de Cauchy-Lipschitz que nous prenons sous la forme \ref{PROPooINLNooDVWaMn} nous indique que ce problème admet une unique solution maximale et que le flot \(   \big( t,(y_0,\mu) \big)     \mapsto  x_{(y_0,\mu)}(t)   \) est de classe \( C^1\).
    ...
    De plus l'application \( (t,\mu)\mapsto y(t,\mu)=x_{(t_0,\mu)}(t)_1\) est de classe \( C^1\).
...
\begin{theorem}[Régularité \( C^p\) du flot et du paramètre]
    Soient les équations différentielles
    \begin{subequations}        \label{EQooOOCNooEeVPfL}
        \begin{numcases}{}
            \frac{ \partial \varphi }{ \partial t }(t,x)=f\big( t,\varphi(t,x) \big)\\
            \varphi(t_0,x)=x
        \end{numcases}
    \end{subequations}
    et
    \begin{subequations}        \label{EQooAXCPooIIcftL}
        \begin{numcases}{}
            \frac{ \partial y }{ \partial t }(t,\lambda)=g\big( t,y(t,\lambda),\lambda \big)\\
            y(t_0,\lambda)=y_0.
        \end{numcases}
    \end{subequations}
    où \( f\) et \( g\) sont des fonctions de classe \( C^p\) localement Lipschitz en \( \Omega\).

    Alors les systèmes admettent chacune une unique solution de classe \( C^p\).
\end{theorem}

\begin{proof}
    Nous avons déjà prouvé dans la proposition \ref{PROPooINLNooDVWaMn} que \( \varphi\) est de classe \( C^1\), et dans la proposition \ref{PROPooPYHWooIZhQST} que \( y\) est de classe \( C^1\). De plus l'application \( A\colon (t,x)\mapsto (d\varphi_t)_x\) vérifie l'équation \eqref{EQooTOJMooLXLfVv} :
    \begin{equation}
        \frac{ \partial A }{ \partial t }(t,x)=F\big( t,A(t,x),x \big).
    \end{equation}
    où $F$ est une fonction de classe \( C^p\). Cela est une équation du type \eqref{EQooAXCPooIIcftL} dont les solutions sont de classe \( C^1\). Étant donné que \( (t,x)\mapsto (d\varphi_t)_x\) est alors de classe \( C^1\), nous savons que \( \varphi\) est de classe \( C^2\).

    Nous savons que l'équation pour $y$ est équivalente à une équation de type \eqref{EQooOOCNooEeVPfL} avec \( f\) de classe \( C^p\). Nous en déduisons que \( y\) est de classe \( C^2\).

    Nous pouvons continuer ainsi en augmentant la régularité de \( y\) et \( \varphi\) l'une après l'autre jusqu'à la classe \( C^p\).
