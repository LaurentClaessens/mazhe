...
Soient une énumération \( (q_i)\) de \( \eQ\cap\mathopen] 0 , 1 \mathclose[\) et une suite \( (r_i)\) telle que \( \sum_{i=0}^{\infty}r_i<\frac{ 1 }{2}\). Nous supposons de plus que \( B(q_i,r_i)\subset \mathopen[ 0 , 1 \mathclose]\).
    ...
    Soit un intervalle \( I\) dans \( \mathopen[ 0 , 1 \mathclose]\). Il existe \( q_i\in I\) et donc\footnote{C'est ici que nous utilisons le fait que \( r_i\) est choisi pour que \( B(q_i,r_i)\) ne déborde par \( \mathopen[ 0 , 1 \mathclose]\). Sinon il aurait fallu chipoter et prendre seulement une partie de la boule.}
    \begin{equation}
...
Vous voulez un truc dingue à propos de la partie \( J\) de l'exemple \ref{EXooVZVIooXZvDaE} ? Le théorème \ref{THOooJNMCooPMvCDq} nous dit qu'il existe dans \( J\) des compacts de mesure arbitrairement proches de \( \lambda(J)\). Il existe donc des compacts de mesure strictement positive tout en étant infiniment découpés.
