...
\begin{proof}
    Vu que \( f\) est positive, nous utilisons la définition \ref{EqooGAFMooZLzjPs}. Soit une fonction étagée \( \psi\) minorant \( f\). Nous la décomposons en
    \begin{equation}
        \psi=\sum_{k=1}^n\psi_k\mtu_{A_k}
    \end{equation}
    où les \( A_k\) sont mesurables et \( \psi_k\in\mathopen[ 0 , \infty \mathclose[\). Nous allons prouver que \( \psi_i\mu(A_k)=0\) pour tout \( k.\).

        \begin{subproof}
        \item[Si \( A_k\cap \Omega_+=\emptyset\)]
            Soit \( x\in A_k\). Nous avons
            \begin{equation}
                \psi=\psi(x)\leq f(x)=0.
            \end{equation}
            Donc \( \psi_k=0\).
        \item[Si \( A_k\subset\Omega_+\)]
...
\begin{proof}
    Il suffit de prendre les cubes de la proposition~\ref{PropSKXGooRFHQst} et de les fermer. Ce que l'on ajoute est de mesure nulle\footnote{Voire \ref{}.}.
\end{proof}
