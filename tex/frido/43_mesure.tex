...
\begin{proof}
    Vu que \( f\) est positive, nous utilisons la définition \ref{EqooGAFMooZLzjPs}. Soit une fonction étagée positive \( \psi\) minorant \( f\). Nous la décomposons en
    \begin{equation}
    ...
    où les \( A_k\) sont mesurables et \( \psi_k\in\mathopen[ 0 , \infty \mathclose[\). Nous allons prouver que \( \psi_i\mu(A_k)=0\) pour tout \( k\), en séparant trois cas.
        ...
        \begin{equation}
            \int_{\Omega}\psid\mu= \sum_{k=1}^n\psi_k\mu(A_k)=0.
        \end{equation}
