...
\begin{lemma}[\cite{ooMKWJooLSkGfh}]\label{LEMSommeTopologique}
  Soit \( V \) un espace vectoriel topologique, et \( \mO \) un voisinage ouvert de \( 0 \). Il existe un voisinage ouvert \( U\) de \( 0 \), symétrique, tel que \( U + U \subset \mO \).
\end{lemma}

\begin{proof}
    Le lemme \ref{LEMooQEFRooHAxOys} donne des voisinages ouverts \( U_1\) et \( U_2\) de \( 0\) dans \( V\) tels que \( U_1+U_2\subset \mO\).. En posant \( U = U_1 \cap U_2 \cap (-U_1) \cap (-U_2) \), on a un sous-ensemble symétrique de \( U_1\) et \(U_2\) qui vérifie \( U + U\subset W \).
\end{proof}


\begin{proposition}[Séparation \cite{ooMKWJooLSkGfh,BIBooUCGNooFLcCoo}]
...
\begin{proof}
    ...
    \item[Quelque chose sans intersection avec \( F\)]
        En appliquant deux fois le lemme \ref{LEMSommeTopologique}, il existe un ouvert symétrique \( U_x\) autour de \( 0\) tel que
        \begin{equation}
...
\begin{theorem}[Espace topologique métrisable\cite{ooMKWJooLSkGfh, MonCerveau}]      \label{THOooAGBXooZnvQLK}
    Si $V$ est un espace vectoriel topologique possédant en tout point une base de topologie\footnote{Définition \ref{DefQELfbBEyiB}.} dénombrable, alors il existe une distance \( d\) sur \( V\) telle que
    \begin{enumerate}
        \item
...
\begin{proof}
    Soit \( \{ A_i \}_{i\in \eN}\) une base de topologie autour de \( 0\) dans \( V\). 

    \begin{subproof}
    \item[Nouvelle base de topologie]
        Nous construisons une nouvelle base de topologie \( \{ U_i \}_{i\in \eN}\) autour de \( 0\) de la façon suivante :
        \begin{subequations}
            \begin{align}
                U_0&=A_0\\
                U_{k+1}+U_{k+1}+U_{k+1}+U_{k+1}&\subset A_k\cap U_{k}.       \label{SUBEQooECMVooLllLhu}
            \end{align}
        \end{subequations}
        Le fait est que \( A_k\cap U_k\) est un ouvert autour de \( 0\); donc le lemme \ref{LEMSommeTopologique} permet de construire un ouvert \( U_{k+1}\) autour de \( 0\) ayant la propriété \eqref{SUBEQooECMVooLllLhu}.

    \item[C'est bien une base]
            Notons d'abord que \( U_k\subset A_k\). Si \( \mO\) est un ouvert autour de \( 0\), il existe \( k\) tel que \( \mO\subset A_k\) (parce que les \( A_k\) forment une base de topologie). Donc \( \mO\subset U_k\).
    ...
    d(x,y) = \inf\{r \in\varphi(\eD_2) \tq y - x \in \phi(r)\}.
    \end{equation}
...
\item[\( d(x,y)=0\) implique \( x=y\)]
    ...
    \item[$d(x,z) \leq d(x,y) + d(y,z)$] Soit \(\epsilon > 0 \). Par définition des distances comme infimums, et grâce au corolaire \ref{CorDensiteDyadiques}, il existe \(r\) et \(s\) dans \( \varphi(\eD_2) \) tels que:
    \begin{equation}
        d(x,y) < r < d(x,y) + \frac \epsilon 2 \quad\text{ et }\quad d(y,z) < s < d(y,z) + \frac \epsilon 2 .
    \end{equation}
    En particuler, \(d(x,y) + d(y,z) <  r + s\). 
