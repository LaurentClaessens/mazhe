...
\begin{definition}[Partie équilibrée\cite{ooMKWJooLSkGfh}]
    Une partie \( E\) d'un espace vectoriel topologique \( V\) est \defe{équilibrée}{partie équilibrée} si pour tout \( | \alpha |<1\) dans \( \eK\), \( \alpha E\subset E\).
\end{definition}
...
\begin{lemma}[\cite{ooMKWJooLSkGfh}]\label{LEMSommeTopologique}
  ...
  \begin{enumerate}
    ...
    \item
        \( U\) est équilibré
    \item
        \( U\) vérifie \( U + U + U + U \subset \mO \). Note : à fortiori \( U+U\subset\mO\).
  \end{enumerate}
\end{lemma}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Symétrique]
            En posant \( U = U_1 \cap U_2 \cap (-U_1) \cap (-U_2) \), on a un sous-ensemble symétrique de \( U_1\) et \(U_2\) qui vérifie \( U + U\subset W \).
        \item[équilibré]
            C'est le moment d'utiliser le lemme \ref{LEMooYSWXooNqAcOQ}.
    \end{subproof}
    <++>
\end{proof}
...
\begin{theorem}[Espace topologique métrisable\cite{ooMKWJooLSkGfh, MonCerveau}]      \label{THOooAGBXooZnvQLK}
    ...
    \begin{enumerate}
        \item
            Tout point admet une base dénombrable de topologie\footnote{Définition \ref{DefQELfbBEyiB}.}.
        \item
