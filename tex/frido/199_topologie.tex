...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    L'égalité \eqref{EQooEUKNooVoJfFC} n'est donc pas possible. Nous déduisons que \( n_0\) n'existe en fait pas et que \( x=y\).
\end{proof}
...
\begin{proof}
    ...
    \item[Nouvelle base de topologie]
        ...
        \begin{subequations}
            ...
            \end{align}
        \end{subequations}
        Le fait est que \( A_k\cap U_k\) est un ouvert autour de \( 0\); donc la proposition \ref{PropSommeTopologique} permet de construire un ouvert \( U_{k+1}\) autour de \( 0\) ayant la propriété \eqref{SUBEQooECMVooLllLhu}.
    ...
    \item[C'est bien une base]
        ...
        \item[Quelques inclusions]
            ...
            \end{equation}
            À fortiori nous avons \( U_{n+1}\subset U_n\) et pour tout \( n,k\in \eN\), nous obtenons alors
            \begin{equation}\label{EqBaseTopoMetriquePf2}
                U_{n+1} + U_{n+2} + \cdots  + U_{n+(k-1)} + U_{n+k} \subset  U_{n+1} + U_{n+1} \subset U_n.
            \end{equation}

        \item[Les ensembles \( D_n\)]
        \item[La fonction \( \phi\)]
    
    Nous définissons maintenant une fonction à valeurs dans les parties de \( V\) :
    \begin{equation}
        \begin{aligned}
            \phi\colon D\cup\mathopen[ 1 , +\infty \mathclose[&\to \partP(V) \\
            r&\mapsto  
    \begin{cases}
        V & \text{si } r \geq 1;\\
