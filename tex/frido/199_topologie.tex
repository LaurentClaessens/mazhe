...
\begin{lemma}[\cite{BIBooSHPPooMkbgoC}]     \label{LEMooQEFRooHAxOys}
    Soit un espace vectoriel topologique \( V\) ainsi qu'un voisinage \( \mO\) de \( 0\) dans \( V\). Il existe des voisinages \( U_1\) et \( U_2\) de \( 0\) dans \( V\) tels que
    \begin{equation}
        U_1+U_2\subset \mO.
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    est continue. Donc la partie \( f^{-1}(\mO)\) est un ouvert de \( V\times V\) (c'est la définition \ref{DefOLNtrxB}\ref{ITEMooEHGWooDdITRV} de la continuité). La partie \ref{ITEMooKOFTooIOWhMG} de la définition \ref{DefIINHooAAjTdY} de la topologie produit, appliquée au point \( (0,0)\in V\times V\) implique qu'il existe des voisinages \( U_1\) et \( U_2\) de \( 0\) dans \( V\) tels que
    \begin{equation}
...
\begin{lemma}[\cite{ooMKWJooLSkGfh}]\label{PropSommeTopologique}
  Soit \( V \) un espace vectoriel topologique, et \( \mO \) un voisinage de \( 0 \). Il existe \( U \) un voisinage de \( 0 \), symétrique, tel que \( U + U \subset \mO \).
\end{lemma}

\begin{proof}
    Le lemme \ref{LEMooQEFRooHAxOys} donne des voisinages \( U_1\) et \( U_2\) de \( 0\) dans \( V\) tels que \( U_1+U_2\subset \mO\).. En posant \( U = U_1 \cap U_2 \cap (-U_1) \cap (-U_2) \), on a un sous-ensemble symétrique de \( U_1\) et \(U_2\) qui vérifie \( U + U\subset W \).
\end{proof}
...
\begin{proof}
    Grâce à la proposition~\ref{PropInvarianceTopologie}, on peut tout ramener en \(0 \) puis faire les transports en tous les points de l'espace. Mieux: grâce à la proposition~\ref{PropSommeTopologique} (appliquée deux fois de suite), on peut créer une base de voisinage\footnote{Définition \ref{DEFooBWZIooXotZLA}.} \( (U_n) \) de \( 0 \) telle que pour tout \(n \in \eN\),
...
\end{equation}
et \( D = \bigcup_{n>0} D_n\). Ensuite, définissons \(\phi \) sur \(D \cup \mathopen[1,+ \infty\mathclose[ \) et à valeurs dans les parties de \( V \):
\begin{equation}
  \phi (r) =
  \begin{cases}
    V & \text{si } r \geq 1;\\
