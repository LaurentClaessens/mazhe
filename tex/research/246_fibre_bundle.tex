...
\begin{proposition}[\cite{BIBooJMRFooTAhhcg}]		\label{PROPooACWKooYLbIom}
	Let \( M\) and \( N\) be smooth manifolds. We consider a smooth map \(f \colon M\to N  \) and \( \omega\in \Omega^{k}(N)\). Then we have
	\begin{equation}		\label{EQooWUMCooLdNqmS}
		f^*(d\omega)=d(f^*\omega),
	\end{equation}
	or \( [f^*,d]=0\).
\end{proposition}
...
\begin{proof}
	Since \( d\) and \( f^*\) are linear, we check the equality \eqref{EQooWUMCooLdNqmS} on \( \omega=a\partial_{\wedge I}^*\) with \( a\in C^{\infty}(N)\) and \( I\) being a \( k\)-multiindex. Since \( \partial_i^*=dx_1\) (lemma \ref{LEMooJCXRooMQxauV}), we have
	\begin{subequations}
		\begin{align}
			f^*(d\omega)(X_1,\ldots,X_{k+1}) & = (da\wedge dx_{i_1}\wedge\ldots\wedge dx_{i_k})\big( df(X_1),\ldots,df(X_{k+1}) \big)                                                                                   \\
			                                 & = \Big( d(a\circ f)\wedge d(x_{i_1}\circ f)\wedge\ldots\wedge d(x_k\circ f) \Big)(X_1,\ldots,X_{k+1}) & \text{prop. \ref{PROPooINQYooXZcfou}}	\label{SUBEQooOCCWooUpOtqi}
		\end{align}
	\end{subequations}

	Now we search for an expression for \( d(f^*\omega)\). For \( x\in M\) we have
	\begin{subequations}
		\begin{align}
			(f^*\omega)_x(X_1,\ldots,X_k) & = \omega_{f(x)}\big( df_xX_1,\ldots,df_xX_k \big)                                                                                                \\
			                              & =a\big( f(x) \big)\partial_{\wedge I}^*\big( df_xX_1,\ldots,df_xX_k \big)                                                                        \\
			                              & = (a\circ f)(x)\Big( d(x_{i_1}\circ f)_x\wedge\ldots\wedge d(x_{i_k}\circ f)_x  \Big)(X_1,\ldots,X_k) & \text{s.b.}  \label{SUBEQooAQVZooNuBcVr}
		\end{align}
	\end{subequations}
	Justification for \eqref{SUBEQooAQVZooNuBcVr}: proposition \ref{PROPooINQYooXZcfou}.

	Now, using the lemma \ref{LEMooAONJooDgEJjA} we have
	\begin{equation}
		d(f^*\omega)=d(a\circ f)\wedge d(x_{i_1}\circ f)\wedge\ldots \wedge d(x_{i_k}\circ f)+(a\circ f)\underbrace{d\Big( d(x_{i_1}\circ f)\wedge\ldots\wedge d(x_{i_k}\circ f) \Big)}_{=0}
	\end{equation}
	This is the same as \eqref{SUBEQooOCCWooUpOtqi}.
...
The following is a corollary of Leibniz rule, proposition~\ref{Leibniz}.
