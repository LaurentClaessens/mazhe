...
\begin{definition}[\cite{MonCerveau}]		\label{DEFooFRPLooTxMUzg}
	...
	\begin{enumerate}
		\item
		      A vector space \( V_x\) for each \( x\in M\) such that \( V_x\cap V_y=\emptyset\) when \( x\neq y\). If you want to put the same vector space \( V\) everywhere, you have to set, for example \( V_x=\{ (x,v)\tq v\in V \}\), so that the \( V_x\) are distinct.
		\item
		      For each chart \( U_{\alpha}\) of \( M\) and each open part \( W\subset \eR^k\), a map
		      \begin{equation}
			      \sigma_{\alpha, W} \colon U_{\alpha}\times W\to \bigcup_{x\in M}V_x.
		      \end{equation}
	\end{enumerate}
	...
	\begin{enumerate}
		\item
		      The maps \(\sigma_{\alpha, W} \colon U_{\alpha}\times W\to \bigcup_{x\in M}V_x  \) make \( \bigcup_{x\in M}V_x\) a smooth manifold.
		\item
		      For every \( s\in U_{\alpha}\) and \( v\in \eR^k\) we have \( \sigma_{\alpha,W}(s,v)\in V_{\varphi_{\alpha}(s)}\) for every open \( W\) containing \( v\).
		\item
...
\begin{propositionDef}		\label{DEFooCSDZooJuzGuE}
	...
	\begin{enumerate}
		...
		\item For each open set \( W\) in \( \eR^{k_1k_2}\) we set
		      \begin{equation}
			      \begin{aligned}
				      \sigma_{\alpha, W}\colon U_{\alpha}\times W      & \to T                                                                                    \\
	...
	\begin{enumerate}
		\item
		      The set	\( T\) with the charts \( \sigma_{\alpha,W} \) is a smooth vector bundle of dimension \( k_1k_2\) over \( M\).
		\item
