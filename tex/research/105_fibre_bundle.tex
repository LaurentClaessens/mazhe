...
\begin{proposition}		\label{PROPooHUDYooLCAYZW}
	...
	\begin{equation}
		s^*\big( g\beta_1\wedge\ldots \beta_n \big)=(s^*g)\big( \det(ds) \big)\alpha_1\wedge\ldots \alpha_n.
	\end{equation}
...
\begin{proof}
	...
	\begin{subequations}
		...
		\end{align}
	\end{subequations}
	Justification for \eqref{SUBEQooVLMLooHPxZFR}. We used the proposition \ref{PROPooRRSZooJXOApq}. Formally we should write \( (\beta_i)_{s(x)}\) and \( (e_j)_x\), but we drop the indices.
\end{proof}
