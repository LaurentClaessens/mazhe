		...
		\begin{subequations}
			\begin{align}
				\tau_{\alpha,s}(e^*_{\wedge I})(X_1,\ldots,X_k) & = \partial^*_{\wedge I}(X_1,\ldots,X_k)                                                                                                              \\
				                                                & =(\partial_{i_1}^*\wedge \cdots \wedge \partial_{i_k}^*)(X_1,\ldots,X_k)                                                                             \\
...
\begin{proof}
	Proof by induction over \( k\). If \( k=1\), we have \( d(df_1)=0\) by definition \ref{PROPooTXFRooMtaVrU}\ref{ITEMooJXMFooHFTFYi}. If not we set \( \omega=df_2\wedge\cdots \wedge df_k\) and we use the Leibniz rule \ref{PROPooTXFRooMtaVrU}\ref{ITEMooBBFLooWpZJuT} :
	\begin{equation}
		d(df_1\wedge\cdots\wedge df_k)=(ddf_1)\wedge\omega-df_1\wedge d\omega.
	\end{equation}
...
\begin{lemma}[\cite{MonCerveau}]		\label{LEMooAONJooDgEJjA}
	Let \( a\) and \( f_1,\ldots,f_k\) be smooth functions. If \( \omega=df_1\wedge\cdots\wedge df_k\), then
	\begin{equation}
...
\begin{proposition}[\cite{MonCerveau}]		\label{PROPooINQYooXZcfou}
	...
	\begin{equation}
		(dg_1\wedge\ldots\wedge dg_k)(df_xX_1,\ldots,df_xX_k)=d(g_1\circ f)_x\wedge\cdots \wedge d(g_k\circ f)_x(X_1,\ldots,X_k).
	\end{equation}
\end{proposition}

\begin{proof}
	First take a look at the formulas of corollary \ref{CORooDQSNooZtMuOJ}. Then we prove by induction over \( k\). We set \( \omega=dg_2\wedge\cdots \wedge dg_k\) and we compute :
	\begin{subequations}
		\begin{align}
	...
	\begin{equation}
		\omega\big( df_xX_{\pi(2)},\ldots,df_xX_{\pi(k)} \big)=d(g_2\circ f)_x\wedge\cdots\wedge d(g_k\circ f)_x\big( X_{\pi(2)},\ldots,X_{\pi(k)} \big),
	\end{equation}
