...
\begin{definition}[\cite{MonCerveau}]		\label{DEFooFRPLooTxMUzg}
	Let \( M\) be a smooth manifold with charts \( \{ \varphi_{\alpha} \colon I_{\alpha}\to M   \}_{\alpha\in \tau}\). A \( k\)-dimensional \defe{vector bundle}{vector bundle} on \( M\) is a couple \( \big( \{ V_x \}_{x\in M}, \{\sigma_{\alpha} \colon U_{\alpha}\times \eR^n\to \bigcup_{x\in M}V_x   \}_{\alpha\in \tau} \big)\) where
	\begin{enumerate}
		\item
		      The \( V_x\) are distinct\footnote{If you want to put the same vector space \( V\) everywhere, you have to set, for example \( V_x=\{ (x,v)\tq v\in V \}\), so that the \( V_x\) are distinct.} \( k\)-dimensional vector spaces.
		\item
		      The maps \(\sigma \colon U_{\alpha}\times \eR^k\to \bigcup_{x\in M}V_x  \) make \( \bigcup_{x\in M}V_x\) a smooth manifold.
		\item
		      For every \( s\in U_{\alpha}\) and \( v\in \eR^k\) we have \( \sigma_{\alpha}(s,v)\in V_{\varphi_{\alpha}(s)}\).
		\item
		      We denote \( E=\bigcup_{x\in M}V_x\). The map
		      \begin{equation}
			      \begin{aligned}
				      \pi\colon E & \to M     \\
				      V_x         & \mapsto x
...
\begin{propositionDef}
	Let \( M \) be a smooth manifold. We consider two vector bundles\footnote{Definition \ref{DEFooFRPLooTxMUzg}.} \( V_x^{(i)}\), \( \sigma_{\alpha}^{(i)}\) with transition functions \(g_{\alpha\beta}^{(i)} \colon U_{\alpha}\cap U_{\beta}\to \GL(\eR,k)  \).
