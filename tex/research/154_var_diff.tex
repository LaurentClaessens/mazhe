...
\begin{definition}
    Let \( \mA\) be a class of functions as \(  C^{\infty}\), \( C^k\), or analytic. A $n$-dimensional \( \mA\)-\defe{manifold}{differentiable!manifold}\index{manifold} is a set $M$ and a system of charts $\{(\mU_{\alpha},\varphi_{\alpha})\}_{\alpha\in I}$ where each set $\mU_{\alpha}$ is open in $\eR^n$ and the maps $\dpt{\varphi_{\alpha}}{\mU_{\alpha}}{M}$ are injective and satisfy the three following conditions:
...
\begin{example}
    Any open set of $\eR^n$ is a smooth manifold if we choose the identity map as chart system.
\end{example} 

Most of surfaces $z=f(x,y)$ in $\eR^3$ are manifolds, depending on certain regularity conditions on~$f$.

\begin{definition}
    Let \( \mA\) be a class of functions (once again you can think with \( \mA= C^{\infty}\)).
\end{definition}
<++>
...
\begin{definition}[\cite{BIBooXDPUooVeTGwz}]
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \begin{enumerate}
                ...
                \item
                    the map \( \psi^{-1}\circ h\circ\varphi\colon U \to V \) is \( C^k\).
            \end{enumerate}
    \end{enumerate}
\end{definition}
