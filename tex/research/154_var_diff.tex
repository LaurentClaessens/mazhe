...
\begin{definition}      \label{DEFooJJVIooDUBwDJ}
    ...
    \end{equation}
    An element of \( T_aM\) is a \defe{tangent vector}{tangent vector} to \( M\) at the point \( a\).
\end{definition}
...
\begin{proposition}[\( T_pM\) is  a vector space]
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            \sum_k\partial_k(f\circ \varphi)(0)\Dsdd{ \varphi^{-1}\big( \gamma(t) \big)_k }{t}{0}\\
            &=\Dsdd{ (f\circ\varphi)\big( \varphi^{-1}(\gamma(t)) \big) }{t}{0}\\
...
\begin{propositionDef}[Differential of a map]
...
\begin{proof}
    Let \( \phi\in C^1(N,\eR)\). The map \( \phi\circ  f\colon M\to \eR\) belongs to \( C^1(M,\eR)\) and we can apply \( \nabla_{\gamma}\) and \( \nabla_{\sigma}\) on it. By hypothesis,
    \begin{equation}
