...
\begin{proposition}[\cite{BIBooDUPSooZjcTHL}]
    Every smooth submanifold is the image of a smooth immersion\footnote{Definition \ref{DEFooZKUIooXWVGvh}.}.
\end{proposition}

\begin{proposition}     \label{PROPooZACHooCNgLSl}
    A manifold \( S\) is a submanifold of \( M\) if \( S\subset M\) (as sets) and the identity \( \iota\colon S\to M\) is regular.
\end{proposition}
