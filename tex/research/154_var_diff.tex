...
\begin{proof}
    ...
    Let \( \gamma\colon \eR\to M\) be an integral curve satisfying \( \gamma(0)=p\).

    Let \( K\) be a compact interval containing \( 0\) in \( \eR\). For each \( a\in K\) we consider a chart \( \varphi_a\colon U_a\to M\) such that \( \gamma(a)\in\varphi(U_a)\).
\end{proof}
<++>
