...
A manifold is a topological space with charts that have some properties. The topology can be reconstructed from the charts. This is proposition \ref{PROPooDEVUooCATTZI}.

\begin{proposition}[\cite{MonCerveau}]		\label{PROPooDEVUooCATTZI}
	Let \( M\) be a \( n\)-dimensional topological manifold. Let \( E\) be a \( n\)-dimensional normed vector space and \( \Lambda\) the set of all the charts on \( M\) starting from open parts of \( E\)\footnote{We want to say that \( \Lambda\) is the set of every maps of \( M\), but this is not a set.}. We consider
	\begin{equation}
		\tau_{\Lambda}=\{ S\subset M\tq \forall \alpha\in\Lambda,\varphi_{\alpha}^{-1}(S\cap M_{\alpha})\in\tau_E \}.
	\end{equation}
...
\begin{proposition}[\cite{MonCerveau}]			\label{PROPooGAZZooWIPVuf}
	...
	%TODOooAZMQooZBIyCx. Prouver ça.
\end{proposition}


%-------------------------------------------------------
\subsection{Vector space as manifold}
%----------------------------------------------------


\begin{definition}[\cite{MonCerveau}]	\label{DEFooEQAWooPrHFEa}
	If \( V\) is a finite dimensional vector space, it becomes a manifold by taking any norm \( \| . \|\) on \( V\) and considering the unique identity chart
	\begin{equation}
		\begin{aligned}
			\varphi\colon V & \to V      \\
			x               & \mapsto x.
		\end{aligned}
	\end{equation}
	Proposition \ref{PROPooKQXDooSGexpm} shows that the choice of the norm does not affect the analytic structure on \( V\).
\end{definition}
