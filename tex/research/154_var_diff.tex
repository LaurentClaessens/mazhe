...
\begin{definition}[\cite{BIBooAQKHooVyiyN}]		\label{DEFooBDJJooAxszUA}
	Let \( \mW=\{ W_{\alpha} \}_{\alpha\in A}\) be a covering of a set \( E\). A covering \( \mY=\{ Y_i \}_{i\in I}\) is a \defe{refinement}{refinementj of \( \mW\) } if for every \( i\in I\), there exits a \( \alpha\in A\) such that \( Y_i\subset W_{\alpha}\).
\end{definition}
...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		We prove that \( \{ B_i \}_{i\in I}\) is locally finite. Let \( x\in M\). The exists a \( i\in I \) such that \( x\in B_i\).
		\begin{subproof}
...
\begin{proposition}[\cite{MonCerveau}]		\label{PROPooYJKOooRwbOXF}
...
\begin{definition}		\label{DEFooVMWRooGQYJwl}
	Let \( \mA\) be a class of functions as \(  C^{\infty}\), \( C^k\), or analytic. A $n$-dimensional \( \mA\)-\defe{manifold}{differentiable!manifold}\index{manifold} is a topological manifold \( M\) with an atlas\footnote{Definition \ref{DEFooRUVIooSZDjlE}.} \( \{ (U_{\alpha}, \varphi_{\alpha}) \}_{\alpha\in\Lambda}\) such that for every \( \alpha,\beta\in\Lambda\),
	\begin{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[The set \( \mR(p)\)]
		%-----------------------------------------------------------
		We define \( \mR(p)=\{ \varphi_{\alpha}\big( C(2) \big) \}_{\alpha\in\Lambda_p}\) and we prove that this is a basis of neighbourhood at \( p\).
	\end{subproof}
\end{proof}
...
\begin{definition}[\( T_pM\) is a vector space\cite{MonCerveau}]		\label{DEFooRNPJooHpUDTi} \label{PROPooEJBWooSbvypo}
...
\begin{lemma}[\cite{MonCerveau}]		\label{LEMooSZTOooBIzMCc}
	Let \( M\) be a \( C^k\) manifold. We consider two charts \(\varphi_{\alpha} \colon U_{\alpha}\to M  \) and \(\varphi_{\beta} \colon U_{\beta}\to M  \). We have
	\begin{equation}
		\partial^*_{\alpha,i}(\partial_{\beta,j})=(d\varphi_{\alpha\beta})_{ij}
	\end{equation}
	where \( \varphi_{\alpha\beta}=\varphi_{\alpha}^{-1}\circ\varphi_{\beta}\).
\end{lemma}
