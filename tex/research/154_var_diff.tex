...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Matrix]
            ...
            \begin{subequations}
                \begin{align}
                    (df_p)_{\alpha i}&=\left( df_p\big( \frac{ \partial  }{ \partial x_i } \big) \right)_{\alpha}\\
                    &=\Big( df_p\circ d\varphi_{\varphi^{-1}(p)}e_i \Big)_{\alpha}.
                \end{align}
            \end{subequations}
...
\begin{theorem}[Rank theorem\cite{BIBooVYIRooZyqygg}]
    ...
    \begin{subproof}
        \item[The case of \( \eR^m\) and \( \eR^n\)]
            ...
            \begin{equation}
                ...
                \end{pmatrix}
            \end{equation}
            where \( A\) is \( k\times k\) and \( *\) is some \( k\times (n-k)\) matrix. Thus we have \( \det(d\varphi_0)=\det(A)\neq 0\). From the inverse function theorem \ref{ThoXWpzqCn}, the map \( \varphi\) is a local diffeomorphism, more precisely there exists an open set \( W_1\subset W\) such that the restriction
            \begin{equation}
