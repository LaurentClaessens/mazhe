...
In this section we deal with Lie groups made from matrices, that is subgroups of \( \GL(n, \eC)\) and their Lie algebra. We will denote the identity either by \( e\) or by \( \mtu\).
...
\begin{normaltext}      \label{NORMooHZGKooJEiamo}
    ...
    \begin{itemize}
        ...
        \item 
            ...
            \end{equation}
            This is a set of matrices on which we can use the bracket \( [X,Y]=XY-YX\) (matrix product). We will see in the proposition \ref{PROPooUKITooLnEKZW} that \( G'\) is a Lie algebra and in the proposition \ref{} that \( G'\) and \( T_eG\) are isomorphic as Lie algebras.
    \end{itemize}
\end{normaltext}
...
\begin{proof}
    ...
    \begin{subproof}
        \item[\( \phi\) is well defined]
            ...
            We consider a chart \( \varphi\colon \mU\to \mO\) where \( \mO\) \( \mU\) is a neighbourhood of \( 0\) in \( \eR^m\) and \( \mO\) is a neighbourhood of \( e\) in \( \GL(n,\eC)\). We suppose that \( \varphi(0)=e\). We set \( \tilde f=f\circ \varphi\), \( \tilde \alpha=\varphi^{-1}\circ \alpha\) and \( \tilde \beta=\varphi^{-1}\circ\beta\). We have
            \begin{subequations}
                \begin{align}
        ...
        \item[\( \phi\) is a Lie algebra isomorphism]
            ...
            \begin{subequations}
                \begin{align}
                    ...
                    &=\phi(X)^L_e\big( \phi(Y)^L(f) \big)-\phi(Y)^L_e\big( \phi(X)^L(f) \big).
                \end{align}
            \end{subequations}
            We focus on the first term:
            \begin{subequations}
                \begin{align}
                    \phi(X)^e\big( \phi(Y)^L(f) \big)&=\Dsdd{ \phi(Y)^L_{\phi(X)^L_e(t)}(f) }{t}{0}\\
