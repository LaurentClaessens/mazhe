...
\begin{proof}
	...
	\begin{subproof}
		...
		Now we show that \( \phi(\lH)=\exp(\lH)\) contains a neighbourhood of \( e\) in \( H\). We recall that the definition \ref{PROPooMAGKooInwNom} says that the topology on \( \lG\) is any norm topology.
...
\begin{proposition}[\cite{BIBooFLEXooPgvAlz}]	\label{PROPooXCMUooJqsTvB}
	Let \( G\) be a smooth Lie group. If \(f \colon G\to \eR  \) is a smooth function, then
	\begin{equation}
		f\big( \exp \big)
	\end{equation}<++>
\end{proposition}
