...
\begin{proposition}[\cite{BIBooFLEXooPgvAlz}]	\label{PROPooSOOTooMrtYmP}
	Let \( G\) be a Lie group and \( H\) be a closed subgroup of \( G\). We consider the part
	\begin{equation}
		\lH=\{ X\in\lG\tq \exp(tX)\in H\,\forall t \}.
	\end{equation}
	We consider a neighbourhood \( U\) of \( 0\) in \( \lG\) such that
	\begin{equation}
		\exp \colon U\cap \lH\to \exp(U)\cap H
	\end{equation}
	is  a bijection\footnote{Lemma \ref{LEMooNPLBooBrwNWY}.}. For each \( h\in H\) we define
	\begin{equation}
		\begin{aligned}
	...
	\begin{enumerate}
		\item		\label{ITEMooQUKXooISAtJV}
		      The set \( H\) with the maps \( \{ \varphi_h \}_{h\in H}\) is a topological manifold.
		\item
		      The topology of the manifold \( H\) the the topology induced from \( G\) to \( H\).
		\item
		      The topological manifold \( H\) is a smooth manifold.
		\item		\label{ITEMooYGZFooUshihY}
		      The manifoold \( H\) is a smooth Lie group.
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	Several parts.
	\begin{subproof}
		\spitem[For \ref{ITEMooQUKXooISAtJV}]
		%-----------------------------------------------------------
		We have to check the condition of proposition \ref{DEFooJOMAooZscKwn}. The conditions \ref{ITEMooDWSWooWdcDdI}, \ref{ITEMooPEXDooNuJBKH} and \ref{ITEMooSRPQooNUPzlj} are easy. The crucial points are the two last ones.
		\begin{subproof}
			\spitem[Condition \ref{ITEMooWFAWooAqQfzZ}]
			%-----------------------------------------------------------
			\spitem[Condition \ref{ITEMooZHLXooBpWSXr}]
			%-----------------------------------------------------------
		\end{subproof}

		We consider on \( H\) the induced topology from \( G\). Since \( G\) is Hausdorff and second countable, the topological space \( H\) is Hausdorff and second-countable (proposition \ref{PROPooAHIMooYhOwUf}). Now we define the charts.
		...
		\spitem[For \ref{ITEMooYGZFooUshihY} : \( H \) is a smooth Lie group]
		%-----------------------------------------------------------
		We have to check the definition \ref{DEFooGDWTooTvINuw}.

		\begin{subproof}
			\spitem[The charts are smooth]
			%-----------------------------------------------------------
		...
		\end{subproof}
	\end{subproof}
	%TODOooBVUQooPaUQcz. Il faut finit la preuve. 
	% C'est déjà dans ma liste. Pas besoin d'ajouter.
	% - prouver que \tau est C^{\infty} d'inverse C^{\infty}.
	% - prouver que la somme est C^{\infty}.
\end{proof}
...
\begin{proof}
	...
	%TODOooBZLSooJeHmXD: prouver l'analycité
