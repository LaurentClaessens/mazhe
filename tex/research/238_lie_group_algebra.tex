...
% Copyright (c) 2013-2015, 2019-2022, 2024-2026
%   Laurent Claessens
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Analytic Lie groups}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooDEZZooMdbrnb}
	If \( G\) is an analytic Lie group and if \( \lG\) is its Lie algebra, then
	\begin{enumerate}
		\item
		      For each \( g\in G\), the map \(L_g \colon G\to G  \) is analytic.
		\item
		      For each \( X\in \lG\), the vector field
		      \begin{equation}
			      \begin{aligned}
				      X^L\colon G & \to TG        \\
				      g           & \mapsto dL_gX
...
\begin{lemma}[\cite{MonCerveau,BIBChatGPT}]	\label{LEMooFUXAooKZfHMl}
...
\begin{proposition}[\cite{BIBChatGPT,MonCerveau}]	\label{PROPooYHTQooDSsftm}
	Every continuous group morphism between Lie groups is \( C^{\infty}\). More precisely, let \(f \colon G_1\to G_2  \) be a continuous group morphism between Lie groupe. Then
	\begin{enumerate}
		\item		\label{ITEMooUASHooGyammh}
		      \( f\) is \( C^{\infty}\) on a neighbourhood of \( e\).
		\item		\label{ITEMooHUZOooWjoVyt}
		      \( f\) is \( C^{\infty}\) everywhere.
		\item
		      There exists a unique map \(A \colon \lG_1\to \lG_2  \) such that
		      \begin{equation}
			      f\big( \exp_1(tX) \big)=\exp_2\big( tA(X) \big).
		      \end{equation}
		\item
		      This \( A\) is linear.
		\item		\label{ITEMooGEZTooZyDSlo}
		      \( A=df_e\),
		\item		\label{ITEMooNPSMooEHkuJw}
		      The map \( A\) is a Lie algebra morphism:
		      \begin{equation}
			      A\big( [X,Y] \big)=\big[   A(X),A(Y) \big].
		      \end{equation}
	\end{enumerate}
\end{proposition}

\begin{proof}
	We denote by \( e_i\) the unit of \( G_i\) and by \( \lG_i\) its Lie algebra. Now go for the proof.
	\begin{subproof}
		\spitem[The map \( F\)]
		%-----------------------------------------------------------
		Proposition \ref{PROPooYFZZooLUOuOj} says that we have neighbourhood \( U_i\) of \( 0\) in \( \lG_i\) and \( V_i\) of \( e_i\) in \( G_i\) such that
		\begin{equation}
			\exp_i \colon U_i\to V_i
		\end{equation}
		is a \( C^k\) diffeomorphism.

		Since \( f\) is continuous and since \( f(e_1)=e_2\), we can choose \( U_1'\subset U_1\) such that \( f\big( \exp_1(U_1') \big)\subset V_2\). The we consider the map
		\begin{equation}
			\begin{aligned}
				G\colon U_1' & \to \lG_2                        \\
				F            & =\exp_2^{-1}\circ f\circ \exp_1.
			\end{aligned}
		\end{equation}

		\spitem[A one parameter subgroup]
		%-----------------------------------------------------------
		Let \( X\in \lG_1\). We consider the map
		\begin{equation}
			\begin{aligned}
				g_X\colon \eR & \to G_2                          \\
				t             & \mapsto f\big( \exp_1(tX) \big).
			\end{aligned}
		\end{equation}
		This is a one-parameter subgroup\footnote{Definition \ref{DEFooNWPFooLTkTSA}.} of \( G_2\) because
		\begin{subequations}
			\begin{align}
				g_X(s+t) & =f\Big( \exp\big( (s+t)X \big) \Big)                                                    \\
				         & =f\big( \exp_1(sX)\exp_1(tX) \big)              & \text{prop. \ref{PROPooNRVJooEDCpOI}} \\
				         & =f\big( \exp_1(sX) \big)f\big( \exp_1(sX) \big) & \text{\( f\) morphism}                \\
				         & =g_X(s)g_X(t).
			\end{align}
		\end{subequations}
		...
		\spitem[The map \( A\)]
		%-----------------------------------------------------------
		By proposition \ref{PROPooBJDCooRnfKcP}, there exists a unique \( Y\in \lG_2\) such that \( g_X(t)=\exp_2(tY)\). Thus we define the map
		\begin{equation}
			A \colon \lG_1\to \lG_2
		\end{equation}
		by the property
		\begin{equation}		\label{EQooFFGHooSxGHuV}
			f\big( \exp_1(tX) \big)=\exp_2\big( tA(X) \big).
		\end{equation}

		Taking \( \exp_2^{-1}\) on both side we get the equation
		\begin{equation}		\label{EQooPXGVooLhbXfK}
			F(tX)=tA(X).
		\end{equation}

		\spitem[\( A(\lambda X)=\lambda A(X)\)]
		%-----------------------------------------------------------
		We write the condition \eqref{EQooFFGHooSxGHuV} with \( \lambda X\) :
		\begin{equation}		\label{EQooCTTQooVpSgyO}
			f\big( \exp_1(t\lambda X) \big)=\exp_2\big( tA(\lambda X) \big).
		\end{equation}
		On the other hand, we write the same equation with \( X\) and \( t\lambda\) instead of \( t\) :
		\begin{equation}		\label{EQooCWOXooXuqriW}
			f\big( \exp_1(\lambda t X) \big)=\exp_2\big( \lambda t A(X) \big).
		\end{equation}
		Equating \eqref{EQooCTTQooVpSgyO} with \eqref{EQooCWOXooXuqriW} we get
		\begin{equation}		\label{EQooEWTFooGskakU}
			\exp_2\big( t A(\lambda X) \big)=\exp_2(t\lambda A(X)).
		\end{equation}
		Taking \( t\) small enough to have \( t A(\lambda X)\in V\) and \( t\lambda A(X)\in V\) we can get \( \exp_2^{-1}\) on both sides of \eqref{EQooEWTFooGskakU} :
		\begin{equation}
			t A(\lambda X)=\lambda tA(X).
		\end{equation}
		Simplifying by \( t\) we get the \( A(\lambda X)=\lambda A(X)\).

		\spitem[\( A\) is continuous]
		%-----------------------------------------------------------
		Consider a sequence \( X_n\stackrel{ \lG_1}{\longrightarrow} X\). Suppose that \( t\) is small enough for \( tX_n\in U_1'\) for every \( n\) large enough. For each \( n\) we have \( tA(X_n)=F(tX_n)\). Since \( F\) is continuous,
		\begin{equation}
			\lim_{n\to \infty}tA(X_n)=\lim_{n\to\infty}F(tX_n)=F(tX)=tA(X).
		\end{equation}

		\spitem[\( A(X+Y)=A(X)+A(Y)\)]
		%-----------------------------------------------------------
		Let \( X,Y\in \lG_1\) and \( t\) be small enough to have \( tX,tY\in U_1'\) and
		\begin{equation}
			\exp_2\big( tA(X) \big),\exp_2\big( tA(Y) \big)\in U_1'.
		\end{equation}
		Using Campbell-Baker-Hausdorff (proposition \ref{PROPooPTYCooCuHEbO}) : there exists a map \(\alpha \colon \eR\to \lG_2  \) such that \( \alpha(t)\to 0\) and
		\begin{equation}		\label{EQooTTRQooMrceNh}
			t\big( A(X)+A(Y) \big)+\frac{ t^2 }{2}\big[  A(X),A(Y) \big]+t^2\alpha(t)=\exp_2^{-1}\Big( \exp_2(tA(X))\exp_2(tA(Y)) \Big).
		\end{equation}
		The same formula also produces\footnote{Take \( t\) still smaller.}
		\begin{equation}		\label{EQooTMWAooNLutkD}
			\exp_1(tX)\exp_1(tY)=\exp_1\big(  Z(t) \big)
		\end{equation}
		with
		\begin{equation}
			Z(t)=t(X+Y)+\frac{ t^2 }{2}[X,Y]+t^2\beta(t)
		\end{equation}
		with \(\beta \colon \eR\to \lG_1  \) and \( \beta(t)\to 0\).

		Taking \( f\) in both sides of \eqref{EQooTMWAooNLutkD} and using the fact that \( f\) is a morphism:
		\begin{equation}
			(f\circ\exp_1)\big( Z(t) \big)=f\big( \exp_1(tX) \big)f\big( \exp_1(tY) \big)=\exp_2\big( tA(X) \big)\exp_2\big( tA(Y) \big).
		\end{equation}
		Taking \( \exp_2^{-1}\) on both sides,
		\begin{subequations}
			\begin{align}
				F\big( Z(t) \big) & =\exp_2^{-1}\Big( \exp_2\big( tA(X) \big)\exp_2(tA(Y)) \Big)                                                                                      \\
				                  & =t\big( A(X)+A(Y) \big)+\frac{ t^2 }{2}\big[    A(X),A(Y) \big]+t^2\alpha(t). & \text{eq. \eqref{EQooTTRQooMrceNh}}.		\label{SUBEQooPWCCooKGpAtO}
			\end{align}
		\end{subequations}

		Let
		\begin{equation}
			U(t)=X+Y+\frac{ t }{2}[X,Y]+t\beta(t).
		\end{equation}
		Using the property \eqref{EQooPXGVooLhbXfK} and \( tU(t)=Z(t)\) we get
		\begin{equation}
			F\big( Z(t) \big)=F\big( tU(t) \big)=tA\big( U(t) \big).
		\end{equation}
		Using the expression \eqref{SUBEQooPWCCooKGpAtO} for	\( F\big( Z(t) \big)\) we find
		\begin{equation}
			tA\big( U(t) \big)=t\big( A(X)+A(Y) \big)+\frac{ t^2 }{2}[A(X),A(Y)]+t^2\alpha(t).
		\end{equation}
		Divide by \( t\) :
		\begin{equation}
			A\Big( X+Y+\frac{ t }{2}[X,Y]+t\beta(t) \Big)=A(X)+A(Y)+\frac{ t^2 }{2}[A(X),A(Y)]+t^2\alpha(t).
		\end{equation}

		Since \( A\) is continuous, we can take the limit \( t\to 0\) on both sides and get
		\begin{equation}
			A(X+Y)=A(X)+A(Y).
		\end{equation}
	\end{subproof}

	\spitem[For \ref{ITEMooUASHooGyammh}]
	%-----------------------------------------------------------
	On \( U_1'\) we have \( f=\exp_2\circ A\circ \exp_1\). This is a composition of smooth maps.

	\spitem[For \ref{ITEMooHUZOooWjoVyt}]
	%-----------------------------------------------------------
	Let \( g\in G_1\). Let \( U\) be a neighbourhood of \( e_1\) on which \( f\) is \( C^{\infty}\). On the neighbourhood \( gU\) of \( g\) we have
	\begin{equation}
		f=L_{f(g)}\circ f\circ L_{g^{-1}}
	\end{equation}
	where the map \( f\) in the middle is the restriction of \( f\) to \( U\). Now on \( gU\), the map \( f\) is the composition of smooth maps.

	\spitem[For \ref{ITEMooGEZTooZyDSlo}]
	%-----------------------------------------------------------
	A general element of \( T_eG_1\) is \( X=\frac{d}{dt} \left[ \exp_1(tX)  \right]_{t=0}\). We have
	\begin{subequations}
		\begin{align}
			df_e(X) & =\frac{d}{dt} \left[ f\big( \exp_1(tX) \big)  \right]_{t=0} \\
			        & =\frac{d}{dt} \left[ \exp_2\big( tA(X) \big)  \right]_{t=0} \\
			        & =A(X).
		\end{align}
	\end{subequations}

	\spitem[For \ref{ITEMooNPSMooEHkuJw}]
	%-----------------------------------------------------------
	We use the equation \eqref{EQooFFGHooSxGHuV} under the form
	\begin{equation}		\label{EQooBFCGooUddXCF}
		f\big( \exp_1(tX) \big)=\exp_2\big( tdf_e(X) \big).
	\end{equation}
	and we use lemma \ref{LEMooGMMNooVlDkNm} for the commutator:
	\begin{subequations}
		\begin{align}
			df_e[X,Y] & =\frac{d}{dt} \left[ f\Big( \exp_1(\sqrt{t}X)\exp_1(\sqrt{t}Y)\exp_1(-\sqrt{t}X)\exp_1(-\sqrt{t}Y) \Big)  \right]_{t=0}                                      \\
			          & =\frac{d}{dt} \left[ f\big( \exp_1(\sqrt{t}X) \big)f\big( \ldots \big)\ldots  \right]_{t=0}                             & \text{\( f\) is morphism}          \\
			          & =\frac{d}{dt} \left[ \exp_2(\sqrt{t}df_e(X))\ldots  \right]_{t=0}                                                       & \text{by \eqref{EQooBFCGooUddXCF}} \\
			          & =[df_e(X),df_e(Y)].
		\end{align}
	\end{subequations}
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooPLBSooNOOmbm}
	Let \( G_1\) and \( G_2\) be smooth Lie groups. Let \(f \colon G_1\to G_2  \) be a group morphism. Then \(df_e \colon T_{e_1}G_1\to T_{e_2}G_2  \) is a morphism of Lie algebras.
\end{proposition}

\begin{proof}
	This is proposition \ref{PROPooYHTQooDSsftm}, items \ref{ITEMooGEZTooZyDSlo} with \ref{ITEMooNPSMooEHkuJw}.
\end{proof}
