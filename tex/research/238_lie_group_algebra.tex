...
\begin{proposition}[\cite{BIBooFLEXooPgvAlz}]	\label{PROPooSOOTooMrtYmP}
	Let \( G\) be a Lie group and \( H\) be a closed subgroup of \( G\). Then \( H\) with its induced topology from \( G\) is a topological manifold.
\end{proposition}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			\varphi_h\colon V & \to H             \\
			X                 & \mapsto h\exp(X).
		\end{aligned}
	\end{equation}
	Lemma \ref{LEMooOBIMooVvIDnb}\ref{ITEMooVGGXooRIXOQc} says that\( \varphi_h\) is homeomorphic.
\end{proof}
