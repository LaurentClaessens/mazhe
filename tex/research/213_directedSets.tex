...
\item[Unicity]
    ...
    \item[\( K\in \tribA_F\)]
        We already have \( \mu^+(K)<\infty\). The equality \( \mu^-(K)=\mu^+(K)\) is from the definition \eqref{EQooSIKUooJeeIqI} itself: the set \( K\) realises the supremum.
    \item[\( K\in \tribA\)]
    ...
    \item[If \( K\) is compact, \( \mu^+(K)<\infty\)]
