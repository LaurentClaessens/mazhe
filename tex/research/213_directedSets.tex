...
% Copyright (c) 2016, 2018, 2021-2022, 2026
%   Laurent Claessens
...
\begin{proof}
	...
	\end{equation}
	This is a bounded set in \( \eC\) because each element is bounded by \( \| f \|\). The closure \( \overline{ C_f(x) }\) is then compact in \( \eC\). The function \( f\) being continuous, it is uniformly continuous on \( \overline{ C_f(x) }\) (proposition \ref{PROPooBWUFooYhMlDp}.). Let \( \epsilon>0\) and an open neighbourhood \( V\) of \( e\) as in the definition of uniform continuity. If \( x,y\in V\) we have
	\begin{equation}
...
\begin{theorem}[Riesz-Markov representation theorem\cite{BIBooMGTQooQQVmPG,BIBooWEPLooDsLEvu,BIBooLREGooRSBfsj, BIBooBNSSooJVSjFi, MonCerveau}]      \label{THOooTWZWooHqGDAx}
	...
	\end{enumerate}
	The unicity part is in the following sense : if \( (X,\tribA, \mu_1)\) and \( (X,\tribA,\mu_2)\) satisfy the conditions, then \( \mu_1=\mu_2\) on \( \tribA\).
\end{theorem}
...
\begin{proof}
	...
	\begin{subproof}
		...
		From lemma \ref{LEMooKYMKooPxZjWN}, we consider an open set \( V\) such that \( K\subset V\) and \( \bar V\) is compact. Urysohn's lemma provides a function \( g\) such that \( \bar V\prec g\). If \( f\in C^0_c(X,\eR)\) satisfies \( f\prec V\), we have \( f\leq g\). Indeed if \( x\in\bar V\), \( g(x)=1\geq f(x)\) and if \( x\notin \bar V\), then \( f(x)=0\leq g(x)\).
		...
		Let \( V_1\) and \( V_2\) be open sets in \( X\). We consider \( V=V_1\cup V_2\) as topological vector space with the induced topology. Since \( V_1\cup V_2\) is open in \( X\), the induced topology makes no difficulties: the open parts of \( V_1\cup V_2\) are the same as the open parts of \( X\).

		Since \(V= V_1\cup V_2\) is open, using point \ref{ITEMooBRBTooAJBxGO} we have
		\begin{equation}
			\mu^+(V_1\cup V_2)=\mu^o(V_1\cup V_2)=\sup\{ m(f):f\prec (V_1\cup V_2) \}.
		\end{equation}
		Let \( \epsilon>0\). Urysohn theorem \ref{THOooKYYEooLFcNpg} provides a function \( g\in C^0_c(X)\) such that \( g\prec V_1\cup V_2\) and
		\begin{equation}        \label{LEMooFYAAooDaOYUy}
			\mu^o(V_1\cup V_2)-\epsilon\leq m(g).
		\end{equation}
		Let \( K=\supp(g)\); we have \( K\subset V_1\cup V_2\), and we are in the position of proposition \ref{PROPooWWBLooJAUjnw}. Let \(\phi_1,\phi_2 \colon V_1\cup V_2\to \eR  \) be such that
		\begin{enumerate}
			\item
			      \( \phi_1(x)+\phi_2(x)=1\) for every \( x\in K\),
			\item
			      \( h_i(K)\subset\mathopen[ 0,1\mathclose]\),
			\item
			      \( \supp(h_i)\subset V_1\).
		\end{enumerate}
