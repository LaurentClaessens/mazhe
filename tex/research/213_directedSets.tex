...
\item[Unicity]
    ...
    \item[If \( \mu^o(X)<\infty\), then \( V\in \tribA_F\)]
        ...
        \end{equation}
        so \( V\in\mA_F\).
    \item[\( \mu^+(V_1\cup V_2)\leq \mu^+(V_1)+\mu^/(V_2)\)]
        ...
        \end{equation}
        Let \( \epsilon>0\). There exists \( g\in C^0_C(X)\) such that \( g\prec V_1\cup V_2\) and \( \mu^o(V_1\cup V_2)-\epsilon\leq m(g)\).
    \item[\( \mu^+(\bigcup_i E_i)\leq \sum_i\mu^+(E_i)\)]
        Let \( E_i\) be parts of \( X\).
    \end{subproof}
\end{proof}
