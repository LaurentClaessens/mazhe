...
% Copyright (c) 2013,2016, 2019, 2021-2022
%   Laurent Claessens
...
\begin{definition}[Uniform continuity]      \label{DEFooUEBTooDqipcL}
    Let \( G\) be a topological group and \( (E,d)\) a metric space. A map \( f\colon G\to E\) is \defe{uniformly continuous}{uniform continuity} if for every \( \epsilon>0\), there exists a neighbourhood \( V\) of \( e\) in \( E\) such that
    \begin{equation}
...
\begin{lemma}[\cite{MonCerveau}]       \label{LEMooDZGCooIGFXnA}
    Let \( X\) be a locally compact topological space.  The part \( C^0_c(X)\) is normal\footnote{Definition \ref{DEFooTUYIooCRkgDm}.}
\end{lemma}
...
\begin{theorem}[\cite{BIBooRNREooMaKTGj}]       \label{THOooUGQCooFVySMP}
    ...
    \end{enumerate}
    For every finite open covering \( U_1,\ldots, U_n\) of \( X\), there exists a \( \mA\)-partition of unity subordinated to \( \{ U_i \}\).
\end{theorem}
