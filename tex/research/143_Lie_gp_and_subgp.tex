...
% Copyright (c) 2013-2015, 2019, 2021-2022, 2024-2026
%   Laurent Claessens
...
\begin{lemma}[\cite{MonCerveau}]	\label{LEMooLCQSooRMmcLM}
	Let \( \alpha,\beta\in\Lambda\). There exists \( k\in \eZ^2\) such that
	\begin{equation}
...
\begin{proposition}[\( 2\)-torus]       \label{PROPooSBVCooOZnszF}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	in other words, \( \varphi_{\alpha}\) is the restriction to \( B(a_{\alpha}, r_{\alpha}) \) of the canonical projection \(p \colon \eR^2\to T  \).
	...
	\begin{enumerate}
		...
		\item
		      \(T= \eR^2/\sim\) is a Lie group.
	\end{enumerate}
	The manifold \(T= \eR^2/\sim\) is the \defe{2-torus}{torus}.
\end{proposition}

\begin{proof}
	Several steps.
	\begin{subproof}
		\spitem[It is a manifold]
		%-----------------------------------------------------------
		...
		\begin{subproof}
			...
			%-----------------------------------------------------------
			This is lemma \ref{LEMooGUUCooWujZAv}.
			\spitem[Open]
			%-----------------------------------------------------------
			We have to see that
			\begin{equation}
		...
		\end{subproof}
	\end{subproof}
	Let's go for the topology. On the one hand the proposition \ref{PROPooDEVUooCATTZI} rebuilds the manifold topology from its charts with the formula
	\begin{equation}
		\tau_T=\{ S\subset T \tq \forall\alpha\in \Lambda,\varphi_{\alpha}^{-1}(S\cap M_{\alpha})\in \tau_2\}
	\end{equation}
	where \( \tau_2\) stands for the metric topology of \( \eR^2\). On the other hand, the quotient topology from \( \eR^2\) is the final topology of the projection \(p \colon \eR^2\to \eR^2/\sim  \) and is given by
	\begin{equation}
		\tau_Q=\{ S\subset \eR^2/\sim\tq p^{-1}(S)\in \tau_2 \}.
	\end{equation}

	\begin{subproof}
		\spitem[\( \tau_T\subset\tau_Q\)]
		%-----------------------------------------------------------
		Let \( S\in\tau_T\). For each \( \alpha\in \Lambda\) we have\( \varphi^{-1}_{\alpha}(S\cap M_{\alpha})\in \tau_2\) where \( M_{\alpha}=\varphi_{\alpha}(U_{\alpha})\) and \( U_{\alpha}\) is a ball in \( \eR^2\) with radius \( <1/2\). We write \( S=\bigcup_{\alpha\in\Lambda}(S\cap M_{\alpha})\) and we have
		\begin{subequations}
			\begin{align}
				p^{-1}(S) & =\bigcup_{\alpha\in \Lambda}p^{-1}(S\cap M_{\alpha})                \\
				          & =\bigcup_{\alpha\in \Lambda}\varphi_{\alpha}^{-1}(S\cap M_{\alpha})
			\end{align}
		\end{subequations}
		because \( \varphi_{\alpha}\) is the restriction of \( p\) on \( M_{\alpha}\). The last line is open in \( \eR^2\) as union of open parts of \( \eR^2\).

		\spitem[\( \tau_Q\subset\tau_T\)]
		%-----------------------------------------------------------
		Let \( S\in\tau_Q\). We write \( S=\bigcup_{\beta\in \Lambda}(S\cap M_{\beta})\) and then
		\begin{subequations}
			\begin{align}
				\varphi_{\alpha}^{-1}(S) & =\bigcup_{\beta\in \Lambda}\varphi_{\alpha}^{-1}(S\cap M_{\beta})                                                                         \\
				                         & =\bigcup_{\beta\in \Lambda}\varphi_{\alpha}^{-1}(S\cap M_{\beta}\cap M_{\alpha})                                                          \\
				                         & =\bigcup_{\beta\in \Lambda}\Big( \varphi_{\alpha}^{-1}(S)\cap\varphi_{\alpha}^{-1}(M_{\beta})\cap\varphi_{\alpha}^{-1}(M_{\alpha}) \Big).
			\end{align}
		\end{subequations}
		So for each \( \beta\) we have an intersection of three open parts of \( \eR^2\).
	\end{subproof}

	We prove that \( T\) is a Lie group.
	\begin{subproof}
		\spitem[The product]
		%-----------------------------------------------------------
		We prove that \(m \colon T\times T\to T  \) is \( C^{\infty}\). The charts for \( T\times T\) are given by the definition \ref{DEFooYOLXooDPrnHa}:
		\begin{equation}
			\begin{aligned}
				\sigma_{\alpha,\beta}\colon U_{\alpha}\times U_{\beta} & \to T\times T                                               \\
				(a,b)                                                  & \mapsto \big( \varphi_{\alpha}(a),\varphi_{\beta}(b) \big),
			\end{aligned}
		\end{equation}
		and we have to look at the maps
		\begin{equation}
			\varphi_{\mu}^{-1}\circ m\circ \sigma_{\alpha\beta} \colon U_{\alpha}\times U_{\beta}\to U_{\mu}.
		\end{equation}
		We have
		\begin{subequations}	\label{SUBEQSooQKOFooXTPNIT}
			\begin{align}
				(\varphi_{\mu}^{-1}\circ m\circ \sigma_{\alpha\beta})(a,b) & =(\varphi_{\mu}^{-1}\circ m)\big( \varphi_{\alpha}(a),\varphi_{\beta}(b) \big) \\
				                                                           & =(\varphi_{\mu}^{-1}\circ m)\big( [a],[b] \big)                                \\
				                                                           & =\varphi_{\mu}^{-1}([a+b]).
			\end{align}
		\end{subequations}
		Let \( (a_0,b_0)\in U_{\alpha}\times U_{\beta}\). Let \( x_0\in U_{\mu}\) such that \( \varphi_{\mu}(x_0)=[a_0+b_0]\). We consider \( r>0\) such that
		\begin{enumerate}
			\item
			      \( B(x_0,r)\subset U_{\mu}\),
			\item
			      The map \(p \colon B(x,r)\to B\big( [x],r \big)  \) is a bicontinuous bijection (see the proposition \ref{PROPooUENHooYsXspd}).
		\end{enumerate}
		Now we consider open parts \( V\subset U_{\alpha}\) and \( W\subset U_{\beta}\) such that \( \varphi_{\alpha}(V)\varphi_{\beta}(W)\subset B\big( [a_0,b_0],r \big)\). Since \( \varphi_{\mu}\) is the restriction of \( p\), the map
		\begin{equation}
			\varphi_{\mu} \colon B(x_0,r)\to B\big( [x_0],r \big)
		\end{equation}
		is a bicontinuous bijection. From the computation \eqref{SUBEQSooQKOFooXTPNIT} we know that there exists a map \(k \colon V\times W\to \eZ^2  \) such that
		\begin{equation}
			(\varphi_{\mu}^{-1}\circ m \circ \sigma_{\alpha\beta})(a,b)=a+b+k(a,b).
		\end{equation}
		Since \( \varphi_{\mu}^{-1}\circ m\circ \sigma_{\alpha\beta}\) is continuous on \( V\times W\) (as composition of continuous maps), the map \( k\) is constant.

		\spitem[The inverse]
		%-----------------------------------------------------------
		The inverse of \( [x]\) is \( [-x]\). There exists a map \(k \colon U_{\beta}\to \eZ^2  \) such that
		\begin{equation}
			(\varphi_{\alpha}^{-1}\circ i\circ \varphi_{\beta})(x)=\varphi_{\alpha}^{-1}([-x])=-x+k(x).
		\end{equation}
		Since the whole is continuous on a small enough neighbourhood, the map \( k\) is constant.
	\end{subproof}
\end{proof}
...
\begin{lemma}       \label{LEMooPIUFooHjyXln}
	...
	\end{equation}
	\begin{enumerate}
		\item		\label{ITEMooCDEUooAZOhkx}
		      The map \( L_h\) is a diffeomorphism.
		\item		\label{ITEMooROZYooXPFOOm}
		      The differential \( (dL_h)_g\) is a vector space isomorphism.
	\end{enumerate}
\end{lemma}

\begin{proof}
	For \ref{ITEMooCDEUooAZOhkx}, the map \( (g,h)\mapsto gh\) is differentiable. The claim here is thus the proposition \ref{PROPooBEYNooJlrFTC} (up to composition with a chart). We can now work on \ref{ITEMooROZYooXPFOOm}.

	Let \( X\in T_gG\) be such that \( (dL_h)_gX=0\). We consider a chart \(\varphi \colon U\to G  \) around \( gh\). The fact for a vector to be zero means that it evaluates to zero when applied to any function. In particular with the function \( \varphi^{-1}\) we have
	\begin{equation}
		\frac{d}{dt} \left[ \varphi^{-1}\big( X(t)h \big)  \right]_{t=0}=0.
	\end{equation}
	The derivative here is the derivative of a path in \( \eR^n\). Thus the path is constant : there exists \( a\in U \) such that \( \varphi^{-1}(X(t)h)=a\) for every \( t\). Now if \( f\) is any function we have
	\begin{equation}
		f\big( X(t) \big)=f\big( X(t)hh^{-1} \big)=f\big( \varphi(a)h^{-1} \big),
	\end{equation}
	so that \( X(f)=0\). This proves that \( X=0\).
\end{proof}
...
\begin{proof}
	If \(f \colon H_1\to H_2  \) is a continuous morphism of groups, it is \( C^k\) and then a morphism of Lie groups by proposition \ref{PROPooYHTQooDSsftm}.
\end{proof}
