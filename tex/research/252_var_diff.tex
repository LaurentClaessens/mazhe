...
\begin{propositionDef}[submanifold]      \label{DEFooLQHWooMOTgzq}
	Let \( M\) be a \( \mA\)-manifold. We consider an atlas\footnote{See \ref{PROPooNVVSooAVVLOM}.} \( \{ (\varphi_{\alpha},U_{\alpha}) \}_{\alpha\in \Lambda}\) with \( U_{\alpha}\subset \eR^n\) for every \( \alpha\in \Lambda\). Let \( Q\subset M\). For \( q\in Q\) we consider
	\begin{equation}
		\Lambda(q)=\{ \alpha\in\Lambda\tq \varphi_{\alpha}^{-1}\big( \varphi_{\alpha}(U_{\alpha})\cap Q \big)=\{ (x_1,\ldots,x_k,0,\ldots,0)\in U_{\alpha} \} \}.
	\end{equation}

	We suppose that \( \Lambda(q)\neq\emptyset\) for every \( q\in Q\). We consider the projection \(\pr_k \colon \eR^n\to \eR^k  \) and
	\begin{equation}
		\begin{aligned}
			j\colon \eR^k & \to \eR^n      \\
			x             & \mapsto (x,0).
