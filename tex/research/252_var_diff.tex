...
\begin{theorem}[Canonical immersion theorem\cite{BIBooUAYYooVSFLtg}]	\label{THOooERBHooVDgCge}
	Let \(f \colon M\to N  \) be an immersion\quext{Probably \( C^1\). We'll see when we'll have a proof.}. Then
	\begin{enumerate}
		\item
		      \( \dim(M)\leq\dim(N)\).
		\item
		      There exists charts \(\varphi_c \colon U\to M  \) and \(\psi_c \colon V\to N  \) such that
		      \begin{equation}
			      \psi^{-1}\circ f\circ\varphi=j
		      \end{equation}
		      where \( j(x)=(x,0)\) is the inclusion map.
	\end{enumerate}
	%TODOooVEGOooJHWRBf. Prouver ça.
\end{theorem}
