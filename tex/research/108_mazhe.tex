...
We are going to determine the irreducible representations of the Lie algebra \( \su(2)\) given by \eqref{EQooFJIDooRtQGjA}:
\begin{equation}
    [t_i,t_j]=\sum_k\epsilon_{ijk}t_k.
\end{equation}

Let \( (\rho,V)\) be such a representation. We write \( J_i=\rho(t_i)\); these \( J_i\) are not the algebra of momentums discussed in \cite{BIBooYTTJooYpPYLT}: there is an additional \( -i\). 
