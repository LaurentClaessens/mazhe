...
% Copyright (c) 2013-2014, 2019-2020, 2022-2026
%   Laurent Claessens
...
\begin{proposition}[Lie group structure for \( \SU(n)\)\cite{MonCerveau,BIBChatGPT}]        \label{PROPooPXRJooLNnMFn}
...
\begin{proof}
	Several parts.
	\begin{subproof}
		\spitem[For \ref{ITEMooHZQRooFGCVjP}]
		%-----------------------------------------------------------
		...
		\spitem[For \ref{ITEMooQLGKooHDIzPS}]
		%-----------------------------------------------------------
		Theorem \ref{THOooSQVCooCyEPOS} allows us to choose an atlas for \( \SU(n)\) in which it becomes analytic.

		\spitem[For \ref{ITEMooXZPDooCNfNji}]
		%-----------------------------------------------------------
		We consider the exponentials \(\exp_1 \colon V_1\to W_1  \) and \(\exp_2 \colon V_2\to W_2  \) where
		\begin{enumerate}
			\item
			      \( V_1\) is a neighbourhood of \( 0\) in \( \su(n)\).
			\item
			      \( V_2\) is a neighbourhood of \( 0\) in \( \gl(n,\eC)\).
			\item
			      \( W_1\) is a neighbourhood of \( e\) in \( \SU(n)\).
			\item
			      \( W_2\) is a neighbourhood of \( e\) in \( \GL(n,\eC)\).
		\end{enumerate}
		By theorem \ref{THOooFMFLooCnLJPr}, the exponentials are analytic diffeomorphisms. These are charts of \( \SU(n)\) and \( \GL(n,\eC)\). The map
		\begin{equation}
			f=\exp_2^{-1}\circ i\circ \exp_1
		\end{equation}
		is the inclusion \(j \colon \su(n)\to \gl(n,\eC)  \). As linear map between finite dimensional normed vector spaces, the map \( j\) is analytic. Therefore \( i\) is analytic.
	\end{subproof}
\end{proof}
