...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
\section{Representations of \( \gU(1)\)}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\begin{proposition}[\cite{MonCerveau}] \label{PROPooNTCFooEcwZwt}
    Let \( V\) be a finite dimensional complex vector space. For a basis \( B=\{ v_1,\ldots, v_n \}\) of \( V\) we define
    \begin{equation}
        d_B\big(  \sum_ka_kv_k,\sum_l b_lv_l \big)=\sqrt{ \sum_k| a_k-b_k |^2 }.
    \end{equation}
    \begin{enumerate}
        \item
            The function \( d_B\) is a distance on \( V\).
        \item
            If \( B\) and \( B'\) are basis of \( V\), the topologies induced by \( d_B\) and \( d_{B'}\) are the same.          
    \end{enumerate}
\end{proposition}
...
\begin{proposition}[Irreducible representations of \( U(1)\)]       \label{PROPooLWWEooUmqbRA}
    ...
    \begin{enumerate}
        ...
        \item       \label{ITEMooUPVQooQddQOJ}
            Every continuous\footnote{For the topology proposed in proposition \ref{PROPooNTCFooEcwZwt}.} irreducible representation of \( U(1)\) is equivalent to one of them.
    \end{enumerate}
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\( \lambda\) is continuous]
            ...
            Let \( g_k\to g\) in \( U(1)\). Since \( \rho\) is continuous we have \( \rho(g_k)\stackrel{\GL(V)}{\longrightarrow}\rho(g)\).
...
\item       \label{ITEMooYEFMooRmGmlF}
    The Lie algebra of \( \SU(n)\) is the set \( \su(n)\) of anti-hermitian matrices with vanishing trace.
    \end{enumerate}
\end{proposition}
...
\begin{proof}
    ...
    For the point \ref{ITEMooYEFMooRmGmlF}, we use the fact that
    \begin{equation}
        \Dsdd{ \det( e^{tX}) }{t}{0}=\tr(X)
    \end{equation}
    which is the corollary \ref{CORooOKKSooHrsYOs}.
\end{proof}
