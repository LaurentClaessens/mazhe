...
\begin{proof}
    ...
    \end{equation}
    That's one thing. An other one is to prove that there exists a neighborhood of \( -\mtu\) which contains no elements of the sequence \( U_k^{-1}U\). Suppose that each neighborhood of \( -\mtu\) contains one of the \( U_k^{-1}U\). At this point we have a subsequence \( (B_k)\) of \( (U_k)\) such that \( B_k^{-1}A\to-\mtu\). Since the multiplication and the inverse are continuous operations\footnote{The group \( \SU(2)\) is a Lie group.}<++>
\end{proof}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[Next steps]
            ...
            For the next steps we subdivise the cases \( \alpha=\mtu\) and \( \alpha\neq \mtu\).

        \item[With $\alpha=\mtu$]
            We subdivise the case \( \alpha=\mtu\) in several steps.
            \begin{subproof}
                \item[A limit in \(\SU(2)\)]

                \item[Setting]
                    We go back to our business. The limite \eqref{EQooCEFUooTCoczi} shows that for each \( X\in V\) we have
                    \begin{equation}
                        \varphi(\alpha_k)X\varphi(\alpha_k)\to X.
                    \end{equation}
                    We cannot conclude that \( \varphi(\alpha_k)\to \pm\mtu\) because we do no know that \( \varphi(\alpha_k)\) has a limit at all. But if we prove that this has a limit, the limit must be \( \mtu\) or \( -\mtu\).
                \item[Subsequence, limit and ok]
                    The sequence \( \varphi(\alpha_k)\) is a sequence in \( \SU(2)\) which is compact\footnote{Proposition \ref{PROPooGLPQooKOfrjl}.}. So every subsequence (which are in \( \SU(2)\) too) has convergent subsequence
            \end{subproof}
