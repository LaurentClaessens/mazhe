...
\begin{proposition}     \label{PROPooRQUZooAoZzwx}
    We still consider \( V\), the real vector space of hermitian matrices with vanishing trace. Let
    \begin{equation}
        \begin{aligned}
...
\begin{proposition}[\cite{BIBooYTTJooYpPYLT,MonCerveau}]
    Let \( \alpha_0\in \SO(3)\) and \( \mO\) be a neighborhood of \( \alpha_0\) such that \( f^{-1}(\mO)=V_1\cap V_2\) with \( V_1\cap V_2=\emptyset\)\footnote{Such choice is possible by the lemma \ref{LEMooRCSSooTvAaJY}.}.

    Let \( \alpha\colon I\to \SO(3)\) be a continuous map whose image is contained in \( \mO\). The map
    \begin{equation}
        \begin{aligned}
            \gamma_1\colon I&\to \SU(2) \\
            t&\mapsto f^{-1}\big( \alpha(t) \big)\cap V_1 
        \end{aligned}
    \end{equation}
    is continuous\footnote{The most agressive mathematician will point out that $f^{-1}\big( \alpha(t) \big)\cap V_1$ is a subset of $\SU(2)$ containing one element while $\gamma(t)$ is that element.}.
\end{proposition}
