...
\begin{definition}[\cite{BIBooUGWHooPbodCu}]        \label{DEFooYHKXooVoJalX}
	...
	\begin{equation}		\label{EQooMKVSooKzxQET}
		(dL_h)_gX_g=X_{hg}.
	\end{equation}
	...
	\end{equation}
	which means that for every \( g,h\in G\),
	\begin{equation}		\label{EQooPKVNooBFHWNM}
		(dR_h)_gY_g=Y_{gh}.
	\end{equation}
...
\begin{propositionDef}      \label{PROPooLEIAooTnnYRw}
	Let \( G\) be a Lie group and \( X,Y\in T_eG\).
	\begin{enumerate}
		\item		\label{ITEMooCFCWooOhEEYZ}
		      The vector field \( X^L\) defined by
		      \begin{equation}        \label{DEFooYPUIooAzcdjP}
			      X^L_g=(dL_g)_eX.
		      \end{equation}
		      is left-invariant.
		\item		\label{ITEMooZTLRooMkOYzH}
		      The vector field \( X^L\) defined by
		      \begin{equation}
			      Y^R_g=(dR_g)_eY.
		      \end{equation}
...
\begin{proof}
	Two parts.
	\begin{subproof}
		\spitem[For \ref{ITEMooCFCWooOhEEYZ}]
		%-----------------------------------------------------------
		We check the condition \eqref{EQooMKVSooKzxQET} :
		\begin{subequations}
			\begin{align}
				(dL_h)_gX_g^L & =(dL_h)_g(dL_g)_eX                                         \\
				              & =d(L_h\circ L_g)_eX & \text{lem. \ref{LEMooGRRAooXxDMuw}}\
				              & =(dL_{hg})_eX                                              \\
				              & =X^L_{hg}.
			\end{align}
		\end{subequations}
		\spitem[For \ref{ITEMooZTLRooMkOYzH}]
		%-----------------------------------------------------------
		We check the condition \eqref{EQooPKVNooBFHWNM}. First note that \( (R_h\circ R_g)x=R_h(xg)=xgh\), so that \( R_h\circ R_g=R_{gh}\). Now the computation is the same as in the left-invariant case.
	\end{subproof}
\end{proof}
...
\begin{proposition}[Leibniz rule\cite{MonCerveau}]     \label{PROPooAXYRooWVhXRa}
	Let \( G\) be a Lie group. We consider two smooth paths \( \gamma_i\colon I\to G\) where \( I\) is an interval containing \( 0\). We suppose that \( \gamma_i(0)=e\). We have
	\begin{equation}
		\Dsdd{ \gamma_1(t)\gamma_2(t) }{t}{0}=\gamma_1'(0)+\gamma_2'(0).
	\end{equation}
\end{proposition}

\begin{proof}
	This is a particular case of proposition \ref{PROPooWKWLooOOrOZs}. Here we have \( \gamma_i(0)=e\) and then \( dR_{\gamma_i(0)}=\id\).
\end{proof}
...
\begin{propositionDef}[\cite{MonCerveau}]	\label{PROPooBVWPooFAHakV}
	Let \( G\) be a smooth Lie group with Lie algebra \( \lG\). Let \( X\in \lG\) and \( g\in G\). There exists a unique map
	\begin{equation}
		\Phi_g^X \colon \eR\to G
	\end{equation}
	such that
	\begin{subequations}		\label{SUBEQSooDJDRooQmsjjY}
		\begin{numcases}{}
			\frac{d}{dt} \left[ \Phi_g^X(t)  \right]_{t=u}=X^L_{\Phi_g^X(u)}\\
			\Phi_g^X(0)=g.
		\end{numcases}
	\end{subequations}
	The map \(\Phi^X \colon G\times \eR\to G  \) is the \defe{flow}{flow of a vector field} of \( X\).
	%TODOooWCZDooVmbiKU. Prouver ça.
\end{propositionDef}
...
\begin{lemma}[\cite{MonCerveau}]       \label{LEMooEQFHooRjUAin}
	We have
	\begin{enumerate}
		\item		\label{ITEMooDPTQooDplUXD}
		      \( \exp(0)=e\).
...
\begin{proof}
	Recall the definition \ref{DEFooOLLZooMHRgsz} of the exponential :
	\begin{equation}
		\begin{aligned}
			\exp\colon \lG & \to G                   \\
			X              & \mapsto \Phi_e^{X^L}(1)
		\end{aligned}
	\end{equation}
	where \( \Phi^X(t,q)\) has to satisfy
	\begin{subequations}		\label{EQooUKCLooTtvIlw}
		\begin{numcases}{}
			\frac{d}{dt} \left[ \Phi^X(t,q)  \right]_{t=s}=X_{\Phi^X(s,q)}\\
			\Phi^X(0,q)=q.
		\end{numcases}
	\end{subequations}
	\begin{subproof}
		\spitem[For \ref{ITEMooDPTQooDplUXD}]
		%-----------------------------------------------------------
		We have \( \exp(0)=\Phi^0(1,e)\). Just check that the constant map \( \varphi(t,q)=q\) satisfy the system \eqref{EQooUKCLooTtvIlw} for \( X=0\).

		\spitem[For \ref{ITEMooTQHIooFLaOnX}]
		%-----------------------------------------------------------
		Here is the computation :
		\begin{subequations}
			\begin{align}
				\exp(-X)\exp(X) & = \Phi^{-X}(1,e)\Phi^X(1,e)                                                  \\
				                & = \Phi^X\big( 1,\Phi^{-X}(1,e) \big) & \text{prop. \ref{PROPooUXFQooIwimav}} \\
				                & = \Phi^X\big( 1,\Phi^X(-1,e) \big)   & \text{lem. \ref{LEMooNZCJooSltfLg}}   \\
				                & = \Phi^X(0,e)                        & \text{eq. \eqref{EQooUKIIooKLzKGx}}   \\
				                & = e.
			\end{align}
		\end{subequations}
	\end{subproof}
...
\begin{theorem}[\cite{MonCerveau,BIBooWPKPooAbnKXK}]	\label{THOooSQNPooHMJluB}
	Let \( G\) and \( H\) be Lie groups. Let \(f \colon G\to  H  \) be a continuous group morphism. Then
	\begin{enumerate}
		\item
		      \( f\) is \( C^{\infty}\).
		\item
		      The differential \(df_e \colon T_eG\to T_eH  \) is a Lie algebra morphism.
	\end{enumerate}
	%TODOooLMEXooBrdUie. Prouver ça.
	% note: la première partie est PROPooYHTQooDSsftm.
\end{theorem}
...
\begin{definition}[One-parameter subgroup\cite{BIBooNSAHooUCPAgZ}]	\label{DEFooNWPFooLTkTSA}
	A \defe{one-parameter subgroup}{one-parameter subgroup} of a Lie group \( G\) is a differentiable map \(c \colon \eR\to G  \) such that
	\begin{equation}
		c(t+s)=c(t)c(s)
	\end{equation}
	for every \( s,t\in \eR\).
...
\begin{proposition}[\cite{BIBooNSAHooUCPAgZ}]	\label{PROPooBJDCooRnfKcP}
	Let \( G\) be a Lie group and \( \lG\) be its Lie algebra. Let \(c \colon \eR\to G  \) be a one parameter subgroup\footnote{Definition \ref{DEFooNWPFooLTkTSA}.}. There exists a unique \( X\in \lG\) such that
	\begin{equation}
		c(t)=\exp(tX)
	\end{equation}
	and \( X=c'(0)\).
	%TODOooMUOUooOGfWxf. Prouver ça.
\end{proposition}
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooEWNLooUiBbtE}
	Let \( G\) be an analytic Lie group. The map
	\begin{equation}
		\begin{aligned}
			a\colon G\times \lG & \to TG           \\
			(g,X)               & \mapsto    X^L_g
		\end{aligned}
	\end{equation}
	is analytic.
...
\begin{proof}
	Let \( (g_0,X_0)\in G\times \lG\). We have to choose the charts of \( G\times \lG\) and \( TG\) in such a way to simplify the computations. We consider a map \(\varphi \colon U\to G  \) of \( G\) for a neighbourhood of \( g_0\). Let \( x_0=\varphi^{-1}(g_0)\). The chart \( \varphi\) creates the basis vectors \( \partial_i\) of \( T_eG\) with
	\begin{equation}
		\partial_i=\frac{d}{dt} \left[ g_0^{-1}\varphi(x_0+te_i)  \right]_{t=0}.
	\end{equation}
	We consider the charts
	\begin{equation}
		\begin{aligned}
			\phi\colon \eR^n & \to \lG                     \\
	...
	\begin{equation}
		\begin{aligned}
			\sigma\colon U\times \eR^n & \to TG                                       \\
			(x,v)                      & \mapsto \sum_i(dL_{\varphi(x)})_e\partial_i.
		\end{aligned}
	\end{equation}
	Now we compute
	\begin{subequations}
		\begin{align}
			a\circ(\varphi,\phi)(x,v) & =a\Big( \varphi(x),\sum_iv_i\partial_i \Big) \\
			                          & =\phi(v)^L_{\varphi(x)}                      \\
			                          & =(dL_{\varphi(x)})_e\phi(v)                  \\
			                          & =(dL_{\varphi(x)})_e\sum_iv_i\partial_i      \\
			                          & =\sum_iv_i(dL_{\varphi(x)})_e\partial_i,
		\end{align}
	\end{subequations}
	and then
	\begin{equation}
		\sigma^{-1}\circ a\circ(\varphi,\phi)(x,v)=(x,v).
	\end{equation}
	This shows that with these chars, \( a\) reads as the identity, which is analytic.
\end{proof}


\begin{proposition}[Flow is analytic\cite{BIBooMistral}]	\label{PROPooDKLRooNahSXV}
	Let \( G\) be an analytic Lie group and \( \lG\) be its Lie algebra. For each \( X\in \lG\), we consider the flow\footnote{Definition \ref{PROPooTSAYooCjvvVV}.} \( \Phi_e^{X^L}\). The map
	\begin{equation}
		\begin{aligned}
			y\colon \eR\times \lG & \to G                   \\
			(t,X)                 & \mapsto \Phi_e^{X^L}(t)
		\end{aligned}
	\end{equation}
	is analytic.
\end{proposition}
...
\begin{proof}
	We consider a chart \(\phi \colon \eR^n\to \lG  \) for the Lie algebra and a chart \(\varphi \colon U\to G  \) for \( G\). Then we consider the map
	\begin{equation}
		\tilde y=\varphi^{-1}\circ y\circ (\id,\phi).
	\end{equation}
	We prove that \( \tilde y\) is analytic. First we compute
	\begin{subequations}		\label{EQooWCUKooOpdGED}
		\begin{align}
			(\partial_t\tilde y)(u,\lambda) & =\frac{d}{dt} \left[ (\varphi^{-1}\circ y)\big( t,\phi(\lambda) \big)  \right]_{t=u}                                                          \\
			                                & =(d\varphi^{-1})_{y (u,\phi(\lambda)) }\frac{d}{dt} \left[ y\big( t,\phi(\lambda) \big)  \right]_{t=u}                                        \\
			                                & =(d\varphi^{-1})_{y (u,\phi(\lambda) )}\frac{d}{dt} \left[ \Phi_e^{\phi(\lambda)^L}(t)  \right]_{t=u}                                         \\
			                                & =(d\varphi^{-1})_{y(y,\phi(\lambda))}\phi(\lambda)^L_{\Phi_e^{\phi(\lambda)^L}(u)}                     & \text{def. \ref{PROPooBVWPooFAHakV}} \\
			                                & =(d\varphi^{-1})_{y(u,\phi(\lambda))}\phi(\lambda)^L_{y(u,\phi(\lambda))}                                                                     \\
			                                & =(d\varphi^{-1})_{\varphi\big( \tilde y(u,\lambda) \big)}a\Big( y\big( u,\phi(\lambda) \big) \Big)
		\end{align}
	\end{subequations}
	where \( a(g,X)=X^L_g\) is the analytic map of proposition \ref{PROPooEWNLooUiBbtE}. Let
	\begin{equation}
		\begin{aligned}
			f\colon U\times \eR^n & \to \eR^n                                                                  \\
			(x,\lambda)           & \mapsto (d\varphi^{-1})_{\varphi(x)}a\big( \varphi(x),\phi(\lambda) \big).
		\end{aligned}
	\end{equation}
	Since \( a\), \( \varphi\) and \( \phi\) are analytic, the map \( f\) is analytic. The equations \eqref{EQooWCUKooOpdGED} read
	\begin{equation}
		(\partial_t\tilde y)(u,\lambda)=f\big( \tilde y(u,\lambda),\lambda \big).
	\end{equation}
	From Picard–Lindelöf theorem \ref{THOooHDHCooLMkfaB} shows that \( \tilde y\) is analytic. This means that \( y\) is analytic.
\end{proof}
...
\begin{proof}
	We know from proposition \ref{PROPooDKLRooNahSXV} that the map
	\begin{equation}
		\begin{aligned}
			y\colon \eR\times \lG & \to G                   \\
			(t,X)                 & \mapsto \Phi_e^{X^L}(t)
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	is analytic as partial map. See proposition \ref{PROPooHCYXooHlUEuV}.
\end{proof}

The proof uses the analytic Picard–Lindelöf theorem \ref{THOooHDHCooLMkfaB}.
...
\begin{theorem}     \label{THOooFMFLooCnLJPr}
	...
	% voir les indices ci-dessous
\end{theorem}

The proof uses:
\begin{enumerate}
	\item
	      \(\exp \colon \lG\to G  \) is analytic, proposition \ref{PROPooTRPRooJVAYwF}.
	\item
