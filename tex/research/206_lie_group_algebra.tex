...
\begin{proposition}[Flow is analytic\cite{BIBooMistral}]	\label{PROPooDKLRooNahSXV}
	Let \( G\) be an analytic Lie group and \( \lG\) be its Lie algebra. For each \( X\in \lG\), we consider the flow\footnote{Definition \ref{DEFooRXUOooTGACym}.} \( \Phi_e^{X^L}\). The map
	\begin{equation}
		\begin{aligned}
