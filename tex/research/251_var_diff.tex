...
\begin{propositionDef}[\( T_pM\) is a vector space\cite{MonCerveau}]		\label{PROPooEJBWooSbvypo}
...
\begin{propositionDef}		\label{PROPooMLSCooEGemdf}
	...
	\begin{equation}
		(\partial_{\alpha, i})_p=\frac{d}{dt} \left[ \varphi_{\alpha}(x_0+te_i)  \right]_{t=0}
	\end{equation}
	where \( x_0=\varphi_{\alpha}^{-1}(p)\) and \( \{ e_i \}\) is the canonical basis of \( \eR^n\).
...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooAPLXooZWqXkv}
	Let \( x_0\in \eR^n\). For each \( u\in \eR^n\) we consider the path
	\begin{equation}
		\gamma_u(t)=x_0+tu
	\end{equation}
	and the tangent vector \( \omega_u=\gamma_u'(0)\).

	We have
	\begin{equation}
		\omega_u(f)=df_{x_0}(u)
	\end{equation}
	where the right hand side is the «usual» differential.

	In particular we have
	\begin{equation}
		\omega_{u+v}=\omega_u+\omega_v
	\end{equation}
	and
	\begin{equation}
		\omega_{\lambda u}=\lambda\omega_u.
	\end{equation}
\end{proposition}

\begin{proof}
	It is nothing else than lemma \ref{LemdfaSurLesPartielles} :
	\begin{equation}
		\omega_u(f)=\gamma_u'(0)(f)=\frac{d}{dt} \left[ f(x_0+tu)  \right]_{t=0}=df_{x_0}(u).
	\end{equation}
\end{proof}

\begin{proposition}[\cite{MonCerveau}]	\label{PROPooHSXOooMFuJdH}
	Let \( M\) be a \( C^k\) manifold and \(f \colon M\to \eR  \) be a \( C^k\) map. We consider a chart \(\varphi \colon U\to M  \) and \( x_0\in U\). We have
	\begin{equation}
		\frac{d}{dt} \left[ (f\circ\varphi)(x_0+tu)  \right]_{t=0}=\sum_iu_i(\partial_if).
	\end{equation}
\end{proposition}

\begin{proof}
	We use the proposition \ref{PROPooAPLXooZWqXkv} on the \( C^k\) map \(f\circ\varphi \colon U\to \eR  \) :
	\begin{subequations}
		\begin{align}
			\frac{d}{dt} \left[ (f\circ\varphi)(x_0+tu)  \right]_{t=0} & =\omega_u(f\circ\varphi)            \\
			                                                           & =\sum_i\omega_{e_i}(f\circ \varphi) \\
			                                                           & =\sum_iu_i(\partial_if)
		\end{align}
	\end{subequations}
	because \( \omega_{e_i}(f\circ\varphi)=\partial_if\).
\end{proof}


\begin{proposition}[\cite{MonCerveau}]	\label{PROPooXVKYooZnoZUp}
	Let \( M\) be a \( n\)-dimensional manifold. Let \(\varphi \colon U\to M  \) be a chart and \( x_0\in U\). Let \(h \colon \eR^n\to \eR^n  \) be a linear bijective map. We consider the chart
	\begin{equation}
		\begin{aligned}
			\phi\colon h^{-1}(U) & \to M                        \\
			x                    & \mapsto (\varphi\circ h)(x).
		\end{aligned}
	\end{equation}
	Then at \( \varphi(x_0)\) we have
	\begin{equation}
		\partial_{\phi,i}=\sum_kH_{ki}\partial_{\varphi,k}
	\end{equation}
	where \( H\) is the matrix of \( h\).
\end{proposition}

\begin{proof}
	Let \( x_0\in U\), \( p=\varphi(x_0)\) and \( y_0=h^{-1}(x_0)\). We have :
	\begin{subequations}
		\begin{align}
			(\partial_{\phi,i})_p(f) & =\frac{d}{dt} \left[ (f\circ\phi)(y_0+te_i)  \right]_{t=0}                 & \text{prop. \ref{PROPooMLSCooEGemdf}}  \\
			                         & =\frac{d}{dt} \left[ (f\circ\phi)h(y_0+te_i)  \right]_{t=0}                                                         \\
			                         & =\frac{d}{dt} \left[ (f\circ\phi)\big( h(y_0)+th(e_i) \big)  \right]_{t=0}                                          \\
			                         & =\frac{d}{dt} \left[ (f\circ\phi)\big( x_0+th(e_i) \big)  \right]_{t=0}                                             \\
			                         & =\sum_kh(e_i)_k(\partial_{\varphi,k})_p(f)                                 & \text{prop. \ref{PROPooHSXOooMFuJdH}}  \\
			                         & =\sum_kH_{ki}(\partial_{\varphi,k})_p(f)                                   & \text{prop. \ref{PROPooGXDBooHfKRrv}}.
		\end{align}
	\end{subequations}
\end{proof}
...
\begin{proof}
	Since \( X_p\neq 0\), by continuity, we consider a neighbourhood of \( p\) on which \( X\neq 0\) everywhere and a chart \(\varphi \colon B(0,\rho)\to M  \) on that neighbourhood with \( \varphi(0)=p\). Now we apply proposition \ref{PROPooXVKYooZnoZUp}, for each \( x\in B(0,\rho)\). For simplicity later, we inverse the rôle of \( h\) and \( h^{-1}\), so that we consider \(h_x \colon \eR^n\to \eR^n  \) and a chart
	\begin{equation}
		\begin{aligned}
			\phi_x\colon h_x\big( B(0,\rho) \big) & \to M                         \\
			x                                     & \mapsto (\varphi\circ h_x)(x)
		\end{aligned}
	\end{equation}
	such that, for every \( x\in B(0,\rho)\),
	\begin{equation}
		(\partial_{\varphi, i})_q=\sum_kH_{ki}(x)(\partial_{\phi_x,k})_q
	\end{equation}
	where \( q=\varphi(x)\). Now the vector field \( X\) reads
	\begin{equation}
		X_q=\sum_iX_i(x)\partial_{\varphi,i}=\sum_{ik}H_{ki}(x)X_i(x)(\partial_{\phi_x,k})_q.
	\end{equation}
	We have to find the maps \( h_x\) in such a way that
	\begin{enumerate}
		\item
		      \( \sum_{i}H_{ki}(x)X_i(x)=\delta_{k,1}\) for every \( k\),
		\item
		      \( H_{ki}(x)\) is continuous with respect to \( x\).
	\end{enumerate}
	The condition \( \sum_iH_{ki}(x)X_i(x)=\delta_{k1}\) means that for every \( x\),
	\begin{equation}
		h_x\big( X(x) \big)=e_1.
	\end{equation}
	To satisfy that condition in a continuous way, we choose vectors \( v_2,\ldots,v_n\in \eR^n\) such that
	\begin{equation}
		\{ X(0), v_2,\ldots,v_n \}
	\end{equation}
	is a basis of \( \eR^n\). By continuity of the determinant\footnote{See lemma \ref{LemJMWCooELZuho}\ref{ItemDWFLooDUePAf}.}, we have \( \rho_1\leq \rho\) such that the part
	\begin{equation}
		\{ X(x), v_2,\ldots,v_n \}
	\end{equation}
	is a basis for every \( x\in B(0,\rho_1)\). Now \( h_x\) is the map which makes
	\begin{equation}
		\begin{aligned}[]
			X(x) & \mapsto e_1 \\
			v_2  & \mapsto e_2 \\
		...
		\end{aligned}
	\end{equation}
	This is the map whose matrix is
	\begin{equation}
		\begin{pmatrix}
			X_1(x) & (v_2)_1 & \cdots & (v_n)_1 \\
			\vdots & \vdots  & \ddots & \vdots  \\
			X_n(x) & (v_2)_n & \cdots & (v_n)_n
		\end{pmatrix}^{-1},
	\end{equation}
	which is continuous with respect to \( x\). Taking \( \rho_2<\rho_1\), we consider such a \( h_x\) for every \( x\) in the compact \( \overline{B(0,\rho_2)}\). The map \( x\mapsto \| h_x \|\) is continuous on this compact. Thus we can get the minimum :
	\begin{equation}
		m=\min_{x\in\overline{B(0,\rho_2)}}\| h_x \|>0.
	\end{equation}
	Then we take \( r<m\rho_2\) in such a way to have
	\begin{equation}
		B(0,r)\subset h_x\big( B(0,\rho_2) \big)
	\end{equation}
	for every \( x\in B(0,\rho_2)\). Now the chart
	\begin{equation}
		\begin{aligned}
			\phi\colon B(0,r) & \to M                      \\
			x                 & \mapsto (\phi\circ h_x)(x)
		\end{aligned}
	\end{equation}
	makes the work.
\end{proof}
