...
\begin{proposition}[Existence of the flow\cite{MonCerveau}]	\label{PROPooPXSSooALBwSG}
	Let \( M\) be a \( \mA\)-manifold, \( q\in M\) and \( X\in \Gamma(TM)\). There exists \( \epsilon>0\), a neighbourhood \( D\) of \( p\) in \( M\) and a map
	\begin{equation}
		\Phi \colon \mathopen] -\epsilon,\epsilon\mathclose[\times D\to M
	\end{equation}
	such that
	\begin{subequations}
		\begin{numcases}{}
		...
		\end{numcases}
	\end{subequations}
	for every \( (s,q)\in \mathopen] -\epsilon,\epsilon\mathclose[\times D\).
	%TODOooSKFKooOITbwt. Prouver ça. -> PROPooPXSSooALBwSG
\end{proposition}
...
The integral curve given by point \ref{PROPooPXSSooALBwSG} is only valid on a neighbourhood given by a local chart.
...
\begin{proof}
	The existence is not an issue because it is part of the definition of a complete vector field. The trick is the unicity. We already know from propositions \ref{PROPooPXSSooALBwSG} and \ref{PROPooJACTooXBSxfE} that we have the existence and unicity on each chart. The difficulty is to glue them together.

	Let \( \gamma\colon \eR\to M\) and \( \sigma\colon \eR\to M\) be integral curves satisfying \( \gamma(0)=\sigma(0)=p\).

	Let \( K\) be a compact interval containing \( 0\) in \( \eR\). For each \( a\in K\), the proposition \ref{PROPooPXSSooALBwSG} provides us an open interval \( I_a\) containing \( a\) and such that there exists a unique \( \alpha\colon I_a\to M\) satisfying
