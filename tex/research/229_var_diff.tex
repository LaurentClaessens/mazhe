...
\begin{propositionDef}[Integral curve, flow of a vector field\cite{MonCerveau,BIBooDPYDooWgxxQH}]       \label{PROPooJACTooXBSxfE}
    Let \( X\) be a \( C^k\) vector field on \( M\) and let \( p\in M\). There exists a unique maximal interval \( I\subset \eR\) and \( C^k\) map\footnote{The map \( \Phi_p\) is \( C^k\). We are not speaking of the regularity with respect to \( p\). For more regularity, see the proposition \ref{PROPooQVQAooJVdwOa}.} \( \Phi_p\colon I\to M\) such that
    \begin{enumerate}
        \item
            \( \Phi_p(0)=p\),
        \item
