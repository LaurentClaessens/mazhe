...
\begin{propositionDef}[Integral curve, flow of a vector field\cite{MonCerveau,BIBooDPYDooWgxxQH}]       \label{PROPooJACTooXBSxfE}
    Let \( X\) be a \( C^k\) vector field on \( M\) and let \( p\in M\). There exists a unique maximal interval \( I\subset \eR\) and \( C^k\) map\footnote{The map \( \Phi_p\) is \( C^k\). We are not speaking of the regularity with respect to \( p\).} \( \Phi_p\colon I\to M\) such that
    \begin{enumerate}
        \item
            \( \Phi_p(0)=p\),
        \item
...
\begin{proof}
    ...
    \begin{subequations}        \label{SUBEQSooBLWCooATQGEn}
        ...
        \end{numcases}
    \end{subequations}
    The function \( v_k\circ\varphi\colon U\to \eR\) is a \( C^k\) function. We know from \ref{} that the system \eqref{SUBEQSooBLWCooATQGEn} has a unique solution on an interval \( I\) around \( 0\) in \( \eR\).
