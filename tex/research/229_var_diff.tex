...
\begin{proposition}[\cite{BIBooPTEZooMZrAGg}]       \label{PROPooKAXOooZCwcwi}
	...
	\begin{enumerate}
		\item
		      We suppose that \(f \colon M\to N  \) is an immersion\footnote{Definition \ref{DEFooZEWNooMVOzWI}.} and that \( g\colon S\to M\) is continuous. The map \( g\) is analytic if and only if \( f\circ g\) is analytic.
		\item
...
\end{proposition}
Note : proposition \ref{PROPooSKXGooQlZdGZ} is part of the proof.
