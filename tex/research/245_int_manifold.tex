...
\begin{propositionDef}[\cite{BIBooTGSMooYDnTxb}]		\label{PROPooIDYRooSXTBtw}
...
\begin{definition}[\cite{BIBooTGSMooYDnTxb}]		\label{DEFooAQPOooJeSRAt}
...
\begin{propositionDef}[\cite{BIBooTGSMooYDnTxb}]		\label{PROPooMKHKooUTiUjF}
	We way that two orientations \( \mA\) and \( \mB\) are \defe{equivalent}{equivalence of orientation} if \( \mA\cup\mB\) is an orientation.
...
\begin{theorem}
	A manifold is orientable if and only if it accepts a volume form\footnote{Volume form, definition \ref{DEFooOBZEooMZauZF}.}.
\end{theorem}
