...
In this whole section, we write
\begin{equation}
...
\begin{definition}[Topological manifold with boundary\cite{BIBooZBZTooYmHemH}]	\label{DEFooNETXooZaZMcm}
	...
	We define its \defe{boundary}{boundary of a manifold} the set of points of \( M\) which have no neighbourhood homeomorphic to \( \eR^n\). And we denote by \( \partial M\) the boundary of \( M\).
\end{definition}
...
\begin{definition}[Smooth manifold with boundary\cite{BIBooZBZTooYmHemH}]	\label{DEFooCATMooWBddgm}
...
\begin{proposition}[\cite{BIBooZBZTooYmHemH}]	\label{PROPooTENAooDxIAbf}
