...
\begin{definition}[Integral over a chart\cite{BIBooZBZTooYmHemH}]	\label{DEFooIKFYooKxKidt}
	Let \( M \) be a smooth oriented manifold and \( \omega\), a \( n\)-form on \( M\). Let \(\varphi_{\alpha} \colon U_{\alpha}\to M  \) be a positively oriented chart. We define
	\begin{equation}
...
\begin{propositionDef}[Integral over a manifold\cite{BIBooZBZTooYmHemH,MonCerveau}]	\label{DEFooDYDPooOWNtfD}
	Let \( M\) be an orientable manifold. Let \( \omega\) be a smooth \( n\) form on \( M\). We consider two oriented atlases with the same orientation
	\begin{subequations}
		\begin{align}
	...
	Then we have
	\begin{equation}
		\sum_{i\in I}\int_i\sigma_i\omega<\infty
	\end{equation}
	and
	\begin{equation}
...
\begin{proof}
	In several parts.
	\begin{subproof}
		\spitem[\( \omega\) supported in \( A\)]
		%-----------------------------------------------------------
		We suppose that \( \omega\) is supported in a part \( A\subset M\) such that \( A\subset\varphi_{\alpha}(U_{\alpha})\) and \( A\subset \varphi_{\beta}(U_{\beta})\). We prove that \( \int_{\alpha}\omega=\int_{\beta}\omega\). Let \( f\) and \( g\) be such that, for every \( m\in A\),
		\begin{equation}
			\omega_m=f(m)\partial_{\alpha,1}^*\wedge\ldots\wedge\partial_{\alpha,n}^*=g(m)\partial_{\beta,1}^*\wedge\ldots\wedge\partial_{\beta,n}^*.
		\end{equation}
		The result is now proposition \ref{PROPooGVQDooNwnGXs}.

		\spitem[A new altas]
		%-----------------------------------------------------------

		We consider this new atlas : \( \{ (W_{\alpha,i},\psi_{\alpha,i}) \}_{(\alpha,i)\in \Lambda\times I}\) where
		\begin{equation}
			W_{\alpha,i}=\varphi_{\alpha}^{-1}(M_{\alpha}\cap M_i)
		\end{equation}
		and
		\begin{equation}
			\begin{aligned}
				\psi_{\alpha,i}\colon W_{\alpha,i} & \to M                        \\
				x                                  & \mapsto \varphi_{\alpha}(x).
			\end{aligned}
		\end{equation}
		The remaining of the proof will be to prove that
		\begin{equation}
			\sum_{\alpha}\int_{\alpha}\rho_{\alpha}\omega=\sum_{\alpha}\sum_i\int_{\alpha,i}\rho_{\alpha}\sigma_i\omega.
		\end{equation}
		...
		%-----------------------------------------------------------
		\begin{subproof}
			\spitem[Commute an integral and a sum]		\label{SPooLLNSooWuysNN}
			%-----------------------------------------------------------
			...
			%-----------------------------------------------------------
			We take the sum over \( \alpha\) on both sides of \eqref{EQooEFMZooDbTpOl} and we take \eqref{EQooQWHLooThOixK} into account :
			\begin{equation}
				\sum_{\alpha\in\Lambda}\int_{\alpha}\rho_{\alpha}\omega=\sum_{\alpha\in\Lambda}\sum_{i\in I}\int_{\alpha}\sigma_i\rho_{\alpha}\omega= \sum_{\alpha\in\Lambda}\sum_{i\in I}\int_{i}\sigma_i\rho_{\alpha}\omega.
			\end{equation}

			Out task now is to commute the two sums. We consider the counting measure on \( I\) and \( \Lambda\) as well as the function
			\begin{equation}
				\begin{aligned}
					f\colon \Lambda\times I & \to \eR                                    \\
			...
			\begin{subequations}
				\begin{align}
					...
					\sum_{\alpha\in\Lambda}\int_{\alpha}|\rho_{\alpha}\omega| \label{SUBEQooJOXSooKjrlEO}                                            \\
					                                                                                           & <\infty	\label{SUBEQooBYFTooMBOuBl}.
				\end{align}
			\end{subequations}
			Indeed:
			\begin{itemize}
				\item
				      For \eqref{SUBEQooJOXSooKjrlEO}, we redo the computation \eqref{EQooQWHLooThOixK} in reverse.
				\item
				      For \eqref{SUBEQooBYFTooMBOuBl}, it is the assumption \eqref{EQooUOPVooUCTfof}.
			\end{itemize}
			The theorem of Fubini \ref{ThoFubinioYLtPI} commutes the sums.
			...
			\begin{subequations}
				...
				\end{align}
			\end{subequations}
			For \eqref{SUBEQooRHHNooXrXKCc}, we need to redo the work of \ref{SPooLLNSooWuysNN} with \( \alpha\) instead of \( i\).
		\end{subproof}
	\end{subproof}
\end{proof}
