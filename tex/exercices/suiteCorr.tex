...
% Copyright (c) 2011,2013-2014,2016, 2019
%   Laurent Claessens
...
et seulement si chacun des coefficients est nul, donc l'équation se
ramène au système
\begin{equation*}
\begin{cases}
b + 4a + 4 = 0\\
...
peut se recoller avec une solution nulle « sur le bord gauche de
l'intervalle » (sur le bord droit, la solution tend vers $\pm
\infty$).
...
limite demandée est donc du type indéterminé « $\frac00$ ». Afin
d'appliquer la \emph{règle de L'Hôpital} on veut d'abord vérifier que
...
\end{equation*}
et est à nouveau du type indéterminé « $\frac00$ ».
...
de point fixe, sauf pour $t = 0$. À titre d'information, on dit que
c'est une action (la variable $t$ « agit » puisqu'elle
...
\begin{enumerate}
\item Un paramétrage est donné, il reste à intégrer
\begin{equation*}
...
\end{equation*}
Cette intégrale est nulle pour les mêmes raisons que ci-dessus.

\item
Le paramétrage est donnée, et on a
\begin{equation*}
...
\end{cases}
\end{equation*}
c'est-à-dire le paramétrage est
\begin{equation*}
...
\item Le morceau de cône se accepte le paramétrage suivant en coordonnées cylindriques :
\begin{equation*}
