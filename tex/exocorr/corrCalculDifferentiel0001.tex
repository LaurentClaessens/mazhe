\begin{corrige}{CalculDifferentiel0001}
	...
	\begin{enumerate}
		...
		\item
			...
			\end{equation}
			qui dépend clairement de $k$ et donc du chemin choisi. La dérivée par rapport à $x$ n'est donc pas continue en $(0,0)$. Cependant, cette dérivée partielle existe :
		...
		\item
			...
			\end{equation}
			En ce qui concerne la dérivée par rapport à $y$ aux points autres que $(0,0)$, nous avons
			\begin{equation}
		...
		\item
			...
			\end{equation}
			En passant aux polaires, le terme central disparaît, et le terme en $b(xy)$ disparaît parce que, proche de $(0,0)$, nous avons évidemment $|xy|<|x|$ et par conséquent $| b(xy) |<b(x)$. En faisant le même genre de jeux avec l'autre terme de \eqref{EqCD15diffc}, nous devons calculer
			\begin{equation}
		...
		\item
            ...
            Un bon exemple de chemin, en polaires, est $\big( (1-t),\theta \big)$. Note : le chemin choisi doit aller vers le point à tester lorsque $t\to 0$. Ici, le chemin $\big( (1-t),\theta \big)$ tend vers le point $(1,\theta)$ qui est bien un point sur le cercle, c'est-à-dire un point à tester. Dans les coordonnées polaires, nous avons $x^2+y^2=r$, et le long de notre chemin nous avons $r=1-t$. Nous avons donc
			\begin{equation}
            ...
            \end{equation}
            En mettant au même dénominateur et en appliquant la règle de L'Hôpital,
            \begin{equation}
		...
		\item
			...
			\end{equation}
			Cette limite se fait en utilisant la règle de L'Hôpital. La dérivée du numérateur vaut
			\begin{equation}
		...
		\item
			...
			\begin{equation}
				...
				\end{cases}
			\end{equation}
			Cette fonction n'est évidemment pas continue sur les points du cercle tels que $x\neq 0$.

		\item
			Les points de raccordement entre les deux zones sont les points $(-a,a)$ et $(a,a)$ avec $a\geq 0$. Évidemment, $x^2=y^2$ en tous ces points, donc la fonction est continue. Pour les dérivées,
			\begin{equation}
				\partial_xf(x,y)=\begin{cases}
					2x	&	\text{si }| x |>y\\
					0	&	 \text{sinon.}
