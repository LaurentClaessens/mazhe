...
\begin{corrige}{0046}
	...
	\end{equation}
	Si nous prenons la limite $r\to 0$ avec $\theta$ constant, cette limite vaut zéro. Mais nous pouvons également prendre une limite sur un chemin plus subtil : un chemin en polaire. Prenons n'importe que $r(t)\to 0$, et choisissons $\theta(t)$ tel que $\cos\big( \theta(t) \big)=r(t)$. Étant donné que $r(t)<1$, un tel $\theta$ existe toujours. Une conséquence est que $\sin\big( \theta(t) \big)\to 1$. Nous avons (nous ne recopions pas toujours toutes les dépendances en $t$) :
	\begin{equation}
		\begin{aligned}[]
