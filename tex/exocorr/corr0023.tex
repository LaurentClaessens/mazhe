...
% Copyright (c) 2011,2013-2014, 2020
%   Laurent Claessens
% See the file fdl-1.3.txt for copying conditions.

\begin{corrige}{0023}

Nous allons faire un usage intensif (et sans justifications trop poussées) des choses dites autour de la règle de L'Hôpital.
...
\begin{alternative}
En utilisant la règle de L'Hôpital, nous trouvons tout de suite
\begin{equation}
...
\end{equation}
La limite est donc zéro. Notez que nous ne pouvons pas applique la règle de L'Hôpital parce que la limite $\lim_{x\to\infty}\sin(x)$ n'existe pas.

\item
Ici par contre, la règle de L'Hôpital fonctionne parce que $\lim_{x\to0}\sin(x)$ existe, et vaut zéro. Nous avons donc
\begin{equation}
...
\end{equation}
en appliquant $n$ fois la règle de L'Hôpital. Dans le cas où $n$ est un entier négatif, nous tombons sur un cas $0\cdot0=0$.
...
\item
En mettant au même dénominateur, nous trouvons un cas $\frac{ 0 }{ 0 }$ qui peut se traiter en utilisant deux fois la règle de L'Hôpital :
\begin{equation}
	\begin{aligned}[]
...
\item
Nous majorons d'abord $|\sin(x)+2|$ par $3$, et puis nous appliquons la règle de L'Hôpital :
\begin{equation}
...
\begin{alternative}
	...
	\begin{enumerate}
		...
		Un cas $\f{0}{0}$. On peut donc appliquer la règle de L'Hôpital :
		...
		Un cas $\f{\infty}{\infty}$. On peut donc appliquer la règle de L'Hôpital:
	\begin{equation}
	\begin{aligned}[]
...
\end{equation}	 
		...
		qui est un cas $\f{0}{0}$. On a donc, par L'Hôpital :
