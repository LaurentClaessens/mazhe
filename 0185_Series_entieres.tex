...
\begin{proof}
    ...
    \begin{equation}
        \exp\colon \eM(n,\eR)\to \eM(n,\eR)
    \end{equation}
    est une application \(  C^{\infty}\) vérifiant \( d\exp_0=\id\). Nous pouvons donc utiliser le théorème d'inversion locale \ref{ThoXWpzqCn} nous donne donc l'existence d'un voisinage \( V\) de \( 0\) dans \( \eM(n,\eR)\) tel que \( W=\exp(V)\) soit un ouvert de \( \GL(n,\eR)\) et que \( \exp\colon V\to W\) soit un difféomorphisme de classe \(  C^{\infty}\).
\end{proof}
<++>
...
\begin{theorem}[Dérivation sous le signe intégral\cite{MesIntProbb}]    \label{ThoMWpRKYp}
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \end{equation}
            pour tout \( x\in B(a,\omega)\) et \( \omega\in\complement A\) avec \( G\) intégrable sur \( \Omega\).
    \end{enumerate}
    Alors la fonction
    \begin{equation}
        F(x)=\int_{\Omega}f(x,\omega)d\mu(\omega)
