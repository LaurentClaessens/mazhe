...
\begin{enumerate}
    ...
    \item
        Géométrie projective. Tout le monde semble définir le birapport en identifiant \( P(\eK^2)\) à \( \hat\eK=\eK\cup\{ \infty \}\). Bien entendu, personne ne semble s'être attribué la mission d'expliciter la dépendance du birapport en le choix de l'identification. Je le fait à la définition \ref{DEFooBFSKooDwzwmO}.
    ...
    \item Géométrie projective. Il y a la formule pour le birapport sur \( P(\eK^2)=\eK\cup\{ \infty \}\) :
        \begin{equation}        \label{EQooAMKTooQeqWMJ}
            [a,b,c,x]=\frac{ (x-b)/(x-a) }{ (c-b)/(c-a) }.
        \end{equation}
        Disons que je ne pige rien à cette formule.
        \begin{enumerate}
            \item
                L'ensemble \( \eK\cup\{ \infty \}\) n'est pas une droite projective. Donc les phrases du type «l'homographie de \( d\) vers \( \eK\cup\{ \infty \}\)» ne veulent rien dire.
            \item
                Via une bijection entre \( P(\eK^2)\) et \( \eK\cup\{ \infty \}\), on peut voir ce dernier comme un espace projectif. Je voudrais une explication de \eqref{EQooAMKTooQeqWMJ} qui montre clairement comment cette identification intervient.
        \end{enumerate}
        Moi je verrais quelque chose comme : si \( \varphi_0\colon P(\eK^2)\to \eK\cup\{ \infty \}\) est l'identification usuelle \( \varphi_0[k_1,k_2]=k_1/k_2\) alors si \( A,B,C\) sont des éléments distincts de \( P(\eK^2)\) non envoyés sur \( \infty\) par \( \varphi_0\) nous avons
        \begin{equation}
            [A,B,C,X]=\frac{ (x-b)/(x-a) }{ (c-b)/(c-a) }
        \end{equation}
        où \( a=\varphi_0(A)\), \( b=\varphi_0(B)\), \( c=\varphi_0(C)\) et \( x=\varphi_0(X)\).

        Même comme ça, j'ai encore des questions : 
        \begin{enumerate}
            \item
                Quid de \( x=\infty\) ? Comment on fait \( (\infty-b)/(\infty-a)\) ?
            \item
                Si on veut parler d'homographie, il me faut un relèvement. On veut plus ou moins dire que
                \begin{equation}
                    \begin{aligned}
                        f\colon P(\eK^2)&\to P(\eK^2) \\
                        X&\mapsto \varphi_0^{-1}\left( \frac{ (x-b)/(x-a) }{ (c-b)/(c-a) } \right) 
                    \end{aligned}
                \end{equation}
                est une homographie. Quel est son relèvement en un isomorphisme d'espace vectoriel \( \bar f\colon \eK^2\to \eK^2\) ?
        \end{enumerate}
\end{enumerate}
