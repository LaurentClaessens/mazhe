...
\begin{center}
   \input{pictures_tex/Fig_ZOCNoowrfvQXsr.pstricks}
\end{center}
...
\begin{equation}
    \begin{aligned}[]
    A(\varphi)=\big\{  \varphi_1\colon P(\eK^2)&\to \hat\eK \tq \varphi_a^{-1}\circ\varphi\phi\\
    &\text{ soit une homographie pour une homographie  } \phi\colon P(\eK^2)  \to P(\eK^2) \big\}
    \end{aligned}
\end{equation}
\end{definition}

\begin{lemma}
    Si \( \varphi_a\in A(\varphi)\) alors pour tout homographie \( \phi'\colon P(E)\to P(\eK^2)\) l'application \( \varphi_a^{-1}\circ\varphi\circ\phi'\) est une homographie.
\end{lemma}

\begin{proof}
    Nous savons que \( \varphi_a\circ\varphi\circ\phi\) est une homographie pour une certaine homographie \( \phi\colon P(\eK^2)\to P(\eK^2)\).

    Nous supposons dans un premier temps que \( \phi'\colon P(\eK^2)\to P(\eK^2)\) soit une homographie. Alors
    \begin{equation}        \label{EQooMVPUooIRzKSu}
        \varphi_a^{-1}\circ\varphi\circ\phi'=\varphi_a^{-1}\circ\varphi\circ\phi\circ\phi^{-1}\circ\phi'.
    \end{equation}
    Vu que la composition de deux homographies est une homographie (proposition \ref{PROPooGVYXooDIiIbW}\ref{ITEMooIZAPooNxEigb}), l'application \( \phi^{-1}\circ\phi'\) est une homographie. De plus par hypothèse \( \varphi_a\circ\varphi\circ\phi\) est une homographie. Donc, toujours par composition, l'application \eqref{EQooMVPUooIRzKSu} est une homographie.

    Si maintenant \( E\) est un espace vectoriel de dimension \( 2\) et si \( \phi'\colon P(E)\to P(\eK^2)\) est une homographie, nous pouvons écrire bêtement(?) sous la forme \( \phi'=\id\circ\phi'\) avec l'identité de \( P(\eK^2)\) qui est une homographie. Alors
    \begin{equation}
        \varphi_a^{-1}\circ\varphi\circ\phi'=\underbrace{\varphi_a^{-1}\circ\varphi\circ\id}_{\text{homographie}}\circ\phi'.
    \end{equation}
    Le tout est une homographie.
\end{proof}
...
\begin{remark}  
    La majorité des sources ne parlent pas de la dependence du birapport en le choix de \( \varphi\) parce que tout le monde ne semble ne considérer que \( \varphi=\varphi_0\) définit en \eqref{EQooSIJDooTHPYMb}. Il est cependant naturel de se demander si la définition dépend effectivement du choix de \( \varphi\). La réponse est oui : ça dépend du choix.
\end{remark}
...
\begin{proposition}     \label{PROPooTFMQooIOQGvs}
    Soit une bijection \( \varphi\colon P(\eK^2)\to \hat\eK\). Si \( \varphi_a\in A(\varphi)\) alors les birapports construits sur \( \varphi\) et \( \varphi_a\) coïncident.
\end{proposition}
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    Il est facile de voir que \( \phi_a=\varphi_a^{-1}\circ\varphi\phi\). En effet, la composée proposée est une homographie parce que \( \varphi_a\in A(\varphi)\). Et de plus
    \begin{subequations}
        \begin{align}
...
\begin{proposition}[\cite{ooDTHEooBAnkGP}]
