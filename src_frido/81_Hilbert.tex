...
\begin{proposition}[\cite{MonCerveau,ooBBNWooHJPWci}]
    Toute partie orthonormée d'un espace de Hilbert séparable se prolonge en une base orthonormée.
\end{proposition}

\begin{proof}
    Soit une partie orthonormée \( U=\{ u_i \}_{i\in I}\) de l'espace de Hilbert \( \hH\). Nous considérons une base orthonormée \( \{ b_{\alpha} \}_{\alpha\in A}\) de \( \hH\) (proposition \ref{PROPooDRFBooWTcunC}). L'idée de la preuve sera de «supprimer» des \( b_{\alpha}\) les composantes déjà engendrées par les \( u_i\) par une projection orthogonale. Ensuite, ce qui reste sera une base avec les \( u_i\).

    La partie \( V=\overline{\Span\{ u_i \}_{i\in I}}\) est un sous-espace vectoriel fermé, convexe de \( \hH\).
    \begin{subproof}
        ...
        \item[Convexe] Un espace vectoriel est toujours convexe.
    \end{subproof}
    En utilisant le théorème de projection \ref{ThoProjOrthuzcYkz} nous décomposons
    \begin{equation}
        b_{\alpha}=v_{\alpha}+b'_{\alpha}
    \end{equation}
    avec \( v_{\alpha}\in V\) et \( b'_{\alpha}\perp V\). Autrement dit, \( v_{\alpha}=\pr_V(b_{\alpha})\).
    
    \( b'_{\alpha}=\pr_V(b_{\alpha})\). Nous en extrayons ceux qui sont non nuls et nous les renormalisons à \( 1\) : \( A'=\{ \alpha\in A \tq b'_{\alpha}\neq 0 \}\) et
    \begin{equation}
        C=\big\{ c_{\alpha}=\frac{ b'_{\alpha} }{ \| b'_{\alpha} \| }\big\}_{\alpha\in A'}.
    \end{equation}
    Nous prétendons que \( U\cup C\) est une base orthonormée de \( \hH\). D'abord vu que les \( b'_{\alpha}\) sont des projections sur \( V\) nous pouvons écrire
    \begin{equation}
        b_{\alpha}=b'_{\alpha}+v_{\alpha}
    \end{equation}
    avec \( v_{\alpha}\in V\) et \( b'_{\alpha}\perp V\). À redéfinition près de \( v_{\alpha}\) par un facteur \( \| b'_{\alpha} \| \) nous avons
    \begin{equation}
        c_{\alpha}=
    \end{equation}
    <++>
...
\begin{remark}
    La fermeture dans la définition de \( V\) est indispensable pour que \( V\) soit fermé. En effet si \( I\) est infini, nous pourrions avoir le vecteur \( \sum_{n\in \eN}un/2^n\) qui n'est pas dans \( \Span\{ u_i \}\) mais bien dans la fermeture.
\end{remark}
<++>
...
\begin{definition}      \label{DEFooUNOKooCitMjL}
    Une forme bilinéaire \( a\colon V\times V\to \eR\) sur un espace vectoriel normé \( V\) est \defe{coercitive}{coercion} s'il existe \( \alpha>0\) tel que \(  a(u,v)\geq \alpha\| u \|^2 \) pour tout \( u\in V\).
\end{definition}
