...
\begin{example}     \label{ExVrasMaxLp}
    ...
    \newcommand{\CaptionFigMaxVraissLp}{La fonction de vraissemblance de l'exemple \ref{ExVrasMaxLp}.}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Cela est une variable aléatoire de distribution \( \dB(1,o)\) où \( p\) est inconnu. Ici, \( \Theta=\mathopen[ 0 , 1 \mathclose]\) est l'ensemble des \( p\) possibles.
...
\begin{subequations}
    \begin{align}
        1-\alpha&=P\big( p\in\mathopen[ \bar X_n-\epsilon , \bar X_n+\epsilon \mathclose] \big)\\
...
L'astuce est évidemment de remplacer \( p\) par \( \bar X_n\), mais il faut le justifier. Une première façon de le justifier est de remarquer que l'événement 
\begin{equation}
