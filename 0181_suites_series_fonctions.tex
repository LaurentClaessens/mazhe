...
\begin{proof}
    Soit \( \psi=\phi^{-1}\); c'est une application continue et donc borélienne\footnote{Par l 31618 6603 }. Étant donné que \( \phi\) et \( \psi\) sont des bijections, nous avons \( \psi(\phi(E))=E\) et donc \( \phi(E)=\psi^{-1}(E)\). L'ensemble \( \phi(E)\) est donc image inverse d'un borélien par une application continue (et donc borélienne).
\end{proof}
