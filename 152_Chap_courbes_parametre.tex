...
\begin{corollary}
    Soit une courbe convexe \( \gamma\) fermée de classe \( C^1\) de graphe \(\Gamma\). Si une droite coupe \( \Gamma\) en trois points alors \( \Gamma\) est en partie un segment de droite.
\end{corollary}

\begin{proof}
    Supposons que la droite \( \ell\) coupe \( \Gamma\) en trois points \( p,q,r\) (dans cet ordre). Si la tangente à \( \Gamma\) au point \( q\) n'est pas cette droite, alors les points \( p\) et \( r\) sont de part et d'autre de \( \ell_q\), ce qui serait une contradiction.

    Donc \( \ell_q\) est la droite passant par \( p\), \( q\) et \( r\).
\end{proof}
<++>
...
\begin{theorem}[Théorème des quatre sommets\cite{KXjFWKA,ooIEJXooIYpBbd}]       \label{THOooFRBBooWKZcfY}
    ...
    \begin{enumerate}
        ...
        \item
            la courbe est convexe\footnote{Par la proposition \ref{PROPooWXUKooPOtPdj} nous pouvons aussi bien demande à la courbure d'être toujours strictement positive, comme le fait \cite{KXjFWKA}.}
    \end{enumerate}
...
L'exemple de l'ellipse montre qu'il n'y a pas lieux de chercher d'autres extrema de \( \kappa\) à part les \( 4\) déjà trouvés.
...
\begin{example}
    ...
    La courbure est
    \begin{equation}
        \kappa(t)=\frac{ \gamma''\cdot J\gamma' }{ \| \gamma' \|^3 }=\frac{-1}{ [a^2\sin^2(t)+b^2\cos^2(t)]^{3/2} }\begin{pmatrix}
            a\cos(t)    \\ 
            b\sin(t)    
        \end{pmatrix}\cdot\begin{pmatrix}
            -b\cos(t)    \\ 
            -a\sin(t)    
        \end{pmatrix}=\frac{ ab }{  [a^2\sin^2(t)+b^2\cos^2(t)]^{3/2}  }.
    \end{equation}
