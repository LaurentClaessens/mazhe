...
\begin{lemmaDef}[\cite{ooIEJXooIYpBbd}]     \label{LEMDEFooLPWJooAnWZjb}
    ...
    \begin{equation}
        \frac{ \gamma'(t) }{ \| \gamma'(t) \| }=\big( \cos\big( \theta(t) \big),\sin\big( \theta(t) \big) \big)
    \end{equation}
    pour tout \( t\in I\).
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            \frac{ \gamma'(t) }{ \| \gamma'(t) \| }+\gamma'(t)\frac{ d  }{ dt }\left( \frac{1}{ \| \gamma'(t) \| } \right)&=\theta'(t)\big( -\sin\big( \theta(t) \big),\cos\big( \theta(t) \big) \big)\\
        \end{align}
    \end{subequations}
    <++>
\end{proof}
