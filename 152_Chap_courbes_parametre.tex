...
\begin{remark}
   Bien que cela puisse paraître évident, nous ne démontrerons que dans la proposition \ref{PROPooOORPooCXrIQi} que \( D\) est l'enveloppe convexe de \( \Gamma\).
\end{remark}

\begin{corollary}       \label{CORooSXDGooJEmVcf}
    Si \( p\in\Int(D)\) alors toute droite passant par \( p\) coupe \( \Gamma\) en exactement \( 2\) points.
\end{corollary}

\begin{proof}
    Vu que \( D\) est borné, toute droite coupe \( \partial D\) en au moins deux points (un dans chaque sens, et en utilisant le lemme de passage de douane \ref{LEMooLKWEooItGnkP}).
...
\begin{proposition}     \label{PROPooOORPooCXrIQi}
    L'ensemble \( D\) définit en \eqref{EQooDYFTooCHRbsD} est l'enveloppe convexe\footnote{Définition \ref{DefNLYYooXUHFUY}.} de \( \Gamma\).
\end{proposition}
...
\begin{corollary}
    Soit une courbe convexe fermée simple \( \gamma\) de classe \( C^2\). L'ensemble \( D\) définit en \eqref{EQooDYFTooCHRbsD} est compact.
\end{corollary}

\begin{proof}
    Nous savons déjà que \( D\) est fermé. Il nous suffit donc de prouver qu'il est borné (théorème de Borel-Lebesgue \ref{ThoXTEooxFmdI}). Nous considérons deux droites perpendiculaires et les \( 4\) tangentes correspondantes par la proposition \ref{PROPooKHUQooIOUxFw}. Vu que \( \Gamma\) est d'un seul côté de chacune de ces tangentes, elle est contenue dans le rectangle délimité par ces \( 4\) droites.
\end{proof}

\begin{corollary}
    Si \( p\in \Int(D)\) alors toute droite passant par \( p\) coupe \( \Gamma\) en exactement deux points.
\end{corollary}
<++>
...
\begin{proposition}[\cite{ooYGXBooTzOAtL}]
...
\begin{theorem}[Théorème des quatre sommets\cite{KXjFWKA}]
    Soit un arc paramétrique \( \gamma\colon \eR\to \eR^2\) de classe \( C^3\) et \( T\)-périodique tel que
    \begin{enumerate}
        \item
            \( \gamma\) est fermé et simple,
        \item
            la paramétrisation est normale
        \item
            la courbure signée\footnote{Définition \ref{DEFooJFWEooXcIVUs}.} est toujours strictement positive.
    \end{enumerate}
    Alors \( \gamma\) possède au moins \( 4\) points critiques sur chaque période.
\end{theorem}

\begin{proof}
   Étant donné que \( \gamma\) est de classe \( C^3\), le vecteur tangent unitaire \( \tau\colon \eR\to \eR^2\) et son vecteur normal unitaire \( \nu\colon \eR\to \eR^2\) sont de classe \( C^2\) et la courbure signée \( \kappa\colon \eR\to \eR\) est de classe \( C^1\). Toutes sont \( T\)-périodiques. En particulier
   \begin{equation}
       \int_0^T\kappa'(s)ds=\kappa(T)-\kappa(0)=0
   \end{equation}
   L'intégrale suivante est par partie, en utilisant la périodicité :
   \begin{subequations}
       \begin{align}
            \int_0^T\kappa'(s)\gamma(s)ds&=\underbrace{\big[ \kappa(s)\gamma(s) \big]_{s=0}^{s=T}}_{=0}-\int_0^T\kappa(s)\gamma'(s)ds\\
            &=klm
       \end{align}
   \end{subequations}
   <++>
