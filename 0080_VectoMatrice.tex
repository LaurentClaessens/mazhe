...
\begin{proof}
    ...
    \end{equation}
    Comptons les degrés. Pour donner une idée de ce calcul de degré, voici comment se présente, au niveau des dimensions, la première ligne du déterminant :
    \begin{equation}    \label{EqJCaATOH}
    \xymatrix{%
        o\ar@{<->}[rrrrr]^{p+q}&&&&&\\
         a_p   & \ldots & a_0 & 0&\ldots&0 \\
         \ar@{<->}[rr]_{p+1}&&& \ar@{<->}[rr]_{q-1}  &&
       }
    \end{equation}
    ...
    Nous pouvons donc calculer la valeur de \( T\) en choisissant un cas particulier. Avec \( P(X)=X^p\) et \( Q(X)=X^q\), il est vite vu que \( R(P,Q)=1\) et donc que \( T=1\).
