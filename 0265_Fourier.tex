...
\begin{proposition}
    Si \( K\) est compact dans \( \Omega\), l'espace \( \swD(K)\) est complet.
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation}
        \| \varphi_k-\varphi_l \|\leq \epsilon,
    \end{equation}
    ce qui montre que pour tout \( x\in K\) la suite \( (\varphi_k(x))\) est de Cauchy dans \( \eR\) et converge. Nous avons donc une limite ponctuelle \( \varphi_k\to\varphi\).
    
    Montrons que \( \varphi\) est continue. Soit \( (x_n)\) une suite dans \( K\) convergeant vers \( x\in K\). Cette suite est de Cauchy (dans \( \eR^d\)) et pour tout \( k\) et \( n\) nous avons la majoration
    \begin{equation}
        \big| f(x_n)-f(x) \big|\leq \big| f(x_n)-f_k(x_n) \big|+\big| f_k(x_n)-f_k(x) \big|+\big| f_k(x)-f(x) \big|.
    \end{equation}
    Nous commençons par choisir 
    Le second terme est rendu plus petit que \( \epsilon\) en choisissant \( n\) assez grand et en invoquant la continuité de \( f_k\)<++> Le premier terme peut être rendu plus petit que \( \epsilon\) en choisissant \( k\) assez grand et en invoquant la convergence ponctuelle
\end{proof}
<++>
...
\begin{proof}
    ...
    \end{equation}
    a un sens et est finie. L'application \( T\colon \swD(I\times \Omega)\to \eC\) ainsi définie est linéaire. Il reste à voir qu'elle est continue. Soit donnée une application \( \varphi\in\swD(K)\). L'application \( t\mapsto T_t(\varphi)\) est continue et il existe donc \( C_{\varphi}>0\) tel que
    \begin{equation}
    ...
    Nous utilisons maintenant le théorème de Banach-Steinhaus \ref{ThoNBrmGIg} avec \( E=\swD(K)\) et \( F=\eC\); il n'existe pas de \( \varphi\in\swD(\Omega)\) tel que \( \sup_{t\in I}\| T_t(\varphi) \|=\infty\). Donc l'ensemble \( \{ \| T_t \| \}_{t\in I}\) est borné. Ici \( \| T_t \|\) est la norme de \( T_t\) en tant qu'opérateur linéaire de \( \swD(\Omega)\) vers \( \eC\).
    %NON, c'est pas ça.
