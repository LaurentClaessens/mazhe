...
\begin{proposition}[\cite{GQYneyj}]
    Soit \( T\in C^k\big( I,\swD'(\Omega) \big)\) et \( 0\leq l\leq k\). Pour tout \( t\in I\) il existe une distribution \( T^{(l)}_t\) telle que
    \begin{equation}
        t\mapsto T_t^{(l)}\in C^{k-l}\big( I,\swD'(\Omega) \big)
    \end{equation}
    et telle que
    \begin{equation}
        \left( \frac{ d^l  }{ dt }T_t(\varphi) \right)(t_0)=T^{(l)}_{t_0}(\varphi)
    \end{equation}
    pour tout \( t_0\in I\) et \( \varphi\in\swD'(\Omega)\).
\end{proposition}

\begin{proof}
    Pour \( k=0\) il suffit de prendre \( T^{(0)}_t=T_t\) et c'est bon. Pour le cas \( k=1\) et \( l=0\) c'est encore \( T_t^{(0)}=T_t\) qui fonctionne.

    Le premier cas non trivial à traiter est \( k=1\) et \( l=1\). Nous considérons \( t_0\in I\) et une suite \( \epsilon_j\to 0\) dans \( \eR\). Ensuite nous posons
    \begin{equation}
        U_j=\frac{1}{ \epsilon_j }\big( T_{t_0+\epsilon_j}-T_{t_0} \big).
    \end{equation}
    Par définition si \( \varphi\in \swD(\eR^d)\) nous avons
    \begin{equation}    \label{EqCTuAfXe}
        U_j(\varphi)=\frac{ T_{t_0+\epsilon_j}(\varphi)-T_{t_0}(\varphi) }{ \epsilon_j }.
    \end{equation}
    Vu que \( (T_t)\in C^k(I,\swD'(\Omega))\), l'application \( t\mapsto T_t(\varphi)\) est de classe \( C^k\) et en particulier l'expression \eqref{EqCTuAfXe} a une limite lorsque \( j\to \infty\); cette limite vaut
    \begin{equation}
        \lim_{j\to \infty} U_j(\varphi)=\Dsdd{ T_t(\varphi) }{t}{t_0}.
    \end{equation}
    Nous allons utiliser le corollaire \ref{CorPGwLluz}.
    %2202023453
\end{proof}
<++>
