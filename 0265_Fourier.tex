...
\begin{proof}
    ...
    Nous utilisons maintenant le théorème de Banach-Steinhaus \ref{ThoPFBMHBN} avec \( E=\swD(K)\) et \( F=\eC\); il n'existe pas de \( \varphi\in\swD(\Omega)\) tel que \( \sup_{t\in I}\| T_t(\varphi) \|=\infty\). Donc l'ensemble \( \{ \| T_t \| \}_{t\in I}\) est borné. Ici \( \| T_t \|\) est la norme de \( T_t\) en tant qu'opérateur linéaire de \( \swD(\Omega)\) vers \( \eC\).
