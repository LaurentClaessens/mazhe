...
\begin{proof}
    ...
    \end{equation}
    Le corollaire \ref{CorPGwLluz} nous dit que si nous posons 
    \begin{equation}
        T_{t_0}^{(1)}(\varphi)=\Dsdd{ T_t(\varphi) }{t}{t_0},
    \end{equation}
    alors nous avons \( U_j(\varphi)\to T_{t_0}^{(1)}(\varphi)\) et \( T_{t_0}^{(1)}\) est une distribution.

    Nous devons encore voir que \( t\mapsto T^{(1)}_t\) est une application \( C^0\big( I,\swD'(\Omega) \big)\).
    211021642
