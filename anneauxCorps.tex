...
\end{equation}
C'est l'ensemble des polynômes qui annulent \( f\) en \( x\). L'idéal \( I_x\) n'est pas réduit à \( \{ 0 \}\) parce que e polynôme minimal de \( f\) fait partie de \( I_x\). Le théorème \ref{ThoCCHkoU} nous assure l'existence d'un unique générateur unitaire dans \( I_x\). Ce générateur est le \defe{polynôme minimal!ponctuel}{polynôme minimal ponctuel} de \( f\) en \( x\).
