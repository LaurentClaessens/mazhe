...
\begin{remark}
    Ce théorème chinois est bien une généralisation du lemme chinois \ref{LemCtUeGA}. En effet, l'élément \( x\) dont il est question est solution du problème \( x=x_i\mod I_i\). L'hypothèse \( I_i+I_j=A\) n'est pas non plus nouvelle étant donné que si \( p\) et \( q\) sont des entiers premiers entre eux nous avons \( p\eZ+q\eZ=\eZ\) par le corollaire \ref{CorgEMtLj}.
\end{remark}
...
\begin{lemma}[Lemme chinois \cite{CongrDuchSyl}]        \label{LemCtUeGA}
    Soient \( p,q\) deux entiers premiers entre eux. Soient \( a_1,a_2\in \eZ\). Les solutions du système
...
\begin{equation}
    I_x=\{ P\in \eK[X]\tq P(f)x=0 \}.
\end{equation}
C'est l'ensemble des polynômes qui annulent \( f\) en \( x\). L'idéal \( I_x\) n'est pas réduit à \( \{ 0 \}\) parce que le polynôme minimal de \( f\) fait partie de \( I_x\). Le théorème \ref{ThoCCHkoU} nous assure l'existence d'un unique générateur unitaire dans \( I_x\). Ce générateur est le \defe{polynôme minimal ponctuel}{polynôme!minimal!ponctuel} de \( f\) en \( x\).
...
\begin{lemma}       \label{Lembcerei}
    Soit \( p\) un nombre premier et \( P\), un polynôme de degré \( n\). Si \( \alpha\in \eF_p[X]/P\) est une racine primitive\index{racine!primitive}\index{primitif!racine}\footnote{C'est à dire que les puissances de \( \alpha\) parcourent tout le groupe multiplicatif \( (\eF_p[X]/P)^*\).} de \( P\) alors les autres racines de \(P\) sont également primitives.
\end{lemma}
