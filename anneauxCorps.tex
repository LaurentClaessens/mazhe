...
\begin{definition}
    Soit \( \eA\) un anneau et \( a,b\in \eA\). Nous disons que \( d\) est un \( \pdcg\)\index{pgcd} de \( a\) et \( b\) si tout diviseur commun de \( a\) et \( b\) divise \( d\).
\end{definition}
...
\begin{proof}
    ...
    \item[\ref{ItemdqPFUiiJ}\( \Rightarrow\)\ref{ItemdqPFUiii}] Soit \( D\) un \( \pgcd\) de \( P\) et \( P'\). D'après le théorème de Bézout il existe \( A,B\in \eK[X]\) tels que 
        \begin{equation}
            AP+BP'=D.
        \end{equation}
        <++>
...
\begin{definition}
    ...
    \begin{enumerate}
        \item
            On dit que \( a\in \eL\) est \defe{séparable}{séparable!élément d'une extension} sur \( \eK\) si son polynôme minimal dans \( \eK[X]\) est séparable sur \( \eK\).
        \item
