...
Le théorème de Bezout aura lieu dans les anneaux principaux, corollaire \ref{CorimHyXy}.
...
Nous disons que deux éléments d'un anneau principal sont \defe{premiers entre eux}{premiers!éléments d'un anneau principal entre eux} si leur PGCD est \( 1\).

\begin{corollary}[Théorème de Bezout\cite{XPXxPl}]\index{Bezout!anneau principal}\label{CorimHyXy}
...
\begin{proof}
    Si le corps \( \eK\) est fini, alors \( \eL\) est également fini.
\end{proof}
<++>
...
\begin{proof}
    En vertu de la proposition \ref{PropUmxJVw} et de l'exemple \ref{ExvQTyBl}, \( \eK\) est une extension séparable de \( \eQ\).
    <++>
\end{proof}
<++>
...
\begin{example}
    Le résultat \ref{rqrNyg} ne tient pas pour les corps non commutatifs. Par exemple si nous considérons pour \( \eK\) le corps des quaternions\index{quaternion} et comme \( G\) le groupe à \( 8\) éléments \( \{ \pm 1,\pm i,\pm j,\pm k \}\). Ce dernier groupe n'est pas cyclique alors qu'il est un groupe fini dans \( \eK^*\).
\end{example}
<++>
