...
\begin{theorem}[Théorème de l'élément primitif]\index{théorème!élément primitif}    \label{ThoqSludu}
    ...
    \end{enumerate}
    Soit \( \alpha\) et \( P\) choisis pour avoir les propriétés citées plus haut. Ils ont les propriétés suivantes.
    \begin{enumerate}
        \item
...
\begin{definition}
    Soit \( p\), un nombre premier et \( P\) un polynôme unitaire irréductible de degré $n$ dans \( \eF_p[X]\). Nous disons que \( P\) est \defe{primitif}{primitif} si les racines de \( P\) sont d'ordre \( p^n-1\) dans \( \eF_p[X]/P\).
\end{definition}

Nous serions donc intéressé à construire $\eF_{q}$ comme quotient de \( \eF_p[X]\) par un polynôme primitif.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item
            Le polynôme \( P\) possède \( q\) racines dans \( \eK\); il est scindé et toutes ses racines sont également racines de \( X^q-X\).
        \item
