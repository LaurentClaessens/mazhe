...
\begin{theorem}[Conséquence du \wikipedia{fr}{Lemme_de_Gauss_(polynômes)}{lemme de Gauss}]\label{ThofiIpXg} \index{primitif!polynôme}
...
Ce passage sur RSA provient de la \wikipedia{fr}{Rivest_Shamir_Adleman}{la page Wikipédia}.
...
\begin{enumerate}
    \item
        Bob choisit deux nombres premiers \( p,q\)
    \item
        On pose \( n=pq\) 
    \item
...
\end{enumerate}
Maintenant la paire est : clef publique \( (n,e)\) et clef privée \( (n,d)\).
...
\end{itemize}
Dans les deux cas nous avons \eqref{EqreeHgn}. Le nombre \( M^{1+k\varphi(n)}-M\) est donc à la fois multiple de \( p\) et de \( q\). Posons
\begin{equation}
    M^{1+k\varphi(n)}-M=ap=bq.
\end{equation}
Dans ce cas \( p\) divise \( bq\), mais \( q\) est premier avec \( p\), donc le lemme de Gauss \ref{LemSdnZNX} nous enseigne que \( p\) divise \( b\). Donc
\begin{equation}
    M^{1+k\varphi(n)}-M
\end{equation}
...
\end{equation}
Ces éléments sont les \defe{racines primitives}{racine!primitive de l'unité}\footnote{parce qu'en prenant les puissances succéssives de l'une d'entre elles, nous retrouvons toutes les racines de l'unité, voir aussi la définition \ref{DefnPNCFO}.} de l'unité dans \( \eC\). Nous avons 
\begin{equation}
...
La preuve donnée ici provient de l'article \wikipedia{fr}{Polynôme_cyclotomique}{polynôme cyclotomique} de wikipédia.
\begin{proof}
...
\begin{lemma}[Lemme de Gauss]   \index{lemme!Gauss!entiers}\index{lemme!Gauss!entiers}      \label{LemSdnZNX}
