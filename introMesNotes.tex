...
Voici une petite liste de questions que je me pose ou de choses dont je ne suis pas sûr dans le texte. Si vous avez un avis ou une réponse à un des points, merci de vous faire connaître :)
\begin{enumerate}
    \item
        Si \( X\) est une variable aléatoire et \( A\) un événement, est-ce qu'il est vrai que \( E\big( P(A|X) \big)=P(A)\) ? Démonstration ?
    \item
        La résolution de l'équation diophantienne \( x^2+2=y^3\) que je donne dans l'exemple \ref{ExmuQisZU} est-elle correcte ?
    \item
