...
Une démonstration plus simple via la densité des diagonalisables est donnée en théorème \ref{ThoCalYWLbJQ}.
\begin{proof}
...
\begin{proof}
    \begin{subproof}
        \item[\ref{ItemZKDMooOrTHkwii}\( \Rightarrow\)\ref{ItemZKDMooOrTHkwi}]
            ...
            \end{equation}
            où les \( \lambda_i\) sont les valeurs propres de \( u\). Le théorème de Cayley-Hamilton \ref{ThoHZTooWDjTYI} dit que \( \chi_u(u)=0\), ce qui permet d'utiliser le théorème de décomposition des noyaux\ref{ThoDecompNoyayzzMWod} :
            \begin{equation}
...
\begin{remark}
    Si \( \eK\) est algébriquement clos (comme \( \eC\) par exemple), alors tous les polynômes sont scindés et toutes les matrices sont trigonalisables. Un exemple un peu simple de cela est la matrice
    \begin{equation}
        u=\begin{pmatrix}
            0    &   -1    \\ 
            1    &   0    
...
\begin{theorem}     \label{ThoDigLEQEXR}
    ...
    \end{enumerate}
\end{theorem}
Dans le cas de trigonalisation nous avons la proposition \ref{PropDSEHooVgipib}.
...
Le lemme suivant est utile en soi et dit que toute matrice complexe est trigonalisable.
\begin{lemma}[Lemme de Schur complexe\cite{NormHKNPKRqV}]\index{lemme!Schur complexe}  \label{LemSchurComplHAftTq}
...
\begin{lemma}
    Une famille de matrices de \( \GL(n,\eC)\) commutant deux à deux est simultanément triangulable.
\end{lemma}

\begin{proof}
    Soit la famille \( (A_i)_{i\in I}\) dans \( \GL(n,\eC)\) et \( A_0\) un de ses éléments. Nous nommons \( \lambda_1,\ldots, \lambda_r\) les valeurs propres distinctes de \( A_0\). Le théorème de décomposition primaire \ref{ThoSpectraluRMLok} nous donne la somme directe d'espaces caractéristiques
    \begin{equation}
...
\begin{proof}
    Nous faisons une récurrence sur \( n\). Si \( n=1\), le résultat est évident : une matrice \( 1\times 1\) est diagonale.

    Nous passons à la récurrence. Soit la famille \( (A_i)_{i\in I}\) dans laquelle nous supposons qu'il n'y a pas de multiples de l'identité. Soit \( A_0\) l'une d'elles et \( \lambda_1,\ldots, \lambda_r\) ses valeurs propres distinctes.

    Vu que \( A_0\in\GL(n,\eC)\) (\( A_0\) est inversible) nous avons un vecteur \( v_0\) tel que \( A_0v_0=e_1\). Dans une base dont le premier vecteur est \( v_0\) nous avons
    \begin{equation}
        A_0=
        \begin{pmatrix} 
            \begin{matrix}
    ...
    En ce qui concerne les espaces caractéristiques\footnote{Définition \ref{DefFBNIooCGbIix}.} nous avons \( E_{\lambda_k}(A_0)\neq E\) parce que sinon \( A_0\) serait un multiple de l'identité. En effet le cas \( E_{\lambda_k}(A_0)=E\) ferait que \( A_0\) n'a qu'une seule valeur propre et que la décomposition \ref{EqCTFHooBSGhYK} se réduit à un seul terme. Du coup les sommes dans la décomposition de Dunford \ref{ThoRURcpW}\ref{ItemThoRURcpWiii} se réduisent à un seul terme (et \( p_i=\id\)).
\end{proof}
