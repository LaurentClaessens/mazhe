...
We suppose \( (M,g)\) to be a \( n\)-dimensional Riemannian manifold and \( S\) to be a \( (n-1)\)-dimensional submanifold. We suppose that both are inside only one chart\footnote{This is an important restriction. If there are several charts, one has to play with partition of the unity as explained in definition \ref{DEFooOMQLooGiJWZS}.} :
\begin{equation}
    \phi\colon D\subset \eR^n\to M
\end{equation}
and
\begin{equation}
...
\begin{definition}[\cite{ooMLEZooCKxedX}]       \label{DEFooPDRCooPiBklC}
    The integral of a \( n\)-form on \( M\) is
    \begin{equation}
        \int_M\omega=\int_D\phi^*\omega.
    \end{equation}
\end{definition}
The last integral is an integral of type \( \int_{D}F(x_1,\ldots, x_n)dx_1\wedge\ldots \wedge dx_n\) on an open set in \( \eR^n\).
...
\begin{proposition}
    ...
    \begin{subequations}
        \begin{align}
            \int_SX&=\int_A\det\big( X,\frac{ \partial \varphi }{ \partial y_1 },\ldots, \frac{ \partial \varphi }{ \partial y_{n-1} } \big)\\
...
\begin{proof}
    ...
    \begin{subequations}
        \begin{align}
            \varphi^*(dx_1\wedge\wedge \widehat{dx_i}\wedge\ldots\wedge dx_n)(u_1,\ldots, u_{n-1})&= (dx_1\wedge\wedge \widehat{dx_i}\wedge\ldots\wedge dx_n)(d\varphi u_1,\ldots, d\varphi u_{n-1})\\
