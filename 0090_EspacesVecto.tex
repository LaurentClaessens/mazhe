...
\begin{theorem}[Lie-Kolchin\cite{PAXrsMn}]
    Tout sous-groupe connexe et résoluble de \( \GL(n,\eC)\) est conjugué à un groupe de matrices triangulaires.
\end{theorem}

\begin{proof}
    Soit \( G\) un sous-groupe connexe et résoluble de \( \GL(n,\eC)\). Nous commençons par voir ce qu'il se passe si il existe un sous-espace vectoriel non trivial \( V\) de \( \eC^n\) stabilisé par \( G\). Pour cela nous considérons une base de \( \eC^n\) dont les premiers éléments forment une base de \( V\) (base incomplète, théorème \ref{ThonmnWKs}). Les éléments de \( G\) s'écrivent, dans cette base,
    \begin{equation}
        \begin{pmatrix}
            g_1    &   *    \\ 
            0    &   g_2    
        \end{pmatrix}.
    \end{equation}
    Les matrices \( g_1\) et \( g_2\) sont carrés. Nous considérons alors les applications \( \psi_i\) (\( i=1,2\)) définies par
    \begin{equation}
        \begin{aligned}
            \psi_i\colon G&\to \GL \\
            g&\mapsto g_i.
        \end{aligned}
    \end{equation}
    Ce sont des morphismes parce que
    \begin{equation}
        \begin{pmatrix}
            g_1    &   *    \\ 
            0    &   g_2    
        ...
        \end{pmatrix},
    \end{equation}
    de telle sorte que \( \psi_i(gh)=\psi_i(g)\psi_i(h)\).

    Si \( \psi\) désigne indifféremment \( \psi_1\) ou \( \psi_2\), d'abord \( \psi(G)\) est résoluble parce que \( \psi\) est une homomorphisme. Ensuite \( \psi(G)\) est connexe. En effet si \( g_1,h_1\ion \Image(\psi)\) alors \( g_1=\psi(g)\), \( h_1=\psi(h)\) pour certains \( g,h\in G\). Vu que \( G\) est connexe, il existe un chemin continu \( g(t)\) dans \( G\) avec \( g(0)=g\) et \( g(1)=h\).
\end{proof}
<++>
