...
\begin{lemma}[Lemme de Morse]\index{lemme!de Morse}
...
%AFAIRE : regarder dans quelle leçon c'est.
...
\begin{proof}
    ...
    \begin{equation}
        f(x)-f(0)=\underbrace{df_0(x)}_{=0}+\int_0^1(1-t)\underbrace{d^2f_{tx}(x,x)}_{x^t(Hf)_{tx}x=\langle Hf_{tx}x, x\rangle }=x^tQ(x)x
    \end{equation}
    avec
    \begin{equation}
        Q(x)=\int_0^1(1-t)(Hf)_{tx}
    \end{equation}
    qui est une intégrale dans \( \aL^{(2)}(\eR,\eR^n)\).

    %AFAIRE : prouver que \( Q\) est de classe C^1.
