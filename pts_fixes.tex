...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Signature de forme quadratique}
%---------------------------------------------------------------------------------------------------------------------------

%AFAIRE : retrouver et citer le document Lemme de Morse.pdf
Nous savons par le théorème de Sylvester (théorème \ref{ThoQFVsBCk}) que \( \eM(n,\eR)\), toute matrice symétrique de signature \( (p,q)\) est semblable à la matrice
\begin{equation}
...
\begin{proposition}
    Les ensembles \( S_n^{p,q}(\eR)\) sont connexes.
\end{proposition}

\begin{proof}
    Nous savons que \( \GL(n,\eR)\) a deux composantes connexes.
\end{proof}
<++>

\begin{lemma}   \label{LemWLCvLXe}
    Soit \( S_n\) l'ensemble des matrices symétriques de \( \eM(n,\eR)\) et \( A_0\), une matrice symétrique inversible. Alors il existe un voisinage \( V\) de \( A_0\) dans \( S_n\) et une application \( \phi\colon V\to \GL(n,\eR)\) qui
    \begin{enumerate}
        \item
            est de classe \( C^1\),
        \item
...
C'est à dire que si nous avons une forme quadratique (\( A_0\in S_n\)) assez proche d'une matrice inversible (\( A\in \GL(n,\eR)\)), alors elle est semblable à la matrice non dégénérée.
...
Si la fonction \( f\) est la fonction qui donne une variété sous la forme \( (x,y,z)\in M\) si et seulement si \( z=f(x,y)\), alors localement la variété peut avoir une carte donnée par le lemme de Morse. En particulier on voit que les points critiques non dégénérés doivent être isolés.
%TODO : rendre ceci plus rigoureux.
