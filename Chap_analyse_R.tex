...
\begin{proof}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	La suite $(b_n)$ est composée d'éléments qui minorent $A$ tandis que $(a_n)$ est composée d'éléments qui ne minorent pas $A$. Par conséquent nous avons toujours $a_n>b_n$ et par conséquent
	\begin{equation}
		x_{n+1}\in\mathopen[ b_n , a_n \mathclose].
	\end{equation}
	La suite $(b_n)$ est majorée par n'importe quel élément de $A$ et est croissante parce que $b_{n+1}$ est soit égale à $b_n$ soit à $x_{n+1}$ qui est plus grande que $b_n$. La suite $b_n$ est donc convergente par le lemme \ref{LemSuiteCrBorncv}. Pour des raisons similaires, la suite $(a_n)$ est décroissante et minorée par n'importe quel minorant de $A$. Elle converge donc également.

	Nous allons maintenant prouver que $\lim a_n=\lim b_n$ et que la limite commune est l'infimum de $A$. Soit $n\in\eN$; il y a deux possibilités. Soit $a_n=a_{n-1}$ et $b_n=x_n$, soit $a_n=x_n$ et $b_n=b_{n-1}$. Supposons que nous soyons dans le premier cas (le second se traite de façon similaire). Alors nous avons
	\begin{equation}
		\begin{aligned}[]
			| a_n-b_n |&=| a_{n-1}-x_n |\\
		...
		\end{aligned}
	\end{equation}
	ce qui prouve que $| a_n-b_n |\to 0$. Nommons $\ell$ la limite commune des suites $(a_n)$ et $(b_n)$.
