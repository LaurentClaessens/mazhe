...
\begin{theorem}[Théorème d'Ascoli\cite{LBLADXV}]
    ...
    \begin{enumerate}
        \item
            \( A\) est équicontinu,
        \item
            \( \forall x\in K\), l'ensemble \( \{ f(x)\tq x\in A \}\) est relativement compact dans \( E\).
    \end{enumerate}
\end{theorem}
...
\begin{lemma}   \label{LemMPkbZxX}
    Tout élément de \( H^1(I)\) admet un représentant continu.
\end{lemma}

\begin{proof}
    Soit \( y_0\in I\) et \( u\in H^1(I)\). Nous considérons la fonction
    \begin{equation}
        \tilde u(x)=\int_{y_0}^xu'(t)dt.
    \end{equation}
    Notons que par définition, \( u'\in L^2\) donc l'intégrale ne pose pas de problèmes. Montrons que \( \tilde u\) est continue sur \( \bar I\). Pour cela nous considérons \( x\in\bar I\) et \( h\) tel que \( x+h\in \bar I\). Alors
    \begin{equation}
        \big| \tilde u(x+h)-\tilde u(x) \big|=\big| \int_x^{x+h}u' \big|\leq \int_x^{x+h}| u' |.
    \end{equation}
    ...
    Nous prouvons à présent que \( \tilde u\) est dans \( H^1(I)\) et que sa dérivée est égale à \( u'\); pour cela nous allons montrer que pour tout \( \varphi\in  C^{\infty}_c(I)\),
    \begin{equation}
        \int_I\tilde u\varphi'=-\int_Iu'\varphi.
    \end{equation}
    Nous avons
    \begin{equation}
            \int_I\tilde u\varphi'=\int_I\left( \int_{y_0}^xu'(t)dt\right)\varphi'(x)dx
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    Notons que \( \varphi(a)=\varphi(b)=0\) parce que \( \varphi\) est à support compact dans \( \mathopen] a , b \mathclose[\). Nous avons donc prouvé que \( \tilde u\) est dans \( H^1(I)\) et que \( \tilde u'=u'\). Par le corollaire \ref{CorEVJYihj}, nous avons une constante \( C\) telle que \( \tilde u=u+C\) presque partout, c'est à dire \( u=\tilde u +C\) dans \( H^1(I)\). 

        En résumé, \( \tilde u+C\) est un représentant continu de \( u\) dans \( L^2(I)\).
...
\begin{proposition}[\cite{KXjFWKA}]
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item
            ...
            Soit \( \mB\) la boule unité dans \( H^1(I)\) et \( u\in \mB\); certes \( u\) est une classe de fonctions dans \( L^2(I)\), mais il n'en reste pas moins que nous allons noter \( u\) un représentant continu de cette classe (existe par la lemme \ref{LemMPkbZxX}).

            Par la proposition \ref{PropGWOIoDg}, nous avons
            \begin{subequations}
                \begin{align}
                ...
                \end{align}
            \end{subequations}
            où nous insistons sur le fait que la continuité n'impliquant pas la dérivabilité, le \( u'\) ici est la dérivé au sens de \( H^1\), et non la dérivée usuelle. Quoi qu'il en soit, l'ensemble \( \mB\) est équicontinu\footnote{Définition \ref{DefUWmVBcZ}}. Nous montrons à présent qu'il est également borné pour la norme uniforme. Soit \( u\in\mB\) (dont nous choisissons un représentant continu) et \( x\in I\); vu la construction du représentant continu au lemme \ref{LemMPkbZxX}, nous avons
            \begin{subequations}
                \begin{align}
