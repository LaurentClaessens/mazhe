...
Attention : dans \ref{subSecCKZSrZK}, le produit scalaire a été défini avec un coefficient \( 1/2\pi\). Voir l'équation \eqref{EqQBcpyyJ}.
...
\begin{enumerate}
    ...
    \item
        La proposition \ref{ThoTAYooZkALUw} pour les espaces \( L^p\big( \mathopen[ 0 , 1 \mathclose] \big)\) avec \( 1<p<2\).
    \item
...
Pour la proposition suivante la notation \( \langle f, g\rangle \) est
\begin{equation}
    \langle f, g\rangle =\int_{\mathopen[ 0 , 1 \mathclose]}fg.
\end{equation}
...
\begin{theorem}[Théorème de représentation de Riesz\cite{LRBWftc}] \label{ThoSCiPRpq}
    Soit \( 1<p<\infty\) et une mesure quelconque. Alors
    \begin{enumerate}
        \item
            L'espace \( L^p\) est réflexif.
        \item
            Si \( p\) et \( q\) sont des nombres tels que \( \frac{1}{ p }+\frac{1}{ q }=1\) alors l'application
            \begin{equation}
                \begin{aligned}
                    \Phi\colon L^q&\to (L^p)' \\
