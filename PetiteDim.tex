...
}		% Fin du siTHZ pour le groupe d'isométrie.
...
\end{equation}
\ifthenelse{\value{siTHZ}=1}{Exponentiating the commutations relations in $\mathfrak{sl}(2,\eR)$, we find}{From commutation relations in $\sldr$, one finds
\begin{subequations}
\begin{align}
\Ad(e^{aH})E&=e^{2a}E,\\
\Ad(e^{aH})F&=e^{-2a}F,
...
\right)=0
\end{equation}
where the dots means that the same lies in the second entry of the Killing form. Using the invariance of the Killing form, the fact that $\epsilon^{-1}=\epsilon$ and \ifthenelse{\value{siTHZ}=1}{}{equation \eqref{eq_AdetE}} that $\Ad(e^{tE})H=H-2tE$, we rewrite the condition under the form
\begin{equation}
...
The conclusion is that both two lines of the light cone intersect the singularity passing by $H$ \emph{and} the one passing by $-H$.  So any point comes from the singularity and returns to the singularity; no point is connected to the infinity.
 ...
 \ifthenelse{\value{siTHZ}=1}{}{The $AdS_2$-black hole is doomed.}
...
We look at $M=\Ad(G)H$ on which $G=SL(2,\eR)$ acts by the adjoint action. The set $A=e^{\eR H}$, which is the abelian part of $G$ with respect to the Iwasawa decomposition, is the stabilizer of $H$. Hence\ifthenelse{\value{siTHZ}=1}{}{the theorem \ref{tho:homeo_action} makes}, up to a double covering
\begin{equation}
AdS_2=G/A=\Ad(G)H.
\end{equation}
...
\]
on which we put the Killing metric in order to use the fact that $\Aut(G)\subset\Iso(G)$\ifthenelse{\value{siTHZ}=1}{}{ (equation \eqref{eq:Aut_Iso})}. For notational convenience, we write $\uG:=G\times G$.
...
\]
Since $\psi(g_1,g_2)=L_{g_1}\circ R_{g_2^{-1}}$, the Killing form is invariant under $\psi(g_1,g_2)$\ifthenelse{\value{siTHZ}=1}{}{ (cf. theorem \ref{tho:bi_invariance})}. Thus $\psi(g_1,g_2)\in\Iso(G)$. Now we define $\tX$: the left invariant vector field generated by $X\in\mG$ and $\utX$, the right invariant one.
\[
