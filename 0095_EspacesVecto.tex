...
\begin{proposition}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    où \( b\) est forme bilinéaire associée à \( q\) est une bijection.
\end{proposition}

\begin{proof}
    Si \( \varphi(q)=\varphi(q')\); alors
    \begin{equation}
        q(x)=\sum_{i,j}\varphi(q)_{ij}x_ix_j=\sum_{i,j}\varphi(q')_{ij}x_ix_j=q'(x).
    \end{equation}
    Donc \( q=q'\). L'application \( \varphi\) est donc injective

    De plus elle est surjective parce que si \( B\in S(n,\eR)\) alors la forme quadratique
    \begin{equation}
        q(x)=\sum_{i,j}B_{ij}x_ix_j
    \end{equation}
    a évidemment \( B\) comme matrice associée.
\end{proof}
