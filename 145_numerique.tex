...
\begin{proof}
    ...
    Le graphe de \( f\) est au dessus de la tangente de \( f\) en \( x=x_n\) (proposition \ref{PropYKwTDPX}\ref{ITEMooBOCZooSTaXDB}). Si nous nommons \( t_x\) la fonction qui donne la tangente en \( x\) nous avons \( t_{x_n}(\alpha)<0\) parce que \( f(\alpha)=0\). Par conséquent
    \begin{equation}
        t_{x_n}(x)=0
    \end{equation}
