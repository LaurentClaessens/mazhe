...
As for notations, when we have a topological space $X$ and a point $x\in X$, we denote by $\mV(x)$ the set of neighbourhood of $x$. If $F$ is a subset of $X$, we denote by $\overline{F}$ the closure of $F$ and by $\intr{F}$, its interior.
...
\begin{definition}
A increasing sequence $(K_m)$ of compacts subset of $\mU$ is \defe{fundamental}{fundamental!sequence of subset} if $K_m\subset\intr{K}_{m+1}$ for all $m$ and
\[ 
 \bigcup_m K_m=\mU 
\]
\end{definition}
...
%%\psframe[linecolor=cyan](0.5,-0.5)(3.5,1.5)
%\psset{PointSymbol=none,PointName=none}
%\pstGeonode(1,1){Pa}(2,1){Pb}(3,1){Pc}
%\pstGeonode(1,0){Palpha}(2,0){Pbeta}(3,0){Pgamma}
%\psline(Pa)(Palpha)
%\psline(Pb)(Pbeta)
%\psline(Pc)(Pgamma)
%\psline(Pa)(Pbeta)
...
%\pstMarquePoint{Pc}{0.3;90}{$c$}
%\pstMarquePoint{Palpha}{0.3;-90}{$\alpha$}
%\pstMarquePoint{Pbeta}{0.3;-90}{$\beta$}
%\pstMarquePoint{Pgamma}{0.3;-90}{$\gamma$}
%\end{pspicture}
%\end{center}
