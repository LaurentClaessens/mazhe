...
\begin{proof}
    ...
    \begin{remark}
        Attention : ceci n'est pas une preuve. En effet, dans la mesure où nous n'avons même pas donné de définition de la limite, il n'est pas possible de donner une \emph{vraie} preuve de quoi que ce soit. Si vous voulez des démonstrations et de la vraie théorie, \href{http://homepages.ulb.ac.be/~lclaesse/GeomAnal.pdf}{vous pouvez regarder ici}.
    \end{remark}
\end{proof}
...
\begin{proposition} \label{PropExistDiffUn}
    ...
    \begin{equation}        \label{eqCritDifffabsrt}
        \begin{aligned}[]
            ...
            &+d\big( (x,y),(a,b) \big)\alpha\Big( d\big( (x,y),(a,b) \big) \Big)
        \end{aligned}
    \end{equation}
    et
    \begin{equation}
...
\end{proposition}
Dans cet énoncé nous avons écrit $d\big( (x,y),(a,b) \big)$ la distance entre $(x,y)$ et $(a,b)$, c'est à dire le nombre $\sqrt{(x-a)^2+(y-b)^2}$. Affin d'écrire l'équation \eqref{eqCritDifffabsrt} sous forme plus compacte, nous introduisons le vecteur
\begin{equation}
...
\begin{equation}        \label{Eqsuitedfnfdsdfu}
    df_p(u)=\nabla f(p)\cdot u=\frac{ \partial f }{ \partial u }(p).
\end{equation}
