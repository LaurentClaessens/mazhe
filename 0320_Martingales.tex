...
\begin{definition}
    ...
    Le temps d'arrêt $T$ est \defe{borné}{borné!temps d'arrêt} si il existe \( M\in \eN\) tel que \( T(\omega)\leq M\) pour presque tout \( \omega\in \Omega\).
\end{definition}
...
\begin{equation}
    E(M_n\tribA_{n-1})=E(S_n|\tribA_{n-1})-\underbrace{E(A_n|\tribA_{n-1})}_{=A_n}=A_n-A_{n-1}+E(S_{n-1}|\tribA_{n-1})-A_n=E(S_{n-1}|\tribA_{n-1})-A_{n-1}.
\end{equation}
