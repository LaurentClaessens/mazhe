...
\begin{lemma}[\cite{RapportArgreg2011}]
...
\begin{theorem}[Décomposition de Frobenius \cite{AutourFrobCompa}]
    Soit \( E\), un \( \eK\)-espace vectoriel et \( u\in \End(E)\). Alors il existe une suite de sous espaces \( E_1,\ldots, E_r\) stables par \( u\) tels que
    \begin{enumerate}
        \item   \label{ItemmpwjnSs}
            \( E=\bigoplus_{i=1}^rE_i\);
        \item
            pour chaque \( E_i\), l'endomorphisme restreint \( u_i=u|_{E_i}\) est cyclique;
        \item
            si \( \mu_i\) est le polynôme minimal de \( u_i\) alors \( \mu_{i+1}\) divise \( \mu_i\);
        \item
            la suite \( (\mu_i)_{i=1,\ldots, r}\) ne dépend que de \( u\) et non du choix de la décomposition du point \ref{ItemmpwjnSs}.
    \end{enumerate}
    En particulier toute matrice est semblable à une matrice diagonale par blocs dont chaque bloc est une matrice compagnon. Il existe une une base dans laquelle la matrice de \( u\) est de la forme
    \begin{equation}
        u!=\begin{pmatrix}
            C_{\mu_1}    &       &       \\
                &   \ddots    &       \\
        ...
        \end{pmatrix}
    \end{equation}
    avec \( \mu_1=\mu_u\) et \( \mu_1\cdot\ldots \mu_r=\chi_u\).
\end{theorem}
<++>
