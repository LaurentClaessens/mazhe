...
\index{groupe!agissant sur un ensemble}
...
\begin{proposition}[\cite{tzHydF}] \label{PropLDIPoZ}
    Le groupe diédral \( D_n\) est engendré par \( s\) et \( r\). De plus tous les éléments de \( D_n\) s'écrivent sous la forme \( s^k\circ r^m\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item
            Supposons à présent que \( f\) soit une réflexion d'axe \( \Delta\). Cette fois, \( \Delta\) ne passe par aucun des points \( A_k\), par contre \( \Delta\) passe par \( 0\). 
            
            Nous considérons un \( q\in \{ 1,\ldots, n-1 \}\). Étant donné que \( f(A_q)\) doit être sur le cercle, \( \Delta\) doit être la médiatrice d'un segment \( \mathopen[ A_q , P \mathclose]\) pour un certain \( P\) sur le cercle. Mais vu que dans ce cas \( f(A_q)=P\), le point \( P\) doit être une des \( A_i\). Étant donné que la médiatrice du segment \( \mathopen[ A_q , A_{q+2k} \mathclose] \) passe par \( A_{p+k}\), la droite \( \Delta\) doit être la médiatrice d'un segment de la forme \( \mathopen[ A_q , A_{q+2k+1} \mathclose]\) pour un certain \( k\).

            La médiatrice de \( \mathopen[ A_q , A_{q+2k+1} \mathclose]\) est aussi médiatrice de \( \mathopen[ A_{q+1} , A_{q+2k} \mathclose]\) et de proche en proche, nous trouvons que \( \Delta\) est la médiatrice d'un segment \( \mathopen[ A_p , A_{p+1} \mathclose]\). Montrons alors que \( f=s\circ r^{n-2p-1}\). Il faut montrer que c'est une réflexion qui envoie \( A_p\) sur \( A_{p+1}\). D'abord c'est une réflexion parce que
            \begin{equation}
