...
\begin{proof}
    ...
    Remarquons que \( O(n)\) est compact par le lemme \ref{LemTLlTAAf} et que par conséquent \( \Conv(O(n))\) est compacte par le corollaire \ref{CorOFrXzIf} et donc fermée. Nous considérons le produit scalaire \( (X,Y)\mapsto\tr(X^tY)\) de la proposition \ref{PropMAQoKAg}. Vu que \( \Conv\big( O(n) \big)\) est un fermé convexe nous pouvons considérer la projection sur \( \Conv(A)\) relativement à ce produit scalaire.

    %AFAIRE : teminer cette démonstration.
\end{proof}
<++>
