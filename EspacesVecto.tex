...
\begin{proof}
    Le fait que ce soit une norme découle entre autre de l'inégalité de Cauchy-Schwartz.
...
\begin{theorem}[Carathéodory\cite{KXjFWKA}]
...
\begin{proof}
    ...
    Remarquons que \( O(n)\) est compact par le lemme \ref{LemTLlTAAf} et que par conséquent \( \Conv(O(n))\) est compacte par le corollaire \ref{CorOFrXzIf} et donc fermée. Nous considérons le produit scalaire \( (X,Y)\mapsto\tr(X^tY)\) de la proposition \ref{PropMAQoKAg}. Nous allons noter par \( X\cdot Y\) ce produit. Vu que \( \Conv\big( O(n) \big)\) est un fermé convexe nous pouvons considérer la projection\footnote{Le théorème de projection : théorème \ref{ThoWKwosrH}.} sur \( \Conv(A)\) relativement à ce produit scalaire.
