...
\begin{equation}    \label{EqFormSesqQrjyPH}
    \langle x, y\rangle =\sum_{k=1}^nx_ky_k
\end{equation}
...
\begin{theorem}
    Les matrices \wikipedia{fr}{Endomorphisme_normal}{normales} forment un espace connexe.
\end{theorem}
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{numcases}
    \end{subequations}
    Ce \( w\) serait donc un vecteur propre simultané pour les valeurs propres \( \lambda_i\) et \( \lambda_j\), ce qui est impossible parce que les espaces propres sont linéairement indépendants.

    Revenons à l'inclusion inverse de \eqref{EqzmNxPi}. Un vecteur \( v\in F_{\lambda_i}(u)\) ne peut avoir aucune composante dans un des espaces \( \ker(u-\lambda_j)^{\alpha_j}\). Étant donné la décomposition \eqref{EqDeFVSaYv}, le vecteur \( v\) doit donc appartenir à \( (u-\lambda_i)^{\alpha_i}\).
\end{proof}
