...
\begin{lemmaDef}
    ...
    Il est nommé le \defe{polynôme minimal}{polynôme!minimal} de \( f\).
\end{lemmaDef}

\begin{proof}
    Soient \( P\) et \( Q\) deux polynômes annulateur de \( f\) de même degré \( k\) et ayant tous deux \( 1\) comme coefficient de \( x^k\). Alors \( P-Q\) est de degré \( k-1\) tout en étant encore annulateur.
\end{proof}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Les noyaux de ces applications sont des idéaux, entre autres par le lemme \ref{LemQWvhYb}. Ils ont donc un unique générateur unitaire (chacun) par le théorème \ref{ThoCCHkoU}. Nous notons \( \mu\) le générateur unitaire du noyau de \( \varphi\) et \( \mu_x\) celui de \( \varphi_x\). Vu que \( \mu\in\ker(\varphi_x)\) pour tout \( x\) nous avons\( \mu_x\divides \mu\) pour tout \( x\).
