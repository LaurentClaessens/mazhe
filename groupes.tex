...
Ce groupe est un groupe cyclique généré par \( \xi\). Les autres générateurs sont les \( \xi^p\) avec \( \pgcd(p,n)=1\). Nous notons \( \Delta_n\) l'ensemble des générateurs de \( \gU_n\) :
\begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \Hom(\eZ/n\eZ)\simeq \mu_n=\{ \xi\in \eC\tq \xi^n=1 \}.
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            \varphi\colon \Hom\Big( (\eZ/n\eZ,+),(\eC^*n\cdot) \Big)&\to \mu_n \\
            g&\mapsto f(1). 
        \end{aligned}
    \end{equation}
    ...
    \begin{equation}
        f(1)^n=f([n])=f(0)=1.
    \end{equation}
    Donc \( f(1)\in \mu_n\). Le \( \varphi\) est injective parce que si \( f(1)=g(1)\) alors \( f=g\) du fait que \( f(k)=f(1)^k=g(1)^k=g(k)\).

    Nous en sommes à avoir prouvé que \( \hat{\eZ/n\eZ}\simeq \mu_n\). Il faudrait encore montrer que \( \mu_n\simeq \eZ/n\eZ\).
