...
Si \( G\) est un groupe et si \( g,h\in G\), nous notons \( [g,h]=ghg^{-1}h^{-1}\)\nomenclature[A]{\( [g,h]\)}{commutateur dans un groupe} le \defe{commutateur}{commutateur!dans un groupe} de \( g\) et \( h\). Le \defe{groupe dérivé}{dérivé!groupe}\index{groupe!dérivé} de \( G\) est le sous groupe note \( D(G)\)\nomenclature[A]{\( D(G)\)}{groupe dérivé} ou \( [G,G]\)\nomenclature[A]{\( [G,G]\)}{groupe dérivé} engendré par les commutateurs.
...
Le \defe{groupe symétrique}{groupe!symétrique} \( S_n\)\nomenclature[A]{\( S_n\)}{le groupe symétrique} est le groupe des permutations de l'ensemble \( \{ 1,\ldots,n \}\). C'est donc l'ensemble des bijections \( \{ 1,\ldots, n \}\to\{ 1,\ldots, n \}\).
...
Le groupe \( A_n\)\nomenclature[A]{\( A_n\)}{groupe alterné} des permutations paires est la \defe{groupe alterné}{alterné!groupe}\index{groupe!alterné}.
...
\begin{theorem}[Théorème de Bezout] \label{ThoBuNjam}
...
Notons que \( \eZ_n=\eZ/n\eZ\) est un groupe pour l'addition tandis que \( \eZ_n^*\) est un groupe pour la multiplication. Il ne peut donc pas y avoir d'équivoque.
...
Nous considérons l'espace affine euclidien \( A=\affE_n(\eR)\) modelé sur \( \eR^n\) avec sa métrique usuelle. Les isométries de cet espace sont données d'une part par les rotations de \( \SO(n)\) et d'autre part par les translations données par les vecteurs de \( \eR^n\)\footnote{Cela n'est pas tout à fait trivial en fait. Une preuve que les isométries de \( A\) soient des applications affines peut être trouvée dans \cite{echa}, dans la partie sur les transformations de Lorentz.}. Un couple \( (v,\Lambda)\in \eR^n\times\SO(n)\) agit sur \( x\in \eR^n\) par
\begin{equation}
...
\begin{enumerate}
    \item
        Pour chaque \( \Lambda\), l'application \( \phi_{\Lambda}\) est un automorphisme du groupe \( \eR^n\) (en tant qu'agissant sur \( A\)).
    \item
        L'application \( \phi\colon \SO(n)\to \Aut(\eR^n)\) est un morphisme de groupe.
    \item
        La loi de groupe donnée par \( \phi\) sur \( \SO(n)\times \eR^n\) par la définition \eqref{EqDRgbBI} est bien la loi de groupe \eqref{EqDiHcut}.
\end{enumerate}

%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Décomposition des groupes abéliens finis}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
Soit \( G\) un groupe d'ordre \( pq\) où \( p\) et \( q\) sont des nombres premiers. Nous supposons que \( p<q\). Montrons que \( G\) ne possède qu'un seul \( q\)-Sylow. Soit \( n_q\) le nombre de \( q\)-Sylow; par les théorèmes de Sylow nous avons
\begin{equation}
    n_q=1\mod q
\end{equation}
...
\begin{proposition}[\cite{PDFpersoWanadoo}]
    Soit \( G\) un groupe d'ordre \( pq\) où \( p\) et \( q\) sont des nombres premiers distincts vérifiant
    \begin{subequations}
        \begin{numcases}{}
            p\neq 1\mod q\\
            q\neq 1\mod p.
...
\begin{proof}
    Soient \( n_p\) et \( n_q\) les nombres de \( p\)-Sylow et \( q\)-Sylow. Par les théorèmes de Sylow, \( n_p\) divise \( pq\) et \( n_p=1\mod p\). Le second point empêche \( n_p\) de diviser \( p\). Par conséquent \( n_p\) divise \( q\) et donc \( n_p\) vaut \( 1\) ou \( q\). La possibilité \( n_p=q\) est exclue par l'hypothèse \( q\neq 1\mod p\). Donc \( n_p=1\), et de la même façon nous obtenons \( n_q=1\).
