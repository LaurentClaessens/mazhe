...
Une \defe{suite exacte}{suite!exacte} est un schéma comme suit :
    \begin{equation}
    \xymatrix{%
    1 \ar[r]        & A\ar[r]^i&E\ar[r]^s&B\ar[r]&1
       }
    \end{equation}
où \( i\) est injective, \( s\) est surjective et où \( \ker(s)=\Image(i)\). Dans le cas des groupes, nous demandons de plus que \( i\) et \( s\) soient des homomorphismes.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \( G=\tilde N\tilde H\). Nous considérons \( g\in G\) et \( h\in \tilde H\) tel que \( s(g)=s(h)\). L'existence d'un tel \( h\) est assurée par le fait que \( s\) est surjective depuis \( \tilde H\). Du coup nous avons \( e=s(gh^{-1})\), c'est à dire \( gh^{-1}\in \ker (s)=\tilde N\). Nous avons donc bien la décomposition \( g=(gh^{-1})h\), et donc \( G=\tilde N\tilde H\).

        \item L'écriture \( g=nh\) avec \( n\in \tilde N\) et \( h\in \tilde H\) est unique. Si \( nh=n'h'\), alors \( n=n'h'h^{-1}\), ce qui signifierait que \( h'h^{-1}^{-1}\in\tilde N\).
