...
\begin{theorem}     \label{ThojrLKZk}
    Soit \( G\) un groupe topologique localement compact et dénombrable à l'infini\footnote{Cela signifie qu'il est une réunion dénombrable de compacts} agissant sur un espace topologique localement compact \( E\). Alors l'application
    \begin{equation}
        \begin{aligned}
...
\begin{lemma}[Décomposition de Schur]\index{décomposition!Schur}        \label{LemSchurDecUrHLCm}
    Soit \( A\) une matrice dans \( \eM(n,\eC)\). Alors il existe une matrice unitaire \( Q\) telle que
    \begin{equation}
        QAQ^{-1}=T
    \end{equation}
...
\begin{proof}
    Nous allons nous contenter de prouver \ref{ItemJZhFPSi}\( \Leftrightarrow\)\ref{ItemJZhFPSii}. Soit \( Q\) la matrice unitaire donnée par la décomposition de Schur (lemme \ref{LemSchurDecUrHLCm}) : \( A=QTQ^{-1}\). Étant donné que \( A\) est normale nous avons
    \begin{equation}
        QTT^*Q^{-1}=QT^*TQ^{-1},
    \end{equation}
