...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
\section{Exponentielle et logarithme : l'approche pseudo-facile}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

Nous voulons définir la fonction exponentielle (sur \( \eR\)) comme étant l'unique solution à l'équation
\begin{subequations}    \label{EqSDPDooVptqVI}
    \begin{numcases}{}
        y'=y\\
        y(0)=1.
    \end{numcases}
\end{subequations}
Remarquons que si une telle fonction existait sur un ouvert, elle serait automatiquement dérivable sur cet ouvert, et donc continue. Si \( y\) est dérivable, \( y'\) l'est également et donc \( y\) est de classe \( C^1\). Au final, \( y\) serait automatiquement de classe \( C^{\infty}\) sur son domaine de définition. 

Les étapes seront :
\begin{enumerate}
    \item
        Définir le logarithme comme primitive de \( x\mapsto \frac{1}{  x }\) s'annulant en \( x=1\).
    \item
        Montrer que la fonction réciproque du logarithme vérifie l'équation \eqref{EqSDPDooVptqVI} (cela pour l'existence de l'exponentielle).
    \item
        Démontrer l'unicité via quelque propriétés simples\footnote{Pour information, l'unicité telle que nous allons la voir est au programme de terminale S, pas l'existence.}.
\end{enumerate}
Notons toutefois que ce programme est très lourd car il utilise entre autres la densité des polynômes dans les fonctions continues (pour démontrer que toutes les fonctions continues ont une primitive). Mais \emph{à part ça}, ce programme est relativement simple.

C'est pour cette raison que nous prenons la série comme définition de l'exponentielle\footnote{Surtout que pour les exponentielles d'opérateurs, il n'y a pas tellement le choix.} et son équation différentielle comme propriété. L'unicité de la solution à \eqref{EqSDPDooVptqVI} donné dans cette section est toutefois encore bien nécessaire, mais n'est vraiment pas lourde.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
est continue et admet donc une unique primitive\footnote{Théorème \ref{ThoEOMRooZPUfJg}} s'annulant en \( x=1\). Nous nommons \defe{logarithme}{logarithme} et notons \( \ln\)\nomenclature[Y]{$\ln$}{logarithme} cette primitive. C'est une fonction
\begin{equation}
...
\begin{proposition}
    La fonction logarithme est une bijection.
\end{proposition}

\begin{proof}
    La fonction logarithme étant la primitive d'une fonction strictement positive, elle est strictement croissante. Elle est donc injective.

    En ce qui concerne la surjectivité du logarithme sur \( \eR\), nous nous servons du lemme \ref{LemPEYJooEZlueU} qui nous dit entre autres (pour \( a>1\) ) que
    \begin{equation}
        \ln(a^k)=k\ln(a).
    \end{equation}
    En faisant \( k\to \infty\) nous trouvons \( \lim_{x\to \infty} \ln(x)=+\infty\). Et en faisant \( b\to \infty\) dans \eqref{EqOOZGooOWkGlA} nous trouvons ensuite que \( \lim_{x\to 0} \ln(x)=-\infty\).
\end{proof}

La fonction logarithme étant une bijection, elle possède une réciproque que nous notons \( \exp\). Nous avons alors l'égalité pour tout \( x\in \eR\) :
\begin{equation}
    \ln\big( \exp(x) \big)=x.    
\end{equation}
En dérivant les deux membres,
\begin{equation}    \label{EqNQDDooYBbARn}
    \frac{ \exp'(x) }{ \exp(x) }=1,
\end{equation}
et donc \( \exp\) est une solution de l'équation différentielle \( y'=y\), \( y(0)=1\).
