...
\begin{definition}
    Soient \( (E,d_E)\) et \( (F,d_F)\) deux espaces métriques, \( \mO\) un ouvert de \( E\), \( f\colon E\to F\) une application et un réel \( k\) strictement positif. Nous disons que \( f\) est \defe{Lipschitzienne}{Lipschitzienne} de constante $k$ sur \( \mO\) si pour tout \( x,y\in\mO\),
    \begin{equation}
