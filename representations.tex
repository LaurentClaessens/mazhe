...
\begin{proof}
    Nous considérons les bases \( \{ e_1,\ldots, e_n \} \) de \( V\) et \( \{ f_1,\ldots, f_m \}\) de \( V'\). Nous notons \( m(g)\) la matrice de \( \rho(g)\) dans cette base : \( m_{ij(g)}=\big( \rho(g) \big)_{ij}\). Puis nous considérons la matrice $F=E_{kl}\in\eM_{m,n}(\eC)$, et nous posons
    \begin{equation}
        F_G=\frac{1}{ | G | }\sum_{s\in G}\rho'(s)\circ F\circ\rho(s)^{-1}.
    \end{equation}
    Montrons que ce \( F_G\) entrelace \( \rho\) et \( \rho'\) :
    \begin{subequations}
        \begin{align}
