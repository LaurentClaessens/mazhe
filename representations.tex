...
\begin{enumerate}
    \item
        ...
        \begin{equation}
            \chi^{(0)}=\chi^{++}+\chi^{-+}.
        \end{equation}
        Il y a maintenant deux façons de voir que la représentation \( \rho^{(0)}\) est réductible en \( \rho^{(0)}=\rho^{++}\oplus\rho^{-+}\). La première est d'invoquer le théorème \ref{ThoWGkfADd}\label{ItemZReOWoHi} pour dire que les caractères étant égaux, les représentations sont équivalentes. La seconde est de trouver un opérateur d'entrelacement. Pour cela nous calculons les matrices :
        \begin{subequations}
            \begin{align}
    ...
    \item
        Complément. Nous pouvions savoir que \( \rho^{(0)}\) n'était pas irréductible même sans remarquer que son caractère était égal à celui de \( \rho^{++}\oplus\rho^{-+}\). Pour cela nous utilisons le théorème \ref{ThoWGkfADd}\ref{ItemZReOWoHii} et nous calculons \( \langle \chi^{(0)}, \chi^{(0)}\rangle \). Nous avons
        %AFAIRE : référentier ici le théorème qui donne la liste
        \begin{subequations}
            \begin{align}
