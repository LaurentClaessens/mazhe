...
\begin{proof}
    ...
    Soit \( x\) non rationnel et une suite \( (q_i)\) convergeant dans \( \eQ\) vers \( x\). Pour chaque \( i\), nous considérons des sous-suite des \( a^{(i)}_n\) et des \( b^{(i)}_n\) vérifiant
    \begin{equation}
        \big| a^{(i)}_nq_i+b^{(i)}_n-g(q_i) \big|\leq \epsilon.
    \end{equation}
    Cela existe par continuité de \( g\) (lemme \ref{LemKLTsHIQ}) 
\end{proof}
<++>
