...
\begin{lemma}[\cite{KXjFWKA}]   \label{LemXOUooQsigHs}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    est \defe{log-convave}{concave!log-concave}\index{log-concave}, c'est à dire que l'application \( \ln\circ\phi\) est concave. Ici \( S^{++}\) est l'ensemble des matrices symétriques strictement définies positives, définition \ref{DefAWAooCMPuVM}.
\end{lemma}

\begin{proof}
    Il faut prouver que si \( \alpha+\beta=1\) (avec \( \alpha,\beta\geq 0\)) et si \( A,B\in S^{++}(n,\eR)\) alors
    \begin{equation}
        \ln\Big( \det(\alpha A+\beta B) \Big)\leq \ln\Big( \det(\alpha A) \Big)+\ln\Big( \det(\beta B) \Big),
    \end{equation}
    <++>

    Nous avons vu dans l'exemple \ref{ExPDRooZCtkOz} que la fonction logarithme était concave.
\end{proof}
<++>
...
\begin{proof}
    ...
    Nous prenons \( (\tilde q_n)\), une sous-suite de \( (q_n)\) convergeant vers \( x\) et \( N\) suffisamment grand pour que pour tout \( n\geq N\) on ait \( | \tilde q_n-x |\leq \epsilon\) et \( | g(\tilde q_n)-g(x) |\leq \epsilon\); ce la est possible grâce à la continuité de \( g\) (lemme \ref{LemKLTsHIQ}). Ensuite les sous-suites \( (\tilde a_n)\) et \( (\tilde b_n)\) sont celles qui correspondent :
    \begin{equation}
