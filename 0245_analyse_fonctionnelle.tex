...
\item[Prolongement pour les boules ouvertes]
    ...
    \end{equation}
    Nous posons\quext{Ici, \cite{KXjFWKA} met \( d(x,Z)\) au numérateur. Je crois que c'est une erreur de sa part parce qu'alors on n'a pas \( u(x)=1\) pour \( x\in Y\).}
    \begin{equation}
        \begin{aligned}
            u\colon X&\to \eR^+ \\
            x&\mapsto \frac{ d(x,Y) }{ d(x,Y)+d(x,Z) } 
        \end{aligned}
    \end{equation}
