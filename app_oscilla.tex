...
Operator symbol are natural framework to build oscillatory integral. Some topological background are given in appendix. Most of this theory comes from the book \cite{Dieu7} of Dieudonné. 
...
We just prove the point \ref{enufiii}.
...
$\lambda^{-m}a_{\lambda})^s$ is a bounded set when $s=-1$. Lemma \ref{lem:bornFRE} shows it for all $s$.
...
\]
The operator $P$ is \defe{elliptic}{elliptic operator}\index{operator!elliptic} if for each non zero $\xi\in T^*M$, the map $\sigma_x^P(\xi_x)$ are all invertible. If $(M,g)$ is a Riemannian manifold, the fact for $P$ to be elliptic is equivalent to be invertible on the \defe{cosphere}{cosphere}
\[ 
