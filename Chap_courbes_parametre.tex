...
Une des notions clefs pour la suite est celle de subdivision d'intervalles. Cette notion sera encore utilisée par la suite à propos des intégrales (chapitre \ref{ChapMultiples}).
...
En analogie avec ce que nous verrons durant le chapitre \ref{ChapMultiples}, et en particulier dans la définition \ref{DefMesureInt}, nous nous attendons que la longueur d'une courbe soit égale à la valeur de l'intégrale de la fonction $1$ sur le long de la courbe. Cela est vrai, mais la définition des intégrales curvilignes dépasse les objectifs de ce cours et donc nous ne pouvons pas donner la preuve de ce fait.   
\end{remark}
%Nous avons ainsi une formule intégrale pour ce que nous avons appelé la \emph{longueur} d'un arc. Mais est-ce que cette longueur correspond bien à ce que nous appelons d'habitude \emph{longueur} ?

%Pour répondre à cette question, le mieux serait de prouver que la longueur de $\gamma$ est bien l'intégrale sur le graphe de $\gamma$ de la fonction $1$ parce que nous avons vu dans la définition \ref{DefMesureInt} que cette intégrale est la mesure de la courbe. Hélas, nous n'avons pas vu comment on peut intégrer une fonction le long d'un chemin; nous allons donc nous contenter de prouver que la distance entre deux points est bien donnée par la longueur de la droite joignant les deux points.
...
\begin{example}     \label{ExCycloLong}
    ...
    \begin{subequations}
        ...
        \end{numcases}
    \end{subequations}
    avec $a>0$ et $t\in\eR$. Comme montré sur la figure \ref{LabelFigCycloide}, la cycloïde donne lieu à un graphe périodique. Il est possible de montrer (le faire) que le premier arc correspond à $t\in\mathopen[ 0 , 2\pi \mathclose]$. Nous voulons donc calculer la longueur de l'arc sur cet intervalle.
    \newcommand{\CaptionFigCycloide}{La cycloïde de paramètre $a=1$ entre $0$ et $4\pi$.}
...
\begin{definition}      \label{DefCourbureNormleUnit}
    Soit $\gamma=(J,\gamma_N)$ un arc paramétré normal de classe $\mathcal{C}^2$. La \defe{normale principale}{normale!principale} est le vecteur $\tau'(s)$. Le \defe{vecteur unitaire normal}{unitaire!normale principale} est le vecteur\nomenclature[C]{$\nu(s)$}{Vecteur unitaire de la normale principale}
    \begin{equation}
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Tracer des courbes dans paramétriques $\eR^2$}     \label{SecTracerParmCourbe}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
