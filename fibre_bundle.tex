...
\]
For all $b\in B(M)_x$, we define $p_B(b)=x$ and the action $B(M)\times GL(n,\eR)\to B(M)$ by $b\cdot g=({\bf b}'_1,\ldots,{\bf b}'_n)$ where
\begin{equation}
  {\bf b}'_j={\bf b}_i\bghd{g}{i}{j}.
\end{equation}
...
a structure of principal bundle\footnote{Much more details and proofs are given in \cite{Naber}.}. If $(\mU_{\alpha},\varphi_{\alpha})$ is a local coordinate chart on $M$, we define
		...
		\end{aligned}
	\end{equation}	
where $A(b)\in GL(n,\eR)$ is defined by the condition ${\bf b}_j=\bghd{A}{j}{i}\partial_i|_x$. The matrix $A(b)$ is the one which transforms the canonical basis (in the trivialization $\varphi_{\alpha}$) into $b\in B(M)_x$. That's for the principal bundle structure. 
