...
\begin{equation}
    \Var(X)=E\Big[ \big( X-E(X) \big)\otimes \big( X-E(X) \big) \Big]
\end{equation}
où par \( \otimes\) nous entendons l'opération suivante. Si \( a\) et \( b\) sont des vecteurs, \( a\otimes b\) est la matrice \( ab^t\), et nous noterons
\begin{equation}
    (a\otimes b)(v,w)=(a\cdot v)(b\cdot w)=v\cdot (a\otimes b)w
\end{equation}
où \( (a\otimes b)w\) est l'action de la matrice \( (a\otimes b)_{ij}=a_ib_j\) sur le vecteur \( w\).
...
\begin{proof}
    ...
    \end{equation}
    parce que \( E(Y_0\otimes Y_0)=\mtu\). Nous avons utilisé les formules de la section \ref{SeOOpHsn}.
\end{proof}
<++>
