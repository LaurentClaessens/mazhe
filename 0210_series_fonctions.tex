...
\begin{lemma}[\cite{DOAooAgGKTi}]
    ...
    \begin{equation}
        \|  e^{tA} \|\leq P\big( | t | \big)\sum_{i=1}^r e^{t\Reel(\lambda_i)}.
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    \end{equation}
    Et c'est maintenant que nous allons utiliser la décomposition de Dunford du théorème \ref{ThoRURcpW} : nous avons \( A=s+n\) avec
    \begin{equation}
        \begin{aligned}[]
    ...
    \begin{equation}
        e^{p_iA}= e^{p_is} e^{p_in}= e^{\lambda_ip_i} e^{(A-\lambda_i\mtu)p_i}.
    \end{equation}
    <++>
\end{proof}
<++>
