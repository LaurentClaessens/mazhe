...
Nous commençons par donner quelque éléments à propos de dérivée et de différentielle pour des fonctions \( \eC\to \eC\) parce que les séries entières vont souvent être des fonctions complexes.
...
\begin{lemma}
    ...
    \begin{equation}
        ...
        \end{pmatrix}.
    \end{equation}
    Une telle matrice est une \defe{similitude}{matrice!de similitude}\index{similitude}
\end{lemma}
...
\begin{proposition}[\cite{GYDXooJJusGH,MonCerveau}]     \label{PropSNMEooVgNqBP}
    Si la série entière \( \sum_{n\geq 0}a_nz^n\) a un rayon de convergence \( R\) alors pour tout \( z_0\in B(0,R)\), on pose
    \begin{subequations}
        \begin{align}
...
\begin{theorem}[\cite{GYDXooJJusGH,MonCerveau}]
