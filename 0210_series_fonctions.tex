...
L'exponentielle est déjà définie par le théorème \ref{ThoRWOZooYJOGgR}. Dans cette section nous faisons les choses suivantes.
\begin{enumerate}
    \item
        Définir le logarithme comme primitive de \( x\mapsto \frac{1}{  x }\) s'annulant en \( x=1\).
    \item
        Montrer que la fonction réciproque du logarithme vérifie l'équation différentielle \( y'=y\), \( y(0)=1\).
\end{enumerate}

Cela pourrait être utilisé pour définir l'exponentielle par son équation différentielle et montrer l'existence par le fait que la réciproque du logarithme donne une solution.  Ce programme serait cependant très lourd car il utilise entre autres la densité des polynômes dans les fonctions continues (pour démontrer que toutes les fonctions continues ont une primitive). Mais \emph{à part ça}, ce programme est relativement simple.

C'est pour cette raison que nous prenons la série comme définition de l'exponentielle\footnote{Surtout que pour les exponentielles d'opérateurs, il n'y a pas tellement le choix.} et son équation différentielle comme propriété. 
...
\begin{equation}    \label{EqTJZPooBmojAo}
\ln\colon \mathopen] 0 , \infty \mathclose[\to \eR.
\end{equation}
...
La fonction logarithme étant une bijection, elle possède une réciproque que nous notons \( f\). Nous avons alors l'égalité pour tout \( x\in \eR\) :
\begin{equation}
    \ln\big( f \big)=x.    
\end{equation}
...
\begin{equation}    \label{EqNQDDooYBbARn}
    \frac{ f'(x) }{ f(x) }=1,
\end{equation}
et donc \( f\) est une solution de l'équation différentielle \( y'=y\), \( y(0)=1\). Par unicité\footnote{Proposition \ref{PropDJQSooYIwwhy}.}, nous avons \( f=\exp\).
