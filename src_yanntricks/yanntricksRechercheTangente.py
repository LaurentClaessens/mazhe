...
def VGZooJnvvZc():
    ...
    pspictDetail.DrawGraphs(corde, v1, v2, h1, h2, h3,
                            f, P, Px, Py, Q, Qx, Qy, Dx, Dy)

    for psp in pspictsSubFig:
        psp.mother.caption = r"\ldots de mieux en mieux \ldots"
    pspictsSubFig[0].mother.caption = r"Pas très non \ldots"
    pspictsSubFig[-1].mother.caption = r"\ldots presque parfait"
    ...
    for i, psp in enumerate(pspictsSubFig):
        ...
        Qi.put_mark(0.3, Qi.advised_mark_angle(pspicts)+180,
                    "$Q_{%s}$" % str(i), pspicts=pspicts, position="corner")
