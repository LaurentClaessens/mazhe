...
\begin{theorem}[\cite{ooCVEMooUxVWwU}]
...
\begin{proof}
    Nous commençons par considérer \( v\in \swD(\eR^d)\) (fonction \(  C^{\infty}\) à support compact). En utilisant la formule d'inversion de Fourier \eqref{EQooWBZTooPeBNeh}
    \begin{subequations}
        \begin{align}
            (\gamma_0 v)(x')&=v(x',0)=\frac{1}{ 2\pi }\int_{\eR^d}\hat v(\xi) e^{i\xi\cdot(x',0)}d\xi\\
            &=\frac{1}{ 2\pi }\int_{\eR^{d-1}}\left( \int_{\eR}\hat v(\xi_1,\ldots, \xi_d) e^{i\xi'\cdot x'}d\xi_d \right)d\xi_1\ldots d\xi_{d-1}   \label{SUBEQooWQQIooWIJfsV} \\
            &=\frac{1}{ 2\pi }\int_{\eR^{d-1}}\left( \int_{\eR}\hat v(\xi',\xi_d)d\xi_d \right) e^{i\xi'\cdot x'}d\xi'       \label{SUBEQooEQYBooHwfUxb}
        \end{align}
    \end{subequations}
    Justifications :
    \begin{itemize}
        \item Pour \eqref{SUBEQooWQQIooWIJfsV} : séparation par Fubini de l'intégrale par rapport à \( \xi_d\). Ici \( \xi'\) est bien la restriction de \( \xi\) aux \( d-1\) premières composantes.
        \item Pour \eqref{SUBEQooEQYBooHwfUxb} : permutation des intégrales en utilisant le théorème de Fubini, et regrouper les \( d-1\) intégrales en une intégrale sur \( \xi'\in \eR^{d-1}\).
    \end{itemize}
    <++>
\end{proof}
<++>
