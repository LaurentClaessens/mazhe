...
\begin{proposition}[\cite{ooBYHTooRZtLLi}]      \label{PROPooMKAFooKDNTbO}
    L'ensemble \( \swD(\eR^d)\) est dense dans \( H^s(\eR^d)\).
\end{proposition}
...
\begin{remark}
    Pour qui a tout compris, cela peut sembler une évidence, mais nous précisions que nous parlons de densité de \( \swD(\eR^d)\) dans \( H^s(\eR^d)\), à aucun moment la topologie de \( \swD(\eR^d)\) n'entre en compte.

    Un peu moins évident : ce que nous avons réellement montré est la densité de \( \iota\big( \swD(\eR^d) \big)\) dans \( H^s(\eR^d)\) où \( \iota\) est l'application «prise de classe». Nous n'avons pas insisté là-dessus, mais il faut dire que dans la preuve de la proposition \ref{PROPooMKAFooKDNTbO}, \( u\) est un représentant d'un élément choisit dans \( H^s(\eR^d)\). Nous avons ensuite prouvé la convergence \( \| \varphi_n-u \|_{H^s(\eR^d)}\to 0\) (au sens de l'opération \( \| . \|_{H^s}\) --qui n'est pas une norme--  sur un espace de fonctions), et nous en avons déduit la convergence \( \varphi_n\stackrel{H^s(\eR^d)}{\longrightarrow}u\) où maintenant \( \varphi_n\) et \( u\) sont des classes dans \( H^s(\eR^d)\).
\end{remark}
