...
\begin{definition}
	If $\Delta$ and $\epsilon$ are the usual co-product and co-unit\footnote{See subsection \ref{SubSecHoptUnivecvgp}} on $\mU(\lB)$, then a \defe{twist based on}{twist!based on $\mU(\lB)$} based on $\mU(\lB)$ is an element $F\in\mU(\lB)\dcr{h}\otimes\mU(\lB)\dcr{h}$ such that
	\begin{subequations}
		\begin{align}
...
The \defe{formal extension lemma}{formal!extension lemma} can now be proved. From the point of view of the star products, it is the
\begin{theorem}
