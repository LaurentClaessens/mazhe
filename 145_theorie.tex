...
\begin{proof}
    ...
    En ce qui concerne les formules relatives à la matrice orthogonale \( Q\) nous savons par la proposition \ref{ooOWMBooHUatNb}\ref{ITEMooOWMBooHUatNb} qu'une matrice orthogonale est une bijection de l'ensemble \(  \{ x\in \eR^n\tq \| x \|=1 \}  \). Par conséquent
    \begin{equation}
...
Soit une matrice inversible \( A\in \GL(n,\eC)\).
\begin{proposition}
    Soit une matrice inversible \( A\), et \( \mu_1\leq\ldots\leq \mu_n\) les valeurs propres de \( A^*A\)<++>
\end{proposition}
<++>
...
\item
    Dans le cas de la méthode de Newton nous avons \( p=2\). Mais les premières valeurs de \( p_i\) peuvent être aussi bien \( 0\) que \( 7\). Après quelque itérations pourtant les \( p_i\) se regroupent autour de \( 2\).
\end{enumerate}
En tout cas, le plus important est de savoir si \( p>1\) ou non. Rappel : nous voulons la superlinéarité parce que nous voulons utiliser le test d'arrêt de la différence entre deux termes.
...
\begin{definition}
    Le nombre \( \alpha\) est une \defe{racine simple}{racine!simple} de \( f(x)=0\) si \( f(\alpha)=0\) et \( f'(\alpha)\neq 0\). Le nombre \( \alpha\) est une \defe{racine multiple}{racine!multiple} d'ordre \( r\) de \( f(x)=0\) si
    \begin{equation}
...
Il se fait que si une racine est multiple, alors l'ordre de convergence de la méthode de Newton est \( 1\).
...
\begin{normaltext}
    ...
    Écrivons la série de Taylor autour de \( x_n\) :
    \begin{equation}
        f(\alpha)=f(x_n+\theta)=f(x_n)+\alpha f'(x_n)+\frac{ \theta^2 }{ 2 }f''(\xi)
    \end{equation}
    où \( \xi\in\mathopen[ \alpha , x_n \mathclose]\). En oubliant un peu le reste nous avons
    \begin{equation}
        f(x_n+\theta)\simeq f(x_n)+\theta f'(x_n).
    \end{equation}
    Mais le terme de gauche est nul, donc
    \begin{equation}
        \theta\simeq -\frac{ f(x_n) }{ f'(x_n) }.
    \end{equation}
    Cela n'est pas l'erreur que nous avons, mais une approximation de l'erreur. Mais \( \alpha=x_n+\theta\), donc en prenant une approximation de \( \theta\) nous avons une approximation de \( \alpha\) :
    \begin{equation}
        x_{n+1}=x_n-\frac{ f(x_n) }{ f'(x_n) }.
    \end{equation}
    Cette dernière formule est la méthode de Newton.
\end{normaltext}
