...
\begin{proposition}
    Un espace métrique séparable\footnote{Qui possède une partie dense dénombrable, définition \ref{DefUADooqilFK}.} a une base de topologie dénombrable.
\end{proposition}

\begin{proof}
    Soit \( A\) dense et dénombrable dans l'espace métrique \( (E,d)\). Nous notons \( \{ a_i \}_{i\in \eN}\) une énumération de \( A\) et \( \{ r_i \}_{i\in \eN}\) une énumération de \( \eQ\). Nous allons montrer que
    \begin{equation}
        \mB=\{ B(a_i,r_j) \}_{i,j\in \eN}
    \end{equation}
    est une base de la topologie de \( E\). Soit \( x\in E\) et \( V\) un voisinage de \( x\). Ce dernier contient une boule \( B(x,r)\) et quitte à prendre \( r\) un peu plus petit nous supposons que \( r\in \eQ\).
    ...
    \end{equation}
    La seconde inégalité est stricte parce que les boules sont ouvertes. Le tout montre que \( y\in B(x,r)\). Par ailleurs \( x\in B(a,\frac{ 2 }{ 3 }r)\) et nous avons trouvé un élément de \( \mB\) contenant \( x\) tout en étant inclus à \( V\). Cela prouve que \( \mB\) est bien une base de la topologie pour \( E\).
\end{proof}
