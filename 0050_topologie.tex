...
\begin{definition}[Limite d'une fonction]\label{DefXGiOZVj}\label{DefYNVoWBx}
    Soient \( X\) et \( Y\) des espaces topologiques, \( A\subset X\) et \( a\in\bar A\). L'élément \( y\in Y\) est une \defe{limite}{limite!d'une fonction} de \( f\) en \( a\) si pour tout voisinage \( V\) de \( y\), il existe un voisinage \( W\) de \( a\) dans \( X\) tel que 
    \begin{equation}
        f(W\cap A)\subset V.
    \end{equation}
...
\begin{definition}[Fonction continue]\label{DefOLNtrxB}
    Une fonction \( f\colon X\to Y\) est \defe{continue au point}{continue!fonction!en un point} \( a\in X\) si \( f(a)\) est une limite\footnote{Définition \ref{DefXGiOZVj}.} de \( f\) en \( a\).
...
La définition suivante donne une topologie sur les espaces métriques en partant des boules et en reprenant mot à mot la définition \ref{DefUOyCQtW}. Par construction, les boules sont une base de la topologie\footnote{Définition \ref{DefQELfbBEyiB}.} d'un espace métrique.
...
\begin{proof}
    D'abord \( \emptyset\in\mT\) parce que tout élément de l'ensemble vide \ldots heu \ldots enfin parce que d'accord hein.
...
\begin{definition}      \label{DefHTGYFpT}
    La topologie mise sur \( E\) par le théorème \ref{ThoORdLYUu} est la \defe{topologie métrique}{topologie!métrique} de \( (E,d)\). En présence d'une distance, sauf mention explicite du contraire, c'est toujours cette topologie-là que nous utiliserons.
\end{definition}
...
\begin{theorem}[Limite et continuité]
...
\begin{proof}
    L'équivalence \ref{ItemYNQpikri} \( \Leftrightarrow\) \ref{ItemYNQpikrii} est la définition \ref{DefOLNtrxB} de la continuité en un point couplée à l'unicité de la limite due à la proposition \ref{PropFObayrf} parce qu'un espace métrique est séparé.
    ...
    Nous prouvons enfin \ref{ItemYNQpikriii} \( \Rightarrow\) \ref{ItemYNQpikrii}. 
\end{proof}
