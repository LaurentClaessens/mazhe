...
A very instructive introduction to the (matricial) structure of the Lorentz group can be found in \cite{Schomblond_em}.
...
See section \ref{sec_Sobolev} for general introduction to Sobolev spaces. We are here interested in $H^{1/2}$, and more precisely, in the pseudo differential operator of example at page \pageref{pg_exem_psdo}. Using all that, here is some ways to write the scalar product in $H^{1/2}$ 
\begin{equation}
\begin{split}
