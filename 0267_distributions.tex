...
\end{proof}
%AFAIRE : teminer en utilisant le fameux corrolaire et en précisant que S est complet et métrisable.

\begin{proposition}
    Soit \( (T_t)\in  C^{\infty}\big( I,\swS'(\Omega) \big)\). L'application \( T\colon \swS(I\times\Omega)\to \eR\) donnée par
    \begin{equation}
        T(\psi)=\int_IT_t(\psi_t)dt
    \end{equation}
    est une distribution. 
\end{proposition}
%AFAIRE : la preuve

\begin{lemma}   \label{LemUDikaVz}
    Si \( u\in C^{\infty}\big( \eR,\swS'(\eR^d) \big)\) alors l'application \( u\colon \swS(\eR\times \eR^d)\to \eR\) donnée par la formule
    \begin{equation}
        u(\psi)=\int_{\eR}u_t(\psi_t)dt
    \end{equation}
    est bien définie et donne un élément de \( \swS'(\eR\times \eR^d)\). C'est à dire que nous avons une application \(  C^{\infty}\big( \eR,\swS'(\eR^d) \big)\to\swS(\eR\times \eR^d)\).
\end{lemma}
%AFAIRE : la preuve
