...
L'espace de Schwartz \( \swS(\Omega)\) est défini dans la définition \ref{DefHHyQooK}; sa topologie y est discutée.
\begin{definition}
...
Quelque propriétés de dérivation des fonctions \( I\to \swD(\Omega)\) seront directement énoncées et démontrées dans le cas des distributions tempérées. Les résultats \ref{PropGKoPbko} et \ref{PropUDkgksG} sont a fortiori valables si on remplace \( \swS\) par \( \swD\).
