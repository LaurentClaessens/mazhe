...
\begin{lemma}[de Gauss\cite{KXjFWKA}]
    Soient \( P,Q\in \eZ[X]\). Alors \( c(PQ)=c(P)c(Q)\).
\end{lemma}
\index{lemme!de Gauss!contenu de polynôme}

\begin{proof}
    Nous posons \( P_1=\frac{1}{ c(P) }P\) et \( Q_1=\frac{1}{ c(Q) }Q\). Ce sont encore des éléments de \( \eZ[X]\) parce \( c(P)\) est un diviseur commun à tous les coefficients de \( P\) ( et idem pour \( Q\)). De plus maintenant nous avons \( c(P_1)=c(Q_1)=1\).
    
    Supposons que \( c(P_1Q_1)>1\); dans ce cas nous avons un nombre premier \( p\) qui le divise et qui divise donc tous les coefficients de \( P\). En effet
