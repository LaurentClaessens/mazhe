...
\begin{proposition}[Critère de Cauchy uniforme\cite{LCbyNWQ}]   \label{PropNTEynwq}
    Soit \( (Y,d)\) un espace topologique complet et \( X\) un espace topologique. La suite de fonction \( f_n\colon X\to Y\) converge uniformément sur \( A\) si et seulement si pour tout \( \epsilon>0\) il existe \( N\in \eN\) tel que si \( k,l>N\) alors
    \begin{equation}
...
\begin{proof}
    ...
    %TODO : il faudra mettre ici une référence vers le théorème qui dit que la convergence ponctuelle sur un compact implique la convergence uniforme.
\end{proof}
