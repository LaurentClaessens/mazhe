...
\ifthenelse{\value{siART}=1\or \value{siTHZ}=1}{It is easy to check that $r_{1}=e^{aJ_{2}}$ and $r_{2}=e^{bV}$ are solutions by noticing that the action of $u^{-1}r_{i}u$ leaves unchanged the first basis vector\footnote{We give in a complement a more intrinsic way to prove that result, see page \pageref{subSecAltreintr}.}.}{%
It is easy to see that $J_{2}$ once again belongs to the Lie algebra of the stabilizer. The new element for $AdS_{4}$ is $V$. Indeed $V^{3}=0$, so the exponential is a three term sum:
\[
   e^{aV}=
\begin{pmatrix}
b+1&0&-b&0&a\\
...
Solutions of these equations\footnote{from now until the determination of symplectic forms, all results are computed by Maxima~\cite{Maxima}.}, parametrized by reals $r$ and $s$ and the corresponding commutators are listed below.
 ...
 The next step is to determine which of these algebras admit a compatible symplectic structure%
\ifthenelse{\value{siART}=1}{. }{ in the sense of definition \ref{DefSympleAlg}. }
...
&[c,d]&=c.
\end{align*}
\ifthenelse{\value{siART}=1 \or \value{siTHZ}=1}{}{%
...
}			 %fin du siART à propos de la métrique
...
\end{proof}
}		% Fin du siTHZ sur la dem que R_3 n'est pas surjective.
...
\ifthenelse{\(\value{siART}=1\) \OR \(\value{siTHZ}=1\)}{.}{, see equation \eqref{EqDefcochaintrivC}.}\
 We pose
\[ 
...
\ifthenelse{\value{siART}=1}{}{ (see appendix \ref{SecAppMaxima})} gives the result
