...
\begin{theorem}[\cite{ooRJDSooXpVtMD}]
...
Nous avons discuté dans la section \ref{SECooLQVJooTGeqiR} de ce qui arrive au corps lorsqu'on l'étend\footnote{En restant dans les plus strictes convenances de la mathématique.}. Dans cette sections nous allons étudier ce qui arrive aux applications linéaires entre deux \( \eK\)-espaces vectoriels lorsque nous étendons le corps \( \eK\) en un corps \( \eL\).

Soit donc un corps \( \eK\) et deux \( \eK\)-espaces vectoriels \( E\) et \( F\).
...
Cela dit, nous entrons dans le vif du sujet\footnote{Le sujet étant le corps étendu.}. Soit \( \eK\) un corps (commutatif) et une extension \( \eL\) de \( \eK\). Soient \( E\) et \( F\), des \( \eK\)-espaces vectoriels de dimension finie. 
...
\begin{definition}
    ...
    \end{equation}
    Ce dernier est le quotient \( \eL\otimes_{\eK}E=(\eL\times E)/~\) par la relation d'équivalence
    \begin{equation}
        (\lambda,v)\sim\big( a\lambda,\frac{1}{ a }v \big)
    \end{equation}
    pour tout \( a\in \eK\). Nous noterons \( [\lambda,v]\) la classe de \( (\lambda,v)\).
\end{definition}
...
\begin{proposition}[\cite{ooEPEFooQiPESf}]
...
\begin{proof}
    ...
    Réciproquement, nous supposons que \( f\) est injective et nous prouvons que \( f_{\eL}\) est injective.
\end{proof}
<++>
