...
\begin{proof}
    ...
    Nous prouvons maintenant que ces affirmations sont équivalentes au fait que \( f\) soit cyclique. Nous allons prouver que (sous l'hypothèse que \( f\) soit diagonalisable) \( f\) a \( n\) valeurs propres distinctes si et seulement si \( f\) est cyclique.
    \end{subproof}
    D'abord si \( f\) n'a pas \( n\) valeurs propres distinctes, prenons une base de \( E\) dans laquelle la matrice de \( f\) est diagonale et pour laquelle la valeur propre de \( e_1\) et \( e_2\) sont égales. Un vecteur \( v\) s'écrit sous la forme \( v=ae_1+be_2+w\) où \( w\) est une combinaisons de \( e_3,\ldots, e_n\). L'application de \( f^k\) donnera 
    \begin{equation}
        f^k(v)=\lambda_1^k(ae_1+be_2)+w'
    \end{equation}
    où \( w'\) est encore une combinaison de \( e_3,\ldots, e_n\). Tout cela ne peut pas engendrer \( \Span\{ e_1,e_2 \}\) vu que la projection de \( f^k(v)\) sur ce sous-espace est toujours la même chose. Le vecteur \( v\) ne peut donc pas être cyclique. Donc si \( f\) est cyclique, il possède \( n\) valeurs propres distinctes.

    Dans le sens inverse nous supposons que toutes les valeurs propres soient distinctes, et prouvons que le vecteur \( v=e_1+\ldots +e_n\) est cyclique. Nous avons
    \begin{equation}
...
\begin{example}
    ...
    \end{pmatrix}\), et dans ce cas n'importe quelle combinaison \( ae_i+be_j\) reste proportionnelle à elle-même après application de \( f\). Si nous avons des valeurs propres différentes par contre, nous avons par exemples dans \( \eR^2\) la matrice \( \begin{pmatrix}
        1    &   0    \\ 
        0    &   2    
    \end{pmatrix}\) et donc \( f(e_1+e_2)=e_1+2e_2\). La partie \( \{ e_1+e_2,e_1+2e_2 \}\) est une base.
\end{example}
