...
Lisez le fichier \info{COMPILATION.md}. Il est plus à jour; en particulier, il tient compte du fait que \info{latexparser} n'existe plus.
...
Parfois le genre d'un objet n'est pas bien défini. Par exemple \( 3/4\) est \emph{la} classe d'équivalence de \( (3,4)\) dans \( \eZ\times \eZ\setminus{0}\); mais en même temps c'est \emph{un} élément de \( \eQ\). Nous utiliserons alors, prudemment, un neutre en disant «\emph{il} est plus grand que \( 1\)».
