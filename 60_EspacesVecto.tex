...
\begin{proof}
    ...
    \item[\( \psi\) est injective]
        ...
        Supposons que \( g,h\in\comC(f)\) tels que \( \psi(g)=\psi(h)\). Vu que \( f\) est diagonalisable nous pouvons décomposer \( x\in  E\) en ses composantes sur les espaces propres :
        \begin{equation}
...
\begin{lemma}[\cite{ooRJDSooXpVtMD}]
    Soit un endomorphisme cyclique \( f\colon E\to E\). Alors il existe \( a\in E\) tel que \( \mu_a=\mu\).
\end{lemma}

\begin{proof}
    Soit \( v\) un vecteur cyclique : \( \{ f^k(v) \}_{k=0,\ldots, n-1}\) est une base de \( E\).
\end{proof}
<++>

\begin{proposition}
    Si \( f\) est un endomorphisme diagonalisable de \( E\) nous avons équivalence entre les faits suivants.
    \begin{enumerate}
        \item
            \( \comC(f)=\eK[f]\).
        \item
        ...
        \item
            \( f\) est cyclique.
        \item
            Les degré du polynôme caractéristique de \( f\) est la dimension de \( E\).
    \end{enumerate}
\end{proposition}
