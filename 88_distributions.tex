...
\end{equation}
Sachant que \( \arctg(x)\) est une primitive de \( \frac{1}{ x^2+1 }\) et avec encore un peu de changement de variables, nous avons
\begin{equation}
    \int_{\eR}\hat f_{\epsilon}(k)dk=\int_{-\infty}^{\infty}\frac{ 2\epsilon }{ k^2+\epsilon^2 }=2[\arctg(x/\epsilon)]_{-\infty}^{\infty}=2\pi.
\end{equation}
Cela montre que si nous introduisons\cite{ooKWQPooPhWNkI} la fonction \( \delta_{\epsilon}\) donnée par
\begin{equation}
