...
s'écrit sous forme paramétrique (avec $\varphi \in \RR$)~:
\begin{equation*}
  \begin{cases}
    x = R + r\cos \varphi\\
    y = 0\\
  ...
  \end{cases}
\end{equation*}
Considérons la rotation de $\RR^3$ d'angle $\theta$ autour de l'axe
...
l'action de $R_\theta$ ($\theta \in \RR$)~:
\begin{equation*}
%   \begin{pmatrix}
%     x\\y\\z
%   \end{pmatrix}
...
\begin{equation}\label{torus_parameq}
  ...
  \begin{cases}
    ...
    z = r \sin \varphi
  \end{cases} \qquad \text{où $\varphi,\theta \in \RR$}
\end{equation}
...
\begin{quote}
  ...
  fonctionne que d'un ouvert de $\RR^n$ dans un ouvert de $\RR^n$,
  avec le même ``$n$''. Le tore ``vit'' dans $\RR^3$ mais n'en est pas un
...
un ouvert $\tilde W \subset \RR^+_0 \times \RR \times \RR$ autour de
$(r, \varphi_0, \theta_0)$ et un ouvert $\tilde U \subset \RR^3$ autour
...
\begin{equation*}
  F_{p_0} : W \subset \RR^2 \to \RR^3 : (\varphi,\theta) \mapsto F(r,\varphi,\theta)
\end{equation*}
où $W = \{ (\varphi,\theta) \in \RR^2 \telque (r,\varphi,\theta) \in \tilde
...
\item $F_{p_0}$ est de classe $C^1$ (comme application d'un ouvert de
  $\RR^2$ dans $\RR^3$) car est composée d'applications
