...
\begin{proposition}\label{propn:series_propdebase}
...
\begin{exemple}\label{exemples:series}
\begin{enumerate}
...
\begin{equation}		\label{eq:seriepuissance}
...
\end{theorem}
Le disque $| z-z_0 |\leq R$ est le \defe{disque de convergence}{Disque de convergence} de la série \eqref{eq:seriepuissance}. Notez que ce théorème ne dit rien pour les points tels que $| z-z_0 |=R$. Il faut traiter ces points au cas par cas. Et le pire, c'est qu'une série donnée peut converger pour certain des points sur le bord du disque, et diverger en d'autres.  Il y a un dessin à la figure \ref{LabelFigDisqueConv}.
...
Le recueil d'exercice de deuxième année\footnote{\url{http://student.ulb.ac.be/~lclaesse/CdI2.pdf}} contient beaucoup de choses intéressantes aussi. 
...
\begin{theoreme}		\label{ThoRapPropDiffSi}
...
\begin{exemple}		\label{Exemple0046Diff}
...
\begin{exemple}Pour $f(x,y) = xy + x^2$, les dérivées partielles
  s'écrivent
  \begin{equation*}
...
%---------------------------------------------------------------------------------------------------------------------------
					\subsection{Intégrale sur un rectangle}
%---------------------------------------------------------------------------------------------------------------------------
...
L'ensemble sur lequel nous intégrons est donné par le produit cartésien d'intervalles $E=[0,1]\times[0,2]$. Le \defe{théorème de Fubuni}{Théorème!Fubini} montre que nous pouvons intégrer séparément sur l'intervalle horizontal et vertical :
\begin{equation}
...
La méthode de Fubini ne fonctionne plus parce que le domaine n'est pas un rectangle. Nous allons donc utiliser l'astuce décrite à la page 462. Considérons le domaine \begin{equation}
	E=\{ (x,y)\in\eR^2\tq a<x<b\text{ et } \alpha(x)<y<\beta(x) \}
\end{equation}
représenté sur la figure \ref{LabelFigIntEcourbe}.
...
En guise d'exemple, montrons comment intégrer la fonction $f(x,y)=\sqrt{1-x^2-y^2}$ sur le domaine délimité par la droite $y=x$ et le cercle $x^2+y^2=y$, représenté sur la figure \ref{LabelFigIntBoutCercle}.
...
\begin{equation}\label{eq:flux-star}
...
\end{equation*}
et grâce à cette paramétrisation l'intégrale \eqref{eq:flux-star}
devient
\begin{equation*}
...
Cette intégrale est en général très compliquée à calculer parce qu'il faut trouver le champ de vecteur normal, puis une paramétrisation de la surface $\partial D$ et ensuite appliquer la méthode décrite au point \ref{sec:intsurfaciques}. 
