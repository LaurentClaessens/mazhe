...
En un point $(a,b)$ avec $a \neq b$, la fonction est constante
  ...
  \begin{equation*}
    ...
    \begin{arrowcases}
      ...
      \limite[x = y] {(x,y)} {(a,a)} f(x,y) = \exp{a-a^2}
    \end{arrowcases}
  \end{equation*}
  Donc la fonction $f$ est continue en $(a,a)$ si et seulement si $a = a^2+1$, c'est à dire jamais.


\item La \og zone à problèmes\fg{} est formée de points du type $(\exp
  b,b)$. En dehors, la fonction est différentiable.

  Le calcul de limite montre que la fonction est continue en $(\exp
  b,b)$ si et seulement si $\exp b + b = 0$ ; cette équation possède
  une unique solution, notons-la $b_0$. Clairement $b_0 < 0$.

  Pour vérifier la différentiabilité en $(\exp{b_0},b_0)$, calculons
\begin{equation}
	\begin{aligned}[]
    \pder f {(1,0)}(\exp{b_0},b_0) &= \limite[h \neq 0] h 0 \frac{f(\exp{b_0}+h,b_0) -f(\exp{b_0},b_0)}h\\
	 &=
    \begin{arrowcases}
      \limite[h > 0] h 0 \frac{1 - 1}h = 0\\
      \limite[h < 0] h 0 \frac{1 - (\exp{b_0} + h + b_0) - 1}h = - 1
    \end{arrowcases}
	\end{aligned}
\end{equation}
  où on a bien sûr utilisé le fait que $\exp{b_0} + b_0 = 0$. Étant
