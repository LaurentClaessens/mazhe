...
En remplaçant $y$ dans l'équation par $f(t) = t^4 \exp {2t}$,
...
\begin{equation}\label{eq:yplusexpy}
y(t) + \exp{y(t)} = \sin(t) + K
\end{equation}
...
\begin{lemma}
La fonction $f : \eR \to \eR : z \mapsto z + \exp z$ est une
bijection.
\end{lemma}
\begin{proof}
...
(\ref{eq:yplusexpy}) sous la forme
\begin{equation*}
f(y(t)) = \sin(t) + K
\end{equation*}
...
\end{equation*}
c'est-à-dire $y^2 = -1 + K \exp{-2 \cos(t)}$ pour une certaine
constante (positive) $K$. Selon la valeur de $K$, ces solutions sont
définies ou non sur tout $\eR$ :
\begin{enumerate}
\item Si $K > \exp{2}$, le membre de droite est strictement positif
pour tout $t$ et on peut en prendre la racine (solution sur $\eR$)
\item Si $K < \exp{2}$, le membre de droite est négatif pour
certaines valeurs de $t$ et on ne peut pas en prendre la racine.
\item Si $K = \exp{2}$, le choix d'une racine du membre de droite
...
\begin{equation*}
F : \eR^3 \to \eR : (x,y,z) \mapsto z \exp z - x - y.
\end{equation*}
...
de $Z$, ce qu'on fait en utilisant la relation $Z \exp Z = x+y$~:
\begin{equation*}
\begin{split}
\partial_x Z (1 + Z) \exp Z = 1 \Rightarrow \partial_x Z = \frac{\exp {-Z}}{1+Z}\\
\partial_y Z (1 + Z) \exp Z = 1 \Rightarrow \partial_y Z = \frac{\exp {-Z}}{1+Z}\\
\end{split}
\end{equation*}
...
\begin{equation*}
F : \eR^2 \to \eR : (x,y) \mapsto \exp{xy} - 1 - x^2 - y
\end{equation*}
...
\begin{equation*}
\exp{x Y} (Y + x Y^\prime) = 2 x + Y^\prime
\end{equation*}
...
\begin{equation*}
\exp{x Y} (Y + x Y^\prime)^2 + \exp{x Y} (Y^\prime + Y^\prime + x
