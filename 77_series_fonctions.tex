...
\begin{theorem}[Théorème de Müntz\cite{jqZSyG,oYGash}]  \label{ThoAEYDdHp}
    Soit \( C=C_0\big( \mathopen[ 0 , 1 \mathclose] \big)\), l'espace des fonctions continues sur \( \mathopen[ 0 , 1 \mathclose]\) muni de la norme \( \| . \|_{\infty}\) ou \( \| . \|_2\) et une suite \( (\alpha_n)\) strictement croissante de nombres strictement positifs. Nous notons \( \phi_{\lambda}\) la fonction \( x\mapsto x^{\lambda}\).

    Alors l'espace \( \Span\{1, \phi_{\alpha_n} \}\) est dense dans \( C\) si et seulement si \( \sum_{n=1}^{\infty}\frac{1}{ \alpha_n }\) diverge.
\end{theorem}

\begin{probleme}
    Dans le rapport du jury d'agrégation 2014, il est dit que c'est \( \Span\{ 1,\phi_{\alpha_i} \}\) qui doit être dense. Je ne vois pas où dans la démonstration la fonction \( 1\) doit être traitée séparément.
\end{probleme}
...
\begin{remark}
    Certaines sources\footnote{Dont \cite{jqZSyG,oYGash}, encore qu'ils ne sont pas très explicites sur le fait que \( \alpha_i\) puisse ou non être nul.} citent le théorème de Müntz en disant
    \begin{equation}        \label{EQooPCSZooUDSzwQ}
        \overline{ \Span\{ \phi_{\alpha_i} \} }=C\big( \mathopen[ 0 , 1 \mathclose] \big) \Leftrightarrow \sum_{i}\frac{1}{ \alpha_i }=+\infty.
    \end{equation}
    Que penser de la présence explicite du \( 1\) (c'est à dire de \( \phi_0\)) ou non dans l'ensemble ?

    Si \( \Span\{ 1,\phi_{\alpha_i} \}\) est dense alors l'ensemble \( \Span\{ \phi_{\beta_i} \}\) où \( \beta_1=0\) et \( \beta_i=\alpha_{i-1}\) est également dense (c'est le même ensemble).  Cependant l'énoncé \eqref{EQooPCSZooUDSzwQ} ne permet pas de conclure à la divergence de \( \sum_{i=0}^{\infty}\frac{1}{ \beta_i }\) parce que \( \beta_0=0\). Et même si nous pouvions le conclure, nous ne pourrions pas dire «en enlevant un terme, ça reste divergent» parce que le terme à enlever pour passer à la somme des \( \frac{1}{ \alpha_i }\) est \( \frac{1}{ 0 }\) avec lequel il ne faut pas rigoler.
\end{remark}
