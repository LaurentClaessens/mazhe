...
Let us now turn our attention to the symplectic matter. There are two symplectic constraint on the choice of the $\mfs_i$. The first one is that each one must be a \defe{symplectic Lie algebra}{symplectic!Lie algebra}\quext{À vérifier : Yannick dit que la condition \eqref{eq:symple_Lie} revient à dire que la forme est fermée au sens de Chevalley, et que donc sa prolongation invariante à gauche sur le groupe est fermée pour de Rahm.} : if $\Omega_i$ is the symplectic $2$-form on $\mfs_i$, then for any $x$, $y$, $z\in\mfs_i$,
%
...
} % Fin du THZ sur l'application de la méthode avec la vielle Iwasawa.
