...
\begin{proposition}     \label{PropNCMToWI}
    Deux fonctions continue égales presque partout pour la mesure de Lebesgue sont égales.
\end{proposition}
...
Nous rappelons qu'une série de nombres \( \sum_{n=0}^{\infty}a_n\) converge \defe{absolument}{convergence!absolue} si la série $\sum_{n=0}^{\infty}| a_n |$ converge. Cette définition s'étend immédiatement aux séries dans n'importe quel espace normé.
...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Logarithme de matrice}
%---------------------------------------------------------------------------------------------------------------------------

%TODO : mettre sans doute ici le théorème qui dit que l'exponentielle est surjective ou un truc du genre.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
La convergence de la série n'est pas un problème : elle converge même normalement :
\begin{equation}
    \frac{ \| A-\mtu \|^{k+1} }{ k+1 }\leq \| A-\mtu \|^{k+1},
\end{equation}
qui est la série harmonique qui converge parce que nous avons supposé que \( \| A-\mtu \|<1\). Afin de voir que cette application est de classe \( C^1\), nous posons
\begin{equation}
    \begin{aligned}
        u_k\colon \eM(n,\eR)&\to \eM(n,\eR) \\
        A&\mapsto (-1)^{k+1}\frac{ (A-\mtu)^{k} }{ k}.
    \end{aligned}
\end{equation}
La différentielle s'exprime de la façon suivante :
\begin{subequations}
    \begin{align}
        (du_k)_{A}(U)&=\frac{ d  }{ d s }\Big[ u_k(A+sU) \Big]_{s=0}\\
        &=\frac{ d  }{ d s }\left.(-1)^{k+1}\frac{ (A+sU-\mtu)^k }{ k }\right|_{s=0}\\
        &=\frac{ (-1)^{k+1} }{ k }\frac{ d  }{ d s }\left.\sum_{a+b+c=k}(A-\mtu)^a(sU)^b(A-\mtu)^c\right|_{s=0} \\
        &=\frac{ (-1)^{k+1} }{ k }\sum_{l=0}^{k+1}(A-\mtu)^{l}U(A-\mtu)^{k-1-l}.
    \end{align}
\end{subequations}
Donc 
\begin{equation}
    \| (du_k)_A \|\leq\frac{1}{ k }\sum_{l=0}^{k-1}\| A-\mtu \|^{k-1}=\| A-\mtu \|^{k-1},
\end{equation}
dont la série converge. Le théorème \ref{ThoLDpRmXQ} nous dit alors que la fonction \( \ln\) est de classe \( C^1\) sur l'ensemble des \( A\) tels que \( \| A-\mtu \|<1\).

\begin{lemma}   \label{LemQZIQxaB}
    Si \( \| m \|<1\) dans \( \eM(n,\eR)\), alors nous avons
    \begin{equation}    \label{EqIKgMabb}
        \ln(\mtu+m)=\sum_{k=0}^{\infty}(-1)^k\frac{ m^{k+1} }{ k+1 }
    \end{equation}
    et cela est un vrai logarithme au sens où
    \begin{equation}
        e^{\ln(\mtu+m)}=\mtu+m.
    \end{equation}
    De plus nous avons
...
Vu que \( \mL_G\) est un sous-espace vectoriel de \( \GL(n,\eR)\), nous pouvons considérer un supplémentaire \( M\).
...
\begin{theorem}[Von Neumann\cite{KXjFWKA]}
