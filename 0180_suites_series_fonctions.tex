...
\begin{proof}
    ...
    Donc \( S\) est une tribu qui contient tous les ensembles de la forme \( \mathopen] a , \infty \mathclose]\). Ces derniers formant une base de la topologie de \( \eR\), la tribu \( S\) contient tous les ouverts de \( \eR\) et donc tous les boréliens de \( \eR\).
\end{proof}
<++>

\begin{lemma}[\cite{NBoIEXO}]
...
\begin{theorem}[\cite{NBoIEXO}\footnote{Modèle non contractuel : des notations et la définition de \( \lambda\)-système peuvent varier entre la référence et le présent texte.}]
...
\begin{proof}
    ...
    \begin{subproof}
        \item[\( \lambda\)-système : différence ensembliste]
            ...
            \end{equation}
            et simulairement pour \( 1\leftrightarrow 2\). Les deux fonctions (de \( y\)) à droite étant mesurables, nous avons la mesurabilité de la fonction \( y\mapsto \mu_1\big( (B\setminus A)_1(y) \big)\).
        ...
        \item[\( \lambda\)-système : limite de suite croissante]
            ...
            \end{equation}
            est mesurable par la proposition \ref{PropFYPEOIJ}.
