...
\begin{proof}
    ...
    \begin{enumerate}
        \item
            La seconde égalité est seulement la définition \eqref{EqLjGYKK}. Nous ne devons que prouver la première. 
            ...
            \begin{remark}
                Nous savons que \( X^{n}-1\) est le produit des \( (X-z)\) où \( z\) parcourt les racines \( n\)\ieme de l'unité, et nous savons qu'il y en a \( n\). Notons juste pour le plaisir que dans le produit \( \prod_{d\divides n}\prod_{z\in\Delta_d}\), il y a bien \( n\) termes parce que \( \Card(\Delta_d)=\varphi(d)\) et \( \sum_{d\divides n}\varphi(d)=n\).
            \end{remark}
...
\begin{proof}
    Nous utilisons la formule de la proposition \ref{PropUImYnL}\ref{ItempnHhYk} en remarquant que seuls \( p\) et \( 1\) divisent \( p\) :
    \begin{equation}
