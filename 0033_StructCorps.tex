...
\begin{proposition}     \label{PropUImYnL}
    ...
    \begin{enumerate}
        \item   \label{ItempnHhYk}
            \( X^n-1=\prod_{d\divides n}\phi_d(X)=\prod_{d\divides n}\prod_{z\in \Delta_d}(X-z)=\prod_{\omega\in \gU_n}(X-\omega)\)
        \item
            \( \phi_n\in \eZ[X]\),
...
\begin{proof}
    ...
    \begin{enumerate}
        \item
            La seconde égalité est seulement la définition \eqref{EqLjGYKK}. Nous commençons par prouver la première.

            Nous connaissons l'union disjointe\footnote{Lemme \ref{LemKcpjee}.} \( \gU_n=\bigcup_{d\divides n}\Delta_d\) qui implique
            \begin{equation}
