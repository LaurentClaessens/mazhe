...
\begin{theorem}[Théorème de Bolzano-Weierstrass]		\label{ThoBolzanoWeierstrassRn}
	Toute suite contenue dans un compact d'un espace vectoriel normé admet une sous-suite convergente.
\end{theorem}
Ce théorème a déjà été démontré dans un espace topologique général par le théorème \ref{ThoBWFTXAZNH}.

\begin{proof}
    Quitte à passer aux composantes nous pouvons supposer que l'espace vectoriel normé est \( \eR^n\) muni de la norme usuelle. Soit $(x_n)$ une suite contenue dans une partie bornée de $\eR^m$. Considérons $(a_n)$, la suite réelle des premières composantes des éléments de $(x_n)$ : pour chaque $n\in\eN$, le nombre $a_n$ est la première composante de $x_n$. Étant donné que la suite $(x_n)$ est bornée, il existe un $M$ tel que $\| x_n \|<M$. En utilisant la relation \eqref{Equilequnorme}, nous avons
	\begin{equation}
		| a_n |\leq\| x_n \|\leq M.
	\end{equation}
...
\begin{proposition}		\label{PropSuiteCompactSScv}
    Une suite contenue dans un compact admet une sous-suite convergente.
\end{proposition}
Cela est le théorème de Bolzano-Weierstrass \ref{ThoBWFTXAZNH}. Une preuve
