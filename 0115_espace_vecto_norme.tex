...
\begin{proof}
    D'abord la série converge absolument\footnote{Définition \ref{DefDOaqApF}.} parce que \( \| A^k \|\leq \| A \|^k\)\footnote{La norme opérateur est une norme algébrique, proposition \ref{PropEDvSQsA}.} et que la série des \( \| A \|^k\) est la série géométrique qui converge. Par conséquent \( \sum_{k}A^k\) est une série absolument convergence et donc convergente par la proposition \ref{}.
    %AFAIRE : prouver que dans un espace complet une série qui converge absolument converge et noter ici que c'est là qu'on utilise le fait que E soit complet (et donc de Banach).
%  Dans le même mouvement, il faut prouver que si E est complet alors GL(E) est complet parce que c'est là-dedans qu'on utilise le coup de la convergence absolue.
