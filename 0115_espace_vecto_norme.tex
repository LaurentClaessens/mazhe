...
\begin{proposition}     \label{PropQAjqUNp}
    Soit \( E\) un espace vectoriel normé. Si \( A\in\aL(E,E)\) avec \( \| A \|<1\) pour la norme opérateur, alors \( (\mtu-A)\) est inversible et son inverse est donné par
    \begin{equation}
...
\begin{proof}
    D'abord la série converge absolument\footnote{Définition \ref{DefDOaqApF}.} parce que \( \| A^k \|\leq \| A \|^k\) et que la série des \( \| A \|^k\) est la série géométrique qui converge. Notons que c'est ici que nous avons utilisé le fait que la norme opérateur était une norme algébrique (proposition \ref{PropEDvSQsA}). Montrons à présent que la somme est l'inverse de \( \mtu-A\) :
    \begin{equation}
