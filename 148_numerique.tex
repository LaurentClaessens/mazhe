...
\begin{theorem}[Décomposition LU\cite{ooDANFooPSmBfd}]       \label{THOooUXKJooYaPhiu}
...
\begin{proof}
    Nous allons prouver par récurrence le fait suivant : pour tout \( 2\leq k\leq n\) il existe des matrices  \( E_i\) ($i=1,\ldots, k-1$) telles que en posant
    \begin{equation}
        A_k=E_{k-1}\ldots E_1A,
    \end{equation}
    \begin{itemize}
        \item \( E_i\) est une transformation gaussienne pour la \( i\)\ieme colonne,
        \item \( \Delta_k(A_k)\) est une matrice triangulaire supérieure.
        \item l'élément de matrice \( (A_k)_{kk}  \) est non nul.
    \end{itemize}
    
    La chose un peu dommage dans cette démonstration est que l'initialisation va être très ressemblante au pas de récurrence.
    \begin{subproof}
        \item[Initialisation : \( k=2\)]

            Vu que \( \Delta_1(A)\) est inversible, l'élément \( A_{11}\) est non nul. Il existe donc une transformation gaussienne \( E_1\) telle que la première colonne de la matrice \( A_2=E_1A\) soit nul sauf la première composante. En particulier \( \Delta_2(A_2)\) est triangulaire supérieure parce que \( (A_2)_{21}=0\).
