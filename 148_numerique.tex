...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
\section{Système linéaires}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

Soit un système d'équations linéaires \( Ax=b\) avec \( A\in \eM(n,\eR)\). Le problème est évidemment de savoir si il existe une unique solution \( x\) et de la déterminer. Nous supposons l'existence et l'unicité. C'est à dire que les conditions équivalentes sont vérifiées :
\begin{enumerate}
    \item
...
\begin{normaltext}
    ...
    Il faut garder en tête le fait que dans la pratique, la matrice \( A\) possède des millions de lignes et colonnes, si pas pire.
\end{normaltext}
...
\begin{probleme}
    Il faut vérifier si «creuse» est le bon mot en français, et donner une définition correcte.
\end{probleme}
...
\begin{remark}
    Pour une matrice de taille \( 10^{6}\), il y a \( 1000\) milliards d'entrées. Même pour la mémoire actuellement disponible, mémoriser \( 1000\) milliards de nombres, ce n'est pas rien.
\end{remark}
...
Une méthode directe consiste à transformer le système en un système équivalent, et encore et encore. Ce sont typiquement les méthodes du type pivot de Gauss et manipulations de lignes et colonnes. 

Ce type de méthodes transforme \( A^{(0)}x=b^{(0)}\) en \( A^{(1)}x=b^{(1)}\) dont la solution est identique. Et encore jusqu'à \( A^{(n-1)}x=b^{(n-1)}\) qui serait un système qui peut être immédiatement résolu, comme par exemple \( A^{(n-1)}\) serait diagonale.

Le problème de ce genre de choses est que si la taille de \( A\) est \( 10^6\), il faut faire de l'ordre de \( 10^6\) pas, chacun devant modifier de l'ordre de \( (10^6)^2\) éléments de matrice, plus les composantes de \( b\). Autant dire que ce n'est pas faisable.

Une méthode directe fonctionne en général avec un nombre de pas fixés par la taille du système. Par exemple pour un système \( n\times n\), la méthode de Gauss demande exactement \( n\) pas, et il n'y a pas moyen de faire mieux.
...
Une décomposition classique est la décomposition \( QR\) avec \( Q\) orthogonale.
...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Système d'immédiate résolution}
%---------------------------------------------------------------------------------------------------------------------------

Dans le cas des méthodes directes, nous nous arrêtons de changer le système lorsque la matrice est de résolution immédiate, c'est à dire lorsque la matrice est telle que la solution peut être immédiatement calculée.
...
La méthode de Gauss est souvent aussi appelée méthode «LU» qui va décomposer \( A=LU\) où \( L\) est triangulaire inférieure et \( U\) est triangulaire supérieure.
