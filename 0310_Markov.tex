...
\end{definition}
Les chaîne de Markov régulières sont celles qui vont converger vers un équilibre, comme le montre le théorème suivant.

\begin{theorem}[\cite{GMbugcT}]
    Soit \( P\) la matrice de transition d'une chaîne de Markov régulière sur un ensemble \( E\) de cardinal \( N\). Alors il existe des nombres \( \pi_1,\ldots, \pi_N>0\) tels que
    \begin{enumerate}
        \item
            \( \pi_1+\ldots +\pi_N=1\)
        \item
            ...
            \end{equation}
    \end{enumerate}
    De plus le vecteur \( \pi=(\pi_1,\ldots, \pi_N)\) est solution de 
    \begin{equation}
        \pi P=\pi.
    \end{equation}
\end{theorem}
