...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{À propos de différentielles dans un Banach}
%---------------------------------------------------------------------------------------------------------------------------

\begin{definition}  \label{DefKZXtcIT}
    Soit une application \( f\colon E\to F\) entre deux espaces de Banach. Nous disons que \( f\) est \defe{différentiable}{différentiable!dans un Banach} en \( a\in E\) si il existe une application linéaire \( T\colon E\to F\) telle que
    \begin{equation}
...
\begin{theorem}
    ...
    \begin{equation}
        \Dsdd{ f(x+tu) }{t}{0}
    \end{equation}
    existe pour tout \( x\in B(a,r)\) et est continue (par rapport à \( x\)) en \( x=a\).

    Alors \( f\) est différentiable en \( a\) et
    \begin{equation}
...
\begin{proof}
    Dans un premier temps nous supposons que pour tout \( u\in E\) nous avons \( \frac{ \partial f }{ \partial u }(a)=0\).
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    Pour la dérivée par rapport à \( s\) nous avons effectué le changement de variables \( s\to ts\), ce qui explique la présence d'un \( t\) en facteur. La fonction \( g\) est donc différentiable en \( a\) et la fonction \( f\) suit.
\end{proof}
...
\begin{proposition}[Inverse dans \( \GL(E)\)\cite{laudenbach2000calcul}]
    Soit \( E\) un espace normé et complet.
    \begin{enumerate}
        \item
