...
\begin{proof}
    ...
    En posant
    \begin{equation}
        g_{\epsilon}(\xi)= e^{-\epsilon| \xi |^2} 
    \end{equation}
    nous avons \( g_{\epsilon}\in \swS(\eR^d)\) et l'intégrale entre crochet n'est autre que \( \hat g_{\epsilon}(y-x)\). Nous savons que si \( g(x)= e^{e-| x |^2/2}\) alors \( g_{\epsilon}(x)=g\big( \sqrt{ 2\epsilon }x \big)\), et donc en utilisant les lemmes \ref{LEMooKGDKooVXSMCn} et \ref{LEMooPAAJooCsoyAJ} et le la proposition \ref{PropKPsjyzT} nous savons à la fois que \( \hat g_{\epsilon}\) est une fonction paire et une fonction dans \( \swS(\eR^d)\) (un facteur d'échelle par rapport à la vraie gaussienne). Nous pouvons donc remplacer \( \hat g_{\epsilon}(y-x)\) par \( \hat g_{\epsilon}(x-y)\) :
    \begin{equation}
    ...
    Nous avons donc
    \begin{equation}
        \clubsuit=\int_{\eR^d}\overline{ f(x) }(f* g_{\epsilon})(x)dx.
    \end{equation}
    <++>
\end{proof}
<++>
