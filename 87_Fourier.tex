...
\begin{proof}
    ...
    \item[Suite régularisante]
        ...
        \end{equation}
        Donc \( \frac{1}{ (2\pi)^d }\hat g_{\epsilon}\) est une suite régularisante.
    \item[Première conclusion]
        Nous reprenons \eqref{EQooWIHNooHutHlS}
        \begin{equation}
            \int_{\eR^d}| \hat f |^2 e^{-\epsilon\xi^2}d\xi=\langle f, f*\hat g_{\epsilon}\rangle_{L^2(\eR^d)}=(2\pi)^d\langle f, f*\frac{1}{ (2\pi)^d\hat g_{\epsilon} }\rangle .
        \end{equation}
        En prenant la limite \( \epsilon\to 0\) (qui commute avec le produit scalaire\footnote{Parce que en fait on pose \( \epsilon=1/n\) et la limite que l'on prend est une limite \( n\to \infty\).}) nous trouvons
        \begin{equation}
            \lim_{\epsilon\to 0}\int_{\eR^d}| \hat f |^2 e^{-\epsilon\xi^2}d\xi=(2\pi)^d\| f \|^2.
        \end{equation}
        Pour effectuer la limite du membre de gauche nous devons remarquer qu'en posant
        \begin{equation}
            g_n(\xi)=| \hat f(\xi) | e^{-\epsilon| \xi |^2},
        \end{equation}
        nous avons une suite décroissante de fonction (c'est à dire que à \( \xi\) fixé, c'est décroissant en \( \epsilon\to 0\)). Par ailleurs ces fonctions sont toujours à valeurs dans \( \mathopen[ 0 , \infty \mathclose]\) et nous pouvons utiliser le théorème de la convergence monotone \ref{ThoRRDooFUvEAN} pour permuter la limite et l'intégrale. Au final :
        \begin{equation}
