...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Mesurabilité des fonctions à valeurs rélles}
%---------------------------------------------------------------------------------------------------------------------------
...
Il est naturel de se demander comment il faut intégrer par rapport à une mesure image. La réponse est dans le théorème suivant.
\begin{theorem}[\cite{ooGMNAooSLnIio}]
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \begin{equation}        \label{EqooSOHXooXSbdoy}
                \int_{S_2}ff\big( \varphi(\mu) \big)=\int_{S_1}(f\circ\varphi)d\mu.
            \end{equation}
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    \begin{subproof}
        ...
        \item[\( f\) est étagée positive]
            ...
            \end{equation}
            avec \( B_k\in\tribF_2\) et \( a_k\in \eR^+\). Nous avons alors, en utilisant la sous-additivité de l'intégrale \ref{}
            \begin{equation}
...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Produit d'une mesure par une fonction}
%---------------------------------------------------------------------------------------------------------------------------

\begin{propositionDef}[\cite{MonCerveau}]\label{PropooVXPMooGSkyBo}
    Soit un espace mesuré \( (S,\tribF,\mu)\) et une fonction mesurable positive \( f\colon S\to \eR\). Alors la formule
    \begin{equation}
        \nu(A)=\int_Afd\mu
    \end{equation}
    pour tout \( A\in \tribF\) définit une mesure positive sir \( (S,\tribF)\) appelée \defe{produit}{produit!d'une mesure par une fonction} de la mesure \( \mu\) par la fonction \( f\).
\end{propositionDef}

\begin{proof}
    D'abord \( \nu(\emptyset)=0\). Ensuite si les ensembles \( A_i\) sont des éléments deux à deux disjoints de \( \tribF\) alors
    \begin{equation}
        \nu(\bigcup_iA_i)=\int_{\bigcup_iA_i}fd\mu=\sum_i\int_{A_i}f=\sum_i\nu(A_i).
    \end{equation}
\end{proof}
