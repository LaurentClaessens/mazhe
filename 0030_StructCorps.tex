...
\begin{proposition}[Propriétés d'extensions algébriques\cite{MonCerveau}]   \label{PropURZooVtwNXE}
    Soit \( \eK\) un corps commutatif\footnote{Juste en passant nous rappelons que tous les corps considérés ici sont commutatifs} et \( a\) un élément algébrique sur \( \eK\), de polynôme minimal \( P\) de degré \( n\). Alors
    \begin{enumerate}
        ...
        \item\label{ItemJCMooDgEHajiii}
            ...
            \end{equation}
         \item
            L'anneau \( \eK[a]\) est l'ensemble des polynômes de degré \( n-1\) à coefficient dans \( \eK\).
        \item\label{ItemJCMooDgEHaji}
            \( \eK(a)=\eK[a]\).
        \item   \label{ItemJCMooDgEHajii}
            \( \eK[a]\simeq\eK[X]/(P)\) (isomorphisme d'anneau).
    \end{enumerate}
\end{proposition}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            Rappelons qu'en vertu de la définition \ref{DefCVMooFGSAgL}, le polynôme minimal \( P\) est unitaire; donc le polynôme \( P(X)-X^n\) est un polynôme de degré \( n-1\). Par conséquent en posant \( S(X)=X^n-P(X)\), le polynôme \( S\) est de degré \( n-1\) et vérifie \( a^n=S(a)\).  
    ...
    \item
        Conséquence immédiate de \ref{ItemJCMooDgEHajiv}.
    \item
        Il s'agit de voir que l'ensemble \( \eK[a]\) contient un élément inverse de \( a\). Pour cela nous remarquons que les polynômes \( P(X)\) et \( X\) sont premiers entre eux, sinon \( P\) ne serait pas un polynôme minimal (ou alors c'est que \( a=0\)). Donc le théorème de Bézout \ref{ThoBezoutOuGmLB} affirme l'existence d'éléments \( U,V\in \eK[X]\) tels que
        \begin{equation}
            UP+VX=1
        \end{equation}
        dans \( \eK[X]\). Nous évaluons cette égalité en \( a\) en tenant compte de \( P(a)=0\) dans \( \eK[a]\) :
        \begin{equation}
            U(a)P(a)+V(a)a=1
        \end{equation}
        dans \( \eK[a]\). Par conséquent \( V(a)a=1\), ce qui signifie que \( V(a)\) est l'inverse de \( a\).
        \item

            Nous considérons l'application
            \begin{equation}
                \begin{aligned}
                    \psi\colon \eK[X]/(P)&\to \eK[a] \\
                    \bar R&\mapsto R(a) 
                \end{aligned}
            \end{equation}
            et nous montrons qu'elle convient. Pour cela, nous nous souvenons que le théorème \ref{ThoCCHkoU} et son corollaire \ref{CorJKIooBqbqax} nous enseignent que \( (P)\), l'idéal engendré par \( P\), est égal à l'idéal des polynômes annulateurs de \( a\) dans \( \eK[X]\). Le polynôme \( P\) divise tous les éléments de cet idéal; voir aussi la définition \ref{DefSKTooOTauAR} de l'idéal \( (P)\). Cela étant mis au point, nous passons à la preuve.
            \begin{subproof}
            \item[\( \psi\) est bien définie]
                
                Si \( \bar R=\bar S\) alors \( R=S+Q\) avec \( Q\in(P)\), et par conséquent \( R(a)=S(a)+Q(a)\) avec \( Q(a)=0\).
            ...
            \item[Injective]
                ...
                Si \( \psi(\bar R)=0\) alors \( R(a)=0\), mais comme mentionné plus haut, \( P\) engendre l'idéal est polynômes annulateurs de \( a\). Donc \( R\in (P)\) et nous avons \( \bar R=0\) dans \( \eK[X]/(P)\).
