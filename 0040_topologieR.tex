...
La définition \ref{DefHTGYFpT} donne la topologie sur un espace métrique.

\begin{proposition}     \label{PropBVXYooJvKSXE}
    La suite \( (x_k)\) dans un espace métrique \( (X,d)\) converge vers \( x\) si et seulement si pour tout \( \epsilon>0\), il existe \( K_{\epsilon}>0\) tel que \( x_k\in B(x,\epsilon)\) pour tout \( k>K_{\epsilon}\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{subproof}
        \item[Sens direct]
            ...
            Nous supposons que \( x_k\to x\) dans \( X\). Soit \( \epsilon>0\); vu que \( B(x,\epsilon)\) est un ouvert contenant \( x\), il existe un \( K_{\epsilon}>0 \) tel que \( k>K_{\epsilon}\) implique \( x_k\in B(x,\epsilon)\). Cela signifie \( d(x,x_k)\leq \epsilon\).

        \item[Réciproque]

            Nous supposons que pour tout \( \epsilon>0\), il existe \( K_{\epsilon}>0\) tel que si \( k>K_{\epsilon}\) alors \( x_k\in B(x,\epsilon)\). Soit un ouvert \( \mO\) autour de \( x\). Nous sommes dans un espace métrique; ergo la topologie est donné par le théorème \ref{ThoORdLYUu} et en particulier la liste des ouverts est donnée par \eqref{EqGDVVooDZfwSf}. Il existe donc une boule \( B(x,\epsilon)\) inclue à \( \mO\). Pour tout \( k>K_{\epsilon}\) nous avons alors \( x_k\in B(x,\epsilon)\subset\mO\). 
...
\begin{proof}
    Soit un espace métrique \( (X,d)\) et \( x_n\to\ell\) une suite convergente. Si \( \epsilon>0\), la proposition \ref{PropBVXYooJvKSXE}, dit qu'il existe \( N\) tel que pour tout \( n>N\) nous ayons \( d(x_n,\ell)<\epsilon\). Par conséquent si \( n,m>N\) alors
    \begin{equation}
...
\begin{proof}
    ...
    Si réciproquement \( d(x,F)=0\), cela signifie que pour tout \( \epsilon\), il existe \( x_{\epsilon}\in F\) tel que \( d(x_{\epsilon},x)\leq \psi\). En prenant \(\epsilon=1/k\) nous construisons une suite \( (x_k)\) d'éléments dans \( F\) vérifiant \( d(x_k,x)=\frac{1}{ k }\). Cela signifie que \( \lim_{k\to \infty} x_k=x\) par la proposition \ref{PropBVXYooJvKSXE}.
...
\begin{theorem}[\cite{MonCerveau}]\label{PropIYmxXuu}
