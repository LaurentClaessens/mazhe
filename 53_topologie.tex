...
\begin{definition}  \label{DefMVNVFsX}
    ...
    Un couple $(E,d)$ formé d'un ensemble et d'une métrique est un \defe{espace métrique}{espace!métrique}.
\end{definition}
...
\begin{lemma}
    L'application \( (x,y)\mapsto | x-y |\) de la définition \ref{DefKCGBooLRNdJf}\ref{ItemooWUGSooRSRvYC} est une distance sur $\eR$.
\end{lemma}

\begin{proof}
    Les conditions de la définition \ref{DefMVNVFsX} sont à vérifier.
    \begin{enumerate}
        \item
...
D'abord nous munissons \( \eR\) et \( \eQ\) de la topologie métrique donnée par la distance
\begin{equation}
    d(x,y)=| x-y |
\end{equation}
où la valeur absolue est celle donnée par la définition 
