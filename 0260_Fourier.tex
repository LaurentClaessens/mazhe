...
\begin{lemma}
    Si \( f\in L^2(\mathopen[ 0 , 2\pi \mathclose])\), il existe une suite \( (P_n)\) de polynômes trigonométriques tels que 
    \begin{equation}
        \| P_n-f \|_{L^2}\to 0.
    \end{equation}
\end{lemma}
Notons la différence avec le lemme \ref{LemXGYaRlC} qui demandait la périodicité (i.e. \( f(0)=f(2\pi)\) pour obtenir la convergence uniforme.

\begin{proof}
    Soit \( f_n\) la fonction définie comme suit :
    \begin{equation}
        f_n(x)=\begin{cases}
            f(x)    &   \text{si \( x\in \mathopen[ 0 , 2\pi-\frac{1}{ n } \mathclose]\)}\\
            n\big( f(2\pi)-f(2\pi-\frac{1}{ n }) \big)x    &    \text{<++>}<++>
        \end{cases}<++>
    \end{equation}
    <++>
    
    sur \( \mathopen[ 0 , 2\pi-\frac{1}{ n } \mathclose]\) nous demandons \( f_n(x)=f(x)\) puis entre
\end{proof}
<++>
...
Nous pouvons être plus précis. Le système trigonométrique \( \{ e_n \}_{n\in \eZ}\) est total pour l'espace de Hilbert \( L^2\big( \mathopen[ 0 , 2\pi \mathclose] \big)\) (sans périodicité particulière). Donc le point \ref{ItemQGwoIxi} du théorème \ref{ThoyAjoqP} nous donne l'égalité
\begin{equation}    \label{EqXMMRpSN}
    f=\sum_{n\in \eZ}c_n(f)e_n
\end{equation}
dans \( L^2\), c'est à dire l'égalité presque partout. La somme sur \( \eZ\) est commutative, donc pas besoin d'être plus précis.

Une autre façon d'exprimer cela est que pour un élément donné \( f\in L^2\big( \mathopen[ 0 , 2\pi \mathclose] \big)\), nous définissons\nomenclature[Y]{\( S_nf\)}{somme partielle de série de Fourier} 
\begin{equation}
...
\begin{definition}
    Une \defe{courbe de Jordan}{courbe!de Jordan}\index{Jorand!courbe} dans le plan est une application \( \gamma\colon S^1\to \eR^2\) qui est continue et injective.
\end{definition}
