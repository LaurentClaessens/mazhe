...
\begin{theorem}[Meillere approximation]
...
\begin{theorem}[Décomposition dans une base orthogonale]     \label{ThoyAjoqP}
    ...
    \begin{enumerate}
        \item
            ...
            \end{equation}
            où la somme est prise au sens de la définition \ref{}. En particulier, la somme converge de façon commutative.
        \item
        ...
        \item
            La suite \( n\mapsto| \langle x, e_n\rangle  |\) appartient à \( \ell^2\).
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
        \item
            Nous vérifions maintenant l'identité de Plancherel. Étant donné que \( \{ u_i \}\) est une base hilbertienne nous avons
            \begin{equation}
                \lim_{N\to \infty} \sum_{k=0}^Nc_k(x)u_k=x.
            \end{equation}
            Étant donné que la norme est une fonction continue nous pouvons la permuter avec la limite et écrire
            \begin{equation}
                \lim_{N\to \infty} \| \sum_{k=0}^Nc_k(x)u_k \|^2=\| x \|^2.
            \end{equation}
            Pour chaque \( N\) nous utilisons le fait que les \( u_k\) forment un système orthonormé :
            \begin{equation}
                \| \sum_{k=0}^Nc_k(x)u_k \|^2=\sum_{k=0}^N\| c_k(x)u_k \|^2=\sum_{k=0}^N| c_k(x) |^2.
            \end{equation}
            Au final nous avons bien
            \begin{equation}
                \lim_{N\to \infty} \sum_{k=0}^N| c_k(x) |^2=\| x \|^2.
            \end{equation}
            <++>
    \end{enumerate}
    <++>
