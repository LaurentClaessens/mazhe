...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item
            Une base hilbertienne de \( A^2(\Omega)\) est donnée par les fonctions
            \begin{equation}
...
\begin{proof}
    ...
    Mais \( L^2(\Omega)\) étant complet, la suite \( (f_n)\) a une limite \( g\in L^2(\Omega)\). Ce que nous voudrions faire est prouver que \( f=g\). Notons que tel quel, ce n'est pas vrai parce que \( f\) est une vraie fonction alors que \( g\) est une classe. Nous devons donc bien comprendre ce qu'est la convergence dans \( L^2\). Nous disons que \( g_n\to g\) lorsque \( [g_n]\to[g]\) où le crochet est la classe.
\end{proof}
<++>
