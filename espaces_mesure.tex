...
\begin{proof}
    ...
    Supposons que les événements \( (A_i)\) sont indépendants. Nous devons vérifier que les tribus le soient, c'est à dire que les événements \( A_i\) et \( \complement A_j\) sont indépendants. Nous avons
    \begin{subequations}
        \begin{align}
            P(A_i\cap \complement A_j)&=P\big(A_i\setminus (A_i\cap A_j)\big)\\
            &=P(A_i)-P(A_i\cap A_j)\\
            &=P(A_i)-P(A_i)P(A_j)\\
            &=P(A_i)\big( 1-P(A_j) \big)\\
            &=P(A_i)P(\complement A_j).
        \end{align}
    \end{subequations}
\end{proof}
