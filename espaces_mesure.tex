...
\begin{lemma}[Slutsky]\index{lemme!de Slutsky}
    Soit \( X_n\) une suite de variables aléatoires réelles qui converge en loi vers la variable aléatoire réelle \( X\) et \( Y_n\), une suite de variables aléatoires réelle qui converge en loi vers la constante \( a\in\eR\). Alors la suite \( (X_n,Y_n)\) converge en loi vers \( (X,Y)\).
\end{lemma}

\begin{proof}
    Cette preuve vient de \cite{MPSmaitrise}.
\end{proof}
<++>
...
\begin{example}
    Soit une suite \( A_n\) de variables aléatoires et \( X\), une autre variable aléatoire. 
\end{example}
<++>
