...
\begin{theorem}[Critère de Cauchy]  \label{ThoCauchyZelUF}
    Une suite de fonctions  \( (f_n)_{n\in\eN}\) sur \( \Omega\) converge uniformément sur \( \Omega\) si et seulement si \( \forall\epsilon>0\), \( \exists N\) tel que
    \begin{equation}
        \| f_n-f_m \|<\epsilon
    \end{equation}
    pour \( n,m>N\).
...
\begin{theorem}[Weierstrass]
    La série de fonctions \( \sum_{n=1}^{\infty}f_n\) converge si \( \| f_n \|\leq M_n\) avec \( \sum_nM_n<\infty\).
\end{theorem}
...
\begin{lemma}
    La formule \eqref{DefProdScalLubrgTj} définit un produit scalaire sur \( \mL^2\).
\end{lemma}
<++>
