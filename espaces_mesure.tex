...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item
            La forme \( \omega\) est de volume, par conséquent de degré maximum et \( d\omega=0\) (\( \omega\) est fermée).
    \end{itemize}
\end{proof}
...
\begin{theorem}[Cauchy-Lipschitz]\index{théorème!Cauchy-Lipschitz}\label{ThokUUlgU}
...
\begin{remark}
    L'équation \( y'=f(t,y)\) est un abus de notation pour demander que pour chaque \( t\) nous demandons \( y'(t)=f(t,y(t))\).
\end{remark}
...
\begin{proof}
    ...
    Prouvons donc que \( \Phi^p\) est contractante pour un certain \( p\).
\end{proof}
<++>
