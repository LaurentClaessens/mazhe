...
\begin{proposition}[Annulation de fonctions polynômiales\cite{WARooZoFOBn}]
...
\begin{proof}
    Nous notons 
    \begin{equation}
        P(z_1,\ldots, z_n)=\sum_{i_1,\ldots, i_n}a_{i_1,\ldots, i_n}z_1^{i_1}\ldots z_n^{i_n},
    \end{equation}
    et nous allons montrer par récurrence sur \( n\) que tous les coefficients \( a_{i_1,\ldots, i_n}\) sont nuls. Le cas \( n=0\) est évident; nous commençons par \( n=1\).

    Nous avons \( P(z)=az+b\). Par hypothèse d'annulation \( 0=P(0)=b\), donc \( P(z)=az\). Ensuite \( 0=P(1)=a\), donc \( a=0\).

    Nous passons maintenant au cas \( n+1\). Pour chaque \( j=1,\ldots, n+1\) nous considérons le polynôme \( \tilde P_j\in \eZ[X_1,\ldots, X_n]\) par 
    \begin{equation}
        \tilde P_j(z_1,\ldots, z_n)=P(z_1,\ldots, 0,\ldots, z_n)
    \end{equation}
    où le \( 0\) est à la \( j\)\ieme position. Par hypothèse de récurrence, tous les coefficients de \( \tilde P_j\) sont nuls. 
    
    Qu'est-ce que cela nous apporte ? Soit un coefficient \( a_{i_1,\ldots, i_{n+1}}\) avec \( i_j=0\); alors \( a_{i_1,\ldots, i_{n+1}}\) sera un des coefficients du polynôme \( \tilde P_j\) et est donc nul. Le polynôme \( P\) se réduit alors à
    \begin{equation}
        P(z_1,\ldots, z_{n+1})=a_{i_1,\ldots, n+1}z_1^{i_1}\cdots z_{n+1}^{i_{n+1}}.
    \end{equation}
    <++>
\end{proof}
<++>
