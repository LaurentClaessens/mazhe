...
\begin{proposition} \label{PropTTiRgAq}
    Soit \( X\) un espace normé et \( A\) une partie dense de \( X\) sur lequel nous considérons la topologie induite de \( X\)\footnote{C'est à dire que \( A\) a la topologie métrique donnée par la restriction de la norme de \( X\) à la partie \( A\).}. Soit \( F\) un espace de Banach. Toute application linéaire continue \( f\colon A\to F\) se prolonge de façon unique en une application linéaire continue \( \tilde f\colon X\to F\). De plus \( \| \tilde f \|=\| f \|\).
\end{proposition}
